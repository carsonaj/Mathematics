...
\end{enumerate}	 
	...
	\begin{defn}
	Let $X$ be a LCH space, $(\mu_n)_{n \in \N} \subset M(X)$ and $\mu \in M(X)$. Then $\mu_n$ is said to \textbf{converge in weak-*}, denoted $\mu_n \conv{w^*}\mu$, if $I_{\mu_n} \conv{w^*} I_{\mu}$, i.e. for each $f \in C_0(X)$, $$\int f d\mu_n \rightarrow \int f d\mu$$
	\end{defn}
