...
\section{Coproduct Measures}
	...
	\begin{proof}\
		\begin{enumerate}
			...
			\item Suppose that $f$ is unbounded. Let $\epsilon >0$. Then there exists $\phi \in L^1$ such that $\phi$ is simple and $\int|f-\phi| < \ep/2$. Since $\phi$ is bounded, there exists $\del >0 $ such that for each $E \in \MA$, if $\mu(E) < \del$, then $\int_E |\phi| < \ep/2$. Let $E \in \MA$. Suppose that $\mu(E) < \del$. Then 
			\begin{align*}
				\int_E|f|
...
\end{align*}		
	...
	\begin{itemize}
		...
		\begin{itemize}
			...
			\item $\MM(X) = \{\mu \in M(X): \mu \text{ is Radon}\}$
	...
	\begin{ex} \lex{ex:introduction_radon_measures:0010.1} 
		...
		$$\nu(E) = \int_E f \dmu.$$
		If $\mu$ is $\sig$-finite, then $\nu \in \MM(X)$. \\
		\tbf{Hint:} consider \rex{ex:integration_complex_functions:00015}.
	\end{ex}

	\begin{proof} Suppose that $\mu$ is $\sig$-finite.
	\begin{enumerate}
		\item Let $K \subset X$. Suppose that $K$ is compact. Since $f \in L^1(X, \MB(X), \mu)$, we have that
		\begin{align*}
			|\nu|(K)
			& = \int_K |f| \dmu \\
			& \leq \int_X |f| \dmu \\
			...
			& < \infty 
		\end{align*}
		Since $K \subset X$ with $K$ is compact, we have that for each $K \subset X$, $K$ is compact implies that $|\nu|(K) < \infty$.
		\item Let $E \in \MB(X)$. By construction, $|\nu|(E) \leq \inf \{|\nu|(U'): \text{$E \subset U'$ and $U'$ is open}\}$. Let $\ep > 0$. 
		\begin{itemize}
			\item Suppose that $\mu(E) < \infty$. \rex{ex:integration_complex_functions:00015} implies that there exists $\del > 0$ such that for each $A \in \MB(X)$, $\mu(A)< \del$ implies that 
			\begin{align*}
				|\nu|(A)
				& = \int_A |f| \dmu \\
				& < \ep.
			\end{align*}
			Since $\mu$ is outer regular on Borel sets, there exists $U \subset X$ such that $U$ is open, $E \subset U$ and $\mu(U) < \mu(E) + \del$. Since $U = E \cup (U \setminus E)$, we have that 
			\begin{align*}
				\mu(U)
				& = \mu(E) + \mu(U \setminus U)
			\end{align*} 
			Therefore 
			\begin{align*}
				\mu(U \setminus U)
			...
			\begin{align*}
				...
				& < |\nu|(E) + \ep
			\end{align*} 
			So there exists $U \subset X$ such that $U$ is open, $E \subset U$ and $|\nu|(U) < |\nu|(E) + \ep$.
			\item Suppose that $\mu(E) = \infty$. Since $\mu$ is $\sig$-finite, there exist $(A_n)_{n \in \N} \subset \MB(X)$ such that $(A_n)_{n \in \N}$ is disjoint, $X = \bigcup\limits_{n \in \N} A_n$ and for each $n \in \N$, $\mu(A_n) < \infty$. For each $n \in \N$, define $E_n = E \cap A_n$. Then $(E_n)_{n \in \N}$ is disjoint,
			\begin{align*}
				E 
				& = E \cap X \\
				& = E \cap \bigg( \bigcup\limits_{n \in \N} A_n  \bigg) \\
				& = \bigcup\limits_{n \in \N} (E \cap A_n) \\
			...
			\begin{align*}
				...
				& < \infty 
			\end{align*}
			From above, for each $n \in \N$, there exists $U_n \subset X$ such that $U_n$ is open and $E_n \subset U_n$ and $|\nu|(U_n) < |\nu|(E_n) + \ep2^{-(n+1)}$. Define $U \subset X$ by $U \defeq \bigcup\limits_{n \in \N} U_n$. Then $U$ is open, 
			\begin{align*}
				E
				& = \bigcup_{n \in \N} E_n \\
				& \subset \bigcup_{n \in \N} U_n \\
				& = U
			\end{align*}
			and 
			\begin{align*}
				|\nu|(U)
				& = |\nu| \bigg( \bigcup_{n \in \N} U_n \bigg) \\
				& \leq \sum_{n \in \N} |\nu|(U_n) \\
				& \leq \sum_{n \in \N} [|\nu|(E_n) + \ep 2^{-(n+1)}] \\
				& = \sum_{n \in \N} |\nu|(E_n) + \ep/2 \\
				& = |\nu| \bigg( \bigcup_{n \in \N} E_n \bigg) + \ep/2 \\
				& = |\nu|(E) + \ep/2 \\
				& < |\nu|(E) + \ep
			\end{align*}
			So there exists $U \subset X$ such that $U$ is open, $E \subset U$ and $|\nu|(U) < |\nu|(E) + \ep$.
		\end{itemize}
		Since there exists $U \subset X$ such that $U$ is open, $E \subset U$ and $|\nu|(U) < |\nu|(E) + \ep$, we have that 
		\begin{align*}
			\inf \{|\nu|(U'): \text{$E \subset U'$ and $U'$ is open}\}
			& \leq |\nu|(U) \\
			& < |\nu|(E) + \ep 
		\end{align*} 
		Since $\ep > 0$ is arbitrary, we have that $\inf \{|\nu|(U'): \text{$E \subset U'$ and $U'$ is open}\} \leq |\nu|(E)$. Thus 
		$$|\nu|(E) = \inf \{|\nu|(U'): \text{$E \subset U'$ and $U'$ is open}\}$$
		and $|\nu|$ is outer regular on $E$. Since $E \in \MB(X)$ is arbitrary, we have that $|\nu|$ is outer regular on Borel sets.
		\item 
	\end{enumerate}  
	\end{proof}
	...
	\subsection{Differentiating Radon Measures}
	...
	\subsection{Differentiating Complex Radon Measures}
	
	\begin{defn}
		Let $(X, d)$ be a metric space, $\mu \in \MM_+(X, d)$ and $\nu \in \MM(X)$. 
	\end{defn}

	\tcr{what should it mean for a complex Radon measure to be $d$-Vitali?}
	
	
	\begin{defn} \ld{def:diff_radon_measures:0014}
	...
	\begin{ex} \lex{ex:diff_radon_measures:0015} \tbf{Lebesgue Differentiation Theorem:} \\
		Let $(X, d)$ be a metric space, $\mu \in \MM_+(X, d)$ and $f \in L^1(X, \MB(X), \mu)$. Suppose that $\mu$ is $d$-Vitali. Then for $\mu$-a.e. $x \in X$,
	...
	\begin{proof}
		Define $\nu \in \MM(X)$ by 
		$$\nu(E) = \int_E f \dmu $$
		\rex{ex:introduction_radon_measures:0010.1} implies that $\nu \in \MM(X, d)$. Since $\nu \ll \mu$, we have that $\nu_{\mu}|_{\MB(X)} = \nu$ and therefore for $\mu$-a.e. $x \in X$,
		\begin{align*}
			f(x)
			& = \frac{d \nu}{d \mu}(x) \\
			& = \frac{d \nu_{\mu}}{d \mu}(x) \\
