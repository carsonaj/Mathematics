...
\begin{document}
	...
	\begin{defn} \ld{00000} 
		Let $f \in \Bnd([a,b], \R)$ and $\MP = \{x_0, \cdots, x_n\}$ a partion of $[a,b]$. Suppose that $f$ is bounded. For $i = 1, \cdots, n$, put 
		$$M^f_i = \sup\limits_{x \in [x_{i-1}, x_i]} f(x)$$ and 
		$$m^f_i = \inf_{x \in [x_{i-1}, x_i]} f(x)$$ 
		We define the \textbf{upper Darboux sum} of $f$ with respect to $\MP$, denoted $U_\MP f$, by 
		$$U_\MP f \defeq \sum\limits_{i=1}^n M^f_i (x_i - x_{i-1})$$ 
		and we define the \textbf{lower Darboux sum} of $f$ with respect to $\MP$, denoted $L_\MP f$, by
		$$L_\MP f \defeq \sum\limits_{i=1}^n m^f_i (x_i - x_{i-1})$$ 
	\end{defn}

	\begin{ex} \lex{00000} 
		Let $f \in \Bnd([a,b], \R)$ and $\MP$ a partition of $[a,b]$. Then $$\bigg[\inf_{x \in [a,b]} f(x) \bigg] (b-a) \leq L_\MP f \leq U_\MP f \leq \bigg[ \sup\limits_{x \in [a,b]} f(x)  \bigg] (b-a)$$
	\end{ex}
	...
	\begin{ex} \lex{00000} 
			Let $f \in \Bnd([a,b], \R)$ and $\MP$, $\MP'$ partitions of $[a,b]$. If $\MP \subset \MP'$, then 
			\begin{enumerate}
	...
	\begin{proof} \
		\begin{enumerate}
			\item Assume that $\MP = \{x_0, \cdots, x_n\}$ and $\MP' = \MP \cup \{x'\}$. Then there exists $j \in \{1, \cdots, n\}$ such that $x_{j-1} < x' < x_j$. Define $$M'_1 = \sup\limits_{x \in [x_{j-1}, x']} f(x), \hspace{.4cm} M'_2 = \sup\limits_{x \in [x', x_{j}]} f(x)$$
	...
	\begin{defn} \ld{00000} 
		...
		$$Uf =\defeq \inf \{U_\MP f: \MP \text{ is a partition of } [a,b]\}$$
		and we define the \textbf{lower Darboux integral} of $f$, denoted $L f$, by 
		$$Lf \defeq \sup \{L_\MP f: \MP \text{ is a partition of } [a,b]\}$$ 
	\end{defn}

	\begin{ex} \lex{00000} 
		Let $f \in \Bnd([a,b], \R)$. Then $$ \bigg[\inf_{x \in [a,b]} f(x) \bigg] (b-a) \leq Lf \leq Uf \leq \bigg[\sup\limits_{x \in [a,b]} f(x) \bigg] (b-a)$$
	\end{ex}

	\begin{proof}
		Clearly $$\bigg[\inf_{x \in [a,b]} f(x) \bigg] (b-a) \leq Lf \hspace{.2cm } \text{ and } \hspace{.2cm } Uf \leq \bigg[\sup\limits_{x \in [a,b]} f(x) \bigg] (b-a)$$ 
	...
	\begin{defn} \ld{00000} 
		Let $f \in \Bnd([a,b], \R)$. Then $f$ is said to be \textbf{Darboux integrable} if $Uf = Lf$. If $f$ is Darboux integrable, we define the \textbf{Darboux integral} of $f$, denoted
		$$\int f \hspace{.2cm} \text{or} \hspace{.2cm} \int f(x) dx,$$  
		by
		$$\int f \defeq Uf$$ The set of bounded, Darboux integrable functions is denoted by $D([a,b])$.
	\end{defn}

	\begin{ex} \lex{00000} 
		Let $f \in \Bnd([a,b], \R)$. Then $f \in D([a,b])$ iff for each $\ep >0$, there exists a partition $\MP$ of $[a,b]$ such that $U_\MP f - L_\MP f < \ep$.
	\end{ex}
	...
	\begin{defn} \ld{def:measurable_spaces:sigma_algebras:0001} 
	...
	\begin{ex} \lex{ex:measurable_spaces:sigma_algebras:0002} 
	...
	\begin{ex} \lex{ex:measurable_spaces:sigma_algebras:0003} 
	...
	\begin{defn} \ld{def:measurable_spaces:sigma_algebras:0004} 
		Let $X$ be a set and $\MC \subset \MP(X)$. Set 
		$$\MS \defeq \{\MA \subset \MP(X): \MA \text{ is a }\sig\text{-algebra on }X \text{ and } \MC \subset \ML\}$$ 
		We define the \textbf{$\sig$-algebra generated by $\MC$} on $X$, denoted $\sig_X(\MC)$, by $$\sig_X(\MC) \defeq \bigcap_{\MA \in \MS} \MA $$
	\end{defn}
	...
	\begin{ex} \lex{ex:measurable_spaces:sigma_algebras:0006} 
	...
	\begin{ex} \lex{ex:measurable_spaces:sigma_algebras:0007} 
	...
	\begin{ex} \lex{ex:measurable_spaces:sigma_algebras:0008}
	...
	\begin{ex} \lex{ex:measurable_spaces:sigma_algebras:0009} 
	...
	\begin{defn} \ld{def:measurable_spaces:sigma_algebras:0010}
	...
	\begin{defn} \ld{def:measurable_functions:00002.1} 
		Let $(X, \MA)$ and $(Y, \MB)$ be measurable spaces and $\phi: X \rightarrow Y$. Then $\phi$ is said to be \tbf{$(\MA, \MB)$-bimeasurable} if 
		\begin{enumerate}
	...
	\begin{defn} \ld{def:measurable_functions:00005}
		...
		\begin{enumerate}
			...
			$$f_*\MA \defeq \{B \subset Y: f^{-1}(B) \in \MA\}$$ 
			\item  \textbf{pullback of $\MB$}, denoted $f^*\MB$, by  
			$$f^*\MB \defeq \{f^{-1}(B):  B \in \MB \}$$
		\end{enumerate}
ka	\end{defn}
	...
	\begin{defn} \ld{def:measurable_functions:00010}
		...
		\begin{align*}
			\sig_X(\MF) 
			& \defeq \sig(\{f_{\al}^{-1}(B): B \in \MA_{\al} \text{ and } \al \in A \})
		\end{align*}	 
	\end{defn}
	...
	\begin{defn} \ld{def:measurable_functions:00012}
		...
		\begin{align*}
			\sig_Y(\MF) 
			& \defeq \sig(\{V \subset Y: \text{ for each $\al \in A$, $f_{\al}^{-1}(V) \in \MA_{\al}$}\})
		\end{align*}	 
	\end{defn}
	...
	\begin{ex} \lex{ex:measurable_functions:00015}
		...
		Let $(X, \MA)$ be a measurable space, $(Y, d)$ a metric space, $(f_n)_{n \in \N} \subset \Meas((X, \MA), (Y, \MB(Y)))$ and $f:X \rightarrow Y$. Suppose that $(Y, \MT_d)$ is separable. If $f_n \conv{p.w.} f$, then $f \in \Meas((X, \MA), (Y, \MB(Y)))$.
	\end{ex}

	\begin{proof}
		Suppose that $f_n \conv{p.w.} f$. Since $(Y,d)$ is separable, there exists $(y_l)_{l \in \N} \subset Y$ such that $(y_l)_{l \in \N}$ is dense in $Y$. For each $n,l, j \in \N$, define $A_{n,l,j} \subset X$ by $A_{n,l,j} \defeq (d(\cdot , y_l) \circ f_n)^{-1}([0, 1/j))$.
		
		Let $n, l,j \in \N$, and $x \in X$. 
		\begin{itemize}
			\item Since $d(\cdot , y_l)$ is continuous and $f_n \in \Meas((X, \MA), (Y, \MB(Y)))$, we have that $\cdot , y_l) \circ f_n \in \Meas((X, \MA), (\R, \MB(R)))$ and 
			\begin{align*}
				A_{n,l,j}
				& = d(\cdot , y_l) \circ f_n)^{-1}([0, 1/j) \\
				& \in \MA.
			\end{align*}
			Since $n \in \N$ is arbitrary, we have that for each $n \in \N$, $A_{n, l,j} \in \MA$. 
			\item Since $f_n(x) \rightarrow f(x)$ and $d(\cdot, x_l):Y \rightarrow \R_+$ is continous, $d(f_n(x), y_l) \rightarrow d(f(x), y_l)$. Then 
			\begin{align*}
				x \in f^{-1}(B_d(y_l, 1/j))
				& \iff f(x) \in B_d(y_l, 1/j) \\
				& \iff d(f(x), y_l) < 1/j \\
				& \iff d(\lim\limits_{n \in \N} f_n(x), y_l) < 1/j \\
				& \iff \lim\limits_{n \in \N} d(f_n(x), y_l) < 1/j \quad \text{(by continuity of $d(\cdot, y_l)$)} \\
				& \iff \exists N \in \N \text{ s.t.} \forall n \in \N, n \geq N \implies d(f_n(x), y_l) < 1/j \quad \text{(by \rex{} \tcr{ref ex in topology section})}.   
			\end{align*}
		\end{itemize}
		Thus 
		\begin{align*}
			f^{-1}(B_d(y_l, 1/j))
			& = \bigcup\limits_{N \in \N} \bigcap\limits_{n \in \N \setminus [N-1]} A_{n,l,j} \\
			& \in \MA. 
		\end{align*}
		Since $l,j \in \N$ are arbitrary, we have that for each $l,j \in \N$, $f^{-1}(B_d(y_l, 1/j)) \in \MA$. \tcr{An ex in anaysis notes topology section} implies that $\{f^{-1}(B_d(y_l, 1/j)): l,j \in \N\}$, is a basis for $\MT_d$. \tcr{ref ex here} implies that $\MB(Y) = \sig_Y(B_d(y_l, 1/j): l,j \in \N)$. \tcr{ref ex here} then implies that $f \in  \Meas((X, \MA), (Y, \MB(Y)))$. 
	\end{proof}
	
	\begin{ex}
		Let $(X, \MA)$ be a measurable space and $(f_n)_{n \in \N} \subset \Meas((X, \MA), (\RE, \MB(\RE)))$ Then . 
		\begin{enumerate}
			\item $\sup\limits_{n \in \N} f_n \in \Meas((X, \MA), (\RE, \MB(\RE))$ 
			\item $\inf\limits_{n \in \N} f_n \in \Meas((X, \MA), (\RE, \MB(\RE))$ 
		\end{enumerate}
	\end{ex}
	...
	\begin{ex} \lex{ex:measurable_functions:00016.01}
	...
	\begin{note}
		\tcr{discuss the information interpretation where we think of maps $f$ as processing and possibly deleting information in $(X_1, \MA_1)$ and $f^*\MA_2$ represents the information in $\MA_1$ preserved by $f$ and $f$ preserves at least as much information as $g$ iff $g = \phi \circ f$ so that $g$ is basically $f$, but with possibly more data processing and loss. This creates a partial order on the set of measurable maps to $(X_3, \MA_3)$. Relate this to category theory with the corresponding poset and the coslice category of maps. Is there a dual notion? dual theorem?}
	\end{note}
	...
	\begin{proof}
		\tcr{A previous exercise} implies that $f: X_1 \rightarrow f(X_1)$ is $(\MA_1,  \MA_2 \cap f(X_1))$-measurable. \tcr{Now apply the previous exercise.} \tcr{FINISH!!!} 
	\end{proof}
	...
	\begin{defn} \ld{def:topology:subspaces:000.1}
		Let $X$ be a set and $E \subset X$. We define the \tbf{inclusion map from $E$ to $B$}, denoted $\iota_E: E \rightarrow X$, by $\iota_E(x) \defeq x$. 
	\end{defn}
	
	\begin{defn} \ld{def:measurable_spaces:subspaces:0001} 
		Let $(X, \MA)$ be a measurable space and $E \subset X$. We define the \tbf{subspace $\sig$-algebra on $E$}, denoted $\MA \cap E$, by 
		$$\MA \cap E \defeq \iota_E^* \MA.$$
	\end{defn}

	\begin{note}
		\rex{ex:measurable_functions:00006} implies that $\MA \cap E$ is a $\sig$-algebra on $E$. 
	\end{note}
	
	\begin{ex} \lex{ex:topology:subspaces:0002}
	...
	\begin{ex} \lex{ex:measurable_spaces:subspaces:0003} 
		...
		\tbf{Hint:} $\sig_X(\MC) \subset (\iota_E)_* \sig_E(\MC \cap E)$
	\end{ex}
	...
	\begin{proof}
		\rex{ex:measurable_spaces:subspaces:0003} implies that 
		\begin{align*}
			\MB(A, \MT \cap A) 
			& = \sig_A(\MT \cap A) \\
			& = \sig_X(\MT) \cap A \\
	...
	\begin{ex} \lex{ex:measurable_spaces:subspaces:0007.1}
		Let $(X, \MA)$ and $E \in \MA$. Then $\iota_E$ is $(\MA \cap E, \MA)$-bimeasurable.
	\end{ex}
	
	\begin{proof}\
		\begin{itemize}
			\item By \rd{def:measurable_spaces:subspaces:0001}, $\iota_E$ is $(\MA \cap E, \MA)$-measurable.
	...
	\begin{ex} \lex{ex:measurable_spaces:subspaces:0009}
	...
	\begin{ex} \lex{ex:measurable_spaces:subspaces:0010}
	...
	\begin{proof}\
		\begin{itemize}
			\item $(\implies):$ \\
			Suppose that $f$ is $(\MA, \MB)$-measurable. \rex{ex:measurable_spaces:subspaces:0009} implies that for each $j \in \N$, $f|_{A_j}$ is $(\MA \cap A_j, \MB)$-measurable.
			\item $(\impliedby):$ \\
	...
	\begin{ex} \lex{ex:measurable_spaces:subspaces:0011} \tbf{Schroder-Bernstein Theorem:} \\
		\tcr{ref Schroder-Bernstein theorem in set theory notes}
		...
		\begin{enumerate}
			...
			\begin{enumerate}
				...
				\begin{align*}
					...
					& \in \MA. 
				\end{align*}
				\item Since $g$ is injective, \tcb{an exercise in the analysis notes section on set theory subsection on bijections} implies that 
				\begin{align*}
					g(f(A)^c)^c
			...
			\begin{align*}
				...
				& \subset f(X).
			\end{align*}
			Define $f_0: f(X) \rightarrow X$ and $g_0:g(Y) \rightarrow Y$ by $f_0 \defeq (f|^{f(X)})^{-1}$ and $g_0 \defeq (g|^{g(Y)})^{-1}$. Since $B^c = f(A)$, $A = g(f(A)^c)^c$ and $f, g$ are injective, \tcb{an exercise in the analysis notes section on set theory:bijections} implies that 
			\begin{align*}
				f_0(B^c) 
				& = (f|^{f(X)})^{-1}(f(A)) \\
				& = f^{-1}(f(A)) \\
			...
			\end{cases}
			\]
			Then $h \circ \be = \id_Y$ and $\be \circ h = \id_X$. Hence $h$ is a bijection and $\be = h^{-1}$. \rex{ex:measurable_spaces:subspaces:0009} implies that $f|_A$ is $(\MA \cap A, \MB)$-measurable. Let $F \in \MB$. Since $g$ is bimeasurable, $g(F) \in \MA$. Therefore 
			\begin{align*}
				(g_0|_{A^c})^{-1}(F)  
				& = g_0^{-1}(F) \cap A^c \\
				& = [(g|^{g(Y)})^{-1}]^{-1}(F) \cap A^c \\
				...
				& \in \MA \cap A^c. 
			\end{align*}
			Since $F \in \MB$ is arbitrary, we have that for each $F \in \MB$, $(g_0|_{A^c})^{-1}(F) \in \MA \cap A^c$. Hence $g_0|_{A^c}$ is $(\MA \cap A^c, \MB)$-measurable. Since $h|_A = f|_A$ and $h|_{A^c} = g_0|_{A^c}$, \rex{ex:measurable_spaces:subspaces:0010} implies that $h$ is $(\MA, \MB)$-measurable. Similarly, $\be$ is $(\MB, \MA)$-measurable. Thus $h$ is a measurable isomorphism.
		\end{enumerate}
	\end{proof}
	...
	\tcb{\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#}
	\\
	\tcr{TODO:} look at \href{https://math.stackexchange.com/questions/248032/is-product-of-borel-sigma-algebras-the-borel-sigma-algebra-of-the-product-of}{https://math.stackexchange.com/questions/248032/is-product-of-borel-sigma-algebras-the-borel-sigma-algebra-of-the-product-of} show that a the borel sigma algebra of an uncountable product of nontrivial hausdorff spaces is always bigger than the product of their borel sigma algebras. \\
	\tcr{\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#\#}
	
	\begin{defn} \ld{25001}
		Let $(X_{\al}, \MA_{\al})_{\al \in A}$ be a collection of measurable spaces. We define the \textbf{product $\sig$-algebra} on $\prod_{\al \in A}X_{\al}$, denoted by $\bigotimes\limits_{\al \in A} \MA_{\al}$, by $$\bigotimes\limits_{\al \in A} \MA_{\al} \defeq \sig(\pi_{\al}: \al \in A)$$
	\end{defn}
	...
	\begin{proof}
		Immediate by \tcr{a previous exercise about the initial $\sig$-algebra}.
	\end{proof}
	...
	\begin{defn} \ld{25007}
		Let $(X_{\al}, \MA_{\al})_{\al \in A}$ and $(Y_{\al}, \MB_{\al})_{\al \in A}$ be collections of measurable spaces and $(f_{\al})_{\al \in A} \in \prod\limits_{\al \in A} Y_{\al}^{X_{\al}}$, i.e. for each $\al \in A$, $f_{\al}:X_{\al} \rightarrow Y_{\al}$. Set $X = \prod\limits_{\al \in A} X_{\al}$ and $Y = \prod\limits_{\al \in A}Y_{\al}$. We define the \tbf{product of $(f_{\al})_{\al \in A}$}, denoted $\prod_{\al \in A} f_{\al}: X \rightarrow Y$, by $\prod_{\al \in A} f_{\al} ((x_{\al})_{\al \in A}) \defeq (f_{\al}(x_{\al}))_{\al \in A}$.
		\tcr{eventually delete, reference set theory section instead}
	\end{defn}
	...
	\begin{proof} 
		Suppose that for each $\al \in A$, $f_{\al}$ is $(\MA_{\al}, \MB_{\al})$-measurable. Let $\al \in A$. Denote the $\al$-th projection maps on $X$ and $Y$ by $\pi^X_{\al}$ and $\pi^Y_{\al}$ respectively.  \tcb{An exercise in the intro-math/analysis notes chapter on set theory} implies that $\pi^Y_{\al} \circ f = f_{\al} \circ \pi^X_{\al}$. Since $f_{\al} \circ \pi^X_{\al}$ is $(\MA_{\al}, \MB_{\al})$-measurable, we have that $\pi^Y_{\al} \circ f$ is $(\MA_{\al}, \MB_{\al})$-measurable. Since $\al \in A$ is arbitrary, a previous exercise implies that $f$ is $(\bigotimes\limits_{\al \in A} \MA_{\al}, \bigotimes\limits_{\al \in A} \MB_{\al})$-measurable.
	\end{proof}
	
	\begin{ex} \lex{25008.01}
		Let $(X, \MT_X)$ be a measurable space and $A$ a set. Then $\del_{X^A}:X \rightarrow X^A$ is $(\MA, \MA^{\otimes A})$-continuous.
		
		\tcr{make ex about $\del_X$ and $\del_{X^{\N}}$ being measurable similar to continuity in the analysis notes, then ref in the polish section/analytic set section}
		\tcr{FINISH!!!}
	\end{ex}
	
	\begin{proof}
		For each $\al \in A$, $\pi_{\al} \circ \del_{X^A} = \id_A$. Since $\id_A$ is $(\MT_X. \MT_X)$-continuous, we have that for each $\al \in A$, $\pi_{\al} \circ \del_{X^A}$ is $(\MT_X, \MT_X)$-continuous. \rex{ex:product_topology:0007} then implies that $\del_{X^A}$ is $(\MT_X, \MT_X^{\otimes A})$-continuous. 
		\tcr{FIX!!!}
	\end{proof}
	
	\begin{ex} \lex{25008.02}
		Let $(X, \MA_X), (Y, \MA_Y), (Z, \MA_Z)$ be measurable spaces, $f:X \rightarrow Y$ and $g_1, g_2:Y \rightarrow Z$. If $g_1 \circ f$ and $g_2 \circ f$ are $(\MA_X, \MA_Z)$-measurable, then for each $W_1, W_2 \in \MA_Z$, $f^{-1}(g_1^{-1}(W_1) \cap g_2^{-1}(W_2)) \in \MA_X$.
	\end{ex}
	
	\begin{proof}
		Suppose that $g_1 \circ f$ and $g_2 \circ f$ are $(\MT_X, \MT_Z)$-continuous. \rex{} implies that $(g_1 \circ f) \times (g_2 \circ f)$ is $(\MT_X^{\otimes 2}, \MT_Z^{\otimes 2})$-continuous. \rex{} \tcr{(ref ex)} implies that $[(g_1 \circ f) \times (g_2 \circ f)] \circ \del_X$ is $(\MT_X, \MT_Z^{\otimes 2})$-continuous. We note that
		\begin{align*}
			[g_1 \times g_2] \circ \del_Y \circ f
			& = [g_1 \times g_2] \circ [f \times f] \circ \del_X \quad \text{(by \rex{ex:set_theory:products:00010.2})} \\
			& = [(g_1 \circ f) \times (g_2 \circ f)] \circ \del_X.
		\end{align*}
		Therefore for each $W_1, W_2 \in \MT_Z$, 
		\begin{align*}
			f^{-1}(g_1^{-1}(W_1) \cap g_2^{-1}(W_2))
			& = f^{-1} [ \del_Y^{-1}(g_1^{-1}(W_1) \times g_2^{-1}(W_2))] \quad \text{(by \rex{ex:set_theory:products:00010.1})} \\
			& = f^{-1} [ \del_Y^{-1}([g_1 \times g_2]^{-1}(W_1 \times W_2))] \quad \text{(by \rex{ex:set_theory:products:0005.1})} \\
			& = ([g_1 \times g_2] \circ \del_Y \circ f)^{-1}(W_1 \times W_2) \\
			& = ([(g_1 \circ f) \times (g_2 \circ f)] \circ \del_X)^{-1}(W_1 \times W_2) \\
			& \in \MT_X
		\end{align*}
		\tcr{FIX!!!}
	\end{proof}
	
	\begin{ex} \lex{25008.03}
		Let $(X, \MA) \in \Obj(\bMeas)$, $(Y, d)$ \tcr{a metric space} and $(f_n)_{n \in \N} \subset \Hom_{\Meas}((X, \MA), (Y, \MB(\MT)))$. If $(Y,d)$ is complete, then $\{x \in X: \text{$(f_n(x))_{n \in \N}$ converges} \} \in \MA$.  
	\end{ex}
	
	\begin{proof}
		Suppose that $(Y,d)$ is complete. 
		\begin{itemize}
			\item Since $d$ is $(\MT \otimes \MT, \MT_{\Rg})$-continuous $d$ is 
			\begin{align*}
				d
				& \in \Hom_{\Meas}((X \times X, \MB(\MT \otimes \MT)), (\Rg, \MB(\MT_{\Rg}))) \\
				& = \Hom_{\Meas}((X \times X, \MB(\MT) \otimes \MB(\MT)), (\Rg, \MB(\MT_{\Rg}))) \quad \text{(by \rex{25005})}.
			\end{align*} 
			Let $M, m,n \in \N$. Since $f_m, f_n \in \Hom_{\Meas}((X, \MA), (Y, \MB(\MT)))$, \rex{25008} implies that $f_m \times f_n \in \Hom_{\Meas}((X \times X, \MA \otimes \MA), (Y \times Y, \MB(\MT) \otimes \MB(\MT)))$. \rex{25008.01} implies that $\del_X \in \Hom_{\Meas}((X, \MA), (X \times X, \MA \otimes \MA))$ Thus $d \circ [f_m \times f_n] \circ \del_X \in \Hom_{\Meas}((X, \MA), (\Rg, \MB(\MT_{\Rg})))$. Since  
			\begin{align*}
				[0, 1/M) 
				& \in \MT_{\Rg} \\
				& \subset \MB(\MT_{\Rg}),
			\end{align*}
			we have that $(d \circ [f_m \times f_n] \circ \del_X)^{-1}([0, 1/M)) \in \MA$. Since $M,m,n \in \N$ are arbitrary, we have that for each $M,m,n \in \N$, $(d \circ [f_m \times f_n] \circ \del_X)^{-1}([0, 1/M)) \in \MA$.
			\item Define $E \subset X$ by $E \defeq \{x \in X: \text{$(f_n(x))_{n \in \N}$ converges} \}$. Since $(Y, d)$ is complete, for each $x \in X$,
			\begin{align*}
				x \in E
				& \iff x \in \{x' \in X: \text{$(f_n(x))_{n \in \N}$ is Cauchy} \} \\
				& \iff \text{$\forall M \in \N$, $\exists N \in \N$ such that $\forall (m,n) \in \N^2$, $m \geq N \wedge n \geq N \rightarrow d(f_m(x'), f_n(x')) < 1/M)$ } \\
				& \iff x \in \bigcap\limits_{M \in \N} \bigcup\limits_{N \in \N} \bigcap\limits_{(m,n) \in \N^2 \setminus [N-1]^2} (d \circ [f_m \times f_n] \circ \del_X)^{-1}([0, 1/M)). 
			\end{align*}
			Thus 
			\begin{align*}
				E
				& = \bigcap\limits_{M \in \N} \bigcup\limits_{N \in \N} \bigcap\limits_{(m,n) \in \N^2 \setminus [N-1]^2} (d \circ [f_m \times f_n] \circ \del_X)^{-1}([0, 1/M)) \\
				& \in \MA.
			\end{align*}
		\end{itemize}
	\end{proof}
	...
	\begin{defn} \ld{25010}
		Let $X, Y$ be sets, $x \in X$ and $y \in Y$. We define the \textbf{slice maps at $x$ and $y$}, denoted $\iota^y_X: X \rightarrow X \times Y$ and $\iota^x_Y: Y \rightarrow X \times Y$ respectively, by $\iota^y_X(\cdot) \defeq (\cdot, y)$ and $\iota^x_Y(\cdot) = (x, \cdot)$ respectively. 
	\end{defn}
	...
	\begin{defn} \ld{25012} 
		...
		\begin{itemize}
			\item we define the \textbf{sections of $E$ at $x$ and $y$}, denoted $E_x$ and $E^y$ respectively, by $E_x \defeq \{y \in Y: (x,y) \in E\}$ and $E^y = \{x \in X: (x,y) \in E\}$ respectively
			\item we define the \textbf{sections of $f$ at $x$ and $y$}, denoted $f_x:Y \rightarrow Z$ and $f^y:X \rightarrow Z$ respectively, by $f_x(\cdot) \defeq f(x,\cdot)$ and$f^y(\cdot) = f(\cdot,y)$  respectively 
		\end{itemize}
	\end{defn}
	...
	\begin{defn} \ld{def:measurable_spaces:coproducts:0001}
		...
		$$\bigoplus\limits_{\al \in A} \MA_{\al} \defeq \sig (\iota_{\al}: \al \in A)$$
	\end{defn}
	...
	\begin{ex} \lex{ex:measurable_spaces:coproducts:0003}
		...
		$$\bigoplus\limits_{\al \in A} \MA_{\al} = \bigg \{\coprod_{\al \in A} B_{\al}: (B_{\al})_{\al \in A} \in \prod\limits_{\al \in A} \MA_{\al} \bigg\}. $$
	\end{ex}

	\begin{proof}
		Set  
		\begin{itemize}
			\item $\MF= \bigg\{ V \subset \coprod_{\al \in A}  X_{\al}: \text{ for each $\al \in A$, $\iota_{\al}^{-1}(V) \in \MA_{\al}$} \bigg \}$ 
			\item $\MG = \bigg \{  \coprod\limits_{\al \in A}  B_{\al}: (B_{\al})_{\al \in A} \in \prod\limits_{\al \in A} \MA_{\al} \bigg\}$
			\end{itemize}
			Let $V \in \MG$. Then for each $\al \in A$, there exists $(B_{\al})_{\al \in A} \in \prod\limits_{\al \in A} \MA_{\al}$ such that $V =  \coprod\limits_{\al \in A}  B_{\al}$. Therefore, for each $\al \in A$, 
			\begin{align*}
				\iota_{\al}^{-1}(V)
			...
			Conversely, let $V \in \MF$. Then for each $\al \in A$, $\iota_{\al}^{-1}(V) \in \MA_{\al}$. Define $(B_{\al}) \in \prod\limits_{\al \in A} \MA_{\al}$ by $B_{\al} \defeq \iota_{\al}^{-1}(V)$. Then \tcr{(ref ex)} implies that
			\begin{align*}
				V 
	...
	\begin{proof} 
		...
		\begin{itemize}
			...
			For each $n \in \N$, define $\iota_{E_n}: E_n \rightarrow \bigcup\limits_{n \in \N} E_n$ by $\iota_{E_n}(x) \defeq x$. 
			\begin{itemize}
...
\begin{defn} \ld{def:measurable_spaces:proj_limits:0001}
	Let $(J, \leq)$ be a directed set, $(X_j, \MA_j)_{j \in J}$ a collection of measurable spaces and for each $(j,k) \in \leq$, $\pi_{j,k}:X_k \rightarrow X_j$ a $(\MA_k, \MA_j)$-measurable map. Suppose that $((X_j, \MA_j)_{j \in J}, (\pi_{j,k})_{(j,k) \in \leq})$ is a $\bMeas$-projective system. We define the \tbf{projective $\sig$-algebra} of $(\MA_j)_{j \in J}$ on $\varprojlim\limits_{j \in J} X_j$, denoted $\varprojlim\limits_{j \in J} \MA_j$, by $$\varprojlim\limits_{j \in J} \MA_j \defeq \sig ( \pi_j: j \in J).$$
\end{defn}

\begin{ex} \lex{ex:measurable_spaces:proj_limits:0002} 
	Let $(J, \leq)$ be a directed set, $(X_j, \MA_j)_{j \in J}$ a collection of measurable spaces and for each $(j,k) \in \leq$, $\pi_{j,k}:X_k \rightarrow X_j$ a $(\MA_k, \MA_j)$-measurable map. Suppose that $((X_j, \MA_j)_{j \in J}, (\pi_{j,k})_{(j,k) \in \leq})$ is a $\bMeas$-projective system. Then 
...
\begin{proof}\
	\begin{itemize}
		...
		\begin{align*}
			...
			& \in \ME. 
		\end{align*}
		Since $F \in \MF \cap \varprojlim\limits_{j \in J} X_j$ is arbitrary, we have that $\MF \cap \varprojlim\limits_{j \in J} X_j \subset \ME$. \rex{ex:measurable_spaces:subspaces:0004} implies that
		\begin{align*}
			\bigg [ \bigotimes_{j \in J} \MA_j \bigg] \cap \varprojlim\limits_{j \in J} X_j 
...
\begin{ex} \lex{ex:measurable_spaces:proj_limits:0002.1}
	Let $(J, \leq)$ be a directed set and $((X_j, \MT_j)_{j \in J}, (\pi_{j,k})_{(j,k) \in \leq})$ is a $\bTop$-projective system. Suppose that $J$ is countable and for each $j \in J$, $X_j$ is second countable. Then 
	$$\MB\bigg(\varprojlim\limits_{j \in J} X_j, \varprojlim\limits_{j \in J} \MT_j  \bigg) = \varprojlim\limits_{j \in J} \MB(X_j, \MT_j).$$
\end{ex}

\begin{proof}
	Since $J$ is countable and for each $j \in J$, $X_j$ is second countable, \rex{25005} implies that $\MB\bigg(\prod\limits_{j \in J} X_j, \bigotimes\limits_{j \in J} \MT_j \bigg) = \bigotimes\limits_{j \in J} \MB(X_j, \MT_j)$. \rex{ex:measurable_spaces:subspaces:0007} and \rex{ex:measurable_spaces:proj_limits:0002} then imply that
	\begin{align*}
...
\begin{ex} \lex{ex:measurable_spaces:proj_limits:0003}
	Let $(J, {\leq})$ be a directed set and $((X_j, \MA_j)_{j \in J}, (\pi_{j,k})_{(j,k) \in \leq})$ a $\bMeas$-projective system. Set $X \defeq \varprojlim\limits_{j \in J} X_j$ and $\MA \defeq \varprojlim\limits_{j \in J} \MA_j$. Define $\MA_0 \subset \MA$ by $\MA_0 = \bigcup\limits_{j \in J} \pi_j^*\MA_j$. Then $\MA_0$ is an algebra on $X$. 
\end{ex}
...
\begin{proof}\
	\begin{enumerate}
		...
		\end{align*} 
		Thus $\MA_0 \neq \varnothing$.
		\item Let $A \in \MA_0$. Since $\MA_0 = \bigcup\limits_{j \in J} \pi_j^*\MA_j $, there exists $j \in J$ such that $A \in \pi_j^*\MA_j$. By definition of $\pi_j^* \MA_j$ (\rd{def:measurable_functions:00005}), there exists $A_j \in \MA_j$ such that $A = \pi_j^{-1}(A_j)$. Since $\MA_j$ is a $\sig$-algebra on $X_j$, $A_j^c \in \MA_j$. Hence 
		\begin{align*}
			A^c
			& = \pi_j^{-1}(A_j)^c \\
			& = \pi_j^{-1}(A_j^c) \\
...
\end{proof}
	...
	\begin{defn} \ld{def:measurable_spaces:dynkins_lemma:0001}
		Let $X$ be a set and $\MP \subset \MP(X)$. Then $\MP$ is said to be a \textbf{$\pi$-system} on $X$ if for each $A,B \in \MP$, $A \cap B \in \MP$.
		\tcr{maybe just rename in terms of $(\MP, \subset)$ being a meet-semilattice}
	\end{defn}
	
	\begin{defn} \ld{def:measurable_spaces:dynkins_lemma:0002}
	...
	\begin{ex} \lex{ex:measurable_spaces:dynkins_lemma:0003}
	...
	\begin{ex}
		\tcr{Intersection of $\pi,\lam$-systems are again $\pi, \lam$-systems}
	\end{ex}
	...
	\begin{defn} \ld{def:measurable_spaces:dynkins_lemma:0004}
		...
		\tcr{change notation, define $\Pi(X) \defeq \{
			MP \subset \MP(X): \text{$\MP$ is a $\pi$-system on $X$}\}$ and switch from $S$ to $\upar_{\Pi(X)}\MC$ and $\pi(\MC) \defeq \bigcap\limits_{\MP \in \Pi(X)} \MP$, there is also a discussion about pi-systems being meet semilattices, while lambda systems are something similar to a join semilattices with a complement, but where the join is only defined on subsets whose infimum is the bottom element}
	\end{defn}
	
	\begin{ex} \lex{ex:measurable_spaces:dynkins_lemma:0005}
	...
	\begin{thm} \lex{ex:measurable_spaces:dynkins_lemma:0006} \textbf{Dynkin's Lemma:} \\
	...
	\begin{ex} \lex{ex:measurable_spaces:dynkins_lemma:0007}
	...
	\begin{ex}  \lex{ex:measurable_spaces:dynkins_lemma:0007.01}
		Let $X$ be a set and $\ME \subset \MP(X)$. Then 
		\begin{enumerate}
			\item $\pi(\ME) =  \bigg \{ \bigcap\limits_{J \in \MJ}J: \text{$J \subset \ME$ and $\# J < \infty$}  \bigg\}$.
			\item $\pi(\ME)$ is countable iff $\ME$ is countable
		\end{enumerate}
	\end{ex}
	
	\begin{proof}
		Define $\MH \subset \MP(X)$ by 
		$$\MH \defeq \bigg \{ \bigcap\limits_{J \in \MJ}J: \text{$\MJ \subset \ME$ and $\# \MJ < \infty$}  \bigg\}.$$ 
		\begin{enumerate}
			\item 
			\begin{itemize}
				\item Since for each $E \in \ME$, $\{E\} \subset \ME$ and 
				\begin{align*}
					\#\{E\} 
					& =1 \\
					& < \infty,
				\end{align*} 
				we have that for each $E \in \ME$, 
				\begin{align*}
					E
					& = \bigcap\limits_{J \in \{E\}}J \\
					& \in \MF.
				\end{align*}
				Thus $\ME \subset \MH$. 
				\item Let $A,B \in \MH$. Then there exists $\MJ_A, \MJ_B \subset \ME$ such that $\# \MJ_A < \infty$, $\# \MJ_B < \infty$, $A =  \bigcap\limits_{J \in \MJ_A}J$ and $B = \bigcap\limits_{J \in \MJ_B} J$. Then 
				\begin{align*}
					\# (\MJ_A \cup \MJ_B) 
					& \leq \# \MJ_A + \# \MJ_B \\
					& < \infty.  
				...
				\begin{align*}
					A \cap B
					& = \bigg[ \bigcap\limits_{J \in \MJ_A}J \bigg] \cap \bigg[ \bigcap\limits_{J \in \MJ_B} J \bigg] \\
					& = \bigcap\limits_{J \in \MJ_A \cup \MJ_B} J \\
					& \in \MH.
				\end{align*} 
				Since $A,B \in \MH$ are arbitrary, we have that for each $A,B \in \MH$, $A \cap B \in \MH$. Thus $\MH$ is a $\pi$-system on $X$. 
				\item Since $\MH$ is a $\pi$-system and $\ME \subset \MH$, we have that 
				\begin{align*}
					\ME
					& \subset \pi(\ME) \\
					& \subset \pi(\MH) \\
	...
	\begin{defn} \ld{00000} 
		...
		$$\liminf_{n \rightarrow \infty} A_n = \sup\limits_{n \in \N} \bigg( \inf_{k \geq n}A_k \bigg), \hspace{.5cm } \limsup_{n \rightarrow \infty} A_n = \inf_{n \in \N} \bigg(\sup\limits_{k \geq n}A_k \bigg)$$
	\end{defn}
	...
	\begin{proof}\
		\begin{enumerate}
			...
			\end{align*}
			So $$\lim\limits_{n \rightarrow \infty} A_n = \sup\limits_{n \in \N}A_n = \bigcup_{n =1}^{\infty}A_n$$
	...
	\begin{defn} \ld{def:standard_borel_spaces:intro:0002}
		We define the 
		\begin{itemize}
			\item \tbf{Cantor space}, denoted $\MC$, by $\MC \defeq \{0,1\}^{\N}$,
			\item \tbf{Baire space}, denoted $\MN$, by $\MN \defeq \N^{\N}$,
	...
	\begin{note}
		We note that \tcb{exercises from analysis notes} $\MC, \MN \H$ are Polish spaces and $\MC, \MN$ are zero-dimensional spaces.
	\end{note}
	...
	\begin{defn} \ld{def:measurable_spaces:standard_borel_spaces:0001} 
		We recall from \tcb{the analysis notes section on cantor space} the following definitions of $\MC, Z, \phi:Z \rightarrow [0,1]$ and for $n \in \N$, $l \in \{0,1\}$, $Z_n^l$: 
		\begin{itemize}
	...
	\begin{ex} \lex{ex:measurable_spaces:standard_borel_spaces:0002} 
		We have that $\MC$ and $Z$ are zero-dimensional Polish spaces and $\phi$ is a continuous bijection. \\
		\tbf{Hint:} cite \tcb{Analysis notes section on zero-dimensional metric spaces subsection on Cantor space}.
	\end{ex}

	\begin{proof}
		\tcb{Analysis notes section on zero-dimensional metric spaces subsection on Cantor space}
	\end{proof}
	
	\begin{ex} \lex{ex:measurable_spaces:standard_borel_spaces:0006} 
		We have that $Z \in \MB(\MC)$. \\
		\textbf{Hint:} \tcb{Analysis notes section on Cantor space}. 
	\end{ex}
	
	\begin{proof}
		\tcb{An exercise in the analysis notes section on zero-dimensional polish spaces subsection cantor space} implies that $Z$ is a $G_{\del}$-set. Thus $Z \in \MB(\MC)$. 
	\end{proof}
	...
	\begin{proof} \
		...
		\begin{align*}
			\theta_1(z) 
			& = \phi^{-1}(2 \phi(z) - z_1) \\
			& = \phi^{-1}(2 \sum\limits_{j \in \N} z_j2^{-j} - z_1) \\
			& = \phi^{-1}(\sum\limits_{j \in \N} z_j2^{-j+1} - z_1) \\
			& = \phi^{-1}(\sum\limits_{j \in \N} z_{j+1}2^{-j}) \\
			& = (z_{j+1})_{j \in \N}
		\end{align*}
	...
	\begin{ex} \lex{ex:measurable_spaces:standard_borel_spaces:0011} 
		...
		\begin{enumerate}
			...
			\textbf{Hint:} Recall that $\MB(Z) = Z \cap \MB(\MC)$ and $\MB(\{0,1\})^{\otimes \N} = \sig_{\MC} (\pi_j: j \in \N)$
			\item $\phi$ is a $(\MB(Z), \MB([0,1]))$-measurable isomorphism.
		\end{enumerate}
	\end{ex}
	...
	\begin{proof}\
		\begin{enumerate}
			...
			\end{enumerate}
			\item Suppose that for each $n \in \N$, $f_n$ is surjective. Since $f = f_1 \circ \pi_1|_Z$, the previous part implies that 
			\begin{align*}
				f(Z)
				& = f_1 \circ \pi_1|_Z(Z) \\
				& = f_1 \circ \pi_1(Z) \\
			...
			\item Suppose that for each $n \in \N$, $f_n$ is injective. Let $x, y \in Z$. Suppose that $f(x) = f(y)$. Let $n \in \N$. Then the previous part implies that
			\begin{align*}
				f_n(\pi_n(x))
				& = f_1(\pi_1(x)) \\
				& = f(x) \\
				...
				& = f_n(\pi_n(y)).
			\end{align*}
			Since $f_n$ is injective, we have that $\pi_n(x) = \pi_n(x)$. Since $n \in \N$ is arbitrary, we have that for each $n \in \N$, $\pi_n(x) = \pi_n(y)$. Hence $x = y$. Since $x, y \in Z$ are arbitrary, we have that for each $x,y \in Z$, $f(x) = f(y)$ implies that $x =y$. Hence $f$ is injective.
			\item Suppose that for each $n \in \N$, $f_n$ is a measurable isomorphism. The previous parts implies that $f$ is a bijection. Define $g: B \rightarrow Z$ by $g \defeq ( f_n^{-1}|_B)_{n \in \N}$. Then for each $x \in B$, $g(x)_m \defeq f_m^{-1}(x)$. Therefore for each $x \in B$,
			\begin{align*}
				f \circ g (x)
				& = f_1 \circ \pi_1(g(x)) \\
				& = f_m \circ \pi_m(g(x)) \\
			...
			\begin{itemize}
				...
				\item Since for each $n \in \N$, $f_n^{-1}$ is measurable, we have that for each $n \in \N$, $f_n^{-1}|_B$ is measurable. \rex{25006.2} then implies that $f^{-1}$ is measurable. Hence $f$ is a measurable isomorphism.  
			\end{itemize} 
			\item Since $X$ is a Polish space, $X^{\N}$ is a Polish space. Since $X$ is a Polish space, $X$ is Hausdorff. \tcb{An exercise in the analysis notes section on separation and topology} and \tcb{an exercise in the analysis notes section on Polish spaces} imply that $\Del_{X^{\N}}$ is closed in $X^{\N}$ and $\Del_{X^{\N}}$ is a Polish space. Since for each $n \in \N$, $Z_n$ is a Polish space, \tcb{an exercise in the analysis notes section on Polish spaces} implies that $Z_0$ is a Polish space. Since $f_0$ is continuous and $\Del_{X^{\N}}$ is closed in $X^{\N}$, we have that $Z$ is closed in $Z_0$. Since $Z_0$ is a Polish space and $Z$ is closed in $Z_0$, \tcb{an exercise in the analysis notes section on Polish spaces} implies that $Z$ is a Polish space. 
			\item Suppose that for each $n \in \N$, $Z_n$ is zero-dimensional. \tcb{An exercise in the analysis notes topology section on products of zero dimensional spaces} implies that $Z_0$ is zero-dimensional. Since $Z \subset Z_0$, \tcb{An exercise in the analysis notes topology section on subsapces of zero dimensional spaces} implies that $Z$ is zero-dimensional.
		\end{enumerate}
	\end{proof}
	...
	\begin{proof} Define $f_0: \coprod\limits_{n \in \N} Z_n \rightarrow \coprod\limits_{n \in \N}B_n$ by $f_0 \defeq \coprod\limits_{n \in \N} f_n$. 
		\begin{enumerate}
			\item 
			\begin{itemize}
				\item Since for each $n \in \N$, $f_n$ is continuous, \tcb{an exercise in the analysis notes section on coproducts} implies that $f_0$ is continuous. 
				\item Define $\phi: \coprod\limits_{n \in \N} B_n \rightarrow \bigcup\limits_{n \in \N} B_n$ by $\phi(n, x) \defeq x$. \tcb{An exercise in the analysis notes section on coproduct topologies} implies that $\phi$ is a homeomorphism. 
			...
			\item Suppose that for each $n \in \N$, $f_n$ is a measurable isomorphism. Since $f_0^{-1} = \coprod_{n \in \N} f_n^{-1}$, \rex{ex:measurable_spaces:coproducts:0006} implies that $f_0$ is a measurable isomorphism. Since $\phi$ is a homeomorphism, $\phi$ is a measurable isomorphism. Since $\phi$ and $f_0$ are measurable isomorphisms, $f$ is a measurable isomorphism.
			\item \tcb{an exercise in the analysis notes topology section on Polish spaces} implies that $\coprod\limits_{n \in \N} Z_n$ is a Polish space.
			\item Since for each $n \in \N$, $Z_n$ is zero-dimensional, \tcb{an exercise in the analysis notes topology section on zero dimensional spaces} implies that $\coprod\limits_{n \in \N} Z_n$ is zero-dimensional. 
		\end{enumerate}
	\end{proof}
	...
	\begin{ex} \lex{ex:measurable_spaces:standard_borel_spaces:0023}
		...
		\begin{enumerate}
			...
			\tbf{Hint:} \rex{ex:measurable_spaces:standard_borel_spaces:0002}
			...
			\tbf{Hint:} Define 
			$\MD \defeq \{B \in \MB(X, \MT_X): \text{there exists a Polish space $Z$ and $f:Z \rightarrow B$ such that $Z$ is zero-dimensional, } \\ \text{$f$ is continuous and $f$ is a measurable isomorphism}\}$ and consider \rex{ex:measurable_spaces:standard_borel_spaces:0021}, \rex{ex:measurable_spaces:standard_borel_spaces:0022} and \rex{ex:measurable_spaces:standard_borel_spaces:0020}
		\end{enumerate}
	\end{ex}
	
	\begin{proof}\
		\begin{enumerate}
			\item \rex{ex:measurable_spaces:standard_borel_spaces:0002} implies that there exists $E \in \MB(\MC)$ and $\phi: E \rightarrow [0,1]$ such that $E$ is a zero-dimensional Polish space and $\phi$ is a continuous bijection. \rex{ex:measurable_spaces:standard_borel_spaces:0011} implies that $\phi$ is a measurable isomorphism. Since $\phi$ is continuous, \tcb{an exercise in analysis notes section on product topology} implies that $\phi^{\N}$ is continuous and $\phi$ is a measurable isomorphism, \rex{25008} implies that $\phi^{\N}$ is a measurable isomorphism. Define $Z \subset E^{\N}$ by $Z \defeq (\phi^{\N})^{-1}(B)$. Since $\phi^{\N}$ is continuous, \tcb{an exercise in the analysis notes section on subspace topology} implies that $\phi^{\N}|^B_Z$ is continuous. Since $\phi^{\N}$ is a measurable isomorphism, \rex{ex:measurable_spaces:subspaces:0008} and \rex{ex:measurable_spaces:subspaces:0009} imply that $\phi^{\N}|^B_Z$ is a measurable isomorphism. \tcb{An exercise in the analysis notes section on Polish spaces} implies that there exists $B \subset [0,1]^{\N}$ and $h: X \rightarrow B$ such that $B$ is a $G_{\del}$-set and $h$ is a homeomorphism. Thus $h^{-1}$ is continuous and $h^{-1}$ is a measurable isomorphism. Define $f:Z \rightarrow X$ by $f \defeq h^{-1} \circ \phi^{\N}|^B_Z$. Then $f$ is continuous and $f$ is a measurable isomorphism. Since $B$ is a $G_{\del}$-set and $\phi^{\N}$ is continuous, we have that $Z$ is a $G_{\del}$-set. Since $E$ is a Polish space, $E^{\N}$ is a Polish space. Since $Z \subset E^{\N}$ and $Z$ is a $G_{\del}$-set, \tcb{an exercise in the analysis notes section on Polish spaces} implies that $Z$ is Polish space. Since $E$ is zero-dimensional, $E^{\N}$ is zero-dimensional. Since $Z \subset E^{\N}$, we have that $Z$ is zero-dimensional. 
...
%	\begin{ex} \lex{ex:measurable_spaces:standard_borel_spaces:0023} \tcr{DELETE}
...
%			\item Let $(B_n)_{n \in \N} \subset \MD$. Then $(B_n)_{n \in \N} \subset \MB(X)$ and for each $n \in \N$, there exists a Polish space $Z_n$ and $f_n:Z_n \rightarrow B_n$ such that $f_n$ is continuous and $f_n$ is a measurable isomorphism. Define $Z \subset \prod\limits_{n \in \N} Z_n$ by $Z \defeq (\prod\limits_{n \in \N} f_n)^{-1}(\Del_{X^{\N}})$. Then \rex{ex:measurable_spaces:standard_borel_spaces:0021} implies that $Z$ is a Polish space and there exists $f: Z \rightarrow \bigcap\limits_{n \in \N} B_n$ such that $f$ is continuous and $f$ is a measurable isomorphism. Thus $\bigcap\limits_{n \in \N} B_n \in \MD$. Since $(B_n)_{n \in \N} \subset \MD$ is arbitrary, we have that for each $(B_n)_{n \in \N} \subset \MD$, $\bigcap\limits_{n \in \N} B_n \in \MD$. 
...
%	\end{proof}
	...
	\begin{defn} \ld{def:measurable_spaces:standard_borel_spaces:0024}
		Let $X$ be a Polish space and $A \subset X$. Then $A$ is said to be \tbf{analytic} if there exists a Polish space $Z$ and $f: Z \rightarrow A$ such that $f$ is continuous and surjective. 
	\end{defn}
	...
	\begin{proof}
		...
		\begin{enumerate}
			\item \rex{ex:measurable_spaces:standard_borel_spaces:0021} implies that there exists a Polish space $Z$ and $f:Z \rightarrow \bigcap\limits_{n \in \N} A_n$ such that $f$ is continuous and surjective. Thus $\bigcap\limits_{n \in \N} A_n$ is analytic. 
			\item \rex{ex:measurable_spaces:standard_borel_spaces:0022} implies that there exists a Polish space $Z$ and $f:Z \rightarrow \coprod\limits_{n \in \N} A_n$ such that $f$ is continuous and surjective. Define $\phi: \coprod\limits_{n \in \N} A_n \rightarrow \bigcup\limits_{n \in \N} A_n$ by $\phi(n,  x) \defeq x$. \tcb{An excercise in the analysis notes section on coproduct topology} implies that $\phi$ is a homeomorphism. Define $g: Z \rightarrow \bigcup\limits_{n \in \N} A_n$ by $g \defeq \phi \circ f$. Since $\phi$ and $f$ are continuous, $g$ is continuous. Since $\phi$ and $f$ are surjective, $g$ is surjective. Thus $\bigcup\limits_{n \in \N} A_n$ is analytic.
			\item Define $f: \prod\limits_{n \in \N} Z_n \rightarrow \prod\limits_{n \in \N} A_n$ by $f \defeq \prod\limits_{n \in \N}f_n$. Since for each $n \in \N$, $Z_n$ is a Polish space, \tcb{an exercise in the analysis notes section on Polish spaces} implies that $\prod\limits_{n \in \N} Z_n$ is a Polish space. Since for each $n \in \N$, $f_n$ is continuous, \tcb{an exercise in the analysis notes section on product topology} implies that $f$ is continuous. Since for each $n \in \N$, $f_n$ is surjective, $\prod\limits_{n \in \N}f_n : \prod\limits_{n \in \N} Z_n \rightarrow \prod\limits_{n \in \N} A_n$ is surjective \tcr{(maybe reference exercise)} %\extrex{analysis-ex:metric_spaces:polish_spaces:0003}.
		\end{enumerate}
	\end{proof}

	\begin{ex} \lex{ex:measurable_spaces:standard_borel_spaces:0025}
		Let $X$ be a Polish space. Then for each $B \in \MB(X)$, $B$ is analytic.
	\end{ex}

	\begin{proof}
		Clear by \rex{ex:measurable_spaces:standard_borel_spaces:0023}.
	\end{proof}
	...
	\begin{defn}\
		\begin{itemize}
			...
			\item for $n \in \N$, we define the \tbf{projection of $\MN$ onto $\N^n$}, denoted $\pi_{[n]}: \MN \rightarrow \N^n$, by $\pi_{[n]}(\gam) \defeq (\gam_1, \ldots, \gam_{n})$.\\
			\tcr{maybe put this definition inside the following exercise, wait to see if it is needed elsewhere first}
		\end{itemize}
	\end{defn}

	\begin{ex} \lex{ex:measurable_spaces:standard_borel_spaces:0026}
		Let $X$ be a Polish space. Suppose that $X \neq \varnothing$. Then there exists $f: \MN \rightarrow X$ such that $f$ is continuous and surjective. \\
		\tbf{Hint:} Define $\Gam \defeq \{\N^n: n \in \N\}$. \tcb{An exercise in the analysis notes section on countability of metric spaces} implies that there exists $(C_{\gam})_{\gam \in \Gam} \subset \MP(X)$ such that 
		\begin{enumerate}
	...
	\begin{proof}\
		\begin{itemize}
			\item Since $X$ is a Polish space, there exists a metric $d$ on $X$ such that $(X,d)$ is a complete separable metric space. Define $\Gam \defeq \{\N^n: n \in \N\}$. \tcb{An exercise in the analysis notes section on countability of metric spaces} implies that there exists $(C_{\gam})_{\gam \in \Gam} \subset \MP(X)$ such that 
			\begin{enumerate}
				\item for each $\gam \in \Gam$,
				\begin{enumerate}
					\item $C_{\gam}$ is closed and $C_{\gam} \neq \varnothing$,
				...
				\item $X = \bigcup\limits_{|\gam| = 1} C_{\gam}$.
			\end{enumerate} 
			We note that Cantor's nested set theorem \tcb{in the analysis notes section on completeness} implies that for each $\om \in \MN$, that there exists $x \in X$ such that $\bigcap\limits_{n \in \N} C_{\pi_{[n]}(\om)} = \{x\}$. The axiom of choice implies that there exists $f:\MN \rightarrow X$ such that for each $\om \in \MN$, $\bigcap\limits_{n \in \N} C_{\pi_{[n]}(\om)} = \{f(\om)\}$. 
...
%	\begin{ex} \lex{ex:measurable_spaces:standard_borel_spaces:0027} \tcr{DELETE}
%		Let $X$ be a Polish space. Then
%		\begin{enumerate}
%			\item there exists a Polish space $Z$ and $f:Z \rightarrow X$ such that $Z$ is zero-dimensional, $f$ is a continuous and $f$ is a measurable isomorphism. \\
%			\tbf{Hint:} \rex{ex:measurable_spaces:standard_borel_spaces:0002}
%			\item for each $B \in \MB(X)$, there exists a Polish space $Z$ and $f:Z \rightarrow B$ such that $Z$ is zero-dimensional, $f$ is a continuous and $f$ is a measurable isomorphism. \\
%			\tbf{Hint:} Define 
%			$\MD \defeq \{B \in \MB(X, \MT_X): \text{there exists a Polish space $Z$ and $f:Z \rightarrow B$ such that $Z$ is zero-dimensional } \\ \text{and $f$ is a continuous bijection}\}$ and consider \rex{ex:measurable_spaces:standard_borel_spaces:0021}, \rex{ex:measurable_spaces:standard_borel_spaces:0022} and \rex{ex:measurable_spaces:standard_borel_spaces:0020}
%			\tcr{rework so that $f$ is continuous and measurable isomprohism}
%		\end{enumerate}
%	\end{ex}
%
%	\begin{proof}\
...
%				\item Let $U \in \MT_X$. \tcb{An exercise in the analysis notes on polish spaces} implies that $U$ is a Polish space. The previous part implies that there exists a Polish space $Z$ and $f:Z \rightarrow U$ such that $Z$ is zero-dimensional, $f$ is continuous and $f$ is a measurable isomorphism. Since $U \in \MT_X$ is arbitrary, we have that $\MT_X \subset \MD$.
%				\item Let $(B_n)_{n \in \N} \subset \MD$. Then $(B_n)_{n \in \N} \subset \MB(X)$ and for each $n \in \N$, there exists a Polish space $Z_n$ and $f_n:Z_n \rightarrow B_n$ such that $Z_n$ is zero-dimensional, $f_n$ is continuous and $f_n$ is a measurable isomorphism. Define $Z \subset \prod\limits_{n \in \N} Z_n$ by $Z \defeq (\prod\limits_{n \in \N} f_n)^{-1}(\Del_{X^{\N}})$. Then \rex{ex:measurable_spaces:standard_borel_spaces:0021} implies that $Z$ is a Polish space, $Z$ is zero-dimensional and there exists $f: Z \rightarrow \bigcap\limits_{n \in \N} B_n$ such that $f$ is continuous and $f$ is a measurable isomorphism. Thus $\bigcap\limits_{n \in \N} B_n \in \MD$. Since $(B_n)_{n \in \N} \subset \MD$ is arbitrary, we have that for each $(B_n)_{n \in \N} \subset \MD$, $\bigcap\limits_{n \in \N} B_n \in \MD$. 
%				\item Let $(B_n)_{n \in \N} \subset \MD$. Suppose that $(B_n)_{n \in \N}$ is disjoint. Set $Z \defeq \coprod\limits_{n \in \N} Z_n$. \tcb{An exercise in the analysis notes section on Polish spaces} implies that $Z$ is a Polish space. \rex{ex:measurable_spaces:standard_borel_spaces:0022} implies that there exists $f: Z \rightarrow \bigcup\limits_{n \in \N} B_n$ such that $f$ is continuous and $f$ is a measurable isomorphism. \tcr{there, Z is zero-dim} Thus $\bigcup\limits_{n \in \N} B_n \in \MD$. Since $(B_n)_{n \in \N} \subset \MD$ such that $(B_n)_{n \in \N}$ is disjoint is arbitrary, we have that for each $(B_n)_{n \in \N} \subset \MD$, if $(B_n)_{n \in \N}$ is disjoint, then $\bigcup\limits_{n \in \N} B_n \in \MD$. 
%			\end{itemize}
%			Thus $\MD$ is a $\del$-system on $X$. Since $X$ is a Polish space, $X$ metrizable. Since $\MT_X \subset \MD$,  \rex{ex:measurable_spaces:standard_borel_spaces:0020} implies that
%			\begin{align*}
%				\MD & \subset \MB(X, \MT_X) \\
...
%	\end{proof}
	...
	\begin{defn} \ld{def:measurable_spaces:standard_borel_spaces:0028}
		Let $X$ be a polish space and $A \subset X$. Then $A$ is said to be \tbf{z-analytic} if there exists a Polish space $Z$ and $f:Z \rightarrow A$ such that $Z$ is zero-dimensional and $f$ is continuous and $f$ is a measurable isomorphism. 
		\tcr{(this definition is not standard and should probably be changed)}
	\end{defn}
	
	\begin{ex} \lex{ex:measurable_spaces:standard_borel_spaces:0029}
	...
	\begin{proof}
		...
		\begin{enumerate}
			\item  \rex{ex:measurable_spaces:standard_borel_spaces:0021} implies that there exists a Polish space $Z$ and $f:Z \rightarrow \bigcap\limits_{n \in \N} A_n$ such that $Z$ is zero-dimensional and $f$ is a continuous bijection.  Hence $\bigcap\limits_{n \in \N} A_n$ is z-analytic.
			\item Suppose that $(A_n)_{n \in \N}$ is disjoint. \rex{ex:measurable_spaces:standard_borel_spaces:0022} implies that there exists $f: \coprod\limits_{n \in \N} Z_n \rightarrow \bigcup\limits_{n \in \N} A_n$ such that $f$ is a continuous bijection. \tcb{An exercise in the analysis notes section on zero-dimensional topological spaces} implies that $\coprod\limits_{n \in \N} Z_n$ is zero-dimensional. Hence $\bigcup\limits_{n \in \N} A_n$ is z-analytic.
			\item \tcr{FINISH!!!}
		\end{enumerate}
	\end{proof}
	
	\begin{ex} \lex{ex:measurable_spaces:standard_borel_spaces:0030}
		Let $X$ be a Polish space. Then for each $B \in \MB(X)$, $B$ is z-analytic.
	\end{ex}
	
	\begin{proof}
		Clear by \rex{ex:measurable_spaces:standard_borel_spaces:0027}.
	\end{proof}
	...
	\begin{ex} \lex{ex:measurable_spaces:standard_borel_spaces:0026}
		...
		\end{enumerate} 
		\tbf{Hint:} Define $\Gam \defeq \bigcup\limits_{n \in \N} N^n$ and $Z_0 \defeq \{z \in Z: \text{$z$ is a condensation point of $Z$}\}$. \tcb{An exercise in the analysis notes section on countability of metric spaces} implies that there exists $(C_{\gam})_{\gam \in \Gam} \subset \MT_d$ and $(z_{\gam})_{\gam \in \Gam} \in \prod\limits_{\gam \in \Gam} C_{\gam}$ such that 
		\begin{enumerate}
			\item \begin{itemize}
			...
			\end{enumerate} 
		\end{enumerate} 
		\tcr{(possibly reformat exercise from analysis notes to say the same more efficiently, maybe index by $\Gam$ including length 0 indices?)}and use Cantor’s nested set theorem.
	\end{ex}
	
	\begin{proof}
		Define $Z_0 \subset Z$ by $Z_0 \defeq \{z \in Z: \text{$z$ is a condensation point of $Z$}\}$. Since $Z$ is Polish and zero-dimensional, \tcb{An exercise in the analysis notes metric space chapter in section on zero dimensional polish spaces} implies that $Z_0$ is Polish, $Z_0$ is zero dimensional and for each $z \in Z_0$, $z$ is a condensation point of $Z_0$. Since $Z$ is Polish, $Z$ is second-countable and \tcb{An exercise in the analysis notes topology section on second countability} implies that $Z_0^c$ is countable. Since $Z$ is uncountable, we have that $Z_0$ is uncountable. In particular, $Z_0 \neq \varnothing$. Since $Z_0$ is a Polish space, $Z_0$ is separable and there exists a metric $d$ on $Z_0$ such that $(Z_0, d)$ is a complete metric space. Define $\Gam \defeq \{\N^n: n \in \N\}$. Since $Z_0$ is separable, $Z_0$ is zero-dimensional, $Z_0 \neq \varnothing$, and for each $z \in Z_0$, $z_0$ is a condensation point of $Z_0$, \tcb{an exercise in the analysis notes section on countability of metric spaces} implies that there exists $(C_{\gam})_{\gam \in \Gam} \subset \MT_d$ and $(z_{\gam})_{\gam \in \Gam} \in \prod\limits_{\gam \in \Gam} C_{\gam}$ such that 
		\begin{enumerate}
			\item 
			\begin{itemize}
				\item there exists $z_0 \in Z_0$ such that $Z_0 \setminus \{z_0\} = \bigcup\limits_{|\gam| = 1} C_{\gam}$,
	...
	\begin{ex} \lex{ex:measurable_spaces:standard_borel_spaces:0027}
		...
		\tcr{replace $f(\MN)^c$ with $B \setminus f(\MN)$ because $f(\MN)^c$ may be interpreted as $X \setminus f(\MN)$.}
	\end{ex}

	\begin{proof}
		\rex{ex:measurable_spaces:standard_borel_spaces:0023} implies that there exists a Polish space $Z$ and $g:Z \rightarrow B$ such that $Z$ is zero-dimensional, $g$ is continuous and $g$ is a measurable isomorphism. Since $B$ is uncountable and $g$ is a bijection, $Z$ is uncountable. \rex{ex:measurable_spaces:standard_borel_spaces:0026} then implies that implies that there exists $h:\MN \rightarrow Z$ such that $h$ is continuous, $h$ is injective and $h(\MN)^c$ is countable. Define $f: \MN \rightarrow B$ by $f \defeq g \circ h$. Then $f$ is continuous and $f$ is a injective. Since $g$ is a bijection,
		\begin{align*}
			f(\MN)^c
			& = g (h(\N))^c \\
			& = g(h(\MN)^c).
	...
	\begin{ex} \lex{ex:measurable_spaces:standard_borel_spaces:0028}
	...
	\begin{proof}
		...
		\begin{align*}
			...
			& \subset \MB(X).
		\end{align*}
		Since $f_0$ is a continuous injection, $f$ is a continuous bijection. Since $\MC$ is compact and $A$ is Hausdorff, the closed map lemma \tcb{(an exercise in the analysis notes subsection on compactness and continuity)} implies that $f$ is a homeomorphism.
	\end{proof}
	...
	\begin{defn} \ld{def:standard_borel_spaces:borel_iso:0001}
		Let $(X, \MA)$ be a measurable space. Then $(X, \MA)$ is said to be a \tbf{standard Borel space} if there exists a Polish space $Z$, $B \in \MB(Z)$ and $f:B \rightarrow Z$ such that $f$ is a $(\MB(B), \MA)$-measurable isomorphism.
	\end{defn}
	
	\begin{ex} \lex{ex:standard_borel_spaces:borel_iso:0002}
	...
	\begin{ex} \lex{ex:standard_borel_spaces:borel_iso:0002.1}
		Define $Z$ and $\phi: Z \rightarrow [0,1]$ as in \rd{def:measurable_spaces:standard_borel_spaces:0001}. Then $(Z, \MB(Z))$ is a standard Borel space.
	\end{ex}
	
	\begin{proof}
		Since $[0,1]$ is a Polish space, \rex{ex:measurable_spaces:standard_borel_spaces:0011} implies that $(Z, \MB(Z))$ is a standard Borel space.
	\end{proof}
	
	\begin{ex} \lex{ex:standard_borel_spaces:borel_iso:0002.2}
		Let $X$ be a Polish space and $B \in \MB(X)$. 
		\begin{enumerate}
			\item \tcr{FINISH!!! (finite)} 
			\item \tcr{FINISH!!! (countably infinite)}
			\item If $B$ is uncountable, then there exists $f:B \rightarrow \MC$ such that $f$ is a $(\MB(X) \cap B, \MB(\MC))$-measurable isomorphism.  \\
			\tbf{Hint:} 
			Consider \rex{ex:measurable_spaces:standard_borel_spaces:0028},
			\rex{} \tcb{(an exercise in the analysis notes section on Polish spaces, or metrizable spaces to get $f:X \rightarrow G \subset \H$ a homeomorphism)} and
			\rex{ex:measurable_spaces:subspaces:0011}.
		\end{enumerate} 
	\end{ex}
	...
	\begin{proof}\
		\begin{enumerate}
			\item \tcr{FINISH!!!}
			\item \tcr{FINISH!!!}
			\item Suppose that $B$ is uncountable. 
			\begin{itemize}
				\item \rex{ex:measurable_spaces:standard_borel_spaces:0028} implies that there exists $A \in \MB(B)$ and $f_0: \MC \rightarrow A$ such that $f_0$ is a homeomorphism. Then $f$ is $(\MB(\MC), \MB(B) \cap A)$-bimeasurable. Define $\iota: A \rightarrow B$ by $\iota(x) \defeq x$. Since $\MB(A) = \MB(B) \cap A$ and $A \in \MB(B)$, \rex{ex:measurable_spaces:subspaces:0007.1} implies that $\iota$ is $(\MB(B) \cap A, \MB(B))$-bimeasurable. Define $f: \MC \rightarrow A$ by $f \defeq \iota \circ f_0$. Therefore $f$ is $(\MB(\MC), \MB(B))$-bimeasurable. Since $f_0$ is a bijective and $\iota$ is injective, $f$ is injective. 
				\item \tcb{An exercise in the analysis notes chapter on metric spaces section on polish spaces} implies that there exists $E \subset \H$ and $g_0: X \rightarrow E$ such that $E$ is a $G_{\del}$-set and $g_0$ is a homeomorphism. Since $g_0$ is a homeomorphism and $B \in \MB(X)$, we have that $g_0$ is a $(\MB(X), \MB(E))$-measurable isomorphism and $g_0(B) \in \MB(E)$. Since $E$ is a $G_{\del}$-set, $E \in \MB(\H)$. Therefore 
				\begin{align*}
					g_0(B)
					& \in \MB(E) \\
					& = \MB(\H) \cap E \\
					& \subset \MB(\H). 
				\end{align*}
				Define $g':B \rightarrow g_0(B)$ by $g'(x) \defeq g_0(x)$. Since $g_0$ is a $(\MB(X), \MB(E))$-measurable isomorphism, $g'$ is a $(\MB(B), \MB(\H) \cap g_0(B))$-measurable isomorphism. Define $Z \subset \MC$ and $\phi:Z \rightarrow [0,1]$ as in \rd{def:measurable_spaces:standard_borel_spaces:0001}. \rex{ex:measurable_spaces:standard_borel_spaces:0011} implies that $\phi$ is a $(\MB(Z), \MB([0,1]))$-measurable isomorphism. \rex{} \tcr{(cite exercise)} then implies that $\phi^{\N}$ is a $(\MB(Z^{\N}), \MB(\H))$-measurable isomorphism. Since \rex{} \tcr{(cite exercise)} implies that $\MB(Z^{\N}) = \MB(\MC^{\N}) \cap Z^{\N}$, we have that $(\phi^{\N})^{-1}$ is a $(\MB(\H), \MB(\MC^{\N}) \cap Z^{\N})$-measurable isomorphism. \tcb{An exercise in the analysis notes chapter on metric spaces, section on zero-dimensional metric spaces, subsection on cantor space} implies that there exists $H: \MC^{\N} \rightarrow \MC $ such that $H$ is a homeomorphism. Therefore $H$ is a $(\MB(\MC^{\N}), \MB(\MC))$-measurable isomorphism. \rex{ex:measurable_spaces:standard_borel_spaces:0006} implies that $Z \in \MB(\MC)$. Therefore
				\begin{align*}
					Z^{\N}
					& = \bigcap\limits_{n \in \N} \pi_n^{-1}(Z) \\
					& \in \MB(\C^{\N}).
				\end{align*} 
				Since $g_0(B) \in \MB(\H)$ and $Z^{\N} \in \MB(\MC^{\N})$, \rex{ex:measurable_spaces:subspaces:0007.1} implies that $\iota_{g_0(B)}$ is $(\MB(\H) \cap g_0(B), \MB(\H))$-bimeasurable and $\iota_{Z^{\N}}$ is $(\MB(\MC^{\N}) \cap Z^{\N}, \MB(\MC^{\N}))$-bimeasurable. Define $g:B \rightarrow \MC$ by $g \defeq H \circ \iota_{Z^{\N}} \circ (\phi^{\N})^{-1} \circ \iota_{g_0(B)} \circ g'$. Since $H$, $\iota_{Z^{\N}}$, $(\phi^{\N})^{-1}$, $\iota_{g_0(B)}$ and $g'$ are injective and bimeasurable, we have that $g$ is injective and $(\MB(B), \MB(\MC))$-bimeasurable. 
			\end{itemize}
			Thus there exist $f: \MC \rightarrow B$ and $g: B \rightarrow \MC$ such that $f,g$ are injective, $f$ is $(\MB(\MC), \MB(B))$-bimeasurable and $g$ is $(\MB(B), \MB(\MC))$-bimeasurable. \rex{ex:measurable_spaces:subspaces:0011} then implies that there exists $h: B \rightarrow \MC$ such that $h$ is a $(\MB(B), \MB(\MC))$-isomorphism.
		\end{enumerate}
	\end{proof}
	
	\begin{ex} \lex{ex:standard_borel_spaces:borel_iso:0003} \tbf{Borel Isomorphism Theorem:} \\
		Let $(X, \MA)$ and $(Y, \MB)$ be a standard Borel spaces. Then $(X, \MA)$ and $(Y, \MB)$ are measurable isomorphic iff $|X| = |Y|$. 
	\end{ex}
	
	\begin{proof}
		\rex{ex:standard_borel_spaces:borel_iso:0002.2}
		\tcr{FINISH!!!}
	\end{proof}
	...
	\begin{itemize}
		\item \tcr{for $P \subset Y $ (i.e. a unary relation (property) on $Y$), define $P_{\text{$\mu$-a.e.}} \subset Y^X $ (i.e. a unary relation (property) on $Y^X$) by $f \sim_{\text{$\mu$-a.e.}} g$ iff there exists $N \in \MA$ such that $\mu(N) = 0$ and $f^{-1}(P^c) \subset N$ (i.e. the set of $x \in X$ such that $f(x)$ does not have property $P$ is contained in a set of measure zero). Question: is there a general structure which describes taking relations on a set to relations on a hom-set?} 
		\item  
	\end{itemize}
	
	\begin{defn} \tcr{(maybe relocate or delete this)}
		Let $X$ be a set, $\ME \subset \MP(X)$ and $\rho: \ME \rightarrow \RG$. Then $\rho$ is said to be 
		\begin{itemize}
			\item \tbf{finitely-additive} if for each $A, B \in \ME$, $A \cap B = \varnothing$ and $A \cup B \in \ME$ implies that $\rho(A \cup B) = \rho(A) + \rho(B)$,
			\item \tbf{$\sig$-additive} if for each $(A_j)_{j \in \N} \subset \ME$, $(A_j)_{j \in \N}$ is disjoint and $\bigcup\limits_{j \in \N} A_j \in \ME$ implies that $\rho \bigg( \bigcup\limits_{j \in \N} A_j \bigg) = \sum\limits_{j \in \N} \rho(A_j)$,
			\item \tbf{increasing} if for each $A, B \in \ME$, $A \subset B$ implies that $\rho(A) \leq \rho(B)$.
		\end{itemize}
	\end{defn}
	...
	\begin{defn} \ld{def:measures:00001}
		...
		\begin{enumerate}
			...
			\item for each $(A_n)_{n \in \N} \subset \MA$. If $(A_n)_{n \in \N}$ is disjoint, then $$\mu\bigg(\bigcup_{n \in \N}A_n \bigg) = \sum\limits_{n \in \N}\mu(A_n)$$
		\end{enumerate}
	\end{defn}
	...
	\begin{ex} \lex{ex:measures:00002.1}
	...
	\begin{ex} \lex{ex:measures:00002.2}
		...
		$$\mu\bigg(\bigcup_{n \in \N}A_n \bigg) = \sum\limits_{n \in \N}\mu(A_n).$$
	...
	\begin{proof}\
		\begin{itemize}
			\item $(\implies):$ \\
			Suppose that $\mu \in M_+(X, \MA)$. \rex{ex:measures:00002.1} implies that $\mu(\varnothing) = 0$.
			\item $(\impliedby):$ \\
			Suppose that $\mu(\varnothing) = 0$. Then there exists $A \in \MA$ such that $\mu(A) < \infty$. Hence $\mu \in M_+(X, \MA)$.
		\end{itemize}
	\end{proof}
	
	\begin{ex} \lex{ex:measures:00002.3}
	...
	\begin{proof} Define $\mu: \MA \rightarrow \RG$ by $\mu \defeq \sum\limits_{j \in \N} \lam_j \mu_j$.
		\begin{enumerate}
			...
			\begin{align*}
				...
				& = \sum\limits_{j \in \N} \lam_j \bigg[ \sum\limits_{k \in \N} \mu_j(E_k) \bigg] \\
				& = \sum\limits_{j \in \N} \bigg[ \sum\limits_{k \in \N} \lam_j \mu_j(E_k) \bigg] \\
	...
	&= \sum\limits_{j \in \N} \mu(E_{\al_j}) \\
	& \geq \sum\limits_{j \in \N} \frac{1}{N} \\
	&= \infty
	\end{align*} 
	...
	\begin{ex} \lex{ex:measures:00004}
		...
		\begin{enumerate}
			...
			\item (subadditivity): for each $(A_n)_{n \in \N} \subset \MA$, $$\mu \bigg( \bigcup_{n \in \N} A_n \bigg) \leq \sum\limits_{n \in \N}\mu(A_n)$$
			\item (continuity from below): for each $(A_n)_{n \in \N} \subset \MA$, if for each $n \in \N$, $A_n \subset A_{n+1}$, then $$\mu\bigg(\sup\limits_{n \in \N} A_n\bigg) = \sup\limits_{n \in \N}\mu(A_n)$$
	...
	\begin{proof}\
		\begin{enumerate}
			...
			\begin{align*}
				...
				&= \sum\limits_{n \in \N}\mu(B_n) \\
				&\leq \sum\limits_{n \in \N}\mu(A_n)
			\end{align*} 
			...
			\begin{align*}
				\sup\limits_{n \in \N}\mu(B_n)
				&= \sup\limits_{n \in \N}\mu(A_1 \setminus A_n)\\
				&= \sup\limits_{n \in \N} \bigg[ \mu(A_1) - \mu(A_n) \bigg]\\
	...
	\begin{proof}\
		\begin{enumerate}
			...
			\begin{align*}
				...
				& = \mu \bigg[\sup\limits_{n \in \N} \bigg(\inf\limits_{k \geq n} A_k \bigg) \bigg] \\
				&= \sup\limits_{n \in \N}  \mu\bigg( \inf_{k \geq n}A_k\bigg)\\
				& = \liminf_{n \rightarrow \infty} \mu\bigg( \inf_{k \geq n}A_k\bigg) \\
				& \leq \liminf_{n \rightarrow \infty}  \mu(A_n) \\
			\end{align*}
			\item Since $\mu\bigg(\sup\limits_{ \geq 1}A_k \bigg) < \infty$, $\bigg(\sup\limits_{k \geq n} \bigg)_{n \in \N}$ is a decreasing and for each $n \in \N$, $A_n \subset \sup\limits_{k \geq n}A_n$, we have that 
			\begin{align*}
				\mu \bigg(\limsup_{n \rightarrow \infty} A_n 
				\bigg) 
				&= \mu \bigg[\inf_{n \in \N} \bigg(\sup\limits_{k  \geq n} A_k \bigg) \bigg] \\
				&= \inf_{n \in \N}\mu\bigg( \sup\limits_{k \geq n} A_k\bigg) \\
				& = \limsup_{n \rightarrow \infty} \mu \bigg( \sup\limits_{k \geq n}A_k \bigg) \\
	...
	\begin{proof}\
		\begin{itemize}
			...
			\begin{align*}
				...
				& = \sup\limits_{n \in \N} \mu(F \cap A_n) \\
				& = 0
			\end{align*}
	...
	\begin{proof} Suppose that $A$ is countable. For each $\al, \be \in A$, define $M_{\al, \be},N_{\al, \be},M,N \in \MA$ as in the previous definition. Since $A$ is countable, $M,N \in \MA$. 
		\begin{enumerate}
			...
			\begin{align*}
				...
				& \leq \sum\limits_{\substack{(\al, \be) \in A^2 \\ \al \leq \be}} \mu(M_{\al, \be}^c) \\
				& = 0
			\end{align*}
	...
	\begin{defn} \ld{def:measures:00012}
		...
		\end{cases}
		\] 
		where $|\cdot|: \MP(X) \rightarrow \RG$ denotes the cardinality of $E$. 
	\end{defn}
	...
	\begin{proof}\
		\begin{enumerate}
			...
			\begin{align*}
				\infty
				& = \sum\limits_{n \in J} 1 \\
				& \leq \sum\limits_{n \in J} |A_n| \\
				& = \sum\limits_{n \in \N} |A_n| \\
				& = \sum\limits_{n \in \N} \#(A_n) 
			\end{align*}
			and 
			\begin{align*}
...
\begin{ex} \lex{ex:measures:00013.1}
	Let $(X, \MA)$ be a measurable space, $\mu \in M_1(X)$ and $x \in X$. Suppose that $\{x\} \in \MA$. Then $\mu = \del_x$ iff $\mu(\{x\}^c) = 0$ \tcr{reword to 'supported on $\{x\}$'}. 
\end{ex}

\begin{proof}\
	\begin{itemize}
		\item $(\implies)$: \\
		Suppose that $\mu = \del_x$. Then
		\begin{align*}
			\mu(\{x\}^c) 
			& = \del_x(\{x\}^c) \\
			& = 0.
		\end{align*}
		\item $(\impliedby)$: \\
		Suppose that $\mu(\{x\}^c) = 0$. Let $E \in \MA$. Since 
		\begin{align*}
			0
			& \leq \mu(E \cap \{x\}^c) \\
			& \leq \mu(\{x\}^c) \\
			& = 0,
		\end{align*}
		we have that $\mu(E \cap \{x\}^c) = 0$. 
		\begin{itemize}
			\item If $x \in E$, then 
			\begin{align*}
				\mu(E)
				& = \mu(E \cap \{x\}) + \mu(E \cap \{x\}^c) \\
				& = \mu(\{x\}) + 0 \\
				& = \mu(\{x\}) + \mu(\{x\}^c) \\
				& =  \mu(\{x\} \cup \{x\}^c) \\
				& = \mu(X) \\
			...
			\end{align*}
		\end{itemize}
		Since $E \in \MB(X)$ is arbitrary, we have that for each $E \in \MB(X)$, 
		\begin{align*}
			\mu(E)
			& = 
			\begin{cases}
...
\begin{ex} \lex{ex:measures:00014}
	Let $X$ be a topological space. Suppose that $\MK(X) \subset \MB(X)$. Let $K \in \MK(X)$ and $\mu \in M_+(X)$. If $\mu(K) > 0$, then there exists $x \in K$ such that for each $U \in \opn{\MN}(x)$, $\mu(U) > 0$.
\end{ex}

\begin{proof}
	Suppose that $\mu(K) > 0$. For the sake of contradiction, suppose that for each $x \in K$, there exists $U \in \opn{\MN}(x)$ such that $\mu(U) = 0$. \tcr{The axiom of choice} implies that there exists $(U_x)_{x \in K} \in \opn{\Cover}(K)$ such that for each $x \in K$, $\mu(U_x) = 0$. Since $K$ is compact, there exists $n \in \N$ and $(x_j)_{j \in [n]} \subset K$ such that $(U_{x_j})_{j \in [n]} \in \opn{\Cover}(K)$. Then
	\begin{align*}
		0 
		& < \mu(K) \\
		& \leq \mu \bigg( \bigcup\limits_{j \in [n]} U_{x_j} \bigg) \\
		& \leq \sum\limits_{j \in [n]} \mu(U_{x_j}) \\
		& = 0,
	\end{align*}
	which is a contradiction. Hence there exists $x \in K$ such that for each $U \in \opn{\MN(x)}$, $\mu(U) > 0$. 
\end{proof}
...
\begin{proof}\
	\begin{enumerate}
		...
		\begin{align*}
			...
			& = \sum\limits_{j \in \N} \mu(f^{-1}(B_j)) \\
			& = \sum\limits_{j \in \N} f_*\mu(B_j)
		\end{align*}
	\end{enumerate}
...
\end{proof}
	...
	\begin{defn} \ld{def:outer_measures:00007} 
		...
		\begin{enumerate}
			...
			\item for each $(A_n)_{n \in \N} \subset \MA_0$, if $(A_n)_{n \in \N}$ is disjoint and $\bigcup\limits_{n \in \N}A_n \in \MA_0$, then $$\mu_0 \bigg(\bigcup_{n\in \N}A_n \bigg) = \sum\limits_{n \in \N}\mu_0(A_n)$$
		\end{enumerate}
	\end{defn}
	...
	\begin{ex} \lex{ex:outer_measures:00007.1}
	...
	\begin{proof}\
		\begin{enumerate}
			...
			\begin{align*}
				...
				& = 0 + \sum\limits_{j \in \N} \mu_0(A_j) \\
				& = \sum\limits_{j \in \N} \mu_0(A_j).
			\end{align*}  
...
%	\end{thm}
%	
%	\begin{ex} \lex{ex:outer_measures:00008.1}
%
...
%	
%	\begin{proof}
%		Since $\MA$ is an alegbra on $X$, \rex{ex:outer_measures:00007.1} implies that $\mu$ is a premeasure on $(X, \MA)$. Since $\MA$ is a $\sig$-algebra on $X$, $\sig(\MA) = \MA$. Since $\mu$ is finite, \rex{ex:outer_measures:00008} implies that there exists a unique $\mu' \in M_+(X, \MA)$ such that $\mu'|_{\MA} = \mu$. Since $\mu'|_{\MA} = \mu'$, we have $\mu' = \mu$. Hence $\mu \in M_+(X, \MA)$.
%	\end{proof}
%	
%	
%	
...
%	\end{proof}
	...
	\begin{defn}  \ld{def:outer_measures:00004} 
		...
		$$\rho^*(A) = \inf \bigg \{\sum\limits_{n \in \N}\rho(E_n): (E_n)_{n \in \N} \subset \ME \text{ and }A \subset \bigcup_{n \in \N}E_n \bigg \}$$ 
	\end{defn}
	...
	\begin{proof}For $A \subset \MP(X)$, set 
		$$V(A) = \bigg \{\sum\limits_{n \in \N}\rho(E_n): (E_n)_{n \in \N} \subset \ME \text{ and }A \subset \bigcup_{n \in \N}E_n \bigg \}$$
		\begin{enumerate}
			...
			$$\sum\limits_{j \in \N} \rho(E_{n,j}) < \rho^*(A_n) + \ep2^{-n}$$
			Then $(E_{n,j})_{n,j \in \N} \subset \ME$ and 
			\begin{align*}
				\sum\limits_{n,j \in \N} \rho(E_{n,j}) 
				& = \sum\limits_{n \in \N} \bigg[ \sum\limits_{j \in \N} \rho(E_{n,j})  \bigg] \\
				& \leq \sum\limits_{n \in \N} (\rho^*(A_n) + \ep 2^{-n}) \\
				& = \sum\limits_{n \in \N} \rho^*(A_n) + \ep
			\end{align*}
			This implies that 
			\begin{align*}
				\rho^* \bigg( \bigcup\limits_{n \in \N} A_n \bigg) 
				& = \inf V \bigg( \bigcup\limits_{n \in \N} A_n \bigg) \\
				& \leq \sum\limits_{n,j \in \N} \rho(E_{n,j})  \\
				& \leq \sum\limits_{n \in \N} \rho^*(A_n) + \ep
			\end{align*}
			Since $\ep >0$ is arbitrary, we have that
			$$\rho^* \bigg( \bigcup\limits_{n \in \N} A_n \bigg) \leq \sum\limits_{n \in \N} \rho^*(A_n)$$
		\end{enumerate}
	...
	\begin{proof}
		...
		\begin{itemize}
			...
			\begin{align*}
				\mu(B)
				& \leq \mu(B_n) \\
				& \leq \sum\limits_{j \in \N} \mu(E_{n,j}) \\
				& < \mu^*(A) + 1/n 
			\end{align*}
	...
	\begin{proof} For each $A$ set $V(A) \defeq \{\nu(E): E \in \MA \text{ and $E$ $\mu^*$-covers $A$} \}$.
		\begin{enumerate}
			...
			\begin{align*}
				...
				& \leq \sum\limits_{n \in \N} \mu^*(A_n \setminus E_n) \\
				& = 0
			\end{align*}
			...
			\end{align*}
			Since $\ep >0$ is arbitrary, 
			$$ \nu_{\mu} \bigg( \bigcup_{n \in \N} A_n \bigg)  \leq \sum\limits_{n \in \N} \nu_{\mu}(A_n) $$
		\end{enumerate} 
		Hence $\nu_{\mu}$ is an outer measure on $X$.
	\end{proof}
	...
	\tcr{reserve $\mu_{|E}$ to mean $\mu^E/ \mu(E)$ if $0 < \mu(E) < \infty$ (i.e. the conditional of $\mu$ on $E$)}

	\begin{ex} \lex{ex:subspace_measures:00002} 
		Let $(X, \MA)$ be a measurable space, $\mu \in M_+(X, \MA)$ and $E \in \MA$. Then 
		\begin{enumerate}
			\item \begin{enumerate}
	...
	\begin{ex} \lex{ex:subspace_measures:00002.01} 
		Let $(X, \MA)$ be a measurable space, $\mu \in M_+(X, \MA)$ and $E \in \MA$. Then $\mu = \mu_E + \mu_{E^c}$ \tcr{(update to countable disjoint partition of $X$)}.
	\end{ex}
	...
	\begin{proof}
		...
		\begin{align*}
			\mu(A)
			& = \mu([A \cap E] \cup [A \cap E^c]) \\
			& = \mu(A \cap E) + \mu(A \cap E^c) \\
			& = \mu_E(A) + \mu_{E^c}(A) \\
			& = [\mu_E + \mu_{E^c}](A).
		\end{align*}
		Thus $\mu = \mu_E + \mu_{E^c}$. 
	\end{proof}

	\begin{ex} \lex{ex:subspace_measures:00002.1}
		Let $(X, \MA)$ be a measurable space, $\mu \in M_+(X, \MA)$. Then 
		\begin{enumerate}
			\item for each $(E_j)_{j \in \N} \subset \MA$, $(E_j)_{j \in \N}$ is disjoint implies that  
			$$\mu_{\bigcup\limits_{j \in \N} E_j} = \sum\limits_{j \in \N} \mu_{E_j},$$
	...
	\begin{proof}\
		\begin{enumerate}
			...
			\begin{align*}
				...
				& = \sum\limits_{j \in \N} \mu(A \cap E_j) \\
				& = \sum\limits_{j \in \N} \mu_{E_j}(A) \\
				& = \bigg[ \sum\limits_{j \in \N} \mu_{E_j} \bigg](A).
			\end{align*}
			Since $A \in \MA$ is arbitrary, we have that for each $A \in \MA$, 
			$$\mu_{\bigcup\limits_{j \in \N} E_j}(A) = \bigg[ \sum\limits_{j \in \N} \mu_{E_j} \bigg](A).$$
			Thus 
			$$\mu_{\bigcup\limits_{j \in \N} E_j} = \bigg[ \sum\limits_{j \in \N} \mu_{E_j} \bigg].$$
	...
	\begin{ex} \lex{ex:subspace_measures:00004} 
		...
		\begin{enumerate}
			...
			\tcr{might need to put pushforward measure section after outer measure section instead of before, need to see if outer measure section uses pushforward, if not move it. Then can use pushforward here}
		\end{enumerate}
	...
	\begin{ex} \lex{ex:subspace_measures:00004.2}
	...
	\begin{proof}
		Let $B \subset E$. Set
		$$V(B) \defeq \bigg \{\sum\limits_{j \in \N} \mu(F_j): (F_j)_{j \in \N} \subset \MA \text{ and } B \subset \bigcup\limits_{j \in \N} F_j \bigg \}$$ 
		and 
		$$V_E(B) \defeq \bigg \{\sum\limits_{j \in \N} \mu|_E(F_j): (F_j)_{j \in \N} \subset \MA \cap E \text{ and } B \subset \bigcup\limits_{j \in \N} F_j \bigg \}$$
		...
		\begin{itemize}
			...
			\begin{align*}
				\mu|_E^*(B)
				& = \inf V_E(B) \\
				& \leq \sum\limits_{j \in \N} \mu|_E(F_j \cap E) \\
				& = \sum\limits_{j \in \N} \mu(F_j \cap E) \\  
				& \leq \sum\limits_{j \in \N} \mu(F_j) \\
				& < \mu^*(B) + \ep \\
				& = \mu^*|_E(B) + \ep  
			\end{align*}
	...
	\begin{ex} \lex{ex:subspace_measures:00005.1} 
		Let $(X, \MA)$ be a measurable space, $\mu \in M_+(X, \MA)$ and $E\in \MA$. Then $(\mu^*)_E = (\mu_E)^*$. 
	\end{ex}
	
	\begin{proof}
		Let $A \subset X$. Define $V(A), V_E(A)$ by 
		$$V(A \cap E) = \bigg \{\sum\limits_{j \in \N} \mu(F_j): (F_j)_{j \in \N} \subset \MA \text{ and } A \cap E \subset \bigcup\limits_{j \in \N} F_j \bigg \}$$ 
		and 
	...
	\tcr{make a note that this is not really a product in the categorical sense since the category of measure/prob measure spaces and measure preserving measurable maps does not have products (make exercise), but this can be given a monoidal category structure \href{https://arxiv.org/abs/math/0206017}{https://arxiv.org/abs/math/0206017} and \href{https://mathoverflow.net/questions/120909/can-one-view-the-independent-product-in-probability-categorially}{https://mathoverflow.net/questions/120909/can-one-view-the-independent-product-in-probability-categorially} as well as the Markov category framewrok by tobias fritz. This is why we use the notation $\mu \otimes \nu$ and not $\mu \times \nu$}
	
	\begin{defn} \ld{00000} 
		\tcr{change notation}
		Let $(X,\MA, \mu), (Y,\MB, \nu)$ be $\sig$-finite measurable spaces. Put $\ME = \{A \times B: A \in \MA \text{ and } B \in \MB\}$. Then $\ME$ is an elementary family and thus $\MM_0 = \{\bigcup_{i =1}^n M_i: (M_i)_{i=1 }^n \subset \ME \text{ are disjoint}\}$ is an algebra on $X \times Y$. We define $\pi_0: \MM_0 \rightarrow \RG$ by $$\pi_0\bigg(\bigcup_{i=1}^n A_i \times B_i \bigg) = \sum\limits_{i=1}^n\mu(A_i)\nu(B_i)$$ Then $\pi_0$ is a premeasure on $(X \times Y, M_0)$. Since $\MA \otimes \MB = \sig(\MM_0)$, we define the \textbf{product measure on $(X \times Y, \MA \otimes \MB)$ induced by $\mu$ and $\nu$}, denoted $\mu \otimes \nu$, to be the unique measure $\mu$ on $(X \times Y, \MA \otimes \MB)$ such that $\mu|_{\MM_0} = \pi_0$. 
		
		
		\begin{note}
			The existence of which is guaranteed by \tcr{a theorem in the previous section}. In particular,
			\begin{align*}
				\mu \otimes \nu(E) 
				&= \inf \bigg\{\sum\limits_{n \in \N}\pi_0(E_i): (E_i)_{i \in \N} \subset \MM_0 \text{ and } E \subset \bigcup_{i \in \N} E_i \bigg\}\\
				&= \inf \bigg\{\sum\limits_{n \in \N}\mu(A_i)\nu(B_i): (A_i \times B_i)_{i \in \N} \subset \ME \text{ and } E \subset \bigcup_{i \in \N} A_i \times B_i \bigg \}
			\end{align*}
		\end{note}
	\end{defn}
...
\begin{defn}
	Let $(X_n,\MA_n, \mu_n)$ be a collection of measure spaces. We define the \textbf{coproduct measure on $\bigg( \coprod_{n \in \N}X_n, \bigoplus\limits_{n \in \N} \MA_n \bigg)$ induced by $(\mu_n)_{n \in \N}$}, denoted $\bigoplus_{n \in \N} \mu_n$, by 
	$$\bigoplus_{n \in \N} \mu_n \bigg( \coprod\limits_{n \in \N} A_n \bigg) \defeq \sum\limits_{n \in \N} \mu_n(A_n).$$ 
\end{defn}

\begin{ex}
	Let $(X_n,\MA_n, \mu_n)$ be a collection of measure spaces. Then $\bigoplus_{n \in \N} \mu_n \in M_+\bigg( \coprod\limits_{n \in \N} X_n, \bigoplus\limits_{n \in \N} \MA_n \bigg)$.
\end{ex}

\begin{proof}
	Set $X \defeq \coprod\limits_{n \in \N} X_n$, $\MA \defeq \bigoplus\limits_{n \in \N} \MA_n$ and $\mu \defeq \bigoplus\limits_{n \in \N} \mu_n$. 
	\begin{enumerate}
		\item Since $\varnothing = \coprod\limits_{n \in \N} \varnothing$, we have that 
		\begin{align*}
			\mu(\varnothing)
			& = \bigoplus_{n \in \N} \mu_n \bigg( \coprod\limits_{n \in \N} \varnothing \bigg) \\
			& = \sum\limits_{n \in \N} \mu_n(\varnothing) \\
			& = \sum\limits_{n \in \N}  0 \\
			& = 0.
		\end{align*}
		\item Let $(E_j)_{j \in \N} \subset \MA$. Suppose that $(E_j)_{j \in \N}$ is disjoint. For each $j, n \in \N$, define $F_{j, n} \in \MA_n$ by $F_{j,n} \defeq \iota_n^{-1}(E_j)$. Then for each $j \in \N$, $E_j = \coprod\limits_{n \in \N} F_{j,n}$. Let $j_1, j_2, n \in \N$. Suppose that $j_1 \neq j_2$. Since $(E_j)_{j \in \N}$ is disjoint, we have that
		\begin{align*}
			F_{j_1, n} \cap F_{j_2, n}
			& = \iota_n^{-1}(E_{j_1}) \cap \iota_n^{-1}(E_{j_2}) \\
			& = \iota_n^{-1}(E_{j_1} \cap E_{j_2}) \\
			& = \iota_n^{-1}(\varnothing) \\
			& = \varnothing.
		\end{align*}
		Since $j_1, j_2 \in \N$ are arbitrary, we have that for each $j_1, j_2 \in \N$, $j_1 \neq j_2$ implies that $F_{j_1, n} \cap F_{j_2, n} = \varnothing$. Hence $(F_{j,n})_{j \in \N}$ is disjoint. Since $n \in \N$ is arbitrary, we have that for each $n \in \N$, $(F_{j,n})_{j \in \N}$ is disjoint. Therefore 
		\begin{align*}
			\mu \bigg( \bigcup\limits_{j \in \N} E_j \bigg)
			& = \mu \bigg( \bigcup\limits_{j \in \N} \bigg[ \coprod\limits_{n \in \N} F_{j,n} \bigg] \bigg) \\
			& = \mu \bigg( \coprod\limits_{n \in \N} \bigg[ \bigcup\limits_{j \in \N} F_{j,n} \bigg] \bigg) \quad \text{(by \tcr{ex in set theory notes coproduct of sets section})} \\
			& = \bigoplus_{n \in \N} \mu_n \bigg( \coprod\limits_{n \in \N} \bigg[ \bigcup\limits_{j \in \N} F_{j,n} \bigg] \bigg) \\
			& = \sum\limits_{n \in \N} \mu_n \bigg( \bigcup\limits_{j \in \N} F_{j,n} \bigg) \\
			& = \sum\limits_{n \in \N} \bigg[ \sum\limits_{j \in \N} \mu_n(F_{j,n}) \bigg] \\
			& = \sum\limits_{j \in \N} \bigg[ \bigoplus_{n \in \N} \mu_n \bigg( \coprod\limits_{n \in \N} F_{j,n} \bigg) \bigg] \\
			& = \sum\limits_{j \in \N} \mu \bigg( \coprod\limits_{n \in \N} F_{j,n} \bigg) \\
			& = \sum\limits_{j \in \N}  \mu (E_j)  \\
		\end{align*}
		Since $(E_j)_{j \in \N} \subset \MA$ with $(E_j)_{j \in \N}$ disjoint is arbitrary, we have that for each $(E_j)_{j \in \N} \subset \MA$, $(E_j)_{j \in \N}$ is disjoint implies that $\mu \bigg( \bigcup\limits_{j \in \N} E_j \bigg) = \sum\limits_{j \in \N}  \mu (E_j)$.
	\end{enumerate}
	Thus $\mu \in M_+(X, \MA)$.
\end{proof}
...
\begin{ex}
	\tcr{If $(E_j)_j \subset \MP(X)$ is null for each $j$, then $\bigcup E_j$ is null, if for some j, then $\bigcap E_j$ is null}
\end{ex}

\tcr{define sigma ideal of null sets (ideal in the order theoretic sense)}

\begin{defn}
	Let $(X, \MA), (Y, \MB) \in \Meas$ and $f,g$. 
\end{defn}
...
\end{proof}
	...
	\begin{defn} \ld{def:integration_nonnegative_functions:0.001}
		Let $(X, \MA, \mu)$ be a measure space and $f \in S^+(X, \MA)$. Suppose that the standard representation of $f$ is $f = \sum\limits_{k=1}^n a_k \chi_{E_k}$. We define the \tbf{integral of $f$ with respect to $\mu$}, denoted 
		$$\int_X f \dmu$$
		by 
		$$\int_X f \dmu \defeq \sum\limits_{k=1}^n a_k \mu(E_k)$$ 
	\end{defn}

	\begin{ex} \lex{ex:integration_nonnegative_functions:0.002}
	...
	\begin{defn} \ld{def:integration_nonnegative_functions:0.003}
		Let $(X, \MA, \mu)$ be a measure space and $f \in L^+(X, \MA)$. We define the \tbf{integral of $f$ with respect to $\mu$}, denoted 
		$$\int_X f \dmu$$
		by 
	...
	\begin{ex}
		\tcr{exercises about integrals of sums}
	\end{ex}

	\begin{thm} \lex{ex:integration_nonnegative_functions:0001} \textbf{Monotone Convergence Theorem:} \\
		Let $(f_n)_{n \in \N} \subset L^+$. Suppose that for each $n \in \N$, $f_n \leq f_{n+1}$. Then 
		$$\sup\limits_{n \in \N} \int_X f_n \dmu = \int \bigg[ \sup\limits_{n \in \N} f_n \bigg] \dmu. $$
	\end{thm}

	\begin{defn} \ld{def:integration_nonnegative_functions:0.004}
		Let $(X, \MA, \mu)$ be a measure space, $f \in L^+(X, \MA)$ and $E \in \MA$. We define the \tbf{integral of $f$ over $E$ with respect to $\mu$}, denoted 
		$$\int_E f \dmu$$
		by 
		$$\int_E f \dmu \defeq \int_X f \chi_E \dmu.$$
	\end{defn}

	\begin{ex} \lex{ex:integration_nonnegative_functions:0.005}
		Let $(X, \MA, \mu)$ be a measure space, $f \in L^+(X, \MA)$ and $E \in \MA$. Then 
		$$\int_E f \dmu = \int f \dmu_E$$ 
	\end{ex}
	
	\begin{proof}\
		\begin{itemize}
			\item Suppose that $f \in S^+(X, \MA)$. Write $f = \sum\limits_{j \in [n]} a_j \chi_{E_j} $ \tcr{in standard representation}. Then 
			\begin{align*}
				\int_E f \dmu 
				& = \int_X f \chi_E \dmu \quad \text{(by \rd{def:integration_nonnegative_functions:0.004})} \\ 
				& = \int_X \bigg[ \sum\limits_{j \in [n]}  a_j \chi_{E_j \cap E} \bigg] \dmu \quad \text{(by \tcr{make/ref prev ex})} \\
				& = \sum\limits_{j \in [n]} a_j \mu(E_j \cap E) \\
				& = \sum\limits_{j \in [n]} a_j \mu_E(E_j) \\
				& = \int_X \bigg[ \sum\limits_{j \in [n]} a_j \chi_{E_j} \bigg] \dmu_E \\
				& = \int_X f \dmu_E.
			\end{align*}
			\item Suppose that $f \in L^+(X, \MA)$. \tcr{prev ex} implies that there exist $(\phi_n)_{n \in \N} \subset S^+(X, \MA)$ such that $\phi_n \convt{p.w.} f$ and for each $n \in \N$, $\phi_n \leq \phi_{n+1}$. Thus
			\begin{align*}
				\int_E f \dmu 
				& = \int_E \bigg[ \sup\limits_{n \in \N} \phi_n \bigg] \dmu \\
				& = \sup\limits_{n \in \N} \int_E \phi_n \dmu \quad \text{(by monotone convergence)} \\
				& = \sup\limits_{n \in \N} \int_X \phi_n \dmu_E \quad \text{(by prev part)} \\
				& = \int_X \bigg[ \sup\limits_{n \in \N} \phi_n \bigg] \dmu_E \text{(by monotone convergence)} \\
				& = \int_X f \dmu_E.
			\end{align*}
		\end{itemize}
	\end{proof}	
	...
	\begin{ex} \lex{ex:integration_nonnegative_functions:0003}
		Let $\mu_1, \mu_2 \in M_+(X,\MA)$. Suppose that $\mu_1 \leq \mu_2$. Then for each $f \in L^+$, 
		$$\int f d\mu_1 \leq \int f d\mu_2.$$
	\end{ex}
	
	\begin{proof}
		\item Suppose that $f \in S^+(X, \MA)$. Then there exist $(a_n)_{i=1}^n \subset \Rg$ and $(E_i)_{i=1}^n \subset \MA$ such that $f = \sum\limits_{i =1}^n a_i \chi_{E_i}$. Then 
		\begin{align*}
			\int f d\mu_1 
		...
		\item Suppose that $f \in L^+(X, \MA)$. Then \tcr{the prev part implies that}
		\begin{align*}
			\int f d\mu_1 
			&= \sup\limits_{\substack{s \in S^+\\s \leq f}} \int s \dmu_1 \\
			& \leq \sup\limits_{\substack{s \in S^+\\s \leq f}} \int s d\mu_2 \\
			&= \int f d\mu_2
		\end{align*}
	\end{proof}
	...
	\begin{thm} \lex{ex:integration_nonnegative_functions:0005}
		Let $(f_n)_{n \in \N} \subset L^+$. Then 
		$$\int \sum\limits_{n \in \N} f_n= \sum\limits_{n \in \N} \int f_n.$$
	\end{thm}
	...
	\begin{proof}
		...
		\begin{align*}
			...
			&= \infty \text{, a contradiction.}
		\end{align*}
		Thus for each $n \in \N$, $\mu(S_n) < \infty$ and $S$ is $\sig$-finite.
	\end{proof}
	
	\begin{ex}  \lex{ex:integration_nonnegative_functions:0006}
		Let $\mu \in M_+(X, \MA)$ and $f \in L^+(X, \MA)$. Then $f = 0$ $\mu$-a.e. iff for each $E \in \MA$, 
		$$\int_E f \dmu = 0.$$
	\end{ex}
	...
	\begin{ex} \lex{ex:integration_nonnegative_functions:0008} 
		...
		$$\int f \dsh =  \sup \bigg \{\sum\limits_{x \in F}f(x): F \subset X \text{ and } \#(F) < \infty \bigg \}$$
	\end{ex} 

	\begin{proof}
		Define $A_1, A_2 \subset \RG$ by 
		$$A_1 = \bigg \{\int \phi \dsh: \phi \in S^+(X, \MA) \text{ and } \phi \leq f \bigg \}, \quad A_2 = \bigg \{\sum\limits_{x \in F}f(x): F \subset X \text{ and } \#(F) < \infty \bigg \}$$ 
		\begin{itemize}
			...
			\begin{itemize}
				...
				\begin{align*}
					a_{j_0} k
					& = \sum\limits_{x \in F_k} \phi(x) \\
					& \leq \sum\limits_{x \in F_k}f(x) \\
					& \in A_2
				\end{align*}
				which implies that
				\begin{align*}
					\sup A_2
					& \geq \sup\limits_{k \in \N}  \sum\limits_{x \in F_k}f(x) \\
					& \geq \sup\limits_{k \in \N} a_{j_0} k
				\end{align*}
				...
				\begin{align*}
					...
					& = \int \phi \dsh \\
					& = \sum\limits_{j =1}^n a_j \#(E_j) \\
					& = \sum\limits_{j \in J} a_j \#(E_j) \\
					& = \sum\limits_{j \in J} \sum\limits_{x \in E_j} \phi(x) \\
					& = \sum\limits_{x \in F} \phi(x) \\
					& \leq \sum\limits_{x \in F} f(x) \\
					& \leq \sup A_2 
				\end{align*}
			\end{itemize}
	...
	\begin{defn} \lex{def:integration_nonnegative_functions:0009.1} 
		Let $(X, \MA, \mu)$ be a measure space and $f \in L^+(X, \MA)$. We define the \tbf{indefinite integral of $f$ with respect to $\mu$}, denoted $\mu^f: \MA \rightarrow \RG$ by 
		$$\mu^f(E) \defeq \int_E f \dmu.$$
		\tcr{STANDARDIZE THIS NOTATION } \tcr{define and change notation from $\mu_f$ to $\mu^f(E) \defeq \int_E f \dmu$}. 
	\end{defn}

	\begin{note}
		\rex{arg1} implies that 
		$$\mu^f(E) = \int_X f \dmu_E.$$
	\end{note}
	
	\begin{ex} \lex{ex:integration_nonnegative_functions:0010} 
		Let $(X, \MA, \mu)$ be a measure space and $f \in L^+(X, \MA)$. Then 
		\begin{enumerate}
			\item $\mu^f \in M_+(X, \MA)$
			\item for each $E \in \MA$, 
			$$\mu^f(E) = \int_X f \dmu_E, $$
			\item for each $g \in L^+(X, \MA)$, 
			$$\int_X g \dmu^f = \int_X g f \dmu. $$
		\end{enumerate}
	\end{ex}
		...
		\begin{enumerate}
			...
			\begin{itemize}
				...
				\begin{align*}
					...
					&= \sum\limits_{i = 1} ^n a_i\mu\bigg(E_i \cap \bigg(\bigcup_{j \in \N} A_j\bigg)\bigg)\\
					&= \sum\limits_{i = 1} ^n a_i\mu\bigg(\bigcup_{j \in \N} E_i \cap A_j\bigg)\\
					&= \sum\limits_{i = 1} ^n a_i \sum\limits_{j \in \N} \mu(E_i \cap A_j)\\
					&= \sum\limits_{j \in \N} \sum\limits_{i = 1} ^n a_i \mu(E_i \cap A_j)\\
					&= \sum\limits_{j \in \N} \int_{A_j} f \dmu\\
					&= \sum\limits_{j \in \N} \mu^f(A_j)
				\end{align*}  
				...
				\begin{align*}
					...
					& = \limn \mu_n(A) \\
					& = \limn \sum\limits_{j \in \N} \mu_n(A_j)  \\
					& = \limn \sum\limits_{j \in \N} \int_{A_j} \phi_n \dmu\\
					& = \limn \sum\limits_{j \in \N} g_n(j) \\
					& = \limn \int g_n \, \dsh \\
					& = \int \limn g_n \, \dsh \hspace{4mm} \text{(by monotone convergence theorem)} \\ 
					& = \sum\limits_{j \in \N} \limn g_n(j) \\
					& = \sum\limits_{j \in \N} \limn \int_{A_j} \phi_n \dmu  \\
					& = \sum\limits_{j \in \N} \int_{A_j} f \dmu \\
					& = \sum\limits_{j \in \N} \mu^f(A_j).
				\end{align*} 
			\end{itemize}
			Hence $\mu^f$ is a measure on $(X, \MA)$.
			\item Clear by \rex{ex:integration_nonnegative_functions:0.005}.
			\item Let $g \in L^+$. 
			\begin{itemize}
				...
				\begin{align*}
					\int g d\lam 
					&= \sum\limits_{i=1}^n a_i \lam(E_i)\\
					&= \sum\limits_{i=1}^n a_i \int_{E_i} f \dmu\\
					&= \int \bigg(\sum\limits_{i=1}^n a_i\chi_{E_i} \bigg) f \dmu\\
					&= \int gf \dmu.
				\end{align*}
				...
				\end{align*}
			\end{itemize}
			Since $g \in L^+(X, \MA)$ is arbitrary, we have that for each $g \in L^+(X, \MA)$, 
			$$\int_X g \dmu^f = \int_X g f \dmu. $$
		\end{enumerate}
	\end{proof}	
	...
	\begin{proof}
		...
		\begin{align*}
 		...
 		\begin{align*}
 			...
 			& = \sum\limits_{j=1}^n a_j \int_{f^{-1}(B)} \chi_{E_j} \circ f \dmu \\
 			& = \sum\limits_{j=1}^n a_j \int_{B} \chi_{E_j} \, d f_*\mu \\
 			& = \int_{B} \sum\limits_{j=1}^n a_j  \chi_{E_j} \, d f_*\mu \\
 			& = \int_{B} g \, d f_*\mu \\
 		\end{align*}
	...
	\begin{ex} \lex{ex:integration_nonnegative_functions:0012.1}
		...
		$$f_*(\mu^{f^*g}) = (f_*\mu)^g.$$
		\tcr{DEFINE $f^*g \defeq g \circ f$ in set theory section}
	\end{ex}

	\begin{proof}
		Let $B \in \MB$. \rex{ex:integration_nonnegative_functions:0012} implies that
		\begin{align*}
			f_*(\mu^{f^*g})(B)
			& = \mu^{f^*g}(f^{-1}(B)) \\
			& = \int_{f^{-1}(B)} g \circ f \dmu \\
	...
	\begin{itemize}
		\item \tcr{Show that if $f: Y \rightarrow \C$ is $\MB$-measurable, then $\pi^*f:X \rightarrow \C$ (i.e. $f \circ \pi$) is $\pi^*\MB$-measurable and
		$$(\pi_*\mu)^f) = (\pi_* \mu)^{\pi^* f}$$  }
	\end{itemize}
	...
	\tcr{need to define 
		$$\ML^1(X, \MA, \mu) \defeq \bigg \{f:X \rightarrow \C : f \text{ is measurable and } \int |f| < \infty \bigg \}$$ and $\sim_{\mu}$ on $\ML^1(X, \MA, \mu)$ by $f \sim_{\mu} g$ iff $f = g$ $\mu$-a.e. Then define $L^1(X, \MA, \mu) \defeq \ML(X, \MA, \mu)/ \sim_{\mu}$}
	...
	\begin{ex} \lex{ex:integration_complex_functions:00006.1}  \tbf{Uniform Convergence Theorem:} \\
		Let $(X, \MA, \mu)$ be a measure space, $(f_n)_{n \in \N} \subset \ML^1(X, \MA)$ and $f \in \ML^0(X, \MA)$. Suppose that $\mu$ is finite. If $f_n \convt{unif} f$, then $f \in \ML^1(X, \MA, \mu)$ and $f_n \conv{\ML^1} f$.
		\tcr{need to define $\ML^0(X, \MA) \defeq \text{measurable functions $X \rightarrow \C$}$ and $\ML^k(X, \MA, \mu) \defeq \pi_{=_{\text{$\mu$-a.e.}}}^{-1}(L^k(X, \MA, \mu))$ or something equivalent and talk about pseudo metrics.}
	\end{ex}
	
	\begin{proof}
		Suppose that $f_n \convt{unif} f$. Let $\ep > 0$. Then there exists $N \in \N$ such that for each $n \in \N$, $n \geq N$ implies that $|f_n - f| < \ep/(\mu(X) + 1)$. 
		\begin{itemize}
			\item Therefore, for each $n \in \N$, $n \geq N$ implies that 
			\begin{align*}
				\int_X |f_n - f| \dmu 
				& \leq \int_X \frac{\ep}{\mu(X) + 1} \dmu \\
				& = \frac{\ep}{\mu(X) + 1} \mu(X) \\
				& < \ep.
			\end{align*}
			\item Since $|f| \leq |f - f_N| + |f_N|$, we have that
			\begin{align*}
				\int_X |f| \dmu 
				& \leq \int_X |f - f_N| + |f_N| \dmu \\
				& = \int_X |f - f_N| \dmu  + \int_X |f_N| \dmu \\
				& < \ep + \int_X |f_N| \dmu \\
				& < \infty \quad \text{(since $f_N \in \ML^1(X, \MA, \mu)$)} 
			\end{align*}
			and $f \in \ML^1(X, \MA, \mu)$.  
			\item Since $\ep > 0$ is arbitrary, we have that for each $\ep >0$, there exists $N \in \N$ such that for each $n \in \N$, $n \geq N$ implies that 
			$$\int_X |f_n - f| \dmu < \ep.$$
			Thus $f_n \conv{\ML^1} f$.
		\end{itemize}
	\end{proof}
	...
	\begin{ex} \lex{ex:integration_complex_functions:00008}
		Let $(f_n)_{n \in \N} \subset L^1$. Suppose that $$\sum\limits_{n \in \N} \int |f_n| < \infty.$$ Then after redefinition on a set of measure zero, $\sum\limits_{n \in \N}f_n \in L^1$ and $$\int \sum\limits_{n \in \N}f_n = \sum\limits_{n \in \N} \int f_n$$
	\end{ex}
	...
	\begin{ex} \lex{ex:integration_complex_functions:00014} 
		...
		$$g(x) = \sum\limits_{n \in \N}2^{-n}f(x -r_n).$$
		Then 
		\begin{enumerate}
	...
	\begin{proof} For convenience, define $f_n: \R \rightarrow \Rg$ by $f_n(x) = f(x-r_n)$ for $x \in \R$.
		...
		\end{align*}
		Hence 
		$$\sum\limits_{n \in \N} \int |2^{-n} f_n| = 2 < \infty.$$
		Therefore after redefinition on a null set, $g \in L^1.$ In particular $\int |g| < \infty$ and so $|g|$ (and hence $g$) are finite almost everywhere. For $(2)$, since $g < \infty$ a.e., so too is $g^{2}$. Let $a,b \in \R$ and suppose that $a<b$. Choose $N \in \N$ such that $r_N \in (a,b)$. Since all the terms in the sum are nonnegative, $g^{2} \geq \sum\limits_{n \in \N} 2^{-2n}f_n^2$ and so 
		\begin{align*}
			\int_{(a,b)} g^2 
			&\geq \int_{(a,b)} \sum\limits_{n \in \N} 2^{-2n}f_n^2\\
			&= \sum\limits_{n \in \N} 2^{-2n} \int_{(a,b)} f_n^2\\
	...
	\begin{proof}
		First assume that $f$ is simple. Then there exist $(a_j)_{j=1}^n \subset \C$ and $(E_j)_{j=1}^n \subset \MP(X)$ such that $(E_j)_{j=1}^n$ is disjoint and $f = \sum\limits_{i = 1}^n a_i\chi_{E_i}$. Choose $j^* \in \{1, \ldots, n\}$ such that $x \in E_{j^*}$. Thus 
		\begin{align*}
		\int f d\del_x 
		&= \int \sum\limits_{j=1}^n c_j \chi_{E_j} d \del_x \\
		&= \sum\limits_{j=1}^n c_j \del_x(E_j) \\
		&= c_j \del_x(E_{j^*}) \\
		&= c_j \\
		&= f(x)
		\end{align*} 
	...
	\begin{ex} \lex{ex:integration_complex_functions:00022} \tcb{Change notation or define categories}
		Let $(X, \MA, \mu), (Y, \MB, \nu) \in \Obj(\bMsr)$ and $f \in \Hom_{\bMsr}((X, \MA), (Y, \MB))$. Then $f$ is measure preserving iff for each $\phi \in L^1(Y, \MB, \nu)$, $\phi \circ f \in L^1(X, \MA, \mu)$ and 
	...
	Let $(X, \MA, \mu)$ be a measure space. Define $\| \cdot \|_*: L^1(\mu) \rightarrow \Rg$ by  
	 $$\| f \|_* = \sup\limits_{A \in \MA} \bigg | \int_A f \dmu \bigg |$$ 
	\end{defn}
	...
	\begin{proof}\
		\begin{enumerate}
			\item First suppose that $f$, $g$ are simple. Then there exist $(A_i)_{i=1}^n \subset \MA$, $(B_j)_{j=1}^m \subset \MB$ and $(a_i)_{i=1}^n, (b_i)_{j=1}^m \subset \C$ such that $f = \sum\limits_{i=1}^n a_i \chi_{A_i}$ and $g = \sum\limits_{j=1}^m b_j \chi_{B_j}$. Then $h = \sum\limits_{i=1}^n \sum\limits_{j=1}^m a_i b_j \chi_{A_i \times B_j}$. So $h$ is $\MA \otimes \MB$-measurable. For general $f,g$, there exist $(f_n)_{n \in \N} \subset S(X, \MA)$ and $(g_n)_{n \in \N} \subset S(Y, \MB)$ such that $f_n \rightarrow f$ pointwise, $g_n \rightarrow g$ pointwise and for each $n \in \N$, $|f_n| \leq |f_{n+1}| \leq |f|$ and $|g_n| \leq |g_{n+1}| \leq |g|$. For $n \in \N$, define $h_n \in S(X \times Y, \MA \otimes \MB)$ by $h_n = f_n g_n$. Then $h_n \rightarrow h$ pointwise and for each $n \in \N$, $|h_n| \leq |h_{n+1}| \leq |h|$. Thus $h$ is $\MA \otimes \MB$-measurable.
			
			\item First suppose $f$ and $g$ are simple as before. Then  
			\begin{align*}
				\int_{X \times Y} |h| \dmu \otimes \nu 
				& \leq \sum\limits_{i=1}^n \sum\limits_{j=1}^m |a_i b_j| \mu(A_i) \nu(B_j)\\ 
				&= \big(\sum\limits_{i=1}^n |a_i| \mu(A_i) \big) \big( \sum\limits_{j=1}^m |b_j| \nu(B_j) \big)\\
			...
			\begin{align*}
				\int_{X \times Y} h \dmu \otimes \nu 
				&= \sum\limits_{i=1}^n \sum\limits_{j=1}^m a_i b_j \mu(A_i) \nu(B_j)\\ 
				&= \big(\sum\limits_{i=1}^n a_i \mu(A_i) \big) \big( \sum\limits_{j=1}^m b_j \nu(B_j) \big)\\
	...
	& \leq \sum\limits_{k \in \N} \mu(B_k) \\
	& \leq  \sum\limits_{k \in \N}\mu(A_{f,N_k,2k}) + \sum\limits_{k \in \N}\mu(A_{g, N_k, 2k}) \\
	&\leq  \sum\limits_{k \in \N} \ep 2^{-(1+k)} + \sum\limits_{k \in \N} \ep 2^{-(1+k)} \\
	&= \frac{\ep}{2} + \frac{\ep}{2} \\
	&= \ep
\end{align*}	 
	...
	\begin{align*}
	\mu(F_k) 
	&\leq \sum\limits_{j \geq k}2^{-j} \\
	&= 2^{1-k}\sum\limits_{j \geq 1}2^{-j}  \\
	&= 2^{1-k}
	\end{align*}
	Let $i, j \in \N$. Suppose that $i \geq j \geq k$. Let $x \in F_k^c$. Then 
	\begin{align*}
	|f_{n_i}(x) - f_{n_j}(x)| 
	& \leq \sum\limits_{l = j}^{i-1} |f_{n_{l+1}}(x) - f_{n_l}(x)| \\
	& < \sum\limits_{l = j}^{i-1} 2^{-l} \\
	& <  \sum\limits_{l \geq j} 2^{-l} \\
	&= 2^{1 - j} \\
	& \leq 2^{1 - k}	 
	\end{align*}
...
\end{align*}		
	...
	\begin{proof}\
		\begin{enumerate}
			...
			\begin{align*}
				...
				& = 0
			\end{align*}
			By construction, $f \chi_N = \sup\limits_{k \in \N} f_{n_k} \chi_N$ which implies that for each $n \in \N$, 
			\begin{align*}
				f_n \chi_N
				& \leq f_{n_n} \chi_N \\ 
				& \leq f \chi_N
	...
	\begin{ex} \lex{35010}  
		...
		\begin{enumerate}
			...
			\item Suppose that $\phi$ is uniformly continuous.
			\begin{itemize}
				\item If $f_n \convt{unif} f$ almost uniformly or in measure, then $\phi \circ f_n \convt{unif} \phi \circ f$. \tcr{move this part to analysis notes}
				\item If $f_n \convt{a.u.} f$, then $\phi \circ f_n \convt{a.u.} \phi \circ f$.
				\item If $f_n \conv{\mu} f$, then $\phi \circ f_n \conv{\mu} \phi \circ f$.
			\end{itemize}
			\item Find a counter example to (2) if $\phi$ is not uniformly continuous.
		\end{enumerate} 
	\end{ex}
	...
	\begin{ex} \lex{35015} 
		Let $(f_n)_{n \in \N} L^0$ and $f \in L^0$. Suppose that for each $\ep >0$, $$\sum\limits_{n \in \N}\mu(\{x \in X: |f_n(x)-f(x)| > \ep\}) < \infty$$
	...
	\begin{proof}
		...
		\begin{align*}
			\int \bigg[ \sum\limits_{n \in \N}\chi_{\{x \in X: |f_n(x)-f(x)| > \ep\}}\bigg] \dmu 
			&= \sum\limits_{n \in \N}\int \chi_{\{x \in X: |f_n(x)-f(x)| > \ep\}}\dmu\\
			&=\sum\limits_{n \in \N}\mu(\{x \in X: |f_n(x)-f(x)| > \ep\})\\
			& < \infty
		\end{align*}
		Thus we also know that $\sum\limits_{n \in \N}\chi_{\{x \in X: |f_n(x)-f(x)| > \ep\}} < \infty$ a.e. Equivalently, we could say that for a.e. $x \in X$, $|\{n \in \N: f_n(x) - f(x) > \ep\}| < \infty$. For $k \in \N$, define $N_k = \{x \in X: \sum\limits_{n \in \N}\chi_{\{x \in X: |f_n(x)-f(x)| > 1/k\}} = \infty\}$. Then for each $k \in \N$, $\mu(N_k) = 0$. Define $N = \bigcup_{k \in \N} N_k$. Then $\mu(N) = 0$. Let $x \in N^c$ and $\ep > 0$. Choose $k \in \N$ such that $1/k < \ep$. Then $\{n \in \N: f_n(x) - f(x) > \ep\} \subset \{n \in \N: f_n(x) - f(x) > 1/k\}$ which is finite because $x \in N_k^c$. Put $M = \max\{n \in \N: f_n(x) - f(x) > \ep\}$. Then for $m \geq M$, $|f_m(x) - f(x) \leq \ep|$. Thus $f_n(x) \rightarrow f(x)$. Hence $f_n \convt{a.e.} f$.
	\end{proof}
	...
	\begin{proof}
		...
		\begin{align*}
			\nu(A')
			& = \sup\limits_{n \in \N} \nu(A' \cap E_n) \\
			& \leq \sup\limits_{n \in \N} \bigg[ \sum\limits_{j \in \N} \nu_{\mu}^{\perp}(A'_j \cap E_n) \bigg] \\
			& = 0
		\end{align*}
	...
	\begin{proof}
		The previous exercise tells us that $|\nu_1 + \nu_2| \leq |\nu_1| + |\nu_2|$. \tcr{Two previous exercises from the section on nonnegative functions} imply that 
		\begin{align*}
			\int |f|d |\nu_1 + \nu_2| 
			& \leq \int |f| d(|\nu_1|+|\nu_2|)\\
			&= \int |f|d |\nu_1| + \int |f| d|\nu_2|
	...
	\begin{proof}
		...
		\begin{enumerate}
			...
			\begin{align*}
				...
				&= \sum\limits_{n \in \N} \int_{ E_n} f^+ \dmu - \sum\limits_{n \in \N} \int_{E_n} f^- \dmu\\
				&= \sum\limits_{n \in \N} \bigg[ \int_{ E_n} f^+ \dmu - \int_{ E_n} f^- \dmu \bigg]\\
				&= \sum\limits_{n \in \N} \int_{ E_n} f \dmu\\
				&= \sum\limits_{n \in \N} \nu(E_n)
			\end{align*}
			...
			\begin{align*}
				\sum\limits_{n \in \N} |\nu(E_n)|
				&=  \sum\limits_{n \in \N} \bigg|\int_{E_n} f \dmu \bigg|\\
				&= \sum\limits_{n \in \N} \bigg| \int_{E_n} f^+ \dmu -  \int_{E_n} f^- \dmu \bigg|\\
				& \leq \sum\limits_{n \in \N}  \int_{E_n} f^+ \dmu  + \sum\limits_{n \in \N}  \int_{E_n} f^- \dmu \\
				&= \int_{\bigcup_{n \in \N} E_n} f^+ \dmu + \int_{\bigcup_{n \in \N} E_n} f^- \dmu\\
				& < \infty
			\end{align*}
			So the sum $\sum\limits_{n \in \N} \nu(E_n)$ converges absolutely and $\nu$ is a signed measure. 
	...
	\begin{ex} \lex{00000} 
		...
		\begin{enumerate}
			\item If for each $n \in \N$, $\nu_n \ll \mu$, then $\sum\limits_{n \in \N} \nu_n \ll \mu$. 
			\item If for each $n \in \N$, $\nu_n \perp \mu$, then $\sum\limits_{n \in \N} \nu_n \perp \mu$.
		\end{enumerate} 
	\end{ex}
	
	\begin{proof}
		\begin{enumerate}
			\item Let $E \in \MA$. Suppose that $\mu(E) = 0$. Then for each $n \in \N$, $\nu_i(E) = 0$ and thus $\sum\limits_{n \in \N} \nu_n(E) = 0$. Hence $\sum\limits_{n \in \N} \nu_n \ll \mu$.
			\item For each $n \in \N$, there exist $N_i, M_i \in \MA$ such that $N_i \cap M_i = \varnothing$, $N_i \cup M_i = X$ and $\nu_i(M_i) = \mu(N_i) = 0$. Put $N = \bigcup_{n \in \N} N_i$ and $M = N^c$. Note that for each $n \in \N$, $M \subset N_i^c = M_i$. So $\mu(N) \leq \sum\limits_{n \in \N} \mu(N_i) = 0$ and $(\sum\limits_{n \in \N} \nu_i) (M) \leq \sum\limits_{n \in \N} \nu_i(M_i) = 0$. Thus $\sum\limits_{n \in \N} \nu_i \perp \mu$.
		\end{enumerate}
	\end{proof}
	...
	\begin{proof}
		\begin{enumerate}
			...
			\begin{align*}
				1
				&= m(X \setminus Z) \\
				&= \sum\limits_{x \in X \setminus Z} f(x)\\
				&= 0
			\end{align*}
	...
	\begin{proof}
		...
		\begin{enumerate}
			...
			\begin{align*}
				...
				& = \sum\limits_{n \in \N} \mu(E_n) + \lam \sum\limits_{n \in \N} \nu(E_n) \\
				& = \sum\limits_{n \in \N} \mu(E_n) + \sum\limits_{n \in \N} \lam \nu(E_n) \\
				& = \sum\limits_{n \in \N}  [\mu(E_n)  + \lam \nu(E_n)] \\
				& = \sum\limits_{n \in \N} [\mu + \lam \nu](E_n) \\
			\end{align*}
			and 
			\begin{align*}
				\sum\limits_{n \in \N} | (\mu + \lam \nu)(E_n) |
				& = \sum\limits_{n \in \N} | \mu(E_n)  + \lam \nu(E_n) | \\
				& \leq \sum\limits_{n \in \N}  | \mu(E_n) |  + |\lam| | \nu(E_n) | \\
				& \leq \sum\limits_{n \in \N}  | \mu(E_n) |  + |\lam|  \sum\limits_{n \in \N} | \nu(E_n) | \\
				& < \infty 
			\end{align*}
	...
	\begin{ex} \lex{ex:complex_measures:0004} 
		Let $(X, \MA)$ be a measurable space, $\nu \in M(X, \MA)$ and $\nu_1, \nu_2 :\MA \rightarrow \R$. If $\nu = \nu_1 + i \nu_2$, then $\nu_1, \nu_2$ are signed measures \tcr{come up with notation for signed measures}.
	\end{ex}
	...
	\begin{proof}
		...
		\begin{enumerate}
			...
			\begin{align*}
				\sum\limits_{n \in \N} |\nu_1 (E_n)|, \sum\limits_{n \in \N} |\nu_2 (E_n)|
				& \leq \sum\limits_{n \in \N} |\nu_1 (E_n)| + |\nu_2 (E_n)|  \\ 
				& \leq \sum\limits_{n \in \N} C |\nu_1 (E_n) +  i \nu_2 (E_n)| \\ 
				& = C \sum\limits_{n \in \N}  |\nu (E_n) |  \\ 
				& < \infty 
			\end{align*}
		\end{enumerate}
		Therefore $\nu_1$ and $\nu_2$ are signed measures \tcr{notation}.
	\end{proof}
	...
	\begin{proof}
		Suppose that for each $A \in \MP$, $\mu(A) = \nu(A)$. \rex{ex:complex_measures:0005} implies that $\ML_{\mu, \nu}$ is a $\lam$-system on $X$. By assumtion, $\MP \subset \ML_{\mu, \nu}$. Dynkin's theorem (\rex{ex:measurable_spaces:dynkins_lemma:0006}) then implies that $\sig(\MP) \subset \ML_{\mu, \nu}$. Thus for each $A \in \sig(\MP)$, $\mu(A) = \nu(A)$.
	\end{proof}
	...
	\begin{proof}
		Since $\MT \subset \MB(X)$ is a $\pi$-system on $X$ \tcr{(cite previous exercise)} and $X \in \MT$, \rex{ex:complex_measures:0006} implies that for each $A \in \sig(\MT)$, $\mu(A) = \nu(A)$. Since $\sig(\MT) = \MB(X)$, $\mu = \nu$. 
	\end{proof}

	\begin{ex} \lex{ex:complex_measures:0007.1} 
		Let $(X, \MT)$ be a topological space, $\ME \subset \MT$ a basis for $\MT$ and $\mu, \nu \in M(X)$. Suppose that $\ME$ is countable and $X \in \ME$. If for each $A \in \pi(\ME)$, $\mu(A) = \nu(A)$, then $\mu = \nu$. 
	\end{ex}
	
	\begin{proof}
		Suppose that for each $A \in \pi(\ME)$, $\mu(A) = \nu(A)$. Since $\ME$ is countable, \rex{ex:measurable_spaces:dynkins_lemma:0007.01} implies that $\pi(\ME)$ is countable. Since $X \in \ME$, we have that $X \in \pi(\ME)$. Since $\ME \subset \MT$ and $\MT \in \Pi(X)$, we have that $\pi(\ME) \subset \MT$. Since $\ME$ is a basis for $\MT$ and 
		\begin{align*}
			\ME
			& \subset \pi(\ME) \\
			& \subset \MT,
		\end{align*}
		\tcr{make/ref ex in analysis top notes} implies that $\pi(\ME)$ is a basis for $\MT$. \rex{ex:measurable_spaces:sigma_algebras:0008} then implies that $\MB(X) = \sig(\pi(\ME))$. Since $\pi(\ME) \in \Pi(X)$, $X \in \pi(\ME)$, and for each $A \in \pi(\ME)$, $\mu(A) = \nu(A)$, \rex{ex:complex_measures:0006} implies that $\mu = \nu$.
	\end{proof}
	
	\begin{ex} \lex{ex:complex_measures:0007.2} 
		Let $(X, \MT)$ be a topological space, $\ME \subset \MT$ a basis for $\MT$, $(\mu_x)_{x \in X}, (\nu_x)_{x \in X} \nu \in M(X)$ and $\lam \in M_+(X)$. Suppose that $\ME$ is countable and $X \in \ME$. If for each $E \in \pi(\ME)$, $(\mu_x(E))_{x \in X} =_{\text{$\lam$-a.e.}} (\nu_x(E))_{x \in X}$, then $(\mu_x)_{x \in X} = (\nu_x)_{x \in X}$ $\lam$-a.e. \tcr{make a note or ex in beginning measure sections that $\forall$ does not generally commute with a.e. quantification, i.e. 
		$$\forall E \in \MA [ \forall_{\lam-\text{a.e.}} x \in X (\mu_x(E) = \nu_x(E))] \not \iff \forall_{\lam-\text{a.e.}} x \in X [\forall E \in \MA (\mu_x(E) = \nu_x(E))]$$
		try $\mu, \lam \defeq \del_x$ and $\nu \defeq m$ on $\R$ or maybe $\lam = \sum\limits_{x \in \text{dense countable subset of $\R$}} \del_x$}
	\end{ex}

	\begin{proof}
		Suppose that for each $E \in \pi(\ME)$, $\mu_x(E) = \nu_x(E)$ $\lam$-a.e.
		\begin{itemize}
			\item By definition, $\pi(\ME) \in \Pi(X)$. Since $X \in \ME$, we have that $X \in \pi(\ME)$. \rex{ex:measurable_spaces:dynkins_lemma:0007.01} implies that $\pi(\ME)$ is countable.
			Since $\ME \subset \MT$ and 
			\begin{align*}
				\tau(\ME)
				& \in \Top_X \\
				& \subset \Pi(X),
			...
			\begin{align*}
				...
				& \subset \pi(\ME) \\
				& \subset \tau(\ME) \\
				& = \MT \quad \text{(since $\ME$ is a basis for $\MT$)}.
			\end{align*}
			\tcr{make/ref ex in analysis top notes} implies that $\pi(\ME)$ is a basis for $\MT$. 
			\item Since $\ME$ is countable, \rex{ex:measurable_spaces:dynkins_lemma:0007.01} implies that $\pi(\ME)$ is countable. By assumption, \tcr{the axiom of choice or ref ex in set theory analysis notes section} implies that there exists $(N_A)_{A \in \pi(\ME)} \in \prod\limits_{A \in \pi(\ME)} \{N \in \MB(X): \text{$\lam(N) = 0$ and for each $x \in N_A^c$, $\mu_x(A) = \nu_x(A)$} \}$. Define $N \in \MB(X)$ by $N \defeq \bigcup\limits_{A \in \pi(\ME)} N_A$. Since $\pi(\ME)$ is countable, we have that  
			\begin{align*}
				\lam(N) 
				& = \lam \bigg( \bigcup\limits_{A \in \pi(\ME)} N_A \bigg) \\
				& \leq \sum\limits_{A \in \pi(\ME)} \lam(N_A) \\
				& = 0.
			\end{align*}
			Let $x \in N^c$. Then  
			\begin{align*}
				x 
				& \in N^c \\
				& = \bigg( \bigcup\limits_{A \in \pi(\ME)} N_A \bigg)^c \\
				& = \bigcap\limits_{A \in \pi(\ME)} N_A^c. 
			\end{align*}
			Thus for each $A \in \pi(\ME)$, $x \in N_A^c$ and $\mu_x(A) = \nu_x(A)$. \rex{ex:complex_measures:0007.1} then implies that $\mu_x = \nu_x$. Since $x \in N^c$ is arbitrary, we have that for each $x \in N^c$, $\mu_x = \nu_x$. Thus $\mu_x = \nu_x$ $\lam$-a.e.
		\end{itemize}
	\end{proof}

	\begin{defn} \lex{def:complex_measures:000.2} 
		Let $(X, \MA)$ be a measurable space, $\mu \in M(X, \MA)$ and $f \in L^1(X, \MA, \mu)$. We define the \tbf{indefinite integral of $f$ with respect to $\mu$}, denoted $\mu^f: \MA \rightarrow \C$, by 
		$$ \mu^f(E) = \int_E f \dmu $$
	\end{defn}

	\begin{ex} \lex{ex:complex_measures:0009} 
		Let $(X, \MA)$ be a measurable space, $\mu \in M(X, \MA)$ and $f \in L^1(X, \MA, \mu)$. Then $\mu^f \in M(X, \MA)$.
	\end{ex}
	...
	Let $(X,\MA)$ be a measurable space and $\nu \in M(X,\MA)$. Write $\nu = \nu_1 + i \nu_2$ with $\nu_1, \nu_2$ signed measures on $(X, \MA)$ \tcr{come up with notation for signed measures}. Define $\mu = |\nu_1| + |\nu_2|$. Then $\nu \ll \mu$ and thus there exists $f \in L^1(\mu)$ such that $d\nu = f \dmu$. We define the \textbf{total variation of $\nu$}, denoted $|\nu|: \MA \rightarrow \Rg$, by 
	$$|\nu|(E) = \int_E |f|\dmu$$. 
	\end{defn}
	
	\tcr{The next exercise implies that the total variation is independent of representing Radon Nikodym decomposition}
	
	\begin{ex} \lex{ex:complex_measures:0013} 
	Let $(X,\MA)$ be a measurable space, $\nu \in M(X,\MA)$ and $\lam \in M_+(X, \MA)$. Suppose that $\lam$ is $\sig$-finite and $\nu \ll \lam$. Define $g \in L^1(\lam)$ by $g \defeq d\nu / d\lam$. Then for each $E \in \MA$, 
	$$|\nu|(E) = \int_E |g| \dlam$$
	\end{ex}
	
	\begin{proof}
	Write $\nu = \nu_1 + i \nu_2$. Then $\nu_1, \nu_2 \ll \lam$. Set $f_1 = d\nu_1 / d\lam$ and $f_2 = d\nu_2 / d\lam$. Then \rex{41016} implies that $d|\nu_1| = |f_1| \dlam$ and $d|\nu_2| = |f_2|d\lam$. Define $\mu \in M_+(X, \MA)$ by $\mu \defeq |\nu_1| + |\nu_2|$. Then $\nu \ll \mu$. Define $f \in L^1(\mu)$ by $f \defeq d\nu / \dmu$. Then by construction, 
	\begin{align*}
	d\mu
	&= d|\nu_1| + d|\nu_2| \\
	&= |f_1| \dlam + |f_2| \dlam \\
	&= (|f_1| + |f_2|) \dlam.
	\end{align*}
	Therefore $\mu \ll \lam$ and $d\mu / d \lam = |f_1| + |f_2|$. Then \rex{ex:complex_measures:0011} implies that $\nu \ll \lam$ and
	\begin{align*}
	g
	& = \frac{d\nu}{d\lam} \quad \text{(by defintion)} \\
	& = \frac{d\nu}{d\mu} \frac{d\mu}{d\lam} \\
	& = f(|f_1|+|f_2|).
	\end{align*}
	Hence for each $E \in \MA$, 
	\begin{align*}
	|\nu|(E) 
	&= \int_E |f| \dmu \\
	...
	\begin{proof}
		Suppose that If $\nu \ll \mu$. Define $f \in L^1(\mu)$ and $E \in \MA$ by $f \defeq d\nu / \dmu$ and $E \defeq \{x: f(x) = 0\}$ \tcr{(Is $E \in \MA?$ if we only know $f$ almost everywhere?, need to emphasize that there exists $f \in L^0 \cap L^1$)}. Let $A \in \MA$ and suppose that $A \subset E$. Then 
		\begin{align*}
			\nu(A) 
			&= \int_A f \dmu\\
			&= 0
	...
	\begin{ex} \lex{ex:complex_measures:0015} 
		Let $(X, \MA)$ be a measurable space. 
		\begin{enumerate}
			\item Let $\nu \in M(X, \MA)$. Write $\nu = \nu_1 + i\nu_2$ with $\nu_1, \nu_2$ signed measures on $(X, \MA)$. Then $|\nu_1|, |\nu_2| \leq |\nu| \leq |\nu_1| + |\nu_2|$.
			\item Let $\nu_1, \nu_2 \in M(X, \MA)$. Then $|\nu_1 + \nu_2| \leq |\nu_1| + |\nu_2|$.
		\end{enumerate}
	\end{ex}
	...
	\begin{proof}\
		\begin{enumerate}
			\item Let $\mu$ and $f = f_1 + i f_2$ be as in the definition of $|\nu|$. Since for each $E \in \MA$, 
			\begin{align*}
				\nu(E) 
				&= \int_E f \dmu\\
			...
			\begin{align*}
				...
				&\leq |\nu_1| + |\nu_2|\\
			\end{align*}
			\item \tcr{make/ref prev ex showing} there exists $\mu \in M_+(X, \MA)$ such that $\nu_1, \nu_2 \ll \mu$. Write $f_1 \defeq d \nu_1 / d \mu$ and $f_2 \defeq d \nu_2 / d \mu$. Then \tcr{prev ex} implies that $\nu_1 + \nu_2 = \mu^{f_1 + f_2}$. \tcr{ref prev ex aboutn independence of radon niko rep} implies that 
			\begin{align*}
				|\nu_1 + \nu_2|(E)
				& = \int_E |f_1 + f_2| \dmu \\
				& \leq \int_E |f_1| + |f_2| \dmu \\
				& = \int_E |f_1|  \dmu + \int_E |f_2| \dmu \\
				& = |\nu_1|(E) + |\nu_2|(E) \\
				& = (|\nu_1| + |\nu_2|)(E).
			\end{align*}
			Since $E \in \MA$ is arbitrary, we have that for each $E \in \MA$, $|\nu_1 + \nu_2|(E) \leq (|\nu_1| + |\nu_2|)(E)$. Hence $|\nu_1 + \nu_2| \leq |\nu_1| + |\nu_2|$.
		\end{enumerate}
	\end{proof}
	
	\begin{ex} \lex{ex:complex_measures:0016} 
		Let  $(X, \MA)$ be a measurable space, $\nu \in M(X, \MA)$ and $\al \in \C$. Then $| \al \nu | = | \al | | \nu |$.
	\end{ex}
	...
	Define $\|\cdot \|: M(X, \MA) \rightarrow \Rg$ by 
	$$\|\mu \| \defeq | \mu | (X)$$ 
	Then $\|\cdot \|$ is a norm on $M(X, \MA)$. 
	\end{ex}
	
	\begin{proof}
		Let $\mu, \nu \in M(X, \MA)$ and $\al \in \C$.
		\begin{enumerate}
			\item \rex{ex:complex_measures:0015} implies that $| \mu + \nu | \leq | \mu | + | \nu |$. Therefore 
			\begin{align*}
				\| \mu + \nu \| 
				& = | \mu + \nu |(X) \\
				& \leq (| \mu | + | \nu |)(X) \\
				...
				& = \| \mu  \|  + \| \nu \|.
			\end{align*}
			\item \rex{ex:complex_measures:0016} implies that $|\al \mu| = |\al| |\mu|$. Therefore
			\begin{align*}
				\|\al \mu\|
				& = |\al \mu|(X) \\
				& = (|\al| |\mu|)(X) \\
			...
			\begin{align*}
				\|\mu\| = 0
				& \iff |\mu|(X) = 0 \\
				& \iff |\mu| = 0  \quad \text{(by \tcr{make and ref ex})}
				& \iff \mu = 0  \quad \text{(by \tcr{make and ref ex, something about $X$ is $\mu$-null}) }. 
			\end{align*}
			Thus $\|\mu\| = 0$ implies that $\mu = 0$. 
		\end{enumerate}
		Hence $\|\cdot\|$ is a norm on $M(X, \MA)$.
	\end{proof}
	...
	Let $(X, \MA)$ be a measurable space, $\mu_1, \mu_2 \in M(X, \MA)$ and $\lam \in \C$. Then for each $f \in L^1(\mu_1 + \lam \mu_2)$, 
	$$\int f d(\mu_1 + \lam \mu_2) = \int f \dmu_1 + \lam \int f \dmu_2$$
	\end{ex}
	...
	\begin{defn} \ld{def:complex_measures:0008} 
		Let $(X, \MA)$ be a measurable space, $\mu \in M(X, \MA)$ and $A \in \MA$. Then $\mu$ is said to be \tbf{supported on $A$} if for each $E \in \MA$, $\mu(E) = \mu(E \cap A)$. \\
		\tcb{need to define for signed measures and measures}
	\end{defn}

	\begin{ex} \lex{ex:complex_measures:0008.1}
		Let $(X, \MA)$ be a measurable space, $\mu \in M(X, \MA)$ and $A \in \MA$. Then $\mu$ is supported on $A$ iff $|\mu|(A^c) = 0$.
	\end{ex}
	
	\begin{proof}\
		\begin{itemize}
			\item $(\implies)$: \\
			Suppose that $\mu$ is supported on $A$. Then 
			\begin{align*}
				|\mu|(A^c)
				& = |\mu|(A^c \cap A) \quad \text{(by \rd{def:complex_measures:0008})} \\
				& = |\mu|(\varnothing) \\
				& = 0. 			
			\end{align*}
			\item $(\impliedby)$: \\
			Suppose that $|\mu|(A^c) = 0$. Let $E \in \MA$. Since $E \cap A^c \subset A^c$, we have that
			\begin{align*}
				0
				& \leq |\mu(E \cap A^c)| \\
				& \leq |\mu|(E \cap A^c) \quad \text{(by \rex{def:complex_measures:0018})} \\
				& \leq |\mu|(A^c) \\
				& = 0.
			\end{align*}
			Hence $|\mu(E \cap A^c)| = 0$. Therefore $\mu(E \cap A^c) = 0$ and 
			\begin{align*}
				\mu(E)
				& = \mu(E \cap A) + \mu(E \cap A^c) \\
				& \mu(E \cap A). 
			\end{align*}
			Since $E \in \MA$ is arbitrary, we have that for each $E \in \MA$, $\mu(E) = \mu(E \cap A)$. Thus $\mu$ is supported on $A$. 
		\end{itemize}
	\end{proof}
	
	\begin{ex} \lex{ex:complex_measures:0008.01} 
		\tcr{maybe define 'supported on' for signed measures and move there?}
		Let $(X, \MA), (Y, \MB) \in \Obj(\bMeas)$ be measurable spaces, $\mu \in M_1(X, \MA)$, $\pi \in \Hom_{\bMeas}((X, \MA), (Y, \MB))$ and $y \in Y$. Then $\mu$ is supported on $\pi^{-1}(\{y\})$ iff $\pi_*\mu = \del_y$. 
	\end{ex}
	
	\begin{proof}
		Since $\mu \in M_1(X, \MA)$, \tcr{make/ref ex} implies that $\pi_* \mu \in M_1(Y, \MB)$.  
		\begin{itemize}
			\item $(\implies)$: \\
			Suppose that $\mu$ is supported on $\pi^{-1}(\{y\})$. Then \tcr{make/ref ex} implies that 
			\begin{align*}
				\pi_*\mu(\{y\}^c)
				& = \mu(\pi^{-1}(\{y\}^c)) \\
				& = \mu(\pi^{-1}(\{y\})^c) \\
			...
			Let $B \in \MB$. 
			\begin{itemize}
				\item Suppose that $y \in B$. Then $\{y\} \subset B$ and 
				\begin{align*}
					1
					& = \pi_*\mu(\{y\}) \\
					& \leq \pi_*\mu(B) \\
				...
				\end{align*}
				Hence $\pi_*\mu(B) = 1$.
				\item Suppose that $y \not \in B$. Then $y \in B^c$. Hence $\{y\} \subset B^c$ and $B \subset \{y\}^c$. Therefore 
				\begin{align*}
					0
					& \leq \pi_*\mu(B) \\
					& \leq \pi_*\mu(\{y\}^c) \\
				...
				Thus $\pi_*\mu(B) = 0$.  
			\end{itemize}
			Since $B \in \MB$ is arbitrary, we have that for each $B \in \MB$, 
			\begin{align*}
				\pi_*\mu(B)
				& = 
				\begin{cases}
			...
			Hence $\pi_*\mu = \del_y$.
			\item $(\impliedby)$: \\
			Suppose that $\pi_*\mu = \del_y$. Then 
			\begin{align*}
				\mu(\pi^{-1}(\{y\})^c) 
				& = \mu(\pi^{-1}(\{y\}^c)) \\
				& = \pi_* \mu(\{y\}^c) \\
				...
				& = 0.  
			\end{align*}
			\tcr{make/ref ex that a positive measure $\mu$ is supported on $A$ iff $\mu(A^c) = 0$} implies that $\mu$ is supported on $\pi^{-1}(\{y\})$.
		\end{itemize}
	\end{proof}
	...
	\begin{proof}\
		\begin{itemize}
			\item \tcr{(need to show if $f \sim_{\mu|_{\MB}} g$, then $f \sim_{\mu} g$, i.e. $L^1$ is set of equivalence classes, need to show this is respected for the inclusion)}
			\item Set $\mu_{\MB} = \mu|_{\MB}$. Let $f \in L^1(X, \MB, \mu_{\MB})$ and $B \in \MB$. Clearly $f$ is $\MA$-measurable. If $f$ is simple, then there exist $(b_i)_{i=1}^n \subset \Rg$ and $(B_i)_{i=1}^n \subset \MB$ such that $$f = \sum\limits_{i=1}^n b_i \chi_{B_i}$$ such that for each $i \in \{1, \cdots, n\}$, 
			\begin{align*}
				\infty 
				&> \mu_{\MB}(B_i) \\
				&= \mu(B_i)
			...
			\begin{align*}
				\int_B f \dmu_{\MB} 
				&= \int_B \sum\limits_{i=1}^n b_i \chi_{B_i} \dmu_{\MB} \\
				&= \sum\limits_{i=1}^n b_i \mu_{\MB}(B_i \cap B)\\
				&= \sum\limits_{i=1}^n b_i \mu(B_i \cap B)\\
				&= \int_B \sum\limits_{i=1}^n b_i\chi_{B_i} \dmu \\
				&= \int_B f \dmu
			\end{align*}
	...
	\begin{defn} \ld{00000} 
		Let $f \in \Ll$. We define its \textbf{Hardy-Littlewood maximal function}, denoted by $Hf$ to be $$Hf(x) = \sup\limits_{r>0} A|f|(x,r) \hspace{4mm} x \in \R^n$$
	\end{defn}
	...
	\begin{proof}
		...
		\begin{align*}
			F(y_1) - F(x) 
			&= \sum\limits_{j=1}^{N-1} \bigg[F(y_j)-F_+(y_{j+1}) + F_+(y_{j+1}) - F(y_{j+1}) \bigg] + F(y_N)- F(x)\\
			& = \sum\limits_{j=1}^{N-1} \bigg[F(y_j)-F_+(y_{j+1}) \bigg] + \sum\limits_{j=1}^{N-1} \bigg[ F_+(y_{j+1}) - F(y_{j+1}) \bigg] + F(y_N)- F(x) \\
			& \geq \sum\limits_{j=1}^{N-1} \bigg[ F_+(y_{j+1}) - F(y_{j+1}) \bigg]  \\
			& \geq (N-1)\ep \\
			& > F(y_1) - F(x)
		\end{align*}
	...
	\begin{defn} \ld{00000} 
		Let $F:\R \rightarrow \C$. Define $T_F:\R \rightarrow \R$ by $$T_F(x) = \sup\bigg \{\sum\limits_{i=1}^{n}|F(x_{i}) - F(x_{i-1})|: (x_i)_{i=0}^n \subset \R \text{ is increasing and } x_n=x  \bigg \} \hspace{4mm} (x \in \R)$$
	...
	\begin{proof}
		Let $x,y \in \R$. Suppose that $x<y_2$. \\Define  $A_x = \big \{\sum\limits_{i=1}^{n}|F(x_{i}) - F(x_{i-1})|: (x_i)_{i=0}^n \subset \R \text{ is increasing and } x_n=x  \big \}$ and \\$A_y = \big \{\sum\limits_{i=1}^{n}|F(x_{i}) - F(x_{i-1})|: (x_i)_{i=0}^n \subset \R \text{ is increasing and } x_n=y  \big \}$. Let $z \in A_x$. Then there exists $(x_i)_{i=0}^n \subset \R$ such that $(x_i)_{i=0}^n$ is increasing,\\ $x_n=x$ and $z = \sum\limits_{i=1}^n |F(x_{i}) - F(x_{i-1})|$. Then
		
		\begin{align*}
			z 
			& \leq z+|F(y)-F(x)|\\
			&= \sum\limits_{i=1}^n |F(x_{i}) - F(x_{i-1})| + |F(y)-F(x)|\\
			& \in A_y\\
		\end{align*} 
	...
	\begin{proof}
		Let $F:\R \rightarrow \C$, $x \in R$ and $(x_i)_{i=0}^n \subset \R$. Suppose that $(x_i)_{i=0}^n$ is increasing and $x_n=x$. Then by the reverse triangle inequality, $$ \sum\limits_{i=1}^n\big||F(x_i)|-|F(x_{i-1})|\big|
		\leq \sum\limits_{i=1}^n\big|F(x_i)-|F(x_{i-1})\big|$$
		Thus 
		\begin{align*}
			T_{|F|}(x) 
			&= \sup\bigg \{\sum\limits_{i=1}^{n}\big||F(x_{i})| - |F(x_{i-1})|\big|: (x_i)_{i=0}^n \subset \R \text{ is increasing and } x_n=x  \bigg \} \\
			& \leq \sup\bigg \{\sum\limits_{i=1}^{n}|F(x_{i}) - F(x_{i-1})|: (x_i)_{i=0}^n \subset \R \text{ is increasing and } x_n=x  \bigg \} \\
			&= T_F(x)
		\end{align*}
		Hence $T_{|F|} \leq T_F$
	\end{proof}
	...
	\begin{note}
		Equivalently, $\TV(F) = \sup \big \{\sum\limits_{i=1}^{n}|F(x_{i}) - F(x_{i-1})|: (x_i)_{i=0}^n \subset [a,b] \text{ is increasing, } x_0=a \text{, and } x_n=b\big \}$ and $F \in \BV(a,b)$ iff $\TV(F) < \infty$. In general, 
	\end{note}
	...
	\begin{proof}
		Suppose that $F$ is bounded and increasing. Then $-\infty<\inf_{x \in \R}F(x) \leq \sup\limits_{x \in \R}F(x)<\infty$. Let $x \in \R$ and $(x_i)_{i=0}^n \subset \R$. Suppose that $(x_i)_{i=0}^n$ is increasing and $x_n=x$. Then 
		\begin{align*}
			\sum\limits_{i=1}^n|F(x_i)-F(x_{i-1})| 
			&= \sum\limits_{i=1}^n F(x_i)-F(x_{i-1})\\
			&= F(x)-F(x_0)
		\end{align*}
		Thus 
		...
		\begin{align*}
			\TV(F) 
			&= \sup\limits_{x \in \R}F(x)-\inf_{x \in \R}F(x)\\
			&<\infty
		\end{align*}
		Hence $F \in \BV$.
	\end{proof}
	...
	\begin{proof}
		...
		\begin{align*}
			\sum\limits_{i=1}^n|F(x_i)-F(x_{i-1})|
			&= \sum\limits_{i=1}^n|F'(c_i)(x_i-x_{i-1})|\\
			&\leq  \sum\limits_{i=1}^nM(x_i-x_{i-1})\\
			&=M(b-a)
		\end{align*}
	...
	\begin{proof}
		...
		\begin{align*}
			\TV(G,[-1,1]) 
			&\geq \sum\limits_{i=1}^n|G(x_i)-G(x_{i-1})| \\
			& > \frac{2}{\pi}\sum\limits_{i=1}^n \frac{1}{i}
		\end{align*}
		Therefore $G \not \in \BV([-1,1])$.
	\end{proof}
	...
	\begin{proof}
		\begin{enumerate}
			\item Let $F, G \in \BV$, $x \in \R$ and $\ep>0$. Since $T_{F+G}(x) < \infty$, $T_{F+G}(x)-\ep< T_{F+G}(x)$. Thus there exists $(x_i)_{i=0}^n \subset \R$ such that $(x_i)_{i=0}^n$ is increasing, $x_n=x$ and $T_{F+G}(x) < \sum\limits_{i=1}^n |(F+G)(x_i) - (F+G)(x_{i-1}))|+ \ep$. Thuerefore 
			
			\begin{align*}
				T_{F+G}(x)
				& < \sum\limits_{i=1}^n |(F+G)(x_i) - (F+G)(x_{i-1}))|+ \ep\\
				& \leq \sum\limits_{i=1}^n |F(x_i)-F(x_{i-1})|+ \sum\limits_{i=1}^n|G(x_i)-G(x_{i-1})| + \ep\\
				& \leq T_F(x) + T_G(x) + \ep
			\end{align*}
			...
			$$\sum\limits_{i=1}^n|F_j(x_i)-F_j(x_{i-1})| \leq \sum\limits_{i=1}^n|F(x_i)-F(x_{i-1})|$$
	...
	\begin{defn} \ld{00000} 
		Let $F: \R \rightarrow \C$. Then $F$ is said to be \textbf{absolutely continuous} if for each $\ep>0$, there exists $\del>0$ such that for each disjoint $((a_i, b_i))_{i=1}^n \subset \MB(\R)$, $\sum\limits_{i=1}^n b_i-a_i < \del$ implies that $\sum\limits_{i=1}^n|F(b_i)-F(a_i)| < \ep$.
	\end{defn}
	
	\begin{defn} \ld{00000} 
		Let $F: [a,b] \rightarrow \C$. Then $F$ is said to be \textbf{absolutely continuous} if for each $\ep>0$, there exists $\del>0$ such that for each disjoint $((a_i, b_i))_{i=1}^n \subset \MB([a,b])$, $\sum\limits_{i=1}^n b_i-a_i < \del$ implies that $\sum\limits_{i=1}^n|F(b_i)-F(a_i)| < \ep$.
	\end{defn}
	...
	\begin{align*}
	\sum\limits_{j=1}^{n'} |F(x'_j) - F(x'_{j-1})| 
	&= \sum\limits_{j=1}^n \sum\limits_{k=k_{j-1}+1}^{k_j} |F(x'_k) - F(x'_{k-1})|  \\
	&< \sum\limits_{j=1}^n 1  \\
	&= n
	\end{align*}
	...
	\begin{proof}
		...
		\begin{align*}
			\sum\limits_{i=1}^n|F(x_i)-F(x_{i-1})| 
			&= \sum\limits_{i=1}^n \bigg| \int_{(x_{i-1},x_i]}f\dm \bigg|\\
			& \leq \sum\limits_{i=1}^n \int_{(x_{i-1},x_i]}|f|\dm \\
			& = \int_{(x_0,x]}|f|\dm\\
			& < \int|f|\dm
		\end{align*}
	...
	\begin{proof}
		Suppose that $F$ is Lipschitz continuous. Then there exists $M>0$ such that for each $x,y \in \R, |F(x)-F(y)| \leq M|x-y|$. Let $\ep >0$. Choose $\del = \frac{\ep}{M}$. Let $((a_i, b_i))_{i=1}^n \subset \MB(\R)$, Suppose that $\sum\limits_{i=1}^n b_i-a_i < \del$. Then 
		\begin{align*}
			\sum\limits_{i=1}^n|F(b_i)-F(a_i)| 
			&\leq \sum\limits_{i=1}^n M(b_i - a_i)\\
			&< M\del\\
			&= \ep
		\end{align*}
	...
	\begin{proof}
		Let $(q_n)_{n\in \N}$ be an ennumeration of $\Q$. Define $F:\R \rightarrow \R$ by $$F =\sum\limits_{n\in \N} 2^{-n}\chi_{[q_n, \infty)}$$ 
		Equivalently, if we define $S_x=\{n \in \N: q_n \leq x\}$, then we may write $$F(x) = \sum\limits_{n \in S_x}2^{-n}$$\\ Let $x, y \in \R$. Suppose that $x < y$. Then $S_x \subsetneq S_y$. So $F(x)<F(y)$ and therefore $F$ is strictly increasing.\\ 
		...
		\begin{align*}
			|F(x)-F(y)| 
			&= \sum\limits_{n \in S_y}2^{-n} - \sum\limits_{n \in S_x}2^{-n}\\
			& = \sum\limits_{n \in S_{x,y}}2^{-n}\\
			& \leq \sum\limits_{n=N}^{\infty}2^{-n}\\
			&<\ep
		\end{align*} 
		...
		\begin{align*}
			|F(x)-F(y)| 
			&= \sum\limits_{n \in S_x}2^{-n} - \sum\limits_{n \in S_y}2^{-n}\\
			& = \sum\limits_{n \in S_{y,x}}2^{-n}\\
			& \leq \sum\limits_{n=N}^{\infty}2^{-n}\\
			&<\ep
		\end{align*}
	...
	\begin{ex} \lex{00000} 
		Let $(F_n)_{n\in \N} \in \NBV$ be a sequence of nonnegative, increasing functions. If for each $x \in \R$, $F(x)=\sum\limits_{n \in \N}F_n(x)< \infty$, then for a.e. $x \in \R$, $F$ is differentiable at $x$ and $F'(x) = \sum\limits_{n\in \N}F_n'(x)$. 
	\end{ex}
	
	\begin{proof}
		
		Define $\mu = \sum\limits_{n \in \N}\mu_{F_n}$. Note that 
		\begin{align*}
			\mu((-\infty,x]) 
			&= \sum\limits_{n \in \N}\mu_{F_n}((-\infty,x]) \\
			&= \sum\limits_{n \in \N}F_n(x)\\
			&= F(x)
		\end{align*}
		Hence $F \in \NBV$ and $\mu=\mu_F$. For each $n \in \N$, there exist $\lam_n \in M(\R)$ and $f \in L^1(\R)$ such that $d\mu_{F_n} = d\lam_n + f_n\dm$ and $\lam \perp m$. Since for each $n \in \N$, $\lam_n, f_n$ are nonnegative, we have that $d\mu_F =  \sum\limits_{n \in \N} d\lam_n + (\sum\limits_{n \in \N}f_n)\dm$. By a previous theorem, for a.e. $x \in \R$, 
		\begin{align*}
			F'(x) 
			&= \lim_{r \rightarrow 0}\frac{\mu_F((x,x+r])}{m((x,x+r])} \\
			&= \sum\limits_{n \in \N}f_n(x)\\
			&= \sum\limits_{n \in \N}\lim_{r \rightarrow 0}\frac{\mu_{F_n}((x,x+r])}{m((x,x+r])} \\
			&= \sum\limits_{n \in \N}F_n'(x)
		\end{align*}
	\end{proof}
	
	\begin{ex} \lex{00000} 
		Let $F:[0,1]\rightarrow [0, 1]$ be the Cantor function. Extend $F$ to $\R$ by setting $F(x) = 0$ for $x<0$ and $F(x)=1$ for $x>1$. Let $([a_n,b_n])_{n \in \N}$ be an ennumeration of the closed subintervals of $[0,1]$ with rational endpoints. For $n \in \N$, define $F_n:\R \rightarrow [0,1]$ by $F_n(x) = F(\frac{x-a_n}{b_n-a_n})$. Define $G:\R \rightarrow \R$ by $G = \sum\limits_{n \in \N}2^{-n}F_n$. Then $G$ is continuous, strictly increasing on $[0,1]$ and $G'=0$ a.e.
	\end{ex}
	...
	\begin{proof}
		...
		\begin{align*}
			G(x) 
			&= \sum\limits_{n \in \N}2^{-n}F_n(x)\\
			&= \sum\limits_{\substack{n \in \N\\ n \neq j}}2^{-n}F_n(x) + 0\\
			& < \sum\limits_{\substack{n \in \N\\ n \neq j}}2^{-n}F_n(y) + 2^{-j}F_n(y)\\
			&=\sum\limits_{n \in \N}2^{-n}F_n(y)\\
			&=G(y)
		\end{align*}
	...
	\begin{proof}\
		\begin{enumerate}
			...
			\begin{align*}
				\sum\limits_{j=1}^n P^{\mu}_{\MB} \chi_{A_j}
				& = P^{\mu}_{\MB} \sum\limits_{j=1}^n \chi_{A_j} \\
				& = P_{B}f_n \\
	...
	\begin{proof}\
		\begin{enumerate}
			...
			\begin{align*}
				...
				& \leq \bigg[\sum\limits_{n \in \N} \mu_{\MB}(E_n^c) \bigg] + \mu_{\MB}(N_2^c) \\
				& = 0
			\end{align*}
		...
		\begin{align*}
			...
			& \leq \bigg[\sum\limits_{n \in \N} \mu_{\MB}(E_n^c) \bigg] + \mu_{\MB}(N_3^c) \\
			& = 0
		\end{align*}
	...
	\begin{proof} Define $F:\R \times X \rightarrow [0,1]$ as in the previous exercise. For each $x \in X$, define $\mu_x: \MB(\R) \rightarrow [0,1]$ to be the unique measure such that for each $a, b \in \R$, $a \leq b$ implies that $\mu_x((a,b]) = F(b, x) - F(a, x)$. Define $\kap: X \times \MB(\R) \rightarrow [0,1]$ by $\kap(A, x) = \mu_x(A)$. \\
		\begin{enumerate}
			...
			\begin{enumerate}
				...
				\begin{align*}
					\kap(x, \R) 
					& = \sup\limits_{n \in \N} \mu_x((-\infty, n])\\
					& = \sup\limits_{n \in \N} F(n, x) \\
					& = 1 
				\end{align*}
	...
	\begin{note}
		\tcr{make definition for $\#_a \in M_+(\N, \MP(\N))$ by $\#_a(A) \defeq \sum\limits_{j \in A} a_j$ in section on counting measure, $\#$}
	\end{note}
	
	\begin{defn}
		Let $a \in \C^{\N}$. We define define $l^{p,a} \defeq L^p(\#_a)$. 
	\end{defn}
	...
	\newpage
	\chapter{Borel and Radon Measures}
	...
	\begin{note}
		\begin{itemize}
			\item We recall the definiton of $\MK(X) \defeq \{K \subset X: \text{$K$ is compact}\}$. \tcr{define $\MK(X)$ in topology notes}
			\item \tcr{ref ex in analysis notes compactness section} implies that for each $x \in X$, $\{x\} \in \MK(X)$.
		\end{itemize}
	\end{note}
	
	\begin{defn} \ld{def:introduction_radon_measures:0001} 
	Let $X$ be a topological space. Suppose that $\MK(X) \subset \MB(X)$. Let $\mu \in M_+(X)$.
	\begin{itemize}
		\item Let $E \in \MB(X)$. 
		Then $\mu$ is said to be 
		\begin{enumerate}
			\item \textbf{outer regular on $E$} if
			$$\mu(E) = \inf \mu (\upar_{\MO(X)} E) $$
			\item \textbf{inner regular on $E$} if
			$$\mu(E) = \sup \mu (\doar_{\MK(X)} E) $$
	...
	\begin{ex}
		Let $X$ be a topological space. Suppose that $\MK(X) \subset \MB(X)$. Let $\mu \in M_+(X)$. Then for each $E \in \MB(X)$,
		\begin{enumerate}
			\item $\mu(E) \leq \inf \mu(\upar_{\MO(X)} E)$.
			\item $\sup \mu(\doar_{\MK(X)} E) \leq \mu(E)$.
		\end{enumerate}
	\end{ex}
	
	\begin{proof}\
		\begin{enumerate}
			\item Let $a \in \mu(\upar_{\MO(X)} E)$. Then there exists $U \in \{U \subset X: \text{$U$ is open and $E \subset U$}\}$ such that $a = \mu(U)$. Since $E \subset U$, we have that  
			\begin{align*}
				\mu(E)
				& \leq \mu(U) \\
				& = a.
			\end{align*}
			Since $a \in \mu(\upar_{\MO(X)} E)$ is arbitrary, we have that for each $a \in \mu(\upar_{\MO(X)} E)$, $\mu(E) \leq a$. Thus $\mu(E) \in \lbd \mu(\upar_{\MO(X)} E)$. Hence $\mu(E) \leq \inf \mu(\upar_{\MO(X)} E)$. 
			\item Let $b \in \mu(\doar_{\MK(X)} E)$. Then there exists $K \in \{K \in \MK(X): K \subset E \}$ such that and $b = \mu(K)$. Since $K \subset E$, we have that 
			\begin{align*}
				b
				& = \mu(K) \\
				& \leq \mu(E).
			\end{align*}
			Since $b \in \mu(\doar_{\MK(X)} E)$ is arbitrary, we have that for each $b \in \mu(\doar_{\MK(X)} E)$, $b \leq \mu(E)$. Thus $\mu(E) \in \ubd \mu(\doar_{\MK(X)} E)$. Hence $\sup \mu(\doar_{\MK(X)} E) \leq \mu(E)$. 
		\end{enumerate}
	\end{proof}

	\begin{ex} \lex{ex:introduction_radon_measures:0001.1}
		Let $X$ be a topological space. Suppose that $\MK(X) \subset \MB(X)$. Let $\mu \in M_+(X)$. Suppose that $\mu$ is finite. If $\mu$ is inner regular, then $\mu$ is regular.
	\end{ex}	

	\begin{proof}
		Suppose that $\mu$ is inner regular. Let $E \in \MB(X)$. \rex{ex:introduction_radon_measures:0001.01} implies that $\mu(E) \leq \inf \mu(\upar_{\MO(X)} E)$. Let $\ep > 0$. Since $\mu$ is finite, $\mu(E^c) -\ep < \mu(E^c)$. Inner regularity implies that there exists $K \subset E^c$ such that $\mu(K) > \mu(E^c) - \ep$. Define $U \in \MB(X)$ by $U \defeq K^c$. Then $U$ is open and $E \subset U$. By construction,
		\begin{align*}
			\mu(U) - \mu(E) 
		...
		\begin{align*}
			...
			& < \mu(E) + \ep 
		\end{align*}
		Since $\ep > 0$ is arbitrary, we have that $\inf \mu(\upar_{\MO(X)} E) \leq \mu(E)$. Therefore $\mu(E) = \inf \mu(\upar_{\MO(X)} E)$ and $\mu$ is outer regular on $E$. Since $E \in \MB(X)$ is arbitrary, we have that $\mu$ is outer regular. Hence $\mu$ is regular.
	\end{proof}

	\begin{defn} \ld{def:introduction_radon_measures:0002} 
	Let $X$ be a topological space. Suppose that $\MK(X) \subset \MB(X)$.
	\begin{itemize}
		\item Let $\mu \in M_+(X)$. Then $\mu$ is said to be \textbf{Radon} if for each $E \in \MB(X)$, 
		\begin{enumerate}
			\item $E \in \MK(X)$ implies that $\mu(E) < \infty$
			\item $\mu$ is outer regular on $E$
			\item $E \in \MO(X)$ implies that $\mu$ is inner regular on $E$
		\end{enumerate}
		\item We define 
		\begin{itemize}
	...
	\begin{ex} \lex{ex:introduction_radon_measures:0002.01} 
		Let $X$ be a topological space. Suppose that $\MK(X) \subset \MB(X)$. Then $X$ is $T_1$ iff $\{\del_x:x \in X\} \subset \MM_1(X)$.
	\end{ex}
	
	\begin{proof}\
		\begin{itemize}
			\item $(\implies)$: \\
			Suppose that $X$ is $T_1$. Let $x \in X$.
			\begin{enumerate}
				\item Since $\del_x \in M_1(X)$, for each $K \in \MK(X)$, 
				\begin{align*}
					\del_x(K)
					& \leq 1 \\
					& < \infty.
				...
				\item Let $E \in \MB(X)$. 
				\begin{itemize}
					\item Suppose that $x \in E$. Then for each $U \in \upar_{\MO(X)} E$, $\del_x(U) = 1$. Hence 
					\begin{align*}
						\del_x(E)
						& = 1 \\
						& = \inf \del_x(\upar_{\MO(X)} E).
					\end{align*}
					\item Suppose that $x \not \in E$. Since $X$ is $T_1$, $\{x\}^c \in \MO(X)$ and $E \subset \{x\}^c$. Since $\del_x(\{x\}^c) \in \del_x(\upar_{\MO(X)} E)$, we have that
					\begin{align*}
						\del_x(E)
						& \leq \inf \del_x(\upar_{\MO(X)} E) \\
						& \leq \del_x(\{x\}^c) \\
						& = 0 \\
						& = \del_x(E).
					\end{align*}
					Hence $\del_X(E) = \inf \del_x(\upar_{\MO(X)} E)$.
				\end{itemize}
				Therefore $\del_x$ is outer regular on $E$. Since $E \in \MB(X)$ is arbitrary, we have that for each $E \in \MB(X)$, $\del_x$ is outer regular on $E$. Thus $\del_x$ is outer regular. 
				\item Let $U \in \MO(X)$. 
				\begin{itemize}
					\item Suppose that $x \in U$. Define $K \subset X$ by $K \defeq \{x\}$. Then $K \subset U$ and \tcr{ref ex in topology compactness section} implies that $K \in \MK(X)$. Therefore $K \in \doar_{\MK(X)} U$
					\begin{align*}
						\del_x(U)
						& = 1 \\
						& = \del_x(K) \\
						& \leq \sup \del_x(\doar_{\MK(X)} U) \\
						& \leq \del_x(U).
					\end{align*}
					Hence $\del_x(U) = \sup \del_x(\doar_{\MK(X)} U)$.
					\item Suppose that $x \not \in U$. Then for each $K \in \doar_{\MK(X)} U$, $x \not \in K$ and $\del_x(K) = 0$. Hence
					\begin{align*}
						\del_x(U) 
						& = 0 \\
						& = \sup \del_x(\doar_{\MK(X)} U).
					\end{align*}
				\end{itemize}
				Therefore $\del_x$ is inner regular on $U$. Since $U \in \MO(X)$ is arbitrary, we have that for each $U \in \MO(X)$, $\del_x$ is inner regular on $U$. 
			\end{enumerate}
			Thus $\del_x \in \MM_1(X)$. Since $x \in X$ is arbitrary, we have that for each $x \in X$, $\del_x \in \MM_1(X)$. Hence $\{\del_x:x \in X \} \subset \MM_1(X)$.
			\item $(\impliedby)$: \\
			Suppose that $X$ is not $T_1$. \tcr{ex in topology notes} implies that there exists $x \in X$ such that $\{x\} \not \in \MC(X)$. Define $E \subset X$ by $E \defeq \{x\}^c$. Then $E \not \in \MO(X)$ and $\upar_{\MO(X)} E = \{X\}$. Hence
			\begin{align*}
				\inf \del_x(\upar_{\MO(X)} E)
				& \del_x(X) \\
				& = 1 \\
				...
				& = \del_x(E).
			\end{align*}
			Therefore $\del_x$ is not outer regular and $\del_x \not \in \MM_1(X)$. Thus $\{\del_x:x \in X\} \not \subset \MM_1(X)$.
		\end{itemize}
	\end{proof}

	\begin{ex} \lex{ex:introduction_radon_measures:0002.1} 
		Let $X$ be a topological space. Suppose that $\MK(X) \subset \MB(X)$. Let $\mu, \nu \in \MM_+(X)$ and $\lam \geq 0$. Then $\mu + \lam \nu \in \MM_+(X)$. \tcr{split into outer/inner regular ex and radon ex, also make countable sum}
	\end{ex}

	\begin{proof}
		\rex{} \tcr{ref ex} implies that $\mu + \lam \nu \in M_+(X)$. 
		\begin{enumerate}
			...
			\begin{align*}
				...
				& < \infty.
			\end{align*} 
			Since $K \in \MK(X)$ is arbitrary, we have that for each $K \in \MK(X)$, $[\mu + \lam \nu](K) < \infty$.
			\item Let $E \in \MB(X)$. \rex{ex:introduction_radon_measures:0001.01} implies that $[\mu + \lam \nu](E) \leq \inf [\mu + \lam \nu](\upar_{\MO(X) E})$.
			\begin{itemize}
				...
				\begin{align*}
					...
					& < \infty
				\end{align*}
				Since $\mu, \nu \in \MM_+(X)$, there exist $U_{\mu}, U_{\nu} \in \upar_{\MO(X)} E$ such that $\mu(U_{\mu}) < \mu(E) + \ep/2$ and $\nu(U_{\mu}) < \nu(E) + \ep/(2 [\lam + 1])$. Set $U \defeq U_{\mu} \cap U_{\nu}$. Then $U \in \upar_{\MO(X)} E$ and
				\begin{align*}
					[\mu + \lam \nu](U)
					& = \mu(U) + \lam \nu(U) \\
					& \leq \mu(U_{\mu}) + \lam \nu(U_{\nu}) \\
					...
					& = \mu(E) + \lam \nu(E) + \ep 
				\end{align*}
				Since $\ep > 0$ is arbitrary, we have that for each $\ep > 0$,
				\begin{align*}
					\inf [\mu + \lam \nu](\upar_{\MO(X)} E)
					& \leq [\mu + \lam \nu](U) \\
					& < \mu(E) + \lam \nu(E) + \ep.
				...
				\begin{align*}
					...
					& \leq \inf [\mu + \lam \nu](\upar_{\MO(X)} E)
				\end{align*}
				and $[\mu + \lam \nu](E) = \inf [\mu + \lam \nu](\upar_{\MO(X)} E)$ and $\mu + \lam \nu$ is outer regular on $E$. Since $E \in \MB(X)$ is arbitrary, we have that for each $E \in \MB(X)$, $\mu + \lam \nu$ is outer regular on $E$. 
			\end{itemize}
			\item Let $U \in \MO(X)$. \rex{ex:introduction_radon_measures:0001.01} implies that $[\mu + \lam \nu](\doar_{\MK(X)} U) \leq [\mu + \lam \nu](U)$. 
			\begin{itemize}
				\item If $\lam = 0$, then  
				\begin{align*}
					\mu + \lam \nu 
				...
				\begin{itemize}
					\item Suppose that $[\mu + \lam \nu](U) = \infty$. Since $[\mu + \lam \nu](U) = \mu(U) + \lam \nu(U)$, we have that $\mu(U) = \infty$ or $\lam \nu(U) = \infty$. 
					\begin{itemize}
						\item Suppose that $\mu(U) = \infty$. Let $M > 0$. Since $\mu \in \MM_+(X)$, there exists $K \in \doar_{\MK(X)} U$ such that $\mu(K) > M$. Then 
						\begin{align*}
							\sup [\mu + \lam \nu] (\doar_{\MK(X)} U) 
							& \geq [\mu + \lam \nu](U) \\
							& \geq \mu(U) \\
							...
							& > M
						\end{align*}
						Since $M > 0$ is arbitrary, we have that for each $M >0$, $\sup [\mu + \lam \nu] (\doar_{\MK(X)} U) > M$. Thus 
						\begin{align*}
							[\mu + \lam \nu](U)
							& = \infty \\
							& = \sup [\mu + \lam \nu] (\doar_{\MK(X)} U).
						\end{align*}
						\item Suppose that $\lam \nu(U) = \infty$. 
						Let $M > 0$. Since $\nu \in \MM_+(X)$, there exists $K \in \doar_{\MK(X)} U$ such that $K$ is compact and $\nu(K) > M/\lam$. Then 
						\begin{align*}
							\sup [\mu + \lam \nu] (\doar_{\MK(X)} U)
							& \geq [\mu + \lam \nu](K) \\
							& \geq \lam \nu(K) \\
					...
					\begin{align*}
							...
							& < \infty.
						\end{align*}
						Since $\lam \neq 0$ and $\lam \nu(U) < \infty$, we have that $\nu(U) < \infty$. Let $\ep > 0$. Then $\mu(U) - \ep/2 < \mu(U)$ and $\nu(U) - \ep/(2 \lam) < \nu(U)$. Since $\mu, \nu \in \MM_+(X)$, there exist $K_{\mu}, K_{\nu} \in \doar_{\MK(X)} U$ such that $\mu(K_{\mu}) > \mu(U) - \ep/2$ and $\nu(K_{\nu}) > \nu(U) - \ep/(2 \lam)$. Set $K \defeq K_{\mu} \cup K_{\nu}$. \tcr{ref ex in analysis notes compactness section} implies that $K \in \doar_{\MK(X)} U$. Therefore
					\begin{align*}
						\sup [\mu + \lam \nu] (\doar_{\MK(X)} U)
						& \geq [\mu + \lam \nu](K) \\
						& = \mu(K) + \lam \nu (K) \\
						...
						& = [\mu + \lam \nu](U) + \ep  
					\end{align*}
					Since $\ep > 0$ is arbitrary, we have that for each $\ep > 0$, $\sup [\mu + \lam \nu] (\doar_{\MK(X)} U) > [\mu + \lam \nu](U) + \ep$. Hence 
					\begin{align*}
						\sup [\mu + \lam \nu] (\doar_{\MK(X)} U)
						& \leq [\mu + \lam \nu](U) \\
						& \leq \sup [\mu + \lam \nu] (\doar_{\MK(X)} U)
					\end{align*}
					and $[\mu + \lam \nu](U) = \sup [\mu + \lam \nu] (\doar_{\MK(X)} U)$. Therefore $\mu + \lam \nu$ is inner regular on $U$. Since $U \in \MO(X)$ is arbitrary, we have that for each $U \in \MO(X)$, $\mu + \lam \nu$ is inner regular on $U$. 
				\end{itemize}
			\end{itemize}
		\end{enumerate}
		Thus $\mu + \lam \nu \in \MM_+(X)$.
	...
	\begin{defn} \ld{def:introduction_radon_measures:0003}
		Let $X$ be a topological space. Suppose that $\MK(X) \subset \MB(X)$. Let $\mu \in \MM_+(X)$. We define 
		\begin{itemize}
			\item the \tbf{$\mu$-null open sets}, denoted $\MN_{\mu} \subset \MB(X)$, by 
			$$\MN_{\mu} \defeq \{U \in \MO(X): \mu(U) = 0\}$$ 
			\item the \tbf{cosupport of $\mu$}, denoted $N_{\mu}$, by 
	...
	\begin{note}
		\tcr{make note/ex about how $\mu$ can be supported on a set $E$ (see def of `supported on') while $\supp \mu \neq E$ and is neither a sub nor super set of $E$ (see disintegration and simmons proof)}
	\end{note}

	\begin{ex} \lex{ex:introduction_radon_measures:0004} 
		Let $X$ be a topological space. Suppose that $\MK(X) \subset \MB(X)$. Let $\mu \in \MM_+(X)$. Then 
		\begin{enumerate}
			\item $N_{\mu} \in \MO(X)$,
			\item $\supp \mu \in \MC(X)$,
			\item $\mu(N_{\mu}) = 0$
		\end{enumerate}
	...
	\begin{proof}\
		\begin{enumerate}
			\item Since $\MN_{\mu} \subset \MO(X)$ and 
			$$N_{\mu} = \bigcup_{U \in \MN_{\mu}} U $$
			we have that $N_{\mu} \in \MO(X)$.  
			\item Since $N_{\mu} \in \MO(X)$ and $\supp \mu = N_{\mu}^c$, we have that $\supp \mu \in \MC(X)$.  
			\item Let $K \in \doar_{\MK(X)} N_{\mu}$. Then $\MN_{\mu} \in \opn{\Cover}(K)$. Thus there exist $U_1, \ldots, U_n \in \MN_{\mu}$ such that 
			$$K \subset \bigcup_{j=1}^n U_j$$ 
			Therefore 
			\begin{align*}
				\mu(K) 
				&\leq \mu\bigg( \bigcup_{j=1}^n U_j \bigg) \\
				& \leq \sum\limits_{j=1}^n \mu(U_j) \\
				&= 0.
			\end{align*}
			Since $K \in \doar_{\MK(X)} N_{\mu}$ is arbitrary, we have that for each $K \in \doar_{\MK(X)} N_{\mu}$, $\mu(K) = 0$. Since $\mu$ is inner regular, 
			\begin{align*}
				\mu(N_{\mu}) 
				&= \sup \mu(\doar_{\MK(X)} N_{\mu}) \\
				&= 0
			\end{align*}
		\end{enumerate}
	\end{proof}

	\begin{ex} \lex{ex:introduction_radon_measures:0004.1}
		Let $X$ be a topological space. Suppose that $\MK(X) \subset \MB(X)$ and $X$ is $T_1$. Then for each $\mu \in \MM_1(X)$ and $x \in X$, $\mu = \del_x$ iff $\supp \mu = \{x\}$. \\
		\tbf{Hint:} \rex{ex:measures:00013.1}
	\end{ex}
	
	\begin{proof}
		Let $\mu \in \MM_1(X)$ and $x \in X$. Since $X$ is $T_1$, we have that $\{x\}^c \in \MO(X)$. Since $\mu \in \MM_1(X)$, we have that $X \neq N_{\mu}$. By \rd{def:introduction_radon_measures:0003}, $N_{\mu} = \bigcup\limits_{U \in \MN_{\mu}}$ and $\supp \mu = (N_{\mu})^c$. Therefore 
		\begin{align*}
			\mu = \del_x
			& \iff \mu(\{x\}^c) = 0 \quad \text{(by \rex{ex:measures:00013.1})} \\
			& \iff \{x\}^c \in \MN_{\mu} \\
			& \iff N_{\mu} = \{x\}^c \\
			& \iff \supp \mu = \{x\}.
		\end{align*}
		Since $\mu \in \MM_1(X)$ and $x \in X$ are arbitrary, we have that for each $\mu \in \MM_1(X)$ and $x \in X$, $\mu = \del_x$ iff $\supp \mu = \{x\}$.
	\end{proof}

	\begin{ex} \lex{ex:introduction_radon_measures:0005}
		Let $X$ be a topological space. Suppose that $\MK(X) \subset \MB(X)$. Let $\mu \in \MM_+(X)$ and $x \in X$. Then $x \in \supp \mu$ iff for each $U \in \MN(x)$, $\mu(U) > 0$.
	\end{ex}
	...
	\begin{ex} \lex{ex:introduction_radon_measures:0006} 
		Let $X$ be a topological space. Suppose that $\MK(X) \subset \MB(X)$. Let $\mu \in \MM_+(X)$ and $E \in \MB(X)$. If $\mu(E) < \infty$, then for each $\ep >0$, 
		\begin{enumerate}
	...
	\begin{ex} \lex{ex:introduction_radon_measures:0007} 
		Let $X$ be a topological space. Suppose that $\MK(X) \subset \MB(X)$. Let $\mu \in \MM_+(X)$ and $E \in \MB(X)$. If $\mu(E) < \infty$, then $\mu$ is inner regular on $E$. \\
	...
	\begin{ex} \lex{ex:introduction_radon_measures:0008} 
		Let $X$ be a topological space. Suppose that $\MK(X) \subset \MB(X)$. Let $\mu \in \MM_+(X)$ and $E \in \MB(X)$. If $E$ is $\sig$-finite with respect to $\mu$, then $\mu$ is inner regular on $E$. \\
	...
	\begin{proof}
		...
		\begin{itemize}
			...
			\begin{align*}
				\mu(E)
				&= \infty \\
				&= \sup\limits_{\substack{ K \subset E \\ \text{ $K$ is compact}}} \mu(K)
			\end{align*}	 
	...
	\begin{ex} \lex{ex:introduction_radon_measures:0009} 
		Let $X$ be a topological space. Suppose that $\MK(X) \subset \MB(X)$. Let $\mu \in \MM_+(X)$. If $\mu$ is $\sig$-finite, then $\mu$ is regular.
	\end{ex}
	...
	\begin{ex} \lex{ex:introduction_radon_measures:0009.1} 
		Let $X$ be a topological space. Suppose that $\MK(X) \subset \MB(X)$. Then for each $\mu \in \MM_1(X)$, $\mu$ is regular.
	\end{ex}
	...
	\begin{ex} \lex{ex:introduction_radon_measures:0010} 
		Let $X$ be a topological space. Suppose that $\MK(X) \subset \MB(X)$. Let $\mu \in \MM_+(X)$. If $X$ is $\sig$-compact, then $\mu$ is $\sig$-finite and $\mu$ is regular.
	\end{ex}
	...
	\subsection{Regularity on Subspaces}
	
	\begin{note} \lnt{note:radon_measures:regularity:subspaces:0001}
		Let $X$ be a topological space and $E \in \MB(X)$. \tcr{An ex in subspace of measurable spaces} implies that $\MB(E) \subset \MB(X)$. \tcr{An ex in analysis notes section on compactness} implies that $\MK(E) = \doar_{\MK(X)} E$. Therefore $\MK(X) \subset \MB(X)$ implies that 
		\begin{align*}
			\MK(E) 
			& = \MK(E) \cap E \\
			& \subset \MK(X) \cap E \\
	...
	\begin{ex}
		Let $X$ be a topological space. Suppose that $\MK(X) \subset \MB(X)$. Let $\mu \in M_+(X)$, $E \in \MB(X)$ and $F \in \MB(E)$. Suppose that $E \in \MC(X)$. 
		\begin{enumerate}
			\item If $\mu$ is outer regular on $F \cap E$, then $\mu_E$ is outer regular on $F$.
			\item If $\mu$ is inner regular on $F \cap E$, then $\mu_E$ is inner regular on $F$. 
		\end{enumerate}
	\end{ex}
	
	\begin{proof}\
		\begin{enumerate}
			\item Suppose that $\mu$ is outer regular on $F \cap E$. \tcr{A prev ex} implies that $\mu_E(F) \leq \inf \mu_E(\upar_{\MO(X)}F)$.
			\begin{itemize}
				\item If $\mu_E(F) = \infty$, then 
				\begin{align*}
					\infty 
					& = \mu_E(F) \\
					& \leq \inf \mu_E(\upar_{\MO(X)}F) \\
					& \leq \infty 
				\end{align*}
				and $\mu_E(F) = \inf \mu_E(\upar_{\MO(X)}F)$.
				\item Suppose that $\mu_E(F) < \infty$. Then  
				\begin{align*}
					\mu(F \cap E)
					& = \mu_E(F) \\
					& < \infty.
				\end{align*}
				Let $\ep > 0$. Since $\mu$ is outer regular on $F \cap E$, there exists $U_0 \in \upar_{\MO(X)} F \cap E$ such that $\mu(U_0) < \mu(F \cap E) + \ep$. Since $E \in \MC(X)$, we have that $E^c \in \MO(X)$. Define $U \in \MO(X)$ by $U \defeq U_0 \cup E^c$. Then 
				\begin{align*}
					F
					& = (F \cap E) \cup (F \cap E^c) \\
					& \subset U_0 \cup E^c \\
					& = U
				\end{align*}
				and $U \in \upar_{\MO(X)} F$. Therefore
				\begin{align*}
					\inf \mu_E( \upar_{\MO(X)} F)
					& \leq \mu_E(U) \\
					& = \mu_E(U_0 \cup E^c) \\
					& \leq \mu_E(U_0) + \mu_E(E^c) \\
					& = \mu_E(U_0) \\
					& \leq \mu(U_0) \\
					& < \mu(F \cap E) + \ep \\
					& = \mu_E(F) + \ep.
				\end{align*}
				Since $\ep > 0$ is arbitrary, we have that for each $\ep > 0$, $\inf \mu_E( \upar_{\MO(X)} F) < \mu_E(F) + \ep$. Hence 
				\begin{align*}
					\inf \mu_E( \upar_{\MO(X)} F) 
					& \leq \mu_E(F) \\
					& \leq \inf \mu_E( \upar_{\MO(X)} F) 
				...
				and $\mu_E(F) = \inf \mu_E( \upar_{\MO(X)} F)$. 
			\end{itemize}
			Thus $\mu_E$ is outer regular on $F$.
			\item Suppose that $\mu$ is inner regular on $F$. \tcr{A prev ex} implies that $\sup \mu_E(\doar_{\MK(X)}F) \leq \mu_E(F)$.
			\begin{itemize}
				\item Suppose that $\mu_E(F) = \infty$. Then 
				\begin{align*}
					\mu(F \cap E)
					...
					& = \infty.
				\end{align*}
				Let $M > 0$. Since $\mu$ is inner regular on $F \cap E$, there exists $K \in \doar_{\MK(X)} F \cap E$ such that $\mu(K) > M$. Since $\doar_{\MK(X)} F \cap E \subset \doar_{\MK(X)} F $, we have that 
				\begin{align*}
					\sup \mu_E(\doar_{\MK(X)} F) 
					& \geq \mu_E(K) \\
					& = \mu(K \cap E) \\
					& = \mu(K) \quad \text{(since $K \subset E$)} \\
					& > M.
				\end{align*}
				Since $M > 0$ is arbitrary, we have that for each $M > 0$, $\sup \mu_E(\doar_{\MK(X)} F) > M$. Thus 
				\begin{align*}
					\infty 
					& = \mu_E(F) \\
					& \geq \sup \mu_E(\doar_{\MK(X)} F) \\
					& \geq \infty
				\end{align*}   
				and $\mu_E(F) = \sup \mu_E(\doar_{\MK(X)} F)$.
				\item Suppose that $\mu_E(F) < \infty$. Let $\ep > 0$. Since $\mu$ is inner regular on $F \cap E$, there exists $K \in \doar_{\MK(X)} F \cap E$ such that $\mu(K) > \mu(F \cap E) - \ep$. Since $\doar_{\MK(X)} F \cap E \subset \doar_{\MK(X)} F $, we have that 
				\begin{align*}
					\sup \mu_E(\doar_{\MK(X)} F) 
					& \geq \mu_E(K) \\
					& = \mu(K \cap E) \\
					...
					& = \mu_E(F) - \ep.
				\end{align*}
				Since $\ep > 0$ is arbitrary, we have that for each $\ep > 0$, $\sup \mu_E(\doar_{\MK(X)} F) \geq \mu_E(F) - \ep$. Thus  
				\begin{align*}
					\sup \mu_E(\doar_{\MK(X)} F)
					& \geq \mu_E(F) \\
					& \geq \sup \mu_E(\doar_{\MK(X)} F)
				...
				and $\mu_E(F) = \sup \mu_E(\doar_{\MK(X)} F)$. 
			\end{itemize}
			Thus $\mu_E$ is inner regular on $F$.
		\end{enumerate}
	\end{proof}
	
	\begin{ex}
		Let $X$ be a topological space. Suppose that $\MK(X) \subset \MB(X)$. Let $\mu \in M_+(X)$, $E \in \MB(X)$ and $F \in \MB(E)$. 
		\begin{enumerate}
			\item If $\mu$ is outer regular on $F$, then $\mu|_E$ is outer regular on $F$.
			\item If $\mu$ is inner regular on $F$, then $\mu|_E$ is inner regular on $F$. 
		\end{enumerate}
	\end{ex}
	
	\begin{proof}\
		\begin{enumerate}
			\item Suppose that $\mu$ is outer regular on $F$. \tcr{A prev ex} implies that $\mu|_E(F) \leq \inf \mu|_E(\upar_{\MO(E)}F)$.
			\begin{itemize}
				\item First, suppose that $\mu|_E(F) = \infty$. Then 
				\begin{align*}
					\infty
					& = \mu|_E(F) \\
					& \leq \inf \mu|_E(\upar_{\MO(E)} F) \\
					& \leq \infty
				\end{align*}
				and therefore $\mu|_E(F) = \inf \mu|_E(\upar_{\MO(E)} F)$
				\item Now, suppose that $\mu|_E(F) < \infty$. Then 
				\begin{align*}
					\mu(F)
					& = \mu|_E(F) \\
					& < \infty
				\end{align*}
				Let $\ep > 0$. Since $\mu$ is outer regular on $F$, there exists $U_0 \in \upar_{O(X)}F$ such that $\mu(U_0) < \mu(F) + \ep$. Define $U \in \upar_{O(E)} F$ by $U \defeq U_0 \cap E$. Then
				\begin{align*}
					\mu|_E(U)
					& = \mu(U) \\
					& \leq \mu(U_0) \\
				...
				Therefore
				\begin{align*}
					\inf \mu|_E(\upar_{\MO(E)} F)
					& \leq \mu|_E(U) \\
					& < \mu|_E(F) + \ep
				\end{align*}
				Since $\ep > 0$ is arbitrary, we have that for each $\ep >0$, $\inf \mu|_E(\upar_{\MO(E)} F) < \mu|_E(F) + \ep$. Hence  
				\begin{align*}
					\inf \mu|_E(\upar_{\MO(E)} F) 
					& \leq \mu|_E(F) \\
					& \leq \inf \mu|_E(\upar_{\MO(X)} F) 
				\end{align*}
				and $\mu|_E(F) = \inf \mu|_E(\upar_{\MO(X)} F)$.
				\end{itemize}
				Thus $\mu|_E$ is outer regular on $F$.
			\item Suppose that $\mu$ is inner regular on $F$. \tcr{A prev ex} implies that $\sup \mu|_E(\doar_{\MK(E)} F) \leq \mu|_E(F)$. 
			\begin{itemize}
				\item Suppose that $\mu|_E(F) = \infty$. Then  
				\begin{align*}
					\mu(F)
					& = \mu|_E(F) \\
					& = \infty.
				\end{align*}
				Let $M > 0$. Since $\mu$ is inner regular on $F$, there exists $K \in \doar_{\MK(X)} F$ such that $\mu(K) > M$. Then 
				\begin{align*}
					\mu|_E(K)
					& = \mu(K) \\
					& > M.
				\end{align*}
				Since $\doar_{\MK(X)} F = \doar_{\MK(E)} F$, we have that 
				\begin{align*}
					\sup \mu|_E(\doar_{\MK(E)} F) 
					& \geq \mu|_E(K) \\
					& = \mu(K) \\
					& > M.
				\end{align*}
				Since $M > 0$ is arbitrary, we have that for each $M > 0$, $\sup \mu|_E(\doar_{\MK(E)} F) > M$. Thus $\sup \mu|_E(\doar_{\MK(E)} F) \geq \infty$. Therefore
				\begin{align*}
					\infty 
					& \leq \sup \mu|_E(\doar_{\MK(E)} F) \\
					& \leq \mu|_E(F) \\ 
					& \leq \infty. 
				\end{align*} 
				and $\mu|_E(F) = \sup \mu|_E(\doar_{\MK(E)} F)$.
				\item Suppose that $\mu|_E(F) < \infty$. Then  
				\begin{align*}
					\mu(F)
					& = \mu|_E(F) \\
					& < \infty.
				\end{align*}
				Let $\ep > 0$. Since $\mu$ is inner regular on $F$, there exists $K \in \doar_{\MK(X)} F$ such that $\mu(K) > \mu(F) - \ep$. Since $\doar_{\MK(X)} F = \doar_{\MK(E)} F$, we have that 
				\begin{align*}
					\sup \mu|_E(\doar_{\MK(E)} F)
					& \geq \mu|_E(K) \\
					& = \mu(K) \\
					...
					& = \mu|_E(F) - \ep. 
				\end{align*}
				Since $\ep >0$ is arbitrary, we have that for each $\ep >0$, $\sup \mu|_E(\doar_{\MK(E)} F) > \mu|_E(F) - \ep$. Thus $\mu|_E(F) \leq \sup \mu|_E(\doar_{\MK(E)} F)$. Hence  
				\begin{align*}
					\sup \mu|_E(\doar_{\MK(E)} F)
					& \leq \mu|_E(F) \\ 
					& \leq \sup \mu|_E(\doar_{\MK(E)} F)
				...
				and $\mu|_E(F) = \sup \mu|_E(\doar_{\MK(E)} F)$. 
			\end{itemize}
			Therefore $\mu|_E$ is inner regular on $F$. 
		\end{enumerate}
	\end{proof}

	%	\begin{ex} \lex{ex:subspace_radon_measures:0001}
%		Let $X$ be a topological space. Suppose that $\MK(X) \subset \MB(X)$. Let $\mu \in \MM_+(X)$ and $E \in \MB(X)$. If $\mu(E) < \infty$, then 
%		\begin{enumerate}
%			\item $\mu_E \in \MM_+(E)$
%			\item $\mu_E$ is regular.
%		\end{enumerate}
%	\end{ex}
%
%	\begin{proof}
%		Suppose that $\mu(E) < \infty$. Then $\mu_E$ is finite.
%		\begin{enumerate}
%			\item \begin{enumerate}
%				\item Since $\mu_E$ is finite, for each $K \in \MK(X)$, $\mu_E(K) < \infty$.
%				\item Let $B \in \MB(X)$. \rex{ex:introduction_radon_measures:0001.01} implies that $\mu_E(B) \leq \inf V^{\mu_E}_O(B)$. Since $\mu_E$ is finite, 
%				\begin{align*}
%					\mu(B \cap E)
%					& = \mu_E(B) \\
%					& < \infty.
%				\end{align*}
%				Let $\ep > 0$. Since $\mu$ is outer regular, there exists $a \in V(B \cap E)$ such that $a < \mu(B \cap E) + \ep$. Then there exists $U_0 \subset X$ such that $U_0$ is open in $X$, $B \cap E \subset U_0$ and $\mu(U_0) = a$. Since $E$ is closed, $E^c$ is open. Hence $U_0 \cup E^c$ is open. Define $U \subset X$ by $U \defeq U_0 \cup E^c$. Then 
%				\begin{align*}
%					B 
%					& = (B \cap E) \cup (B \cap E^c) \\
%					& \subset U_0 \cup E^c \\
%					& = U
%				\end{align*}
%				and 
...
%					U \cap E
%					& = (U_0 \cup E^c) \cap E \\
%					& = (U_0 \cap E) \cup (E^c \cap E) \\
%					& = (U_0 \cap E) \cup \varnothing \\
%					& = U_0 \cap E.
%				\end{align*}
%				Since $B \subset U$ and $U$ is open, $\mu_E(U) \in V^{\mu_E}_O(B)$. Hence
%				\begin{align*}
%					\inf V^{\mu_E}_O(B) 
%					& \leq \mu_E(U) \\
%					& = \mu(U \cap E) \\
...
%					& = \mu_E(B) + \ep. 
%				\end{align*}
%				Since $\ep > 0$ is arbitrary, we have that for each $\ep > 0$, $\inf V^{\mu_E}_O(B) \leq \mu_E(B) + \ep$. Hence $\inf V^{\mu_E}_O(B) \leq \mu_E(B)$. 
%				Since $\mu_E(B) \leq \inf V^{\mu_E}_O(B)$ and $\inf V^{\mu_E}_O(B) \leq \mu_E(B)$, we have that $\mu_E(B) = \inf V^{\mu_E}_O(B)$ and $\mu_E$ is outer regular on $B$. Since $B \in \MB(X)$ is arbitrary, we have that for each $B \in \MB(X)$, $\mu_E$ is outer regular on $B$. Thus $\mu_E$ is outer regular.
%				\item Let $U \subset X$. Suppose that $U$ is open. \rex{ex:introduction_radon_measures:0001.01} implies that $\sup V^{\mu_E}_I(U) \leq \mu_E(U)$. Since $\mu_E$ is finite, $\mu_E(U) < \infty$. Let $\ep > 0$. 
%				Since $\mu$ is inner regular on $U$, there exists $K \in \MK(X)$ such that $K \subset U$ and $\mu(U) - \ep < \mu(K) $. Then 
%				\begin{align*}
%					\mu_E( U ) - \mu_E(K)
%					& = \mu_E(U \setminus K) \\
%					& = \mu_E(U \cap K^c) \\
...
%					\mu_E(U) 
%					& < \mu_E(K) + \ep \\
%					& \leq \sup V^{\mu_E}_I(U) + \ep. 
%				\end{align*}
%				Since $\ep > 0$ is arbitrary, we have that for each $\ep > 0$, $\mu_E(U) < \sup V^{\mu_E}_I(U) + \ep$. Thus $\mu_E(U) \leq \sup V^{\mu_E}_I(U)$. Since $\sup V^{\mu_E}_I(U) \leq \mu_E(U)$ and $\mu_E(U) \leq \sup V^{\mu_E}_I(U)$, we have that $\mu_E(U) = \sup V^{\mu_E}_I(U)$. Thus $\mu_E$ is inner regular on $U$. Since $U \subset X$ with $U$ open is arbitrary, we have that for each $U \subset X$, $U$ is open implies that $\mu_E$ is inner regular on $U$. 
%			\end{enumerate}
%			Thus $\mu_E$ is Radon.
%			\item Clear by \rex{ex:introduction_radon_measures:0009}.
%		\end{enumerate}
%	\end{proof}
%
%	
...
%		Let $X$ be a topological space. Suppose that $\MK(X) \subset \MB(X)$. Let $\mu \in \MM_+(X)$ and $E \in \MB(X)$. If $\mu$ is regular, then $\mu|_E$ is regular.
%		\tcr{Maybe add radon part first then do regular?}
%	\end{ex}
%
%	\begin{proof}
%		Suppose that $\mu$ is regular. We note that 
%		Since $\MB(E) = \MB(X) \cap E$ and $E \in \MB(X)$, we have that 
%		\begin{align*}
%			\MB(E)
%			& = \MB(X) \cap E \\
%			& \subset \MB(X).
...
%					& = \inf V_E(B).
%				\end{align*}
%				In particular, $\inf V_E(B) \leq \mu|_E(B)$.
%				\item Now, suppose that $\mu|_E(B) < \infty$. Then 
%				\begin{align*}
%					\mu(B)
%					& = \mu|_E(B) \\
%					& < \infty
%				\end{align*}
%				Let $\ep > 0$. Since $\mu$ is Radon, there exists $U_0 \subset X$ such that $U_0$ is open in $X$, $B \subset U_0$ and $\mu(U_0) < \mu(B) + \ep$. Set $U \defeq U_0 \cap E$. Then $U$ is open in $E$, $B \subset U$ and 
%				\begin{align*}
%					\mu|_E(U)
%					& = \mu(U) \\
%					& \leq \mu(U_0) \\
...
%					& < \mu|_E(B) + \ep
%				\end{align*}
%				Since $\ep > 0$ is arbitrary, we have that $\inf V_E(B) \leq \mu|_E(B)$. 
%			\end{itemize}
%			Since $\mu|_E(B) \leq \inf V_E(B)$ and $\inf V_E(B) \leq \mu|_E(B)$, we have that $\mu|_E(B) = \inf V_E(B)$. Thus $\mu|_E$ is outer regular on $B$.
%			\item Let $B \in \MB(E)$.  Set 
%			$$V_E(B) \defeq \{\mu|_E(K): K \in \MK(B)\}$$
%			Clearly $\sup V_E(U) \leq \mu|_E(B)$.
%			\begin{itemize}
%				\item Suppose that $\mu(B) = \infty$. Let $M >0$. Inner regularity of $\mu$ implies that there exists $K \in \MK(B)$ such that $\mu(K) > M$. Then $K \in \MB(E)$ and
%				\begin{align*}
%					\mu|_E(K)
%					& = \mu(K) \\
%					& > M
...
%					& = \sup V_E(B)
%				\end{align*} 
%				\item Suppose that $\mu(B) < \infty$. Let $\ep > 0$. Inner regularity of $\mu$ implies that there exists $K \in \MK(B)$ such that $\mu(K) > \mu(B) - \ep$. Then $K \in \MB(E)$ and
%				\begin{align*}
%					\sup V_E(B)
%					& \geq \mu|_E(K) \\
%					& = \mu(K) \\
...
%	\begin{ex} \lex{ex:subspace_radon_measures:0001}
%		Let $X$ be a topological space. Suppose that $\MK(X) \subset \MB(X)$. Let $\mu \in \MM_+(X)$ and $E \in \MB(X)$. Then $\mu_E \in \MM_+(E)$.
%	\end{ex}
%
%	\begin{proof}\
%		\begin{enumerate}
...
%	\end{proof}
	...
	\begin{proof}
		...
		\begin{enumerate}
			...
			\begin{align*}
				...
				& \subset \MB(X)
			\end{align*}
			\tcr{A prev ex} implies that $\mu|_E(B) \leq \inf V_E(B)$.
			\begin{itemize}
				\item First, suppose that $\mu|_E(B) = \infty$. Then clearly 
				\begin{align*}
					\mu|_E(B)
			...
			\end{itemize}
			Thus $\mu|_E$ is outer regular on $F$.
			\item Let $B \in \MB(E)$. \tcr{A prev ex} implies that $\sup \mu|_E(\doar_{\MK(X)} F) \leq \mu|_E(F)$. Since $E \in \MB(X)$, 
			\begin{align*}
				B
				& \in \MB(E) \\
				& \subset \MB(X)
	...
	\begin{ex} \lex{ex:lebesgue_decomposition_radon_measures:0001}
		Let $X$ be a topological space. Suppose that $\MK(X) \subset \MB(X)$. Let and $\mu, \nu \in M_+(X)$. If $\nu$ is Radon, then for each $A \subset X$,  
	...
	\begin{ex} \lex{ex:lebesgue_decomposition_radon_measures:0002}
		...
		\tbf{Hint:} similar to the proof of (ii) of the Riesz representation theorem in ~\cite{folland1999real} \tcr{include page number in citation}
	\end{ex}
	...
	\begin{defn} \ld{def:complex_radon_measures:0001}
		...
		\begin{itemize}
			...
			\item We define $\MM(X) = \{\mu \in M(X): \text{$\mu$ is Radon}\}$
		\end{itemize}
	\end{defn}
	
	\begin{ex} \lex{ex:complex_radon_measures:0002}
		Let $X$ be a topological space. Suppose that $\MK(X) \subset \MB(X)$. Let $\nu \in M(X)$. Then 
		$$\text{$\mu \in \MM(X)$ iff $(\Re \nu)^+, (\Re \nu)^-, (\Im \nu)^+, (\Im \nu)^- \in \MM_+(X)$.}$$
	\end{ex}
	...
	\begin{defn} \ld{def:complex_radon_measures:0002.1}
		Let $X$ be a topological space. Suppose that $\MK(X) \subset \MB(X)$.
		\begin{itemize}
			\item Let $\mu \in \MM(X)$. We define the \tbf{support of $\mu$}, denoted $\supp \mu \in \MB(X)$, by $\supp \mu \defeq \supp |\mu|$. 
			\item We define $\MM^c(X) \defeq \{\mu \in \MM(X): \text{$\supp \mu$ is compact}\}$.
		\end{itemize}
	\end{defn}
	
	\begin{ex} \lex{ex:complex_radon_measures:0002.2}
		Let $X$ be a topological space. Suppose that $\MK(X) \subset \MB(X)$. Let $\mu \in \MM(X)$. Then 
		$$\supp \mu = \supp (\Re \mu)^+ \cup \supp (\Re \mu)^- \cup \supp (\Im \mu)^+ \cup \supp (\Im \mu)^-.$$
	\end{ex}
	...
	\begin{ex} \lex{ex:complex_radon_measures:0003}
		Let $X$ be a topological space. Suppose that $\MK(X) \subset \MB(X)$. Then $\MM(X)$ is a vector space. 
	\end{ex}
	...
	\begin{ex} \lex{ex:introduction_radon_measures:0010.1} 
		Let $X$ be a topological space. Suppose that $\MK(X) \subset \MB(X)$. Let $\mu \in \MM_+(X)$ and $f \in L^1(X, \MB(X), \mu)$. Define $\nu \in M(X)$ by 
		$$\nu(E) = \int_E f \dmu.$$
		Then 
		\begin{enumerate}
			\item $|\nu|$ is inner regular, \\
	...
	\begin{proof}\
		\begin{enumerate}
			...
			\begin{itemize}
				...
				\item Suppose that $\mu(E) = \infty$. Define $A \in \MB(X)$ by $A \defeq \{x \in X: |f(x)| > 0\}$. \rex{ex:integration_nonnegative_functions:0005.1} implies that $A$ is $\sig$-finite with respect to $\mu$. Then \rex{ex:subspace_measures:00004.2} implies that $E \cap A$ is $\sig$-finite with respect to $\mu$. \rex{ex:introduction_radon_measures:0008} implies that $\mu$ is inner regular on $E \cap A$. Since $E \cap A \in \MB(A)$, there exists $K \subset E \cap A$ such that $K$ is compact and $\mu(K) > \mu(E \cap A) - \del$. Then $K \subset E$ and $\mu[(E \cap A) \setminus K] < \del$. Therefore $|\nu|[(E \cap A) \setminus K] < \ep$. Since $|\nu|(E \cap A) < \infty$ and $|\nu|(E \cap A) = |\nu|(K) + |\nu|[(E \cap A) \setminus K]$, we have that 
				\begin{align*}
					|\nu|(E \cap A) - |\nu|(K) 
	...
	\begin{defn}
		Let $X$ be a topological space. Suppose that $\MK(X) \subset \MB(X)$. Let $\mu \in M_+(X)$ and $\nu \in \MM(X)$. 
		\begin{itemize}
			\item If $\Im \nu = 0$, we define $\nu_{\mu} \in M(X)$ by 
			$$\nu_{\mu} = \nu^+_{\mu} - \nu^-_{\mu}$$
			\item We define $\nu_{\mu} \in M(X)$ by 
	...
	\begin{ex} \lex{ex:borel_radon:LCH_rep:0100} \textbf{Representation Theorem II:}\\
		Let $X$ be a LCH space. Then the map $\phi: \MM(X) \rightarrow C_0(X)^*$ given by $\phi(\mu) \defeq I_{\mu}$ is an isometric linear isomorphism. 
	\end{ex}
	...
	\begin{note} \lnt{note:borel_measures_metric_spaces:0001}
		Let $X$ be a metric space and $A \subset X$. For $\ep >0$, we write $A_{\ep} = \{x \in X: d(x, A) < \ep\}$ and recall that $A_{\ep}$ is open.
	\end{note}
	
	\begin{ex} \lex{ex:borel_measures_metric_spaces:0002}
		Let $X$ be a metric space, $\mu \in M_1(X)$ and $E \in \MB(X)$. Then 
	...
	\begin{ex} \lex{ex:borel_measures_metric_spaces:0003}
		Let $X$ be a metric space and $\mu \in M_1(X)$. Then for each $C \subset X$, if $C$ is closed, then $\mu$ is outer regular on $C$. \\
	...
	\begin{ex} \lex{ex::borel_measures_metric_spaces:0004}
		Let $X$ be a metric space and $\mu \in M_1(X)$. Set 
		...
		\begin{enumerate}
			...
			\begin{itemize}
				...
				\begin{align*}
					...
					& \leq \sum\limits_{n \in \N} \mu(U_n \setminus E_n) \\
					&= \sum\limits_{n \in \N} [\mu(U_n) - \mu(E_n)] \\
					& \leq \sum\limits_{n \in \N} \ep 2^{-n-1} \\
					&= \frac{\ep}{2} \\
					& < \ep
				\end{align*}
				...
				\begin{align*}
					...
					&\leq \bigg[ \sum\limits_{n \in \N} \mu(E_n \cap C_n^c) \bigg] + \frac{\ep}{2}\\
					&=\bigg[ \sum\limits_{n \in \N} \mu(E_n) - \mu(C_n) \bigg] +\frac{\ep}{2}\\
					&\leq \bigg[ \sum\limits_{n \in \N} 2^{-n-1}\ep \bigg] + \frac{\ep}{2}\\
					&= \frac{\ep}{2} + \frac{\ep}{2} \\
					&= \ep 
				\end{align*}
	...
	\begin{ex} \lex{ex:borel_measures_metric_spaces:0005}
		Let $X$ be a metric space and $\mu \in M_1(X)$. Then $\mu$ is outer regular.
	\end{ex}
	
	\begin{proof}
		Set $\MT \defeq \{U \subset X: X \text{ is open}\}$ and define $\MA$ as in the previous exercise. \tcr{The previous exercises} imply that $\MT \subset \MA$. Since $\MB(X) = \sig(\MT)$, we have that $\MB(X) \subset \MA$. Therefore $\MB(X) = \MA$ and $\mu$ is outer regular.  
	\end{proof}
	
	\begin{ex} \lex{ex:borel_measures_metric_spaces:0006}
		Let $X$ be a metric space and $\mu \in M_1(X)$. If $\mu$ is inner regular on $X$, then $\mu$ is inner regular.
	\end{ex}
	...
	\begin{ex} \lex{ex:borel_measures_metric_spaces:0007}
		Let $X$ be a Polish space and $\mu \in M_1(X)$. Then $\mu$ is inner regular. \\
	...
	\begin{proof}
		...
		\begin{align*}
			...
			&\leq \sum\limits_{m \in \N} \mu \bigg( \bigg[ \bigcup\limits_{n = 1}^{n_m} \cl B(x_n, 1/m) \bigg]^c \bigg) \\
			&= \sum\limits_{m \in \N} \bigg[ \mu(X) - \mu \bigg(  \bigcup\limits_{n = 1}^{n_m} \cl B(x_n, 1/m)  \bigg) \bigg] \\
			& \leq \sum\limits_{m \in \N} 2^{-m-1}\ep \\
			&= \frac{\ep}{2} \\
			&< \ep
		\end{align*} 
	...
	\begin{ex} \lex{ex:borel_measures_metric_spaces:0008} \textbf{Ulam's Theorem:} \\
		Let $X$ be a Polish space and $\mu \in M_1(X)$. Then $\mu$ is regular and Radon.
	\end{ex}
	
	\begin{proof}
		Clear by preceeding exercises \tcr{(list exercises specifically)}.
	\end{proof}
	
	
	\begin{note} \lnt{note:borel_measures_metric_spaces:0009}
	...
	\begin{defn} \ld{def:borel_radon:weak*:0001}
		Let $X$ be a topological space. 
		\begin{itemize}
			\item For $f \in C_0(X)$, we define $\lam_f: M(X) \rightarrow \C$ by 
			$$\lam_f(\mu) \defeq \int_X f \dmu.$$
			\item We define the \tbf{weak-$*$ topology on $M(X)$}, denoted $\MT_{w^*}$, by $\MT_{w^*} \defeq \tau_{M(X)}(\lam_f: f \in C_0(X))$.  
			\item For $(\mu_{\al})_{\al \in A} \subset M(X)$ a net and $\mu \in M(X)$, we write $\mu_{\al} \conv{w^*} \mu$ if $\mu_{\al} \rightarrow \mu$ in $(X, \MT_{w^*})$.
		\end{itemize}
	\end{defn}
	
	\begin{note}\
		\begin{itemize}
			\item By definition, $\mu_{\al} \conv{w^*} \mu$ iff for each $f \in C_0(X)$, 
			$$\int_X f \dmu_{\al} \rightarrow \int_X f \dmu.$$
			\item We note that if $X$ is a LCH space, $(\mu_{\al})_{\al \in A} \subset \MM(X)$ and $\mu \in \MM(X)$, then $\mu_{\al} \conv{w^*} \mu$ iff $I_{\mu_{\al}} \conv{w^*} I_{\mu}$ in $(\MM(X), \|\cdot\|)^*$.
		\end{itemize}
	\end{note}
	
	\begin{ex} \lex{ex::borel_radon:weak*:0002}
		Let $X$ be a LCH space. Then $(\MM(X), \MT_{w^*})$ is Hausdorff.
	\end{ex}
	
	\begin{proof}
		\tcr{FINISH!!!}
		Let $(\mu_{\al})_{\al \in A} \subset \MM(X)$ be a net and $\mu_1, \mu_2 \in \MM(X)$. Suppose that $\mu_{\al} \conv{w^*} \mu_1$ and $\mu_{\al} \conv{w^*} \mu_2$. Then for each $f \in C_0(X)$, 
		\begin{align*}
			\lam_f(\mu_1)
			& = \int f \dmu_1 \\
			& = \lim\limits_{\al} \int f \dmu_{\al} \\
			& = \int f \dmu_2 \\
			& = \lam_f(\mu_2).
		\end{align*} 
		Since $X$ is LCH, \rex{ex:borel_radon:LCH_rep:0100} implies that $(\MM(X), \|\cdot\|_{TV}) \cong_{\bBan} (C_0(X)^*, \|\cdot\|_{\infty}^*)$. Thus $(\MM(X), \MT_{w^*}) \cong_{\bTopVect} (C_0(X)^*, \MT_{w^*})$ \tcr{make/ref ex here and make precise}. \tcr{ref ex in analysis notes section on duality in topological vect spaces chapter} implies that $(C_0(X)^*, \MT_{w^*})$ is locally convex. \rex{} \tcr{ref ex about locally convex implies Hausdorff} then implies that $(C_0(X)^*, \MT_{w^*})$ is Hausdorff. Hence $(\MM(X), \MT_{w^*})$ is Hausdorff.
	\end{proof}
	
	\begin{ex}
		\tcr{show (or at least make a note of why) $\MM(X)$ is first-countable, so sequences are sufficient}
	\end{ex}
	...
	\begin{ex} \lex{00000} 
		Let $X$ be a compact Hausdorff topological space and $Y$ a LCH space and $f:X \rightarrow Y$. If $f:X \rightarrow Y$ is continuous, then $f_*: \MM(X) \rightarrow \MM(Y)$ is weak-$*$ continuous. \tcr{can be generalized to when $X$ is LCH and $f$ is proper}
	\end{ex}
	
	\begin{proof}
		Suppose that $f$ is continuous. Let $(\mu_{\al})_{\al \in A} \subset \MM(X)$ be a net and $\mu \in \MM(X)$. Suppose that $\mu_{\al} \conv{w^*} \mu$. Let $\phi \in C_0(Y)$. Since $X$ is compact, $C_0(X) = C(X)$. Therefore $\phi \circ f \in C_0(X)$ and 
		\begin{align*}
			\int_Y \phi \, d f_* \mu_{\al}
			& = \int_X \phi \circ f \dmu_{\al} \\
			& \rightarrow  \int_X \phi \circ f \dmu \\
			& = \int_Y \phi \, d f_* \mu 
		\end{align*}
		Since $\phi \in C_0(Y)$ is arbitrary, we have that for each $\phi \in C_0(X)$, 
		$$\int_Y \phi \, d f_* \mu_{\al} \rightarrow \int_Y \phi \, d f_* \mu .$$ 
		Hence $f_*\mu_{\al} \conv{w^*} f_* \mu$. Since $(\mu_{\al})_{\al \in A} \subset \MM(X)$ a net and $\mu \in \MM(X)$ with $\mu_{\al} \conv{w^*} \mu$ are arbitrary, we have that for each net $(\mu_{\al})_{\al \in A} \subset \MM(X)$ and $\mu \in \MM(X)$, $\mu_{\al} \conv{w^*} \mu$ implies that $f_*\mu_{\al} \conv{w^*} f_* \mu$. Thus $f_*: \MM(X) \rightarrow \MM(Y)$ is weak-* continuous. \tcr{rework using $\sig(\MM(X), C_0(X))$ as in topological vector space section} 
	\end{proof}
	
	\begin{ex}
		Let $X$ be a LCH space and $\mu \in \MM(X)$. Then $C_c(X)$ is dense in $L^p(\mu)$. 
	\end{ex}
	
	\begin{proof}
		Let $E \in \MB(X)$. Suppose that $\mu(E) < \infty$. \tcr{FINISH!!!}
	\end{proof}
	...
	\begin{ex}
		Let $X$ be a LCH space, $(\mu_n)_{n \in \N} \subset \MM(X)$ and $\mu \in \MM(X)$. Suppose that $\sup\limits_{n \in \N} \|\mu_n\| < \infty$. If for each $f \in C_c(X)$, 
		$$\int\limits_X f \dmu_n \rightarrow  \int\limits_X f \dmu,$$ 
		then $\mu_n \conv{w^*} \mu$.  
	\end{ex}
	
	\begin{proof}
		Suppose that for each $f \in C_c(X)$, 
		$$\int\limits_X f \dmu_n \rightarrow  \int\limits_X f \dmu.$$ 
		Since $C_c(X)$ is dense in $C_0(X)$ and $(\mu_n)$ is $\|\cdot\|$-bounded, \tcr{an exercise in the analysis notes banach space chapter duality section} implies that $\mu_n \conv{w^*} \mu$.  
	\end{proof}
	
	
	\begin{defn}
		
		\tcr{new idea: go back a few chapters ot Radon nikodym deriv where we define 'supported on', then define simple measures as those supported on a finite set. Then show that a finite measure $\mu \in M_+(X)$ is simple iff it equals a sum of dirac measures at the finite supporting set. Then here show that for $\mu \in \MM_+(X)$, $\supp(\mu)$ is a smallest closed (in measure) set on which $|\mu|$ is supported or something like that (should be basically a definitional exercise), then denote $\MM_s(X) \defeq \{\text{simple/finite support in $\MM_+(X)$}\}$, might need to assume $\MK(X) \subset \MB(X)$ and $X$ is $T_1$ so that for each $x \in X$, $\del_x \in \MM_1(X)$ (as in prev exercise in intro to radon measures section), (make ex as a corollary of prev ex that if $X$ is $T_1$, then $M_s(X) \subset \MM(X)$ and is a vector space).}
		Let $X$ be a topological space. Suppose that $\MK(X) \subset \MB(X)$.
		\begin{itemize}
			\item Let $\mu \in \MM(X)$. Then $\mu$ is said to be \tbf{simple} if $\# \supp \mu < \infty$.
			\item We define $\MM_s(X) \defeq \{\mu \in \MM(X): \text{$\mu$ is simple}\}$.
		\end{itemize}
	\end{defn}
	...
	\begin{ex}
		Let $X$ be a topological space. Suppose that $\MK(X) \subset \MB(X)$. Let $\mu \in \MM(X)$. Then $\mu \in \MM_s(X)$ iff there exists $n \in \N$, $(x_j)_{j \in [n]} \subset X$ and $(\lam_j)_{j \in [n]} \subset \C$ such that $\mu = \sum\limits_{j \in [n]} \lam_j \del_{x_j}$.
	\end{ex}
	...
	\begin{ex}
		If $X$ is Polish, then $\MM_1(X)$ is separable. 
	\end{ex}
	...
	\begin{ex}
		If $X$ is Polish, then $\MM_1(X)$ is a closed subset of $M(X)$. and $\mu_n \conv{w^*} \mu$
	\end{ex}
	\begin{proof}
	...
	\begin{defn}
		Let $X$ be a topological space. 
		\begin{itemize}
			\item For $f \in C_b(X)$, we define $\lam_f: M(X) \rightarrow \C$ by 
			$$\lam_f(\mu) \defeq \int_X f \dmu$$.
			\item We define the \tbf{weak topology on $M(X)$} or \tbf{vague topology on $M(X)$}, denoted $\MT_w$, by $\MT_w \defeq \tau_{M(X)}(\lam_f: f \in C_0(X))$.  
			\item For $(\mu_{\al})_{\al \in A} \subset M(X)$ a net and $\mu \in M(X)$, we write $\mu_{\al} \conv{w} \mu$ if $\mu_{\al} \rightarrow \mu$ in $(X, \MT_w)$.
		\end{itemize}
	\end{defn}
	...
	\begin{note}\
		\begin{itemize}
			\item 
			\item By definition, $\mu_{\al} \conv{w} \mu$ iff for each $f \in C_b(X)$, 
			$$\int_X f \dmu_{\al} \rightarrow \int_X f \dmu.$$
			\item \tcr{explain naming of weak topology and how it relates to the weak topology of a topological vector space (it doesn't really since $C_b(X) \neq (M(X), \|\cdot\|)^*$ is not the dual), however, it does relate to weak-$*$ convergence if $X$ is nice enough to have a stone cech compactification}
		\end{itemize}
	\end{note}
	
	\tcr{Change $\MM(X)$ to $M(X)$?}
	\begin{defn}
		Let $X$ be a topological space. For $f \in C_b(X)$, define $\lam_f: M(X) \rightarrow \C$ by $$\lam_f(\mu) = \int f \dmu$$.
	\end{defn}
	...
	\newpage
	\section{Projective Systems of Complex Radon Measures}
	\tcr{need to move this section or include sections on other structures like products, subspaces, etc}
	
	\begin{defn} \ld{def:borel_msr:proj_systems:0001}
		Let $(J, {\leq})$ be a directed set $((X_j)_{j \in J}, (\pi_{j,k})_{(j,k) \in \leq})$ a projective system of topological spaces and $(\mu_j)_{j \in J} \in \prod\limits_{j \in J} M(X_j)$. Set $(X, (\pi_j)_{j \in J}) \defeq \varprojlim\limits_{j \in J} ((X_j)_{j \in J}, (\pi_{j,k})_{(j,k) \in \leq})$. Suppose that $(X, (\pi_j)_{j \in J})$ is \tcr{perfect}. For each $j \in J$, we define the \tbf{quotient measure induced by $\pi_j$}, denoted $\mu^Q_j \in M(X^Q_j)$, by $\mu^Q_j \defeq (\bar{\pi}_j^{-1})_*\mu_j$. 
	\end{defn}
	
	\begin{ex} \lex{ex:borel_msr:proj_systems:0002}
	...
	\begin{ex} \lex{ex:borel_msr:proj_systems:0003}
	...
	\tcr{DELETE:}\rex{ex:measurable_spaces:proj_limits:0002}
	
	
	
	\tcr{idea: use this consistency theorem to obtain guassian measure $\mu$ on $\C^{\N}$ with the product sigma algebra where $(\pi_F)_*\mu$ is a gaussian measure on $\C^{|F|}$ for finite subsets $F \subset \N$ with a prescribed mean and covariance function, then try to generalize theorem 2.4 in the book on gaussian measures in hilbert spaces, for rkhs.}
	
	\begin{ex} \lex{ex:borel_msr:proj_systems:0004} \tbf{Kolmogorov Extension Theorem:} \\
		Let $(J, {\leq})$ be a directed set and $((X_j, \MT_j, \mu_j)_{j \in J}, (\pi_{j,k})_{(j,k) \in {\leq}})$ a $\bTopRadMsr$-projective system. Set $X \defeq \varprojlim\limits_{j \in J} X_j$ and $\MT \defeq \varprojlim\limits_{j \in J} T_j$. Suppose that 
		\begin{itemize}
			\item $J$ is countable
		...
		\begin{enumerate}
			...
			\begin{enumerate}
				...
				\tbf{Hint:} sequential compactness
				\item there exists $y_0 \in X$ such that for each $m \in \N$, $\pi_{j_m}(y_0) = y_{0,m}$ and $y_0 \in \bigcap\limits_{m \in \N} B_m$\\
				\tbf{Hint:} diagonal argument 
			\end{enumerate}
			\item $\mu_0$ is a premeasure on $(X, \MA_0)$\\
			\tbf{Hint:} \rex{ex:outer_measures:00007.1} 
			\item there exists a unique measure $\mu \in \MM_+(X)$ such that $((X, \mu), (\pi_j)_{j \in J})$ is a $\bTopRadMsr$-projective limit of $((X_j, \mu_j)_{j \in J}, (\pi_{j,k})_{(j,k) \in {\leq}})$. 
		\end{enumerate}
	\end{ex}
	
	\begin{proof}\
		\begin{enumerate}
			\item \rex{ex:measurable_spaces:proj_limits:0003} implies that $\MA_0$ is an algebra on $X$. By definition, $\sig(\MA_0) = \varprojlim\limits_{j \in J} \MB(X_j, \MT_j)$. \rex{ex:measurable_spaces:proj_limits:0002.1} implies that
			\begin{align*}
				\sig(\MA_0)
				& = \varprojlim\limits_{j \in J} \MB(X_j, \MT_j) \\
				& = \MB\bigg(\varprojlim\limits_{j \in J} X_j, \varprojlim\limits_{j \in J} \MT_j  \bigg)
			...
			\begin{enumerate}
				...
				\begin{itemize}
					\item Let $n \in \N$ and $m \in [n]$. Since $\pi_{j_m}$ is surjective \tcr{(assume)}, 
					\begin{align*}
						\pi_{j_m}(D_n)
						& \subset \pi_{j_m}(C_m) \\
					...
					\begin{align*}
						...
						& < \frac{\del}{2^m}.
					\end{align*}
					Since $\mu_0$ is finitely additive, \rex{?} \tcr{(ref ex here)} implies that $\mu_0$ is finitely-subadditive. Since $(B_n)_{n \in \N}$ is decreasing, we have that for each $n \in \N$,
					\begin{align*}
						...
						& \leq \sum\limits_{m=1}^n \mu_0(B_m \cap C_m^c) \\
						& = \sum\limits_{m=1}^n \mu_0(B_m \setminus C_m) \\
						& < \sum\limits_{m=1}^n \frac{\del}{2^m} \\
						& < \del.
					\end{align*}
				\end{itemize}
				...
				\begin{align*}
					...
					& < \infty, 
				\end{align*}
				\rex{?} \tcr{(ref exercise about finitely additive set functions )} implies
				\begin{align*}
					\mu_0(B_n \setminus D_n)
					& = \mu_0(B_n) - \mu_0(D_n) \\
					& \geq \del - \mu_0(D_n). 
				...
				\begin{itemize} 
					\item Let $j \in J$. \tcb{A previous part} implies that there exists $m \in \N$ such that $j \leq j_m$. Then $m \in A_j$ and $A_j \neq \varnothing$. Since $j \in J$ is arbitrary, we have that for each $j \in J$, $A_j \neq \varnothing$.
					...
					\end{align*}
				\end{itemize}
				Therefore $y_0$ is well-defined. Let $(j,k) \in {\leq}$. \tcb{A previous part} implies that there exists $n \in \N$ such that $k \leq j_n$. Then $n \in A_k$, $n \in A_j$ and 
				\begin{align*}
				...
				\end{align*}
			\end{enumerate} 
			\item Let $(B_n)_{n \in \N} \subset \MA_0$. Suppose that $(B_n)_{n \in \N}$ is decreasing and $\inf\limits_{n \in \N} B_n = \varnothing$. Set $\del \defeq \inf\limits_{n \in \N} \mu_0(B_n)$. For the sake of contradiction, suppose that $\del > 0$. \tcb{The previous parts} imply that $\bigcap\limits_{m \in \N} B_m \neq \varnothing$. This is a contradiction. Hence $\del = 0$. Since $(B_n)_{n \in \N} \subset \MA_0$ such that $(B_n)_{n \in \N}$ is decreasing and $\inf\limits_{n \in \N} B_n = \varnothing$ is arbitrary, we have that for each $(B_n)_{n \in \N} \subset \MA_0$, $(B_n)_{n \in \N}$ is decreasing and $\inf\limits_{n \in \N} B_n = \varnothing$ implies that $\inf\limits_{n \in \N} \mu_0(B_n) = 0$. \rex{ex:outer_measures:00007.1} then implies that $\mu_0$ is a premeasure on $(X, \MA_0)$. 
			\item Set $\MC \defeq \bTopRadMsr$. Since $\sig(\MA_0) = \MB(X, \MT)$ and $\mu_0$ is finite, \rex{ex:outer_measures:00008} implies that there exists a unique $\mu \in M_+(X)$ such that $\mu|_{\MA_0} = \mu_0$. Since for each $j \in J$, $X_j$ is a Polish space, \tcb{an exercise in the analysis notes section on Polish spaces} implies that $X$ is a Polish space. Since 
			\begin{align*}
				\mu(X)
				& = \mu_0(X) \\
				...
				& < \infty, 
			\end{align*}
			\rex{} \tcr{(Ulam's theorem) exercise on radon measures} implies that $\mu \in \MM_+(X)$.
			\begin{itemize}
				...
				\begin{align*}
					...
					& = \mu_j(E).
				\end{align*}
				Since $E \in \MB(X_j, \MT_j)$ is arbitrary, we have that $(\pi_j)_*\mu = \mu_j$. Since $j \in J$ is arbitrary, we have that for each $j \in J$, $(\pi_j)_*\mu = \mu_j$. Hence $(\pi_j)_{j \in J} \in \prod\limits_{j \in J} \Hom_{\MC}((X, \mu), (X_j, \mu_j))$. Since $(X, (\pi_j)_{j \in J})$ is a $\bTop$-projective limit of $((X_j)_{j \in J}, (\pi_{j,k})_{(j,k) \in {\leq}})$, for each $(j,k) \in {\leq}$, 
				\begin{align*}
				...
				\begin{itemize}
					\item \tbf{Existence:} \\
					Since $Y \in \Obj(\bTop)$, $(\tau_j)_{j \in J} \in \prod\limits_{j \in J} \Hom_{\bTop}(Y,X_j)$ and $(X, (\pi_j)_{j \in J})$ is a $\bTop$-projective limit of $((X_j)_{j \in J}, (\pi_{j,k})_{(j,k) \in {\leq}})$, there exists a unique $\phi \in \Hom_{\bTop}(Y, X)$ such that for each $j \in J$, $\pi_j \circ \phi = \tau_j$. Then for each $j \in J$,
					\begin{align*}
						(\pi_j)_*[\phi_*\nu]
						& = (\pi_j \circ \phi)_*\nu \\
						& = (\tau_j)_*\nu \\
					...
					\item \tbf{Uniqueness:} \\
					Clear by uniqueness of $\phi$ in $\bTop$. \tcr{(add details)}
				\end{itemize}
			\end{itemize}
	...
	\begin{ex}
		\tcr{Here make an exercise about $\sup\limits_{j \in J} \mu_j(X_j) < \infty$ iff there exists a complex measure $\mu \in \MM(X)$ such that it gives a $\bTopRadMsrC$ projective limit. Then relate/comment/explore how this is related to the feynman path integral nonexistence, which is why we do a wick rotation.}
	\end{ex}
	...
	\begin{note}
		\tcr{Recall here the definitions from the subsection on products of measurable spaces}
	\end{note}
	
	\begin{ex} \lex{ex:borel_msr:proj_systems:0005}
		Let $(X_j, \mu_j)_{j \in \N} \subset \Obj(\bTopRadMsrone)$. Suppose that for each $j \in \N$, $X_j$ is a Polish space. Let $\mu \in \MM_1( \prod\limits_{j \in \N} X_j)$. Suppose that for each $(A_j)_{j \in \N} \in \prod\limits_{j \in \N} \MB(X_j)$, 
	...
	\begin{ex} \lex{ex:borel_msr:proj_systems:0006}
		Let $(X_j, \mu_j)_{j \in \N} \subset \Obj(\bTopRadMsrone)$. Suppose that for each $j \in \N$, $X_j$ is a Polish space.
	...
	\begin{proof}\
		\begin{itemize}
			\item \tbf{Existence:} \\
			For each $n \in \N$, define $Y_n \defeq \prod\limits_{j=1}^n X_j$. Set $Y \defeq \prod\limits_{j \in \N} X_j$ and for each $n,k \in \N$, if $n \leq k$, define $\tau_{n,k}: Y_k \rightarrow Y_n$ by $\tau_{n,k}((x_j)_{j=1}^k) \defeq (x_j)_{j=1}^n$. For each $n \in \N$, define $\tau_n: Y \rightarrow Y_n$ by $\tau_n((x_j)_{j \in \N}) \defeq (x_j)_{j=1}^n$. \tcr{An exercise} implies that $(Y, (\tau_n)_{n \in \N})$ is a $\bTop$-projective limit of $((Y_n)_{n \in \N}, (\tau_{n,k})_{(n \leq k)})$. For each $n \in \N$, define $\nu_n \in \MM_+(Y_n)$ by $\nu_n \defeq \bigotimes\limits_{j \in [n]} \mu_j$. \tcr{need exercise showing products of Radon measures is Radon, see theorem 7.20 in Folland, I dont think we need to assume that the spaces are LCH, so maybe just cite ulams theorem since any partial product $\nu_n$ is finite on a product of Polish spaces which is Polish.} Then $(Y_n, \nu_n)_{n \in \N} \subset \Obj(\bTopRadMsrone)$. Let $n,k \in \N$. Suppose that $n \leq k$. Let $(A_j)_{j \in [k]} \in \prod\limits_{j=1}^n \MB(X_k)$. Then
			\begin{align*}
			...
			\end{align*}
			\rex{25005} \tcr{(or maybe other exercise)} and \tcr{need exercise about uniqueness of product measure in section on product measures} implies that $\pi_{n,k}^*\nu_k = \nu_n$. Thus $((Y_n, \nu_n)_{n \in \N}, (\tau_{n,k})_{n \leq k})$ is a $\bTopRadMsr$-projective system. \rex{??} \tcr{make exercise about this}	implies that $(Y, (\tau_n)_{n \in \N}) = \varprojlim\limits_{n \in \N} ((Y_n)_{n \in \N}, (\tau_{n, k})_{n \leq k})$ in $\bTop$.  We observe that 
			\begin{itemize}
				\item \tcr{the analysis notes section on products of polish spaces} implies that for each $n \in \N$, $Y_n$ is Polish
				\item by construction, for each $n \in \N$ $\tau_n$ is surjective,
				\item since for each $n \in \N$, $\mu_j(X_j) = 1$, $\sup\limits_{n \in \N} \nu_n(Y_n) = 1$.
			\end{itemize}
			\rex{ex:borel_msr:proj_systems:0004} implies that there exists a unique $\mu \in \MM_+(Y)$ such that $((Y, \mu), (\tau_n)_{n \in \N})$ is a $\bTopRadMsr$-projective limit of $((Y_n, \nu_n)_{n \in \N}, (\tau_{n,k})_{n \leq k})$. Let $(A_j)_{j \in \N} \in \prod\limits_{j \in \N} \MB(X_j)$. For $n \in \N$, define $B_n \in \MB(Y)$ by $B_n \defeq \tau_n^{-1} \bigg( \prod\limits_{j=1}^n A_j \bigg)$. Then for each $n \in \N$, 
			\begin{align*}
				B_{n+1}
			...
			\begin{align*}
				...
				& < \infty. 
			\end{align*}	
			Since $\prod\limits_{j \in \N} A_j = \bigcap\limits_{n \in \N} B_n$, \rex{ex:measures:00004} implies that
			\begin{align*}
			...
			\end{align*}
			\tcr{maybe need some results on infinite products of (positive) real numbers (especially in $[0,1]$) like if order matters, etc so that the last equality is true}
			...
			\item \tbf{Uniqueness:} \\
			\tcr{Alternatively, use previous exercise, and uniqueness from kolmogorov consistency theorem?}
			...
			\end{align*} 
			\tcr{An exercise in the analysis notes section on product topology} implies that $\MB$ is a basis for $\MT_X$ and $\MB$ is countable. 
			
			\tcr{need to show $\MB$ is a $\pi$-system on $X$ and $X \in \MB$ (put this in an exercise in product measurable space section)}. 
			...
			Since $\MB$ is countable, $\MB$ is a $\pi$-system on $X$, $X \in \MB$ and for each $U \in \MB$ $\nu(U) = \mu(U)$, \rex{ex:complex_measures:0007.1} implies that $\mu = \nu$. 
			\tcr{FINISH!!!, see red above}
	...
	\begin{defn}
		Let $(X_j, \mu_j)_{j \in \N} \subset \Obj(\bTopRadMsrone)$. Suppose that for each $j \in \N$, $X_j$ is a Polish space. We define the \tbf{product measure of $(\mu_j)_{j \in \N}$}, denoted $\ot\limits_{j \in \N} \mu_j \in \MM_1( \prod\limits_{j \in \N} X_j)$, to be the unique $\mu \in \MM_1(\prod\limits_{j \in \N} X_ j)$ such that for each $(A_j)_{j \in \N} \in \prod\limits_{j \in \N} \MB(X_j)$, 
		$$\mu \bigg(\prod_{j \in \N} A_j \bigg) = \prod_{j \in \N} \mu_j(A_j).$$
	\end{defn}
	
	
	\tcr{TODO:  Kolmogorov consistency theorem to show the existence of markov kernels like in section 4 of martin harier's notes \href{https://www.hairer.org/notes/Imperial.pdf}{https://www.hairer.org/notes/Imperial.pdf}Just note that the setup might not be the best, since it looks like those notes want to work with the product sigma algebra on $X^{\R}$ where $X$ is polish, but (make an ex, see href in section on product sigma algebras TODO at beginning) since the product sigma alg here is bigger than the product of borel sig alg's, it seems like it might be better to try and restrict to when we can consider $X^{\R}$ as a separable hilbert space and see if we can do something similar on $l^2(\N)$ and then generalize like we do with showing the existence of any gaussian measure with specified mean vector and covariance kernel}. 
	...
	\begin{note}
		Let $X$ be a Hausdorff topological vector space. The analsysis notes topological vector spaces chapter section on compactness implies that
		\begin{itemize}
			\item for each $A \in \Conv(X)$, $\cl A \in \Conv(X)$
			\item for each $K \in \MK(X)$, $\cnv K \in \MK(X)$,
			\item \rex{} \tcr{ex in analysis notes section on locally convex spaces} implies that if $X$ is locally convex, then for each $E \subset X$, $E$ is precompact implies that $\cnv E$ is precompact
			\item \rex{ex:introduction_radon_measures:0002.01} implies that $\{\del_x: x \in X\} \subset \MM_1(X)$. 
		\end{itemize}
	\end{note}
	...
	\begin{note}
		\tcr{note the following results} \rex{ex:introduction_radon_measures:0002.01}, 
		we pull from ~\cite{phelps2002lectures}.
	\end{note}
	
	\begin{defn}
		Let $X$ be a locally convex space, $x \in X$ and $\mu \in \MM_1^c(X)$. Then $\mu$ is said to \tbf{represent $x$} if for each $\phi \in X^*$, $\phi(x) = \mu(\phi)$.  
	\end{defn}
	
	\begin{ex}
		Let $X$ be a locally convex space. Then for each $x \in X$, $\del_x$ represents $x$. 
	\end{ex}
	
	\begin{proof}
		Let $x \in X$. Then for each $\phi \in X^*$, 
		\begin{align*}
			\del_x(\phi)
			& = \int_X \phi \del_x \\
			& = \phi(x).
		\end{align*}
		Thus $\del_x$ represents $x$. Since $x \in X$ is arbitrary, we have that for each $x \in X$, $\del_x$ represents $x$. 
	\end{proof}
	
	\begin{defn} 
		Let $X$ be a locally convex space $E \in \MK(X)$, $x \in X$ and $\mu \in \MM_1(E)$. Then $\mu$ is said to \tbf{represent $x$} if $(\iota_E)_*\mu$ represents $x$. 
	\end{defn}
	
	\begin{ex}
		Let $X$ be a locally convex space $E \in \MK(X)$, $x \in X$ and $\mu \in \MM_1(E)$. Then $\mu$ represents $x$ iff for each $\phi \in X^*$, $\mu(\phi|_E) = \phi(x)$.
	\end{ex}
	
	\begin{proof}\
		\begin{itemize}
			\item $(\implies)$: \\
			Suppose that $\mu$ represents $x$. Then for each $\phi \in X^*$,
			\begin{align*}
				\mu(\phi|_E)
				& = \int \phi|_E \dmu \\
				& = \int \phi \circ \iota_E \dmu \\
				& = \int \phi \, d (\iota_E)_*\mu \\
				& = (\iota_E)_*\mu(\phi) \\
				& = \phi(x) \quad \text{(by \tcr{defn})}.
			\end{align*}
			\item $(\impliedby)$: \\
			Suppose that for each $\phi \in X^*$, $\mu(\phi|_E) = \phi(x)$. Then for each $\phi \in X^*$, 
			\begin{align*}
				(\iota_E)_*\mu(\phi)
				& = \int \phi \, d (\iota_E)_*\mu \\
				& = \int \phi \circ \iota_E \dmu \\
				...
				& = \phi(x). 
			\end{align*}
			Thus $\mu$ represents $x$. 
		\end{itemize}
	\end{proof}
	
	\begin{ex}
		Let $X$ be a locally convex space, $E \in \MK(X)$ and $\mu \in \MM_1(X)$. Define $F \in \MK(X) \cap \Conv(X)$ by $F \defeq \cl (\cnv E)$. Suppose that $\supp \mu \subset E$. 
		\begin{enumerate}
			\item For each $\phi \in X^*$, define $H_{\phi} \subset F$ by $H_{\phi} \defeq F \cap \phi^{-1}(\{\mu(\phi)\})$. Then for each $(\phi_j)_{j \in [n]} \subset X^*$, $\bigcap\limits_{j \in[n]} H_{\phi_j} \neq \varnothing$ implies that $\bigcap\limits_{\phi \in X^*} H_{\phi} \neq \varnothing$. \\
			\tbf{Hint:} compactness
			\item Let $(\phi_j)_{j \in [n]} \subset X^*$. Define $y \in \R^n$ by $y \defeq (\mu(\phi_j))_{j \in [n]}$. Then there exists $T \in L(X, \R^n)$ such that $\bigcap\limits_{j \in [n]} H_{\phi_j} = T^{-1}(\{y\}) \cap F$ and $\bigcap\limits_{j \in [n]} H_{\phi_j} \neq \varnothing$ iff $y \in T(F)$. 
			\item If $y \not \in T(F)$, then there exists $a \in \R^n$ such that $\sup\limits_{x \in F} \l a, T(x) \r  < \l a, y \r$. \\
			\tbf{Hint:} \tcr{analysis notes section on convex Hahn-Banach Separation Theorem 2}
			\item There exists a unique $x \in F$ such that $\mu$ represents $x$. \\
			\tbf{Hint:} consider $g \in X^*$ defined by $g \defeq \sum\limits_{j \in [n]} a_j \phi_j$
		\end{enumerate}
	\end{ex}
	
	\begin{proof}\
		\tcr{cover relative complements in subset section of set theory notes} 
		\begin{enumerate}
			\item Suppose that $\bigcap\limits_{\phi \in X^*} H_{\phi} = \varnothing$. Define $H \subset F$ by $H \defeq \bigcap\limits_{\phi \in X^*} H_{\phi}$. 
			\begin{itemize}
				\item Let $\phi \in X^*$. Since $\{\mu(\phi)\}$ is closed in $\R$, we have that $\phi^{-1}(\{\mu(\phi)\})$ is closed in $X$. Since $H_{\phi} = F \cap \phi^{-1}(\{\mu(\phi)\})$ and $\phi^{-1}(\{\mu(\phi)\})$ is closed in $X$, we have that $H_{\phi}$ is closed in $F$. Hence $F \setminus H_{\phi}$ is open in $F$. Since $\phi \in X^*$ is arbitrary, we have that for each $\phi \in X^*$, $F \setminus H_{\phi}$ is open in $F$. 
				\item Since $H = \varnothing$, we have that 
				\begin{align*}
					\bigcup\limits_{\phi \in X^*} (F \setminus H_{\phi})
					& = F \setminus \bigg[ \bigcap\limits_{\phi \in X^*} H_{\phi} \bigg] \\
					& = F \setminus H \\
					& = F \setminus \varnothing \\
					& = F.
				\end{align*}
				Thus $(F \setminus H_{\phi})_{\phi \in X^*}$ is an open cover of $F$. Since $F \in \MK(X)$, there exists $(\phi_j)_{j \in [n]}$ such that $(F \setminus H_{\phi_j})_{j \in [n]}$ is an open cover of $F$. Hence 
				\begin{align*}
					\varnothing
					& = F \setminus F \\
					& = F \setminus \bigg[ \bigcup\limits_{j \in [n]} (F \setminus H_{\phi_j}) \bigg] \\
					& = \bigcap\limits_{j \in [n]} [F \setminus (F \setminus H_{\phi_j})] \\
					& = \bigcap\limits_{j \in [n]} H_{\phi_j}.
				\end{align*} 
			\end{itemize}
			Therefore 
			$$(\text{$\bigcap\limits_{\phi \in X^*} H_{\phi} = \varnothing$}) \implies \bigg(\text{there exists $(\phi_j)_{j \in [n]}$ such that $\bigcap\limits_{j \in [n]} H_{\phi_j} = \varnothing$} \bigg)$$
			By contrapositive, 
			$$\bigg(\text{for each $(\phi_j)_{j \in [n]}$, $\bigcap\limits_{j \in [n]} H_{\phi_j} \neq \varnothing$} \bigg) \implies (\text{$\bigcap\limits_{\phi \in X^*} H_{\phi} \neq \varnothing$}).$$
			\item Define $T \in L(X;\R^n)$ by $T \defeq (\phi_j)_{j=1}^n$. Then 
			\begin{align*}
				x \in T^{-1}(\{y\}) \cap F 
				& \iff \text{$T(x) = y$ and $x \in F$} \\
				& \iff \text{$(\phi_j(x))_{j \in [n]} = (\mu(\phi_j))_{j \in [n]}$ and $x \in F$} \\
				& \iff \text{$x \in F$ and for each $j \in [n]$, $\phi_j(x) = \mu(\phi_j)$} \\
				& \iff \text{$x \in F$ and for each $j \in [n]$, $x \in \phi_j^{-1}(\{\mu(\phi_j)\})$} \\
				& \iff \text{for each $j \in [n]$, $x \in F$ and $x \in \phi_j^{-1}(\{\mu(\phi_j)\})$} \\
				& \iff \text{for each $j \in [n]$, $x \in F \cap \phi_j^{-1}(\{\mu(\phi_j)\})$} \\
				& \iff \text{for each $j \in [n]$, $x \in H_{\phi_j}$} \\
				& \iff x \in \bigcap\limits_{j \in [n]} H_{\phi_j}.
			\end{align*}
			Hence $T^{-1}(\{y\}) \cap F = \bigcap\limits_{j \in [n]} H_{\phi_j}$. Therefore
			\begin{align*}
				\bigcap\limits_{j \in [n]} H_{\phi_j} \neq \varnothing
				& \iff T^{-1}(\{y\}) \cap F \neq \varnothing \\
				& \iff \text{there exists $x \in X$ such that $x \in T^{-1}(\{y\}) \cap F $} \\
				& \iff \text{there exists $x \in F$ such that $x \in T^{-1}(\{y\})$} \\
				& \iff \text{there exists $x \in F$ such that $T(x) = y$} \\
				& \iff y \in T(F).
			\end{align*}
			\item Suppose that $y \not \in T(F)$. Since $F \in \MK(X) \cap \Conv(X)$ and $T \in L(X, \R^n)$, $T(F) \in \MK(\R^n) \cap \Conv(\R^n)$. Since $T(F) \in \MK(\R^n)$ and $\{y\}$ is closed, \tcr{Hahn-Banach Separation Theorem 2} implies that there exists $\lam \in (\R^n)^*$ such that $\sup\limits_{x \in F} \lam(T(x)) < \lam(y)$. \tcr{The Riesz representation theorem} implies that there exists $a \in \R^n$ such that $\lam = \l a, \cdot \r$. Then $\sup\limits_{x \in F} \l a, T(x) \r < \l a, y \r$.
			\item 
			\begin{itemize}
				\item \tbf{Existence:} \\
				For the sake of contradiction, suppose that $y \not \in T(F)$. Write $a = (a_{j})_{j \in [n]}$ and define $g \in X^*$ by $g \defeq \sum\limits_{j \in [n]} a_j \phi_j$. Then 
				\begin{align*}
					\sup\limits_{x \in F} g(x)
					& = \sup\limits_{x \in F} \sum\limits_{j \in [n]} a_j \phi_j(x) \\
					& = \sup\limits_{x \in F} \sum\limits_{j \in [n]} \l a, T(x) \r \\
					& < \l a, y \r \\
					& = \sum\limits_{j \in [n]} a_j \mu(\phi_j) \\
					& = \mu\bigg( \sum\limits_{j \in [n]} a_j \phi_j \bigg) \\
					& = \mu (g) \\
					& = \int_X g \dmu \\
					& = \int_E g \dmu \quad \text{(since $\supp \mu \subset E$)} \\
					& \leq \int_E \bigg[ \sup\limits_{x \in E} g(x) \bigg] \dmu \\
					& = \bigg[ \sup\limits_{x \in E} g(x) \bigg] \mu(E) \\
					& = \sup\limits_{x \in E} g(x) \quad \text{(since $\mu(E) = 1$)} \\
					& \leq \sup\limits_{x \in F} g(x) \quad \text{(since $E \subset F$)},
				\end{align*} 
				which is a contradiction. Hence $y \in T(F)$. Since 
				\begin{align*}
					y \in T(F)
					& \iff \bigcap\limits_{j \in [n]} H_{\phi_j} \neq \varnothing \\
					& \implies \bigcap\limits_{\phi \in X^*} H_{\phi} \neq \varnothing \\
					& \iff \text{there exists $x \in X$ such that $x \in  \bigcap\limits_{\phi \in X^*} H_{\phi}$} \\
					& \iff \text{there exists $x \in X$ such that $x \in  \bigcap\limits_{\phi \in X^*} F \cap \phi^{-1}(\{\mu(\phi)\})$} \\
					& \iff \text{there exists $x \in X$ such that $x \in F \cap \bigg[ \bigcap\limits_{\phi \in X^*} \phi^{-1}(\{\mu(\phi)\}) \bigg]$} \\
					& \iff \text{there exists $x \in X$ such that $x \in F$ and for each $\phi \in X^*$, $x \in \phi^{-1}(\{\mu(\phi)\})$} \\
					& \iff \text{there exists $x \in F$ such that for each $\phi \in X^*$, $x \in \phi(x) = \mu(\phi)$} \\
					& \iff  \text{there exists $x \in F$ such that $\mu$ represents $x$},
				\end{align*}
				we have that there exists $x \in F$ such that $\mu$ represents $x$.
				\item \tbf{Uniqueness:} \\
				Suppose that there exists $x' \in F$ such that $\mu$ represents $x'$. Then for each $\phi \in X^*$
				\begin{align*}
					\phi(x - x')
					& = \phi(x) - \phi(x') \\
					& = \mu(\phi) - \mu(\phi) \\
					& = 0.
				\end{align*}
				Since $X$ is locally convex, \tcr{ref ex in analysis notes section on locally convex spaces} implies that $X^*$ separates the points of $X$. Therefore $x-x' = 0$ and $x = x'$. Since $x' \in F$ with $\mu$ represents $x'$ is arbitrary, we have that for each $x' \in F$, $\mu$ represents $x'$ implies that $x' = x$. 
			\end{itemize}
		\end{enumerate}
	\end{proof}
	...
	\begin{ex}
		Let $X$ be a locally convex space and $E \in \MK(X)$. Define $F \in \MK(X) \cap \Conv(X)$ by $F \defeq \cl (\cnv E)$. Then there exists $f: \MM_1(E) \rightarrow F$ such that 
		\begin{enumerate}
			\item for each $\mu \in \MM_1(E)$, $\mu$ represents $f(\mu)$. 
			\item $f$ is $(\MT_{w^*}, \MT_X)$-continuous.
		\end{enumerate}
	\end{ex}
	
	\begin{proof}\
		\begin{enumerate}
			\item For each $\mu \in \MM_1(E)$, define $K_{\mu} \subset X$ by $K_{\mu} \defeq \{x \in F: \text{$\mu$ represents $x$}\}$. 
			
			\tcr{The prev ex} implies that for each $x \in F$, $\# K_{\mu} = 1$. \tcr{exercise in set theory notes on products implies that} there exists a unique $f: \MM_1(E) \rightarrow X$ such that for each $\mu \in \MM_1(X)$, $f(\mu) \in K_{\mu}$. 
			\item \tcr{need exercise showing that $(\MM_1(E), \MT_{w^*})$ is first-countable (note make ex in analysis notes that locally convex spaces are first countable iff they are metrizable), i.e. need to show that $(\MM_1(E), \MT_{w^*})$ is metrizable, see weak* metrizability of unit ball of dual of banach space. In the Banach space setting, the idea is easy, if $X$ is separable, then $\MT_{w^*}$ on $X^*$, should be generated by the countable family of seminorms $|\hat{x_n}|:n \in \N$ and is therefore metrizable, now just take $X = \MC(E)$, $X^* = \MM(E)$ (Alaglu says $B_{X^*}(0,1)$ is weak-* compact and since $\MM_1(E)$ is a weak-* closed  subset (check), it is also weak-* compact) and just show that $C(E)$ is separable, which should be a theorem you can find in folland. Note space must be metrizable \href{https://proofwiki.org/wiki/Space_of_Complex-Valued_Continuous_Functions_on_Compact_Hausdorff_Space_is_Separable_iff_Space_is_Metrizable}{cont funct is sep iff space is metrizable}}. Let $(\mu_n)_{n \in \N} \subset \MM_1(X)$ and $\mu \in \MM_1(X)$. Suppose that $\mu_n \conv{w^*} \mu$. Define $(x_n)_{n \in \N} \subset X$ and $x \in X$ by $x_n \defeq f(\mu_n)$ and $x \defeq f(\mu)$. Then for each $\phi \in X^*$, 
			\begin{align*}
				\phi(x_n)
				& = \mu_n(\phi|_E) \\
				& \rightarrow \mu(\phi|_E) \\
				& = \phi(x).
			\end{align*}
			Thus $x_n \conv{w} x$. Since $X$ is a locally convex space, $E \in \MK(X)$, $(x_n)_{n \in \N} \subset E$ and $x \in E$, \tcr{ref ex in analysis notes section on duality implies} $x_n \rightarrow x$. Since $(\mu_n)_{n \in \N} \subset \MM_1(X)$ and $\mu \in \MM_1(X)$ are arbitrary, we have that for each $(\mu_n)_{n \in \N} \subset \MM_1(X)$ and $\mu \in \MM_1(X)$, $\mu_n \conv{w^*} \mu$ implies that $f(\mu_n) \rightarrow f(\mu)$. Thus $f$ is $(\MT_{w^*}, \MT_X)$-continuous.
		\end{enumerate}
	\end{proof}
	
	
	\begin{note}
		\tcr{I think you can relax the prev exercise to get $f: \MM_{1,c}(X) \rightarrow X$ i.e. probability measures with compact support. If $\mu_n \conv{w^*} \mu$, then there should be a compact set $E \subset X$ such that $\supp \mu_n \subset E$, then do rest of the exercise}
	\end{note}
	
	\begin{ex}
		Let $X$ be a locally convex space and $E \in \MK(X)$. Define $F \in \MK(X) \cap \Conv(X)$ by $F \defeq \cl (\cnv E)$. Then for each $x \in X$, $x \in F$ iff there exists $\mu \in \MM_1(E)$ such that $\mu$ represents $x$. \\
		\tbf{Hint:} Hahn-Banach Separation Theorem 2
	\end{ex}
	...
	\begin{proof}
		...
		\begin{itemize}
			\item $(\implies)$: \\
			Suppose that $x \in F$. Since $F = \cl (\cnv E)$, there exists a net $(x_{\al})_{\al \in A} \subset \cnv E$ such that $x_{\al} \rightarrow x$. Then for each $\al \in A$, there exists $n_{\al} \in \N$, $(\lam_{\al, j})_{j \in [n_{\al}]} \subset \Del^{n_{\al} - 1}$ and $(x_{\al, j})_{j \in [n_{\al}]} \subset E$ such that $x_{\al} = \sum\limits_{j \in [n_{\al}]} \lam_{\al, j} x_{\al, j}$. Define $(\mu_{\al})_{\al \in A} \subset \MM_1(E)$ by $\mu_{\al} \defeq \sum\limits_{j \in [n_{\al}]} \lam_{\al, j} \del_{x_{\al, j}}$. Since $E \in \MK(X)$, \tcr{Riesz Kakutani rep theorem} implies that $C(E)^* \cong_{\bBan} \MM(E)$. \tcr{ref/make ex in Radon Measure section using Banach Alaoglu theorem} implies that $\MM_1(E)$ is $w^*$-compact. \tcr{An ex in topology section on compactness on subspaces} implies that there exists a subnet $(\mu_{\al_{\be}})_{\be \in B} \subset (\mu_{\al})_{\al \in A}$ and $\mu \in \MM_1(E)$ such that $\mu_{\al_{\be}} \conv{w^*} \mu$. Let $\phi \in X^*$. Since $x_{\al} \rightarrow x$, \tcr{ref ex on subnets} then implies that  $x_{\al_{\be}} \rightarrow x$. Thus $\phi(x_{\al_{\be}}) \rightarrow \phi(x)$. Since $\mu_{\al_{\be}} \conv{w^*} \mu$, we have that 
			\begin{align*}
				\phi(x_{\al_{\be}}) 
				& = \phi \bigg( \sum\limits_{j \in [n_{\al_{\be}}]} \lam_{\al_{\be}, j} x_{\al_{\be}, j} \bigg) \\
				& = \sum\limits_{j \in [n_{\al_{\be}}]} \lam_{\al_{\be}, j} \phi(x_{\al_{\be}, j}) \\
				& = \sum\limits_{j \in [n_{\al_{\be}}]} \lam_{\al_{\be}, j} \phi|_E(x_{\al_{\be}, j}) \\
				& = \int \phi|_E \dmu_{\al_{\be}} \\
				& \rightarrow \int \phi|_E \dmu \\
				& = \mu(\phi|_E).
			\end{align*}
			Since $\phi(x_{\al_{\be}}) \rightarrow \phi(x)$ and $\phi(x_{\al_{\be}}) \rightarrow \mu (\phi|_E)$, we have that $\phi(x) = \mu (\phi|_E)$. Since $\phi \in X^*$ is arbitrary, we have that for each $\phi \in X^*$, $\phi(x) = \mu (\phi|_E)$. Hence $\mu$ represents $x$.  
			\item $(\impliedby)$: \\
			Suppose that there exists $\mu \in \MM_1(E)$ such that $\mu$ represents $x$. For the sake of contradiction, suppose that $x \not \in F$. Since $\mu(E) = 1$, $E \neq \varnothing$. Since $E \subset F$, $F \neq \varnothing$. We note that  \begin{itemize}
				\item $\{x\} \neq \varnothing$ and $F \neq \varnothing$
				\item $\{x\}, F \in \Conv(X) \cap \MK(X)$ and thus $F$ is closed.
				\item $\{x\} \cap F = \varnothing$.
			\end{itemize}
			The \tcr{Hahn-Banach Separation Theorem 2} implies that there exists $\phi \in X^*$ such that $\sup \phi(F) < \phi(x)$. Therefore 
			\begin{align*}
				\phi(x)
				& = \mu(\phi|_E) \\
				& = \int \phi|_E \dmu \\
				& \leq [\sup \phi(E)] \mu(E) \\
				& = \sup \phi(E) \\
				& \leq \sup \phi(F) \quad \text{(since $E \subset F$)} \\
				& < \phi(x), 
			\end{align*}
			which is a contradiction. Hence $x \in F$. 
		\end{itemize}
	\end{proof}
	
	\begin{ex}
		Let $X$ be a locally convex space, $E \in \MK(X) \cap \Conv(X)$, $\mu \in MM_1(E)$ and $x \in X$. If $\mu$ represents $x$, then $x \in E$. 
	\end{ex}
	...
	\begin{ex}  \tcr{move to radon section}
		Let $X$ be a topological space. Suppose that $\MK(X) \subset \MB(X)$ and $X$ is $T_1$. Let $x \in X$ and $\mu \in \MM_1(X)$. Then $\mu = \del_x$ iff $\supp \mu = \{x\}$. 
	\end{ex}
	
	\begin{proof}\
		\begin{itemize}
			\item $(\implies)$: \\
			Suppose that $\mu = \del_x$. We note that $\{x\}^c \subset \MN_{\mu}$ and for each $U \in \MN_{\mu}$, $U \subset \{x\}^c$. Thus 
			\begin{align*}
				N_{\mu}
				& = \bigcup\limits_{U \in \MN_{\mu}} U \\
				& = \{x\}^c.
			\end{align*}
			Hence 
			\begin{align*}
			...
			\end{align*}
			\item $(\impliedby)$: \\
			Suppose that $\supp \mu = \{x\}$. Let $E \in \MB(X)$. \rex{ex:introduction_radon_measures:0004} implies that 
			\begin{align*}
				0
				& \leq \mu(E \cap N_{\mu}) \\
				& \leq \mu(N_{\mu}) \\
				& = 0.
			\end{align*}
			Hence $\mu(E \cap N_{\mu}) = 0$. If $x \in E$, then 
			\begin{align*}
				\mu(E)
				& = \mu(E \cap \{x\}) + \mu(E \cap \{x\}^c) \\
				& = \mu(\{x\}) + \mu(E \cap [\supp \mu]^c) \\
				& = \mu(\supp \mu) + \mu(E \cap N_{\mu}) \\
				& = \mu(\supp \mu) \\
				& = \mu(X) \\
				& = 1.
			\end{align*}
			...
			\begin{align*}
				\mu(E)
				& = \mu(E \cap \{x\}) + \mu(E \cap \{x\}^c) \\
				& = \mu(\varnothing) + \mu(E \cap [\supp \mu]^c) \\
				& = \mu(\varnothing) + \mu(E \cap N_{\mu}) \\
				& = 0.
			\end{align*}
			Since $E \in \MB(X)$ is arbitrary, we have that for each $E \in \MB(X)$, 
			\begin{align*}
	...
	\begin{ex} \tcr{move to radon section}
		Let $X$ be a topological space. Suppose that $\MK(X) \subset \MB(X)$. Let $K \in \MK(X)$ and $\mu \in \MM_+(X)$. If $\mu(K) > 0$, then there exists $x \in K$ such that for each $U \in \opn{\MN}(x)$, $\mu(U) > 0$ 
	\end{ex}
	
	\begin{proof}
		Suppose that $\mu(K) > 0$. For the sake of contradiction, suppose that for each $x \in K$, there exists $U \in \opn{\MN}(x)$ such that $\mu(U) = 0$. \tcr{The axiom of choice} implies that there exists $(U_x)_{x \in K} \in \opn{\Cover}(K)$ such that for each $x \in K$, $\mu(U_x) = 0$. Since $K$ is compact, there exists $n \in \N$ and $(x_j)_{j \in [n]} \subset K$ such that $(U_{x_j})_{j \in [n]} \in \opn{\Cover}(K)$. Therefore
		\begin{align*}
			0 
			& < \mu(K) \\
			& \leq \mu \bigg( \bigcup\limits_{j \in [n]} U_{x_j} \bigg) \\
			...
			& = 0,
		\end{align*}
		which is a contradiction. Hence there exists $x \in K$ such that for each $U \in \opn{\MN(x)}$, $\mu()$
	\end{proof}
	
	\begin{ex}
		Let $X$ be a locally convex space, $E \in \MK(X) \cap \Conv(X)$ and $x \in E$. Then $x \in \extrm E$ iff $\rep_E(x) = \{\del_x\}$. 
	\end{ex}
	
	\begin{proof}\
		\begin{itemize}
			\item $(\implies)$: \\
			Suppose that $x \in \extrm E$. Let $\mu \in \rep_E(x)$. Let $K_1 \in \MK(E)$. Suppose that $K_1 \subset E \setminus \{x\}$. For the sake of contradiction, suppose that $\mu(K_1) > 0$.  
			\item $(\impliedby)$: \\
		\end{itemize}
	\end{proof}
	...
	\newpage
	\chapter{Differentiation of Radon Measures on Metric Spaces}
	
	\section{Covering Lemmas and Vitali Measures}
	
	\subsection{Vitali Measures}
	
	\tcb{Add Besicovitch Covering Lemma}
	
	\begin{note}
		We make use of some results about maps $\MP: \N \rightarrow \Part(X)$ which are decreasing. See the \tcr{section on ultrametric spaces in the analysis notes for details}. 
	\end{note}
	
	\begin{defn} \ld{def:diff_radon_measures:covering_lemmas:vitali_measures:0001}
		...
		Let $A \subset X$. 
		\begin{itemize}
			\item Let $\MV \subset \bar{\MB}_d$. Then $\MV$ is said to be a \tbf{$d$-centered covering} of $A$ if for each $x \in A$, there exists $r > 0$ such that $\bar{B}_d(x, r) \in \MV$. We define 
			$$\MC_d(A) \defeq \{ \MV \subset \bar{\MB}_d: \MV \text{ is a $d$-centered covering of } A\}$$
			\item Let $\MV \in \MC_d(A)$ and $x \in A$. Then $\MV$ is said to be \tbf{$d$-fine} at $x$ if 
			$$\inf \{r > 0 : \bar{B}_d(x,r) \in \MV\} = 0$$
			\item Let $\MV \in \MC_d(A)$. Then $\MV$ is said to be \tbf{$d$-fine on $A$} if for each $x \in A$, $\MV$ is $d$-fine at $x$.
		\end{itemize}
	\end{defn}
	
	\begin{defn} \ld{def:diff_radon_measures:covering_lemmas:vitali_measures:0002}
		Let $(X, d)$ be a metric space and $\mu \in M_+(X)$. Then $\mu$ is said to be \tbf{$d$-Vitali} if for each $A \subset X$ and $\MV \in \MC_d(A)$, $\MV$ is $d$-fine on $A$ implies that there exists $\MF \subset \MV$ such that 
		\begin{enumerate}
	...
	\begin{ex} \lex{ex:diff_radon_measures:covering_lemmas:vitali_measures:0003}
		Let $(X, d)$ be a metric space, $\phi: \Rg \rightarrow \Rg$ and $\mu \in M_+(X)$. Suppose that $\phi$ is strictly increasing and $\bTop$-metric preserving. Then $\mu$ is $d$-Vitali iff $\mu$ is $d^{\phi}$-Vitali.
	\end{ex}
	
	\begin{proof}
		Set $d' \defeq d^{\phi}$, Since $\phi$ is increasing and $\bTop$-metric preserving, \tcr{an exercise in the anlysis notes section on metric eqivalent metrics} implies that 
		\begin{itemize}
			\item $\phi^{-1}(\{0\}) = \{0\}$
			\item $\phi$ is subadditive
			\item $\phi$ is continuous
		\end{itemize}
		Since $\phi$ is strictly increasing \tcr{an ex in the anlysis notes section on metric eqivalent metrics} implies that for each $x \in X$ and $r >0$, $\bar{B}_d(x, r) =  \bar{B}_{d'}(x,\phi(r))$ and $\bar{\MB}_d \subset \bar{\MB}_{d'}$.
		\begin{itemize}
			\item $(\implies)$: \\
			Suppose that $\mu$ is $d$-Vitali. Let $A \subset X$ and $\MV' \subset \MC_{d'}(A)$. Suppose that $\MV'$ is $d'$-fine on $A$. Define $\MV \subset \bar{\MB}_d$ by 
			$$\MV \defeq \{\bar{B}_d(x, r): \text{$x \in X$, $r >0$ and $\bar{B}_{d'}(x, \phi(r)) \in \MV'$}\}.$$ 
			Then for each $x \in X$, $r > 0$, $\bar{B}_d(x, r) \in \MV$ iff $\bar{B}_{d'}(x, \phi(r)) \in \MV'$.
			\begin{itemize}
				\item Let $x \in A$. Since $\phi$ is continuous and $\phi^{-1}(\{0\}) = \{0\}$, there exists $\ep > 0$ such that $(0, \ep) \subset \phi((0, \infty))$ \tcr{need to ref ex here, make ex about connectedness and continuity}. Since $\MV'$ is $d'$-fine on $A$, $\inf \{r' > 0: \bar{B}_{d'}(x, r') \in \MV'\} = 0$. Thus there exists $r' > 0$ such that $\bar{B}_{d'}(x, r') \in \MV'$ and $r' < \ep$. Since $(0, \ep) \subset \phi((0, \infty))$, there exists $r > 0$ such that $\phi(r) = r'$. By definition, $\bar{B}_d(x, r) \in \MV$. Since $x \in A$ is arbitrary, we have that for each $x \in A$, there exists $r > 0$, such that $\bar{B}_d(x, r) \in \MV$. Thus $\MV \in \MC_d(A)$.
				\item Let $\del >0$ and $x \in A$. As \tcr{in the prev part}, there exists $\ep > 0$ such that $(0, \ep) \subset \phi((0, \infty))$. Since $\MV'$ is $d'$-fine on $A$, $\inf \{r' > 0: \bar{B}_{d'}(x, r') \in \MV'\} = 0$. Thus there exists $r' > 0$ such that $\bar{B}_{d'}(x, r') \in \MV'$ and $r' < \min(\phi(\del), \ep)$. Since $(0, \ep) \subset \phi((0, \infty))$, there exists $r > 0$ such that $\phi(r) = r'$. By definition, $\bar{B}_d(x, r) \in \MV$. Thus 
				\begin{align*}
					0
					& \leq \inf \{r > 0: \bar{B}_d(x, r) \in \MV\} \\
					& \leq r' \\
					& < \del. 
				\end{align*}
				Since $\del > 0$ is arbitrary, we have that $\inf \{r > 0: \bar{B}_d(x, r) \in \MV\} = 0$. Thus $\MV$ is $d$-fine at $x$. Since $x \in A$ is arbitrary, we have that for each $x \in A$, $\MV$ is fine at $x$. Therefore $\MV$ is fine on $A$.
				\item Since $\mu$ is $d$-Vitali, there exists $\MF \subset \MV$ such that $\MF$ is countable, $\MF$ is disjoint and $\MF$ $\mu^*$-covers $A$. Define $\MF' \subset \bar{\MB}_{d'}$ by 
				$$\MF' \defeq \{\bar{B}_{d'}(x, \phi(r)): \text{$x \in X$, $r >0$ and $\bar{B}_d(x,r) \in \MF$}\}.$$ 
				\begin{itemize}
					\item Let $S \in \MF'$. Then there exists $x \in X$ and $r > 0$ such that $S = \bar{B}_{d'}(x,\phi(r))$ and $\bar{B}_d(x,r) \in \MF$. Since $\bar{B}_d(x,r) = \bar{B}_{d'}(x, \phi(r))$, we have that
					\begin{align*}
						S 
						& = \bar{B}_{d'}(x,\phi(r)) \\
						& \bar{B}_d(x,r) \\
						& \in \MF. 
					\end{align*}
					Since $S \in \MF'$ is arbitrary, we have that for each $S \in \MF'$, $S \in \MF$. Hence $\MF' \subset \MF$. 
					\item Let $S \in \MF$. Since $\MF \subset \MV$, there exists $x \in X$ and $r > 0$ such that $S = \bar{B}_d(x, r)$ and $\bar{B}_{d'}(x,\phi(r)) \in \MV'$. Since $\bar{B}_d(x,r) = \bar{B}_{d'}(x, \phi(r))$, we have that 
					\begin{align*}
						S
						& = \bar{B}_d(x, r) \\
						& = \bar{B}_{d'}(x, \phi(r)) \\
						& \in \MF' \quad \text{(by definition of $\MF'$)}.
					\end{align*}
					Since $S \in \MF$ is arbitrary, we have that for each $S \in \MF$, $S \in \MF'$. Hence $\MF \subset \MF'$.
				\end{itemize}
				Since $\MF' \subset \MF$ and $\MF \subset \MF'$, we have that $\MF' = \MF$. 
				\begin{itemize}
					\item Since for each $x \in X$ and $r>0$, 
					\begin{align*}
						\bar{B}_{d'}(x, \phi(r)) \in \MF'
						& \iff \bar{B}_d(x,r) \in \MF \\
						& \implies \bar{B}_d(x,r) \in \MV \\
						& \iff \bar{B}_{d'}(x, \phi(r)) \in \MV' \quad \text{(by definition of $\MV$),}
					\end{align*}
					we have that $\MF' \subset \MV'$.
					\item Since $\MF$ is countable and $\MF = \MF'$, we have that $\MF'$ is countable. 
					\item Since $\MF$ is disjoint and $\MF = \MF'$, we have that $\MF'$ is disjoint.
					\item Since $\MF$ $\mu^*$-covers $A$ and $\MF = \MF'$, we have that $\MF'$ $\mu^*$-covers $A$.
				\end{itemize}
			\end{itemize}
			Since $A \subset X$ and $\MV' \subset \MC_{d'}(A)$ such that $\MV'$ is $d'$-fine on $A$ are arbitrary, we have that for each $A \subset X$ and $\MV' \subset \MC_{d'}(A)$, $\MV'$ is $d'$-fine on $A$ implies that there exists $\MF' \subset \MV'$ such that $\MF'$ is countable, $\MF'$ is disjoint and $\MF'$ $\mu^*$-covers $A$. Hence $\mu$ is $d'$-Vitali.
			\item $(\impliedby)$: \\
			Suppose that $\mu$ is $d'$-Vitali. Let $A \subset X$ and $\MV \subset \MC_d(A)$. Suppose that $\MV$ is $d$-fine on $A$. Define $\MV' \subset \bar{\MB}_{d'}$ by 
			$$\MV' \defeq \{\bar{B}_{d'}(x, \phi(r)): \text{$x \in X$, $r >0$ and $\bar{B}_d(x, r) \in \MV$}\}.$$ 
			Then for each $x \in X$, $r > 0$, $\bar{B}_{d'}(x, \phi(r)) \in \MV'$ iff $\bar{B}_d(x, r) \in \MV$.
			\begin{itemize}
				\item Let $x \in A$. Since $\MV \in \MC_d(A)$, there exists $r > 0$ such that $\bar{B}_d(x, r) \in \MV$. Define $r' > 0$ by $r' \defeq \phi(r)$. Then 
				\begin{align*}
					\bar{B}_{d'}(x, r')
					& = \bar{B}_{d'}(x, \phi(r)) \\
					& \in \MV'.
				\end{align*}
				Since $x \in A$ is arbitrary, we have that for each $x \in A$, there exists $r' > 0$ such that $\bar{B}_{d'}(x, r') \in \MV'$. Thus $\MV' \in \MC_{d'}(A)$.
				\item Let $x \in A$ and $\ep >$. Since $\phi$ is continuous and $\phi^{-1}(\{0\}) = \{0\}$, there exists $\del > 0$ such that $\phi((0, \del)) \subset (0,\ep)$. Since $\MV$ is $d$-fine on $A$, there exists $r > 0$ such that $\bar{B}_d(x, r) \in \MV$ and $r < \del$. Define $r' > 0$ by $r' \defeq \phi(r)$. Then $r' < \ep$ and  
				\begin{align*}
					\bar{B}_{d'}(x, r')
					& = \bar{B}_{d'}(x, \phi(r)) \\
					& \in \MV'.
				...
				\begin{align*}
					0 
					& \leq \inf \{r' > 0: \bar{B}_{d'}(x, r') \in \MV'\} \\
					& < \ep.
				\end{align*}
				Since $\ep > 0$ is arbitrary, we have that $\inf \{r' > 0: \bar{B}_{d'}(x, r') \in \MV'\} = 0$. Thus $\MV'$ is fine at $x$. Since $x \in A$ is arbitrary, we have that for each $x \in A$, $\MV'$ is fine at $x$. Therefore $\MV'$ is fine on $A$.
				\item Since $\mu$ is $d'$-Vitali, there exists $\MF' \subset \MV'$ such that $\MF'$ is countable, $\MF'$ is disjoint and $\MF'$ $\mu^*$-covers $A$. Define $\MF \subset \bar{\MB}_d$ by 
				$$\MF \defeq \{\bar{B}_d(x, r): \text{$x \in X$, $r > 0$ and $\bar{B}_{d'}(x, \phi(r)) \in \MF'$}\}.$$ 
				\begin{itemize}
					\item Let $S \in \MF$. Then there exists $x \in X$ and $r > 0$ such that $S = \bar{B}_d(x,r)$ and $\bar{B}_{d'}(x,\phi(r)) \in \MF'$. Since $\bar{B}_d(x,r) = \bar{B}_{d'}(x, \phi(r))$, we have that 
					\begin{align*}
						S
						& = \bar{B}_d(x,r) \\
						& = \bar{B}_{d'}(x, \phi(r)) \\
					...
					\end{align*}
					Since $S \in \MF$ is arbitrary, we have that for each $S \in \MF$, $S \in \MF'$. Hence $\MF \subset \MF'$.
					\item Let $S \in \MF'$. Since $\MF' \subset \MV'$ we have that $S \in \MV'$. Therefore, there exists $x \in X$ and $r > 0$ such that $S = \bar{B}_{d'}(x,\phi(r))$ and $\bar{B}_d(x, r) \in \MV$. Since $\bar{B}_d(x,r) = \bar{B}_{d'}(x, \phi(r))$, we have that 
					\begin{align*}
						S 
						& = \bar{B}_{d'}(x,\phi(r)) \\
						& = \bar{B}_d(x, r) \\
					...
					Since $S \in \MF'$ is arbitrary, we have that for each $S \in \MF'$, $S \in \MF$. Hence $\MF' \subset \MF$.
				\end{itemize}
				Since $\MF \subset \MF'$ and $\MF' \subset \MF$, we have that $\MF \subset \MF'$. 
				\begin{itemize}
					\item Since for each $x \in X$ and $r>0$, 
					\begin{align*}
						\bar{B}_d(x,r) \in \MF 
						& \iff \bar{B}_{d'}(x, \phi(r)) \in \MF' \\
						& \implies \bar{B}_{d'}(x, \phi(r)) \in \MV' \\
						& \iff \bar{B}_d(x,r) \in \MV \quad \text{(by definition of $\MV$),}
					\end{align*}
					we have that $\MF \subset \MV$.
					\item Since $\MF'$ is countable and $\MF = \MF'$, we have that $\MF$ is countable. 
					\item Since $\MF'$ is disjoint and $\MF = \MF'$, we have that $\MF$ is disjoint.
				...
				\end{itemize}
			\end{itemize}
			Since $A \subset X$ and $\MV \subset \MC_d(A)$ such that $\MV$ is $d$-fine on $A$ are arbitrary, we have that for each $A \subset X$ and $\MV \subset \MC_d(A)$, $\MV$ is $d$-fine on $A$ implies that there exists $\MF \subset \MV$ such that $\MF$ is countable, $\MF$ is disjoint and $\MF$ $\mu^*$-covers $A$. Hence $\mu$ is $d$-Vitali.
		\end{itemize}
	\end{proof}
	...
	\begin{ex} \lex{ex:diff_radon_measures:covering_lemmas:vitali_measures:0004}
		Let $(X, d)$ be a separable ultrametric space and $\mu \in M_+(X)$. If $(X, \MT_d)$ is compact, then $\mu$ is $d$-Vitali. \\
		\tbf{Hint}: 
		\begin{itemize}
			\item Since $(X, \MT_d)$ is compact, \tcb{an exercise in the analysis notes section on ultrametric spaces} implies that $d(X \times X) \setminus \{0\}$ is discrete. 
			\item For each $A \subset X$ and $x \in A$, there exists a maximal $\bar{\pi}^{d}_{a_n}(x) \in \MV$.
		\end{itemize}
	\end{ex}
	
	\begin{proof}
		Suppose that $X$ is compact. Define $R \subset \Rg$ by $R \defeq d(X \times X) \setminus \{0\}$. \tcb{An exercise in the analysis notes section on ultrametric spaces} implies that $R$ is discrete. 
		\begin{itemize}
			\item Suppose that $R$ is finte. Write $R \cup \{0\} = (r_j)_{j=1}^n$ with $r_1 > \cdots > r_n = 0$.  \tcr{FINISH!!!}
			\item Suppose that $R$ is infinte. Write $R = (r_n)_{n \in \N}$ with $(r_n)_{n \in \N}$ strictly decreasing. \tcb{An exercise in the analysis notes section on ultrametric spaces} implies that $r_n \rightarrow 0$. Define $\MP: \N \rightarrow \Part(X)$ by $\MP \defeq \MP^d$. \tcr{ref def in analysis notes section on ultrametric spaces}. Then for each $n \in \N$, $\MP_n = X/\simeq_{r_n}$ and $\pi^{\MP}_n(x) = \bar{B}(x, r_n)$. \tcr{An exercise in the analysis notes section on ultrametric spaces} implies that $\MP$ is ultrametric equivalent. Let $A \subset X$ and $\MV \in \MC(A)$. Suppose that $\MV$ is $d$-fine on $A$. Let $a \in A$. Since $\MV \in \MC(A)$, there exists $r > 0$ such that $\bar{B}(a, r) \in \MV$. 
			\begin{itemize}
				...
				\end{align*}
			\end{itemize}
			Since $a \in A$ is arbitrary, we have that for each $a \in A$, $\{n \in \N: \pi^{\MP}_n(a) \in \MV \} \neq \varnothing$. Since $X$ is separable, \tcr{ref ex in the analysis notes section on metric subspaces} implies that $A$ is separable. Thus there exists $(a_n)_{n \in \N} \subset A$ such that $(a_n)_{n \in \N}$ is dense in $A$. For each $k \in \N$, set $n_k \defeq \min \{n \in \N: \pi^{\MP}_n(a_k) \in \MV \}$. Define $\MF \subset \MV$ by $\MF = \{\pi^{\MP}_{n_k}(a_k): k \in \N\}$. 
			\begin{enumerate}
				...
				\item Let $a \in A$. Since $\MV \in \MC(A)$, there exists $r > 0$ such that $\bar{B}(a, r) \in \MV$. Therefore there exists $n \in \N$ such that $\bar{B}(a, r) = \pi^{\MP}_n(a)$. \tcb{An exercise in the section on ultrametric spaces in the analysis notes} implies that $\bar{B}(a, r)$ is open in $X$. Then $\bar{B}(a, r) \cap A$ is open in $A$. Since $(a_n)_{n \in \N}$ is dense in $A$, there exists $k \in \N$ such that 
				\begin{align*}
					a_k 
					& \in \bar{B}(a, r) \cap A \\
					& \subset \bar{B}(a, r) \\
	...
	\subsection{Vitali Measures on Subspaces}
	
	\begin{defn} \ld{def:diff_radon_measures:0004.1} \tcr{(move this and next definition somewhere else, seems really specific here and hard to find given it is used all over the next section)} \\
		Let $(X, d)$ be a metric space and $\mu \in M_+(X)$. Then
		\begin{itemize}
			\item $\mu$ is said to be \tbf{$d$-finite} if for each $x \in X$ and $r > 0$, $\mu(\bar{B}(x, r)) < \infty$. 
			\item  $\mu$ is said to be \tbf{locally $d$-finite} if for each $x \in X$, there exists $r > 0$ such that $\mu(\bar{B}(x, r)) < \infty$. 
		\end{itemize}
		We define $M_+(X, d)$, $M_+^{\loc}(X, d) \subset M_+(X)$ by
		\begin{itemize} 
			\item $M_+(X, d) \defeq \{\mu \in M_+(X): \mu \text{ is $d$-finite}\}$
			\item $M_+^{\loc}(X, d) \defeq \{\mu \in M_+(X): \mu \text{ is $d$-locally finite}\}$
		\end{itemize}
	\end{defn}
	
	\begin{defn} \tcr{move with prev def}
		Let $(X, d)$ be a metric space, $\mu \in M_+^{\loc}(X)$ and $x \in X$. We define the \tbf{radius of finitude of $\mu$ at $x$}, denoted $r_x^{\mu} \in (0, \infty]$, by $r_x^{\mu} \defeq \sup \{r > 0: \mu(\bar{B}(x, r)) < \infty \}$.
	\end{defn}
	
	\begin{defn} \ld{def:diff_radon_measures:covering_lemmas:subspaces:0001}
		Let $(X, d)$ be a metric space and $\mu \in M_+(X)$. Then $\mu$ is said to be \tbf{locally $d$-Vitali} if for each $x \in X$, there exists $r >0$ such that $\mu_{\bar{B}(x, r)}$ is $d|_{\bar{B}(x, r)^2}$-Vitali. 
	\end{defn}
	
	\begin{ex} \lex{ex:diff_radon_measures:covering_lemmas:subspaces:0002}
		Let $(X, d)$ be a metric space and $\mu \in M_+(X)$. If $\mu$ is $d$-Vitali, then
		\begin{enumerate}
			\item for each $x \in X$ and $r >0$, $\mu_{\bar{B}(x, r)}$ is $d|_{\bar{B}(x, r)^2}$-Vitali,
			\item $\mu$ is locally $d$-Vitali. 
		\end{enumerate}
	...
	\begin{proof}
		...
		\begin{enumerate}
			\item Let $x_0 \in X$ and $r_0 > 0$. Define $X_0 \defeq \bar{B}(x_0, r_0)$, $d' \defeq d|_{X_0^2}$ and $\mu' \defeq \mu_{X_0}$. Let $A \subset X_0$ and $\MV' \in \MC_{d'}(A)$. Suppose that $\MV'$ is $d'$-fine on $A$. Define $\MV \subset \bar{\MB}_d$ by 
			$$\MV \defeq \{\bar{B}_d(x, r): \text{$x \in X_0$, $r > 0$ and $\bar{B}_d(x,r) \cap X_0 \in \MV'$}\}.$$ 
			\tcr{(ref ex in metric subspace section of analysis notes)} implies that for each $x \in X_0$, and $r>0$, $\bar{B}_{d'}(x, r) = \bar{B}_d(x, r) \cap X_0$. Therefore for each $x \in X_0$ and $r > 0$,
			\begin{align*}
				\bar{B}_{d'}(x, r) \in \MV'
				& \iff \bar{B}_d(x, r) \cap X_0 \in \MV' \\
				& \iff \bar{B}_d(x, r) \in \MV.
			\end{align*}
			Hence $\MV' = \MV \cap X_0$.
			\begin{itemize}
				\item Let $x \in A$. Since $\MV' \in \MC_{d'}(A)$, there exists $r >0$ such that $\bar{B}_{d'}(x, r) \in \MV'$. Since 
				\begin{align*}
					\bar{B}_d(x, r) \cap X_0   
					& = \bar{B}_{d'}(x, r) \\
					& \in \MV'.
				\end{align*}
				Hence $\bar{B}_d(x, r) \in \MV$. Since $x \in A$ is arbitrary, we have that for each $x \in A$, there exists $r >0$ such that $\bar{B}_d(x, r) \in \MV$. Hence $\MV \in \MC_d(A)$.
				\item Let $x \in A$. Since $A \subset X_0$, we have that $x \in X_0$. \tcr{from above (ref)} then implies that $\{r > 0: \bar{B}_{d'}(x, r) \in \MV'\} = \{r > 0: \bar{B}_d(x, r) \in \MV\}$. Since $\MV'$ is $d'$-fine on $A$, we have that
				\begin{align*}
					\inf \{r > 0: \bar{B}_d(x, r) \in \MV\} 
					& = \inf \{r > 0: \bar{B}_{d'}(x, r) \in \MV'\} \\
					& = 0.
				\end{align*}
				Since $x \in A$ is arbitrary, we have that for each $x \in A$, $\inf \{r > 0: \bar{B}_d(x, r) \in \MV\} = 0$. Hence $\MV$ is $d$-fine on $A$. 
				\item Since $\mu$ is $d$-Vitali, there exists $\MF \subset \MV$ such that $\MF$ is countable, $\MF$ is disjoint and $\MF$ $\mu^*$-covers $A$.
				\begin{itemize}
					\item Define $\MF' \subset \bar{\MB}_{d'}$ by $\MF' \defeq \MF \cap X_0$. Since $\MF \subset \MV$, \tcr{from above (ref)} implies that 
					\begin{align*}
						\MF'
						& = \MF \cap X_0 \\
						& \subset \MV \cap X_0 \\
						& = \MV'.
					\end{align*}
					\item Since $\MF$ is countable, we have that $\MF'$ is countable \tcr{(give details?)}.
					\item Since $\MF$ is disjoint, we have that $\MF'$ is disjoint \tcr{(give details?)}.
					\item Since $A \subset X_0$, 
					\begin{align*}
						A \setminus \bigg(\bigcup_{S \in \MF} (S \cap X_0) \bigg)
						& = A \cap \bigg(\bigcup_{S \in \MF} S \cap X_0 \bigg)^c \\
						& = A \cap \bigg(\bigcap_{S \in \MF}(S \cap X_0)^c \bigg) \\
						& = A \cap \bigg(\bigcap_{S \in \MF}(S^c \cup X_0^c)  \bigg) \\
						& = \bigcap_{S \in \MF}[A \cap (S^c \cup X_0^c)] \\
						& = \bigcap_{S \in \MF}[(A \cap S^c) \cup (A \cap X_0^c)] \\
						& = \bigcap_{S \in \MF}[(A \cap S^c) \cup \varnothing] \\
						& = \bigcap_{S \in \MF} (A \cap S^c) \\
						& = A \cap \bigg( \bigcap_{S \in \MF} S^c \bigg) \\
						& = A \cap \bigg( \bigcup_{S \in \MF} S \bigg)^c \\
						& = A \setminus \bigg( \bigcup_{S \in \MF} S  \bigg).
					\end{align*}
					Since $\MF$ $\mu^*$-covers $A$, we then have that
					\begin{align*}
						(\mu')^* \bigg[ A \setminus \bigg(\bigcup_{S \in \MF'} S \bigg) \bigg] 
						& = (\mu^*)_{X_0} \bigg[ A \setminus \bigg(\bigcup_{S \in \MF'} S \bigg) \bigg] \quad \text{(by \rex{ex:subspace_measures:00005.1})} \\
						& = \mu^* \bigg[ A \setminus \bigg(\bigcup_{S \in \MF'} S \bigg) \bigg] \quad \text{(since $A \subset X_0$)} \\
						& = \mu^* \bigg[ A \setminus \bigg(\bigcup_{S \in \MF} (S \cap X_0) \bigg) \bigg] \\
						& = \mu^* \bigg[ A \setminus \bigg( \bigcup_{S \in \MF} S  \bigg) \bigg] \quad \text{(by \tcr{ref above})} \\
						& = 0. 
					\end{align*} 
					Thus $\MF'$ $(\mu')^*$-covers $A$.
				\end{itemize}
			\end{itemize}
			Since $A \subset X_0$ and $\MV' \subset \MC_{d'}(A)$ such that $\MV$ is $d'$-fine on $A$ are arbitrary, we have that for each $A \subset X_0$ and $\MV' \subset \MC_{d'}(A)$, $\MV$ is $d'$-fine on $A$ implies that there exists $\MF' \subset \MV'$ such that 
			\begin{itemize}
				\item $\MF$ is countable,
				\item $\MF$ is disjoint,
				\item $\MF$ $(\mu')^*$-covers $A$.
			\end{itemize}
			Hence $\mu'$ is $d'$-Vitali. Since $x_0 \in X$ and $r_0 > 0$ are arbitrary, we have that for each $x \in X$ and $r > 0$, $\mu_{\bar{B}(x, r)}$ is $d|_{\bar{B}(x, r)^2}$-Vitali.
			\item Let $x \in X$. \tcr{the prev part (ref)} implies that $\mu_{\bar{B}(x, 1)}$ is $d|_{\bar{B}(x, 1)^2}$-Vitali. Since $x \in X$ is arbitrary, we have that for each $x \in X$, there exists $r>0$ such that $\mu_{\bar{B}(x, r)}$ is $d|_{\bar{B}(x, r)^2}$-Vitali. Thus $\mu$ is locally $d$-Vitali.
		\end{enumerate}
	\end{proof}
	
	\begin{ex} \lex{ex:diff_radon_measures:covering_lemmas:subspaces:0003}
		Let $(X, d)$ be a metric space, $A \subset X$, $\MV \in \MC_d(A)$ and $X' \subset X$. Define $d': X' \times X' \rightarrow \Rg$, $A' \subset X'$ and $\MV' \subset \bar{\MB}_{d'}$ by $d' \defeq d|_{X'}$, $A' \defeq A \cap X'$ and 
		$$\MV' \defeq \{\bar{B}_{d'}(x,r): \text{$x \in X'$, $r > 0$ and $\bar{B}_d(x,r) \in \MV$}\}.$$ 
		Then
		\begin{enumerate}
			\item $\MV' \in \MC_{d'}(A')$,
			\item $\MV$ is $d$-fine on $A$ implies that $\MV'$ is $d'$-fine on $A'$.
		\end{enumerate}
	\end{ex}
	
	\begin{proof}
		Suppose that $\MV$ is $d$-fine on $A$.
		\begin{enumerate}
			\item We note that 
			\begin{align*}
				\MV' 
				& = \{\bar{B}_d(x,r) \cap X': \text{$x \in X'$, $r > 0$ and $\bar{B}_d(x,r) \in \MV$}\} \\
				& = \{ \bar{B}_{d'}(x,r): \text{$x \in X'$, $r > 0$ and $\bar{B}_d(x,r) \in \MV$}\} \\
				& \subset \bar{\MB}_{d'}.
			\end{align*}  
			Let $x \in A'$. Then $x \in A$ and $x \in X'$. Since $\MV \in \MC_d(A)$, there exists $r>0$ such that $\bar{B}_d(x, r) \in \MV$. Therefore 
			\begin{align*}
				\bar{B}_{d'}(x,r)
				& \in \{ \bar{B}_{d'}(x,r): \text{$x \in X'$, $r > 0$ and $\bar{B}_d(x,r) \in \MV$} \} \\
				& = \MV'.
			\end{align*}
			Since $x \in A'$ is arbitrary, we have that for each $x \in A'$, there exists $r>0$ such that $\bar{B}_{d'}(x, r) \in \MV'$. Hence $\MV' \in \MC_{d'}(A')$. 
			\item Suppose that $\MV$ is $d$-fine on $A$. Let $x \in A'$ and $\ep >0$. Then $x \in A$ and $x \in X'$. Since $\MV$ is $d$-fine on $A$, $\inf \{r > 0: \bar{B}_d(x, r) \in \MV\} = 0$. Thus there exists $r' > 0$ such that $\bar{B}_d(x, r') \in \MV$ and $r < \ep$. Since $x \in X'$, we have that 
			\begin{align*}
				\bar{B}_{d'}(x,r')
				& \in \{ \bar{B}_{d'}(x,r): \text{$x \in X'$, $r > 0$ and $\bar{B}_d(x,r) \in \MV$} \} \\
				& = \MV'.
			...
			\begin{align*}
				...
				& \leq 0.
			\end{align*}
			Thus $\inf \{r > 0: \bar{B}_{d'}(x, r) \in \MV'\} = 0$. Therefore $\MV'$ is $d$-fine at $x$. Since $x \in A'$ is arbitrary, we have that for each $x \in A'$, $\MV'$ is $d$-fine at $x$. Hence $\MV'$ is $d$-fine on $A'$.		
		\end{enumerate}
	\end{proof}
	
	\begin{ex}\lex{ex:diff_radon_measures:covering_lemmas:subspaces:0004}
		Let $(X, d)$ be a metric space and $\mu \in M_+(X)$. Suppose that $X \neq \varnothing$. If $(X, \MT_d)$ is separable and $\mu$ is locally $d$-Vitali, then for each $n \in \N$, there exists $\MB \subset \bar{\MB}_d$ such that $\MB$ is countable, $\MB$ is disjoint, $X = \bigcup\limits_{B \in \MB} B$ and for each $B \in \MB$, $\diam(B) \leq 1/n$ and $\mu_B$ is $d|_B$-Vitali. 
	\end{ex}
	
	\begin{proof}
		\tcr{(find out how to rigorously handle these types of proofs where objects are defined inductively and the process may or may not terminate, i.e. proofs that are essentially a while-loop)} \\
		Suppose that $X$ is separable and $\mu$ is locally $d$-Vitali. Let $n \in \N$. Since $X$ is separable, there exists $S \subset X$ such that $S$ is countable and $S$ is dense in $X$. Since $X \neq \varnothing$, we have that $S \neq \varnothing$.
		\begin{itemize}
			\item Suppose that $\# S = 1$. Write $S = \{x\}$. Since $\mu$ is locally $d$-Vitali, there exists $r' > 0$ such that $\mu_{\bar{B}_d(x, r')}$ is $d|_{\bar{B}_d(x, r')}$-Vitali. Define $\MB \defeq \{\bar{B}_d(x, 1/2n)\}$. Then $\MB$ is countable, $\MB$ is disjoint, $X = \bigcup\limits_{B \in \MB} B$ and for each $B \in \MB$, $\diam(B) \leq 1/n$ and $\mu_B$ is $d|_B$-Vitali. 
			\item Suppose that $\# S \in \N \setminus \{1\}$. Write $S = \{x_j\}_{j \in [J]}$ where $j_1 \neq j_2$ implies $x_{j_1} \neq x_{j_2}$. Since $\mu$ is locally $d$-Vitali, for each each $j \in [J]$, there exists $r'_j$ such that $\mu_{\bar{B}_d(x, r'_j)}$ is $d|_{\bar{B}_d(x, r'_j)}$-Vitali. Define $r > 0$, $(r_j)_{j \in [J]} \in (0, \infty)^{[J]}$ and $\MB \subset \bar{\MB}_d$ by $r \defeq \min\{d(x, y): \text{$x,y \in S$ and $x \neq y$}\}$, $r_j \defeq 2^{-1}\min\{r, n^{-1}, r'_j\}$ and $\MB \defeq \{\bar{B}_d(x_j, r_j)\}_{j \in [J]}$. Let $j \in [J]$. Since $r_j < r'_j$, $\bar{B}_d(x_j, r_j) \subset \bar{B}_d(x_j, r'_j)$. \rex{ex:diff_radon_measures:covering_lemmas:subspaces:0002} then implies that $\mu_{\bar{B}_d(x_j, r_j)}$ is $d|_{\bar{B}_d(x_j, r_j)}$-Vitali. Since $j \in [J]$ is arbitrary, we have that for each $j \in [J]$, $\mu_{\bar{B}_d(x_j, r_j)}$ is $d|_{\bar{B}_d(x_j, r_j)}$-Vitali. Then $\MB$ is countable, $\MB$ is disjoint, $X = \bigcup\limits_{B \in \MB} B$ and for each $B \in \MB$, $\diam(B) \leq 1/n$ and $\mu_B$ is $d|_{B}$-Vitali. 
			\item Suppose that $\# S = \infty$. Write $S = \{x_j\}_{j \in \N}$ where $j_1 \neq j_2$ implies $x_{j_1} \neq x_{j_2}$. We define $\MB \subset \bar{B}_d$ inductively as follows:
			\begin{itemize}
				\item Since $\mu$ is locally $d$-Vitali, there exists $r'_1 > 0$ such that $\mu_{\bar{B}(x_1, r'_1)}$ is $d|_{\bar{B}(x_1, r'_1)}$-Vitali. We define $j_1 \defeq 1$, $r_1 \defeq \min((2n)^{-1}, r'_1)$ and $B_1 \defeq \bar{B}(x_{j_1}, r_1)$. Since $r_1 \leq r'_1$, $\bar{B}(x_1, r_1) \subset \bar{B}(x_1, r'_1)$. \rex{ex:diff_radon_measures:covering_lemmas:subspaces:0002} then implies that $\mu_{\bar{B}(x_1, r_1)}$ is $d|_{\bar{B}(x_1, r_1)}$-Vitali.
				\item For $k \geq 2$, define $J_k \subset \N$ by $J_k \defeq \bigg \{j \in \N: x_j \not \in \bigcup\limits_{k' \in [k-1]} \bar{B}(x_{j_{k'}}, r_{k'}) \bigg \}$.
				\begin{itemize}
					\item If $J_k \neq \varnothing$, define $j_k \defeq \min J_k$. Since $\mu$ is locally $d$-Vitali, there exists $r'_k > 0$ such that $\mu_{\bar{B}(x_{j_k}, r'_k)}$ is $d|_{\bar{B}(x_{j_k}, r'_k)}$-Vitali. Since $\bigg [ \bigcup\limits_{k' \in [k-1]} \bar{B}(x_{j_{k'}}, r_{k'}) \bigg]^c$ is open and $x_{j_k} \in \bigg [ \bigcup\limits_{k' \in [k-1]} \bar{B}(x_{j_{k'}}, r_{k'}) \bigg]^c$, there exists $\rho_k > 0$ such that $\rho_k \leq (2n)^{-1}$ and $B(x_{j_k}, \rho_k) \subset \bigg [ \bigcup\limits_{k' \in [k-1]} \bar{B}(x_{j_{k'}}, \rho_{k'}) \bigg]^c$. Define $r_k \defeq \min \{\rho_k, r'_k\}$ and $B_k \defeq \bar{B}(x_{j_k}, r_k)$. Since $r_k < r'_k$, $\bar{B}_d(x_{j_k}, r_k) \subset \bar{B}_d(x_{j_k}, r'_k)$. \rex{ex:diff_radon_measures:covering_lemmas:subspaces:0002} then implies that $\mu_{\bar{B}_d(x_{j_k}, r_k)}$ is $d|_{\bar{B}_d(x_{j_k}, r_k)}$-Vitali. 
					\item If $J_k = \varnothing$, set $\MB \defeq \{B_{k'}\}_{k' \in [k-1]}$. Then $\MB$ is countable, $\MB$ is disjoint, $X = \bigcup\limits_{B \in \MB} B$ and for each $B \in \MB$, $\diam(B) \leq 1/n$ and $\mu_B$ is $d|_B$-Vitali. 
				\end{itemize}
			\end{itemize}
			Define $\MB \defeq \{B_k\}_{k \in \N}$. Then $\MB$ is countable, $\MB$ is disjoint, $X = \bigcup\limits_{B \in \MB} B$ and for each $B \in \MB$, $\diam(B) \leq 1/n$ and $\mu_B$ is $d|_B$-Vitali. 
		\end{itemize}
		Since $n \in \N$ is arbitrary, we have that for each $n \in \N$, there exists $\MB \subset \bar{B}_d$ such that $\MB$ is countable, $\MB$ is disjoint, $X = \bigcup\limits_{B \in \MB} B$ and for each $B \in \MB$, $\diam(B) \leq 1/n$ and $\mu_B$ is $d|_B$-Vitali.  
	\end{proof}
	...
	\subsection{Vitali Measures on Coproduct Spaces}
	
	\begin{ex} \lex{ex:diff_radon_measures:covering_lemmas:coproducts:0001}
		Let $(X_n, d_n)$ be a collection of metric spaces and $(\mu_n)_{n \in \N} \in \prod\limits_{n \in \N} M_+(X_n)$. Set $X \defeq \coprod\limits_{n \in \N} X_n$, $d \defeq \bigoplus\limits_{n \in \N} d_n$ and $\mu \defeq \bigoplus_{n \in \N} \mu_n$.  Suppose that for each $n \in \N$, $X_n$ is separable. Then $\mu$ $d$-Vitali iff for each $n \in \N$, $\mu_n$ is $d_n$-Vitali.
	\end{ex}
	
	\begin{proof}\
		\begin{itemize}
			\item Suppose that $\mu$ is $d$-Vitali.
			\item Suppose that for each $n \in \N$, $\mu_n$ is $d_n$-Vitali. Let $A \subset X$ and $\MV \in \MC_d(A)$. Suppose that $\MV$ is $d$-fine on $A$. \tcr{} 
		\end{itemize}
	\end{proof}
	
	\begin{ex}
		Let $(X_n, d_n)$ be a collection of metric spaces and $(\mu_n)_{n \in \N} \in \prod\limits_{n \in \N} M_+(X_n)$. Set $X \defeq \coprod\limits_{n \in \N} X_n$, $d \defeq \bigoplus\limits_{n \in \N} d_n$ and $\mu \defeq \bigoplus_{n \in \N} \mu_n$.  Suppose that for each $n \in \N$, $X_n$ is separable. If for each $n \in \N$, $\mu_n$ is $d_n$-Vitali, then $\mu$ $d$-Vitali iff $\mu$ is locally $d$-finite.
	\end{ex}
	
	\begin{proof}
		\rex{ex:diff_radon_measures:covering_lemmas:subspaces:0003}  \rex{ex:diff_radon_measures:covering_lemmas:vitali_measures:0003}
	\end{proof}
	
	\begin{proof}\
		\begin{itemize}
			\item Suppose that $X$ is separable. Then 
			\item 
		\end{itemize}
	\end{proof}
	...
	\section{Differentiation of Radon Measures}
	
	\subsection{Differentiation of Positive Radon Measures}

	\begin{defn} \ld{def:diff_radon_measures:0004.1}
		Let $(X, d)$ be a metric space and $\mu \in \MM_+(X)$. We define $\MM_+(X, d)$, $\MM_+^{\loc}(X, d) \subset \MM_+(X)$ by
		\begin{itemize} 
			\item $\MM_+(X, d) \defeq M_+(X, d) \cap \MM_+(X)$
			\item $\MM_+^{\loc}(X, d) \defeq M_+^{\loc}(X, d) \cap \MM_+(X)$
		\end{itemize}
	\end{defn}
	...
	\begin{ex} \lex{ex:diff_radon_measures:009}
		Let $X$ be a metric space and $\mu \in \MM_+(X, d)$. Let $r > 0$. Define $f_r: X \rightarrow \Rg$ by $f_r(x) \defeq \mu(\bar{B}(x, r))$. Then $f_r$ is upper semi-continuous.
	\end{ex}
	...
	\begin{proof}
		...
		\begin{align*}
			\limsup_{n \rightarrow \infty} f_r(x_n)
			& = \limsup_{n \rightarrow \infty}  \mu(\bar{B}(x_n, r)) \\
			& = \inf_{N \in \N} \sup\limits_{n \geq N} \mu(\bar{B}(x_n, r)) \\
			& \leq \sup\limits_{n \geq N_0} \mu(\bar{B}(x_n, r)) \\
			& \leq \mu(\bar{B}(x, r + k^{-1}))
		\end{align*} 
		Since $k \in \N$ is arbitrary, we have that 
		$$\limsup\limits_{n \rightarrow \infty} f_r(x_n) \leq \inf\limits_{k \in \N} \mu(\bar{B}(x, r + k^{-1}))$$ 
		Since $\mu \in \MM_+(X, d)$, we have that $\mu(\bar{B}(x, r + 1)) < \infty$. Since $(\bar{B}(x, r + k^{-1}))_{k \in \N}$ is decreasing and $ \inf\limits_{k \in \N} \bar{B}(x, r + k^{-1}) = \bar{B}(x, r)$, we have that 
		\begin{align*}
			\limsup_{n \rightarrow \infty} f_r(x_n)
			...
			& = f_r(x)
		\end{align*}
		Since $(x_n)_{n \in \N} \subset X$ and $x \in X$ with $x_n \rightarrow x$ are arbitrary, we have that $f_r$ is upper semicontinuous. 
	\end{proof}
	...
	\begin{defn} \ld{def:diff_radon_measures:0005}
		...
		\begin{itemize}
			...
			\ub{D}_{\mu}\nu(x) \defeq
			\begin{cases}
				\liminf\limits_{r \in (0, r^{\mu}_x \wedge r^{\nu}_x)} \frac{\nu(\bar{B}(x, r))}{\mu(\bar{B}(x, r))}, & x \in \supp \mu \\
			...
			\ol{D}_{\mu}\nu(x) \defeq
			\begin{cases}
				\limsup\limits_{r \in (0, r^{\mu}_x \wedge r^{\nu}_x)} \frac{\nu(\bar{B}(x, r))}{\mu(\bar{B}(x, r))}, & x \in \supp \mu \\
				\infty, & x \in (\supp \mu)^c    
			\end{cases}
			\]
		\end{itemize}
		\tcr{find better notation for $\limsup\limits_{r \rightarrow 0^+}$ but restricted to $r < r^{\mu}_x \defeq \sup \{r >0: \mu(\bar{B}(x, r)) < \infty\} $}
	\end{defn}
	...
	\begin{proof}
		For each $r > 0$, define $f_r: X \rightarrow \RG$ and $g_r: X \rightarrow (0, \infty)$ by $f_r(x) \defeq \nu(\bar{B}(x, r))$ and $g_r(x) \defeq \mu(\bar{B}(x, r))$. Let $r > 0$. \tcr{prev ex} implies that for each $r > 0$, $f_r, g_r \in \ML^+(X)$. \tcr{make/ref ex} then implies that $f_r |_{\supp \mu}, g_r |_{\supp \mu} \in \ML_+( \supp \mu)$. Since $g_r |_{\supp \mu} > 0$, \tcr{make ref ex} implies that $f_r|_{\supp \mu}/g_r|_{\supp \mu} \in \ML^+(\supp \mu)$. Since $r >0$ is arbitrary, we have that for each $r > 0$, $f_r|_{\supp \mu}/g_r|_{\supp \mu} \in \ML^+(\supp \mu)$. Define $c: (\supp \mu)^c \rightarrow \RG$ by $c(x) \defeq \infty$. Then $c \in \ML^+((\supp \mu)^c)$. Define $\phi: (\supp \mu) \coprod (\supp \mu)^c \rightarrow X$ \tcr{as in the ex in measurable coproducts section about measurable isomorphism between union and disjoint union of disjoint subset sets}. For each $r > 0$, define $h_r:X \rightarrow \RG$ by $h_r \defeq [(f_r|_{\supp \mu}/g_r|_{\supp \mu}) \coprod c] \circ \phi^{-1}$. Since $\supp \mu \in \MB(X)$, \tcr{make/ref ex} implies that for each $r > 0$,
		\begin{align*}
			h_r
			& = [(f_r|_{\supp \mu}/g_r|_{\supp \mu}) \coprod c] \circ \phi \\
			& \in \ML^+(X). 
		\end{align*} 
		\begin{enumerate}
			\item Since $D_{\mu} \nu = \liminf\limits_{r \rightarrow 0^+} h_r$ and $(h_r)_{r > 0} \subset \ML^+(X)$, \tcr{make/ref ex} implies that $D_{\mu} \nu \in \ML^+(X)$.
			\item Since $D_{\mu} \nu = \limsup\limits_{r \rightarrow 0^+} h_r$ and $(h_r)_{r > 0} \subset \ML^+(X)$, \tcr{make/ref ex} implies that $D_{\mu} \nu \in \ML^+(X)$.
		\end{enumerate}
	\end{proof}
	
	\begin{note}
		\tcr{Think of an example where if $\nu \in \MM_+^{\loc}(X)$ and the upper/lower derivate by $\mu$ is not measurable}
	\end{note}

	\begin{ex} \lex{ex:diff_radon_measures:0006}
		Let $(X, d)$ be a metric space, $\al, \be \in \MM_+(X, d)$ and $c \in (0,\infty)$. Then 
		\begin{enumerate}
	...
	\begin{proof}\
		\begin{enumerate}
			\item Let $x \in \supp \al \cap \supp \be$. Define $r_0  \in (0, \infty]$ by $r_0 \defeq r^{\mu}_x \wedge r^{\nu}_x$. Then for each $s \in (0,r_0)$, $\al(\bar{B}(x, s)), \be(\bar{B}(x, s)) < \infty$. Since $x \in \supp \al \cap \supp \be$, \rex{ex:introduction_radon_measures:0005} implies that for each $r > 0$, $\al(\bar{B}(x, r)), \be(\bar{B}(x, r)) > 0$. Hence $(\al(\bar{B}(x, s)))_{s \leq r_0}$, $(\be(\bar{B}(x, s)))_{s \leq r_0} \subset (0,\infty)$. Since $x \in \supp \al \cap \supp \be$, the definition of $\ub{D}_{\al} \be (x)$ and $\ol{D}_{\be}\al(x)$ implies that 
			$$\ol{D}_{\be}\al(x) = \limsup\limits_{r \in (0, r_0)} \frac{\al(\bar{B}(x, r))}{\be(\bar{B}(x, r))}$$ 
			and 
			$$\ub{D}_{\al} \be (x) = \liminf\limits_{r \in (0, r_0)} \frac{\be(\bar{B}(x, s))}{\al(\bar{B}(x, s))}$$ 
			Since the map $(0, \infty) \rightarrow (0, \infty)$ given by $x \mapsto x^{-1}$ is order reversing, we have that
			\begin{align*}
				\ol{D}_{\be}\al(x) > c^{-1}
				& \iff \limsup_{r \in (0, r_0)} \frac{\al(\bar{B}(x, s))}{\be(\bar{B}(x, s))} > c^{-1} \\
				& \iff \bigg[ \limsup_{r \in (0, r_0)} \frac{\al(\bar{B}(x, s))}{\be(\bar{B}(x, s))} \bigg]^{-1}  <  c \\
				& \iff \liminf_{r \in (0, r_0)} \bigg[ \frac{\al(\bar{B}(x, s))}{\be(\bar{B}(x, s))} \bigg]^{-1}  <  c \\
				& \iff  \liminf_{r \in (0, r_0)} \frac{\be(\bar{B}(x, s))}{\al(\bar{B}(x, s))}  < c \\
			...
			\begin{itemize}
				...
				\begin{align*}
					\inf_{r > 0} \bigg[ \sup\limits_{s \in (0,r]} \frac{\al(\bar{B}(x, s))}{\be(\bar{B}(x, s))} \bigg] 
					& = \limsup_{r \in (0, r_0)} \frac{\al(\bar{B}(x, r))}{\be(\bar{B}(x, r))} \\
					& = \ol{D}_{\be}\al (x) \\
					& > c^{-1} 
				\end{align*}
	...
	\begin{proof}\
		\begin{enumerate}
			...
			\begin{itemize}
				...
				\begin{align*}
					\sup\limits_{r>0} \inf_{s \in (0, r]} \frac{ \al(\bar{B}(x, s)) }{ \be(\bar{B}(x, s)) }
				...
				\begin{align*}
					...
					& = \sum\limits_{B \in \MF} \al(B) \\
					& \leq c \sum\limits_{B \in \MF} \be(B) \\
	...
	\begin{proof} Suppose that $\mu$ is $d$-Vitali.
		\begin{enumerate}
			...
			\begin{align*}
				...
				& \leq \sum\limits_{n \in \N} \mu_{\mu}(I \cap A_n) \\
				& = 0 
			\end{align*}
			...
			\begin{align*}
				...
				& \leq \sum\limits_{\substack{a,b \in \Q \\ 0 < a < b}} \mu_{\mu}(R_{a,b}) \\
				& = 0
			\end{align*}
	...
	\begin{defn} \ld{def:diff_radon_measures:0010}
		Let $(X, d)$ be a metric space and $\mu, \nu \in \MM_+^{\loc}(X, d)$. 
		\begin{itemize}
			\item Let $h \in \ML^0(X, \MB(X))$. Then $h$ is said to be a \tbf{derivate of $\nu$ with respect to $\mu$} if $h = \ub{D}_{\mu}\nu$ $\mu$-a.e. and $h = \ol{D}_{\mu}\nu$ $\mu$-a.e. 
			\item Then $\nu$ is said to be \tbf{derivable with respect to $\mu$} if there exists $h \in \ML^0(X, \MB(X))$ such that $h$ is a derivate of $\nu$ with respect to $\mu$.
		\end{itemize}
	\end{defn}
	...
	\begin{proof}
		...
		\end{cases}
		\]
		Since $(f_n)_{n \in \N}$, $(g_n)_{n \in \N} \subset \ML^0(X, \MB(X))$ and $N \in \MB(X)$, we have that $(h_n)_{n \in \N} \subset \ML^0(X, \MB(X))$ \tcr{(make this an exercise in the earlier chapters)}. Then $h_n \convt{p.w.} h$. \rex{ex:measurable_functions:00015} implies that $h \in \ML^0(X, \MB(X))$. Since $\{x \in X: h(x) \neq \ol{D}_{\mu}\nu(x)\} \cup \{x \in X: h(x) \neq \ub{D}_{\mu}\nu(x)\} \subset N$, we have that $h = \ub{D}_{\mu}\nu$ $\mu$-a.e. and $h = \ol{D}_{\mu}\nu$ $\mu$-a.e. Thus $h$ is a derivate of $\nu$ by $\mu$.
	\end{proof}
	...
	\begin{defn} \ld{def:diff_radon_measures:0012}
		...
		\tcr{define $\pi_{L^0}: \ML^0(X, \MA) \rightarrow L^0(X, \MA)$ somewhere in measurable function section}
	\end{defn}
	...
	\begin{proof}\
		\begin{enumerate}
			...
			\begin{align*}
				...
				& = \sum\limits_{m \in \Z} \nu_{\mu}(E_m) \\
				& \leq \sum\limits_{m \in \Z} t^{m+1} \mu(E_m) \\
				& = t\sum\limits_{m \in \Z} t^m \mu(E_m) \\
				& = t\sum\limits_{m \in \Z} \int_{E_m} t^m \dmu \\
				& \leq t \sum\limits_{m \in \Z} \int_{E_m} D_{\mu} \nu \dmu \\
			...
			\begin{align*}
				\nu_{\mu}(E)
				& = \sum\limits_{m \in \Z} \nu_{\mu}(E_m) \\
				& \geq \sum\limits_{m \in \Z} t^m \mu(E_m) \\
				& = t^{-1} \sum\limits_{m \in \Z}  t^{m+1} \mu(E_m) \\
				& = t^{-1} \sum\limits_{m \in \Z} \int_{E_m} t^{m+1} \dmu \\
				& \geq t^{-1} \sum\limits_{m \in \Z} \int_{E_m} D_{\mu} \nu \dmu \\
			...
			\item Clear by \rd{def:diff_radon_measures:0005}.
		\end{enumerate}
	\end{proof}
	...
	\begin{itemize}
		\item \tcr{show that if $\nu \in \MM(X)$ is Radon, then its real and imaginary parts are Radon and if $\nu$ is finite signed, then its positive and negative parts are Radon.}
		\item \tcr{(maybe just combine this subsection with the previous one if there is not much more added)}
	\end{itemize}
	
	\begin{note}
		Let $\nu \in \MM(X)$. Since $\Re(\nu)^{\pm}$ and $\Im(\nu)^{\pm}$ are finite, we have that $\Re(\nu)^{\pm}, \Im(\nu)^{\pm} \in \MM_+(X, d)$.
	\end{note}
	
	\begin{defn} \ld{def:differentiation_complex_measures:0001}
		Let $(X, d)$ be a metric space and $\nu \in \MM(X)$.
		\begin{itemize}
			\item If $\Im \nu = 0$, we define $D_{\mu} \nu = D_{\mu} \nu^+ - D_{\mu} \nu^-$ \tcr{(should generalize to signed measures, since this only covers finite signed measures)}.
	...
	\begin{proof}\
		\begin{enumerate}
			\item By \rd{def:differentiation_complex_measures:0001},  
			\begin{align*}
				D_{\mu} \nu
				& = D_{\mu} (\Re \nu) + i D_{\mu} (\Im \nu) \\
				& = [D_{\mu} (\Re \nu)^+ - D_{\mu} (\Re \nu)^-] + i [D_{\mu} (\Im \nu)^+ - D_{\mu} (\Im \nu)^-] \\
				& = \bigg[ \frac{ d(\Re \nu)^+_{\mu}}{d\mu} - \frac{d (\Re \nu)^-_{\mu}}{d \mu} \bigg] + i \bigg[ \frac{d (\Im \nu)^+_{\mu}}{d \mu} - \frac{d (\Im \nu)^-_{\mu}}{d \mu} \bigg] \quad \text{(by \rex{ex:diff_radon_measures:0013})} \\
				& = \frac{d \nu_{\mu}}{d \mu}
			\end{align*}
			...
			\begin{align*}
				D_{\mu} \nu(x)
				& = [D_{\mu} (\Re \nu)^+ - D_{\mu} (\Re \nu)^-] + i [D_{\mu} (\Im \nu)^+ - D_{\mu} (\Im \nu)^-] \\
				& = \bigg[ \lim_{r \rightarrow 0^+} \frac{ (\Re \nu)^+(\bar{B}(x, r))}{\mu(\bar{B}(x, r))} - \lim_{r \rightarrow 0^+} \frac{ (\Re \nu)^-(\bar{B}(x, r))}{\mu(\bar{B}(x, r))} \bigg] \\
				& \quad \quad \quad \quad  + i \bigg[ \lim_{r \rightarrow 0^+} \frac{ (\Im \nu)^+(\bar{B}(x, r))}{\mu(\bar{B}(x, r))} - \lim_{r \rightarrow 0^+} \frac{ (\Im \nu)^- (\bar{B}(x, r))}{\mu(\bar{B}(x, r))} \bigg] \quad \text{(by \rex{ex:diff_radon_measures:0013})} \\
				& = \lim_{r \rightarrow 0^+} \bigg[  \frac{ (\Re \nu)^+(\bar{B}(x, r))}{\mu(\bar{B}(x, r))} - \frac{ (\Re \nu)^-(\bar{B}(x, r))}{\mu(\bar{B}(x, r))} \bigg] \\
				& \quad \quad \quad \quad  + i \lim_{r \rightarrow 0^+} \bigg[  \frac{ (\Im \nu)^+(\bar{B}(x, r))}{\mu(\bar{B}(x, r))} - \frac{ (\Im \nu)^- (\bar{B}(x, r))}{\mu(\bar{B}(x, r))} \bigg] \\
				& = \lim_{r \rightarrow 0^+} \frac{ [\Re \nu](\bar{B}(x, r))}{\mu(\bar{B}(x, r))} + i \lim_{r \rightarrow 0^+} \frac{ [\Im \nu](\bar{B}(x, r))}{\mu(\bar{B}(x, r))} \\
				& = \lim_{r \rightarrow 0^+} \bigg[ \frac{ [\Re \nu](\bar{B}(x, r))}{\mu(\bar{B}(x, r))} + i \frac{ [\Im \nu](\bar{B}(x, r))}{\mu(\bar{B}(x, r))} \bigg] \\
				& = \lim_{r \rightarrow 0^+} \frac{\nu(\bar{B}(x, r))}{\mu(\bar{B}(x, r))}
			\end{align*}
		\end{enumerate}
	\end{proof}
	...
	\begin{ex}
		Let $(X, d)$ be a separable ultrametric space and $\mu \in \MM_+(X)$. If $(X, \MT_d)$ is locally compact, then $\mu$ is $d$-Vitali. $\mu \in \MM_+(X, d)$ and $\nu \in \MM(X)$.
	\end{ex}
	
	\begin{proof}
		\rex{ex:diff_radon_measures:covering_lemmas:vitali_measures:0004} 
	\end{proof}
	...
	\subsection{Continuity of Derivatives of Radon Measures}
	
	\tcr{add discussion on when the radon-nikodym derivative is continuous, try to generalize or reframe} \href{https://doi.org/10.1016/0167-6911(83)90047-6}{https://doi.org/10.1016/0167-6911(83)90047-6} in terms of the tools in the previous section to more general measures (possibly infinite ones?)  add to bibliography
	
	\tcr{need to show that measures with finite support are dense in $\MM(X)$ when $X$ is like a separable metric space}
...
\rex{ex:integration_complex_functions:00006.1} \rex{ex:diff_radon_measures:0013} 

\tcr{}

\begin{defn}
	\tcr{Define disintegration here or in section 6.6.1.19?}
	Let $(X, \MA, \mu)$ be a measure space, $(Y, \MB)$ a measurable space, $\pi: X \rightarrow Y$ and $(\mu_y)_{y \in Y} \subset M_+(X, \MA)$. Suppose that $\pi$ is $(\MA, \MB)$-measurable. Then $(\mu_{|y})_{y \in Y}$ is said to be a \tbf{disintegration of $\mu$ with respect to $\pi$} if 
	\begin{enumerate}
		\item for each $A \in \MA$, $\mu_{| \cdot}(A) \in \ML^+(Y, \MB)$,
		\item for each $A \in \MA$, 
		$$\mu(A \cap \pi^{-1}(B)) = \int_B \mu_{|y}(A) \, d \pi_*\mu(y) $$
		\item for $\pi_*\mu$-a.e. $y \in Y$, $\mu_{|y}$ is supported on $\pi^{-1}(\{y\})$ \tcr{(maybe make note and make/ref ex about how this implies that $\supp \mu_{|y} \subset  \cl \pi^{-1}(\{y\})$. )}. 
		\item for each $f \in L^1(X, \MA, \mu)$, 
		$$\int f \dmu = \int \bigg[ \int f \, d\mu_{|y}(x) \bigg] \, d \pi_*\mu(y)$$ \tcr{can I drop this and make into an exercise?}
	\end{enumerate}
\end{defn}

\begin{defn}
	Let $(X, \mu) \in \Obj(\bTopMsrC)$, $(Y,d)$ a metric space, $\pi \in \Hom_{\bMeas}(X, Y)$, $y \in Y$ and $\mu_{|y}^{\pi} \in M_1(X)$. Then $\mu_{|y}^{\pi}$ is said to be a \tbf{topological conditional measure of $\mu$ with respect to $\pi$ given $y$} if $(\mu_{|\pi^{-1}(\bar{B}(y, \ep))})_{\ep > 0}$ converges t \tcr{FINISH!!!}   
\end{defn}
...
\begin{ex}
	Let $(X, d_X)$ be a compact metric space and $(Y, d_Y)$ a locally compact separable ultrametric space.
\end{ex}

\tcr{For proof following Simmon's paper}, 

\begin{proof}
	Since $(X, \MT_{d_X})$ is compact, \tcr{an ex in analysis notes section on Polish spaces} implies that $(X, \MT_{d_X})$ is Polish. Since $(Y, \MT_{d_Y})$ is a separable LCH space, \tcr{make/ref ex in analysis notes section on Polish spaces} implies that $(Y, \MT_{d_Y})$ is Polish.
	\begin{itemize}
		\item Since $X$ is a metric space Since $X$ is compact, $\mu \in \MM_1(X)$
		\item For each $\phi \in C(X)$, define $\nu_{\phi} \in \MM(X)$ by 
		$$\nu_{\phi}(E) \defeq \int_E \phi \dmu.$$
		Then for each $\phi \in C(X)$, $\nu_{\phi} \ll \mu$. \rex{43012} then implies that for each $\phi \in C(X)$, $\pi_*\nu_{\phi} \ll \pi_*\mu$. 
		\item Since $(Y, \MT_{d_Y})$ is Polish and $\pi_*\mu \in M_1(Y)$, \rex{ex:borel_measures_metric_spaces:0008} implies that $\pi_*\mu \in \MM_1(Y)$. For each $\phi \in C(X)$ and $y \in Y$, define $(A_{\phi, n})_{n \in \N} \subset \Rg^Y$ and $N_{\phi}', N_{\phi} \subset Y$ by 
		\[
		A_{\phi, n}(y) \defeq 
		\begin{cases}
			\frac{\pi_*\nu_{\phi}(\bar{B}(y, n^{-1}))}{\pi_*\mu(\bar{B}(y, n^{-1}))}, & y \in \supp \pi_*\mu \\
			0, & y \not \in \supp \pi_*\mu,
		\end{cases}
		\]
		$$N_{\phi}' \defeq \{y \in Y: \text{$(A_{\phi, n}(y))_{n \in \N}$ does not converge}\}$$ 
		and 
		$$N_{\phi} \defeq N_{\phi}' \cup \bigg\{ y \in (N_{\phi}')^c:  \frac{d\pi_*\nu_{\phi}}{d\pi_*\mu}(y) \neq \limn A_{\phi,n}(y) \bigg\}.$$
		\begin{itemize}
			\item Let $\phi \in C(X)$.  \rex{ex:diff_radon_measures:009}, \rex{ex:measurable_functions:00014.1} and \rex{} \tcr{(show that ratios of measures of balls is measurable function of the center as done in previous ex, then cite in the prev ex and here to shorten)} implies that for each $n \in \N$, $A_{\phi, n} \in \Hom_{\Meas}((Y, \MB(Y)), (\C, \MB(\C)))$. \rex{25008.03} then implies that $N_{\phi}' \in \MB(X)$. Since $(A_{\phi,n})_{n \in \N} \subset \Hom_{\bMeas}((Y, \MB(Y)), (\C, \MB(\C)))$ and $\chi_{N_{\phi}'} \in \Hom_{\bMeas}((Y, \MB(Y)), (\C, \MB(\C)))$, \tcr{ref ex about products of measurable functions is measurable} implies that $(A_{\phi,n} \chi_{N_{\phi}'})_{n \in \N} \subset \Hom_{\bMeas}((Y, \MB(Y)), (\C, \MB(\C)))$. \rex{} \tcr{ref ex about limit of measurable functions is measurable} implies that $\limn A_{\phi,n}\chi_{N_{\phi}'} \in \Hom_{\bMeas}((Y, \MB(Y)), (\C, \MB(\C)))$. Since 
			$$N_{\phi} = N_{\phi}' \cup \bigg\{ y \in (N_{\phi}')^c: \limn [A_{\phi,n}\chi_{N_{\phi}'}](y) \neq \frac{d\pi_*\nu_{\phi}}{d\pi_*\mu} \chi_{N_{\phi}' (y)} \bigg\},$$ 
			we have that $N_{\phi} \in \MB(Y)$.  
			\item \rex{ex:differentiation_complex_measures:0002} implies that for $\mu$-a.e. $y \in Y$, $(A_{\phi,n}(y))_{n \in \N}$ converges and
			\begin{align*}
				\frac{d\pi_*\nu_{\phi}}{d\pi_*\mu}(y)
				& = \frac{d(\pi_*\nu_{\phi})_{\pi_*\mu}}{d\pi_*\mu}(y) \\
				& = \lim_{n \rightarrow \infty} \frac{ \pi_*\nu_{\phi}(\bar{B}(y, n^{-1}))}{\pi_*\mu(\bar{B}(y, n^{-1}))} \\
				& = \lim_{n \rightarrow \infty} A_{\phi,n}(y),
			\end{align*}
			we have that $\mu(N_{\phi}) = 0$. 
		\end{itemize}
		Since $\phi \in C(X)$ is arbitrary, we have that for each $\phi \in C(X)$, $N_{\phi} \in \MB(Y)$ and $\mu(N_{\phi}) = 0$.
	\end{itemize}
	
	
	
	
%	Since $X$ is compact Hausdorff, \tcr{Stone-Weierstrasss theorem} implies that there exists $\Phi \subset C(X)$ such that $\Phi$ is dense in $C(X)$. For each $\phi \in C(X)$, define $$  Define $N \in \MB(X)$ by $N \defeq \bigcup\limits_{\phi \in \Phi} N_{\phi}$.  
%	
	
	\item Since $X$ is compact Hausdorff, \rex{} \tcr{ref ex in analysis notes on compact Hausdorff spaces} implies that $C(X)$ is separable. Thus there exists $\Phi \subset C(X)$ such that $\Phi$ is dense in $C(X)$ and $\Phi$ is countable. Define $N_{\Phi} \in \MB(X)$ by $N_{\Phi} \defeq \bigcup\limits_{\phi \in \Phi}N_{\phi}$. Then 
	\begin{align*}
		\mu(N_{\Phi}) 
		& = \mu \bigg( \bigcup\limits_{\phi \in \Phi}N_{\phi} \bigg) \\
		& \leq \sum\limits_{\phi \in \Phi} \mu(N_{\phi}) \\
		& = 0.
	\end{align*}
	\item Let $\phi \in C(X)$. Since $\Phi$ is dense in $C(X)$, there exists $(\phi_k)_{k \in \N} \subset \Phi$ such that $\phi_k \convt{unif} \phi$. 
	\begin{itemize}
		\item For each $k \in \N$, define $\nu_k \in \MM(X)$ by $\nu_k \defeq \nu_{\phi_k}$. Then for each $k \in \N$, $\nu_k \ll \mu$. \tcr{ref ex} implies that $\pi_*\nu_k \ll \pi_* \mu$. Define $(A_{k, n})_{k,n \in \N} \subset \R^Y$ by 
		$$A_{k,n}(y) \defeq \frac{\pi_*\nu_k(\bar{B}(y, n^{-1}))}{\pi_*\mu(\bar{B}(y, n^{-1}))}.$$
		Then \rex{arg1} \tcr{ref newton quotient of measure exercise} implies that $(A_{k, n})_{k,n \in \N} \subset \Hom_{\bMeas}((Y, \MB(Y)), (\C, \MB(\C)))$ and \tcr{from above} we have that for each $k \in \N$ and $y \in N_{\Phi}^c$,
		\begin{align*}
			\frac{d\pi_*\nu_k}{d\pi_*\mu}(y)
			& = \frac{d(\pi_*\nu_k)_{\pi_*\mu}}{d\pi_*\mu}(y) \\
			& = \lim_{n \rightarrow \infty} \frac{ \pi_*\nu_k(\bar{B}(y, n^{-1}))}{\pi_*\mu(\bar{B}(y, n^{-1}))} \\
			& = \lim_{n \rightarrow \infty} A_{k,n}(y).
		\end{align*}
		\item Since $\phi_k \convt{unif} \phi$ and $\mu \in \MM_1(X)$, \rex{ex:integration_complex_functions:00006.1} implies that for each $y \in Y$ $A_{k,n}(y) \convt{unif} A_{\phi,n}(y)$. 
		
		\tcr{alter to $A_{k,E} \defeq \int_E \phi_k \dmu$ and move to section on integration of complex functions:} and $n \in N$,
		\begin{align*}
			|A_{k,n}(y) - A_{\phi,n}(y)|
			& = \bigg| \frac{ \pi_*\nu_k(\bar{B}(y, n^{-1}))}{\pi_*\mu(\bar{B}(y, n^{-1}))} - \frac{ \pi_*\nu_{\phi}(\bar{B}(y, n^{-1}))}{\pi_*\mu(\bar{B}(y, n^{-1}))} \bigg| \\
			& = \bigg| \frac{1}{\bar{B}(y, n^{-1})} \int_{\bar{B}(y, n^{-1})} \phi_k \dmu  - \frac{1}{\bar{B}(y, n^{-1})} \int_{\bar{B}(y, n^{-1})} \phi \dmu  \bigg| \\
			& = \bigg| \frac{1}{\bar{B}(y, n^{-1})} \int_{\bar{B}(y, n^{-1})} \phi_k - \phi \dmu  \bigg| \\
			& \leq \frac{1}{\bar{B}(y, n^{-1})} \int_{\bar{B}(y, n^{-1})} |\phi_k - \phi| \dmu \\
			&  < 
		\end{align*}
		
		
		\tcr{ref ex in analysis notes on Moore-Osgood theorem} then implies that for each $y \in N_{\Phi}^c$, $(A_{\phi, n}(y))_{n \in \N}$ converges, $([d\pi_*\nu_k/d\pi_*\mu](y))_{k \in \N}$ converges and
		\begin{align*}
			\limn A_{\phi, n}(y)
			& = \limn \bigg[ \limk A_{k, n}(y) \bigg] \\
			& = \limk \bigg[ \limn A_{k, n}(y) \bigg] \\
			& = \limk \frac{d\pi_*\nu_k}{d\pi_*\mu}(y). 
		\end{align*}
		Thus $N_{\Phi}^c \subset N_{\phi}^c$ and $N_{\phi} \subset N_{\Phi}$. Since $\phi \in C(X)$ is arbitrary, we have that for each $\phi \in C(X)$, $N_{\phi} \subset N_{\Phi}$. 
		
		\tcr{rework a little, define $I: Y \times C(X) \rightarrow \C$ by} $I(y, \phi) \defeq 
		\begin{cases}
			\limn \frac{\pi_*\nu_{\phi}(\bar{B}(y, 1/n))}{\pi_*\mu(\bar{B}(y, 1/n))}, & y \in N_{\Phi}^c \\
			0, & y \in N_{\Phi}. 
		\end{cases}
		$, then for each $\phi \in C(X)$, $I(\cdot, \phi) \in \ML(X)$, and for each $y \in Y$, $I_y \in C(X)^*$, $I_y$ is positive and $\|I_y\| = 1$. Thus for each $y \in Y$, there exists $\mu_{|y} \in \MM_1(X)$ such that for each $\phi \in C(X)$, $I_y = \int \phi \dmu_{|y}$. To show $\mu_{|\cdot}(A):Y \rightarrow [0,1]$ is measurable, since $C_c(X) = C(X)$ is dense in $L^1(X, \mu_{|y})$, we choose $(\phi_k)_{k \in \N} \subset C(X)$ such that $\phi_k \conv{L^1} \chi_A$. Then
		\begin{align*}
			\mu_{|y}(A)
			& = \int \chi_A \dmu_{|y} \\
			& = \int \limk \phi_k \dmu_{|y} \\
			& = \limk \int \phi_k \dmu_{|y} \\
			& = \limk I_y(\phi_k).
		\end{align*}
		
		\item \tcr{ref ex in analysis notes or early integration notes that integration is continous map on $L^1$} Define $\bar{I}:Y \times L^1(X, \mu_{|y})$ by $\bar{I}_y(\phi) \defeq \int \phi \dmu_{|y}$. Then $\bar{I}_y|_{C(X)} = I_y$. Let $A \in \MB(X)$. Since $C_c(X) = C(X)$ is dense in $L^1(X, \mu_{|y})$, there exists $(\phi_k)_{k \in \N} \subset C(X)$ such that $\phi_k \conv{L^1(X, \mu)} \chi_A$, \tcr{(show that this implies $\phi_k \conv{L^1(X, \mu_{|y})} \chi_A$.)}. 
		\begin{align*}
			\int_B \mu_{|y}(A) \, d \pi_* \mu(y) 
			& = \int_{\pi^{-1}(B)} \mu_{|\pi(x)}(A) \dmu(x) \\
			& = \int_{\pi^{-1}(B)} \bigg[ \int \chi_A \dmu_{|\pi(x)} \bigg] \dmu(x) \\
			& = \int_{\pi^{-1}(B)} \bar{I}_{\pi(x)}(\chi_A)  \dmu(x) \\
			& = \int_{\pi^{-1}(B)} \bar{I}_{\pi(x)} \bigg(\limk \phi_k \bigg) \dmu(x) \\
			& = \int_{\pi^{-1}(B)} \limk [\bar{I}_{\pi(x)} ( \phi_k )] \dmu(x) \quad \text{(\tcr{by convergence of $\phi_k$ in $L^1(X, \mu_{|y})$ and continuity of $\bar{I}_{\pi(x)}$ on $L^1(X, \mu_{|\pi(x)})$})} \\
			& = \int_{\pi^{-1}(B)} \limk [I_{\pi(x)} ( \phi_k )] \dmu(x) \\
			& = \int_{\pi^{-1}(B)} \limk [I^{\phi_k}(\pi(x))] \dmu(x) \\
			& = \int_{\pi^{-1}(B)} \bigg[ \limk I^{\phi_k} \bigg] (\pi(x)) \dmu(x) \quad \text{(\tcr{need to show $I^{\phi_k}$ converges p.w. a.e.})} \\
			& = \int_{\pi^{-1}(B)} \bigg[ \limk I^{\phi_k} \bigg] \circ \pi(x) \dmu(x) \\
			& = \int_{\pi^{-1}(B)} \bigg[ \limk I^{\phi_k} \bigg] \circ \pi \dmu \\
			& = \int_B \limk I^{\phi_k} \, d \pi_* \mu \\
			& = \limk \int_B I^{\phi_k} \, d \pi_* \mu \quad \text{(\tcr{need to show $I^{\phi_k}$ converges p.w. $\pi_* \mu$-a.e.})} \\
			& = \limk \int_B \frac{d \pi_*\nu_{\phi_k}}{d \pi_* \mu} \, d \pi_* \mu \\
			& = \limk \pi_*\nu_{\phi_k}(B) \\
			& = \limk \nu_{\phi_k}(\pi^{-1}(B)) \\
			& = \limk \int_{\pi^{-1}(B)} \phi_k \dmu \\
			& = \int_{\pi^{-1}(B)} \limk \phi_k \dmu \quad \text{(\tcr{by convergence in $L^1(X, \mu)$})} \\ 
			& = \int \chi_{\pi^{-1}(B)} \chi_A \dmu \\
			& = \int \chi_{A \cap \pi^{-1}(B)} \dmu \\
			& = \mu(A \cap \pi^{-1}(B)).
		\end{align*}
		\item 
		\begin{itemize}
			\item Let $B \in \MB(Y)$. \tcr{From above}, we have that
			\begin{align*}
				\int_Y \chi_B \, d \pi_* \mu
				& = \pi_* \mu(B) \\
				& = \mu(\pi^{-1}(B)) \\
				& = \int_Y \mu_{|y}(\pi^{-1}(B)) \, d \pi_* \mu(y). 
			\end{align*} 
			Therefore $\chi_B = \mu_{| \cdot }(\pi^{-1}(B))$ $\pi_* \mu$-a.e. Thus there exists $N_B \in \MB(Y)$ such that $\pi_* \mu(N) = 0$ and for each $y \in N^c$, 
			\begin{align*}
				\pi_* \mu_{| y }(B)
				& = \mu_{| y }(\pi^{-1}(B)) \\
				& = \chi_B(y) \\
				& = \del_y(B).
			\end{align*}
			Since $B \in \MB(Y)$ is arbitrary, we have that for each $B \in \MB(Y)$, there exists $N_B \in \MB(Y)$ such that $\mu(N_B) = 0$ and for each $y \in N_B^c$, $\pi_* \mu_{| y }(B) = \del_y(B)$. Hence for each $B \in \MB(Y)$, $(\pi_* \mu_{| y }(B))_{y \in Y} =_{\text{$\pi_*\mu$-a.e.}} (\del_y(B))_{y \in Y}$.
			\item Since $(Y, \MT_{d_Y})$ is separable, there exists $(y_j)_{j \in \N} \subset Y$ such that $(y_j)_{j \in \N}$ is $\MT_{d_Y}$-dense in $Y$. Define $\ME \subset \MT_{d_Y}$ by 
			$$\ME \defeq \{ B_{d_Y}(y_j, 1/m): j,m \in \N \} \cup \{X\}.$$
			Then $\ME$ is countable and \tcr{ref ex in metric spaces notes} implies that $\ME$ is a basis for $\MT_X$. Since $\pi(\ME) \subset \MB(Y)$, \tcr{the prev part} implies that for each $B \in \pi(\ME)$, $(\pi_* \mu_{|y}(B))_{y \in Y} =_{\text{$\pi_*\mu$-a.e.}} (\del_y(B))_{y \in Y}$. \rex{ex:complex_measures:0007.2} then implies that $(\pi_* \mu_{|y})_{y \in Y} =_{\text{$\pi_*\mu$-a.e.}} (\del_y)_{y \in Y}$. \rex{ex:complex_measures:0008.01} then implies that for $\pi_*\mu$-a.e. $y \in Y$, $\mu_{|y}$ is supported on $\pi^{-1}(\{y\})$.
		\end{itemize}
	\end{itemize}
	\tcr{need to generalize \rex{ex:diff_radon_measures:covering_lemmas:vitali_measures:0004} to locally compact spaces}
\end{proof}
...
\rex{ex:differentiation_complex_measures:0002}


~\cite{simmons2012conditional}
	...
	\begin{note}
	This section assumes familiarity with topological groups. 
	\end{note}
	...
	\begin{proof}
		...
		\begin{align}
			\mu(K) 
			& \leq \sum\limits_{k =1}^n \mu(x_kU) \\
			& = \sum\limits_{k =1}^n \mu(U) \\
			& = 0
		\end{align} 
	...
	\begin{equation*}
	H_{d, \del}(E) = \inf \bigg \{\sum\limits_{j \in \N} \diam(A_j)^d: (A_j)_{j \in \N} \in \MA_{E, \del} \bigg \}
	\end{equation*}
	\end{defn}
	...
	\begin{note}
	This section assumes familiarity with induced metrics on orbit spaces of metric spaces under isometric group actions. 
	\end{note}
	...
	\begin{proof}
	A previous exercise implies that $\bar{\mu} \ll \bar{H}_p^X$. Set $f = d \mu /d H_p^X$. Since $\mu$ is $G$-invariant, $f$ is $G$-invariant. Since $f$ is continuous, \tcr{an exercise in section $9.2$ of analysis notes } implies that $\bar{f}$ is continuous and $f = \bar{f} \circ \pi$. Let $E \in \MB(X/G)$. Then 
	\begin{align*}
...
\begin{proof}
	\tcr{Immediate by exercise about initial $\sig$-algebra.}
\end{proof}
...
\begin{ex} \textbf{Mourier's Theorem:} \\
	...
	\textbf{Hint:} Let $(x_n)_{n \in \N} \subset X$ be a dense subset. An exercise in the section on duality implies that there exist $(\phi_n)_{n \in \N} \subset X^*$ such that for each $n \in \N$, $\|\phi_n\| = 1$ and $\phi_n(x_n) = \|x_n\|$ and for each $x \in X$, $\|x\| = \sup\limits_{n \in \N} |\phi_n(x)|$. Then $ \cl B(0, 1) \in \ME(X)$.  \tcr{probability distributions on Banach spaces - Vakhania, pg 17}
\end{ex}

\begin{proof}
	Suppose that $X$ is separable. Then there exists $(x_n)_{n \in \N} \subset X$ such that $(x_n)_{n \in \N}$ is dense in $X$. tcr{An exercise from the section on duality in analysis notes } implies that there exists $(\phi_n)_{n \in \N} \subset X^*$ such that for each $n \in \N$, $\|\phi_n\| = 1$ and $\phi_n(x_n) = \|x_n\|$. A previous exercise implies that for each $x \in X$, $\|x\| = \sup\limits_{n \in \N} |\phi_n(x)|$. Let $x \in X$ and $r > 0$. Then $r^{-1}\|x - y \| = \sup\limits_{n \in \N} |r^{-1}\phi_n(x - y)|$ and 
	\begin{align*}
		\cl B(x,r)
		& = \{y \in X: \|x -y \| \leq r\} \\
...
\begin{proof}
	...
	\begin{align*}
		\|\MF(\mu)\| 
		& = \sup\limits_{\phi \in X^*} |\MF[\mu](\phi)| \\
		& \leq \|\mu\|
	\end{align*}
	which implies that $\MF \in L(M(X), C_b(X^*))$ and $\|\MF\| \leq 1$.
\end{proof}

More details can be found in ~\cite{vakhania2012probability}
...
\end{proof}
	...
	\begin{ex}
		Let $(X, \MA)$ be a measurable space, $Y$ a Banach space. Let $\phi, \psi \in S_Y(X, \MA)$ and $\lam \in \C$. Suppose that the standard representations of $\phi$ and $\psi$ are 
		$$\phi = \sum\limits_{j=1}^n\chi_{A_j}a_j$$ 
		and 
		$$\psi = \sum\limits_{j=1}^m\chi_{B_k}b_k$$ 
		respectively.
		\begin{enumerate}
			\item Then $(E_j)_{j=1}^n$ are disjoint and $\bigcup\limits_{j=1}^n E_j = X$.
			\item Set
			$$L \defeq \{(j,k) \in \N^2: j \in [n], k \in [m], \text{ and } A_j \cap B_k \neq \varnothing\}$$  
			Then the standard representation of $\phi + \psi$ is 
			$$\phi + \psi = \sum\limits_{(j,k) \in L} \chi_{A_j \cap B_k}(a_j + b_k)$$
		\end{enumerate} 
		\tcr{Clean UP, look at definition of standard rep in measruability section 3.3.028}
	\end{ex}
...
\end{align*}
	...
	\begin{ex} \lex{00000} 
	Let $(X, \MA, \mu)$ be a measure space, $Y$ a Banach space, $\phi \in S_Y$ and $A \in \MA$. Write $\phi = \sum\limits_{j=1}^n\chi_{E_j}y_j$ in the standard representation. Then $$\int_A \phi d\mu = \sum\limits_{j=1}^n \mu(A \cap E_j)y_j$$
	\end{ex}
	...
	&= \int \sum\limits_{(j,k) \in L} \chi_{A_j \cap B_k}(a_j + \lam b_k) d \mu \\
	&= \sum\limits_{(j,k) \in L} \mu(A_j \cap B_k)(a_j + \lam b_k) \\
	&= \sum\limits_{j = 1}^n \sum\limits_{k=1}^m \mu(A_j \cap B_k)(a_j + \lam b_k)  \\ 
	&= \sum\limits_{j = 1}^n \sum\limits_{k=1}^m \mu(A_j \cap B_k)a_j + \lam \sum\limits_{j = 1}^n \sum\limits_{k=1}^m \mu(A_j \cap B_k) b_k \\
	&= \sum\limits_{j = 1}^n \mu(A_j)a_j +  \lam\sum\limits_{k=1}^m \mu(B_k) b_k \\
...
Put $I_\phi = \lim\limits_{n \rightarrow \infty} \int \phi_n d\mu$ and $I_{\psi} = \lim\limits_{n \rightarrow \infty} \int \psi_n d\mu$. Then there exists $N_2 \in \N$ such that for each $n \in \N$, if $ n \geq N_2$, then $$\bigg \| \int \phi_n d\mu - I_\phi \bigg \| < \frac{\ep}{3}$$ and $$\bigg \| \int \psi_n d\mu - I_\psi \bigg \| < \frac{\ep}{3}$$ 
	...
	\begin{enumerate}
		\item \begin{align*}
			\|\psi_j\|_1 
			& = \int \|\psi_j\| \dmu \\
			& = \int \sum\limits_{n \in \N} \|y_n\| \chi_{E_n^j} \dmu \\
			& =  \sum\limits_{n \in \N} \int_{E_n^j} \|y_n\| \dmu \\
			& \leq \frac{j}{j-1} \sum\limits_{n \in \N} \int_{E_n^j} \|f\| \dmu \\
		...
		\begin{align*}
			\|\psi_j - f\|_1 
			& = \int \|\psi_j - f\| \dmu \\
			& = \int_{f^{-1}(\{0\})} \|\psi_j - f\| \dmu +  \sum\limits_{n \in \N} \int_{E_n^j} \|\psi_j - f\| \dmu \\
			& = \sum\limits_{n \in \N} \int_{E_n^j} \|y_n - f\| \dmu \\
			& \leq  \sum\limits_{n \in \N} \int_{E_n^j}  \frac{1}{j - 1} \|y_n\| \dmu \\
			& \leq  \sum\limits_{n \in \N} \int_{E_n^j}  \frac{1}{j - 1} \|f\| \dmu \\
...
\end{align*}	 
	...
	\begin{itemize}
		...
		\item \tcr{Talk abour sober measurable spaces (analogous to sober topological spaces) \href{https://ncatlab.org/nlab/show/sober+measurable+space}{https://ncatlab.org/nlab/show/sober+measurable+space}}
	\end{itemize}
	...
	\begin{defn}
		Let $(X, \MA)$ be a measurable space, $G$ a monoid and $\al:G \times X \rightarrow X$ a monoid action. 
		\begin{itemize}
			\item Let $E \in \MA$. Then $E$ is said to be \tbf{$\al$-invariant} if for each $g \in G$, $\al_g^{-1}(E) = E$.
			\item We define the \tbf{$\al$-invariant sub $\sig$-algebra of $\MA$}, denoted $\MA^{\al} \subset \MA$, by $\MA^{\al} \defeq \{E \in \MA: \text{$E$ is $\al$-invariant}\}$.
		\end{itemize}
	\end{defn}
	
	\begin{ex}
		Let $(X, \MA)$ be a measurable space, $G$ a monoid, $\al:G \times X \rightarrow X$ a monoid action. Then $\MA^{\al}$ is a $\sig$ algebra on $X$. 
	\end{ex}
	
	\begin{proof}\
		\begin{enumerate}
			\item Clearly, for each $g \in G$, $\al_g^{-1}(\varnothing) = \varnothing$. Hence $\varnothing \in \MA^{\al}$ and $\MA^{\al} \neq \varnothing$. 
			\item Let $E \in \MA^{\al}$. Then for each $g \in G$, 
			\begin{align*}
				\al_g^{-1}(E^c)
				& = [\al_g^{-1}(E)]^c \\
				& = E^c.
			\end{align*}
			Hence $E^c \in \MA^{\al}$. Since $E \in \MA^{\al}$ is arbitrary, we have that for each $E \in \MA^{\al}$, $E^c \in \MA^{\al}$.
			\item Let $(E_n)_{n \in \N} \subset \MA^{\al}$. Then for each $g \in G$,
			\begin{align*}
				\al_g^{-1} \bigg( \bigcup\limits_{n \in \N} E_n \bigg)
				& = \bigcup\limits_{n \in \N} \al_g^{-1}(E_n) \\
				& = \bigcup\limits_{n \in \N} E_n.
			\end{align*}
			Hence $\bigcup\limits_{n \in \N} E_n \in \MA^{\al}$. Since $(E_n)_{n \in \N} \subset \MA^{\al}$ is arbitrary, we have that for each $(E_n)_{n \in \N} \subset \MA^{\al}$, $\bigcup\limits_{n \in \N} E_n \in \MA^{\al}$. 
		\end{enumerate}
		Thus $\MA^{\al}$ is a $\sig$-algebra on $X$.
	\end{proof}
	
	\begin{defn}
		Let $(X, \MA, \mu)$ be a measure space, $G$ a monoid and $\al:G \times X \rightarrow X$ a monoid action. 
		\begin{itemize}
			\item Let $E \in \MA$. Then $E$ is said to be \tbf{$\mu$-a.e. $\al$-invariant} if for each $g \in G$, $\mu(\al_g^{-1}(E) \Del E) = 0$.
			\item We define the \tbf{$\al$-invariant sub $\sig$-algebra of $\MA$}, denoted $\MA^{\al} \subset \MA$, by $\MA^{\al} \defeq \{E \in \MA: \text{$E$ is $\al$-invariant}\}$.
		\end{itemize}
	\end{defn}
	
	\begin{defn}
		We define the \tbf{$\al$-invariant sub $\sig$-algebra of $\MA$}, denoted $\MA^{\al} \subset \MA$, by $\MA^{\al} \defeq \{E \in \MA: \text{$E$ is $\al$-invariant}\}$
	\end{defn}
	
	
	
	\tcr{Outline of de Finetti's theorem for standard Borel spaces:}
	\begin{enumerate}
		\item  Consider $X$ a compact Polish space like the Cantor space $\MC$. Then $\MM_1(X)$ is $w^*$-compact by Banach-Alaoglu theorem.
		\item Then $\MM_1^{\al}(X)$ is $w^*$-compact/closed and convex
		\item Choquet theorem implies that for each $\nu \in \MM_1^{\al}(X)$, there exists $\mu \in \MM_1(\MM_1^{\al}(X))$ such that $\supp \mu \subset \text{extreme points of $\MM_1^{\al}(X)$}$ and $\nu = \int_{\MM_1^{\al}(X)} \rho \dmu(\rho)$ (in a Pettis sense?). Then extend to a Bochner sense. 
		\item For general standard Borel space, $X$, map to Cantor space, apply theorem, then map back. 
		\item For de Finetti theorem specifically, 
		\begin{enumerate}
			\item Repeat argument for $Y = X^{\N}$ ~\cite{hewitt1955symmetric}
			\item Choquet theorem implies that for each $\nu \in \MM_1(X)$, since $\nu^{\otimes \N} \in \MM_1^{S^{\text{fin}}_{\N}}(X^{\N})$ there exists $\mu \in \MM_1(\MM_1^{S^{\text{fin}}_{\N}}(X^{\N}))$ such that $\supp \mu \subset \text{extreme points of $\MM_1^{S^{\text{fin}}_{\N}}(X^{\N})$}$ and $\nu = \int_{\MM_1^{S^{\text{fin}}_{\N}}(X^{\N})} \rho \dmu(\rho)$ (in a Pettis sense?). ~\cite{phelps2002lectures} ~\cite{stefansson1992pettis} Then extend to a Bochner sense. 
			\item For general standard Borel space, $X$, map to Cantor space, apply theorem, then map back. 
		\end{enumerate}
	...
	\begin{defn} \ld{00000} 
		Let $f:X \rightarrow \Rg$, Then we define $$\sum\limits_{x \in X} f(x) := \sup\limits_{\substack{F \subset X \\ F \text{ finite}}} \sum\limits_{x \in F} f(x)$$ This definition coincides with the usual notion of summation when $X$ is countable. For $f:X \rightarrow \C$, we can write $f = g +ih$ where $g,h:X \rightarrow \R$. If $$\sum\limits_{x \in X}|f(x)| < \infty,$$ then the same is true for $g^+,g^-,h^+,h^-$. In this case, we may define $$\sum\limits_{x \in X} f(x)$$ in the obvious way.
	\end{defn} 
	
	The following note justifies the notation $\sum\limits_{x \in X}f(x)$ where $f:X \rightarrow \C$.
	
	\begin{note}
		Let $f:X \rightarrow \C$ and $\al:X \rightarrow X$ a bijection. If $\sum\limits_{x \in X}|f(x)|< \infty$, then $\sum\limits_{x \in X}f( \al (x)) = \sum\limits_{x \in X}f(x) $.
	\end{note}
	...
	\begin{defn}
		\begin{itemize}\
			...
			\item $\bTopRadMsr$: 
			\begin{itemize}
				\item $\Obj(\Meas) \defeq \{(X, \mu): X \in \Obj(\bTop) \text{ and } \mu \in \MM_+(X) \}$
				\item $\Hom_{\Meas}((X, \mu), (Y, \nu)) \defeq \{f \in \Hom_{\bTop}(X, Y): f_*\mu = \nu\}$.
			\end{itemize}
			\item $\bTopRadMsrone$: 
			\begin{itemize}
				\item $\Obj(\Meas) \defeq \{(X, \mu): X \in \Obj(\bTop) \text{ and } \mu \in \MM_1(X) \}$
				\item $\Hom_{\Meas}((X, \mu), (Y, \nu)) \defeq \{f \in \Hom_{\bTop}(X, Y): f_*\mu = \nu\}$.
			\end{itemize}
		\end{itemize}
	\end{defn}
