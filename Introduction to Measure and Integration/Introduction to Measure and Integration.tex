...
\begin{proof}\
	\begin{enumerate}
		...
		\item Set $S \defeq \sup\limits_{j \in J} \mu_j(X_j)$. \tcr{(maybe do case when $S = \infty$)} Since $J$ is a directed set, $J \neq \varnothing$. Thus there exists $j_0 \in J$. Then
		\begin{align*}
			\mu_0(X) 
			& = \mu_0(\pi_j^{-1}(X_j)) \\
			& = \mu_{j_0}(X_{j_0}) \\
		...
		\item Since $\MB(X) = \sig(\MA_0)$ and $\mu_0$ is finite, \rex{ex:outer_measures:00008} implies that there exists a unique $\mu \in M_+(X)$ such that $\mu|_{\MA_0} = \mu_0$. Since $X$ is a Polish space \tcr{(assume)} and 
		\begin{align*}
			\mu(X)
			& = \mu_0(X) \\
		...
		\tcr{show $\mu$ is radon and consider category of top radon measr spaces}, to show radon, maybe assume $X$ is nice (polish space) and since $\mu$ is finite it is radon
		\tcr{Change} $\TopMsrpos$ to top radon measure positive
	\end{enumerate}
\end{proof}
