...
\end{align*}		
	...
	\begin{proof}
		Since $\MT \subset \MB(X)$ is a $\pi$-system on $X$, the previous exercise implies that for each $A \in \sig(\MT)$, $\mu(A) = \nu(A)$. Since $\sig(\MT) = \MB(X)$, $\mu = \nu$. 
	\end{proof}
...
\end{align*}	 
	...
	\begin{ex} \textbf{Mourier's Theorem:} \\
		...
		\textbf{Hint:} Let $(x_n)_{n \in \N} \subset X$ be a dense subset. An exercise in the section on duality implies that there exist $(\phi_n)_{n \in \N} \subset X^*$ such that for each $n \in \N$, $\|\phi_n\| = 1$ and $\phi_n(x_n) = \|x_n\|$ and for each $x \in X$, $\|x\| = \sup\limits_{n \in \N} |\phi_n(x)|$. Then $ \bar{B}(0, 1) \in \ME(X)$.  
	\end{ex}
	...
	\begin{proof}
		...
		\begin{align*}
			...
			& = \bigcap_{n \in \N} \phi_n^{-1}(\bar{B}_{\C}(\phi_n (x), r)) \\
			& \in \ME(X)
		\end{align*}
	...
	\begin{ex}
		Let $X$ be a real normed vector space and $\mu \in \MM(X)$. Then $\hat{\mu} \in C_b(X^*)$ (in the norm topology).
	\end{ex}

	\begin{proof}
		Let $\phi \in X^*$. Then $\hat{\mu} : X^* \rightarrow \C$ is bounded.
		\begin{align*}
			|\hat{\mu}(\phi)|
