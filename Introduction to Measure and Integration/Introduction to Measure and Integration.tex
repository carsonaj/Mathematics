...
\begin{document}
	...
	\begin{defn} \ld{00000} 
		...
		$$U_\MP f \defeq \sum_{i=1}^n M^f_i (x_i - x_{i-1})$$ 
		and we define the \textbf{lower Darboux sum} of $f$ with respect to $\MP$, denoted $L_\MP f$, by
		$$L_\MP f \defeq \sum_{i=1}^n m^f_i (x_i - x_{i-1})$$ 
	\end{defn}

	\begin{ex} \lex{00000} 
		Let $f \in \Bnd([a,b], \R)$ and $\MP$ a partition of $[a,b]$. Then $$\bigg[\inf_{x \in [a,b]} f(x) \bigg] (b-a) \leq L_\MP f \leq U_\MP f \leq \bigg[ \sup_{x \in [a,b]} f(x)  \bigg] (b-a)$$
	\end{ex}
...
\begin{defn} \ld{def:measurable_spaces:proj_limits:0001}
	Let $(J, \leq)$ be a directed set, $(X_j, \MA_j)_{j \in J}$ a collection of measurable spaces and for each $(j,k) \in \leq$, $\pi_{j,k}:X_k \rightarrow X_j$ a $(\MA_k, \MA_j)$-measurable map. Suppose that $((X_j, \MA_j)_{j \in J}, (\pi_{j,k})_{(j,k) \in \leq})$ is a $\Meas$-projective system. We define the \tbf{projective $\sig$-algebra} of $(\MA_j)_{j \in J}$ on $\varprojlim\limits_{j \in J} X_j$, denoted $\varprojlim\limits_{j \in J} \MA_j$, by $$\varprojlim\limits_{j \in J} \MA_j \defeq \sig ( \pi_j: j \in J).$$
\end{defn}
...
\end{proof}
	...
	\tcr{define and change notation from $\mu_f$ to $\mu^f(E) \defeq \int_E f \dmu$}. 
	...
	\begin{itemize}
		\item \tcr{In the previous section, define
			$$\mu_f(A) \defeq \int_A f \dmu$$ and in this section change notation to define
			$$\mu^f(A) \defeq \int_X f \dmu_A$$ and show that when $f$ is $\pi^*\MB$-measurable,
		$$\pi_* (\mu^f) = (\pi_* \mu)^{\pi^* f}$$ 
		show something similar for $\mu_f$, However, this conflicts with borel measure on $\R$ notation $\mu_F$}
	\item Actually, get rid of $\mu_f$, write $\mu^f$. Turns out $\mu_f = \mu^f$ (see previous section). and hide proof of $\mu^f$ being a measure in \rex{ex:integration_complex_functions:00006.1}.
	\end{itemize}
...
%	\begin{defn}
%		Let $(X, \MA, \mu)$ be a measure space and $f \in L^1(X, \MA, \mu)$. We define $\mu^f: \MA \rightarrow \R$ by $$\mu^f(E) \defeq \int_X f \dmu_E$$
%		\tcr{hide this definition, can show that $\int_X f \dmu_E = \int_E f \dmu$.}
%	\end{defn}
%
%
%	\begin{ex} \lex{ex:integration_complex_functions:00006.1}
%		\tcr{hide this exercise, can show that $\int_X f \dmu_E = \int_E f \dmu$.}
%		\tcr{change notation FIX!!!}
%		Let $(X, \MA, \mu)$ be a measure space and $f \in L^1(X, \MA, \mu)$. Suppose that $f \geq 0$.
%		Then $\mu^f \in M_+(X, \MA)$. \\
%		\tbf{Hint:} \rex{ex:outer_measures:00008.1}
%	\end{ex}
%	
%	\begin{proof} \
%		\tcr{hide this proof, can show that $\int_X f \dmu_E = \int_E f \dmu$.}
%		\begin{itemize}
%			\item Let $A, B \subset \MA$. Suppose that $A \cap B = \varnothing$. \rex{ex:subspace_measures:00002.1} and \rex{ex:integration_nonnegative_functions:0002} imply that 
%			\begin{align*}
%				\mu^f(A \cup B) 
%				& = \int_X f \dmu_{A \cup B} \\
%				& = \int_X f \, d(\mu_A + \mu_B)  \\
%				& = \int_X f \dmu_A + \int_X f \dmu_B \\
%				& = \mu^f(A) + \mu^f(B).
%			\end{align*}	
%			So $\mu^f$ is finitely additive. 
%			\item Let $(B_n)_{n \in \N} \subset \MA$. Suppose that $(B_n)_{n \in \N}$ is decreasing and $\inf\limits_{n \in \N} B_n = \varnothing$. 
%			\begin{itemize}
%				\item Suppose that $f$ is simple. Since $\mu(X) < \infty$, $(B_n)_{n \in \N}$ is decreasing and $\inf\limits_{n \in \N} B_n = \varnothing$, \rex{ex:measures:00004} implies that for each $A \in \MA$, $\mu(A \cap B_n) \rightarrow 0$. Then there exist $(a_k)_{k=1}^m \subset \Rg$ and $(E_k)_{k=1}^m \subset \MA$ such that $(E_k)_{k=1}^m$ is disjoint and $f = \sum\limits_{k=1}^m a_k \chi_{E_k}$. Then 
%				\begin{align*}
%					\mu^f(B_n)
%					& = \int_X f \dmu_{B_n} \\
%					& = \sum_{k=1}^m a_k \mu_{B_n}(E_k) \\
%					& = \sum_{k=1}^m a_k \mu(E_k \cap B_n) \\
%					& \rightarrow 0.
%				\end{align*}
%				\item Suppose that $f$ is not simple. Then there exists $(\phi_j)_{j \in \N} \subset S^+(X, \MA)$ such that $\phi_j \convt{p.w.} f$ and for each $j \in \N$, $\phi_j \leq \phi_{j+1}$. Let $\ep >0$. Dominated convergence implies that 
%				$$\lim_{j \rightarrow \infty} \int_X f - \phi_j \dmu = 0.$$ 
%				Therefore there exists $j \in \N$ such that 
%				$$\int_X f - \phi_j \dmu < \frac{\ep}{2}.$$
%				The previous part implies that there existst $N \in \N$ such that for each $n \in \N$, $n \geq N$ implies that $\mu_{\phi_j}(B_n) < \ep/2$. \rex{ex:subspace_measures:00002.1} and \rex{ex:integration_nonnegative_functions:0003} then imply that
%				\begin{align*}
%					\mu^f(B_n)
%					& = \bigg[ \mu^f(B_n) - \mu_{\phi_j}(B_n) \bigg] + \mu_{\phi_j}(B_n) \\
%					& = \bigg[ \int_X f \dmu_{B_n} - \int_X \phi_j \dmu_{B_n} \bigg] + \mu_{\phi_j}(B_n) \\
%					& = \int_X f - \phi_j  \dmu_{B_n} + \mu_{\phi_j}(B_n) \\
%					& \leq  \int_X f - \phi_j  \dmu + \mu_{\phi_j}(B_n) \\
%					& < \frac{\ep}{2} + \frac{\ep}{2} \\
%					& = \ep.
%				\end{align*}
%				Since $\ep >0$ is arbitrary, we have that for each $\ep > 0$, there exists $N \in \N$ such that for each $n \in \N$, $n \geq N$ implies that $\mu^f(B_n) < \ep$. Hence $\mu^f(B_n) \rightarrow 0$. 
%			\end{itemize}
%		\end{itemize}
%		\rex{ex:outer_measures:00008.1} then implies that $\mu^f \in M_+(X, \MA)$.
%	\end{proof}
...
\end{align*}		
	...
	\begin{proof}\
		\begin{itemize}
			\item $(\implies)$: \\
			Suppose that $X$ is $T_1$. Let $E \in \MB(X)$. Let $x \in X$.
			\begin{enumerate}
				\item Since $\del_x(E) \in \{0,1\}$, $E$ is compact implies that $\del_x(E) < \infty$.
				\item 
				\begin{itemize}
				...
				\end{itemize}
				Therefore $\del_x$ is outer regular on $E$. 
				\item Suppose that $E$ is open.
				\begin{itemize}
					\item Suppose that $x \in E$. Define $K \subset X$ by $K \defeq \{x\}$. Then $K \subset E$ and \tcr{ref ex in topology compactness section} implies that $K \in \MK(X)$. Therefore 
					\begin{align*}
						\del_x(E)
						& = 1 \\
						& = \del_x(K) \\
						& \leq \sup V^{\del_x}_I(E) \\
						& \leq \del_x(E).
					\end{align*}
					Hence $\del_x(E) = \sup V^{\del_x}_I(E)$.
					\item Suppose that $x \not \in E$. Then for each $K \in \MK(X)$, $K \subset E$ implies that $\del_x(K) = 0$. Hence
					\begin{align*}
						\del_x(E) 
						& = 0 \\
						& = \sup V^{\del_x}_I.
					\end{align*}
				\end{itemize}
				Therefore $\del_x$ is inner regular on $E$. 
			\end{enumerate}
	...
	\tcr{note that for $K \in \MK(X)$, $K \cap E \in \MK(E)$, ref ex in topology notes, need to show that so if $\MK(X) \subset \MB(X)$, then $\MK(E) \subset \MB(E) = \MB(X) \cap E$}
	...
	\begin{ex} \lex{ex:borel_msr:proj_systems:0004} \tbf{Kolmogorov Extension Theorem:} \\
		Let $(J, {\leq})$ be a directed set and $((X_j, \MT_j, \mu_j)_{j \in J}, (\pi_{j,k})_{(j,k) \in {\leq}})$ a $\TopRadMsr$-projective system. Set $X \defeq \varprojlim\limits_{j \in J} X_j$ and $\MT \defeq \varprojlim\limits_{j \in J} T_j$. Suppose that 
		\begin{itemize}
			\item $J$ is countable
		...
		\begin{enumerate}
			...
			\item there exists a unique measure $\mu \in \MM_+(X)$ such that $((X, \mu), (\pi_j)_{j \in J})$ is a $\TopRadMsr$-projective limit of $((X_j, \mu_j)_{j \in J}, (\pi_{j,k})_{(j,k) \in {\leq}})$. 
		\end{enumerate}
	\end{ex}
	...
	\begin{proof}\
		\begin{enumerate}
			...
			\item Set $\MC \defeq \TopRadMsr$. Since $\sig(\MA_0) = \MB(X, \MT)$ and $\mu_0$ is finite, \rex{ex:outer_measures:00008} implies that there exists a unique $\mu \in M_+(X)$ such that $\mu|_{\MA_0} = \mu_0$. Since for each $j \in J$, $X_j$ is a Polish space, \tcb{an exercise in the analysis notes section on Polish spaces} implies that $X$ is a Polish space. Since 
			\begin{align*}
				\mu(X)
				& = \mu_0(X) \\
			...
			\begin{itemize}
				...
				\begin{itemize}
					...
					\item \tbf{Uniqueness:} \\
					Clear by uniqueness of $\phi$ in $\Top$. \tcr{(add details)}
				\end{itemize}
			\end{itemize}
	...
	\begin{ex}
		\tcr{Here make an exercise about $\sup_{j \in J} \mu_j(X_j) < \infty$ iff there exists a complex measure $\mu \in \MM(X)$ such that it gives a $\TopRadMsrC$ projective limit. Then relate/comment/explore how this is related to the feynman path integral nonexistence, which is why we do a wick rotation.}
	\end{ex}
	...
	\begin{proof}\
		\begin{itemize}
			...
			\end{align*}
			\rex{25005} \tcr{(or maybe other exercise)} and \tcr{need exercise about uniqueness of product measure in section on product measures} implies that $\pi_{n,k}^*\nu_k = \nu_n$. Thus $((Y_n, \nu_n)_{n \in \N}, (\tau_{n,k})_{n \leq k})$ is a $\TopRadMsr$-projective system. \rex{??} \tcr{make exercise about this}	implies that $(Y, (\tau_n)_{n \in \N}) = \varprojlim\limits_{n \in \N} ((Y_n)_{n \in \N}, (\tau_{n, k})_{n \leq k})$ in $\Top$.  We observe that 
			\begin{itemize}
	...
	\begin{ex} \lex{ex:diff_radon_measures:covering_lemmas:vitali_measures:0003}
		Let $(X, d)$ be a metric space, $\phi: \Rg \rightarrow \Rg$ and $\mu \in M_+(X)$. Suppose that $\phi$ is strictly increasing and $\Top$-metric preserving. Then $\mu$ is $d$-Vitali iff $\mu$ is $d^{\phi}$-Vitali.
	\end{ex}
	
	\begin{proof}
		Set $d' \defeq d^{\phi}$, Since $\phi$ is increasing and $\Top$-metric preserving, \tcr{an exercise in the anlysis notes section on metric eqivalent metrics} implies that 
		\begin{itemize}
			\item $\phi^{-1}(\{0\}) = \{0\}$
...
\begin{proof}
	...
	\begin{itemize}
		...
		\item For each $\phi \in C(X)$ and $y \in Y$, define $(A_{\phi, n}(y))_{n \in \N} \subset \Rg$ and $N_{\phi} \in \MB(X)$ by 
		$$A_{\phi, n}(y) \defeq \frac{\pi_*\nu_{\phi}(\bar{B}(y, n^{-1}))}{\pi_*\mu(\bar{B}(y, n^{-1}))}$$
		and $N_{\phi} \defeq \{y \in Y: \text{$(A_{\phi, n}(y))_{n \in \N}$ does not converge}\}$. 
		\item \rex{ex:differentiation_complex_measures:0002} implies that
		\begin{align*}
			\frac{d\pi_*\nu_{\phi}}{d\pi_*\mu}(y)
