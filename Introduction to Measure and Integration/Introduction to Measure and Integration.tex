...
\end{align*}		
	...
	\begin{ex}
		Let $X$ be a topological space. Suppose that $\MK(X) \subset \MB(X)$. Then $X$ is $T_1$ iff for each $x \in X$, $\del_x \in \MM_1(X)$.
	\end{ex}
	
	\begin{proof}
		\begin{itemize}
			\item $(\implies)$: \\
			Suppose that $X$ is $T_1$. Let $E \in \MB(X)$.
			\begin{enumerate}
	...
	\tcr{note that for $K \in \MK(X)$, $K \cap E \in \MK(E)$, ref ex in topology notes}
	...
	\begin{proof}\
		\begin{enumerate}
			...
			\item Set $\MC \defeq \TopRadMsrpos$. Since $\sig(\MA_0) = \MB(X, \MT)$ and $\mu_0$ is finite, \rex{ex:outer_measures:00008} implies that there exists a unique $\mu \in M_+(X)$ such that $\mu|_{\MA_0} = \mu_0$. Since for each $j \in J$, $X_j$ is a Polish space, \tcb{an exercise in the analysis notes section on Polish spaces} implies that $X$ is a Polish space. Since 
			\begin{align*}
				\mu(X)
				& = \mu_0(X) \\
	...
	\begin{proof}\
		\begin{itemize}
			...
			\end{align*}
			\rex{25005} \tcr{(or maybe other exercise)} and \tcr{need exercise about uniqueness of product measure in section on product measures} implies that $\pi_{n,k}^*\nu_k = \nu_n$. Thus $((Y_n, \nu_n)_{n \in \N}, (\tau_{n,k})_{n \leq k})$ is a $\TopRadMsrpos$-projective system. \rex{??} \tcr{make exercise about this}	implies that $(Y, (\tau_n)_{n \in \N}) = \varprojlim\limits_{n \in \N} ((Y_n)_{n \in \N}, (\tau_{n, k})_{n \leq k})$ in $\Top$.  We observe that 
			\begin{itemize}
