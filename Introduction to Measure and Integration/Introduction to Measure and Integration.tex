...
\end{align*}		
	...
	\begin{note}
		In this section, some methods from Banach space theory are used. See analysis notes for details.
	\end{note}
	...
	\begin{proof}\
		\begin{enumerate}
			...
			\item Let $f \in L^1(X, \MA, \mu)$. For each $B \in \MB$, 
			\begin{align*}
				\nu_{P_{\MB}f}(B) 
				& = \int P_{\MB} f \dmu \\
				& = \int P_{\MB} f \dmu_{\MB} \\
			...
			\begin{align*}
				...
				& =  P_{\MB} f
			\end{align*}
			Hence $P^2_{\MB} = P_{\MB}$ and $P_{\MB}$ is idempotent.
		\end{enumerate}
	\end{proof}
	...
	\begin{ex} 
		Let $(X, \MA, \mu)$ be a measure space, $(Y, \MB)$ a measurable space, $f \in L^1(X, \MA, \mu)$ and $g: X \rightarrow Y$. Suppose that for each $y \in Y$, $\{y\} \in \MB$, $g$ is surjective and $g$ is $(\MA, \MB)$-measurable. Then there exists a unique $\phi \in L^0(Y, \MB)$ such that $\phi \circ g = \MP_{g^*\MB}f$. \\
		\textbf{Hint:} Doob-Dynkin lemma
	\end{ex}	
	
	\begin{proof}
		Since $P_{g^*\MB}f \in L^1(X, g^*\MB, \mu_{g^*\MB})$ and $\MB$, the Doob-Dynkin lemma implies that there exists a unique $\phi \in L^0(Y, \MB)$ such that $\phi \circ g = P_{g^*\MB}f$. 
	\end{proof}

	\begin{defn} 
		Let $(X, \MA)$ and $(Y, \MB)$ be measure spaces. and $\nu: Y \times \MA \rightarrow \Rg$. Then $\nu$ is said to be a \textbf{transition kernel from $(Y, \MB)$ to $(X, \MA)$} if 
		\begin{enumerate}
			\item for each $A \in \MA$, $\nu(\cdot, A)$ is $\MB$-measurable
			\item for each $y \in Y$, $\nu(y, \cdot)$ is a measure on $(X, \MA)$
		\end{enumerate}
	\end{defn}

	\begin{defn}
		Let $(X, \MA, \mu)$ be a finite measure space, $(Y, \MB)$ a measurable space and $g: X \rightarrow Y$. Suppose that for each $y \in Y$, $\{y\} \in \MB$, $g$ is surjective and $g$ is $(\MA, \MB)$-measurable. For $A \in \MA$, define $\phi_A \in L^0(Y, \MB )$ to be the unique $\phi \in L^0(Y, \MB)$ such that $\phi \circ g = P_{g^*\MB} \chi_A$. For $y \in Y$, we define the \textbf{conditional of $\mu$ on $y$}, denoted $\mu_y : \MA \rightarrow \Rg$, by $\mu_y(A) = \phi_A(y)$. 
	\end{defn}

	\begin{ex}
		Let $(X, \MA, \mu)$ be a finite measure space, $(Y, \MB)$ a measurable space and $g: X \rightarrow Y$. Suppose that for each $y \in Y$, $\{y\} \in \MB$, $g$ is surjective and $g$ is $(\MA, \MB)$-measurable. Define $\nu: Y \times \MA \rightarrow \Rg$ by $\nu(y, A) = \mu_y(A)$. Then $\nu$ is a transition kernel from $(Y, \MB)$ to $(X, \MA)$.  
	\end{ex}

	\begin{proof}\
		\begin{enumerate}
			\item Let $A \in \MA$. Since $\phi_A \in L^0(Y, \MB)$, $\nu(\cdot, A) = \phi_A(\cdot)$ is $\MB$-measurable.
			\item Let $y \in Y$
			\begin{itemize}
				\item Since $\chi_{\varnothing} = 0$, $P_{g^*\MB}\chi_{\varnothing} = 0$. Therefore
				\begin{align*}
					0 \circ g 
					& =  0 \\
					& = P_{g^*\MB}\chi_{\varnothing}
				\end{align*}
				Uniqueness of $\phi_{\varnothing}$ implies that $\phi_{\varnothing} = 0$. Hence 
				\begin{align*}
					\nu(y, \varnothing) 
					& = \mu_y(\varnothing) \\
					& = \phi_{\varnothing}(y) \\
				...
				\begin{align*}
					...
					& = \sum\limits_{j \in \N} \phi_{A_j} \circ g 
				\end{align*}
				Since $g$ is surjective, $\phi_{\bigcup\limits_{j \in \N} A_j} = \sum\limits_{j \in \N} \phi_{A_j}$. Hence 
				\begin{align*}
					\nu \bigg(y, \bigcup\limits_{j \in \N} A_j \bigg)
					& = \mu_y \bigg( \bigcup\limits_{j \in \N} A_j \bigg) \\
			...
			Hence $\nu(y, \cdot)$ is a measure on $(X, \MA)$. 
		\end{enumerate}
		\textbf{FINISH!!!} there are still some a.e. technicalities to address. 
	\end{proof}
