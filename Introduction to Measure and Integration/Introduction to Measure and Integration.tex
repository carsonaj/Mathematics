...
\end{itemize}		
	...
	\begin{defn} \ld{00000} 
		Let $(X, \MA, \mu)$ be a measure space and $(Y, \MB)$ a measurable space. Define 
		\begin{itemize}
			\item $L^{+}(X, \MA, \mu) = \{f:X \rightarrow \RG : f \text{ is measurable}\}$
			\item $L^0(X, \MA, \mu) = \{f:X \rightarrow \C : f \text{ is measurable}\}$ 
			\item $L_Y^{0}(X, \MA, \mu) = \{f:X \rightarrow Y : f \text{ is measurable}\}$
		\end{itemize}
	\end{defn}
	...
	\begin{ex}
		...
		\textbf{Hint:} set $\MG = \{\pi_{\al}^{-1}(E_{\al}): \al \in A \text{ and } E_{\al} \in \ME_{\al}\}$ and for $\al \in A$, consider the pushforward $\sig$-algebra on $X_{\al}$, $\pi_{\al}^* \sig( \MG)$
	\end{ex}
 	...
 	\begin{proof}
 		...
 		\end{align*}
 		Let $\al \in A$. By definition, for each $V \subset X_{\al}$, $V \in \pi_{\al}^* \sig(\MG)$ iff $\pi_{\al}^{-1}(V) \in \sig(\MG)$. Thus $\ME_{\al} \subset \pi_{\al}^* \sig(\MG)$ which implies that 
 		\begin{align*}
 			\MA_{\al} 
 			& = \sig(\ME_{\al}) \\
 			& \subset \pi_{\al}^* \sig(\MG)
 		\end{align*}
 		Since $\al \in A$ is arbitrary, $\bigotimes\limits_{\al \in A} \MA_{\al} \subset \sig(\MG)$.
 	\end{proof}
 	...
 	\begin{ex}
 		Let $(X_j, \MT_j)_{j = 1}^n$ be a collection of topological spaces.  Then 
 		\begin{enumerate}
 			\item $$\bigotimes\limits_{j=1}^n  \MB(X_j) \subset \MB \bigg( \prod_{j=1}^n X_{j} \bigg)$$
 			\item if for each $j \in \{1, \ldots, n\}$, $X_j$ is second-countable, then $$\bigotimes\limits_{j=1}^n  \MB(X_j) = \MB \bigg( \prod_{j=1}^n X_{j} \bigg)$$
 		\end{enumerate}
 	\end{ex}
 
 	\begin{proof} Set $X = \prod_{j=1}^n X_j$ and denote the product topology on $X$ by $\MT$. 
 		\begin{enumerate}
 			\item By definition, $\MB(X) = \sig(\MT)$ and for each $j \in \{1, \ldots, n \}$, $X_j \in \MT_j$ and $\MB(X_j) = \sig(\MT_{j})$. Set 
 			$$\MB = \bigg \{ \prod\limits_{j=1}^n E_j:  \text{for each $j \in \{1, \ldots, n\}$, } E_j \in \MT_j \bigg \}$$ 
 			The previous exercise implies that $\bigotimes_{j=1}^n \MB(X_j) = \sig(\MB)$. Since $\MB$ is a basis for $\MT$ we have that $\MB \subset \MT$. Thus
 			\begin{align*}
 				\bigotimes_{j=1}^n \MB(X_j) 
 				& = \sig(\MB) \\
 				& \subset \sig(\MT) \\
 				& = \MB(X)
 			\end{align*}
 			\item Suppose that for each $j \in \{1, \ldots, n\}$, $X_j$ is second-countable. Let $\j \in \{1, \ldots, n\}$. Since $X_j$ is second-countable, there exists $\MB_j \subset \MT_j$ such that $\MB_j$ is a countable basis for $\MT_j$. Set $\MB =  \bigg\{ \prod\limits_{j=1}^n B_j: \text{for each $j \in \{1, \ldots, n\}$, $B_j \in \MB_j$} \bigg\}$. Then $\MB \subset \bigotimes\limits_{j=1}^n \MB(X_j)$ and $\MB$ is a countable basis for $\MT$. An exercise in the section on $\sig$-algebras implies that $\MB(X) = \sig(\MB)$. Hence
 			\begin{align*}
 				\MB(X) 
 				& = \sig(\MB) \\
 				& \subset \bigotimes_{j=1}^n \MB(X_j) 
 			\end{align*}
 		\end{enumerate}
 	\end{proof}
	...
	\begin{thm} \textbf{Dynkin's Lemma:} \\
		Let $X$ be a set.
		\begin{enumerate}
			\item Let $\MP$ be a $\pi$-system on $X$ and $\ML$ a $\lam$-system on $X$. If $\MP \subset \ML$, then $\sig(\MP) \subset \ML$.
			\item Let $\MP$ be a $\pi$-system on $X$. Then $\sig(\MP) = \lam(\MP)$
		\end{enumerate} 
...
\end{align*}		
	...
	\begin{proof}
		For each $n, k \in \N$, define $E_{n, k} = \{x \in X: \vert f_n(x) - f(x) \vert \geq \frac{1}{k} \}$ and $F_{n,k} = \bigcup\limits_{m \geq n}E_{m,k}$. Then $F_{n,k}$ is decreasing in $n$ and $$\bigcap\limits_{n \in \N}F_{n,k} \subset \{x: f_n(x) \not \rightarrow f(x)\}$$ 
	...
	\begin{ex} \lex{43007} 
		Let  $(X, \MA)$ be a measurable space, $\nu \in \MM(X, \MA)$ and $c \in \C$. Then $\vert c \nu \vert = \vert c \vert \vert \nu \vert$.
	\end{ex}
	...
	\begin{ex} \lex{43010} 
	Let $(X, \MA)$ be a measurable space, $\mu_1, \mu_2 \in \MM(X, \MA)$ and $\lam \in \C$. Then for each $f \in L^1(\mu_1 + \lam \mu_2)$, $$\int f d(\mu_1 + \lam \mu_2) = \int f d \mu_1 + \lam \int f d\mu_2$$
	\end{ex}
	...
	\newpage
	\section{Applications to Differentiation}
...
\end{enumerate}	 
	...
	\begin{defn} \ld{00000} 
	Let $X$ be a topological space. For $\mu \in \MM(X)$, define $I_{\mu} :C_0(X) \rightarrow \C$ by $$I_{\mu} (f) = \int f d\mu$$.
	\end{defn}
	...
	\begin{defn} \ld{00000} 
	Let $X$ be a LCH space, $(\mu_n)_{n \in \N} \subset \MM(X)$ and $\mu \in \MM(X)$. Then $\mu_n$ is said to  \textbf{converge to $\mu$ in weak-*}, denoted $\mu_n \conv{w^*}\mu$, if $I_{\mu_n} \conv{w^*} I_{\mu}$, i.e. for each $f \in C_0(X)$, $$\int f d\mu_n \rightarrow \int f d\mu$$
	\end{defn}
	...
	\begin{ex} \lex{00000} 
		...
		\begin{enumerate}
			...
			\item for each $f \in L^1 \cup L^+$, $$\int R_{y^{-1}} f d \mu = \Del(y) \int f d \mu$$
		\end{enumerate}
	\end{ex}
	...
	\begin{proof}\
		\begin{enumerate}
			...
			\begin{align*}
				\int R_{y^{-1}} f d \mu 
				& = \int R_{y^{-1}} \chi_U d \mu \\
				& = \int \chi_{Uy} d \mu \\
				& = \mu(Uy) \\
				&= \mu(r_y^{-1}(U)) \\
				&= {r_y}_* \mu(U) \\
				& = \Del(y) \mu(U) \\
				& = \Del(y)  \int \chi_U d \mu \\
				&= \Del(y)  \int f d \mu
			\end{align*}
			By linearity of $R_{y^{-1}}$, for $f \in S^+$, $$\int R_{y^{-1}} f d \mu = \Del(y) \int f d \mu$$
			...
			\begin{align*}
				\int R_{y^{-1}} f d \mu 
				& = \limn \int R_{y^{-1}} \phi_n d \mu \\
				& = \limn \Del(y) \int \phi_n d \mu \\
				& = \Del(y) \int f d \mu \\
			\end{align*}
			Let $f \in L^1$. If $f$ is real valued, write $f = f^+ - f^-$. Then $R_{y^{-1}} f = R_{y^{-1}} f^+ - R_{y^{-1}} f^-$ and 
			\begin{align*}
				\int R_{y^{-1}} f d \mu 
				& = \int R_{y^{-1}} f^+ d \mu - \int R_{y^{-1}} f^- d \mu \\
				& = \Del(y) \int f^+ d \mu - \Del(y) \int f^- d \mu \\
				& = \Del(y) \int f d \mu
			\end{align*}
			If $f$ is complex valued, write $f = g + ih$ with $g, h \in L^1$ real valued. Then 
			\begin{align*}
				\int R_{y^{-1}} f d \mu 
				& = \int R_{y^{-1}} g d \mu + i \int R_{y^{-1}} h d \mu \\
				& = \Del(y) \int g d \mu +i \Del(y) \int h d \mu \\
				& = \Del(y) \int f d \mu
			\end{align*}
		\end{enumerate}
	\end{proof}
	...
	\begin{align*}
	\int_A L_g f d\mu
	&= \int_A f \circ l_{g}^{-1} \,d\mu \\
	&= \int_A f \circ l_g^{-1} \, d\mu \\
...
\end{align*}	 
	...
	\newpage
	\section{TODO}
	Add background for banach space valued measures like riesz representation theorem and radon-nikodym derivatives to be be able to talk about condition expectation of banach space valued random variables
