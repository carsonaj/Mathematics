...
\begin{document}
	...
	\begin{defn}
		Let $X$ be a set, $\mu$ an outer measure on $X$, $\MF \subset \MP(X)$ and $A \subset X$. Then $\MF$ is said to \tbf{$\mu$-cover $A$} if 
		$$\mu \bigg(A \setminus \bigg[ \bigcup_{F \in \MF} F \bigg] \bigg) = 0$$
	\end{defn}
...
\end{align*}		
	...
	\newpage
	\section{Differentiation on ?? Spaces}
	...
	\subsection{Borel Outer Measures}
	
	\begin{defn}
		Let $X$ be a topological space and $\mu$ and outer measure on $X$. Then $\mu$ is said to be \tbf{Borel} if $\MB(X) \subset \MA_{\mu}$. 
	\end{defn}
	...
	\begin{ex}
		Let $\mu, \nu$ be Radon measures. Then for each $A \in \MB(X)$,  
