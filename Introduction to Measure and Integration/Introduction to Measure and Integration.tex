...
\end{proof}
	...
	\begin{note}
		We note that since $\{0,1\}$, $\N$ and $[0,1]$ are second-countable and $\MC$, $\MN$ and $\H$ are countable products of second-countable spaces, we have that $\MC$, $\MN$ and $\H$ are second-countable.
	\end{note}
	...
	\begin{proof}\
		\begin{enumerate}
			...
			\item Suppose that for each $n \in \N$, $f_n$ is a measurable isomorphism. The previous part implies that $f$ is a bijection. Define $g: B \rightarrow Z$ by $g \defeq ( f_n^{-1}|_B)_{n \in \N}$. Then for each $x \in B$, $g(x)_m \defeq f_m^{-1}(x)$. Therefore for each $x \in B$,
			\begin{align*}
				f \circ g (x)
				& = f_1 \circ \pi_1(g(x)) \\
				& = f_m \circ \pi_m(g(x)) \\
			...
			\end{itemize} 
			\item Suppose that for each $n \in \N$, $Z_n$ is zero-dimensional. \tcr{An exercise in the previous section (about products)} implies that $Z_0$ is zero-dimensional. Since $Z \subset Z_0$, \rex{} \tcr{An exercise in the previous section (about subsapces)} implies that $Z$ is zero-dimensional.
		\end{enumerate}
	\end{proof}

	\begin{ex} \lex{ex:measurable_spaces:standard_borel_spaces:0022}
		Let $X$ be a Polish space, $(Z_n)_{n \in \N}$ a collection of Polish spaces, $(B_n)_{n \in \N} \subset \MP(X)$ and $(f_n)_{n \in \N} \in \prod\limits_{n \in \N} B_n^{Z_n}$. Suppose that $(B_n)_{n \in \N}$ is disjoint and for each $n \in \N$, $f_n$ is continuous and $f_n$ is surjective. 
		\begin{enumerate}
			...
			\item If for each $n \in \N$, $f_n$ is surjective, then $f$ is surjective.
			\item If for each $n \in \N$, $f_n$ is injective, then $f$ is injective.
			\item If for each $n \in \N$, $f_n$ is a measurable isomorphism, then $f$ is a measurable isomorphism. \tcr{FINISH!!!}
		\end{enumerate}
	\end{ex}
	...
	\begin{proof}
		...
		\begin{enumerate}
			\item \rex{ex:measurable_spaces:standard_borel_spaces:0021} implies that there exists a Polish space $Z$ and $f:Z \rightarrow \bigcap\limits_{n \in \N} A_n$ such that $f$ is continuous and surjective. Thus $\bigcap\limits_{n \in \N} A_n$ is analytic. \tcr{FIX!!! these results are for borel sets, but $A_n$ are not Borel sets}
			\item \rex{ex:measurable_spaces:standard_borel_spaces:0022} implies that there exists a Polish space $Z$ and $f:Z \rightarrow \coprod\limits_{n \in \N} A_n$ such that $f$ is continuous and surjective. Define $\phi: \coprod\limits_{n \in \N} A_n \rightarrow \bigcup\limits_{n \in \N} A_n$ by $\phi(n,  x) \defeq x$. \tcb{An excercise in the analysis notes section on coproduct topology} implies that $\phi$ is continuous. Clearly $\phi$ is surjective. Define $g: Z \rightarrow \bigcup\limits_{n \in \N} A_n$ by $g \defeq \phi \circ f$. Since $\phi$ and $f$ are continuous, $g$ is continuous. Since $\phi$ and $f$ are surjective, $g$ is surjective. Thus $\bigcup\limits_{n \in \N} A_n$ is analytic.
	...
	\begin{ex} \lex{ex:measurable_spaces:standard_borel_spaces:0025}
		Let $X$ be a Polish space. Then for each $A \in \MB(X)$, $A$ is analytic.
	\end{ex}
	...
	\begin{proof}\
		\begin{itemize}
			\item Since $X$ is a Polish space, there exists a metric $d$ on $X$ such that $(X,d)$ is a complete separable metric space. \tcb{An exercise in the analysis notes section on countability of metric spaces} implies that there exists $(C_{\gam})_{\gam \in \Gam} \subset \MP(X)$ such that 
			\begin{enumerate}
				\item for each $\gam \in \Gam$,
				\begin{enumerate}
					\item $C_{\gam}$ is closed and $C_{\gam} \neq \varnothing$,
			...
			\item Let $(\om_k)_{k \in \N} \subset \MN$ be a net and $\om \in \MN$. Suppose that $\om_k \rightarrow \om$. Then for each $n \in \N$, $\pi_{[n]}(\om_k) \rightarrow \pi_{[n]}(\om)$. Let $\ep > 0$. Choose $N \in \N$ such that $N^{-1} < \ep$. We note that 
			\begin{align*}
				f(\om) 
	...
	\begin{ex}
		Let $X$ be a Polish space. Suppose that $X \neq \varnothing$. Then
		\begin{enumerate}
			\item there exists a Polish space $Z$ and $f:Z \rightarrow X$ such that $Z$ is zero-dimensional and $f$ a continuous bijection.
			\item for each $B \in \MB(X)$, there exists a Polish space $A$ and $f:Z \rightarrow B$ such that $Z$ is zero-dimensional and $f$ a continuous bijection. \\
			\tbf{Hint:} consider \rex{ex:measurable_spaces:standard_borel_spaces:0020}
		\end{enumerate}
	\end{ex}

	\begin{proof}\
		\begin{enumerate}
			\item \rex{ex:measurable_spaces:standard_borel_spaces:0006} implies that there exists $Z \subset \MC$ and $\phi: Z \rightarrow [0,1]$ such that $\phi$ is a continuous bijection. Then $\phi^{\N}: Z^{\N} \rightarrow [0,1]^{\N}$ is a continuous bijection. \tcr{An exercise in the analysis notes section on Polish spaces} implies that there exists $B \subset [0,1]^{\N}$ and $h: X \rightarrow B$ such that $B$ is a $G_{\del}$-set and $h$ is a homeomorphism. Define $Z \subset Z^{\N}$ and $f:Z \rightarrow X$ by $Z \defeq (\phi^{\N})^{-1}(B)$ and $f \defeq h^{-1} \circ \phi^{\N}|_Z$. Since $B$ is a $G_{\del}$-set and $\phi^{\N}$ is continuous, we have that $Z$ is a $G_{\del}$-set. \tcr{An exercise in the analysis notes section on Polish spaces} implies that $Z$ is Polish space. Since $Z \subset \MC$ and $\MC$ is zero-dimensional, $Z$ is zero-dimensional. Thus $Z^{\N}$ is zero-dimensional. Since $Z \subset Z^{\N}$, we have that $Z$ is zero-dimensional. Since $\phi^{\N}:Z^{\N} \rightarrow [0,1]^{\N}$ is a continuous bijection, $\phi^{\N}|_Z:Z \rightarrow B$ is a continuous bijection. Thus $f$ is a continuous bijection. 
			\item Denote the topology on $X$ by $\MT_X$. \\
			Define $\MD \defeq \{B \in \MB(X, \MT_X): \text{there exists a Polish space $Z$ and $f:Z \rightarrow B$ such that $Z$ is zero-dimensional } \\ \text{and $f$ is a continuous bijection}\}$.
			\begin{itemize}
				\item Let $U \in \MT_X$. \tcb{An exercise in the analysis notes on polish spaces} implies that $U$ is a Polish space. The previous part implies that there exists a Polish space $Z$ and $f:Z \rightarrow U$ such that $Z$ is zero-dimensional and $f$ a continuous bijection. Since $U \in \MT_X$ is arbitrary, we have that $\MT_X \subset \MD$.
				\item Let $(B_n)_{n \in \N} \subset \MD$. Then $(B_n)_{n \in \N} \subset \MB(X)$ and for each $n \in \N$, there exists a Polish space $Z_n$ and $f_n:Z_n \rightarrow B_n$ such that $Z_n$ is zero-dimensional and $f_n$ is a continuous bijection. Define $Z \subset \prod\limits_{n \in \N} Z_n$ by $Z \defeq (\prod\limits_{n \in \N} f_n)^{-1}(\Del_{X^{\N}})$. Then \rex{ex:measurable_spaces:standard_borel_spaces:0021} implies that $Z$ is a Polish space and there exists $f: Z \rightarrow \bigcap\limits_{n \in \N} B_n$ such that $f$ is a continuous bijection. \tcr{An exercise in the previous section} \rex{} implies that $\prod\limits_{n \in \N} Z_n$ is zero-dimensional. Since $Z \subset \prod\limits_{n \in \N} Z_n$, \tcr{an exercise in the previous section} implies that $Z$ is zero-dimensional. Thus $\bigcap\limits_{n \in \N} B_n \in \MD$. Since $(B_n)_{n \in \N} \subset \MD$ is arbitrary, we have that for each $(B_n)_{n \in \N} \subset \MD$, $\bigcap\limits_{n \in \N} B_n \in \MD$. 
				\item Let $(B_n)_{n \in \N} \subset \MD$. Suppose that $(B_n)_{n \in \N}$ is disjoint. Set $Z \defeq \coprod\limits_{n \in \N} Z_n$. \tcb{An exercise in the analysis notes section on Polish spaces} implies that $Z$ is a Polish space. \rex{ex:measurable_spaces:standard_borel_spaces:0022} implies that there exists $f: Z \rightarrow \bigcup\limits_{n \in \N} B_n$ such that $f$ is a continuous bijection. \tcr{An exercise in a previous section} \rex{} implies that $Z$ is zero-dimensional. Thus $\bigcup\limits_{n \in \N} B_n \in \MD$. Since $(B_n)_{n \in \N} \subset \MD$ such that $(B_n)_{n \in \N}$ is disjoint is arbitrary, we have that for each $(B_n)_{n \in \N} \subset \MD$, if $(B_n)_{n \in \N}$ is disjoint, then $\bigcup\limits_{n \in \N} B_n \in \MD$. 
			\end{itemize}
			Thus $\MD$ is a $\del$-system on $X$. Since $X$ is a Polish space, $X$ metrizable. Since $\MT_X \subset \MD$,  \rex{ex:measurable_spaces:standard_borel_spaces:0020} implies that
			\begin{align*}
				\MD & \subset \MB(X, \MT_X) \\
	...
	\begin{defn}
		Let $X$ be a polish space and $A \subset X$. Then $A$ is said to be \tbf{z-analytic} if there exists a Polish space $Z$ and $f:Z \rightarrow A$ such that $Z$ is zero-dimensional and $f$ is a continuous bijection. 
	\end{defn}
	...
	\begin{ex} \lex{}
		Let $Z$ be a Polish space. Suppose that $Z$ is zero-dimensional. \tcr{An exercise in analysis notes about zero-dimensional Polish spaces} implies that $Z_0$ is Polish and zero dimensional and for each $z \in Z_0$, $z$ is a condensation point of $Z_0$.
		\begin{enumerate}
			\item There exists 
			\item there exists $h:\MN \rightarrow Z$ such that $h$ is continuous and injective and $Z \setminus h(\MN)$ is countable.
			\item 
		\end{enumerate}
	\end{ex}
	...
	\begin{proof}\
		\begin{enumerate}
			\item Let $\MT$ be a the topology on $Z_0$. Since $Z_0 \neq \varnothing$, there exists $z \in Z_0$. Define $U \in \MT \cap Z_0$ $U \defeq Z_0 \setminus \{z\}$. \tcr{make exercise} and exercise in the analysis notes section on separation implies that $U$ is not compact. \tcr{An exercise in the analysis notes section on Second-Countability and Zero-Dimensional Metric Spaces} implies that there 
		\end{enumerate}
		\tcr{FINSH!!!}
	\end{proof}
