...
ka	\end{defn}
	...
	\begin{ex} \lex{ex:measurable_functions:products:00015}
		\tcr{pointwise convergece implies measurability} 
		Let $(X, \MA)$ be a measurable space, $(Y, d)$ a complete metric space and $(f_n)_{n \in \N} \subset \Meas((X, \MA), (Y, \MB(Y)))$. Then $\{x \in X: \text{$(f_n(x))_{n \in \N}$ does not converge}\} \in \MA$.
	\end{ex}
	
	\begin{proof}
		Define $A \subset X$ by $\{x \in X: \text{$(f_n(x))_{n \in \N}$ does not converge}\}$. For each $m,n,k \in \N$, define $A_{m,n,k} \subset X$ by $A_{m,n,k} \defeq \{x \in X: d(f_n(x) , f_m((x))) \geq 1/k\}$. Let $m,n,k \in \N$. Since $f_m, f_n \in \Meas((X, \MA), (Y, \MB(Y)))$ \tcr{make/ref prev ex about tuples} implies that $(f_m, f_n) \in \Meas((X, \MA), (Y^2, \MB(Y)^{\otimes 2}))$. Since $d:Y \times Y \rightarrow \R_+$ is continuous \tcr{ref prev exs}, implies that $d \circ (f_m, f_n) \in \Meas((X, \MA), (\R, \MB(R)))$. Therefore 
		\begin{align*}
			A_{m,n,k}
			& = (d \circ (f_m, f_n))^{-1}([1/k, \infty)) \\
			& \in \MA. 
		\end{align*}
		Since $m,n,k \in \N$ are arbitrary, we have that for each $m,n,k \in \N$, $A_{m,n,k} \in \MA$. Since $(Y, d)$ is complte, for each $x \in X$, $(f_n(x))_{n \in \N}$ converges iff $(f_n(x))_{n \in \N}$ is Cauchy. Therefore
		\begin{align*}
			A 
			& = \bigcup\limits_{k \in \N} \bigcap\limits_{N \in \N} \bigcup\limits_{n,m \in \N \setminus [N-1]} \{x \in X: d(f_n(x) , f_m((x))) \geq 1/k\} \\
			& \in \MA.
		\end{align*}
	\end{proof}
...
%	\end{proof}
	...
	\begin{defn} \tcr{(maybe relocate or delete this)}
		...
		\begin{itemize}
			...
			\item \tbf{$\sig$-additive} if for each $(A_j)_{j \in \N} \subset \ME$, $(A_j)_{j \in \N}$ is disjoint and $\bigcup\limits_{j \in \N} A_j \in \ME$ implies that $\rho \bigg( \bigcup\limits_{j \in \N} A_j \bigg) = \sum_{j \in \N} \rho(A_j)$,
...
\begin{ex} \lex{ex:measures:00013.1}
	Let $(X, \MA)$ be a measurable space, $\mu \in M_(X)$ and $x \in X$. Suppose that $\{x\} \in \MA$. Then $\mu = \del_x$ iff $\mu(\{x\}^c) = 0$.
\end{ex}
...
\end{proof}
	...
	\begin{ex} \lex{ex:subspace_measures:00002.01} 
		Let $(X, \MA)$ be a measurable space, $\mu \in M_+(X, \MA)$ and $E \in \MA$. Then $\mu = \mu_E + \mu_{E^c}$.
	\end{ex}
...
\end{proof}
	...
	\begin{thm} \lex{ex:integration_nonnegative_functions:0001} \textbf{Monotone Convergence Theorem:} \\
		...
		$$\sup_{n \in \N} \int_X f_n \dmu = \int \bigg[ \sup_{n \in \N} f_n \bigg] \dmu. $$
	\end{thm}
	...
	\begin{proof}\
		\begin{itemize}
			\item Suppose that $f \in S^+(X, \MA)$. Write $f = \sum_{j \in [n]} a_j \chi_{E_j} $ \tcr{in standard representation}. Then 
			\begin{align*}
				\int_E f \dmu 
				& = \int_X f \chi_E \dmu \quad \text{(by \rd{def:integration_nonnegative_functions:0.004})} \\ 
				& = \int_X \bigg[ \sum_{j \in [n]}  a_j \chi_{E_j \cap E} \bigg] \dmu \quad \text{(by \tcr{make/ref prev ex})} \\
				& = \sum_{j \in [n]} a_j \mu(E_j \cap E) \\
				& = \sum_{j \in [n]} a_j \mu_E(E_j) \\
				& = \int_X \bigg[ \sum_{j \in [n]} a_j \chi_{E_j} \bigg] \dmu_E \\
				& = \int_X f \dmu_E.
			\end{align*}
		...
		\begin{enumerate}
			...
			\begin{itemize}
				...
				\begin{align*}
					...
					&= \sum_{i=1}^n a_i \int_{E_i} f \dmu\\
					&= \int \bigg(\sum_{i=1}^n a_i\chi_{E_i} \bigg) f \dmu\\
					&= \int gf \dmu.
				\end{align*}
	...
	\begin{itemize}
		\item \tcr{Show that if $f$ is $\pi^*\MB$-measurable, then
	...
	\begin{ex} \lex{ex:integration_complex_functions:00022} \tcb{Change notation or define categories}
		Let $(X, \MA, \mu), (Y, \MB, \nu) \in \Obj(\bProb)$ and $f \in \Meas((X, \MA), (Y, \MB))$. Then $f$ is measure preserving iff for each $\phi \in L^1(Y, \MB, \nu)$, $\phi \circ f \in L^1(X, \MA, \mu)$ and 
...
\end{align*}		
	...
	\begin{proof}
		Since $\MB$ is countable \rex{ex:measurable_spaces:sigma_algebras:0008} implies that $\MB(X) = \sig(\MB)$. Since $\MB$ is a $\pi$-system, $X \in \MB$, and for each $A \in \MB$, $\mu(A) = \nu(A)$, \rex{ex:complex_measures:0006} implies that $\mu = \nu$.
	\end{proof}
	...
	\begin{note}
		\tcr{make definition for $\#_a \in M_+(\N, \MP(\N))$ by $\#_a(A) \defeq \sum_{j \in A} a_j$ in section on counting measure, $\#$}
	\end{note}
	...
	\begin{note}
		We recall the definiton of $\MK(X) \defeq \{K \subset X: \text{$K$ is compact}\}$. \tcr{define $\MK(X)$ in topology notes}
	\end{note}
	...
	\begin{itemize}
		...
		\begin{itemize}
			\item $V^{\mu}_O(E) \defeq  \{ \mu(U): \text{$U \subset X$, $E \subset U$ and $U$ is open}\}$
			\item $V^{\mu}_I(E) \defeq \{ \mu(K):\text{$K \subset X$, $K \subset E$ and $K$ is compact}\}$.
		\end{itemize}
	...
	\begin{ex} \lex{ex:introduction_radon_measures:0001.01}
		Let $X$ be a topological space. Suppose that $\MK(X) \subset \MB(X)$. Let $\mu \in M_+(X)$. Then for each $E \in \MB(X)$,
		\begin{enumerate}
			\item $\mu(E) \leq \inf V^{\mu}_O(E)$.
			\item $\sup V^{\mu}_I(E) \leq \mu(E)$.
		\end{enumerate}
	\end{ex}
	
	\begin{proof}\
		\begin{enumerate}
			\item Let $a \in V^{\mu}_O(E)$. Then there exists $U \in \{U \subset X: \text{$U$ is open and $E \subset U$}\}$ such that $a = \mu(U)$. Since $E \subset U$, we have that  
			\begin{align*}
				\mu(E)
				& \leq \mu(U) \\
				& = a.
			\end{align*}
			Since $a \in V^{\mu}_O(E)$ is arbitrary, we have that for each $a \in V^{\mu}_O(E)$, $\mu(E) \leq a$. Thus $\mu(E) \in \lbd V^{\mu}_O(E)$. Hence $\mu(E) \leq \inf V^{\mu}_O(E)$. 
			\item Let $b \in V^{\mu}_I(E)$. Then there exists $K \in \{K \in \MK(X): K \subset E \}$ such that and $b = \mu(K)$. Since $K \subset E$, we have that 
			\begin{align*}
				b
				& = \mu(K) \\
				& \leq \mu(E).
			\end{align*}
			Since $b \in V^{\mu}_I(E)$ is arbitrary, we have that for each $b \in V^{\mu}_I(E)$, $b \leq \mu(E)$. Thus $\mu(E) \in \ubd V^{\mu}_I(E)$. Hence $\sup V^{\mu}_I(E) \leq \mu(E)$. 
		\end{enumerate}
	\end{proof}
	...
	\begin{proof}
		Suppose that $\mu$ is inner regular. Let $E \in \MB(X)$. \rex{ex:introduction_radon_measures:0001.01} implies that $\mu(E) \leq \inf V_O(E)$. Let $\ep > 0$. Since $\mu$ is finite, $\mu(E^c) -\ep < \mu(E^c)$. Inner regularity implies that there exists $K \subset E^c$ such that $\mu(K) > \mu(E^c) - \ep$. Define $U \in \MB(X)$ by $U \defeq K^c$. Then $U$ is open and $E \subset U$. By construction,
		\begin{align*}
			\mu(U) - \mu(E) 
		...
		\begin{align*}
			...
			& < \mu(E) + \ep 
		\end{align*}
		Since $\ep > 0$ is arbitrary, we have that for each $\ep >0$, $\inf V_O(E) < \mu(E) + \ep$. Hence $\inf V_O(E) \leq \mu(E)$. Since $\mu(E) \leq \inf V_O(E)$ and $\inf V_O(E) \leq \mu(E)$, we have that $\mu(E) = \inf V_O(E)$. Thus $\mu$ is outer regular on $E$. Since $E \in \MB(X)$ is arbitrary, we have that for each $E \in \MB(X)$, $\mu$ is outer regular on $E$. Thus $\mu$ is outer regular. Hence $\mu$ is regular.
	\end{proof}

	\begin{defn} \ld{def:introduction_radon_measures:0002} 
	Let $X$ be a topological space. 
	\begin{itemize}
		\item Let $\mu \in M_+(X)$. Then $\mu$ is said to be \textbf{Radon} if 
		\begin{enumerate}
			\item for each $K \in \MK(X)$, $\mu(K) < \infty$,
			\item $\mu$ is outer regular,
			\item for each $U \subset X$, $U$ is open implies that $\mu$ is inner regular on $U$
		\end{enumerate}
		\item We define 
		\begin{itemize}
	...
	\begin{proof}\
		\begin{itemize}
			...
			\begin{enumerate}
				...
				\item Let $E \in \MB(X)$. 
				\begin{itemize}
					\item Suppose that $x \in E$. Then for each $U \subset X$, $U$ is open and $E \subset U$ implies that $\del_x(U) = 1$. Hence 
					\begin{align*}
						\del_x(E)
						& = 1 \\
						& = \inf V^{\del_x}_O.
					\end{align*}
					\item Suppose that $x \not \in E$. Since $X$ is $T_1$, $\{x\}^c$ is open and $E \subset \{x\}^c$. Since $\del_x(\{x\}^c) \in V^{\del_x}_O(E)$, we have that
					\begin{align*}
						\del_x(E)
						& \leq \inf V^{\del_x}_O(E) \\
						& \leq \del_x(\{x\}^c) \\
						& = 0 \\
						& = \del_x(E).
					\end{align*}
					Hence $\del_X(E) = \inf V^{\del_x}_O(E)$.
				\end{itemize}
				...
				\item Let $U \subset X$. Suppose that $U$ is open.
				\begin{itemize}
					\item Suppose that $x \in U$. Define $K \subset X$ by $K \defeq \{x\}$. Then $K \subset U$ and \tcr{ref ex in topology compactness section} implies that $K \in \MK(X)$. Therefore 
					\begin{align*}
						\del_x(U)
						& = 1 \\
						& = \del_x(K) \\
						& \leq \sup V^{\del_x}_I(U) \\
						& \leq \del_x(U).
					\end{align*}
					Hence $\del_x(U) = \sup V^{\del_x}_I(U)$.
					\item Suppose that $x \not \in U$. Then for each $K \in \MK(X)$, $K \subset U$ implies that $\del_x(K) = 0$. Hence
					\begin{align*}
						\del_x(U) 
						& = 0 \\
						& = \sup V^{\del_x}_I.
					\end{align*}
				\end{itemize}
				Therefore $\del_x$ is inner regular on $U$. Since $U \subset X$ with $U$ open is arbitrary, we have that for each $U \subset X$, $U$ is open implies that $\del_x$ is inner regular on $U$. 
			\end{enumerate}
			...
			\item $(\impliedby)$: \\
			Suppose that $X$ is not $T_1$. \tcr{ex in topology notes} implies that there exists $x \in X$ such that $\{x\}$ is not closed. Define $E \subset X$ by $E \defeq \{x\}^c$. Then $E$ is not open and $\{U \subset X: \text{$U$ is open and $E \subset U$}\} = \{X\}$. Then 
			\begin{align*}
				\inf V^{\del_x}_O(E)
				& \del_x(X) \\
				& = 1 \\
				...
				& = \del_x(E).
			\end{align*}
			Hence $\del_x$ is not outer regular and $\del_x \not \in \MM_1(X)$. Thus $\{\del_x:x \in X\} \not \subset \MM_1(X)$.
		\end{itemize}
	\end{proof}
		...
		\begin{enumerate}
			...
			\begin{itemize}
				...
				\begin{align*}
					...
					& = [\mu + \lam \nu](E) + \ep.
				\end{align*}
				Since $\ep > 0$ is arbitrary, we have that for each $\ep > 0$, $[\mu + \lam \nu](U) < [\mu + \lam \nu](E) + \ep$. Thus 
				\begin{align*}
					\inf V_O^{\mu + \lam \nu}(E)
					& \leq [\mu + \lam \nu](U) \\
					& \leq [\mu + \lam \nu](E).
				\end{align*}
			\end{itemize}
			Since $[\mu + \lam \nu](E) \leq \inf V^{\mu + \lam \nu}_O(E)$and $\inf V_O^{\mu + \lam \nu}(E) \leq [\mu + \lam \nu](E)$, we have that $[\mu + \lam \nu](E) = \inf V^{\mu + \lam \nu}_O(E)$. Since $E \in \MB(X)$ is arbitrary, we have that for each $E \in \MB(X)$, $[\mu + \lam \nu](E) = \inf V^{\mu + \lam \nu}_O(E)$. Thus $\mu + \lam \nu$ is outer regular.
			\item Let $U \subset X$. Suppose that $U$ is open.  \rex{ex:introduction_radon_measures:0001.01} implies that $\sup V_I^{\mu + \lam \nu}(U) \leq [\mu + \lam \nu](U)$. If $\lam = 0$, then $\mu + \lam \nu = \mu$ and since $\mu \in \MM_+(X)$, we have that
			\begin{align*}
				\sup V_I^{\mu + \lam \nu}(U)
				& = \sup V_I^{\mu}(U) \\
				& = \mu(U) \\
				& = [\mu + \lam \nu](U).
			\end{align*}
			In particular $[\mu + \lam \nu](U) \leq \sup V_I^{\mu + \lam \nu}(U)$. Suppose that $\lam \neq 0$. 
			\begin{itemize}
				...
				\begin{itemize}
					...
					\begin{align*}
						...
						& > M
					\end{align*}
					Since $M > 0$ is arbitrary, we have that for each $M > 0$, $\sup V_I^{\mu + \lam \nu}(U) > M$. Thus
					\begin{align*}
						\sup V_I^{\mu + \lam \nu}(U)
						& = \infty \\
						& \geq [\mu + \lam \nu](U).
				...
				\begin{align*}
					...
					& = [\mu + \lam \nu](U) + \ep  
				\end{align*}
				Since $\ep > 0$ is arbitrary, we have that for each $\ep > 0$, $\sup V_I^{\mu + \lam \nu}(U) > [\mu + \lam \nu](U) + \ep$. Hence $\sup V_I^{\mu + \lam \nu}(U) \geq [\mu + \lam \nu](U)$. 
			\end{itemize}
			Since $[\mu + \lam \nu](U) \leq \sup V_I^{\mu + \lam \nu}(U)$ and $\sup V_I^{\mu + \lam \nu}(U) \leq [\mu + \lam \nu](U)$, we have that $\sup V_I^{\mu + \lam \nu}(U) = [\mu + \lam \nu](U)$. Thus $\mu + \lam \nu$ is inner regular on $U$. Since $U \subset X$ with $U$ open is arbitrary, we have that for each $U \subset X$, $U$ is open implies that $\mu + \lam \nu$ is inner regular on $U$.
		\end{enumerate}
	...
	\begin{ex} \lex{ex:introduction_radon_measures:0004.1}
		Let $X$ be a topological space. Suppose that $\MK(X) \subset \MB(X)$ and $\mu \in \MM_1(X)$. Then $\mu = \del_x$ iff $\supp \mu = \{x\}$. 
	\end{ex}
	
	\begin{proof}
		Since $\{x\}^c$ is open, $N_{\mu} = \bigcup\limits_{U \in \MN_{\mu}}$, $X \not \in \MN_{\mu}$ and $\supp \mu = (N_{\mu})^c$, we have that 
		\begin{align*}
			\mu = \del_x
	...
	\begin{ex}
		If $f: X \rightarrow Y$ is proper and $\mu \in MM_+(X)$, then $f_*\mu \in \MM_+(Y)$.  
	\end{ex}
	...
	\begin{ex}\textbf{Representation Theorem II:}\\
	...
	\begin{ex}
		\tcr{Here make an exercise about $\sup_{j \in J} \mu_j(X_j) < \infty$ iff there exists a complex measure $\mu \in \MM(X)$ such that it gives a $\bTopRadMsrC$ projective limit. Then relate/comment/explore how this is related to the feynman path integral nonexistence, which is why we do a wick rotation.}
	\end{ex}
	...
	\begin{proof}\
		...
		\begin{enumerate}
			...
			\begin{itemize}
				...
				\begin{align*}
					\sup_{x \in F} g(x)
					& = \sup_{x \in F} \sum\limits_{j \in [n]} a_j \phi_j(x) \\
					& = \sup_{x \in F} \sum\limits_{j \in [n]} \l a, T(x) \r \\
					& < \l a, y \r \\
	...
	\begin{ex}
		Let $X$ be a locally convex space and $E \in \MK(X)$. Define $F \in \MK(X) \cap \Conv(X)$ by $F \defeq \cl (\cnv E)$. Then there exists $f_E: \MM_1(E) \rightarrow F$ such that 
		\begin{enumerate}
			\item for each $\mu \in \MM_1(E)$, $\mu$ represents $f_E(\mu)$. 
			\item $f_E$ is $(\MT_{w^*}, \MT_X)$-continuous.
		\end{enumerate}
	\end{ex}
	
	\begin{proof}\
		\begin{enumerate}
			\item For each $\mu \in \MM_1(E)$, define $K_{\mu} \subset X$ by $K_{\mu} \defeq \{x \in F: \text{$\mu$ represents $x$}\}$. \tcr{The prev ex} implies that for each $\mu \in \MM_1(E)$, $\# K_{\mu} = 1$. \tcr{Axiom of choice \tcr{(ex in product set section)} implies that} there exists \tcr{(a unique?)} $f_E: \MM_1(E) \rightarrow X$ such that for each $\mu \in \MM_1(X)$, $f_E(\mu) \in K_{\mu}$. Let $(\mu_{\al})_{\al \in A} \subset \MM_1(E)$ be a net and $\mu \in \MM_1(E)$. Suppose that $\mu_{\al} \conv{w^*} \mu$. Define $(x_{\al})_{\al \in A} \subset F$ and $x \in F$ by $x_{\al} \defeq f_E(\mu_{\al})$ and $x \defeq f_E(\mu)$. Then for each $\phi \in X^*$, 
			\begin{align*}
				\phi(x_{\al})
				& = \mu_{\al}(\phi|_E) \\
				...
				& = \phi(x).
			\end{align*}
			Thus $x_{\al} \conv{w} x$. Since $X$ is a locally convex space, $E \in \MK(X)$, $(x_{\al})_{\al \in A} \subset E$ and $x \in E$, \tcr{ref ex in analysis notes section on duality implies} $x_{\al} \rightarrow x$. Since $(\mu_{\al})_{\al \in A} \subset \MM_1(E)$ and $\mu \in \MM_1(E)$ are arbitrary, we have that for each $(\mu_{\al})_{\al \in A} \subset \MM_1(E)$ and $\mu \in \MM_1(E)$, $\mu_{\al} \conv{w^*} \mu$ implies that $f_E(\mu_{\al}) \rightarrow f_E(\mu)$. Thus $f$ is $(\MT_{w^*}, \MT_X)$-continuous.
		\end{enumerate}
	\end{proof}
	...
	\begin{ex}
		Let $X$ be a locally convex space, $E \in \MK(X) \cap \Conv(X)$ and $x \in X$. If $ \rep_E(x) \neq \varnothing$, then $x \in E$. 
	\end{ex}
	...
	\begin{ex}
		Let $X$ be a locally convex space, $E \in \MK(X) \cap \Conv(X)$ and $x \in E$. Then $x \in \extrm E$ iff $\rep_E(x) = \{\del_x\}$. \tcr{make into multiple subparts}  
	\end{ex}
	...
	\begin{proof}\
		\begin{itemize}
			\item $(\implies)$: \\
			\begin{itemize}
				\item Suppose that $x \in \extrm E$. Let $\mu \in \rep_E(x)$. Let $K_1 \in \MK(E, \MT_E)$. Suppose that $K_1 \subset E \setminus \{x\}$. For the sake of contradiction, suppose that $\mu(K_1) > 0$. \rex{ex:measures:00014} implies that there exists $y \in K_1$ such that for each $U_E \in \opn{\MN}(y, \MT_E)$, $\mu(U) > 0$. Since $X$ is Hausdorff, there exists $U_0 \in \MT_X$ such that $y \in U_0$ and $U_0 \subset \{x\}^c$.
				
				\item Since $X$ is locall convex, \tcr{make ref ex in analysis notes section on lcs } implies that there exists $C \in \Conv(X) \cap \MN(y, \MT_X)$ such that $C \subset U_0$ and $C$ is $\MT_X$-closed. Since $E, C \in \Conv(X)$, we have that $E \cap C \in \Conv(X)$. Since $C$ is $\MT_X$-closed and $E$ is $\MT_X$-closed, we have that $E \cap C$ is $\MT_X$-closed. Since $E \in \MK(X)$, $E \cap C \in \MK(X)$. Define $K_2 \in \MK(X) \cap \Conv(X)$ by $K_2 \defeq E \cap C$. 
				...
				\begin{align*}
					...
					& = E \setminus \{x\}.
				\end{align*}
				Since $C \in \MN(y, \MT_X)$, we have that $\Int (C, \MT_X) \in \opn{\MN}(y, \MT_X)$. Therefore $\Int (C, \MT_X) \cap E \in \opn{\MN}(y, \MT_E)$ and 
				\begin{align*}
					0
					& < \mu(\Int (C, \MT_X) \cap E) \\
					& \leq \mu (C \cap E) \\
					& = \mu(K_2). 
				\end{align*}
				For the sake of contradiction, suppose that $\mu(K_2) = 1$. Then $\mu|_{K_2} \in \rep_{K_2}(x)$ \tcr{(add details)}. Hence $\rep_{K_2}(x) \neq \varnothing$. Since $K_2 \in \MK(X) \cap \Conv(X)$, and $\rep_{K_2}(x) \neq \varnothing$, \tcr{a prev ex} implies that.
				\begin{align*}
					x
					& \in K_2 \\
					& \subset E \setminus \{x\},
				\end{align*}
				which is a contradiction. Hence $\mu(K_2) < 1$. Since 
				\begin{align*}
					\mu(E \setminus K_2) 
					& = \mu(E) - \mu(K_2) \\
					& = 1 - \mu(K_2)
				\end{align*}
				and $0 < \mu(K_2) < 1$, we have that $0 < 1 - \mu(K_2) < 1$. 
				
				\item Define $r \in (0, 1)$ by $r \defeq \mu(K_2)$. Define $\mu_1, \mu_2 \in M_1(E)$ by $\mu_1 \defeq r^{-1} \mu_{K_2}$ and $\mu_2 \defeq (1-r)^{-1}\mu_{E \setminus K_2}$. \rex{ex:subspace_radon_measures:0001} and \rex{ex:introduction_radon_measures:0002.1} imply that $\mu_1, \mu_2 \in \MM_1(E)$. \tcr{prev ex} implies that there exists $x_1, x_2 \in E$ such that $\mu_1 \in \rep_E(x_1)$ and $\mu_2 \in \rep_E(x_2)$. \rex{ex:subspace_measures:00002.01} implies that
				\begin{align*}
					r \mu_1 + (1-r) \mu_2
					& = \mu_{K_2} + \mu_{E \setminus K_2} \\
					& = \mu.
				\end{align*}
				Therefore for each $\phi \in X^*$,
				\begin{align*}
					\phi(rx_1 + (1-r)x_2)
					& = r\phi(x_1) + (1-r)\phi(x_2) \\
					& = r \int_E \phi|_E \dmu_1 +  (1-r) \int_E \phi|_E \dmu_2 \\
					& = \int_E \phi|_E \d [r\mu_1] + \int_E \phi|_E \d[(1-r)\mu_2] \\
					& = \int_E \phi|_E \d [r\mu_1 + (1-r) \mu_2] \\
					& = \int_E \phi|_E \dmu \\
					& = \phi(x).
				\end{align*}
				Since $X$ is locally convex, $X^*$ separates the pointes of $X$. Hence 
				\begin{align*}
					x 
					& = rx_1 + (1-r)x_2 \\
					& \in E \setminus (\extrm E),
				\end{align*} 
				which is a contradiction. Thus $\mu(K_1) = 0$. Since $K_1 \subset E \setminus \{x\}$ is arbitrary, we have that for each $K \subset E \setminus \{x\}$, $\mu(K) = 0$. Since $\mu \in \MM_1(E)$ \rex{ex:introduction_radon_measures:0009.1} implies that $\mu$ is regular. Thus 
				\begin{align*}
					\mu(E \setminus \{x\})
					& = \sup V^{\mu}_I(E \setminus \{x\}) \\
					& = 0. 
				\end{align*}
				Hence $\supp \mu = \{x\}$. \rex{ex:introduction_radon_measures:0004.1} then implies that $\mu = \del_x$. 
			\end{itemize}
			\item $(\impliedby)$: \\
			Suppose that $\rep_E(x) = \{\del_x\}$. For the sake of contradiction, suppose that $x \not \in \extrm E$. Then there exists \tcr{ref ex in analysis notes} $x_1, x_2 \in E$ such that $x_1 \neq x_2$ and $x = 2^{-1}x_1 + 2^{-1}x_2$. Since $\del_{x_1} \in \rep_E(x_1)$ and $\del_{x_2} \in \rep_E(x_2)$, \tcr{make/ref ex} implies that 
			\begin{align*}
				2^{-1}\del_{x_1} + 2^{-1}\del_{x_2} 
				& \in \rep_E(2^{-1}x_1 + 2^{-1}x_2) \\
				& = \rep_E(x) \\
				& = \{\del_x\}.
			\end{align*}
			Hence $2^{-1}\del_{x_1} + 2^{-1}\del_{x_2} = \del_x$. Thus 
			\begin{align*}
				\del_x(\{x_1\})
				& = 2^{-1}\del_{x_1}(\{x_1\}) + 2^{-1}\del_{x_2}(\{x_1\}) \\
				& = 2^{-1} + 0 \quad \text{(since $x_1 \neq x_2$)},
			\end{align*} 
			which is a contradiction since $\del_{x}(\{x_1\}) \in \{0,1\}$. Hence $x \in \extrm E$. 
		\end{itemize}
	\end{proof}
	...
	\begin{defn} \ld{def:diff_radon_measures:0004.1} \tcr{(maybe change reference id?)} \\
	...
	\begin{defn} \ld{def:diff_radon_measures:0010}
		Let $(X, d)$ be a metric space, $\mu, \nu \in \MM_+^{\loc}(X, d)$ and $h \in \ML^0(X, \MB(X))$. Then $h$ is said to be a \tbf{derivate of $\nu$ by $\mu$} if $h = \ub{D}_{\mu}\nu$ $\mu$-a.e. and $h = \ol{D}_{\mu}\nu$ $\mu$-a.e.
	\end{defn}
	...
	\begin{proof}\
		\begin{enumerate}
			...
			\item By \rd{def:diff_radon_measures:0005}, 
		\end{enumerate}
	\end{proof}
...
\begin{ex}
	Let $X$ be a compact metric space, $Y$ a locally compact separable ultrametric space and $\mu \in \MM_1(X)$. 
\end{ex}
...
\begin{proof}\
	\begin{itemize}
		\item \tcr{make /ref ex in analysis notes, every l(ocally) compact metric space is complete}. Since $X$ is compact\tcr{???}, \tcr{ref ex in anlysis notes} implies that $C(X)$ is separable. Thus there exists $\Phi \subset C(X)$ such that $\Phi$ is dense in $C(X)$ and $\Phi$ is countable. 
		\item For each $\phi \in C(X)$, define $\nu_{\phi} \in \MM(X)$ by 
		$$\nu_{\phi}(E) \defeq \int_E \phi \dmu.$$
		Then for each $\phi \in C(X)$, $\nu_{\phi} \ll \mu$. \tcr{ref ex} then implies that for each $\phi \in C(X)$, $\pi_*\nu_k \ll \pi_*\mu$. 
		\item For each $\phi \in C(X)$ and $n \in \N$, define $A_{\phi, n}:Y \rightarrow \C$ by
		$$A_{\phi, n}(y) \defeq \frac{\pi_*\nu_{\phi}(\bar{B}(y, n^{-1}))}{\pi_*\mu(\bar{B}(y, n^{-1}))}$$
		\rex{ex:diff_radon_measures:009} \tcr{(make/ref version for complex measures)} and \tcr{ref ex on products of measurable functions being measurabl} imply that for each $\phi \in C(X)$, $(A_{\phi, n})_{n \in \N} \subset \Meas(Y, \C)$. For each $\phi \in C(X)$, define $N_{\phi} \subset Y$ by 
		$$N_{\phi} \defeq \bigg\{y \in Y: \text{$(A_{\phi, n}(y))_{n \in \N}$ does not converge to $\frac{d\pi_*\nu_{\phi}}{d\pi_*\mu}(y)$} \bigg\}.$$ 
		\item \rex{ex:differentiation_complex_measures:0002} implies that $N_{\phi}$ is $\mu$-null and for each $y \in N_{\phi}^c$,
		\begin{align*}
			\frac{d\pi_*\nu_{\phi}}{d\pi_*\mu}(y)
			& = \frac{d(\pi_*\nu_{\phi})_{\pi_*\mu}}{d\pi_*\mu}(y) \\
			& = \lim\limits_{n \rightarrow \infty} \frac{ \pi_*\nu_{\phi}(\bar{B}(y, n^{-1}))}{\pi_*\mu(\bar{B}(y, n^{-1}))} \\
			& = \lim\limits_{n \rightarrow \infty} A_{\phi,n}(y).
		\end{align*}
	\end{itemize}
...
%	
	...
	Let $\phi \in C(X)$. Since $\Phi$ is dense in $C(X)$, there exists $(\phi_k)_{k \in \N} \subset \Phi$ such that $\phi_k \convt{unif} \phi$. 
	 
	For $k \in \N$, define $\nu_k \in \MM(X)$ by $\nu_k \defeq \nu_{\phi_k}$. Then for each $k \in \N$, $\nu_k \ll \mu$. \tcr{ref ex} implies that $\pi_*\nu_k \ll \pi_* \mu$. For each $k,n \in \N$, define $A_{k, n}:Y \rightarrow \C$ by 
	$$A_{k,n}(y) \defeq \frac{\pi_*\nu_k(\bar{B}(y, n^{-1}))}{\pi_*\mu(\bar{B}(y, n^{-1}))}.$$
	\tcr{Show $A_{n,k}$ is measurable}
	Define $(N_k)_{k \in \N} \subset \MB(X)$ by
	and 
	$$N_k \defeq \bigg\{y \in Y: \text{$(A_{\phi, n}(y))_{n \in \N}$ does not converge to $\frac{d\pi_*\nu_k}{d\pi_*\mu}(y)$} \bigg\}.$$ 
	\rex{ex:differentiation_complex_measures:0002} implies that for each $k \in \N$ $N_k$ is $\mu$-null and for each $y \in N_k^c$,
	\begin{align*}
		\frac{d\pi_*\nu_k}{d\pi_*\mu}(y)
		& = \frac{d(\pi_*\nu_k)_{\pi_*\mu}}{d\pi_*\mu}(y) \\
		& = \lim\limits_{n \rightarrow \infty} \frac{ \pi_*\nu_k(\bar{B}(y, n^{-1}))}{\pi_*\mu(\bar{B}(y, n^{-1}))} \\
		& = \lim\limits_{n \rightarrow \infty} A_{k,n}(y).
	\end{align*}
	Let $\ep > 0$. Then there exists $K \in \N$ such that for each $k \in \N$, $k \geq K$ implies that $\sup\limits_{x \in X} \|\phi_k - \phi\| < \ep/2$. Let $k \in \N$. Suppose that $k \geq K$. Then for each $n \in \N$,
	\begin{align*}
		|A_{k,n}y - A_{\phi, n}(y)|
		& = \bigg|  \frac{\pi_*\nu_k(\bar{B}(y, n^{-1})) - \pi_*\nu_{\phi}(\bar{B}(y, n^{-1}))}{\pi_*\mu(\bar{B}(y, n^{-1}))} \bigg| \\
		& =  \frac{1}{\pi_*\mu(\bar{B}(y, n^{-1}))} \bigg| \int_{\pi_*\mu(\bar{B}(y, n^{-1}))} \phi_k \dmu - \int_{\pi_*\mu(\bar{B}(y, n^{-1}))} \phi \dmu \bigg| \\
		& =  \frac{1}{\pi_*\mu(\bar{B}(y, n^{-1}))} \bigg| \int_{\pi_*\mu(\bar{B}(y, n^{-1}))} \phi_k - \phi \dmu \bigg| \\
		& \leq \frac{1}{\pi_*\mu(\bar{B}(y, n^{-1}))} \int_{\pi_*\mu(\bar{B}(y, n^{-1}))} |\phi_k - \phi| \dmu \\
		& \leq   \frac{1}{\pi_*\mu(\bar{B}(y, n^{-1}))} \int_{\pi_*\mu(\bar{B}(y, n^{-1}))} \sup\limits_{x \in X} \|\phi_k - \phi\| \dmu \\
		& \leq  \frac{\ep \pi_*\mu(\bar{B}(y, n^{-1}))}{2 \pi_*\mu(\bar{B}(y, n^{-1}))} \\
		& = \frac{\ep}{2}.
	\end{align*}
	Hence 
	\begin{align*}
		\sup\limits_{n \in \N} |A_{k,n}(y) - A_{\phi, n}(y)|
		& \leq \frac{\ep}{2} \\
		& < \ep. 
	\end{align*}
	Since $k \in \N$ with $k \geq K$ is arbitrary, we have that for each $k \in \N$, $k \geq K$ implies that $\sup\limits_{n \in \N} |A_{k,n} - A_{\phi, n}| < \ep$. Since $\ep > 0$ is arbitrary, we have that for each $\ep > 0$, there exists $K \in \N$ such that for each $k \in \N$, $k \geq K$ implies that $\sup\limits_{n \in \N} |A_{k,n} - A_{\phi, n}| < \ep$. Thus $(A_{k,n})_{n \in \N} \convt{unif} (A_{\phi, n})_{n \in \N}$. The Moore-Osgood theorem \tcr{(ref ex in analysis notes here)} then implies that 
	\begin{align*}
		\lim\limits_{k \rightarrow \infty} \frac{d\pi_*\nu_k}{d\pi_*\mu}(y)
		& = \lim\limits_{k \rightarrow \infty} [\lim\limits_{n \rightarrow \infty} A_{k,n}(y)] \\
		& = \lim\limits_{n \rightarrow \infty} [\lim\limits_{k \rightarrow \infty} A_{k,n}(y)] \\
		& = \lim\limits_{n \rightarrow \infty} A_{\phi,n}(y) \\
		& = \frac{d\pi_*\nu_{\phi}}{d\pi_*\mu}(y) 
	\end{align*}
	Thus $y \in N_{\phi^c}$. Since $y \in \bigcap\limits_{k \in \N} N_k^c$ is arbitrary, we have that for each $y \in \bigcap\limits_{k \in \N} N_k^c$, $y \in N_{\phi}^c$. Thus $\bigcap\limits_{k \in \N} N_k^c \subset N_{\phi}^c$. Since 
	\begin{align*}
		N_{\phi}
		& \subset \bigcup\limits_{k \in \N} N_k \\
		& \subset \bigcup\limits_{\phi' \in \Phi} N_{\phi'}, 
	\end{align*}
	we have that 
	\begin{align*}
		\pi_*\mu(N_{\phi})
		& \leq \pi_*\mu \bigg( \bigcup\limits_{\phi' \in \Phi} N_{\phi'} \bigg)  \\
		& \leq \sum\limits_{\phi' \in \Phi} \pi_*\mu(N_{\phi'}) \\
		& = \sum\limits_{\phi' \in \Phi} 0 \\
		& = 0.
	\end{align*}
	Since $\phi \in C(X)$ is arbitrary, we have that for each $\phi \in C(X)$, $\pi_*\mu(N_{\phi}) = 0$. 
\end{proof}
...
~\cite{simmons2012conditional}
	...
	\begin{ex}
		...
		\begin{enumerate}
			\item for each $f \in \Meas[(X, \MA), (G, \MB(G))]$, $(f^{-1})_*\lam \ll \mu$ and 
			$$\frac{d f^{\odot -1}_*\lam}{d \mu} = \frac{d f_*\lam }{d\mu} \circ \iota_{\mu} \text{ \hspace{.2cm} $\mu$-a.e.}$$ 
			\item for each $f,g \in \Meas[(X, \MA), (G, \MB(G))]$, $(f^{-1})_*\lam \ll \mu$ and 
	...
	\begin{proof}\
		\begin{enumerate}
			\item Let $f \in \Meas[(X, \MA), (G, \MB(G))]$. The previous exercise implies that $(\iota_{\mu})_* \mu = \mu$. SInce we have that $\iota_{\mu}$ is an isomorphism, \rex{43013} implies that $(\iota_{\mu})_* f_*\lam \ll (\iota_{\mu})_*\mu$ and therefore
			\begin{align*}
	...
	\begin{proof}
	\rex{ex:complex_measures:0013} implies that
	\begin{align*}
	\|g \cdot \nu\| 
...
\end{align*}	 
	...
	\begin{defn}
		\begin{itemize}\
			\item $\bMeas$:
			\begin{itemize}
				\item $\Obj(\bMeas) \defeq \{(X, \MA): (X, \MA) \text{ is a measurable space} \}$
				\item $\Meas((X, \MA), (Y, \MB)) \defeq \{f:X \rightarrow Y: f \text{ is $(\MA, \MB)$-measurable}\}$.
			\end{itemize}
			\item $\bTopRadMsr$: 
			\begin{itemize}
				\item $\Obj(\bMeas) \defeq \{(X, \mu): X \in \Obj(\bTop) \text{ and } \mu \in \MM_+(X) \}$
				\item $\Meas((X, \mu), (Y, \nu)) \defeq \{f \in \Hom_{\bTop}(X, Y): f_*\mu = \nu\}$.
			\end{itemize}
			\item $\bTopRadMsrone$: 
			\begin{itemize}
				\item $\Obj(\bMeas) \defeq \{(X, \mu): X \in \Obj(\bTop) \text{ and } \mu \in \MM_1(X) \}$
				\item $\Meas((X, \mu), (Y, \nu)) \defeq \{f \in \Hom_{\bTop}(X, Y): f_*\mu = \nu\}$.
			\end{itemize}
		\end{itemize}
	\end{defn}
