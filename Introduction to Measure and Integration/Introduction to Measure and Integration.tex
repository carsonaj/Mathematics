...
\end{align*}	 
	...
	\begin{proof} Please refer to the definition of the sum in the appendix. First suppose that $f(X) \subset \Rg$. For $n \in \N$, put $X_n = \{x \in X: f(x) > 1/n\}$ and define $X^* = \{x \in X: f(x) > 0\}$, $X_0 = \{x \in X: f(x) = 0\}$ Then $X^* = \bigcup\limits_{n \in \N}X_n$. Since $f \in L^1$, we have that for each $n \in \N$,
		\begin{align*}
			\infty 
			&> \int f d\#\\
			&\geq \int_{X_n} f d\# \\
	...
	\subsection{The Riesz-Markov-Kakutani  Representation Theorem}
...
\end{enumerate}	 
	...
	\begin{ex}
	Let $X$ be a LCH space and $\mu: \MB(X) \rightarrow \RG$ a Radon measure. Define $$\MN_{\mu} = \{U \subset X: U \text{ is open and } \mu(U) = 0\}$$ and $$N_{\mu} = \bigcup_{U \in \MN_{\mu}} U$$ 
	Then $N_{\mu}$ is open and $\mu(N_{\mu}) = 0$.
	\end{ex}
