...
\tcr{just go over basic category theoretic systems and limits in the analysis notes, like product systems, equalizer systems, projective systems and their limits: products, equalizers, projective limits, etc}.

\begin{ex} \lex{ex:measurable_spaces:proj_limits:0001}
...
\section{Coproduct Measures}
	...
	\begin{ex} \lex{ex:integration_nonnegative_functions:0002} 
		Let $(X, \MA)$ be a measurable space, $(\mu_j)_{j = 1}^m \subset M_+(X, \MA)$, $(\lam_j)_{j = 1}^m \subset \Rg$ and $f \in L^+(X, \MA)$. Define $\mu \in M_+(X, \MA)$ by $\mu \defeq \sum\limits_{j = 1}^m \lam_j \mu_j$. Then 
		$$\int_X f \dmu = \sum_{j = 1}^m \lam_j \int_X f \dmu_j. $$  
	\end{ex}

	\begin{proof}
		Suppose that $f$ is simple. Then there exist $(a_k)_{k=1}^n \subset \Rg$ and $(E_k)_{k=1}^n \subset \MA$ such that $(E_k)_{k=1}^n $ is disjoint and $f = \sum\limits_{k =1}^n a_k \chi_{E_k}$. Then 
		\begin{align*}
			\int_X f \dmu
			& = \sum \limits_{k =1}^n a_k \mu(E_k) \\
			& = \sum \limits_{k =1}^n a_k \bigg[ \sum_{j = 1}^m \lam_j \mu_j \bigg](E_k) \\
			& = \sum \limits_{k =1}^n a_k \bigg[ \sum_{j = 1}^m \lam_j \mu_j(E_k) \bigg] \\
			& = \sum \limits_{k = 1}^n \bigg[ \sum_{j = 1}^m a_k \lam_j \mu_j(E_k) \bigg] \\
			& = \sum \limits_{j =1}^n \bigg[ \sum_{k =1}^n a_k \lam_j \mu_j(E_k) \bigg] \\
			& = \sum \limits_{j =1}^n \lam_j \bigg[ \sum_{k = 1}^n a_k \mu_j(E_k) \bigg] \\
			& = \sum_{j = 1}^m \lam_j \int_X f \dmu_j 
		\end{align*}
		...
		\begin{align*}
			...
			& = \limn  \bigg[ \sum_{j = 1}^m \lam_j \int_X \phi_n \dmu_j  \bigg] \\
			& = \sum_{j = 1}^m \bigg[ \limn  \lam_j \int_X \phi_n \dmu_j  \bigg] \\
			& = \sum_{j = 1}^m  \lam_j \bigg[ \limn \int_X \phi_n \dmu_j  \bigg] \\
			& = \sum_{j = 1}^m  \lam_j \int_X f \dmu_j  \\
		\end{align*}
	\end{proof}
	...
	\begin{defn} \lex{def:integration_nonnegative_functions:0009.1} 
		Let $(X, \MA, \mu)$ be a measure space and $f \in L^+(X, \MA)$. We define $\mu_f: \MA \rightarrow \RG$ by $\mu_f(E) \defeq \int_E f \dmu$.
	\end{defn}
	
	\begin{ex} \lex{ex:integration_nonnegative_functions:0010} 
		Let $(X, \MA, \mu)$ be a measure space and $f \in L^+(X, \MA)$. Define $\mu_f: \MA \rightarrow \RG$ by 
		$$\mu_f(E) = \int_E f d\mu$$ 
		Then 
		\begin{enumerate}
			\item $\mu_f$ is a measure on $(X, \MA)$ 
			\item for each $g \in L^+(X, \MA)$, 
			$$\int g d\mu_f = \int g f d\mu$$
		\end{enumerate}
	\end{ex}
	
	\begin{proof}\
		\begin{enumerate}
			\item Clearly $\mu_f(\varnothing) = 0$. Let $(A_j)_{j \in \N} \subset \MA$ and suppose that for each $i, j \in \N$, if $i \neq j$, then $A_i \cap A_j = \varnothing$. Suppose that $f \in S^+(X, \MA)$. Then there exist $E_1, E_2, \cdots, E_n \in \MA$ and  $a_1, a_2, \cdots, a_n \in \Rg$ such that $f = \sum\limits_{i=1}^n a_i \chi_{E_i}$. Then 
			\begin{align*}
				\mu_f\bigg(\bigcup_{j \in \N} A_j\bigg) 
	...
	\begin{proof}
		...
		\begin{align*}
			...
			& = (f_*\mu)_g(B).
		\end{align*}
		Since $B \in \MB$ is arbitrary, we have that for each $B \in \MB$, $f_*(\mu_{f^*g})(B) = (f_*\mu)_g(B)$. Thus $f_*(\mu_{f^*g}) = (f_*\mu)_g$. 
	\end{proof}
...
\begin{ex} \lex{} \tbf{Kolmogorov Extension Theorem:} \\
	...
	\begin{itemize}
		...
		\item $\sup\limits_{j \in J} \mu_j(X_j) < \infty$ \tcr{(maybe not necessary)}
	\end{itemize}
...
\begin{proof}\
	\begin{enumerate}
		\item Clear by \rex{ex:measurable_spaces:proj_limits:0001}
		...
		\begin{enumerate}
			...
			\begin{itemize}
				...
				\begin{align*}
					...
					& < \frac{\del}{2^m}.
				\end{align*}
				Since $\mu_0$ is finitely additive, \rex{arg1} \tcr{(ref ex here)} implies that $\mu_0$ is finitely-subadditive. Since $(B_n)_{n \in \N}$ is decreasing, we have that for each $n \in \N$,
				\begin{align*}
			...
			\begin{align*}
				...
				& < \infty, 
			\end{align*}
			\rex{arg1} \tcr{(ref exercise about finitely additive set functions )} implies
			\begin{align*}
				\mu_0(B_n \setminus D_n)
				& = \mu_0(B_n) - \mu_0(D_n) \\
				& \geq \del - \mu_0(D_n). 
...
\rex{ex:integration_complex_functions:00006.1}
