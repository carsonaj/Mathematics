...
\begin{document}
	...
	\begin{itemize}
		...
		\item Let $S$ be a set. We define the \tbf{arity map}, denoted $\ar: S^* \rightarrow \N_0$, by 
	...
	\begin{defn} 
		Let $\MA$ be a set, $\MV \subset \MA$, $\MF \subset \bigcup\limits_{k \in \N_0} A^k$, $\MC \subset \bigcup\limits_{k \in \N_0} \MF^{(\MF^k)}$. Then $(\MA, \MV, \MA)$ is said to be a \tbf{propositional calculus} 
		if 
		\begin{enumerate}
			\item 
			\begin{itemize}
