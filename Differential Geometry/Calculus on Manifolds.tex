...
\begin{document}
	...
	\begin{defn}
		Let $\{A_i\}_{i \in I}$ be a collection of sets. The \textbf{disjoint union of} $\{A_i\}_{i \in I}$, denoted $\coprod\limits_{i \in I} A_i$, is defined by $$\coprod_{i \in I}A_i = \bigcup_{i\in I} \{i\} \times A_i$$ Define the \textbf{projection map} $\pi: \coprod\limits_{i \in I}A_i \rightarrow I$ by $$\pi(i, A_i) = i$$
	\end{defn}

	\begin{defn}
		Let $A, B$ be sets and $\pi : A \rightarrow B$ and $\sig:B \rightarrow A$. Then $\sig$ is said to be a \textbf{section of $\pi$} if $\pi \circ \sig = \id_B$.
	\end{defn}

	\begin{ex}
		Let $\{A_i\}_{i \in I}$ be a collection of sets, $\pi: \coprod\limits_{i \in I} A_i \rightarrow I$ the projection map and $\sig: I \rightarrow \coprod\limits_{i \in I} A_i$. If $\sig$ is a section of $\pi$, then for each $i \in I$, $\sig(i) \in \{i\} \times A_i$.
	\end{ex}

	\begin{proof}
		Suppose that $\sig$ is a section of $\pi$. Let $i \in I$. Then there exists $j \in I$ and $a \in A_j$ such that $\sig(i) = (j, a)$. Since $\pi \circ \sig = \id_I$, we have that
		\begin{align*}
			i 
			&= \pi \circ \sig (i) \\
			&= \pi(j, a) \\
			&= j
		\end{align*}
	So $i = j$ and $\sig(i) = (i, a) \in \{i\} \times A_i$
	\end{proof}
	...
	\begin{proof}
		Since $U$ is star-shaped, $\{p + t(x-p): 0 \leq t \leq 1\} \subset U$. By the chain rule, 
		$${\dv{t}}f(p+t(x-p)) = \sum_{i=1}^n {\pdv{f}{x_i}}(p+t(x-p)) (x_i - p_i)$$
...
\newpage
	...
	\begin{defn}
		We define the \textbf{tangent bundle of $M$}, denoted $TM$, by $$TM = \bigcup_{p \in M} T_pM$$ 
		and we define the \textbf{cotangent bundle of $M$}, denoted $T^*M$, by 
		$$T^*M = \bigcup_{p \in M} T_p^*M$$
	\end{defn}

\begin{defn}
	Let $(U, \phi) \in \MA$ with $\phi = (x_1, \cdots, x_n)$ and $I = (i_1, \cdots, i_k) \in \MI_k$. For $p \in U$, define $$\eval{\pdv{x_I}}_{p} = \bigg( \eval{\pdv{x_{i_1}}}_{p}, \cdots, \eval{\pdv{x_{i_k}}}_{p} \bigg)$$ and define $$\pdv{x_I}:U \rightarrow TM$$ by $$p \mapsto \eval{\pdv{x_I}}_{p}$$
\end{defn}
	...
	\begin{defn}
		Let $(N, \MB)$ be a smooth manifold, $F: M \rightarrow N$ a diffeomorphism and $p \in M$. We define the \textbf{push forward of $F$ at $p$}, denoted $(F_*)_p: T_pM \rightarrow T_{F(p)}N$, by $$\bigg[ (F_*)_p(D) \bigg] (f) = D (f \circ F)$$  for $D \in T_pM$ and $f \in C^{\infty}_{F(p)}(N)$.
	\end{defn}
	...
	\begin{defn}
		Let $(N, \MB)$ be a smooth manifold, $F: M \rightarrow N$ a diffeomorphism. Define the \textbf{push forward of $F$}, denoted $$F_*:M \rightarrow \bigcup_{p \in M} \iso(T_pM, T_{F(p)}N)$$ by $$p \mapsto (F_*)_p$$.
	\end{defn}
	...
	\begin{defn}
		We define $$\Lam_k (TM) = \bigcup_{p \in M} \Lam_k(T_p M)$$
	\end{defn}
	
	\begin{defn}
		Let $(U, \phi) \in \MA$ with $\phi = (x_1, \cdots, x_n)$. Then 
		we define $dx_1, \cdots, dx_n :U \rightarrow \Lam_1 (TU)$,  by $$(dx_i)_p \bigg (\eval{\pdv{x_j}}_{p} \bigg) = \del_{i,j}$$ That is, for each $p \in U$, $\{(dx_1)_p, \cdots, (dx_n)_p\}$ is the dual basis of $\bigg \{\eval{\pdv{x_1}}_{p}, \cdots, \eval{\pdv{x_n}}_{p} \bigg \}$ for $T_p^*M$
	\end{defn}
	
	\begin{defn}
		Let $\om: M \rightarrow \Lam_k (TM)$. Then $\om$ is said to be a \textbf{$k$-form on $M$} if for each $p \in M$, $\om_p \in \Lam_k(T_pM)$.\\
		Let $(U, \phi)$ be a coordinate chart on $M$ with $\phi = (x_1, \cdots, x_n)$. Then $\om$ is said to be a \textbf{smooth on $U$} if for each $I \in \MI_k$, there exist $f_I \in C^{\infty}(U)$ such that $$\om\bigg(\pdv{x_I} \bigg): U \rightarrow \R$$ is smooth.\\
		
		$$\om|_U = \sum_{I \in \MI_k} f_I dx_I$$ where for $I = (i_1, \cdots, i_k) \in \MI_k$, $(dx_I)_p = (dx_{i_1})_p \wedge \cdots \wedge (dx_{i_k})_p$.\\
		If for each $(U, \phi) \in \MA$, $\om$ is smooth on $U$, then $\om$ is said to be a \textbf{differential $k$-form on $M$}. The set of differential $k$-forms on $M$ is denoted $\Psi_k(M)$.\\
	\end{defn} 
	...
	\begin{defn}
		Let $(N, \MB)$ be a smooth manifold and $F: M \rightarrow N$ be a diffeomorphism. Define the \textbf{pullback of $F$}, denoted $F^*: \Om_k(N) \rightarrow \Om_k(M)$ by  $$(F^* \om)_p (D_1, \cdots, D_k) = \om_{F(p)} ((F_*)_p(D_1), \cdots, (F_*)_p(D_k))$$ for $p \in M$ and $D_1, \cdots, D_k \in T_{p}M$
	\end{defn}
