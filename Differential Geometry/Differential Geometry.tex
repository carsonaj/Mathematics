...
\newpage
	...
	\begin{defn}
		Let $(U, \phi) \in \MA$ with $\phi = (x_1, \cdots, x_n)$ and $I = (i_1, \cdots, i_k) \in \MI_k$. For $p \in U$, define $$\eval{\pdv{x_I}}_{p} = \bigg( \eval{\pdv{x_{i_1}}}_{p}, \cdots, \eval{\pdv{x_{i_k}}}_{p} \bigg)$$
	\end{defn}
	...
	\begin{defn}
		Let $X: M \rightarrow TM$. Then $X$ is said to be a \textbf{vector field on $M$} if for each $p \in M$, $X_p \in T_p M$. Let $(U, \phi) \in \MA$ with $\phi = (x_1, \cdots, x_n)$. Then $X$ is said to \textbf{smooth on $U$} if there exist $f_1, \cdots, f_n \in C^{\infty}(U)$ such that $$X|_U = \sum_{i=1}^n f_i \pdv{x_i}$$ 
		Finally, $X$ is said to be \textbf{smooth} if for each $(U, \phi) \in \MA$, $X$ is smooth on $U$. We denote the set of smooth vector fields on $M$ by $\Gam(M)$.
	\end{defn}

	\begin{defn}
		Let $(U, \phi) \in \MA$ with $\phi = (x_1, \cdots, x_n)$ and define $\pdv{x_I} \in \Gam(M)$ by $$p \mapsto \eval{\pdv{x_I}}_{p}$$
	\end{defn}
	...
	\begin{defn}
		...
		Let $(U, \phi)$ be a coordinate chart on $M$ with $\phi = (x_1, \cdots, x_n)$. Then $\om$ is said to be a \textbf{smooth on $U$} if for each $I \in \MI_k$,  The map $$\om\bigg(\pdv{x_I} \bigg): U \rightarrow \R$$ given by $$p \mapsto \om_p \bigg(\eval{\pdv{x_I}}_{p} \bigg)$$ is smooth 
		and $\om$ is said to be a \textbf{differential $k$-form on $M$} if for each $(U, \phi) \in \MA$, $\om$ is smooth on $U$. The set of differential $k$-forms on $M$ is denoted $\Om_k(M)$.\\
	\end{defn} 
	...
	\begin{defn}
		Let $\om \in \Om_k(M)$. We define the \textbf{exterior derivative of $\om$} by 
		\begin{enumerate}
			\item $(d\om)_p (D) = D \om $ for $p \in M$ and $D \in T_pM$
			\item $d \om $
		\end{enumerate}
		$df \in \Om_1(M)$ by $$(df)_p (D) = D f $$ 
		for $p \in M$ and $D \in T_pM$. 
	\end{defn}
	...
	\begin{defn}
		...
		Define where for $I = (i_1, \cdots, i_k) \in \MI_k$, $(dx_I)_p = (dx_{i_1})_p \wedge \cdots \wedge (dx_{i_k})_p$.\\
	\end{defn}

	\begin{ex}
		Let $\om: M \rightarrow \Lam_k(TM)$ be a k-form on $M$ and $(U, \phi) \in \MA$ with $\phi = (x_1, \cdots, x_n)$. Then $\om$ is smooth on $U$ iff for each $I \in \MI_k$ there exist $f_I \in C^{\infty}(U)$ such that $$\om|_U = \sum_{I \in \MI_k} f_I dx_I$$ 
	\end{ex}
