...
\begin{document}
	...
	\begin{defn}
		Let $\MC, \MD$ be categories and $X \in \MD$. We define the \textbf{constant functor} from $\MC$ onto $X$, denoted $\Del_X: \MC \rightarrow \MD$ by 
		\begin{itemize}
			\item $\Del_X (A) = X$
			\item $\Del_X (f) = \id_X$
		\end{itemize}
	...
	\begin{ex}
		Let $\MC, \MD$ be categories and $X \in \Obj(\MD)$. Then $\Del_X : \MC \rightarrow \MD$ is a functor.
	\end{ex}
	...
	\begin{proof}\
		\begin{enumerate}
			...
			\end{align*}
		\end{enumerate}
		So $\Del_X : \MC \rightarrow \MD$ is a functor.
	\end{proof}
	...
	\begin{defn}
		Let $\MC, \MD$ be a categories, $X \in \Obj(\MD)$ and $F: \MC \rightarrow \MD$. We define the \textbf{comma category from $F$ to $X$}, denoted $(F \downarrow X)$, by $(F \downarrow X) = (F \downarrow \Del_X)$ where $\Del_X: \1 \rightarrow \MD$ is the constant functor. \\
	...
	\begin{defn}
		Let $\MC, \MD$ be a categories, $X \in \Obj(\MD)$ and $F: \MC \rightarrow \MD$. We define the \textbf{comma category from $X$ to $F$}, denoted $(X \downarrow F)$, by $(X \downarrow F) = (\Del_X \downarrow F)$ where $\Del_X: \1 \rightarrow \MD$ is the constant functor. \\
		We may make the following identification:
		\begin{itemize}
	...
	\begin{defn} \ld{14008}
		Let $\MC$, $\MD$ be categories, $X, Y \in \Obj(\MD)$ and $f \in \Hom_{\MD}(X, Y)$. We define the \textbf{constant natural transformation} at $f$, denoted $\del_f: \Del_X \Rightarrow \Del_Y$, by
		$$(\del_f)_A = f$$
	\end{defn}
	
	\begin{ex} \lex{14009}
		Let $\MC$, $\MD$ be categories, $X, Y \in \Obj(\MD)$ and $f \in \Hom_{\MD}(X, Y)$. Then $\del_f: \Del_X \Rightarrow \Del_Y $ is a natural transformation.
	\end{ex}
	
	\begin{proof} \
		\begin{enumerate}
			\item By definition, for each $A \in \Obj(\MC)$ $(\del_f)_A \in \Hom_{\MD}(\Del_X(A), \Del_Y(A))$.
			\item Let $A, B \in \Obj(\MC)$ and $g \in \Hom_{\MC}(A, B)$. Then 
			\begin{align*}
				\Del_Y(g) \circ (\del_f)_A 
				& = \id_Y \circ f \\
				& = f \\
				& = f \circ \id_X \\
				& = (\del_f)_B \circ \Del_X(g)  
			\end{align*}
			i.e. the following diagram commutes:
			\[ 
			\begin{tikzcd}[baseline= 7]
				\Del_X(A)  \arrow[r, "(\del_f)_A"]  \arrow[d, "\Del_X(g)"']  & \Del_Y(A)   \arrow[d, "\Del_Y(g)"]\\
				\Del_X(B) \arrow[r, "(\del_f)_B"] &  \Del_Y(B) \\
			\end{tikzcd}
			=
			\begin{tikzcd}[baseline= 7]
			...
			\]
		\end{enumerate}
		So $\del_f: \Del_X \Rightarrow \Del_Y$ is a natural transformation.
	\end{proof}
	 
	\begin{ex} \lex{14010}
		Let $\MC, \MD$ be categories, $X, Y, Z \in \Obj(\MD)$, $f \in \Hom_{\MD}(X,Y)$ and $g \in \Hom_{\MD}(Y,Z)$. Then $\del_{g \circ f} = \del_g \circ \del_f$.
	\end{ex}
	...
	\begin{proof}
		...
		\begin{align*}
			(\del_{g \circ f})_A
			& = g \circ f \\
			& = (\del_g)_A \circ (\del_f)_A \\
			& = (\del_g \circ \del_f)_A
		\end{align*}
		Since $A \in \Obj(\MC)$ is arbitrary, $\del_{g \circ f} = \del_g \circ \del_f$.
	\end{proof}

	\begin{ex} \lex{14011}
		Let $\MC, \MD$ be categories and $X \in \Obj(\MD)$. Then $\del_{\id_X} = \id_{\Del_X}$.
	\end{ex}
	...
	\begin{proof}
		...
		\begin{align*}
			...
			& = (\id_{\Del_X})_A 
		\end{align*}
		Since $A \in \Obj(\MC)$ is arbitrary, $\del_{\id_X} = \id_{\Del_X}$
	\end{proof}

	\begin{defn} \ld{14011}
		Let $\MC$, $\MD$ be categories. Suppose that $\MC$ is small. We define the \textbf{$\MC$-ary diagonal functor} on $\MD$, denoted by $\Del: \MD \rightarrow \MD^{\MC}$, by
		\begin{itemize}
			\item $\Del(X) = \Del_X$
			\item $\Del(f) = \del_f$
		\end{itemize}
	...
	\begin{ex} \lex{14012}
		Let $\MC$, $\MD$ be categories. Suppose that $\MC$ is small. Then $\Del: \MD \rightarrow \MD^{\MC}$ is a functor.
	\end{ex}

	\begin{proof}\
		\begin{enumerate}
			\item \rex{14009} implies that for each $X, Y \in \Obj(\MD)$ and $f \in \Hom_{\MD}(X, Y)$, $\Del(f) \in \Hom_{\MD^{\MC}} (\Del(X), \Del(Y))$
			\item \rex{14010} implies that for each $X, Y, Z \in \Obj(\MD)$, $f \in \Hom_{\MD}(X, Y)$ and $g \in \Hom_{\MD}(Y, Z)$, $\Del(g \circ f) = \Del(g) \circ \Del(f)$
			\item \rex{14011} implies that for each $X \in \Obj(\MD)$, $\Del(\id_X) = \id_{\Del(X)}$
		\end{enumerate}
		 So $\Del : \MD \rightarrow \MD^{\MC}$ is a functor.
	\end{proof}
	...
	\begin{proof}
		Let $\MC \in \Obj(\Cat)$. It is clear that $\Hom_{\Cat}(\MC, \1) = \{ \Del_* \}$. Hence $\1$ is final in $\Cat$.
	\end{proof}

	\begin{defn} \ld{15017}
		Let $\MC$, $\MD$ be categories and $0 \in \Obj(\MD)$ and $F: \MC \rightarrow \MD$. Suppose that $0$ is initial in $\MD$. Then for each $A \in \Obj(\MC)$, there exists $f_A \in \Hom_{\MD}(0, F(A))$ such that $\Hom_{\MD}(0, F(A)) = \{f_A\}$. We define the \textbf{initial natural transformation induced by $0$} from $\Del_0$ to $F$, denoted $\zeta_{0}: \Del_0 \Rightarrow F$, by 
		$(\eta_0)_A = f_A$.
	\end{defn}

	\begin{defn} \ld{15017.1}
		Let $\MC$, $\MD$ be categories and $1 \in \Obj(\MD)$ and $F: \MC \rightarrow \MD$. Suppose that $1$ is final in $\MD$. Then for each $A \in \Obj(\MC)$, there exists $f_A \in \Hom_{\MD}(F(A), 1)$ such that $\Hom_{\MD}(F(A), 1) = \{f_A\}$. We define the \textbf{final natural transformation induced by $1$} from $F$ to $\Del_1$, denoted $\phi_{1}: F \Rightarrow \Del_1$, by 
		$(\phi_{1})_A = f_A$.
	\end{defn}

	\begin{ex} \lex{15018}
		Let $\MC$, $\MD$ be categories and $0 \in \Obj(\MD)$ and $F: \MC \rightarrow \MD$. Suppose that $0$ is initial in $\MD$. Then $\eta_0: \Del_0 \Rightarrow F$ is a natural transformation.
	\end{ex}

	\begin{proof} \
		\begin{enumerate}
			\item By definition, for each $A \in \Obj(\MC)$, $(\eta_0)_A \in \Hom_{\MD}(\Del_0(A), F(A))$
			...
			\[ 
			\begin{tikzcd}[baseline= 7]
				\Del_0(A)  \arrow[r, "(\eta_0)_A"]  \arrow[d, "\Del_0(f)"']  & F(A)   \arrow[d, "F(f)"]\\
				\Del_0(B) \arrow[r, "(\eta_0)_B"] &  F(B) \\
			\end{tikzcd}
			=
			\begin{tikzcd}[baseline= 7]
			...
			\]
		\end{enumerate}
		So $\eta_0: \Del_0 \Rightarrow F$ is a natural transformation.
	\end{proof}
	
	\begin{ex} \lex{15019}
		Let $\MC$, $\MD$ be categories and $1 \in \Obj(\MD)$ and $F: \MC \rightarrow \MD$. Suppose that $1$ is final in $\MD$. Then $\phi_{1}: F \Rightarrow \Del_0$ is a natural transformation.
	\end{ex}
	...
	\begin{ex} \lex{15020}
		Let $\MC$, $\MD$ be categories and $0 \in \Obj(\MD)$. Suppose that $\MC$ is small. If $0$ is initial in $\MD$, then $\Del_0$ is initial in $\MD^{\MC}$. 
	\end{ex}

	\begin{proof}
		Suppose that $0$ is initial in $\MD$. Let $F \in \Obj(\MD^{\MC})$, $\al \in \Hom_{\MD^{\MC}}(\Del_0, F)$ and $A \in \Obj(\MC)$. Then
		\begin{align*}
			\al_A 
			& \in \Hom_{\MD}(\Del_0(A), F(A)) \\
			& =  \Hom_{\MD}(0, F(A)) \\
			& = \{(\eta_0)_A\}
		\end{align*}
		Hence $\al_A = (\eta_0)_A$. Since $A \in \Obj(\MC)$ is arbitrary, $\al = \eta_0$. Since $\al \in  \Hom_{\MD^{\MC}}(\Del_0, F)$ is arbitrary, $ \Hom_{\MD^{\MC}}(\Del_0, F) = \{\eta_0\}$. Therefore $\Del_0$ is initial in $\MD^{\MC}$.
	\end{proof}


	\begin{ex} \lex{15021}
		Let $\MC$, $\MD$ be categories and $1 \in \Obj(\MD)$. Suppose that $\MC$ is small. If $1$ is final in $\MD$, then $\Del_1$ is final in $\MD^{\MC}$. 
	\end{ex}
