...
\begin{document}
	...
	\begin{proof}
		\begin{itemize}
			Let $(g, f ) \in \Hom_{\MC}^{(2)}$. Since $\MC$ is a category, there exists $g$ \tcr{FINISH!!!}
		\end{itemize}
	\end{proof}
	...
	\tcr{maybe move the examples from above here or rename or something}
	...
	\begin{defn}
		...
		\begin{enumerate}
			\item \tbf{proset} if $\MP$ is locally small and for each $a, b \in \Obj(\MP)$, $\# \Hom_{\MP}(a,b) \leq 1$
			\item \tbf{poset} if $\MP$ is a proset and for each $a,b \in \Obj(\MP)$, $\# \Hom_{\MP}(a,b) = 1$ and $\# \Hom_{\MP}(b,a) = 1$ implies that $a = b$.  
		\end{enumerate}
	\end{defn}
	...
	\tcr{equivalence relation on a set is a type of proset}
	\begin{defn}
...
\end{defn}
	...
	\begin{defn} \ld{15000.1}
		Let $\MC$ be a category, $A \in \Obj(\MC)$ and $f \in \Hom_{\MC}(A, A)$. Then $f$ is said to be an \textbf{endomorphism of $A$}. We define the \tbf{class of endomorphisms of $A$}, denoted $\Endo_{\MC}(A)$, by 
	...
	\begin{defn} \ld{15002}
		Let $\MC$ be a category, $A,B \in \Obj(\MC)$ and $f \in \Hom_{\MC}(A, B)$. Then $f$ is said to be an \textbf{isomorphism} if there exists $g \in \Hom_{\MC}(B, A)$ such that $g \circ f = \id_{A}$ and $f \circ g = \id_{B}$. We define the \tbf{class of isomorphisms from $A$ to $B$}, denoted $\Iso(A,B)$, by 
		$$\Iso(A, B) = \{f \in \Hom_{\MC}(A,B): f \text{ is an isomorphism}\}$$
	\end{defn}

	\begin{defn} \ld{15002.1}
		Let $\MC$ be a category, $A \in \Obj(\MC)$ and $f \in \Endo_{\MC}(A)$. Then $f$ is said to be an \textbf{automorphism} if $f$ is an isomorphism. We define the \tbf{class of automorphisms of $A$}, denoted $\Aut(A)$, by 
...
\end{defn}
	...
	\begin{defn}
		Let $\MC, \MD$ be a categories, $X \in \Obj(\MD)$, $F: \MC \rightarrow \MD$, $A \in \Obj(\MC)$ and $f \in \Hom_{\MD}(F(A), X)$. Then $(A, f)$ is said to be a \textbf{universal morphism} from $F$ to $X$ if for each $A' \in \Obj(\MC)$ and $f' \in \Hom_{\MD}(F(A'), X)$, there exists a unique $\al \in \Hom_{\MC}(A', A)$ such that $f' = f \circ F(\al)$, i.e. the following diagram commutes:
		\[
		\begin{tikzcd}
			X   & F(A) \arrow[l, "f"'] \\
	...
	\begin{note}
		\tcr{make a comment on how if $(A, f)$ is universal from $X$ to $F$, then for each $(A', f')$, $f'$ is a post-processing of $f$}
	\end{note}
	...
	\begin{note}
		\tcr{make a comment on how if $(A, f)$ is universal from $F$ to $X$, then for each $(A', f')$, $f'$ is a pre-processing of $f$}
	\end{note}
	...
	\begin{exmp}
		Define $\MJ$ and $\MD$ \tcr{as in previous example}. Let $(X, \phi) \in \Obj(\Cone(D))$. We can visualize $(X, \phi)$ as follows:
		\[
		\begin{tikzcd}
