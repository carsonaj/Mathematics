...
We define the \textbf{topology generated by $\ME$} on $X$, denoted $\tau(\ME)$, by $$\tau(\ME) = \bigcap_{\MT \in \MS} \MT$$
	...
	\begin{ex} \lex{31008}
	Let $X$ be a set and $\MT \subset \MP(X)$ a topology on $X$ and $\MB \subset \MT$. Then $\MB$ is a basis for $\MT$ iff for each $U \in \MT$, there exists a collection $\MC \subset \MB$ such that $$U = \bigcup\limits_{V \in \MC} V$$
	\end{ex}
	
	\begin{proof}
	Suppose that $\MB$ is a basis for $\MT$. Let $U \in \MT$. Since since $\MB$ is a basis for $\MT$, for each $x \in U$, there exists $\MB_x \subset \MB \subset \MT$ such that $\MB_x$ is a local basis for $\MT$ at $x$. This implies that for each $x \in U$, there exists $V_x \in \MB_x$ such that $x \in V_x \subset U$. Then $(V_x)_{x \in U} \subset \MB$ satisfies $U = \bigcup\limits_{x \in U} V_x$. \\
	Conversely, suppose that for each $U \in \MT$, there exists a collection $\MC \subset \MB$ such that $U = \bigcup\limits_{V \in \MC} V$. Let $x \in X$. Set $\MB_x = \{V \in \MB:x \in V\}$. Let $U \in \MT$. Suppose that $x \in U$. By assumption, there exists a collection $\MC \subset \MB$ such that $U = \bigcup\limits_{V \in \MC} V$. Since $x \in U$, there exists $V \in \MC$ such that $x \in V$. By definition, $V \in \MB_x$ and $V \subset U$. Hence $\MB_x$ is a local basis for $\MT$ at $x$. Since $x \in X$ is arbitrary, we have that for each $x \in X$, there exists $\MB_x \subset \MB$ such that $\MB_x$ is a local basis for $\MT$ at $x$. Thus $\MB$ is a basis for $\MT$.
	\end{proof}
...
\end{align*}	
	...
	\begin{ex} \textbf{Topology in Terms of Nets: } \\
...
\end{proof}
	...
	\begin{defn}
	Let $(X_{\al})_{\al \in A}$ be a collection of topological spaces. We define the \textbf{product topology} on $\prod_{\al \in A}X_{\al}$ to be the initial (weak) topology on $\prod\limits_{\al \in A} X_{\al}$ generated by the projection maps $(\pi_{\al})_{\al \in A}$.
	\end{defn}
	...
	\begin{ex}
		Let $(X_j, \MT_j)_{j =1}^n$ be a collection of topological spaces and for each $j \in \{1, \ldots, n\}$, $\MB_{j}$ a basis for $\MT_{j}$. Set $X = \prod_{j=1}^nX_j$ and denote the product topology on $X$ by $\MT$. Set $\MB = \bigg \{\prod_{j=1}^n B_{j}: \text{ for each $j \in \{1, \ldots, n\}$, } B_j \in \MB_{j} \bigg \}$. Then $\MB$ is a basis for $\MT$.
	\end{ex}

	\begin{proof}
		Set $\MC = \bigg \{\prod\limits_{j=1}^n A_j: \text{for each $j \in \{1, \ldots, n\}$, } A_j \in \MT_{j} \bigg \}$. The previous exercise implies that $\MC$ is a basis for $\MT$. Let $U \in \MT$. Since $\MC$ is a basis for $\MT$, there exists $(C_{\lam})_{\lam \in \Lam} \subset \MC$ such that $U = \bigcup\limits_{\lam \in \Lam}C_{\lam}$. By definition, for each $\lam \in \Lam$ and $j \in \{1, \ldots, n\}$, there exist $A_{\lam, j} \in \MT_j$ such that $C_{\lam} = \prod\limits_{j=1}^n A_{\lam, j}$. Let $\lam \in \Lam$ and $j \in \{1, \ldots, n\}$. Since $\MB_{j}$ is a basis for $\MT_j$, there exists $(B_{\gam_{\lam, j}})_{\gam_{\lam, j} \in \Gam_{\lam, j}} \subset \MB_{j}$ such that $A_{\lam ,j} = \bigcup\limits_{\gam_{\lam, j} \in \Gam_{\lam, j}} B_{\gam_{\lam, j}}$. Hence 
		\begin{align*}
			U 
			& = \bigcup\limits_{\lam \in \Lam} C_{\lam} \\
			& = \bigcup\limits_{\lam \in \Lam} \bigg[ \prod\limits_{j=1}^n A_{\lam, j} \bigg] \\
			& = \bigcup\limits_{\lam \in \Lam} \bigg[ \prod\limits_{j=1}^n \bigg( \bigcup\limits_{\gam_{\lam, j} \in \Gam_{\lam, j}} B_{\gam_{\lam, j}} \bigg) \bigg] \\
			& = \bigcup\limits_{\lam \in \Lam} \bigg[ \bigcup\limits_{\gam_{\lam, 1} \in \Gam_{\lam, 1}} \cdots \bigcup\limits_{\gam_{\lam, n} \in \Gam_{\lam, n}} \bigg( \prod\limits_{j=1}^n    B_{\gam_{\lam, j}}  \bigg)  \bigg] 
		\end{align*}
		Since $\prod\limits_{j=1}^n    B_{\gam_{\lam, j}} \in \MB$ and $U \in \MT$ is arbitrary, we have that $\MB$ is a basis for $\MT$.
	\end{proof}
...
by definition of the quotient topology, $\bar{f}^{-1}(V) \in \pi_*\MA$. So $\bar{f}$ is $\MA$-$\pi_*\MA$ continuous.
	...
	\begin{defn}
		Let $(X, \MT)$ be a topological space. Then $(X, \MT)$ is said to be \textbf{second-countable} if there exists $\MB \subset \MT$ such that 
		\begin{enumerate}
	...
	\begin{ex}
		Let $(X_j, \MT_j)_{j=1}^n$ be a collection of topological spaces. Set $X = \prod\limits_{j=1}^n X_j$ and denote the product topology on $X$ by $\MT$. If for each $j \in \{1, \ldots, n\}$, $(X_j, \MT_j)$ is second-countable, then $(X, \MT)$ is second-countable.
	\end{ex}

	\begin{proof}
		Suppose that for each $j \in \{1, \ldots, n\}$, $(X_j, \MT_j)$ is second-countable. Then for each $j \in \{1, \ldots, n\}$, there exists $\MB_j \subset \MT_{j}$ such that $\MB_j$ is a basis for $\MT_j$ and $\MB_j$ is countable. Set $\MB =  \bigg\{ \prod\limits_{j=1}^n B_j: \text{for each $j \in \{1, \ldots, n\}$, $B_j \in \MB_j$} \bigg\}$. An exercise in the section on the product topology implies that $\MB$ is a basis for $\MT$. Since $\MB$ is countable, $\MT$ is second-countable. 
	\end{proof}
...
\end{proof}
	...
	\begin{defn}
		Let $X,Y$ and $Z$ be topological vector spaces (over the same field) and $b : X \times Y \rightarrow Z$. Then $b$ is said to be a \textbf{pairing of $X$ with $Y$ over $Z$} if $b$ is bilinear. \\
	\end{defn}

	\begin{defn}
		Let $X,Y$ and $Z$ be topological vector spaces and $b : X \times Y \rightarrow Z$ a pairing. We define the \textbf{dual pairing} of $b$, denoted $b^*:Y \times X \rightarrow Z$, by $b^*(y,x) = b(x,y)$. Then $b$ is a pairing. 
	\end{defn}
	...
	\begin{defn}
		Let $X,Y$ and $Z$ be topological vector spaces and $b : X \times Y \rightarrow Z$ a pairing. We define the \textbf{weak topology on $X$ induced by $b$}, denoted $\sig_b(X, Y)$ by 
		$$\sig_b(X,Y) = \tau_{X}(b(\cdot, y): X \rightarrow Z: y \in Y)$$ 
		We define the \textbf{weak topology on $Y$ induced by $b$}, denoted $\sig_b(Y, X)$, by $\sig_b(Y,X) = \sig_{b^*}(Y, X)$.
	\end{defn}
	...
	\begin{defn}
		Let $X,Y$ and $Z$ be topological vector spaces and $b : X \times Y \rightarrow Z$ a pairing. Then 
		\begin{itemize}
			\item $Y$ is said to \textbf{separate the points of $X$ via $b$} if
			for each $x \in X$, $x \neq 0$ implies that there exists $y \in Y$ such that $b(x,y) \neq 0$
			\item $X$ is said to \textbf{separate the points of $Y$ via $b$} if $X$ separates the points of $Y$ via $b^*$
		\end{itemize}
	\end{defn}
...
\end{proof}		
	...
	\begin{thm} \textbf{Open Mapping Theorem:} \\
...
So $$\int_{(x_0, x_0 + h]}f - f(x_0) dm = o(\|h\|) \hspace{1cm}\text{ as }h \rightarrow 0$$ 
	...
	\begin{defn}
		Let $(x_0,y_0) \in U$. Then $f$ is said to be \textbf{partial Frechet differentiable with respect to $X$ at $(x_0, y_0)$} if $f^{y_0}$ is Frechet differentiable at $x_0$. \\
		Suppose that $f$ is partial Frechet differentiable with respect to $X$ at $(x_0, y_0)$. We define the \textbf{partial Frechet derivative of $f$ with respect to $X$ at $(x_0, y_0)$}, denoted $D_X f(x_0, y_0) \in L(X, Z)$, by 
		$$D_Xf(x_0, y_0) = Df^{y_0}(x_0)$$
		Suppose that for each $y \in Y$, $f^{y}$ is Frechet differentiable. We define the \textbf{partial Frechet derivative of $f$ with respect to $X$}, denoted $D_X f: X \times Y \rightarrow L(X, Z)$, by 
		$$D_Xf(x, y) = Df^{y}(x)$$
	...
	\begin{defn} \ld{}
	Let $H$ be a Hilbert space, $f: H \rightarrow \C$ and $x_0 \in H$. Suppose that $f$ is Frechet differentiable at $x_0$. Then $Df(x_0) \in H^*$. We define the \textbf{gradient of $f$ at $x_0$}, denoted $\nabla f(x_0) \in H$, via the Riesz representation theorem to be the unique element of $H$ satisfying $$\l \nabla f(x_0), y \r = Df(x_0)(y) \hspace{.3cm} \text{ for each } y \in H$$
	\end{defn}
