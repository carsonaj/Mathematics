...
\end{align*}		
	...
	\begin{proof}\
	\begin{enumerate}
	\item Clear since $\| \cdot \|'_p$ is a norm on $\R^n$.
...
\end{proof}		
	...
	\begin{defn}
	Let $X, Y$ and $Z$ be normed vector spaces and $\phi \in L^2(X,Y ; Z) $. Then $(Z, \phi)$ is said to be a \textbf{tensor product} of $X$ with $Y$ if for each normed vector space $W$ and $\psi \in L^2(X,Y;W)$, there exists a unique $\psi' \in L(Z, W)$ such that $\psi' \circ \phi = \psi$, i.e. such that the following diagram commutes: 
	\[ \begin{tikzcd}
...
\end{tikzcd}
	\]
	If $(Z, \phi)$ is a tensor product of $X$ with $Y$. We write $Z = X \otimes Y$ and for each $x\in X$, $y \in Y$, we write $\phi(x,y) = x \otimes y$.
	\end{defn}	
	
	\begin{ex} \textbf{Uniqueness:}\\
	Let $X, Y$ and $Z$ be normed vector spaces and $\phi \in L^2(X,Y ; Z)$. Suppose that $(Z, \phi)$ is a tensor product of $X$ with $Y$. Then $(Z, \phi)$ is unique up to isomorphism. 
	\end{ex}
	
	\begin{proof}
	Let $W$ be a normed vector space and $\psi \in  L^2(X,Y ; W)$. Suppose that $(W, \psi)$ is a tensor product of $X$ with $Y$. Since $(Z, \phi)$ is a tensor product of $X$ with $Y$, there exists a unique $\psi' \in L(Z, W)$ such that $\psi' \circ \phi = \psi$. Since $(W, \psi)$ is a tensor product of $X$ with $Y$, there exists a unique $\phi' \in L(W,Z)$ such that $\phi' \circ \psi = \phi$. Thus the following diagram commutes: 
	\[ \begin{tikzcd}
	& W  \arrow[d, "\phi'"] \\
...
\end{tikzcd}
	...
	\begin{defn}
	\textbf{projection norm}
	\end{defn}
	
	\begin{ex} \textbf{Existence:} \\
	Let $X$ and $Y$ be normed vector spaces. Define $\phi \in L^2(X, Y; L^2(X,Y;\C)^*)$ by $\phi(x,y)(A) = A(x,y)$. Set $Z = \spn \phi(X \times Y)$. Then $(Z, \phi)$ is a tensor product of $X$ with $Y$.
	\end{ex}
...
\end{align*}	 
	...
	Let $X, Y$ be a banach spaces, $A \subset X$ open, $f:A \rightarrow Y$ and $x_0 \in A$. If $f$ is Frechet differentiable at $x_0$, then $f$ is continuous at $x_0$. This implies that $f(x) \rightarrow f(x_0)$ as $x \rightarrow x_0$ and therefore $f$ is continuous.
	\end{ex}
	
	\begin{proof}
	Suppose that $f$ is Frechet differentiable at $x_0$. Then $f(x) - f(x_0) = Df(x_0)(x - x_0) + o(\|x - x_0\|)$ as $x \rightarrow x_0$. Hence $\|f(x) - f(x_0)\| \leq \| Df(x_0)\| \|x - x_0 \| + o(\|x - x_0\|)$ as $x \rightarrow x_0$.
	\end{proof}
