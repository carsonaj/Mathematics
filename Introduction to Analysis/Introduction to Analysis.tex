...
is convex.  
	...
	\begin{ex}
	Let $X$ be a Banach space, $A \subset X$ open and convex, $f:A \rightarrow \R$ convex, $x_0 \in A$ and $x \in X$. Then there exists $t_0 > 0$ such that 
	\begin{enumerate}
	\item the right-hand difference quotient $$q(t) = \frac{f(x_0 + tx) - f(x_0)}{t}$$ is increasing on $(0, t_0)$
	\item the left-hand difference quotient $$q(-t) = \frac{f(x_0 - tx) - f(x_0)}{-t}$$ is decreasing on $(0, t_0)$
\end{enumerate}	 
	...
	\begin{proof}
	\begin{enumerate}
	\item Since $A$ is open, there exists $\del >0$ such that $B(x_0, \del) \subset A$. Let $0 < s \leq t$. Choose $t_0 = \frac{\del}{\|x\| + 1}$. Suppose that $t < t_0$. Then $x_0 +sx$, $x_0 + tx \in B(x_0, \del) \subset A$. Note that since $0 < s \leq t$, $\frac{s}{t} \in (0, 1]$ and $1- \frac{s}{t} = \frac{t-s}{t} \in (0, 1]$. Note that since $B(x_0, \del)$ is convex, we have that $$ \bigg( \frac{t-s}{t} \bigg) x_0 +  \bigg(\frac{s}{t} \bigg) (x_0 + tx) \in B(x_0, \del) \subset A$$
	...
	and so finally, dividing both sides by $st$, we obtain $$ \frac{f(x_0 + sx) - f(x_0)}{s} \leq \frac{f(x_0 + tx) - f(x_0)}{t} $$
	as desired.
	\item Similar to $(1)$.
	\end{enumerate}
	\end{proof}
	...
	\begin{ex}
	Let $X$ be a Banach space, $A \subset X$ open and convex, $f:A \rightarrow \R$ convex, $x_0 \in A$ and $x \in X$. Then there exists $t_0 > 0$ such that for each $t \in (0, t_0)$, $$q(-t) \leq q(t)$$ (\textbf{Hint:} for sufficiently small $t$, convexity of $f$ implies that $f(x_0) \leq \frac{1}{2} f(x_0 - 2tx) + \frac{1}{2} f(x_0 + 2tx)$)
	\end{ex}
	...
	\begin{align*}
	d^-f(x_0)(x)
	&= \sup_{t \in (0, t_0)} q(-t; x) \\
	&= \sup_{t \in (0, t_0)} \frac{f(x_0 + -tx) - f(x_0)}{-t} \\
	&= - \inf_{t \in (0, t_0)} \frac{f(x_0 + -tx) - f(x_0)}{t} \\
	&= - \inf_{t \in (0, t_0)} q(t; -x) \\
	&= - d^+f(x_0)(-x)
	\end{align*}
	\end{enumerate}
	\end{proof}
	...
	\begin{ex}
	Let $X$ be a Banach space, $A \subset X$ open and convex and $f:A \rightarrow \R$ convex. Then for each $x_0 \in A$, $d_+(x_0): X \rightarrow \R$ exists and is a sublinear functional.
	\end{ex}
	
	\begin{proof}
	Let $x_0 \in A$ and $x \in X$. Let $0< s < t$ and suppose that 
	\end{proof}
