...
\end{proof}
	...
	\subsection{Second Countability and Subspaces}
	
	

	\subsection{Second Countability and Product Spaces}
	\begin{ex}
	...
	\subsection{Second Countability and Quotient Spaces}
...
\begin{ex}
	...
	\begin{enumerate}
		...
		\item If $xy, yx \in G(X)$, then $x,y \in G(X)$.
	\end{enumerate}
\end{ex}
...
\begin{ex}
	Let $X$ be a unital Banach algebra and $x \in X$. Then
	\begin{enumerate}
		\item $R_x: \rho(x) \rightarrow G(X)$ is continuous
		...
		$$R_x'(\lam) = -R_x(\lam)^2$$
		\item $R_x \in C^{\infty}(\rho(x))$
	\end{enumerate}
\end{ex}

\begin{proof}\
	\begin{enumerate}
		\item A previous exercise implies that inversion $\iota_{\mu} : G(X) \rightarrow G(X)$ given by $\iota_{\mu}(a) =  a^{-1}$ is continuous. Since the map $\al_x: \rho(x) \rightarrow G(X)$ given by $\al_x(\lam) =  \lam e - x$ is continuous, we have that $R_x = \\iota_{\mu} \circ al_x$ is continuous.  
		...
		\begin{align*}
			...
			& = - R_x(\lam) R_x(\mu)
		\end{align*} 
		Since $R_x$ is continuous, we have that 
		\begin{align*}
			R_x'(\lam)
		...
		\item Since $R_x$ is differentiable and  $R_x' = -R_x^2$, by induction we have that $R_x \in C^{\infty}(\rho(x))$.
	\end{enumerate}
\end{proof}
...
\end{proof}
	...
	\begin{ex}
		...
		\tbf{Hint:} $\lam^n e -x^n = ab = ba$ where $a = \lam e - x$ and $ b = \sum_{j=0}^{n-1} \lam^j x^{n-1 -j} $
	\end{ex}

	\begin{proof}
		Suppose that $\lam^n \in \rho(x^n)$. Then $\lam^n e -x^n \in G(X)$. Set $a = \lam e - x$ and $b = \sum_{j=0}^{n-1} \lam^j x^{n-1 -j}$. Since $\lam^n e -x^n = ab$ and $\lam^n e -x^n = ba$, we have that $ab, ba \in G(X)$. A previous exercise implies that $a, b \in G(X)$. In particular, $\lam \in \rho(x)$. Therefore $\lam^n \in \rho(x^n)$ implies that $\lam \in \rho(x)$. Hence $\lam \in \sig(x)$ implies that $\lam^n \in \sig(x^n)$.
	\end{proof}
