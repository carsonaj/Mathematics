...
\end{proof}
	...
	\begin{ex} \lex{36012}
		Let $(X, \MA)$ $(Y, \MB)$ be topological spaces, $\sim$ an eqivalence relation on $X$ and $f:X \rightarrow Y$. Suppose that $f$ is a quotient map and $f$ is open. If for each $a,b \in X$ $a \sim b$ iff $f(a) = f(b)$, then $\bar{f}: {X/ \sim} \rightarrow Y$ is a $(\MA, \MB)$-homeomorphism. \\
		\tbf{Hint:} for $U \in \MT_{X/\sim}$, $\bar{f}(U) = f(\pi^{-1}(U))$
	\end{ex}

	\begin{proof}
		Suppose that for each $a,b \in X$ $a \sim b$ iff $f(a) = f(b)$. Let $\bar{y} \in Y$. Since $f$ is surjective there exists $x \in X$ such that $f(x) = y$. Then 
		\begin{align*}
			\bar{f}(\bar{x})
			& = \ol{f(x)} \\
			& = \bar{y}
		\end{align*}
		so that $\bar{f}$ is surjective. Let $\bar{a}, \bar{b} \in {X/ \sim}$. Then 
		\begin{align*}
			\bar{f}(\bar{a}) = \bar{f}(\bar{b})
			& \implies \ol{f(a)} = \ol{f(b)} \\
			& \implies f(a) = f(b) \\
			& \implies a \sim b \\
			& \implies \bar{a} = \bar{b}
		\end{align*}
		So that $\bar{f}$ is injective. Hence $\bar{f}$ is a bijection. \\
		Since $f$ is $(\MA, \MB)$-continuous and $\sim$-invariant, the previous exercise implies that $\bar{f}: {X / \sim} \rightarrow Y$ is $(\MT_{X/\sim}, \MB)$-continuous. \\
		Let $U \subset \MT_{X/\sim}$. Then $\pi^{-1}(U) \in \MA$. Since $f$ is open, 
		\begin{align*}
			\bar{f}(U) 
			& = f(\pi^{-1}(U)) \\
			& \in \MB
		\end{align*}
		Since $U \subset \MT_{X/\sim}$ is arbitrary, $\bar{f}$ is open. Therefore, $\bar{f}^{-1}$ is continuous. So $\bar{f}$ is a homeomorphism.
	\end{proof}
