...
\end{tikzcd}
	...
	\begin{defn}
	Let $X$ and $Y$ be Banach spaces. For $x \in X$ and $y \in Y$, define $x \otimes y \in L(L^2(X,Y;\C)^*, \C)$ by $x \otimes y( T) = T(x,y)$. Set $X \otimes^{\text{alg}} Y = \spn \{ x \otimes y: x \in X \text{ and } y \in Y \}$. 
	\end{defn}
	
	\begin{ex}
	Let $X$ and $Y$ be Banach spaces. Then the map $\otimes : X \times Y \rightarrow X \otimes^{\text{alg}} Y$ is bilinear.
	\end{ex}
