...
\end{proof}
	...
	\tcr{(try to move subspace material to subspace subsection)}
	
	\begin{defn} \ld{def:topology:compactness:subspaces:00001}
		\tcr{(maybe move to countability section since Lindeloff uses the concept of open cover)}. Let $(X, \MT)$ be a topological space $E \subset X$ and $\MU \subset \MP(X)$. Then $\MU$ is said to be an \tbf{open cover} of $E$ in $(X, \MT)$ if 
		\begin{enumerate}
			\item $\MU \subset \MT$ 
...
\begin{proof}\
	\begin{itemize}
		\item For each $x \in X$, define $E_x \subset S^{\N}$ by $E_x \defeq \{(x_n)_{n \in \N} \subset S: x_n \rightarrow x \text{ and $f_0(x_n)$ converges}\}$. Let $x \in X$. Since $S$ is dense in $X$, there exists $(x_n)_{n \in \N} \subset S$ such that $x_n \rightarrow x$. Then $(x_n)_{n \in \N}$ is Cauchy in $(X, d_X)$ \tcr{(make exercise)}. \rex{ex:metric_spaces:introduction:0001.1} implies that $(f_0(x_n))_{n \in \N}$ is Cauchy in $(Y, d_Y)$. Since $(Y, d_Y)$ is complete, there exists $y_x \in Y$ such that $f_0(x_n) \rightarrow y_x$. Thus $E_x \neq \varnothing$. Since $x \in X$ is arbitrary, we have that for each $x \in X$, $E_x \neq \varnothing$. The axiom of choice implies that there exists $\phi \in \prod\limits_{x \in X} E_x$. Define $f: X \rightarrow Y$ by $f(x) \defeq \limn f_0(\phi(x)_n)$.  \tcr{FINISH!!!}
		\item Let $\ep > 0$. Since $f_0$ is uniformly continuous, there exists $\del_0 > 0$ such that for each $a,b \in S$, $d_X(a,b) < \del_0$ implies $d_Y(f_0(a), f_0(b)) < \ep/3$. Define $\del > 0$ by $\del \defeq \del_0/3$. Let $a,b \in X$. Suppose that $d_X(a,b) < \del$. Since $\phi(a)_n \rightarrow a$, $\phi(b)_n \rightarrow b$, there exist $M_a, M_b \in \N$ such that for each $n \in \N$, $n \geq M_a$ implies that $d_X(\phi(a)_n, a) < \del$ and $n \geq M_b$ implies that $d_X(\phi(b)_n, b) < \del$. Define $M \in \N$ by $M \defeq \max \{M_a, M_b\}$. Since $d_X(a,b) < \del$, for each $n \in \N$, $n \geq M$ implies that
		\begin{align*}
			...
			& = \del_0. 
		\end{align*}
		Since $f_0(\phi(a)_n) \rightarrow f(a)$ and $f_0(\phi(b)_n) \rightarrow f(b)$, there exist $N_a, N_b \in \N$ such that for each $n \in \N$, $n \geq N_1$ implies that $d_Y(f_0(\phi(a)_n), f(a)) < \ep/3$ and $n \geq N_2$ implies that $d_Y(f_0(\phi(b)_n), f(b)) < \ep/3$. Define $N \in \N$ by $N \defeq \max \{M, N_a, N_b\}$. Then for each $n \in \N$, $n \geq N$ implies that
		\begin{align*}
			d_Y(f(a), f(b))
			& \leq d_Y(f(a), f_0(\phi(a)_n)) + d_Y(f_0(\phi(a)_n), f_0(\phi(b)_n)) + d_Y(f(a), f_0(\phi(a)_n)) \\
		...
		\item 
		\begin{itemize}
			\item Let $x \in S$,  $(a_n)_{n \in \N}$ and $(b_n)_{n \in \N} \in E_x$. By definition, $(a_n)_{n \in \N}, (b_n)_{n \in \N} \subset S$, $a_n, b_n \rightarrow x$ and there exists $y_a, y_b \in Y$ such that $f_0(a_n) \rightarrow y_a$ and $f_0(b_n) \rightarrow y_b$. Let $\ep > 0$. Since $f_0$ is uniformly continuous, there exists $\del > 0$ such that for each $a,b \in S$, $d_X(a,b) < \del$ implies $d_Y(f_0(a), f_0(b)) < \ep/3$. Since $f_0(a_n) \rightarrow y_a$ and $f_0(b_n) \rightarrow y_b$, there exist $N_a, N_b \in \N$ such that for each $n \in \N$, $n \geq N_1$ implies that $d_Y(f_0(a_n), y_a) < \ep/3$ and $n \geq N_2$ implies that $d_Y(f_0(b_n), y_b) < \ep/3$. Since $a_n \rightarrow x$ and $b_n \rightarrow x$, we have that 
			\begin{align*}
				d_X(a_n, b_n) 
				& \leq d_X(a_n, x) + d_X(x, b_n) \\
				& \rightarrow 0.
			\end{align*}
			Thus there exists $N_0 \in \N$ such that for each $n \in \N$, $n \geq N_0$ implies that $d_X(a_n, b_n) < \del$. Set $N \defeq \max \{N_0, N_1, N_2\}$. Then for each $n \in \N$, $n \geq N$ implies that
			\begin{align*}
				d_Y(y_a, y_b)
		...
		\end{itemize}
		\item Let $g:X \rightarrow Y$. Suppose that $g$ is uniformly continuous and $g|S = f_0$. Let $x \in X$. Since $E_x \neq \varnothing$, there exists $(x_n)_{n \in \N} \in E_x$. Then from above, we have that
		\begin{align*}
			f(x)
			& = \limn f_0(\phi(x)_n) \\
			& = \limn f(x_n) \\
	...
	\end{itemize}
	\tcr{Try to rephrase using quotient map on set of cauchy sequences in $X$}
\end{proof}
