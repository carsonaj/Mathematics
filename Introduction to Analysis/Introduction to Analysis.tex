...
\begin{defn}
	Let $(J, \leq)$ be a directed poset $((X_j)_{j \in J}, (\pi_{j,k})_{(j,k) \in \leq})$ be a projective system of topological spaces. Set $X \defeq \varprojlim\limits_{j \in J} X_j$. For each $j \in J$, we define the \tbf{$j$-th projective equivalence relation on $X$}, denoted ${\sim_j} \subset X \times X$, by $x \sim_j y$ iff $\pi_j(x) = \pi_j(y)$ and we denote the projection of $X$ onto $X/{\sim_j}$ by $\pi^Q_j: X \rightarrow X/{\sim_j}$.
\end{defn}
...
\begin{proof}\
	\begin{enumerate}
		\item Let $j \in J$. By construction, $\pi_j$ is continuous. By assumption, $\pi_j$ is surjective. Since for each $j' \in J$, $X_{j'}$ is compact Hausdorff, \rex{ex:topology:proj_limits:0005} implies that $\pi_j$ is closed. \rex{ex:quotient_topology:0007} implies that $\pi_j$ is a quotient map. \rex{ex:quotient_topology:0013.2} then implies that there exists a unique $\bar{\pi}:X/{\sim_j} \rightarrow X_j$ such that $\pi_j = \bar{\pi}_j \circ \pi^Q_j$ and $\bar{\pi}_j$ is a homeomorphism.
		\item Let $(j,k) \in {\leq}$. \tcr{looke here} \rex{ex:quotient_topology:0013.3} Define $\pi_{j,k}^Q : X/{\sim_k} \rightarrow X/{\sim_j}$ by $\pi_{j,k}^Q \defeq \bar{\pi}_j^{-1} \circ \pi_{j,k} \circ \bar{\pi}_k$. Since $\bar{\pi}_j^{-1}$, $\pi_{j,k}$, $\bar{\pi}_k$ are continuous, we have that $\pi_{j,k}^Q$ is continuous. By construction,
		\begin{align*}
			\pi^Q_{j,k} \circ \pi^Q_k
			& = (\bar{\pi}_j^{-1} \circ \pi_{j,k} \circ \bar{\pi}_k) \circ \pi^Q_k
		\end{align*}
	\end{enumerate}
\end{proof}
...
\begin{ex}
	For each $j \in J$, define $C_j \subset C(X)$ by 
	$$C_j \defeq \{f \in C(X): \text{ for each $x,y \in X$, $\pi_j(x) = \pi_j(y)$ implies that $f(x) = f(y)$}\}$$
	Define $C \subset C(X)$ by $C \defeq \bigcup\limits_{j \in J} C_j$. Then 
	\begin{enumerate}
