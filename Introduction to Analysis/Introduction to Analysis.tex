...
\newpage
	...
	\end{defn}	
		...
		\begin{proof}
			Suppose that $x,y$ are invertible. Then 
			\begin{align*}
				(y^{-1}x^{-1})(xy)
				& = y^{-1}(x^{-1} (x y)) \\
				& = y^{-1}((x^{-1} x) y) \\
				...
				& = e
			\end{align*}
			Similarly, $(xy)(y^{-1}x^{-1}) = e$. Hence $xy$ is invertible and $(xy)^{-1} = y^{-1}x^{-1}$.
		\end{proof}
	...
	\begin{ex} \lex{}
		...
		\begin{enumerate}
			\item For each $x \in X$, if $\|e- x \|< 1$, then $x \in G(X)$ and $$x^{-1} = \sum_{n=0}^{\infty}(e-x)^n$$
			\item For each $x,y \in X$, if $x \in G(X)$  and $\|x-y \|< \|x^{-1} \|^{-1}$, then $y \in G(X)$ and $$y^{-1} = \sum_{n=1}^{\infty} (e - x^{-1}yx^{-1})$$. 
			\item $G(X)$ is open.
		\end{enumerate}
	\end{ex}
	...
	\begin{proof}\
		\begin{enumerate}
			\item Let $x \in X$. Suppose that $\|e-x \|< 1$. Then $$\sum_{n=0}^{\infty} \|(e -x)^n \| \leq \sum_{n=0}^{\infty} \|e -x \|^{n} < \infty$$ Since $X$ is a complete, $\sum\limits_{n=0}^{\infty}(e-x)^n$ converges in $X$.\\
			Define $(s_k)_{k=0}^{\infty} \subset X$ and $s \in X$ by $s_k = \sum\limits_{n=0}^{k} (e-x)^n$ and \\ $s = \sum\limits_{n=0}^{\infty}(e-x)^n$. Then for each $k \in \N$,
			\begin{align*}
				s_k x
				&= s_k - s_k(e-x) \\
				&= (e-x)^0 - (e-x)^{k+1} \\
				&= e - (e-x)^{k+1}
			\end{align*}
			Therefore $\|s_k x - e \|\leq \|e-x \|^{k+1}$ and $s_kx \rightarrow e$ as $k \rightarrow \infty$. Since multiplication on Banach algebras is continuous, we have that $s_kx \rightarrow sx$ as $k \rightarrow \infty$. Uniqueness of limits implies that $sx = e$. Similarly $x s = e$. Thus $x \in G(X)$ and $x^{-1} = s$. \vspace{.5cm}\\
			\item  Let $x, y \in X$. Suppose that $x \in G(X)$ and $\|x-y \|< \|x^{-1} \|^{-1}$. Then 
			\begin{align*}
				\|e - x^{-1}y \|
				& = \|x^{-1}(x - y) \|\\
				& \leq \|x^{-1} \|\|x -y \|\\
				&< 1
			\end{align*}
			Hence $x^{-1}y \in G(X)$ and $y = x (x^{-1}y) \in G(X)$. \vspace{.5cm}\\
			\item Let $x \in G(X)$. Choose $\del = \|x^{-1}\|^{-1}$. By (2), $B(x, \del) \subset G(X)$.
		\end{enumerate}
	\end{proof}	
	...
	\begin{defn}
		Let $X$ be a unital Banach algebra and $x \in X$.
		\begin{itemize}
			\item We define the \textbf{resolvent} of $x$, denoted $\rho(x)$, by 
			$$\rho(x) = \{\lam \in \C: \lam e - x \in G(X)\}$$
			\item We define the \textbf{spectrum} of $x$, denoted $\sig(x)$, by 
			$$\sig(x) = \rho(x)^c$$ 
		\end{itemize}
	\end{defn}
