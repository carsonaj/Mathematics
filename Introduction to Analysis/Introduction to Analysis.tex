...
So $$\int_{(x_0, x_0 + h]}f - f(x_0) dm = o(\|h\|) \hspace{1cm}\text{ as }h \rightarrow 0$$ 
	...
	\begin{ex}
	Let $X$ be a Banach space, $A \subset X$ open and convex, and $f: A \rightarrow \R$. Suppose that $f$ is Frechet differentiable. Then $f$ is convex (resp. strictly convex) iff for each $x \in A$, $f(y) \geq f(x) + Df(x)(y-x)$  (resp. $>$).
	\end{ex}	
	...
	\begin{proof}
	Suppose that for each $x \in A$, $D^2f(x)$ is positive semidefinite. Let $x, y \in A$. Put $h = y-x$. Taylor's theorem implies that 
	\begin{align*}
	f(y) - f(x)
	&= f(x+h) - f(x) \\ 
	&= Df(x)(h) + \int_{(0,1)} (1-t)D^2f(x + th)(h, h)d m(t) \\
	& \geq Df(x)(h) \\
	&= Df(x)(y-x)  
	\end{align*}
	So $f$ is convex. Conversely, suppose that $f$ is convex.
	\end{proof}
