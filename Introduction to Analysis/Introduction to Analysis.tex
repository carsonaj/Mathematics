...
\end{align*}	 
	...
	\begin{note}
	Recall that for Banach spaces $X$ and $Y$, there isomorphic isometry $L(X, L(X, \cdots, L(X, Y)) \cdots) \rightarrow L^n(\bigoplus\limits_{i=1}^n X, Y)$ given by $\phi \mapsto \psi_{\phi}$ where $$\psi_{\phi}(x_1, x_2, \cdots, x_n) = \phi(x_1)(x_2),\cdots,(x_n)$$
	\end{note}	
	...
	Let $x_0 \in A$ and $n \geq 2$.  Then $f$ is said to be \textbf{$n$-th order Frechet differentiable at $x_0$} if $f$ is $(n-1)$-th order Frechet differentiable and $D^{n-1}f$ is Frechet differentiable at $x_0$. If $f$ is $n$-th order Frechet differentiable at $x_0$, we define $D^nf(x_0) \in L^n(\bigoplus\limits_{i=1}^n X, Y)$ by 
	$D^nf(x_0) = D[D^{n-1}f](x_0)$. Finally, $f$ is said to be \textbf{$n$-th order Frechet differentiable} if $f$ is $(n-1)$-th order Frechet differentiable and for each $x \in A$, $D^{n-1}f$ is Frechet differentiable at $x$. If $f$ is $n$-th order Frechet differentiable, we define the \textbf{$n$-th order Frechet derivative of $f$}, denoted $D^nf:A \rightarrow  L^n(\bigoplus\limits_{i=1}^n X, Y)$ by $D^nf = D[D^{n-1}f]$.
	\end{defn}	
	
	\begin{ex}
		Let $X, Y$ be a Banach spaces, $A \subset X$ open, $f:A \rightarrow Y$, $x_0 \in A$ and $n \in \N$. Then $f$ is $n$-th  order Frechet differentiable at $x_0$ iff for each $i \in \{1, \cdots, n\}$, there exists $\phi_i \in L^i(\bigoplus\limits_{j=1}^i X, Y)$ such that $$f(x + h) = \sum_{i=1}^n \phi_i(h, \cdots, h) + o(\|h\|^n)$$
	\end{ex}
