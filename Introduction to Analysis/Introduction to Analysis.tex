...
Then $V_{\ep}^o \in \MN_{x_0}$ is open and 
	...
	\begin{defn}
	Let $X$ be a vector space and $p:X \rightarrow \R$ a sublinear functional. Then $p$ is said to be \textbf{bounded} if there exists $M >0$ such that for each $x \in X$, $p(x) \leq M\|x\|$. 
	\end{defn}
	
	\begin{ex}
	Let $X$ be a vector space and $p:X \rightarrow \R$ a sublinear functional. Then $p$ is bounded iff $p$ is Lipschitz. 
	\end{ex}
	...
	\begin{ex}
	Let $X$ be a vector space and $p:X \rightarrow \R$ a sublinear functional. Then there exists a linear function $F: X \rightarrow \R$ such that for each $x \in X$, $F(x) \leq p(x)$.
	\end{ex}
	...
	\begin{ex}
	Let $X$ be a Banach space and $p:X \rightarrow \R$ a bounded sublinear functional. Then there exists $\phi \in X^*$ such that for each $x \in X$, $\phi(x) \leq p(x)$.
	\end{ex}
	
	\begin{proof}
	The previous exercises imply that $p$ is Lipschitz and that there exists $phi: X \rightarrow \R$ such that $\phi$ is linear and $\phi \leq p$. Since $p$ is Lipschitz, there exists $M >0$ such that for each $x \in X$, $|p(x)| \leq M \|x\|$. Let $x \in X$. Then 
	\begin{align*}
	\phi(x) 
	&\leq p(x) \\
	&\leq |p(x)| \\
	...
	\begin{defn}
	Let $X,Y$ be a Banach spaces, $A \subset X$ open, $f:A \rightarrow Y$ and $x_0 \in A$. Then $f$ is said to be 
	\begin{enumerate}
	...
	\begin{defn}
	Let $X, Y$ be a banach spaces, $A \subset X$ open, $f:A \rightarrow Y$ and $x \in A$. Then $f$ is said to be \textbf{Frechet differentiable} at $x$ if there exists $\phi \in L(X,Y)$ such that, $$f(x + h) = f(x) + \phi(h) + o(h ) \hspace{.5cm} \text{ as } h \rightarrow 0$$  
	If $f$ is Frechet differentiable at $x$, we define the \textbf{Frechet derivative} of $f$ at $x$, denoted by $Df(x): X \rightarrow Y$, to be $Df(x) = \phi$.
	\end{defn}
	
	\begin{ex}
	Let $X,Y$ be a banach spaces and $\phi \in L(X,Y)$. If $\phi(h) = o(h)$, then $\phi = 0$. 
	\end{ex}
	...
	\begin{ex}
	Let $X, Y$ be a banach spaces, $A \subset X$ open, $f:A \rightarrow Y$ and $x \in A$. If $f$ is Frechet differentiable at $x$, then its Frechet derivative is unique. 
	\end{ex}
	
	\begin{proof}
	Suppose that there exist $\phi, \psi \in L(X,Y)$ such that $$f(x + h) = f(x) + \phi(h) + o(h ) \hspace{.5cm} \text{ as } h \rightarrow 0$$ 
	and 
	$$f(x + h) = f(x) + \psi(h) + o(h ) \hspace{.5cm} \text{ as } h \rightarrow 0$$ 
	Then $\phi(h) - \psi(h) = o(h)$. Since $\phi - \psi \in L(X,Y)$, the previous exercise implies that $\phi = \psi$.
	\end{proof}
...
\begin{defn}
	Let $x_1, x_2 \in H$ and $S \subset H$. Then $x_1$ and $x_2$ are said to be \textbf{orthogonal} if $\l x_1,x_2\r = 0$ and $S$ is said to be \textbf{orthogonal} if for each $x_1,x_2 \in S$, $x_1,x_2$ are orthogonal. 
\end{defn}
...
%
%\begin{proof}
%	Let $x_1, \cdots, \psi_n \in S$ and $c_1, \cdots, c_n \in \C$. Suppose that $\sum\limits_{j =1}^n c_j \psi_j = 0 $. Then 
%	\begin{align*}
%		0
...
%	Let $x_1,x_2 \in H$. Then $\l x_1,x_2\r^2 \leq \l x_1,x_1 \r \l x_2,x_2 \r$
%\end{thm}
%
%\begin{ex}
%	Define $\|\cdot \| : H \rightarrow \R_{\geq 0}$ by $$\|x_1\| = \l x_1, x_1\r^{1/2}$$ Then $\|\cdot\|$ is a norm on $H$.
%\end{ex}
%
%\begin{proof}
...
%		\| x_1+ x_2 \|^2 
%		&= \l x_1+x_2 , x_1+x_2\r \\
%		&= \l x_1,x_1 \r + \l x_1,x_2\r + \l x_2,x_1 \r + \l x_2,x_2 \r \\
%		&= \|x_1\| + \| x_2 \| + 2 Re(\l x_1,x_2 \r) \\
%		& \leq \|x_1\| + \| x_2 \| + 2| \l x_1 , x_2 \r| \\
%		& \leq  \|x_1\| + \| x_2 \| + 2 \| x_1\| \|x_2\| \\ 
%		&= (\|x_1 \| + \| x_2 \|)^2
%	\end{align*}
%	Hence $$\| x_1+ x_2 \| \leq \|x_1 \| + \| x_2 \|$$
%\end{proof}
%
%\begin{defn}
%	S set $S \subset H$ is said to be \textbf{orthonormal} if $S$ is orthogonal and for each $x_1 \in S$, $\|x_1 \| = 1$.
%\end{defn}
%
%\begin{defn}
...
%	Let $A,B$ be operators on $H$. Then $B$ is said to be the \textbf{adjoint} of $A$ if for each $x_1$, $x_2 \in H$, $$\l x_1 , Ax_2 \r = \l B x_1 , x_2 \r$$  If B is the adjoint of $A$, we write $$B = A^{*}$$
%\end{defn}
%
%\begin{note}
...
\end{align*}	
	...
	\begin{ex}
	Let $X$ be a Banach space, $A \subset X$ open and convex, $f:A \rightarrow \R$ convex and $x_0 \in A$. If $f$ is continuous at $x_0$, then $f$ is locally Lipschitz at $x_0$. (\textbf{Hint:} Given $x_1, x_2$ near $x_0$ Choose a $z$ near $x_0$ s.t. $x_1$ is a convex combination of $x_2$ and $z$. Then repeat but with $x_2$ as a convex combination of $x_1$ and $z$)
	\end{ex}
...
\end{enumerate}	 
	...
	\begin{ex}
	Let $X$ be a Banach space, $A \subset X$ open and convex, $f:A \rightarrow \R$ convex and $x_0 \in A$. If $f$ is continuous at $x_0$, then there exists $\phi \in X^*$ such that $\phi \leq df(x_0)$.
	\end{ex}	
	...
	\begin{proof}
	Suppose that $f$ is continuous at $x_0$. The previous exercise tells us that there exists $\phi \in X^*$ such that $\phi \leq d^+f(x_0)$. Let $x \in X$. Define $T = \{t \in \R: x_0 + t(x-x_0) \in A\}$ similarly to earlier. Clearly $1 \in T$ and  
	\begin{align*}
	\phi(x-x_0) 
	& \leq d^+f(x_0)(x - x_0) \\
