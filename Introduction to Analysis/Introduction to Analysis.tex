...
Then $V_{\ep}^o \in \MN_{x_0}$ is open and 
	...
	\begin{thm}\textbf{Hahn-Banach Theorem:}
		Let $X$ be a vector space, $p:X \rightarrow \R$ a sublinear functional, $M \subset X$ a subspace and $f:M \rightarrow \C$ a linear functional. If for each $x \in M$, $ f(x)  \leq p(x)$, then there exists a linear functional $F:X \rightarrow \C$ such that for each $x \in X$, $F(x) \leq p(x)$ and $F|_{M}=f$.
	\end{thm}
	
	\begin{ex}
	Let $X$ be a vector space, $p:X \rightarrow \R$ a sublinear functional. Then there exists a linear function $F: X \rightarrow \C$ such that for each $x \in X$, $F(x) \leq p(x)$.
	\end{ex}
	...
	\begin{ex}
	Let $X$ be a Banach space, $A \subset X$ open, $f:A \rightarrow \R$ convex, $a \in A$ and $b \in X$. Define $T = \{ t \in \R: a+tb \in A\}$. Then $T$ is an open neighborhood of $0$. 
	\end{ex}
	
	\begin{proof}
	Continuity of scalar multiplication and addition implies that $T$ is an open neighborhood of $0$.
	\end{proof}
	
	\begin{defn}
	Let $X$ be a banach space, $A \subset X$ open, $f:A \rightarrow \R$, $a \in A, b \in X$. Define $T = \{ t \in \R: a+tb \in A\}$, which is an open neighborhood of $0$, and $g: T \rightarrow \R$ by $g(t) = f(a +tb)$. Then $f$ is said to be 
	\begin{enumerate}
	...
	exists. If $f$ is right-hand-differentiable at $a$ in the direction $b$, we define the \textbf{right-hand derivative} of $f$ at $a$ in the direction $b$ by $$d^+ f(a; b) = g'_+(0)$$ 
	
	\item \textbf{left-hand-differentiable} at $a$ in the direction $b$ if $g$ is differentiable from the left at $t=0$, i.e. if 
	$$  g'_-(0) =  \lim_{t \rightarrow 0^-} \frac{f(a +tb) - f(a)}{t}$$
	exists. If $f$ is left-hand-differentiable at $a$ in the direction $b$, we define the \textbf{left-hand derivative} of $f$ at $a$ in the direction $b$ by $$d^- f(a; b) = g'_-(0)$$
	...
	\begin{enumerate}
	\item \textbf{right-hand Gateaux differentiable} at $a$ if for each $b \in X$, $d^+ f(a; b)$ exits. We define the \textbf{right-hand Gateaux derivative} of $f$ at $a$, denoted $d^+ f(a) : X \rightarrow \R$, to be $$d^+ f(a)(b) = d^+ f(a;b)$$ 
	
	\item \textbf{left-hand Gateaux differentiable} at $a$ if for each $b \in X$, $d^- f(a; b)$ exits. We define the \textbf{left-hand Gateaux derivative} of $f$ at $a$, denoted $d^- f(a) : X \rightarrow \R$, to be $$d^- f(a)(b) = d^- f(a;b)$$
	...
	\begin{ex}
	Let $(x_j)_{j =1}^n \subset H$. Then $$\bigg \l \sum_{i=1}^n x_i , \sum_{j=1}^n x_j \bigg \r = \sum_{i=1}^n \sum_{j=1}^n \l x_i , x_j \r $$
\end{ex}
...
%
%\begin{defn}
%	Let $H$ be a vector space and $\l \cdot , \cdot\r: H \times H \rightarrow \C$. Then $H$ is said to be a \textbf{Hilbert space} if $H$ is a complete with respect to the norm on $H$ induced by $\l \cdot , \cdot \r$. 
%\end{defn}
%
%\begin{note}
...
%	Let $A,B$ be operators on $H$. Then $B$ is said to be the \textbf{adjoint} of $A$ if for each $\psi_1$, $\psi_2 \in H$, $$\l \psi_1 , A\psi_2 \r = \l B \psi_1 , \psi_2 \r$$  If B is the adjoint of $A$, we write $$B = A^{*}$$
%\end{defn}
%
%\begin{note}
%	In mathematics, the adjoint of $A$ is typically denoted by $A^*$, but we will adopt the notation of $A^{*}$ from physics.
%\end{note}
%
%\begin{ex}
%	Let $A$ and $B$ be operators on $H$ and $\lam \in \C$, then \begin{enumerate}
%		\item $(A^{*})^{*} = A$
%		\item $(A + B)^{*} = A^{*} + B^{*}$
%		\item $(AB)^{*} = B^{*}A^{*}$
%		\item $(\lam A)^{*} = \lam^*A^{*}$
...
%\end{ex}
%
%\begin{proof} Let $\psi_1$, $\psi_2 \in H$. Then
%	\begin{enumerate}
%		\item 
%		\begin{align*}
%			\l A \psi_1 , \psi_2 \r
%			&= \l \psi_2 , A \psi_1 \r^*\\
%			&= \l A^{*}\psi_2 ,  \psi_1 \r^* \hspace{.5cm} \text{(by definition)}\\
%			&= \l  \psi_1 , A^{*}\psi_2 \r
%		\end{align*}
%		\item 
%		\begin{align*}
...
%			&= \l A^{*} \psi_1 , \psi_2 \r + \l B^{*} \psi_1 , \psi_2 \r \\
%			&= \l (A^{*} + B^{*})  \psi_1 , \psi_2 \r  \\
%		\end{align*}
%		\item 
%		\begin{align*}
...
%			&= \lam \l A^{*}\psi_1 , \psi_2 \r \\
%			&= \l \lam^* A^{*} \psi_1 , \psi_2 \r 
%		\end{align*}
...
%
%\begin{defn}
%	An linear operator $Q$ on $H$ is said to be \textbf{self-adjoint} if $$Q = Q^{*}$$
%\end{defn}
%
%\begin{ex}
...
%
%\begin{ex}
%	Let $A, B$ be self-adjoint operators on $H$ and $ \lam \in \R$. If $A$ and $B$ commute and then $\lam AB$ is self-adjoint.
%\end{ex}
%
%\begin{proof}
%	\begin{align*}
...
%	Let $\psi \in H$. We define the \textbf{adjoint} of $\psi$, denoted $\psi^{*}: H \rightarrow \C$, by $\psi^{*} \phi = \l \psi, \phi\r$. 
%\end{defn}
%
%\begin{note}
%	In mathematics, where linearity of the inner product is in the first argument, $\psi^{*}$ is typically referred to by $u_{\psi} \in H^{*} $ where $u_{\psi}(\phi) = \l \phi, \psi\r$. In physics, where the inner product with linearity in the second argument, $\psi^{*} \phi$ is usually written in the so-called ``bra-ket" notation as $\l \psi | \phi \r$ which works smoothly since it aligns with the linearity of $u_{\psi}(\phi_1 + \lam \phi_2)$ and the conjugate-linearity of $u_{\psi_1 + \lam \psi_2}(\phi)$. In this way, it generalizes the notation for $\l x, y\r = x^T y$ for $\R^n$ to $\l x, y\r = x^*y^*$ for $\C^n$. 
%\end{note}
%
%\begin{ex}
%	Let $\psi, \phi \in \H$ and $ \lam \in \C$. Then 
%	\begin{enumerate}
%		\item $(\psi + \phi){^*} =  \psi^*+ \phi^*$
%		\item $(\lam \psi)^* = \lam^* \psi^*$
%	\end{enumerate}
%\end{ex}
%
%\begin{proof}
...
%
%\begin{defn}
%	Let $\psi, \phi \in H$ and $A$ an operator on $H$. We define 
%	\begin{enumerate}
%		\item the linear functional on $H$, $\psi ^*A: H \rightarrow \C $ by $(\psi ^*A) \al = \psi ^*(A \al)$
%		\item the operator on $H$, $\psi \phi ^*: H \rightarrow H$ by $(\psi \phi \da) \al = (\phi^*\al) \psi$
%	\end{enumerate}
%\end{defn}
%
%\begin{ex}
%	Let $A$ be an operator on $H$ and $\psi \in H$. Then $$(A \psi)^*= \psi ^*A^*$$
%\end{ex}
%
%\begin{proof}
%	Let $\phi \in H$. Then 
...
%	Let $H_1, H_2$ be Hilbert spaces. Define $$\otimes: H_1 \times H_2 \rightarrow \C^{H_1 \times H_2} ,\hspace{.5cm} (\psi, \phi) \mapsto \psi \otimes \phi$$ by $$\psi \otimes \phi(x,y) = \l x, \psi  \r \l y, \phi\r$$ 
%\end{defn}
%
%\begin{note}
...
%
%\begin{defn}
%	Define $T(H_1, H_2) = \spn \{\psi \otimes \phi: \psi \in H_1 \text{ and } \phi \in H_2\}$ and define $\l \cdot, \cdot \r : T(H_1, H_2) \rightarrow \C $ by $$\l \psi_1 \otimes \phi_1 , \psi_2 \otimes \phi_2 \r = \l \psi_1, \psi_2 \r \l \phi_1 ,  \phi_1\r$$ and extending sesquilinearly so that $$ \l \sum_{i=1}^m \al_i \psi_i \otimes \phi_i , \sum_{j=1}^n \beta_j \Xi_j \otimes \Gam_j\r = \sum_{i=1}^m \sum_{j=1}^n \al_i^* \beta_j \l \psi_i \otimes \phi_i,  \Xi_j \otimes \Gam_j \r$$
%\end{defn}
%
%\begin{ex}
...
%		\end{align*}
%		we have that $(\psi_i \otimes \phi_j)_{i,j \in \N}$ is orthonormal. Let $\psi = \sum\limits_{i \in \N} a_i \psi_i \in H_1$ and $\phi = \sum\limits_{j \in \N} b_j \phi_j \in H_2$. Then $\phi \otimes \psi = \sum\limits_{i \in \N} \sum\limits_{j \in \N} a_ib_j \psi_i \otimes \phi_j$, so $(\psi_i \otimes \phi_j)_{i,j \in \N}$ is a dense subset of $T(H_1, H_2)$, which is dense in $H_1 \otimes H_2$. Hence $(\psi_i \otimes \phi_j)_{i,j \in \N}$ is dense in $H_1 \otimes H_2$ and is a basis.
%	\end{proof}
%
%	\begin{note}
...
%
%	\begin{defn}
%		Let $A$ and $B$ be operators on $H_1$ and $H_2$ respectively. We define the operator $A \otimes B$ on $H_1 \otimes H_2$ by setting $$A \otimes B (\psi \otimes \phi) = A \psi \otimes B \phi$$ and extending linearly to $T(H_1, H_2)$ and then extending continuously to $H_1 \otimes H_2$
%	\end{defn}
...
\end{enumerate}	 
	...
	\begin{ex}
	Let $X$ be a Banach space, $A \subset X$ open and convex, $f:A \rightarrow \R$ convex, $x_0 \in A$ and $x \in X$. Define $t_0$ as before. Then for each $t \in (0, t_0)$, $$q(-t) \leq q(t)$$ (\textbf{Hint:} for sufficiently small $t$, convexity of $f$ implies that $f(x_0) \leq \frac{1}{2} f(x_0 - 2tx) + \frac{1}{2} f(x_0 + 2tx)$)
	\end{ex}
	...
	\begin{defn}
	Let $X$ be a Banach space, $A \subset X$ open and convex, $f:A \rightarrow \R$ convex and $x_0 \in A$. We define the \textbf{subdifferential of $f$ at $x_0$}, denoted $\p f(x_0)$, to be $$\p f(x_0) = \{ \phi \in X^*: \text{for each } x \in X, f(x_0) + \phi(x-x_0) \leq f(x)\}$$
	\end{defn}
	...
	\begin{proof}
	Let $x_0 \in A$. Note that The Hahn banach theorem implies that there exists $\phi \in X^*$ such that $\phi \leq d^+(x_0)$
	\end{proof}
