...
\end{proof}
	...
	\begin{defn} \ld{def:topology:compactness:00002}
		Let $(X, \MT)$ be a topological space. Then $(X, \MT)$ is sais to be \tbf{compact} if for each $\MU \subset \MP(X)$, $\MU$ is an open cover of $X$ in $(X, \MT)$ implies that there exists $\MU_0 \subset \MU$ such that $\MU_0$ is an open cover of $X$ in $(X, \MT)$ and $\MU_0$ is finite.
	\end{defn}
