...
\end{align*}	 
	...
	\begin{ex}
	Let $X,Y$ be Banach spaces, $A \subset X$ open, $f \in C^n(A, Y), x_0 \in A$ and $h \in X$. Suppose $B(x_0, \del) \subset A$ and $\|h\|<\del$. Define $c:(0,1) \rightarrow X$ by $$c(t) = x_0 + th$$ and $g:(0,1) \rightarrow Y$ by $$g = f \circ c$$
	Then for each $k \in \{1 \dots, n\}$, $$g^{(k)}(t) = D^kf(x_0 + th)(h^{\oplus k})$$
	\end{ex}
...
So $$\int_{(x_0, x_0 + h]}f - f(x_0) dm = o(\|h\|) \hspace{1cm}\text{ as }h \rightarrow 0$$ 
	...
	Let $X$ be a Banach space, $Y$ a separable Banach space, $A \subset X$ open and convex, $f\in C^n(A, Y)$ and $x_0 \in A$. Then $$f(x_0 + h) = \sum_{k=0}^n \frac{1}{k!} D^k f(x_0)(h^{\oplus k}) + o(\|h\|^n) \hspace{1cm }\text{ as } h \rightarrow 0$$
	\textbf{Hint:} Define $g: (0,1) \rightarrow Y$ by $$g(t) = f(x_0 +t h)$$ Then use the previous two exercises and the fundamental theorem of calculus.
	\end{ex}
