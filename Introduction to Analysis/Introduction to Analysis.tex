...
\begin{document}
	...
	\begin{defn}
		Let $X, Y, Z$ be sets. We define the \tbf{currying operator}, denoted $\cur: Y^{X \times Y} \rightarrow (Z^Y)^X$, by 
...
\begin{ex} \tbf{Basis for Subspace Topology:} \\
...
\end{proof}
	...
	\begin{proof}
		Suppose that $f$ is a quotient map. Then $\MB = f_* \MA$. \tcb{An exercise in the section on continuous maps} implies that $g$ is continuous iff $g \circ f$ is continuous.
	\end{proof}
	...
	\begin{proof}\
		\begin{itemize}
			...
			\begin{enumerate}
				...
				\item for each $n \in \N$, $U_{x, n+1} \subset U_{x, n}$
			\end{enumerate}
			For the sake of contradiction, suppose that $f$ is not continuous. Then there exists $x \in X$ such that $f$ is not continuous at $x$. Thus there exists $V \in \MN_{f(x)}$ such that for each $U \in \MN_x$, $f(U) \not \subset V$.
			...
			Let $U \in \MN_x$. Since $(U_{x,n})_{n \in \N}$ is a local basis for $\MT$ at $x$, there exists $N \in \N$ such that $U_{x,N} \subset \Int U$. Then for each $n \in \N$, $n \geq N$ implies that 
			\begin{align*}
				x_n
				& \in U_{x,n} \\
				& \subset U_{x,N} \\
				...
				& \subset U
			\end{align*}
			Hence $(x_n)_{n \in \N}$ is eventually in $U$. Since $U \in \MN_x$ is arbitrary, $x_n \rightarrow x$. By assumption, $f(x_n) \rightarrow f(x)$. This is a contradiction since for each $n \in \N$, $f(x_n) \in V^c$ and therefore it is not the case that $(f(x_n))_{n \in \N}$ is eventually in $V$. Hence $f$ is continuous.  
		\end{itemize}
	\end{proof}
...
\begin{ex}
	\tcb{Show that the open balls form a basis for $\MT_d$}
\end{ex}
...
\begin{defn} \ld{}
	Let $X$ be a set, $d_1, d_2: X \times X \rightarrow \Rg$ metrics on $X$. Then $d_1$ and $d_2$ are said to be  \tbf{$\Top$-equivalent}, denoted $d_1 \sim_{\Top} d_2$, if for each $(x_n)_{n \in \N} \subset X$ and $x \in X$, $x_n \conv{d_1} x$ iff $x_n \conv{d_2} x$.	
\end{defn}	
...
\begin{defn}
	Let $(X, d)$ be a metric space and $\phi: \Rg \rightarrow \Rg$. Suppose that $\phi$ is said to be $\Top$ metric-preserving. We define the \tbf{$\phi$-iterate of $d$}, denoted $d_{\phi}$, by $d_{\phi} = \phi \circ d$.  
\end{defn}
...
\begin{defn}
	Let $(X_n, d_n)_{n \in \N}$ be a collection of metric spaces and $d$ a metric on $\prod_{n \in \N} X_n $. Then $d$ is said to be \tbf{product compatible} if there exists $f: \Rg^{\N} \rightarrow \Rg$ such that 
	\begin{itemize}
...
\tcb{want to categorize when a discrete valued metric is basically a tree. }
...
\end{proof}
	...
	\begin{proof}
		Define the action $\phi: M \times X \rightarrow X$ by $m \cdot x = x + m$. Then $M \cdot x = x+ M$. Since for each $m \in M$, the map $x \mapsto x +m$ is continuous, \tcb{ an exercise in the section on the quotient topology} implies that $\pi: X \rightarrow X/M$ is open.  
	\end{proof}
...
\end{proof}		
	...
	\begin{ex} \tbf{Uniform Boundedness Principle:} \\		
...
\begin{defn} \ld{}
	Let $(H, \l \cdot, \cdot \r)$ be an inner product space. Then $(H, \l \cdot, \cdot \r_H)$ is said to be a \tbf{Hilbert space} if $(H, \| \cdot \|)$ is a Banach space.
\end{defn}
...
\begin{ex} \tbf{Riesz Representation Theorem:} \\
...
\begin{note}
	\tcb{An exercise in introduction section} implies that $P_E$ is well-defined.
\end{note}
...
\begin{proof}\
	\begin{enumerate}
		...
		\item By definition, $\Im P_E \subset E$. The previous part implies that $E \subset \Im P_E$. Thus $\Im P_E = E$. 
		...
		So $\|P_E\| = 1$.
		\item \tcb{FINISH!!!}
		\item Since $P \in L(H)$, $P|_E = I_E$ and $\Im P \subset E$, \tcb{a previous exercise} implies that $P_E$ is idempotent.
	\end{enumerate}
\end{proof}
...
\begin{defn}
	Let $H \in \Obj(\Hilb)$, $E \subset H$ a closed subspace of $H$ and $P \in L(H, E)$. Then $P$ is said to be an \tbf{orthogonal projection onto $E$} if 
	\begin{enumerate}
		\item $P$ is self-adjoint
		\item $P^2 = P$
		\item $\Im P = E$ 
...
\begin{defn}
	Let $H \in \Obj(\Hilb)$, $E \subset H$ a closed subspace of $H$ and $P \in L(H, E)$. Then $P$ is said to be an \tbf{orthogonal projection onto $E$}, denoted $P_E \in L(H, E)$, by 
...
\end{enumerate}	 
	...
	\begin{defn} \ld{00000} 
		Let $G$ be a group. Define $\iota_{\mu}:G \rightarrow G$ by $\iota_{\mu}(x) = x^{-1}$. 
	\end{defn}
	
	\begin{ex} \lex{00000} 
		Let $G$ be a topological group. Then $\iota_{\mu} \in \Homeo(G)$.
	\end{ex}

	\begin{proof}
		By assumption $\iota_{\mu}$ is continuous. We know from basic group theory that $\iota_{\mu}$ is a bijection with $\iota_{\mu}^{-1} = \iota_{\mu}$. 
	\end{proof}

	\begin{defn} \ld{00000} 
		Let $G$ be a group and $S \subset G$, then $S$ is said to be \tbf{symmetric} if $\iota_{\mu}(S) = S$, ( i.e. $S^{-1} = S$).
	\end{defn}
	...
	\begin{ex} \lex{00000} 
	Let $G$ be a topological group. Then $\iota_{\mu} \in \Aut(G)$ iff $G$ is abelian. 
	\end{ex}
	
	\begin{proof}
	Basic group theory tells us that $\iota_{\mu}$ is a homomorphism iff $G$ is abelian.
	\end{proof}
	...
	\end{ex}
	\begin{proof}
		Let $U \subset G$ and $g \in G$. Suppose that $U$ is open. Since $l_g, r_g$ and $\iota_{\mu}$ are homeomorphisms, $l_g(U) = gU$, $r_g(U) = Ug$ and $\iota_{\mu}(U) = U^{-1}$ are open. 
	\end{proof}
