...
\end{proof}
	...
	\begin{ex}
		Let $(X, \sim_X)$, $(Y, \sim_Y)  \in \Obj(\TopEq)$,  $\gam_X \in \Hom_{\Top}(X/{\sim_X}, X), (Y, \sim_Y))$ and $\iota \in \Hom_{\TopEq}((X, \sim_X)$. Suppose that $\gam_X$ is a section of $(X, X/{\sim_X}, \pi_X)$. and $((X, \sim_X), \iota)$ is a contraction of $(Y, \sim_Y)$. Then $\pi_Y \circ \iota \circ \gam_X = \bar{\iota}$, i.e. the following diagram commutes: 
		\[ 
		\begin{tikzcd}
			X  \arrow[r, "\iota"]  \arrow[d, "\pi_X"']  & Y   \arrow[d, "\pi_Y"]\\
			\arrow[bend left=80]{u}{\gam_X} X/ {\sim_X}  \arrow[r, "\bar{\iota}"'] &  Y / {\sim_Y} \\
		\end{tikzcd}
		\]
	\end{ex}
...
\end{proof}		
	...
	\newpage
	\section{The Baire Category and Closed Graph Theorems}
...
\end{proof}
	...
	\begin{defn}
		Let $X$ be a unital Banach algebra and $x \in X$. We define the \tbf{spectral radius of $x$}, denoted by $r_(x)$, by $$r_(x) = \sup \{|\lam|: \lam \in \sig(x)\}$$
	\end{defn}

	\begin{ex}
		Let $X$ be a unital Banach algebra and $x \in X$ and $\lam \in \C$. If $\lam \in \sig(x)$, then $\lam^n \in \sig(x^n)$. \\
		\tcb{Hint:} $\lam^n e -x^n = (\lam e - x) \sum_{j=0}^{n-1} \lam^j x^{n-1 -j}$
	\end{ex}
