...
Then $V_{\ep}^o \in \MN_{x_0}$ is open and 
	...
	\begin{defn}
		Let $X$ be a vector space and $A \subset X$. Then $A$ is said to be
		\begin{itemize}
			\item \textbf{convex} if for each $x, y \in A$, $t \in [0,1]$, $tx +(1-t)y \in A$.
			\item \textbf{absorbing} if for each $x \in X$, there exists $r > 0$ such that for each $c \in \R$, $|c| \geq r$ implies that $x \in cA$.
			\item \textbf{balanced} if for each $x \in A$, $c \in \C$, $|c| \leq 1$ implies that $cx \in A$.
			\item an \textbf{absorbing disk} if $A$ is convex, absorbing and balanced.
		\end{itemize}
	\end{defn}
	...
	\begin{ex}
		Let $X$ be a vector space, $A \subset X$ and $K \in \C$. Suppose that $A$ is balanced. If $|K| \leq 1$, then $KA$ is balanced.
	\end{ex}

	\begin{proof}
		Suppose that $|K| \leq 1$.  Since $A$ is balanced, $KA \subset A$. Let $x \in KA$ and $c \in \C$. Then there exists $a \in A$ such that $x = Ka$. Suppose that $|c| \leq 1$. Then $cx = cKa$. Since $|cK| < 1$, $cx \in $
	\end{proof}
	...
	\begin{ex}
		Let $X$ be a vector space and $A \subset X$. Suppose that $A$ is an absorbing disk. Then 
		\begin{enumerate}
			\item $p_A:X \rightarrow \Rg$ 
			\item $p(0) = 0$
			\item $p_A$ is a seminorm on $X$
		\end{enumerate}
	\end{ex}
	
	\begin{proof} Let $x, y \in X$ and $\lam \in \C$. 
		\begin{enumerate}
			...
			\item \begin{itemize}
				...
				\begin{align*}
					...
					&= |\lam| p_A(x)
				\end{align*}
				Suppose that $\lam \neq 0$. For $t >0$, $\lam t^{-1} x \in A$ iff $|\lam| t^{-1} x \in A$. So
				\begin{align*}
					p_A(\lam x) 
					&= \inf \{t > 0: \lam x \in tA\} \\
	...
	\begin{proof}
		Let $x \in A$. Since $A$ is open, $A \in \MN_x$. Since $1 + 1/n \rightarrow 1$, $(1 + 1/n)x \rightarrow x$. Therefore, there exits $N \in \N$ such that for each $n \in \N$, $n \geq N$ implies that $(1 + 1/n)x \in A$. In particular, $x \in 1/ (1 + 1/N)A$. Hence $1/(1 + 1/N) \in T_x^A$ and
		\begin{align*}
			p_A(x)
			&= \leq 1/(1 + 1/N) \\
			& < 1
		\end{align*}
		So $x \in B_{p_A}(0,1)$ and $A \subset B_{p_A}(0,1)$. \\
		Conversely, let $x \in  B_{p_A}(0,1)$. Then $p_A(x) <1$. By definition, there exists $t \in (0,1)$ such that $x \in tA$. Since $A$ is balanced, $tA \subset A$. Hence $x \in A$. So $A = B_{p_A}(0,1)$.
	\end{proof}
