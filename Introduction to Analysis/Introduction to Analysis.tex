...
\end{proof}
	...
	\begin{proof}
		Set $\Gam = \{U \in \MN_0: U \text{ is open}\}$ and order $\Gam$ by reverse inclusion, so that $\Gam$ is a directed set. For the sake of contradiction, suppose that for each $U \in \Gam$, $(A + U) \cap B \neq \varnothing$. Then for each $\gam \in \Gam$, there exist $a_{\gam} \in A$ and $u_{\gam} \in \gam$ such that $a_{\gam} + u_{\gam} \in B$. Since $A$ is compact, there exists $a \in A$ and a subnet $(a_{\gam_{\ze}})$
	\end{proof}
	...
	\begin{ex} \textbf{Hahn-Banach Separation Theorem 1:} \\
		...
		\begin{enumerate}
			...
			\item For each $a \in A$, $b \in B$, $\phi(a) < \phi(b)$. Set $c = \sup\limits_{a \in A}\phi(a)$. If for some $a_0 \in A$, $\phi(a_0) = c$, then there exists $N \in \N$ such that $a_0 + \frac{1}{N}x_0 \in A$.
		\end{enumerate}
	\end{ex}
	...
	\begin{proof}\
		\begin{enumerate}
			...
			\begin{align*}
				...
				& < 1
			\end{align*}
			So $\phi(a) < \phi(b)$. Set $c = \sup\limits_{a \in A}\phi(a)$. For the sake of contradiction, suppose that there exists $a_0 \in A$, $\phi(a_0) = c$. Since $A$ is open, and 
			\begin{align*}
				a_0 + \frac{1}{n}x_0 
				&\rightarrow a_0 \\
				&\in A
			\end{align*}
			there exists $N \in \N$ such that $a_0 + \frac{1}{N}x_0 \in A$. Then 
			\begin{align*}
				\phi \bigg(a_0 + \frac{1}{N}x_0 \bigg)
				&= \phi(a_0) + \frac{1}{N} \phi(x_0) \\
				&= c + \frac{1}{N} \\
				& > c
			\end{align*}
			This is a contradiction since for each $x \in A$, $\phi(a) < 1$. So for each $x \in A$, $y \in B$, 
			$$\phi(x) < c \leq \phi(y)$$
		\end{enumerate}
	\end{proof}
