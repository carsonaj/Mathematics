...
\begin{ex}
	Let $X$ be a unital Banach algebra and $x,y \in G(X)$. If $xy = e$ and $y \in G(X)$, then 
	\begin{enumerate}
		\item $yx = e$
		\item $x \in G(X)$ and $x^{-1} = y$
	\end{enumerate}
\end{ex}

\begin{proof}
	Suppose that $xy = e$ and $y \in G(X)$. 
	\begin{enumerate}
		\item Since $xy = e$, we have that
		\begin{align*}
			y 
			& = ye \\
			& = y(xy) \\
			& = (yx)y 
		\end{align*}
		Hence $(e - yx)y = 0$. Since $y \in G(X)$, we have that $e-yx = 0$ which implies that $yx = e$. 
		\item Since $xy = yx = e$, $x \in G(X)$ and $x^{-1} = y$.
	\end{enumerate}
\end{proof}
...
\end{proof}
	...
	\begin{proof}
		Suppose that $\lam^n \in \rho(x^n)$. For the sake of contradiction, suppose that $\lam \in \rho(x)$.
	\end{proof}
