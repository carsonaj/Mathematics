...
\begin{document}
	...
	Let $(X_{\al})_{\al \in A}$ be a collection of sets. We define the \tbf{Cartesian product} of $(X_{\al})_{\al \in A}$, denoted $\prod\limits_{\al \in A} X_{\al}$, by 
	\begin{equation*}
	\prod_{\al \in A}X_{\al} = \{f:A \rightarrow \bigcup_{\al \in A} X_{\al}: \text{ for each $\al \in A$, $f(\al) \in X_{\al}$}\}
	\end{equation*}
	\end{defn}
	
	\begin{defn} \ld{def:set_theory:products:0002}
	Let $(X_{\al})_{\al \in A}$ be a collection of sets. For $\al \in A$, we define the \tbf{$\al$-th projection map of $\prod_{\al \in A} X_{\al}$ onto $X_{\al}$}, denoted $\pi_{\al}:\prod\limits_{\al \in A}X_{\al} \rightarrow X_{\al}$, by 
	\begin{equation*}
	\pi_{\al}(f) = f(\al)
	\end{equation*}
	\end{defn}
	...
	\begin{defn} \ld{def:set_theory:products:0006}  
...
We define the \tbf{topology generated by $\ME$} on $X$, denoted $\tau(\ME)$, by $$\tau(\ME) = \bigcap_{\MT \in \MS} \MT$$
	...
	\begin{itemize}
		\item the \tbf{collection of open subsets of $A$}, denoted $\MU_A$, by 
		$$\MU_{A} \defeq \{U \subset X:U \subset A \text{ and $U$ is open}\},$$ 
		\item the \tbf{collection of closed supersets of $A$}, denoted $\MC_A$, by 
...
\end{proof}
	...
	\begin{defn}
		\tcr{define slice maps and then define the below sets in terms of them.}
	\end{defn}
	...
	\begin{note}
		Let $\Gam$ be a directed set and $\gam_0 \in \Gam$. From \rd{def:nets:0008.1}, we recall the $\gam_0$-tail operator $L_{\gam_0}: X^{\Gam} \rightarrow X^{[\gam_0, \infty)}$. 
	\end{note}
	...
	\begin{ex} \lex{ex:quotient_topology:0009}
	...
	\begin{proof} \
		\begin{enumerate}
			\item Clear.
			\item Suppose that $f$ is a quotient map. \rex{ex:quotient_topology:0013} implies that then there exists $\bar{f}: X/{\sim_f} \rightarrow Y$ such that $\bar{f}$ is a  $(\MT_{X/{\sim_f}}, \MB)$-homeomorphism and $\bar{f} \circ \pi = f$. 
		\end{enumerate}
	\end{proof}
	...
	\begin{defn} \ld{def:topology:separation:0004}
		Let $X$ be a a topological space. We define the \tbf{diagonal of $X \times X$}, denoted $\Del_X \subset X \times X$, by $\Del_X \defeq \{(x,x):x \in X\}$.
	\end{defn}
...
%			Since $x \in \pi^{-1}(\pi(U))$ is arbitrary, we have that for each $x \in \pi^{-1}(\pi(U))$, there exists $U' \in \MT$ such that $x \in U'$ and $U' \subset \pi^{-1}(\pi(U))$. Hence $\pi^{-1}(\pi(U)) \in \MT$. Since $U \in \MT$ is arbitrary, we have that for each $U \in \MT$, $\pi^{-1}(\pi(U)) \in \MT$. Since $\pi: X \rightarrow X / \sim_0$ is a quotient map, 
	...
	\begin{ex} \lex{ex:topology:compactness:00008}
	...
	\begin{defn} \ld{def:topology:compactness:00012}
		Let $X$ be a set and $\MA \subset \MP(X)$. Then $\MA$ is said to have the \tbf{finite intersection property} if for each $\MB \subset \MA$, $\MB$ is finite implies that $\bigcap\limits_{B \in \MB} B \neq \varnothing$. We define 
		$$\FIP(X) = \{\MA \subset \MP(X): \MA \text{ has the finite intersection property} \}$$ 
		and order $\FIP(X)$ by inclusion. 
	\end{defn}

	\begin{ex} \rex{ex:topology:compactness:00013}
	...
	\begin{ex} \rex{ex:topology:compactness:00014}
	...
	\begin{ex} \rex{ex:topology:compactness:00015}
	...
	\begin{note}
		Recall the definition of $\MC_A$ in \rd{31013}.
	\end{note}
	
	\begin{ex} \rex{ex:topology:compactness:00016}
	...
	\begin{ex} \rex{ex:topology:compactness:00017}
	...
	\begin{ex} \rex{ex:topology:compactness:00018} \tbf{Tychonoff's Theorem:} \\ 
	...
	\begin{proof}
		Set $X \defeq \prod\limits_{\al \in A} X_{\al}$ and $\MT \defeq \bigotimes\limits_{\al \in A} \MT_{\al}$. Let $\MC \subset \MC_{\varnothing}$. Suppose that $\MC \in \FIP(X)$. \tcb{A previous exercise} implies that there exists $\MD \in \FIP(X)$ such that $\MD$ is maximal in $[\MC, \infty)$. Let $\al \in A$. Set $\MD_{\al} \defeq \{\pi_{\al}(D):D \in \MD\}$. 
...
\begin{proof}
	...
	\begin{align*}
		...
		& = \bigcup\limits_{U \in \MU} U.
	\end{align*}
	Hence $\MU$ is an open cover of $K$ in $(X, \MT_X)$. Since $K$ is compact, there exists $\MU_0 \subset \MU$ such that $\MU_0$ is an open cover of $K$ in $(X, \MT_X)$ and $\MU_0$ is finite. By choice \tcr{(maybe make set theory exercise about how finite choice is given by ZFC)}, there exists $(\MV_U)_{U \in \MU_0} \subset \MV$ such that for each $U \in \MU_0$, $U = f^{-1}(V_U)$. Set $\MV_0 \defeq \{V_U:U \in \MU_0 \}$. Then $\MV_0$ is finite and
	\begin{align*}
		f(K)
		& \subset f \bigg( \bigcup\limits_{U \in \MU_0} U \bigg) \\
		& = \bigcup\limits_{U \in \MU_0} f(U) \\
...
\begin{proof}\
	\begin{itemize}
		...
		\item $(\impliedby):$ \\
		Conversely, suppose that $(E, \MT \cap E)$ is a Polish space.
	\end{itemize}
\end{proof}
