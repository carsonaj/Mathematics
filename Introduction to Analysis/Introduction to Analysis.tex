...
So $$\int_{(x_0, x_0 + h]}f - f(x_0) dm = o(\|h\|) \hspace{1cm}\text{ as }h \rightarrow 0$$ 
	...
	\begin{note}
	In mathematics, inner products are traditionally defined to be linear in the first argument. However, in my opinion, the physics tradition of defining inner products to be linear in the second argument makes more sense.
	\end{note}
...
\begin{proof}
	Let $x_1, \cdots, x_n \in S$ and $c_1, \cdots, c_n \in \C$. Suppose that $\sum\limits_{j =1}^n c_j x_j = 0 $. Then 
	\begin{align*}
		0
		&= \bigg \l \sum_{i=1}^n c_i x_i , \sum_{j=1}^n c_jx_j \bigg \r \\
		&= \sum_{i=1}^n \sum_{j=1}^n \l x_i, x_j \r \\
		&= \sum_{j=1}^n  |c_j|^2 \l x_j, x_j \r
	\end{align*}
	So for $j = 1 , \cdots, n$, $c_j = 0$ and $S$ is linearly independent.
\end{proof}

\begin{defn}
	Let $H$ be a Hilbert space and $S \subset H$. Then $S$ is said to be \textbf{orthonormal} if $S$ is orthogonal and for each $x \in S$, $\|x \| = 1$.
\end{defn}
...
\begin{ex}\textbf{(Pythagorean theorem):}\\
	Let $H$ be a Hilbert space and $(x_j)_{j \in \N} \subset H$ an orthogonal set. Suppose that $\sum\limits_{j \in \N} x_j$ converges, then $$\bigg \|\sum\limits_{j \in \N} x_j  \bigg \|^2 = \sum\limits_{j \in \N} \|x_j \|^2$$
\end{ex}
...
\begin{proof}
	...
	\begin{align*}
		\bigg \|\sum\limits_{j \in \N} x_j  \bigg \|^2
		&= \lim_{n \rightarrow \infty} \bigg \|\sum\limits_{j =1}^n x_j  \bigg \|^2 \\
		&= \limn \sum\limits_{j =1}^n \| x_j \|^2 \\
		&= \sum\limits_{j \in \N} \|x_j \|^2
	\end{align*}
\end{proof}

\begin{defn}
	Let $H$ be a Hilbert space and $S \subset H$. Then $S$ is said to \textbf{span} $H$ if $\spn S$ is dense in $H$ and $S$ is said to be a \textbf{basis} for $H$ if $S$ spans $H$ and $S$ linearly independent.
\end{defn}
...
%	Let $x \in H$. We define the \textbf{adjoint} of $x$, denoted $x^{*}: H \rightarrow \C$, by $x^{*} \phi = \l x, \phi\r$. 
%\end{defn}
%
%\begin{note}
%	In mathematics, where linearity of the inner product is in the first argument, $x^{*}$ is typically referred to by $u_{x} \in H^{*} $ where $u_{x}(\phi) = \l \phi, x\r$. In physics, where the inner product with linearity in the second argument, $x^{*} \phi$ is usually written in the so-called ``bra-ket" notation as $\l x | \phi \r$ which works smoothly since it aligns with the linearity of $u_{x}(\phi_1 + \lam \phi_2)$ and the conjugate-linearity of $u_{x_1 + \lam x_2}(\phi)$. In this way, it generalizes the notation for $\l x, y\r = x^T y$ for $\R^n$ to $\l x, y\r = x^*y^*$ for $\C^n$. 
%\end{note}
%
%\begin{ex}
%	Let $x, \phi \in \H$ and $ \lam \in \C$. Then 
%	\begin{enumerate}
%		\item $(x + \phi){^*} =  x^*+ \phi^*$
%		\item $(\lam x)^* = \lam^* x^*$
%	\end{enumerate}
...
%
%\begin{defn}
%	Let $x, \phi \in H$ and $A$ an operator on $H$. We define 
%	\begin{enumerate}
%		\item the linear functional on $H$, $x ^*A: H \rightarrow \C $ by $(x ^*A) \al = x ^*(A \al)$
%		\item the operator on $H$, $x \phi ^*: H \rightarrow H$ by $(x \phi \da) \al = (\phi^*\al) x$
%	\end{enumerate}
%\end{defn}
%
%\begin{ex}
%	Let $A$ be an operator on $H$ and $x \in H$. Then $$(A x)^*= x ^*A^*$$
%\end{ex}
%
%\begin{proof}
%	Let $\phi \in H$. Then 
