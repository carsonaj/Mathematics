...
\end{align*}	 
	...
	\begin{ex}
	Let $A \subset R$ be open and $f:A \rightarrow R$. Then $f$ is differentiable iff $f$ is Frechet differentiable and for each $x_0 \in A$ and $h \in \R$ $$Df(x_0)(h) = hf'(x_0) $$
	\end{ex}
	...
	\begin{ex}
	Let $Y$ be a separable Banach space, $f:[a,b] \rightarrow Y$ continuous, $G: (a,b) \rightarrow Y$ Frechet differentiable and $c \in Y$. Then 
	\begin{enumerate}
	\item $f$ is Bochner integrable 
	\item the map $F: (a,b) \rightarrow Y$ defined by $$F(x) = \int_{[a, x]}f dm + c$$ is continuously Frechet differentiable and for each $x_0 \in (a,b)$, $$DF(x_0)(h) = f(x_0)h$$
	\item for each $x,y \in (a,b)$, if $x < y$, then $$F(y)-F(x) = \int_{(x,y]}DF(x)(1)dm(x)$$
	\item If $DG = DF$, then $F-G$ is constant.
	\end{enumerate}
	\end{ex}
	
	\begin{proof}
	\begin{enumerate}
	\item Continuity implies that $f \in L_Y^0$ and 
	\begin{align*}
	\int \| f \| dm 
	&\leq \| f\|_{\infty}(b-a) \\
	&< \infty
\end{align*}
	...
	\item Let $x_0 \in (a,b)$ and $h \in (0, b-x_0)$. Then continuity implies that
	\begin{align*}
...
So $$\int_{[x_0, x_0 + h]}f - f(x_0) dm = o(\|h\|) \hspace{1cm}\text{ as }h \rightarrow 0$$ 
	...
	F(x_0 + h)
	&= \int_{[a, x_0 + h]} f dm + c \\
	&= \int_{[a, x_0]} f dm + c + \int_{[x_0, x_0 + h]} fdm \\
	&= \int_{[a, x_0]} f dm + c + f(x_0)h + \int_{[x_0, x_0 + h]} f - f(x_0) dm \\ 
	&= F(x_0 + h) + f(x_0)h + o(\|h\|) \hspace{1cm }\text{ as } h \rightarrow 0\\
	\end{align*}
	The case is similar for $h \in (x_0 - b, 0)$. So $F$ is Frechet differentiable at $x_0$ and $DF(x_0)(h) = f(x_0)h$.
	\item Let $x,y \in (a,b)$. Suppose that $x < y$. Then \begin{align*}
	F(y) - F(x) 
	&= \int_{(x,y]} f(x) dm(x) \\
	&= \int_{(x,y]} DF(x)(1) dm(x)
	\end{align*}
	\item Suppose that $DG = DF$. Then $D(F-G) = 0$. A previous exercise implies that $F - G$ is constant.
	\end{enumerate}
	\end{proof}
	...
	\begin{enumerate}
	\item the map $Df(\cdot)(1): [x_0,x] \rightarrow Y$ is Bochner integrable 
	\item  $$f(x) - f(x_0) = \int_{[x_0, x]}Df(t)(1)dm(t)$$ 
	\end{enumerate}
	\end{ex}

	\begin{proof}
	Let $x, x_0 \in (a,b)$ and suppose that $x_0 < x$. 
	\begin{enumerate}
	\item Since $Df: (a,b) \rightarrow Y$ is continuous, $Df(\cdot)(1): [x_0,x] \rightarrow Y$ is continuous and by the previous exercise, Bochner integrable. 
	\item Define $g: [x_0,x] \rightarrow Y$ by $$g(t) = \int_{[x_0, t]}Df(t)(1)dm(t)$$
	Then the previous exercise implies that $g$ is Frechet differentiable and for each $t \in (x_0, x)$ and $h \in \R$, 
	\begin{align*}
	Dg(t)(h) 
	&= Df(t)(1)h \\
	&= Df(t)(h)
	\end{align*}
	The previous exercise implies that $f-g$ is constant on $(x_0, x)$ so there exists $c \in Y$ such that $f=g+c$ on $(x_0, x)$. Since $f-g$ is continous, $f-g = c$ on $[x_0, x]$. Then 
	\begin{align*}
	f(x) - f(x_0)
	&= g(x)+c - (g(x_0)+c) \\
	&= g(x) - g(x_0) \\
	...
	\begin{ex}
		Let $X, Y$ be a Banach spaces, $A \subset X$ open, $f:A \rightarrow Y$, $x_0 \in A$ and $n \in \N$. If $Y$ is separable, then $f$ is $n$-th  order Frechet differentiable at $x_0$ iff for each $j \in \{1, \cdots, n\}$, there exists $\phi_j \in L^j(X, Y)$ such that $$f(x + h) = \sum_{j=1}^n \phi_j(h, \cdots, h) + o(\|h\|^n)$$
	\end{ex}
