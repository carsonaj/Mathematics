...
Then $V_{\ep}^o \in \MN_{x_0}$ is open and 
	...
	We define $$L^n \bigg(\prod_{j=1}^n X_j, Y\bigg ) = \bigg\{T: \prod\limits_{i=1}^n X_i \rightarrow Y: T \text{ is multilinear and bounded}\bigg \}$$ 
	If $X_1, \cdots, X_n = X$, we write $L^n(X,Y)$ in place of $L^n ( X^n, Y) $
	\end{defn}
...
So $$\int_{(x_0, x_0 + h]}f - f(x_0) dm = o(\|h\|) \hspace{1cm}\text{ as }h \rightarrow 0$$ 
	...
	Let $Y$ be a separable Banach space, $A \subset X$ open and convex, $f\in C^n_Y(A)$ and $x_0 \in A$. Then $$f(x_0 + h) = \sum_{k=0}^n \frac{1}{k!} D^k f(x_0)(h^{\oplus k}) + o(\|h\|^n) \hspace{1cm }\text{ as } h \rightarrow 0$$
