...
\end{enumerate}	
	...
	\begin{defn}
Let $Y$ be a Banach space, $A \subset \R$ open and $f:A \rightarrow Y$. Then $f$ is said to be \textbf{Gateaux differentiable} if for each $x \in A$, $f$ is Gateaux differentiable at $x$. 
\end{defn}	
	...
	\begin{ex}
	Let $X, Y$ be Banach spaces, $A \subset X$ open, $f:A \rightarrow Y$ and $x_0 \in A$. Suppose that $f$ is Gateaux differentiable at $x_0$. Then for each $\lam \in \R$ and $x \in X$, $$df(x_0)(\lam x) = \lam df(x_0)(x) \in X^*$$
	\end{ex}
...
\end{align*}	 
	...
	\begin{defn}
	Let $Y$ be a Banach space, $A \subset \R$ open, $f:A \rightarrow Y$ and $x_0 \in A$. Suppose that $f$ is Gateaux differentiable at $x_0$. We define 
	\begin{align*}
	f'(x_0) 
	&= \lim_{t \rightarrow 0} \frac{f(x_0 + t) - f(x_0)}{t} \\
	&= df(x_0)(1)
	\end{align*}
	If $f$ is Gateaux differentiable, we define $f':A \rightarrow Y$ by $x \mapsto f'(x)$. 
	\end{defn}	
	...
	\begin{ex}
	Let $Y$ be a Banach space, $A \subset \R$ open and $f:A \rightarrow Y$. Then $f$ is differentiable iff $f$ is Frechet differentiable and for each $x_0 \in A$ and $h \in \R$ $$Df(x_0)(h) = hf'(x_0) $$
	\end{ex}
	...
	\begin{ex}
	Let $Y$ be a separable Banach space, $f:[a,b] \rightarrow Y$ continuous so that $f$ is Bochner-integrable. Define $F:(a,b) \rightarrow Y$ by  $$F(x) = \int_{(a, x]}f dm$$ Then $F \in C^1_Y((a,b))$ and for each $x_0 \in (a,b)$ and $h \in \R$, $DF(x_0)(h) = hf(x_0)$ 
	\end{ex}
...
So $$\int_{(x_0, x_0 + h]}f - f(x_0) dm = o(\|h\|) \hspace{1cm}\text{ as }h \rightarrow 0$$ 
	...
	&= F(x_0 + h) + hf(x_0) + o(\|h\|) \hspace{1cm }\text{ as } h \rightarrow 0\\
	\end{align*}
	The case is similar for $h \in (x_0 - b, 0)$. Since the map $h \mapsto f(x_0)h$ is bounded, $F$ is Frechet differentiable at $x_0$ and $DF(x_0)(h) = f(x_0)h$.
	Let $x,y \in (a,b)$ and $h \in \R$. Then 
	\begin{align*}
	\|(DF(x)- DF(y))(h)\| 
	&= \|DF(x)(h) - DF(y)(h)\| \\
	&=  \|hf(x) - hf(y)\| \\
	...
	&= \|f(x) - f(y)\||h|
	\end{align*}
	So $\|DF(x) - DF(y)\| \leq \|f(x) - f(y)\|$. Then continuity of $f$ implies continuity of $DF$. Hence $F \in C^1_Y(a,b)$.
	\end{proof}
	...
	\begin{enumerate}
	\item $Df(\cdot)(1)$ is Bochner integrable on $(x_0, x]$ 
	\item  $$f(x) - f(x_0) = \int_{(x_0, x]}Df(t)(1)dm(t)$$ 
	\end{enumerate}
	\end{ex}

	\begin{proof}
	\begin{enumerate}
	\item Since $Df: (a,b) \rightarrow Y$ is continuous and for each $x,y \in (a,b)$, 
	\begin{align*}
	\|Df(x)(1) - Df(y)(1)\| 
	&= \|(Df(x) - Df(y))(1)\| \\
	& \leq \|Df(x) - Df(y)\|
\end{align*}
	and we have that $Df(\cdot)(1):(a,b) \rightarrow Y$ is continuous. Let $x, x_0 \in (a,b)$. Suppose that $x_0 < x$. Choose $c,d \in (a,b)$ such that $a < c < x_0< x< d < b$. Then $Df(\cdot)(1)$ is continuous on $[c,d]$ and hence Bochner-integrable on $(c,d]$ and $(x_0,x]$. 
	\item Define $g: (c,d) \rightarrow Y$ by $$g(\xi) = \int_{(c, \xi]}Df(t)(1)dm(t)$$
	Then the previous exercise implies that $g \in C^1_Y$  and for each $t \in (c, d)$ and $h \in \R$, 
	\begin{align*}
	Dg(t)(h) 
	&= hDf(t)(1) \\
	&= Df(t)(h)
	\end{align*}
	A previous exercise implies that there exists $c \in Y$ such that $f = g + c$ on $(c, d)$. Then 
	\begin{align*}
	f(x) - f(x_0)
	&= g(x)+c - (g(x_0)+c) \\
	&= g(x) - g(x_0) \\
	&= \int_{(c, x]}Df(t)(1)dm(t) - \int_{(c, x_0]}Df(t)(1)dm(t)\\
	&= \int_{(x_0, x]}Df(t)(1)dm(t)
	\end{align*}
	\end{enumerate}
	\end{proof}
	...
	\begin{ex}
		Let $X, Y$ be a Banach spaces. Suppose $Y$ is separable. Let $A \subset X$ open, $f:A \rightarrow Y$, $x_0 \in A$ and $n \in \N$. If $f \in C^n_Y(A)$, then for each $x \in A$ $$f(x + h) = f(x) + \sum_{j=1}^n \phi_j(h, \cdots, h) + o(\|h\|^n)\hspace{1cm }\text{ as } h \rightarrow 0$$
	\end{ex}
