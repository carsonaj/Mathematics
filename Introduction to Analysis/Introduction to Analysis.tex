...
\begin{document}
	...
	\begin{defn}
	Let $f:X \rightarrow \R$ and $x \in X$. Then $f$ is said to be \textbf{Gateaux differentiable} at $x$ if for each $x \in A$, 
	\end{defn}
	...
	\begin{note}
	For the remainder of this section, we assume $X$ is a real Banach space. 
	\end{note}	
	
	\begin{defn}
	Let $A \subset X$ and $x,y \in A$. We define the \textbf{line segment} between $x$ and $y$, denoted $l_{x,y}$, to be $$l_{x,y} = \{tx + (1-t)y : t \in [0,1] \}$$
	\end{defn}

	\begin{defn}
	Let $A \subset X$. Then $A$ is said to be $\textbf{convex}$ if for each $x, y \in A$, $l_{x,y} \subset A$. 
	\end{defn}	
	
	\begin{note} 
	For the remainder of this section, we assume that $A \subset X$ is convex.
	\end{note}
	
	\begin{defn}
	Let $f:A \rightarrow \R$. Then $f$ is said to be  \textbf{convex} if for each $x,y \in A$, $t \in [0,1]$, $$f(tx + (1-t)y) \leq tf(x) + (1-t)f(y)$$ We define $$\MC(A) = \{f:A \rightarrow \R: f \text{ is convex}\}$$ 
	\end{defn}
	
	\begin{ex}
	Let $f,g:A \rightarrow \R$ and $\lam \geq 0$. If $f,g$ are convex, then 
	\begin{enumerate}
	\item $f+ g$ is convex 
...
\end{align*}		 
	...
	\begin{ex}
	Let $f:\R \rightarrow \R$ and $g: A \rightarrow \R$. If $f$ is convex and increasing and $g$ is convex, then $f \circ g$ is convex.
	\end{ex}	
...
So $f \circ g$ is convex.
	...
	\begin{ex}
	Let $A \subset \R$ and $f:A \rightarrow \R$. Then $f$ is convex iff for each $x,y \in A$, if $x \neq y$  then for each $z \in l_{x,y}$, $$f(z) \leq f(x) + \frac{f(y) - f(x)}{y-x}(z-x)$$ that is, between $x$ and $y$, the graph of $f$ lies below its secant line. 
	\end{ex}
	
	\begin{proof}
	Suppose that $f$ is convex. Let $x,y \in A$. Suppose that $x \neq y$. Let $z \in l_{x,y}$. Then there exists $t \in [0,1]$ such that $z = tx + (1-t)y$. Define $s:\R \rightarrow \R$ by $$s(z) = f(x) + \frac{f(y) - f(x)}{y-x}(z-x)$$ 
	Then 
	\begin{align*}
	s(z) 
	...
	&= f(x) + (f(y) - f(x))(1-t) \\
	&= tf(x) + f(y)(1-t) \\
	& \geq f(tx + (1-t)y) \hspace{2cm} \text{(by convex ity)}\\ 
	& = f(z)	
	\end{align*}
	Conversely, suppose that for each $x,y \in A$, if $x \neq y$  then for each $z \in l_{x,y}$, $$f(z) \leq f(x) + \frac{f(y) - f(x)}{y-x}(z-x)$$ 
	Let $x, y \in A$. Suppose that $x \neq y$. Define $s:\R \rightarrow \R$ as above. Then similarly, $s(z) =  tf(x) + f(y)(1-t)$. Let $t \in [0,1]$. Put $z = tx + (1-t)y \in l_{x,y}$. By assumption, 
	\begin{align*}
	f(tx + (1-t)y) 
	&= f(z) \\
	& \leq s(z) \\
	...
	If $K$ is convex, then $\phi$ is convex. 
	\end{ex}	
