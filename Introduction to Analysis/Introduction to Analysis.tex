...
\end{proof}
	...
	\begin{proof}\
		\begin{enumerate}
			\item Let $x \in \N$. Since $(X, \MT)$ is first-countable, there exists $(E_{x, j})_{j \in \N} \subset \MT$ such that $(E_{x, j})_{j \in \N}$ is a local basis for $\MT$ at $x$. Define $(U_{x,n})_{n \in \N} \subset \MT$ by 
	...
	\begin{defn}
		Let $(X, \MT)$ be a topological space and $\MU \subset \MT$. Then $\MU$ is said to be an \tbf{open cover} of $X$ if $X \subset \bigcup\limits_{U \in \MU} U$. 
	\end{defn}

	\begin{defn}
		Let $(X, \MT)$ be a topological space. Then $(X, \MT)$ is sais to be \tbf{compact} if for each open cover $\MU \subset \MT$, there exists a subcover $\MU_0 \subset \MU$ such that $\MU_0$ is finite.
	\end{defn}
...
\begin{defn}
	...
	Let $\MP: \Rgp \rightarrow \Part(X)$. 
	\begin{itemize}
		\item For each $r \in \Rgp$, we define the \tbf{$r$-th partition relation} on $X$, denoted $\sim_{\MP_r}$, by $x \sim_{\MP_r} y$ iff there exists $E \in \MP_r$ such that $x \in E$ and $y \in E$.
		\item For $r \in \Rgp$, we denote the projection of $X$ onto $X/\sim_{\MP_r}$ by $\pi^{\MP}_r: X \rightarrow X/ \sim_{\MP_r}$ so that $\MP_r = \{\pi^{\MP}_r(x): x \in X\}$. 
		\item 
		For $x,y \in X$, we define $A^{\MP}(x,y) = \{r \in \Rgp: \pi^{\MP}_r(x) = \pi^{\MP}_r(y)\}$ and $\al^{\MP}(x,y) = \sup A^{\MP}(x,y)$. 
	\end{itemize}
	Then 
	\begin{itemize}
		\item $\MP$ is said to \tbf{separate points} if for each $x,y \in X$, $x \neq y$ implies that there exists $r > 0$ such that $\pi^{\MP}_r(x) \neq \pi^{\MP}_r(y)$
		\item $\MP$ is said to \tbf{collect points} if for each $x,y \in X$, there exists $r >0$ such that $\pi^{\MP}_{r}(x) = \pi^{\MP}_{r}(y)$.
		\item $\MP$ is said to be 
		\tbf{decreasing} if for each $r,s \in \Rgp$, $r \leq s$ implies that for each $x \in X$, there exists $\MF_x \subset X$ such that $\pi^{\MP}_r(x) = \bigcup\limits_{y \in \MF_x} \pi^{\MP}_s(y)$
		\item  $\MP$ is said to be \tbf{left-continuous} if for each $x,y \in X$, $A^{\MP}(x,y) \neq \varnothing$ and $\al^{\MP}(x,y) < \infty$ implies that $\al^{\MP}(x,y) \in A^{\MP}(x,y)$
	\end{itemize}
\end{defn}

\begin{ex}
	Let $X$ be a set and $\MP: \Rgp \rightarrow \Part(X)$. Suppose that $\MP$ is decreasing. Then for each $x,y \in X$ and $s \in \Rgp$, if $\pi^{\MP}_s(x) = \pi^{\MP}_s(y)$, then for each $r \in \Rgp$, $r \leq s$ implies that $\pi^{\MP}_r(x) = \pi^{\MP}_r(y)$.
\end{ex}

\begin{proof}
	Let $x,y \in X$ and $s \in \Rgp$. Suppose that $\pi^{\MP}_s(x) = \pi^{\MP}_s(y)$. Let $r \in \Rgp$. Suppose that $r \leq s$. Since $\MP$ is decreasing, there exists $\MF_x \subset X$ such that $\pi^{\MP}_r(x) = \bigcup\limits_{z \in \MF_x} \pi^{\MP}_s(z)$. Since $\MP_s$ is a partition of $X$,  there exists $x' \in \MF_x$ such that $\pi^{\MP}_s(x') = \pi^{\MP}_s(x)$. Since $\pi^{\MP}_s(x) = \pi^{\MP}_s(y)$, we have that
	\begin{align*}
		 y
...
\begin{ex}
	Let $X$ be a set and $\MP: \Rgp \rightarrow \Part(X)$. Suppose that $\MP$ is decreasing. Let $x,y \in X$. Suppose that there exists $r >0$ such that $\pi^{\MP}_r(x) \neq \pi^{\MP}_r(y)$. Then for each $s \geq r$, $\pi^{\MP}_s(x) \neq \pi^{\MP}_s(y)$.
\end{ex}
...
\begin{ex}
	 Let $X$ be a set and $\MP: \Rgp \rightarrow \Part(X)$. Suppose that $\MP$ separates points, collects points, is decreasing and is left-continuous. Then for each $x,y \in X$, if $x \neq y$, then $\al^{\MP}(x,y)$ exists and $A^{\MP}(x,y) = [0, \al^{\MP}(x,y)]$
\end{ex}
...
\begin{ex} \tbf{(Fundamental Example):} 
	Let $X$ be a set and $\MP: \Rgp \rightarrow \Part(X)$. Suppose that $\MP$ separates points, collects points, is decreasing and is left-continuous. Define $d^{\MP}: X \times X \rightarrow \Rgp$ by 
	\[
	d^{\MP}(x,y) =
	\begin{cases}
			e^{-a(x,y)} & x \neq y \\
...
\begin{ex}
	...
	\begin{enumerate}
		\item $\MP^d$ separates points
		\item $\MP^d$ collects points
		\item $\MP^d$ is decreasing
		\item $\MP^d$ is left-continuous
	\end{enumerate}
\end{ex}

\begin{proof}\
	\begin{enumerate}
		\item Let $x, y \in X$. Suppose that $x \neq y$. Then $d(x,y) > 0$. Set $r = 2^{-1}d(x,y)^{-1}$. Then  
		\begin{align*}
		...
		\begin{align*}
			...
			& \neq \pi^{\MP^d}_r(x)
		\end{align*}
		So $\MP^d$ separates points.
		\item Let $x,y \in X$. Choose $r > d(x,y)$. Then 
		\begin{align*}
			\pi^{\MP^d}_{r^{-1}}(x) 
			& = \bar{B}(x, r) \\
			& = \bar{B}(y,r) \\
		...
		\item Let $x,y \in X$. Suppose that $A^{\MP^d}(x,y) \neq \varnothing$ and $\al^{\MP^d}(x,y) < \infty$. Then there exists $(\al_n)_{n \in \N} \subset A^{\MP^d}(x,y)$ such that for each $n \in \N$, $\al^{\MP^d}(x,y) = \sup\limits_{n \in \N} \al_n$ and $\al_n \neq \al^{\MP^d}(x,y)$. Let $n \in \N$. Since $\al_n \in A^{\MP^d}(x,y)$, 
		\begin{align*}
