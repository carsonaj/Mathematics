...
is convex.  
	...
	&= \mu(U) 
	\end{align*}
	Therefore, since $\mu$ is outer regular, $\mu(\sig(E)) \leq \mu(E)$. Since $\sig_n^{-1} \rightarrow \sig^{-1}$, we may apply the above argument to obtain that 
	\begin{align*}
	\mu(E) 
	&= \mu(\sig^{-1}(\sig (E))) \\
	&\leq  \mu(\sig(E))
...
Hence $\mu(E) = \mu(\sig(E))$. Applying the whole argument above thus far to $\sig^{-1}$, we see that $\mu(E) = \mu(\sig^{-1}(E))$. Since $E \subset X$ is an arbitrary closed set and $\MB(X) = \sig(E \subset X: E \text{ is closed})$, we have that $\mu = \sig_*\mu$. Thus $\sig \in \Aut(X, \mu)$ which implies that $\Aut(X, \mu)$ is closed. 
	...
	\begin{defn}
	Let $(M, d)$ be a metric space, $(G, \tau)$ a topological group, and $\cdot: G \times M \rightarrow M$ a group action. We define 
	$\bar{d}: M / G \rightarrow \Rg$ by 
	$$\bar{d}(o_x, o_y) = \inf_{\substack{a \in o_x \\ b \in o_y}} d(a,b) $$
	\end{defn}
	
	\begin{ex}
	Let $(M, d)$ be a metric space, $(G, \tau)$ a topological group, and $\cdot: G \times M \rightarrow M$ a group action. Suppose that for each $g \in G$, the map $x \mapsto g \cdot x$ is an isometry. Then for each $x, y \in M$, $$\bar{d}(o_x, o_y) = \inf_{g \in G} d(g \cdot x, y)$$
	\end{ex}
	
	\begin{proof}
	Let $x, y \in M$, $a \in o_x$ and $b \in o_y$. Then there exists there exists $g_x, g_y \in G$ such that $a = g_x \cdot g$ and $b = g_y \cdot y$. Set $g = g_y^{-1}g_x$. Since the map $z \mapsto g_y^{-1} \cdot z$ is an isometry, 
	\begin{align*}
	d(a,b) 
	...
	\begin{ex}
	Let $(M, d)$ be a metric space, $(G, \tau)$ a topological group, and $\cdot: G \times M \rightarrow M$ a group action. Suppose that for each $g \in G$, the map $x \mapsto g \cdot x$ is an isometry. If for each $x \in M$, $o_x$ is closed, then $\bar{d}$ is a metric.
	\end{ex}
	...
	\begin{ex}
	Let $(M, d)$ be a metric space, $(G, \tau)$ a topological group, and $\cdot: G \times M \rightarrow M$ a group action. Suppose that for each $g \in G$, the map $x \mapsto g \cdot x$ is an isometry, $G$ is compact and for each $x \in M$, the map $g \mapsto g \cdot x$ is continuous. Then $\bar{d}: M/ G \rightarrow \Rg$ is a metric. 
	\end{ex}
	
	\begin{proof}
	Let $x \in M$. Since $G$ is compact and the map $g \mapsto g \cdot x$ is continuous, $o_x = G \cdot x$ is compact and therefore closed. The previous exercise implies that $\bar{d}$ is a metric.
	\end{proof}
	...
	\begin{ex}
	Consider the metric space $(\C^{n \times d}, \|\cdot\|_F)$, topological group $(U_d, \|\cdot\|_F)$ and  the (right) action $X \cdot U = XU$. Then this action is continuous, $U_d$ is compact and for each $U \in U_d$, the map $X \mapsto XU$ is an isometry. Thus $\bar{d}$ is a metric on $\C^{n \times d}/ U_d$.
	\end{ex}
	...
	\begin{ex}
	Let $(X, \MA, \mu)$ be a measure space. Then $\| \cdot \|_*$ is a norm on $L^1(\mu)$.
	\end{ex}
	...
	\begin{ex}
	Let $X$ be a compact metric space and $\mu:\MB(X) \rightarrow \RG$ a Borel measure. Define the (right) group action \\ $\cdot: (L^1(\mu), \|\cdot\|_*) \times \Aut(X, \mu) \rightarrow (L^1(\mu), \|\cdot\|_*) $ by $$f \cdot \sig = f \circ \sig$$ Then for each $\sig \in \Aut(X, \mu)$, the map $f \mapsto f \cdot \sig$ is an isometry. 
	\end{ex}
...
Then 
	\begin{align*}
	\|f \cdot \sig\|_* 
	&= \sup_{A \in \MA} \bigg | \int_A f \circ \sig d\mu \bigg | \\
	&= \sup_{A \in \MA} \bigg | \int_{\sig(A)} f d \sig_* \mu \bigg | \\
	&= \sup_{A \in \MA} \bigg | \int_{\sig(A)} f d \mu \bigg | \\
	&= \sup_{A \in \MA} \bigg | \int_{A} f d \mu \bigg | \\
	&= \|f\|_* 
	\end{align*}	 
	\end{proof}
