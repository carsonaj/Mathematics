...
\end{align*}
	...
	\begin{ex}
		Let $X, Y$ be a Banach spaces, $A \subset X$ open, $f:A \rightarrow Y$, $x_0 \in A$ and $n \in \N$. If $Y$ is separable, then $f$ is $n$-th  order Frechet differentiable at $x_0$ iff for each $j \in \{1, \cdots, n\}$, there exists $\phi_j \in L^j(X, Y)$ such that $$f(x + h) = f(x) + \sum_{j=1}^n \phi_j(h, \cdots, h) + o(\|h\|^n)\hspace{1cm }\text{ as } h \rightarrow 0$$
	\end{ex}
