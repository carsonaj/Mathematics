...
\begin{frame}
\frametitle{Applications to Gaussian Processes}
\begin{rem}
Let $T$ be a set and $x = (x_j)_{j=1}^n \in T^n$, $y = (y_j)_{j=1}^n \in \R^n$. From Gaussian processes, we know that if 
\begin{align*}
y_i &= f(x_i) + \ep_i \\
\ep_i &\sim N(0, \sig^2) \\
f &\sim GP(0, c)
...
\begin{frame}
\begin{rem}
If $c(x,x) + \sig^2I$ is too costly to invert, we may set up the following convex optimization problem to approximate the posterior mean and posterior covariance functions:
$$\hat{h}(t) = \argmin_{h \in H} \sum_{j=1}^n (y_j - h(t_j))^2 + \sig^2 \|h\|_H$$ where $H$ is the RKHS corresponding to the p.d. kernel $c$
\end{rem}
\end{frame}
