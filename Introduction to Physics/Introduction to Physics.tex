...
\begin{document}
	...
	\section{Basic Concepts (RENAME)}
	
	\begin{itemize}
		\item Define length, time???, other units of measurement. Define position, velocity, acceleration, etc.
		\item Record experiment showing acceleration near us can be modeled as constant. 
		\begin{itemize}
			\item Use something simple with the same tools as Galileo. For example, release a block (try using oil or ice to reduce friction) from rest on a ramp and approximate velocity of a block at the bottom. Maybe you an use metronomes to binary search the amount of time the block travels a very small distance right before the bottom? Repeat for various heights, various weights. Repeat taking taking velocity measurements at various points along the ramp. Show that velocity at time t is proportional to the time elapsed. So acceleration is constant. Show the height of release (top of ramp) determines acceleration. Postulate that maximum acceleration happens at vertical ramp. Show that acceleration proportional to cosine of angle of incidence of ramp to horizontal (floor or table) and vertical is where cosine is 1 which is the maximum
			\item Show that the velocity is proportional to the square root of height of release. 
		\end{itemize}
		\item Do experiment with pendulum (release at string aligned perpendicular to vertical), try to find a simple way to measure speed, show acceleration is constant and speed at bottom is proportional square root of string length (i.e. height of release)
		\item Do more general experiment where a block can slide on an organic surface (like a skateboarder in a skate park), so for example the graph of $z = - x^2 - y^2$ or other. Show that at different points of a level set, when released, the block slides in the direction of steepest descent (be precise in terms of paths on the surface, i.e. shortest paths on the surface i.e. geodesics)
		\item Talk about the idea of potential energy, and quantifying it in terms
	\end{itemize}
	...
	\begin{itemize}
		\item Come up with satisfactory model of angular momentum. Probably has somethign to do with areas, wedge products, etc 
		\item Should be able to be generalized to manifolds
		\item Should be able to reproduce the electron like in \href{https://quicycle.com/understanding-electrons/}{https://quicycle.com/understanding-electrons/}, see paper 'Is The Electron A Photon With Toroidal Topology?' by williamson, including on manifolds or spacetime
		\item Is there a good continuous model for densities on $\R^{2N}$ or more generally function spaces so that the angular momentum can be treated in a stat mech framework? \href{https://physics.stackexchange.com/questions/650944/how-is-angular-momentum-defined-on-symplectic-space}{https://physics.stackexchange.com/questions/650944/how-is-angular-momentum-defined-on-symplectic-space}
		\item 
	\end{itemize}
	...
	\newpage
	\chapter{Spin}
	\tcr{idea for getting around cloning theorem:} While it is true that you cant copy the whole state of the system, this is also true for regular computations as well and various ``classical categories" as well. Generally speaking a copy as a morphism $\text{copy}: X \rightarrow X \otimes X$ satisfying some property and we can use this to form the ``tuple" of morphisms $(f,g): X \rightarrow A \times B$ where $f:X \rightarrow A$ and $g:X \rightarrow B$ by $(f,g) \defeq (f \otimes g) \circ \text{copy}$. We use $(f,g)$ all the time in various contexts like in set theory or other math subjects, but the ability to form this morphism is equivalent to having a $\text{copy}$ morphism (\tcr{check this}). However not all categories have such a morphism.   \\
	
	It is said that the NOR gate is functionally complete because you can form any logical operator $f:B^n \rightarrow B$ with $B=\{0,1\}$ and $n \in \N_0$, but this is misleading. Using logic gates on different inputs, you cannot create a unary operator such as the identity or NOT gate just by using the NOR gate. This is because the arity of a composition of maps is bounded below by the lowest arity among the maps. So if you start with $2$-ary maps, you can never get a $1$-ary map. What is meant by this however, is that if you allow $f(g_1, \ldots, g_n)$, (i.e you consider the clone generated by the functions of interest, plus the porjections), then you can get all operators. Note that this is equivalent to copying because you form the tuple $(g_1, \ldots, g_n)$, so really you are not forming the NOT gate out of the NOR gate, but out of the NOR gate $+$ copy. \\
	
	\tcr{(need to check this)} The thing about quantum computing and the no cloning theorem is that the proof uses the assumption that you are clonging the entire information of the system. If we think about what is happening with the logic gates, the state of the system before and after processing thorugh a gate is described by the voltage and current, (at least in switch logic gates?). However for computation, we only use the voltage information. I think you can show that it is impossible to copy the voltage and current information simultaneously, so we basically copy the voltage information from before and after a COPY gate at the cost of splitting the current. Can you do something similar with Qbits? Can we descibe the state of the system using two pieces of info like spin and total spin squared $S_x, S^2$, they have a simultaneous basis since they commute, and compete similar to how voltage and current compete, maybe we can find a basis that we can copy only the spin info $S$ at the cost of messing with the total spin info $S^2$?
	...
	\begin{itemize}
		...
		\item \tcr{BIG GOAL:} similar to how given a markov kernel $P^t(x, A) = P^t_x(A)$, we can define the operator $Q_t$ in $C^{\infty}(M)$ by 
