...
\begin{defn}\
	\begin{itemize}
		...
		\item Let $S$ be a set and $k \in \N_0$. We define the \tbf{set of $k$-ary operation on $S$}, denoted $\MF^k(S)$, by $\MF^k(S) \defeq S^{(S^k)}$. We define the \tbf{set of finitary operations on $S$}, denoted $\MF^*(S)$, by
...
\begin{defn}
	Let $S$ be a set, $\MF \subset \MF^*(S)$ and $C \subset S$. Then $C$ is said to be  \tbf{$\MF$-closed} if for each $k \in \N_0$, $f \in \MF_k$ and $a_1, \ldots, a_k \in C$, $f(a_1, \ldots, a_k) \in C$.
\end{defn}
