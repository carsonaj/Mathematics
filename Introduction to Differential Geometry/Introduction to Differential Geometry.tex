...
%		\tcr{Make this an exercise in the analysis notes section on topology and subspaces, then just cite that exercise here in the context of smooth manifolds.}
	...
	\begin{ex} \lex{ex:submanifolds:sub_embedded:001} \tcr{rework with below exercise to make iff}
	...
	\begin{ex} \lex{ex:submanifolds:sub_embedded:002}
	...
	\begin{defn}
		Let $M, S \in \Obj(\Maninf)$ and $p \in S$. Suppose that $S \subset M$ and $S$ is an embedded submanifold of $M$. We define the \tbf{embedded tangent space of $S$ with respect to $M$ at $p$}, denoted $\wt{T}_p S$, by 
...
\begin{ex} \lex{ex:submanifolds:tangent_space:0006}
	Let $M \in \Obj(\Maninf)$. Define $\al_N \in \Iso_{\VectR}(T_{(p,p)}(M \times M), T_pM \times T_pM)$ as in \rex{ex:tangent_spaces:products:0002}. Then for each $p \in M$, $\Im \al_N \circ D\iota_{\Del_M}(p,p) = \Del_{T_p M}$.
\end{ex}

\begin{proof}
	Let $p \in M$. Set $n \defeq \dim M$. Define $F \in \Hom_{\Maninf}(M, M \times M)$ by $F \defeq (\id_M, \id_M)$. \rex{ex:immersions:0006.01} implies that $F$ is an embedding. \rex{ex:submanifolds:embedded:embedded_images:0004} implies that $F|^{\Del_M} \in \Iso_{\Maninf}(M, \Del_M)$. Set $G \defeq F|^{\Del_M}$. Let $(U, \phi) \in \MA_M$. Write $\phi = (x^1, \ldots, x^n)$. Define $(V, \psi) \in \MA_M \otimes \MA_M$ by $V \defeq U \times U$ and $\psi \defeq \phi \times \phi$. Write $\psi = (\tl{y}^1, \ldots, \tl{y}^n, \tl{z}^1, \ldots, \tl{z}^n)$.
	...
	\end{align*}
	Hence $\al_N \circ DF(p)(T_p M) = \Del_{T_p M}$. \rex{ex:submanifolds:tangent_space:0002} then implies that 
	\begin{align*}
...
%		\tcr{a previous exercise in the section on intro to submanifolds} implies that there exists $V \in \MT_S$ such that $p \in V$ and $(V, \MT_S \cap V, \MA_S|_V)$ is an embedded submanifold of $(M, \MT_M, \MA_M)$. 
	...
	\begin{proof}
		...
		Define $\al_1 \in \Iso_{\VectR}(T_{(p,q)}(S_1 \times E_1), T_pS_1 \times T_q E_1)$, $\al_2 \in \Iso_{\VectR}(T_{(p,q)}(S_2 \times E_2), T_pS_2 \times T_q E_2)$ and $\al \in \Iso_{\VectR}(T_{(p_S, p_E)} (M \times N), T_{p_S} M \times T_{p_E} N)$ as in \rex{ex:tangent_spaces:products:0002}. \rex{ex:tangent_spaces:products:0004} then implies that
	...
	\begin{ex} \lex{ex:submanifolds:transverality:0007}
		Let $M, N, S \in \Obj(\Maninf)$ and $F \in \Hom_{\Maninf}(N, M)$. Suppose that $S \subset M$, $S$ is an embedded submanifold of $M$. If $F$ is transverse to $S$, then $F^{-1}(S)$ is an embedded submanifold of $N$ and $\codim F^{-1}(S) = \codim S$. \tcr{(I think I can relax this to immersed submanifolds since locally it is embedded and this is a local result)}\\
	...
	\begin{proof}\
		\begin{enumerate}
			...
			Define $N \in \Obj(\Maninf)$ by $N \defeq \pi(p) \times \{p\}$. Define $\al_N$ as in \rex{arg1}. \rex{arg1} then implies that $N$ is an embedded submanifold of $Z$ and $\rex{ex:tangent_spaces:products:0002}$ implies that $\al_N \circ D \iota_N(p,p) = $. 
			
			We note that \rex{arg1} implies that $\Del_{S'}$ is an embedded submanifold of $S' \times S'$. Since $S'$ is an embedded submanifold of $M$ and $\Del_{S'}$
			
			\rex{ex:submanifolds:tangent_space:0006} implies that $\al_N \circ D \iota_{\Del_{S'}}(p,p) = \Del_{T_pM}$.
			
			
			and  \rex{ex:submanifolds:product_embedded:001} implies that $M \times \{p\}$ and 
			 \tcr{FINISH!!!} 
