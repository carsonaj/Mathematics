...
\begin{defn} \lex{ex:top_submanifolds:embedded:0.01}
	Let $M, N \in \Obj(\Man0)$ and $F \in \Hom_{\Top}(N, M)$. Then $F$ is said to be a $\Top$-embedding if $F|^{F(M)} \in \Iso_{\Top}(M,F(M))$.  \tcr{here $F|^{F(M)}: M \rightarrow F(M)$ is the restriction on the codomain, make sure to use standardized notation if it changes}.
\end{defn}
...
\begin{defn} \ld{def:smooth_maps_between_manifolds:0001}
	...
	\begin{itemize}
		\item \tbf{$(\MA, \MB)$-smooth} if for each $p \in M$, there exist $(U, \phi) \in \MA$ and $(V, \psi) \in \MB$ such that $p \in U$, $F(U) \subset V$ and $\psi \circ F \circ \phi^{-1}$ is smooth. 
...
\begin{proof}
	Suppose that $F$ is smooth. Let $p \in M$. By defintion, there exist $(U, \phi) \in \MA$ and $(V, \psi) \in \MB$ such that $p \in U$, $F(U) \subset V$ and $\psi \circ F \circ \phi^{-1}$ is smooth. Define $F_0: \phi(U) \rightarrow \psi(V) $ by $$ F_0 = \psi \circ F \circ \phi^{-1}$$  
...
%	\end{defn}
	...
	\begin{ex} \lex{ex:tangent_spaces:products:0006}
		Let $M,N_1, N_2 \in \Obj(\ManBndinf)$, $F_1 \in \Hom_{\ManBndinf}(M, N_1)$, $F_2 \in \Hom_{\ManBndinf}(M, N_2)$ and $p \in M$. Define $G \in \Hom_{\ManBndinf}(M, N_1 \times N_2)$ by $G \defeq (F_1, F_2)$ and define $\al_N \in \Iso_{\VectR}(T_{(F_1(p), F_2(p))} N_1 \times N_2, T_{F_1(p)} N_1 \times  T_{F_2(p)} N_2)$ as in \rex{ex:tangent_spaces:products:0002}. Then 
...
\begin{proof}\
	\begin{enumerate}
		...
		\item Suppose that $F_1$ and $F_2$ are embeddings. The previous part implies that $F_1 \times F_2$ is an immersion. Since $F_1$ and $F_2$ are embeddings, $F_1|^{F_1(M_1)} \in \Iso_{\Top}(M_1, F_1(M_1))$ and $F_2|^{F_2(M_2)} \in \Iso_{\Top}(M_2, F_2(M_2))$. \rex{} \tcr{(cite exercise here)} implies that 
		\begin{align*}
...
\begin{proof}
	...
	\begin{enumerate}
		...
		\item Suppose that $F_1$ and $F_2$ are embeddings. The previous part implies that $G$ is an immersion. Since $F_1$ and $F_2$ are embeddings, $F_1|^{F_1(M)} \in \Iso_{\Top}(M, F_1(M))$ and $F_2|^{F_2(M)} \in \Iso_{\Top}(M, F_2(M))$. \rex{} \tcr{cite exercise here} implies that 
		\begin{align*}
			G|^{G(M)} 
...
%		\tcr{Make this an exercise in the analysis notes section on topology and subspaces, then just cite that exercise here in the context of smooth manifolds.}
	...
	\begin{proof}
		...
		\begin{align*}
			...
			& \in \MT_N.
		\end{align*}
		Since $V \in \MT_S$ is arbitrary, we have that for each $V \in \MT_S$, $F^{-1}(V) \in \MT_N$. Hence $F \in \Hom_{\Top}(N, S)$. Since $S$ is an embedded submanifold of $M$, $S$ is an immersed submanifold of $M$. \rex{ex:submanifolds:introduction:0021} then implies that $F \in \Hom_{\Maninf}(N, S)$.
	\end{proof}
	...
	\begin{proof}
		...
		\begin{enumerate}
			...
			\begin{itemize}
				\item Since $(S, \MT_S, \MA_S)$ is an embedded submanifold of $(M, \MT_M, \MA_M)$, $\iota \in \Hom_{\Maninf}[(S, \MT_S, \MA_S), (M, \MT_M, \MA_M)]$. Since $\iota(S) = S$, \rex{ex:submanifolds:introduction:0022} implies that $\iota \in \Hom_{\Maninf}[(S, \MT_S, \MA_S), (S, \MT_M \cap S, \MA_M|_S)]$. 
				\item Since $(S, \MT_M \cap S, \MA_M|_S)$ is an embedded submanifold of $(M, \MT_M, \MA_M)$, $\iota^{-1} \in \Hom_{\Maninf}[(S, \MT_M \cap S, \MA_M|_S), (M, \MT_M, \MA_M)]$. Since $\iota^{-1}(S) = S$, \rex{ex:submanifolds:introduction:0022} again implies that $\iota^{-1} \in \Hom_{\Maninf}[(S, \MT_M \cap S, \MA_M|_S), (S, \MT_S, \MA_S)]$. 
			\end{itemize}
	...
	\begin{note}
		Let $M, N \in \Obj(\ManBndinf)$ and  $F \in \Hom_{\ManBndinf}(N, M)$. Suppose that $F$ is a  $\ManBndinf$-embedding. We recall that \rex{ex:top_submanifolds:embedded:0001} implies that $(F(N), \MT_M \cap F(N)) \in \Obj(\Man0)$.
	\end{note}
	...
	\begin{proof}
		Define $F \in \Hom_{\Maninf}(M, M \times M)$ by $F \defeq (\id_M, \id_M)$. \rex{ex:immersions:0006.01} implies that $F$ is an embedding. Since $F(M) = \Del_M$, \rex{ex:submanifolds:embedded:embedded_images:0004} implies that $\Del_M$ is an embedded submanifold of $M \times M$.
	\end{proof}
	...
	\begin{defn} \ld{def:submanifolds:embedded:level_sets:0011}
		Let $M, N \in \Obj(\Maninf)$ $S \subset M$ and $F \in \Hom_{\Maninf}(M,N)$. Then $S$ is said to be a \tbf{regular level set of $F$} if there exists $q \in N$ such that $q$ is a regular value of $F$ and $S = F^{-1}(\{q\})$.
	\end{defn}
	...
	\begin{proof}
		...
		\begin{align*}
			...
			& = F|_U^{-1}(\{q\}),
		\end{align*}
		\rex{ex:submanifolds:embedded:level_sets:0008} implies that $S$ satisfies the local $(m-n)$-slice condition with respect to $U$ and $S$ is a properly embedded submanifold of $U$. Since $U \in \MT_M$, $U$ is a properly embedded submanifold of $M$. Hence $F^{-1}(\{q\})$ is a properly embedded submanifold of $M$. \tcr{(flesh out some of the last details here, like composition of proper maps is proper, composition of $\Maninf$-embeddings is a $\Maninf$-embedding, etc)}
		\tcr{FINISH!!!}
	\end{proof}
	...
	\begin{proof}\
		\begin{itemize}
			\item $(\implies):$ \\
			Suppose that $N$ is an embedded submanifold of $S$. Define $F \in \Hom_{\Maninf}(N, S)$ by $F(p) \defeq p$. Since $N$ is an embedded submanifold of $S$, $F$ is a $\Maninf$-embedding. We note that $\iota_N = \iota_S \circ F$. Since $S$ is an embedded submanifold of $M$, $\iota_S$ is a $\Maninf$-embedding. \rex{} \tcr{cite exercise about composition of embeddings is embedding} then implies that $\iota_N$ is a $\Maninf$-embedding. Since $\iota_N(N) = N$, \rex{ex:submanifolds:embedded:embedded_images:0004} implies that $N$ is an embedded submanifold of $M$.
			\item $(\impliedby):$ \\
			Suppose that $N$ is an embedded submanifold of $M$. Define $F:N \rightarrow S$ by $F \defeq \iota_N|^S$. Since $\iota_N \in \Hom_{\Maninf}(N, M)$, \rex{ex:submanifolds:introduction:0022} implies that $F \in \Hom_{\Maninf}(N, S)$. Since $N$ is an embedded submanifold of $M$, $\iota_N \in \Hom_{\Maninf}(N, M)$ is a $\Maninf$-embedding. Let $p \in N$. Since $\iota_N = \iota_S \circ F$, we have that 
			\begin{align*}
				D\iota_N(p) 
	...
	\begin{proof}
		Since $\iota_E$ and $\iota_F$ are $\Maninf$-embeddings, \rex{ex:immersions:0006} implies that $\iota_E \times \iota_F$ is a $\Maninf$-embedding. Since $\iota_E \times \iota_F = \iota_{E \times F}$, $E \times F$ is an embedded submanifold of $M \times N$.  
	\end{proof}
	...
	\begin{defn}
		Let $M, S \in \Obj(\Maninf)$ and $p \in S$. Suppose that $S \subset M$ and $S$ is an embedded submanifold of $M$. We define the \tbf{embedded tangent space of $S$ with respect to $M$ at $p$}, denoted $\wt{T}_p (S, M)$, by 
	...
	\begin{proof}
		...
		\begin{enumerate}
			\item \rex{ex:submanifolds:embedded:embedded_images:0004} implies that $G \in \Iso_{\Maninf}(N, S)$. Thus $DG(p) \in \Iso_{\VectR}(T_pN, T_{F(p)}S)$.
			\item Since 
			\begin{align*}
				DF(p)
				& = D[\iota_S \circ G](p) \\
	...
	\begin{ex} \lex{ex:submanifolds:tangent_space:0002.1}
		Let $M, S \in \Obj(\Maninf)$ and $p \in S$. Suppose that $S \subset M$ and $S$ is an embedded submanifold of $M$. Then $\dim \wt{T}_p S = \dim T_pS$. 
	\end{ex}

	\begin{proof}
		Define $F \in \Hom_{\Maninf}(S, M)$ by $F \defeq \iota_S$. \rex{ex:submanifolds:tangent_space:0002} implies that
		\begin{align*}
			\dim \wt{T}_p S
			& = \dim \wt{T}_{F(p)} F(S) \\
			& = \dim T_p S.
...
\begin{ex} \lex{ex:submanifolds:tangent_space:0006}
	Let $M \in \Obj(\Maninf)$ and $p \in M$. Define $\al \in \Iso_{\VectR}(T_{(p,p)}(M \times M), T_pM \times  T_pM)$ as in \rex{ex:tangent_spaces:products:0002}. Then $\al (\wt{T}_{(p,p)} \Del_M) = \Del_{T_p M}$ and $\dim T_{(p,p) \Del_M} = \dim T_p M$.
\end{ex}

\begin{proof}
	Set $n \defeq \dim M$. Define $F \in \Hom_{\Maninf}(M, M \times M)$ by $F \defeq (\id_M, \id_M)$. \rex{ex:immersions:0006.01} implies that $F$ is an embedding. \rex{ex:submanifolds:embedded:embedded_images:0004} implies that $F|^{\Del_M} \in \Iso_{\Maninf}(M, \Del_M)$. Set $G \defeq F|^{\Del_M}$. Let $(U, \phi) \in \MA_M$. Write $\phi = (x^1, \ldots, x^n)$. Define $(V, \psi) \in \MA_M \otimes \MA_M$ by $V \defeq U \times U$ and $\psi \defeq \phi \times \phi$. Write $\psi = (\tl{y}^1, \ldots, \tl{y}^n, \tl{z}^1, \ldots, \tl{z}^n)$.
...
\begin{ex} \lex{ex:submanifolds:tangent_space:0006.1}
	Let $M, S \in \Obj(\Maninf)$ and $p \in S$. Suppose that $S \subset M$ and $S$ is an immersed submanifold of $M$. Define $\al_M \in \Iso_{\VectR}(T_{(p,p)}(M \times M), T_pM \times  T_pM)$ as in \rex{ex:tangent_spaces:products:0002}. Then for each $p \in M$, 
	$$\al_M (\wt{T}_{(p,p)} (\Del_S, M \times M)) = \Del_{\wt{T}_p (S, M)}.$$
\end{ex}

\begin{proof}
	Set $n \defeq \dim M$. Define $\al_S \in \Iso_{\VectR}(T_{(p,p)}(S \times S), T_pS \times  T_pS)$ as in \rex{ex:tangent_spaces:products:0002}. \rex{ex:submanifolds:tangent_space:0006} implies that $\al_S(\wt{T}_{(p,p)} (\Del_S, S \times S)) = \Del_{T_p S}$. \rex{ex:tangent_spaces:products:0004} then implies that $\al_M \circ D\iota_{S \times S}(p,p) = {D \iota_S(p) \times D \iota_S (p)} \circ \al_S$,
...
\begin{ex} \lex{ex:submanifolds:tangent_space:0006.2}
	Let $M, S \in \Obj(\Maninf)$ and $p \in S$. Define $\prj_1 \in \Hom_{\Maninf}(M \times M, M)$ as in \ld{arg1} \tcr{(cite def)}. Suppose that $S \subset M$ and $S$ is an immersed submanifold of $M$. Then 
...
\begin{proof}
	Define $\al_0 \in \Iso_{\VectR}(T_{(p,q)}(S \times E), T_pS \times T_q E)$ and $\al \in \Iso_{\VectR}(T_{(p, q)} (M \times N), T_p M \times T_q N)$ as in \rex{ex:tangent_spaces:products:0002}. \rex{ex:tangent_spaces:products:0004} then implies that $\al \circ D\iota_{S \times E}(p,q) = {D \iota_S(p) \times D \iota_E (q)} \circ \al_0$,
	i.e. following diagram commutes: 
	\[ 
	\begin{tikzcd}
...
%		\tcr{a previous exercise in the section on intro to submanifolds} implies that there exists $V \in \MT_S$ such that $p \in V$ and $(V, \MT_S \cap V, \MA_S|_V)$ is an embedded submanifold of $(M, \MT_M, \MA_M)$. 
	...
	\begin{ex} \lex{ex:submanifolds:transverality:0003.1}
		Let $M, S \in \Obj(\Maninf)$ and $p \in S$. Suppose that $S \subset M$, $S$ is an embedded submanifold of $M$ and $\dim S < \dim M$. Then there exists $S' \in \Obj(\Maninf)$ such that $S' \subset M$, $S'$ is an embedded submanifold of $M$, $p \in S'$ and $S$, $S'$ are strictly transverse. 
	\end{ex}
	...
	\begin{defn} \ld{def:submanifolds:transverality:0005}
		Let $M, N, S \in \Obj(\Maninf)$ and $F \in \Hom_{\Maninf}(N, M)$. Suppose that $S \subset M$, $S$ is an immersed submanifold of $M$. Then $F$ is said to be \tbf{transverse to $S$} if for each $p \in F^{-1}(S)$, 
			...
			\begin{enumerate}
				...
				\begin{itemize}
					...
					\begin{align*}
						...
						& = Z.
					\end{align*}
					\item Define $N \in \Obj(\Maninf)$ by $N \defeq N$. \rex{ex:quotients:intro:0002} implies that $\pi(p)$ is an embbeded submanifold of $M$. \rex{arg1} \tcr{(need exercise)} implies that $\{p\}$ is an embedded submanifold of $M$. \rex{ex:submanifolds:product_embedded:001} then implies that $N$ is an embedded submanifold of $M \times M$. We note that by definition, $N \subset R$. Since for each $(a,p) \in N$, 
					\begin{align*}
						\prj_2(a, p) 
						& = p \\
						& \in S',
				...
				\item Define $\al \in \Iso_{\VectR}(T_{(p,p) M \times M}, T_pM \times T_p M)$ as in \rex{ex:tangent_spaces:products:0002}. \rex{ex:submanifolds:tangent_space:0006.1} and \rex{ex:submanifolds:tangent_space:0007} then imply that
				\begin{align*}
...
\begin{ex} \lex{ex:quotients:godement:0003} \tbf{Slice Theorem:}
	Let $M, R \in \Obj(\Maninf)$. Suppose that $R$ is a properly embedded submanifold of $M \times M$, $R$ is an equivlance relation on $M$. Then for each $p \in M$, there exists $U \in \MT_M$, $S \in \Obj(\Maninf)$ and $q \in \Hom_{\Maninf}(U, S)$ such that $p \in U$, $S \subset U$ and $S$ is a properly embedded submanifold of $U$ and $(S,q)$ is a $R$-slice of $U$. 
\end{ex}

\begin{proof}
	Let $p \in M$. Define $S' \in \EmbSubmaninf(M)$ by $S' \defeq \pi(p)$. Then $\dim S' = \tcr{????}$. \rex{ex:submanifolds:transverality:0003.1} implies that there exists  $S \in \EmbSubmaninf(M)$, such that $p \in S$ and $S, S'$ are strictly transverse. Define $Z \in \EmbSubmaninf(R)$ by $Z \defeq \prj_2|_R^{-1}(S)$. \rex{ex:quotients:intro:0003} implies that \tcr{(verify assumption in this ex)} $D\prj_1|_Z(p,p) \in \Iso_{\VectR}(T_{(p,p)}Z, T_pM)$. \tcr{ref ex on local diffeomorphisms} implies that there exists \tcr{FINISH!!!}
\end{proof}
...
\begin{defn}
	...
	\begin{itemize}
		...
		$$ \phi_{23} \circ_{\LieGrp} \phi_{12} = \phi_{23} \circ_{\bSet} \phi_{12}$$
	\end{itemize}
\end{defn}
...
\end{proof}
	...
	\begin{defn} \ld{def:fiber_bundles:0001}
		Let $E, M, F \in \Obj(\bSet)$, $\pi \in \Hom_{\bSet}(E, M)$ a surjection, $U \subset M$ and $\Phi: \pi^{-1}(U) \rightarrow U \times F$. Then $(U, \Phi)$ is said to be a \tbf{local trivialization with respect to $\pi$ of $E$ over $U$ with fiber $F$}  if 
		\begin{enumerate}
	...
	\begin{ex} \lex{ex:fiber_bundles:0002}
		Let $E, M, F \in \Obj(\bSet)$ and $\pi \in \Hom_{\bSet}(E, M)$ a surjection, $U \subset M$ and $\Phi: \pi^{-1}(U) \rightarrow U \times F$ a local trivialization with respect to $\pi$ of $E$ over $U$ with fiber $F$. Then for each $A \subset U$, $$\Phi( \pi^{-1}(A)) = A \times F$$
	...
	\begin{ex} \lex{ex:fiber_bundles:0005} \tbf{$\Man0$ Fiber Bundle Chart Lemma:} \\
		Let $E \in \Obj(\bSet)$, $M, F \in \Obj(\Man0)$, $\pi: E \rightarrow M$ a surjection, $\Gam$ an index set and for each $\al \in \Gam$, $U_{\al} \subset M$ and $\Phi_{\al}: \pi^{-1}(U_{\al}) \rightarrow U_{\al} \times F$. Set $n = \dim M$ and $k = \dim F$. Suppose that 
		\begin{itemize}
	...
	\begin{ex} \lex{ex:fiber_bundles:0008} \tbf{$\Maninf$ Fiber Bundle Chart Lemma:} \\
		Let $E \in \Obj(\bSet)$, $M, F \in \Obj(\Maninf)$, $\pi: E \rightarrow M$ a surjection, $\Gam$ an index set and for each $\al \in \Gam$, $U_{\al} \subset M$ and $\Phi_{\al}: \pi^{-1}(U_{\al}) \rightarrow U_{\al} \times F$. Set $n \defeq \dim M$ and $k \defeq \dim F$. Suppose that 
		\begin{itemize}
			\item for each $\al \in \Gam$, $U_{\al} \in \MT_M$
			\item $M \subset \bigcup\limits_{\al \in \Gam} U_{\al}$ 
			\item for each $\al \in \Gam$, $(U_{\al}, \Phi_{\al})$ is a local trivialization with respect to $\pi$ of $E$ over $U_{\al}$ with fiber $F$ 
...
\begin{ex} \lex{ex:vector_bundles:0001.02} \tbf{Smooth Vector Bundle Chart Lemma:} \\
	Let $M \in \Obj(\Maninf)$ and $(E_p)_{p \in M} \subset \Obj(\VectR)$. Set $n \defeq \dim M$. Suppose that for each $p \in M$, $\dim E_p = k$. We define $E \in \Obj(\bSet)$ and $\pi \in \Hom_{\bSet}(E, M)$ by
