...
%	\end{defn}
	...
	\begin{ex} \lex{ex:submersions:0002.1}
	...
	\begin{defn} \ld{def:submersions:0003}
		Let $E, M \in \Obj(\ManBndinf)$, $\pi \in \Hom_{\ManBndinf}(E, M)$, $U \in \MT_M$ and $\sig \in \Hom_{\Maninf}(U, E)$. Then 
		\begin{itemize}
			\item $(U, \sig)$ is said to be a \tbf{smooth local section of $\pi$} if $\sig$ is a section of $\pi|_{\pi^{-1}(U)}: \pi^{-1}(U) \rightarrow U$, 
...
\begin{proof}
	...
	\begin{align*}
		...
		& = (q, \tau(q)(v)),
	\end{align*}
	is a vector space isomorphism. Thus $\tau(q)$ is a linear bijection and therefore $\tau(q) \in GL(k, \R)$. \tcr{need to show $\tau$ is smooth, use hint in book, make exercise in a previous section about actions}
\end{proof}
