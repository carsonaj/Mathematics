...
\end{proof}		
	...
	\begin{ex}
		Let $A \subset \H^n$. Suppose that $A$ is open in $\H^n$. Then $A$ is open in $\R^n$ iff $A \cap \p \H^n = \varnothing$. \\
		\tbf{Hint:} simply connected?
		\tcb{FINISH!!!}
	\end{ex}	

	\begin{proof}
		Suppose that $A$ is open in $\R^n$. For the sake of contradiction, suppose that $A \cap \p \H^n \neq \varnothing$. Then there exists $x \in A$ such that $x \in \p \H^n$. Since $A$ is open in $\R^n$, there exists $B \subset A$ such that $B$ is open in $\R^n$, $p \in B$ and $B$ is simply connected. Set $B' \defeq B \setminus \{x\}$. Then $B'$ is not simply connected.  get a simply connected ball around $p$
		\tcb{FINISH!!!}
	\end{proof}
	...
	\begin{defn}
		...
		\begin{itemize}
			\item \tbf{interior chart} if $\phi(U) \cap \p \H^n = \varnothing$
			\item \tbf{boundary chart} if $\phi(U) \cap \p \H^n \neq \varnothing$
		\end{itemize}
	...
	\begin{proof}\
		\begin{enumerate}
			\item By definition, $X_{\Int}(M) \cup X_{\p}(M) \subset X(M)$. Let $(U, \phi) \in X(M)$. Since $(U, \phi)$ is a coordinate chart on $M$, $\phi(U)$ is open in $\H^n$. If  $\phi(U) \cap \p \H^n = \varnothing$, then 
			\begin{align*}
				(U, \phi) 
				& \in X_{\Int}(M) \\
			...
			\item For the sake of contradiction, suppose that $X_{\Int}(M) \cup X_{\p}(M) \neq \varnothing$. Then there exists $(U, \phi) \in X(M)$ such that $(U, \phi) \in X_{\Int}(M)$ and $(U, \phi) \in X_{\p}(M)$. Therefore $\phi(U) \cap \p \H^n = \varnothing$ and $\phi(U) \cap \p \H^n \neq \varnothing$. This is a contradiction. Hence $X_{\Int}(M) \cup X_{\p}(M) = \varnothing$.
		\end{enumerate}
	\end{proof}
	...
	\begin{proof}
		Suppose that $(U, \phi) \in X_{\Int}(M)$. Then $\phi(U) \cap \p \H^n = \varnothing$. Since $\phi(U)$ is open in $\H^n$, \tcb{a previous exercise} implies that $\phi(U)$ is open in $\R^n$. 
		Conversely, suppose that $\phi(U)$ is open in $\R^n$. Since $\phi(U)$ is open in $\H^n$, \tcb{a previous exercise} implies that $\phi(U) \cap \p \H^n = \varnothing$. Thus $(U, \phi) \in X_{\Int}(M)$.
	\end{proof}
...
\begin{ex}
	\tcb{show $\H^m \times \Int \H^n \in \Man0$}.
\end{ex}

\begin{ex}
	Let $m \in \N$ and $n \in \N_0$. Then there exists $\lam: \H^m \times \Int \H^n \rightarrow \H^{m+n}$ such that 
	\begin{enumerate}
		\item $\lam$ is a $(\MT_{\H^m} \otimes \MT_{\Int \H^n}, \H^{m+n})$-homeomorphism
		\item $\lam(\p \H^m \times \Int \H^n) = \p \H^{m+n}$
	\end{enumerate}
\end{ex}

\begin{proof}
	Define $\lam: \H^m \times \Int \H^n \rightarrow \H^{m+n}$ by 
	$$\lam((x^1, \ldots, x^{m-1}, x^m), (y^1, \ldots, y^n)) = (x^1, \ldots, x^{m-1}, \log y^1, \ldots, \log y^n, x^m)$$
	\begin{enumerate}
		\item Clearly $\lam$ is a homeomorphism.
		\item Clearly $\lam(\p \H^m \times \Int \H^n) = \p \H^{m+n}$
	\end{enumerate}
\end{proof}

\begin{defn}
	We define 
	$$\Aut_{0}^{m,n} = \{\lam: \H^m \times \Int \H^n \rightarrow \H^{m+n} : \lam \in \Aut_{\Man0}(\H^m \times \Int \H^n, \H^{m+n}) \text{ and } \lam(\p \H^m \times \Int \H^n) = \p \H^{m+n} \}$$
\end{defn}

\begin{ex}
	Let $m \in \N$ and $n \in \N_0$. Then $\Aut_{0}^{m,n} \neq \varnothing$.
\end{ex}
...
\begin{ex}
	Let $(M, \MT_M)$, $(N, \MT_N)$ be topological manifolds. Set $m = \dim M$ and $n = \dim N$. Suppose that $\p N = \varnothing$. Then for each $\lam \in \Aut_0^{m,n}$, $(U, \phi) \in X^m(M, \MT_M)$ and $(V, \psi) \in X^n(N, \MT_N)$, 
	$$(U \times V, \lam|_{\phi(U) \times \psi(V)} \circ [\phi \times \psi]) \in X^{m+n}(M \times N, \MT_M \otimes \MT_N)$$ 
\end{ex}

\begin{proof}
	Let $\lam \in \Aut_0^{m,n}$, $(U, \phi) \in X^m(M, \MT_M)$ and $(V, \psi) \in X^n(N, \MT_N)$. 
	\begin{itemize}
		\item Since $U \in \MT_M$ and $V \in \MT_N$, $U \times V \in \MT_M \otimes \MT_N$. 
		\item Since $\phi(U) \in \MT_{\H^m}$ and $\psi(V) \in \MT_{\H^n}$, $\phi(U) \times \psi(V) \in \MT_{\H^m} \otimes \MT_{\H^n}$. Since $\p N = \varnothing$, $(V, \psi) \in X^n_{\Int}(N, \MT_N)$ and therefore $\psi(V) \subset \Int \H^n$. Since $\lam: \H^m \times \Int \H^n \rightarrow \H^{m+n}$ is a homeomorphism, 
		\begin{align*}
			\lam|_{\phi(U) \times \psi(V)} \circ [\phi \times \psi](U \times V)
			& = \lam (\phi(U) \times \psi(V)) \\
			& \in \MT_{\H^{m+n}}
		\end{align*}
		\item Since $\phi:U \rightarrow \phi(U)$ is a $(\MT_M \cap U, \MT_{\H^m} \cap \phi(U))$-homeomorphism and $\psi: V \rightarrow \psi(V)$ is a $(\MT_N \cap V, \MT_{\H^n} \cap \psi(V))$-homeomorphism, \tcb{an exercise in the section on product topologies in the analysis notes} implies that $\phi \times \psi: U \times V \rightarrow \phi(U) \times \phi(V)$ is a $([\MT_M \otimes \MT_N] \cap [U \times V], [\MT_{\H^m} \otimes \MT_{\H^n}] \cap [\phi(U) \times \psi(V)])$-homeomorphism. Since $\lam|_{\phi(U) \times \psi(V)}: \phi(U) \times \psi(V) \rightarrow \lam(\phi(U) \times \psi(V))$ is a $([\MT_{\H^m} \otimes \MT_{\Int \H^n}] \cap[ \phi(U) \times \psi(V)], \MT_{\H^{m+n}} \cap \lam(\phi(U) \times \psi(V)))$-homeomorphism, $\lam|_{\phi(U) \times \psi(V)} \circ (\phi \times \psi)$ is a $([\MT_M \otimes \MT_N] \cap [U \times V], \MT_{\H^{m+n}} \cap \lam(U \times V))$-homeomorphism.
	\end{itemize}
	Hence $(U \times V,\lam|_{\phi(U) \times \psi(V)} \circ [\phi \times \psi]) \in X^{m+n}(M \times N, \MT_M \otimes \MT_N)$. Since $\lam \in \Aut_0^{m,n}$, $(U, \phi) \in X^m(M, \MT_M)$ and $(V, \psi) \in X^n(N, \MT_N)$ are arbitrary, we have that for each $\lam \in \Aut_0^{m,n}$, $(U, \phi) \in X^m(M, \MT_M)$ and $(V, \psi) \in X^n(N, \MT_N)$, $$(U \times V, \lam|_{\phi(U) \times \psi(V)} \circ [\phi \times \psi]) \in X^{m+n}(M \times N,  \MT_M \otimes \MT_N)$$ 
\end{proof}

\begin{ex}
	Let $M$, $N$ be topological manifolds. Set $m = \dim M$ and $n = \dim N$. Suppose that $\p N = \varnothing$. Then for each $\lam \in \Aut_0^{m,n}$, $(U, \phi) \in X^m_p(M, \MT_M)$ and $(V, \psi) \in X^n(N, \MT_N)$, 
	$$(U \times V, \lam|_{\phi(U) \times \psi(V)} \circ [\phi \times \psi]) \in X^{m+n}_{\p}(M \times N,  \MT_M \otimes \MT_N)$$ 
\end{ex}

\begin{proof}
	Let $\lam \in \Aut_0^{m,n}$, $(U, \phi) \in X^m_{\p}(M)$ and $(V, \psi) \in X^n(N)$. Define $\eta: U \times V \rightarrow \lam(\phi(U) \times \psi(V))$ by 
	$$\eta \defeq \lam|_{\phi(U) \times \psi(V)} \circ [\phi \times \psi]$$ 
	Since $(U, \phi) \in X^m_{\p}(M)$, $\phi(U) \cap \p \H^m \neq \varnothing$. Then there exists $p \in U$ such that $\phi(p) \in \p \H^m$. So $\eta(p,q) \in \p \H^{m+n}$. Thus $\eta(U \times V) \cap \p \H^{m+n} \neq \varnothing$ and $(U \times V, \eta) \in X^{m+n}_{\p}(M \times N)$. Since $\lam \in \Aut_0^{m,n}$, $(U, \phi) \in X^m_{\p}(M)$ and $(V, \psi) \in X^n(N, \MT_N)$ are arbitrary, we have that for each $\lam \in \Aut_0^{m,n}$, $(U, \phi) \in X^m_p(M, \MT_M)$ and $(V, \psi) \in X^n(N, \MT_N)$, 
	$$(U \times V, \lam|_{\phi(U) \times \psi(V)} \circ [\phi \times \psi]) \in X^{m+n}_{\p}(M \times N,  \MT_M \otimes \MT_N)$$ 
\end{proof}
...
\begin{proof}
	 ...
	 \begin{enumerate}
	 	...
	 	\begin{itemize}
	 		...
	 		\item Let $a \in M \times N$. Then there exist $p \in M$ and $q \in N$ such that $a = (p,q)$. Since $M$ and and $N$ are locally Euclidean, there exist $(U, \phi) \in X^m(M)$ and $(V, \psi) \in X^n(N)$ such that $p \in U$ and $q \in V$. Then $(p,q) \in U \times V$. \tcb{The previous exercise} implies that $(U \times V, \lam \circ [\phi \times \psi]) \in X^{m+n}(M \times N)$. Since $a \in M \times N$ is arbitrary, $M \times N$ is locally Euclidean of dimension $m+n$.   
	 	\end{itemize}
 		Thus $M \times N$ is an $(m+n)$-dimensional topological manifold.
	 	\item 
	 	\begin{itemize}
	 		\item Let $a \in \p(M \times N)$. Then there exists $p \in M$ and $q \in N$ such that $a = (p,q)$. Since $(M, \MT_M)$ and and $(N)$ are locally Euclidean, there exist $(U, \phi) \in X^m(M)$ and $(V, \psi) \in X^n(N)$ such that $p \in U$ and $q \in V$. Define $\eta: U \times V \rightarrow \lam(\phi(U) \times \psi(V))$ by 
	 		$$\eta \defeq \lam|_{\phi(U) \times \psi(V)} \circ [\phi \times \psi]$$ 
	 		\tcb{The previous exercise} implies that $\eta \in X^{m+n}(M \times N)$. Since $(p, q) \in \p (M \times N)$, \tcb{a previous exercise} implies that $\eta \in X^{m+n}_{\p}(M \times N)$ and $\eta(p,q) \in \p \H^{m+n}$. Therefore 
	 		\begin{align*}
	 			\phi \times \psi(p,q)
	 			& = \lam|_{\phi(U) \times \psi(V)}^{-1} \circ \eta \\
	 		...
	 		\item Let $a \in \p M \times N$. Then there exists $p \in \p M$ and $q \in N$ such that $a = (p,q)$. By definition, there exists $(U, \phi) \in X^m_{\p}(M)$ and $(V, \psi) \in X^n(N)$ such that $p \in U$, $q \in V$ and $\phi(p) \in \p \H^m$. Since $\p N = \varnothing$, $\psi(q) \in \Int \H^n$. Define $\eta: U \times V \rightarrow \lam(\phi(U) \times \psi(V))$ by 
	 		$$\eta \defeq \lam|_{\phi(U) \times \psi(V)} \circ [\phi \times \psi]$$ 
	 		\tcb{A previous exercise} implies that $(U \times V, \eta) \in X^{m+n}(M \times N \otimes \MT_N)$. Then
	 		\begin{align*}
	 			\eta(a)
	 			& = \eta(p,q) \\
	 			& = \lam(\phi(p), \psi(q)) \\
...
\end{proof}
	...
	\begin{defn}
		Let $n \in \N_0$. We define the \tbf{standard smooth structure} on $\R^n$, denoted $\MA_{\R^n}$, by $\MA_{\R^n} = \al_{\R^n}(\R^n, \id_{\R^n})$. 
	\end{defn}
...
\tcb{in order for a smooth structure on a subspace to make sense, we need to have a notion of smoothness of a map $A \rightarrow \R^k$ to make sense when $A \subset \R^n$. But we already have this, think of them as banach spaces, use the definitions of differentiation at a point in $A$ in terms of locally extend to an open neighborhood containing the point}.
...
\begin{proof}\
	\begin{enumerate}
		\item Set $\MB = \{(V, \psi) \in \MA: V \subset U\}$. Let $(U', \phi) \in \MA|_U$, $(V, \psi) \in \MA$ and $a \in \phi(U' \cap V)$. Set $p = \phi^{-1}(a)$. \tcb{A previous exercise} implies that $\MB$ is a smooth atlas on $U$. Thus there exists $(W, \al) \in \MB$ such that $p \in W$. Set $A \defeq W \cap U' \cap V$ and $A_0 \defeq \phi(A)$. Then $p \in A$, $a \in A_0$, $A$ is open in $M$ and $A_0$ is open in $\phi(U' \cap V)$. Define $f: \phi(W \cap U') \rightarrow \al(W \cap U')$, $g: \al(W \cap V) \rightarrow \psi(W \cap V)$ and $h: \phi(U' \cap V) \rightarrow \psi(U' \cap V)$ by $f \defeq \al|_{W \cap U'} \circ \phi|_{W \cap U'}^{-1}$, $g \defeq \psi|_{W \cap V} \circ \al|_{W \cap V}^{-1}$ and $h \defeq \psi_{U' \cap V} \circ \phi|_{U' \cap V}^{-1}$. Since $\MB \subset \MA$, $g$ is smooth. Since $\MB \subset \MA|_U$, $f$ is smooth. \tcb{An exercise in the section on differentiability on subspaces} implies that $f|_{A_0}$ is smooth. Since $h|_{A_0} = g \circ f|_{A_0}$, \tcb{an exercise in the section on differentiability} implies that $h|_{A_0}$ is smooth. Since $a \in \phi(U' \cap V)$ is arbitrary, we have that for each $a \in \phi(U' \cap V)$, there exists $A_0 \subset \phi(U' \cap V)$ such that $a \in A_0$, $A_0$ is open in $\phi(U' \cap V)$ and $h|_{A_0}$ is smooth. \tcb{An exercise in the section on differentiability on subspaces} implies that $h$ is smooth. Similarly $h^{-1}$ is smooth. Thus $h$ is a diffeomorphism. Therefore $(V, \psi)$ and $(U', \phi)$ are smoothly compatible. Since $(V, \psi) \in \MA$ is arbitrary, we have that $\{(U', \phi)\} \cup \MA$ is a smooth atlas. Since $\MA$ is maximal, $\{(U', \phi)\} \cup \MA = \MA$. Thus $(U', \phi) \in \MA$. Since $(U', \phi) \in \MA|_U$ is arbitrary, we have that $\MA|_U \subset \MA$. 
		\item By definition, 
		\begin{align*}
			\MB
			& \subset \al_U(\MB) \\
...
\section{To Do}
\tcb{Define a $\E$-space as an open subset of a real finite-dim vector space and define differentiation on these and show that products of $\E$-spaces are $\E$-spaces, then define differentiation on subspaces of euclidean spaces and define $\H$-spaces as half-spaces of $\E$-spaces}

\begin{ex}
	\tcb{show $\H^m \times \Int \H^n \in \Maninf$}.
\end{ex}

\begin{ex}
	Let $m \in \N$ and $n \in \N_0$. Then there exists $\lam: \H^m \times \Int \H^n \rightarrow \H^{m+n}$ such that 
	\begin{enumerate}
		\item $\lam$ is a diffeomorphism \tcb{(need to define differentiation on product spaces)}
		\item $\lam(\p \H^m \times \Int \H^n) = \p \H^{m+n}$
	\end{enumerate}
\end{ex}
...
\begin{proof}
	...
	$$\lam((x^1, \ldots, x^{m-1}, x^m), (y^1, \ldots, y^n)) = (x^1, \ldots, x^{m-1}, \log y^1, \ldots, \log y^n, x^m)$$
	\begin{enumerate}
		\item Clearly $\lam$ is a diffeomorphism \tcb{(show in detail that locally can be extended to smooth function on open neighborhood for each boundary point)}.
		\item Clearly $\lam(\p \H^m \times \Int \H^n) = \p \H^{m+n}$
	\end{enumerate}
\end{proof}
...
\begin{defn}
	We define 
	$$\Aut_{\infty}^{m,n} = \{\lam: \H^m \times \Int \H^n \rightarrow \H^{m+n} : \lam \in \Aut_{\Maninf}(\H^m \times \Int \H^n, \H^{m+n}) \text{ and } \lam(\p \H^m \times \Int \H^n) = \p \H^{m+n} \}$$
\end{defn}

\begin{ex}
	Let $m \in \N$ and $n \in \N_0$. Then $\lam \in \Aut_{\infty}^{m,n} \neq \varnothing$.
\end{ex}
...
\begin{defn}
	Let $M$, $N$ be smooth manifolds of dimension $m$ and $n$ respectively, $\MA \subset X^m(M)$ and $\MB \subset X^n(N)$. Suppose that $\MA$ and $\MB$ are smooth atlases on $M$ and $N$ respectively and $\p N = \varnothing$. We define the \tbf{product atlas of $\MA$ and $\MB$ on $M \times N$}, denoted $\MA \otimes' \MB$, by 
	$$\MA \otimes' \MB = \{(U \times V, \lam|_{\phi(U) \times \psi(V)} \circ [\phi \times \psi]): \lam \in \Aut_{\infty}^{m,n} \text{, }(U, \phi) \in \MA \text{ and } (V, \psi) \in \MB\}$$ 
\end{defn}

\begin{ex}
	Let $M$, $N$ be smooth manifolds of dimension $m$ and $n$ respectively, $\MA \subset X^m(M)$ and $\MB \subset X^n(N)$. Suppose that $\MA$ and $\MB$ are smooth atlases on $M$ and $N$ respectively and $\p N = \varnothing$. Then $\MA \otimes' \MB$ is a smooth atlas on $M \times N$. 
\end{ex}

\begin{proof}\
	\begin{itemize}
		\item \tcb{An exercise in the section on products of topological manifolds} implies that $\MA \otimes' \MB$ is an atlas on $M \times N$. 
		\item Let $(W_1, \eta_1), (W_2, \eta_2) \in \MA \otimes' \MB$. Then there exist $\lam_1, \lam_2 \in \Aut_{\infty}^{m,n}$, $(U_1, \phi_1), (U_2, \phi_2)\in \MA$ and $(V_1, \psi_1), (V_2, \psi_2) \in \MB$ such that $W_1 = U_1 \times V_1$, $W_2 = U_2 \times V_2$, $\eta_1 = \lam_1|_{\phi_1(U_1) \times \psi_1(V_1)} \circ [\phi_1 \times \psi_1]$ and $\eta_2 = \lam_2|_{\phi_2(U_2) \times \psi_2(V_2)} \circ [\phi_2 \times \psi_2]$. For notational convenience, set $U \defeq U_1 \cap U_2$ and $V \defeq V_1 \cap V_2$. Then $W_1 \cap W_2 = U \cap V$ and
		\begin{align*}
			\eta_2|_{W_1 \cap W_2} \circ \eta_1|_{W_1 \cap W_2}^{-1}
			& = \eta_2|_{U \cap V} \circ \eta_1|_{U \cap V}^{-1} \\
			& = \lam_2|_{\phi_2(U) \times \psi_2(V)} \circ [\phi_2 \times \psi_2]|_{U \times V} \circ [\phi_1 \times \psi_1]|_{U \times V}^{-1} \circ \lam_1|_{\phi_1(U) \times \psi_1(V)}^{-1} \\
			& = \lam_2|_{\phi_2(U) \times \psi_2(V)} \circ [\phi_2|_U \times \psi_2|_V] \circ [\phi_1|_U^{-1} \times \psi_1|_V^{-1}] \circ \lam_1|_{\phi_1(U) \times \psi_1(V)}^{-1} \\
			& = \lam_2|_{\phi_2(U) \times \psi_2(V)} \circ [(\phi_2|_U \circ \phi_1|_U^{-1}) \times( \psi_2|_V \circ \psi_1|_V^{-1})] \circ \lam_1|_{\phi_1(U) \times \psi_1(V)}^{-1} 
		\end{align*} 
		Since $(U_1, \phi_1)$ and $(U_2, \phi_2)$ are smoothly compatible, $\phi_2|_U \circ \phi_1|_U^{-1}$ is a diffeomorphism. Similarly, $\psi_2|_V \circ \psi_1|_V^{-1}$ is a diffeomorphism. Thus $(\phi_2|_U \circ \phi_1|_U^{-1}) \times( \psi_2|_V \circ \psi_1|_V^{-1})$ is a diffeomorphism. Since $\lam_1|_{\phi_1(U) \times \psi_1(V)}^{-1}$ and $\lam_2|_{\phi_2(U) \times \psi_2(V)}$ are diffeomorphisms, we have that $\eta_2|_{W_1 \cap W_2} \circ \eta_1|_{W_1 \cap W_2}^{-1}$ is a diffeomorphism. Hence $(W_1, \eta_1)$ and $(W_2, \eta_2)$ are smoothly compatible. Since $(W_1, \eta_1), (W_2, \eta_2) \in \MA \otimes' \MB$ are arbitrary, we have that $\MA \otimes' \MB$ is smooth. 
	\end{itemize}
\end{proof}
...
\begin{defn}
	...
	$$\MA \otimes \MB = \al_{M \times N}(\MA \otimes' \MB)$$ 
...
\begin{proof}
	...
	By definition, $F_0$ is smooth. \tcb{An exercise in the section on differentiation in subspaces} implies that $F_0$ is continuous. Since $\phi$ and $\psi$ are homeomorphisms and $F|_{U} = \psi^{-1} \circ F_0 \circ \phi$, we have that $F|_{U}$ is continuous. In particular, $F$ is continuous at $p$. Since $p \in M$ is arbitrary, $F$ is continuous.
\end{proof}

\begin{ex} \tbf{Equivalence of Smoothness:} \\
...
\begin{proof} Set $m \defeq \dim M$ and $n \defeq \dim N$.
	\begin{enumerate}
		\item $(1) \implies (2)$: \\
		Suppose that $F$ is smooth. Let $\MA_0 \subset \MA$ and $\MB_0 \subset \MB$. Suppose that $\MA_0$ is an atlas on $M$ and $\MB_0$ is an atlas on $N$. Let $(U_0, \phi_0) \in \MA_0$ and $(V_0, \psi_0) \in \MB_0$. Since $\MA_0 \subset \MA$ and $\MB_0 \subset \MB$, we have that $(U_0, \phi_0) \in \MA$ and $(V_0, \psi_0) \in \MB$. Since $F$ is smooth, $F$ is continuous and therefore $U_0 \cap F^{-1}(V_0)$ is open in $M$. Define $F_0:\phi_0(U_0 \cap F^{-1}(V_0)) \rightarrow \psi_0(V_0)$ by $F_0 \defeq \psi_0 \circ F \circ \phi_0|_{U_0 \cap F^{-1}(V_0)}^{-1}$. Let $a \in \phi_0(U_0 \cap F^{-1}(V_0))$. Define $p \in M$ by $p \defeq \phi_0^{-1}(a)$. Since $F$ is smooth, by definition 
		%\tcb{$(2)$ in the previous exercise on smooth equivalence} implies that 
		there exists $(U_1, \phi_1) \in \MA$ and $(V_1, \psi_1) \in \MB$ such that $p \in U_1$, $F(p) \in V_1$, $F(U_1) \subset V_1$ and $\psi_1 \circ F \circ \phi_1^{-1}$ is smooth. Define $U \subset M$, $\al: \phi_1(U_0 \cap U_1) \rightarrow \phi_0(U_0 \cap U_1)$, $\be: \psi_1(V_0 \cap V_1) \rightarrow \psi_0(V_0 \cap V_1)$ and $F_1: \phi_1(U_1) \rightarrow \psi_1(V_1)$ by $U = U_0 \cap U_1 \cap F^{-1}(V_0 \cap V_1)$, $\al = \phi_0|_{U_0 \cap U_1} \circ \phi_1|_{U_0 \cap U_1}^{-1}$, $\be = \psi_0|_{V_0 \cap V_1} \circ \psi_1|_{V_0 \cap V_1}^{-1}$ and $F_1 = \psi_1 \circ F \circ \phi_1^{-1}$.
		...
		\begin{itemize}
			...
			\item since $F_1$ is smooth, \tcb{an exercise in the section on differentiation on subspaces} implies that $F_1|_{\phi_1(U)}$ is smooth
			\item since $\al^{-1}$ is smooth, \tcb{an exercise in the section on differentiation on subspaces} implies that $ \al|_{\phi_1(U)}^{-1}$ is smooth
			\item since $F_0|_{\phi_0(U)} = \be \circ F_1 \circ \al|_{\phi_1(U)}^{-1}$, we have that $F_0|_{\phi_0(U)}$ is smooth
		\end{itemize}
		Since $a \in \phi_0(U_0 \cap F^{-1}(V_0))$ is arbitrary, we have that for each $a \in \phi_0(U_0 \cap F^{-1}(V_0))$, there exists $A \subset \phi_0(U_0 \cap F^{-1}(V_0))$ such that $a \in A$, $A$ is open in $\phi_0(U_0 \cap F^{-1}(V_0))$ and $F_0|_A$ is smooth. \tcb{An exercise in the section on differentiation on subspaces} implies that $F_0$ is smooth. 
		...
		\begin{itemize}
			\item Let $p \in M$. By assumption, there exist $(U, \phi) \in \MA$ and $(V, \psi) \in \MB$ such that $p \in U$, $F(p) \in V$, $U \cap F^{-1}(V)$ is open in $M$ and $\psi \circ F \circ \phi|_{U \cap F^{-1}(V)}^{-1}$ is smooth. Define $A \subset M$, $A_1 \subset \H^m$ and $F_1: A_1 \rightarrow \R^n$ by $A \defeq U \cap F^{-1}(V)$, $A_1 \defeq \phi(A)$ and $F_1 \defeq \psi \circ F \circ \phi|_{A}^{-1}$. Since $F_1$ is smooth, \tcb{an exercise in the section on differentiability on subspaces} implies that $F_1:A_1 \rightarrow \R^n$ is continuous. Since $\phi|_{A}$ and $\psi$ are homeomorphisms, 
			\begin{align*}
				F|_A
...
\begin{ex} \tbf{Locality of Smoothness:} \\
	Let $(M, \MA)$ and $(N, \MB)$ be smooth manifolds and $F: M \rightarrow N$. Then the following are equivalent: 
	\begin{enumerate}
		\item $F$ is smooth
...
\begin{proof}
	...
	\begin{align*}
		...
		& = \al^{-1} 
	\end{align*}
	Since $\al$ is a diffeomorphism, we have that $\id_{\phi(U)} \circ \phi \circ \psi|_{V \cap \phi^{-1}(\phi(U))}^{-1}$ and $\psi \circ \phi^{-1} \circ \id_{\phi(U)}|_{\phi(U) \cap (\phi^{-1})^{-1}(V)}$ are smooth. Since $\MA_{\phi(U)} = \al(\id_{\phi(U)})$, $\MA = \al(\MA)$ and $(V, \psi) \in \MA$ is arbitrary, \tcb{a previous exercise about smoothness depending on a smooth atlas} implies that $\phi$ and $\phi^{-1}$ are smooth. Hence $\phi$ is a diffeomorphism. 
\end{proof}
...
\begin{ex}
	Let $(M,\MA)$, $(N, \MB)$, $(E, \MC)$ be smooth manifolds, $\MA_0 \subset \MA$, $\MB_0 \subset \MB$, $\MC_0 \subset \MC$ and $F: M \times N \rightarrow E$. Suppose that $\p N = \varnothing$, $\MA_0$ is an atlas on $M$, $\MB_0$ is an atlas on $N$ and $\MC_0$ is an atlas on $E$. Then $F$ is smooth iff for each $(U, \phi) \in \MA_0$, $(V, \psi) \in \MB_0$, $(W, \chi) \in \MC_0$, $\chi \circ F \circ (\phi \times \psi)|_{(U \times V) \cap F^{-1}(W)}^{-1}$ is smooth. 
\end{ex}
...
\begin{proof}
	...
	\begin{itemize}
		\item $(\implies)$: \\
		Suppose that $F$ is smooth. Let $(U, \phi) \in \MA_0$, $(V, \psi) \in \MB_0$ and $(W, \chi) \in \MC_0$. \tcb{An exercise in the section on product manifolds} implies that there exists $\lam: \H^m \times \Int \H^n \rightarrow \H^{m+n}$ such that $\lam$ is a diffeomorphism and $\lam(\p \H^m \times \Int \H^n) = \p \H^{m+n}$. Set $\eta \defeq \lam|_{\phi(U) \times \psi(V)} \circ (\phi \times \psi)$. \tcb{An exercise in the section on product manifolds} implies that $\eta \in \MA \otimes \MB$. Since $F$ is smooth \tcb{ and exercise in the section on smooth maps} implies that $\chi \circ F \circ \eta|_{(U \times V) \cap F^{-1}(W)}^{-1}$ is smooth. Since
		$$ \chi \circ F \circ (\phi \times \psi)|_{(U \times V) \cap F^{-1}(W)}^{-1} = \chi \circ F \circ \eta|_{(U \times V) \cap F^{-1}(W)}^{-1} \circ \lam|_{(\phi(U) \times \phi(V)) \cap (\phi \times \psi)(F^{-1}(W))} $$
		we have that $\chi \circ F \circ (\phi \times \psi)|_{(U \times V) \cap F^{-1}(W)}^{-1}$ is smooth. 
		
		Since $(U, \phi) \in \MA_0$, $(V, \psi) \in \MB_0$ and $(W, \chi) \in \MC_0$ are arbitrary, we have that for each $(U, \phi) \in \MA_0$, $(V, \psi) \in \MB_0$, $(W, \chi) \in \MC_0$, $\chi \circ F \circ (\phi \times \psi)|_{(U \times V) \cap F^{-1}(W)}^{-1}$ is smooth. 
		\item $(\impliedby)$: \\
		Suppose that for each $(U, \phi) \in \MA_0$, $(V, \psi) \in \MB_0$, $(W, \chi) \in \MC_0$, $\chi \circ F \circ (\phi \times \psi)|_{(U \times V) \cap F^{-1}(W)}^{-1}$ is smooth. Let $(U, \phi) \in \MA_0$, $(V, \psi) \in \MB_0$, $(W, \chi) \in \MC_0$ and $\lam \in \Aut_{\infty}^{m, n}$. 
		
		 Set $G \defeq \chi \circ F \circ (\phi \times \psi)|_{(U \times V) \cap F^{-1}(W)}^{-1}$. By assumption, $G$ is smooth. Since $\lam$ is a diffeomorphism, $G \circ \lam|_{(\phi(U) \times \psi(V)) \cap (\phi \times \psi)(F^{-1}(V))}^{-1}$ is smooth. Since 
		$$\chi \circ F \circ [\lam|_{\phi(U) \times \psi(V)} \circ (\phi \times \psi)]|_{(U \times V) \cap F^{-1}(W)}^{-1} = G \circ \lam|_{(\phi(U) \times \psi(V)) \cap (\phi \times \psi)(F^{-1}(V))}^{-1}$$
		we have that $\chi \circ F \circ [\lam|_{\phi(U) \times \psi(V)} \circ (\phi \times \psi)]|_{(U \times V) \cap F^{-1}(W)}^{-1}$ is smooth. 
		
		\tcb{An exercise in the section on products of manifolds} implies that $\{(U \times V, \lam|_{\phi(U) \times \psi(V)} \circ (\phi \times \psi)): \text{$\lam \in \Aut_{\infty}^{m,n}$, $(U, \phi) \in \MA_0$ and $(V, \psi) \in \MB_0$} \}$ is an atlas on $M \times N$. 
		
		Since $(U, \phi) \in \MA_0$, $(V, \psi) \in \MB_0$, $(W, \chi) \in \MC_0$ and $\lam \in \Aut_{\infty}^{m, n}$ are arbitrary, \tcb{an exercise in the section on smooth maps} implies that $F$ is smooth. 
	\end{itemize}
\end{proof}
...
\begin{ex}
	Let $M$ and $N$ be smooth manifolds. Then $\pi_M: M \times N \rightarrow M$ and $\pi_N: M \times N \rightarrow N$ are smooth.
\end{ex}

\begin{proof}
	Set $m = \dim M$ and $n = \dim N$. Let $(U, \phi)$, $(U', \phi') \in \MA_M$ and $(V, \psi) \in \MA_N$. Then for each $(a,b) \in \phi(U) \times \psi(V)$
	\begin{align*}
	...
	where $\prj_1: \R^m \times \R^n \rightarrow \R^m$ is the usual projection map. Since $(U, \phi)$, $(U', \phi') \in \MA_M$, $(U, \phi)$ and $(U', \phi')$ are smoothly compatible. Hence $\phi'|_{U \cap U'} \circ \phi|_{U \cap U'}^{-1}$ is smooth and therefore $\phi'|_{U' \cap U} \circ \pi_M \circ [\phi \times \psi]^{-1}|_{\phi(U) \times \psi(V)}$ is smooth. Since \tcb{fix here } and $(V, \psi) \in \MA_N$ are arbitrary, we have that $\pi_M: M \times N \rightarrow M$ is smooth.  we have that $(U, \phi)$ and $(U', \phi')$ are smoothly compatible. Thus $\phi'|_{U \cap U'} \circ \phi^{-1}|_{U \cap U'}^{-1}$ is smooth.  
\end{proof}
...
\begin{ex}
	Let $M$ and $N$ be smooth manifolds and $(p,q) \in M \times N$. Then $\iota^M_q: M \rightarrow M \times N$ and $\iota^N_p: N \rightarrow M \times N$ are smooth
	\begin{enumerate}
		\item $\iota^M_q: M \rightarrow M \times N$ and $\iota^N_p: N \rightarrow M \times N$ are smooth
		\item 
	\end{enumerate}
\end{ex}
...
\newpage
\begin{defn}
	Let $(M, \MA)$ and $(N, \MB)$ be smooth manifolds. We define the \tbf{canonical projection maps}, denoted $\pi_M: M \times N \rightarrow M$ and  $\pi_N: M \times N \rightarrow N$, by $\pi_M(p,q) = p$ and $\pi_N(p,q) = q$ respectively. 
\end{defn}

\begin{ex}
	Let $(M, \MA)$ and $(N, \MB)$ be smooth manifolds. Then $\pi_M: M \times N \rightarrow M$ and $\pi_N: M \times N \rightarrow N$ are smooth.
\end{ex}

\begin{proof}
	Let $(U_0, \phi_0) \in \MA$, $(V_0, \psi_0), (V_1, \psi_1) \in \MB$ and $\lam \in \Aut_{\infty}^{m,n}$. Then for each $(a,b) \in \R^m \times \R^n$, we have that
	\begin{align*}
			\psi_1 \circ \pi_M \circ (\lam \circ [\phi_0 \times \psi_0])^{-1}(a,b)
			& = \psi_1 \circ \pi_M \circ [\phi_0^{-1} \times \psi_0^{-1}] \circ \lam^{-1} (a,b) \\
			& = \psi_1 \circ \phi_0^{-1} \circ \lam^{-1}(a) \\
			& = \psi_1 \circ \phi_0^{-1} \circ \lam^{-1} \circ \pi_{\R^m}(a,b)
		\end{align*}
\end{proof}
...
\begin{defn}
	Let $(M, \MA)$, $(N, \MB)$ be smooth manifolds and $(p_0, q_0) \in M \times N$. We define the \tbf{product inclusion maps}, denoted $\iota^M_{q_0}: M \rightarrow M \times N$ and $\iota^N_{p_0}: N \rightarrow M \times N$, by $\iota^M_{q_0}(p) = (p, q_0)$ and $\iota^N_{p_0}(q) = (p_0, q)$ respectively.
\end{defn}

\begin{ex}
	Let $(M, \MA)$, $(N, \MB)$ be smooth manifolds and $(p_0, q_0) \in M \times N$. Then $\iota^M_{q_0}$ and $\iota^N_{p_0}$ are smooth.
\end{ex}
...
%	\end{defn}
	...
	\begin{proof}
		...
		\tcb{A previous exercise implies that }
...
\end{enumerate}	 
	...
	\begin{defn}
		Let $(E, M, \pi, F)$ be a $\Maninf$ fiber bundle, $\Gam$ an index set and for each $\al \in \Gam$, $(U_{\al}, \Phi_{\al})_{\al \in \Gam}$ a local trivializations with respect to $\pi$ of $E$ over $U_{\al}$. Then $(U_{\al}, \Phi_{\al})_{\al \in \Gam}$ is said to be a \tbf{$\Maninf$ bundle atlas on $E$} if $(U_{\al})_{\al}$ is an open cover of $E$. 
	\end{defn}
...
\begin{defn}
	Let $G$ be a smooth manifold and group. Then $G$ is said to be a \tbf{Lie group} if $\mult:G \times G \rightarrow G$ is smooth and $\inv: G \rightarrow G$ is smooth.
\end{defn}
...
\begin{proof}\
	\begin{itemize}
		...
		\item $(\impliedby)$: \\
		Suppose that $f$ is smooth.  \tcb{An exercise in the section on smooth maps on product manifolds} implies that the embedding $\iota^2_e: G \rightarrow G \times G$, given by $\iota^2_e(h) = (e, h)$, is smooth. Since $\inv = f \circ \iota^2_e$, $\inv$ is smooth. Therefore $id_G \times \inv$ is smooth and since $\mult = f \circ (\id_G \times \inv)$, $\mult$ is smooth. Since $\mult$ and $\inv$ are smooth, $G$ is a Lie group.
	\end{itemize}
\end{proof}

\begin{ex}
	Let $G$ and $H$ be Lie groups and $\phi: G \rightarrow H$. Then $\phi$ is said to be a \tbf{Lie group homomorphism} if $\phi \in \Hom_{\Grp}(G, H) \cap \Hom_{\Maninf}(G, H)$.
\end{ex}
...
\begin{defn} \ld{00000} 
	Let $G$ be a group and $g \in G$. We define the \tbf{left and right translation maps}, denoted $l_g:G \rightarrow G$ and $r_g:G \rightarrow G$ respectively, by $l_g(x) = gx$ and $r_g(x) = xg^{-1}$ respectively. 
\end{defn}
...
\begin{proof}
	By definition, for each $g \in G$, $l_g, r_g \in \End_{\LieGrp}(G)$ and \tcb{FINISH!!!}.   
\end{proof}
