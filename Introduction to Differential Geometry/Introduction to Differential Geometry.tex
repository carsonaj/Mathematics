...
\end{flushleft}
	...
	\tcr{maybe assume differentiation of Banach spaces so we can skip this stuff?}
	...
	\begin{defn} \ld{def:differentiation:0002}
		Let $U \subset \R^n$ be open, $f: U \rightarrow \R$ and $a \in U$. Then $f$ is said to be 
		\begin{itemize}
			\item \tbf{differentiable with respect to $x^i$ at $a$} if $$\lim\limits_{h \rightarrow 0} \frac{f(a + he^i) - f(a)}{h}$$ exists. If $f$ is differentiable with respect to $x^i$ at $a$, we define the \tbf{partial derivative of $f$ with respect to $x^i$ at $a$}, denoted $$\frac{\p f}{\p x^i} (a) \text{ or } \frac{\p}{\p x^i} f $$ to be the limit above.
			\item \tbf{differentiable at $a$} if there exists $L \in L(\R^m; \R^n)$ such that $F(a + h) = F(a) + L(h) + o(h)$ as $h \rightarrow 0$.
		\end{itemize}
	\end{defn}
...
%	\end{proof}
	...
	\begin{ex} 
		\tcr{make this an ex in analysis notes}
		Let $U_F, U_G \in \MT_X$, $F: U_F \rightarrow Y$, $G: U_G \rightarrow Y$ and $a \in U_1 \cap U_2$. If there exists $U \in \MT_X$ such that $a \in U$, $U \subset U_1 \cap U_2$ and $F|_U = G|_U$, then $DF(a) = DG(a)$. 
	\end{ex}
	
	\begin{proof}
		Suppose that there exists $U \in \MT_X$ such that $a \in U$, $U \subset U_1 \cap U_2$ and $F|_U = G|_U$. Then \tcr{(make a prev ex that for each $x \in X$, $D\iota_U(x) = \id_X$)}
		\begin{align*}
			DF(a)
			& = DF(a) \circ \id_X
			& = DF(a) \circ D\iota_U(a) \\
			& = D(F \circ \iota_U)(a) \\
			& = DF|_U(a) \\
			& = DG|_U(a) \\
			& = D(G \circ \iota_U)(a) \\
			& = DG(a) \circ D\iota_U(a) \\
			& = DF(a) \circ \id_X \\
			& = DG(a).
		\end{align*}
	\end{proof}
	...
	\begin{defn} \ld{def:differentiation_on_subspaces:0001}
		Let $A \subset \R^m$ and $F: A \rightarrow \R^n$. Then $F$ is said to be \tbf{smooth} if for each $a \in A$, there exists $B \in \MT_{\R^m}$ and $g:B \rightarrow \R^n$ such that $a \in B$, $g$ is smooth and $g|_{A \cap B} = F|_{A \cap B}$. 
	\end{defn}
	...
	\begin{ex} \lex{ex:differentiation_on_subspaces:0004}
		Let $A \subset \R^m$ and $f:A \rightarrow \R^n$. Then $f$ is smooth iff for each $a \in A$, there exists $U \in \MT_{\R^m} \cap A$ such that $a \in U$ and $f|_U$ is smooth.  
	\end{ex}
	...
	\begin{ex}
		Let $A \subset \R^m$ and $F:A \rightarrow \R^n$. If $F$ is smooth, then $[DF]:A \rightarrow \R^{n \times m}$ is smooth. 
	\end{ex}
...
\begin{defn} \lex{ex:top_submanifolds:embedded:0.01}
	Let $M, N \in \Obj(\bManz)$ and $F \in \Hom_{\bTop}(N, M)$. Then $F$ is said to be a $\bTop$-embedding if $F|^{F(M)} \in \Iso_{\bTop}(M,F(M))$.  \tcr{here $F|^{F(M)}: M \rightarrow F(M)$ is the restriction on the codomain, make sure to use standardized notation if it changes}.
\end{defn}

\begin{ex} \lex{ex:top_submanifolds:embedded:0001}
	Let $M, N \in \Obj(\bManz)$ and $F \in \Hom_{\bTop}(N, M)$. Define 
	$$F_*X^n(N, \MT_N) \defeq \{(F(V), \psi \circ F|_V^{-1}): (V, \psi) \in X^n(N, \MT_N)\}.$$ 
	If $F$ is a $\bTop$-embedding, then 
	\begin{enumerate}
		\item $F_*X^n(N, \MT_N) \subset X^n(F(N), \MT_M \cap F(N))$.
		\item $(F(N), \MT_M \cap F(N)) \in \Obj(\bManz)$.
	\end{enumerate}
\end{ex}

\begin{proof}
	Suppose that $F$ is a $\bTop$-embedding. Set $n \defeq \dim N$. Since $F$ is a $\bTop$-embedding, $F \in \Iso_{\bTop}((N, \MT_N), (F(N), \MT_M \cap F(N)))$.
	\begin{enumerate}
		...
		\begin{itemize}
			...
			\begin{itemize}
				...
				\begin{align*}
					...
					& \in \MT_M \cap F(N).
				\end{align*}
				\item Since $F \in \Iso_{\bTop}((N, \MT_N), (F(N), \MT_M \cap F(N)))$, we have that 
				\begin{align*}
					F|_V 
					& \in \Iso_{\bTop}((V, \MT_N \cap V), (F(V), [\MT_M \cap F(N)] \cap F(V))) \\
					& = \Iso_{\bTop}((V, \MT_N \cap V), (F(V), \MT_M \cap F(V))).
				\end{align*}
				...
				\begin{align*}
					...
					& \in \MT_{\R^n}.
				\end{align*}
				\item Since $\psi \in \Iso_{\bTop}((V, \MT_N \cap V), (\psi(V), \MT_{\R^n} \cap \psi(V))$, and $F|_V^{-1} \in \Iso_{\bTop}((F(V), \MT_M \cap F(V)), (V, \MT_N \cap V))$, we have that $\psi \circ F|V^{-1} \in \Iso_{\bTop}((F(V), \MT_M \cap F(V)), (\psi(V), \MT_{\R^n} \cap \psi(V))$. 
			\end{itemize}
		...
		\item 
		\begin{enumerate}
			\item Since $F \in \Iso_{\bTop}((N, \MT_N), (F(N), \MT_M \cap F(N)))$ and $(N, \MT_N)$ is Hausdorff, $(F(N), \MT_M \cap F(N))$ is Hausdorff.
			\item Since $F \in \Iso_{\bTop}((N, \MT_N), (F(N), \MT_M \cap F(N)))$ and $(N, \MT_N)$ is second-countable, $(F(N), \MT_M \cap F(N))$ is second-countable.
			\item Let $p \in F(N)$. Then there exists $q \in N$ such that $F(q) = p$. Since $N \in \Obj(\bManz)$, there exists $(V, \psi) \in X^n(N, \MT_N)$ such that $q \in V$. Define $(U, \phi) \in F_*X^n(N, \MT_N)$ by $U \defeq F(V)$ and $\phi \defeq \psi \circ F|_V^{-1}$. By definition, $(U, \phi) \in F_*X^n(N, \MT_N)$. Furthermore,  
			\begin{align*}
				p
				& = F(q) \\
				& \in F(V) \\
		...
		\end{enumerate}
		Thus $(F(N), \MT_M \cap F(N)) \in \Obj(\bManz)$. 
	\end{enumerate}
\end{proof}
...
\end{proof}
	...
	\begin{defn} \ld{def:smooth_manifold_introduction:0002}
		...
		\begin{itemize}
			\item Let $\MA \subset X^n(M, \MT)$. Then $\MA$ is said to be an \tbf{atlas on $M$} if  $M \subset \bigcup\limits_{(U,\phi) \in \MA} U$.
...
\begin{defn} \ld{def:smooth_maps_between_manifolds:0001}
	Let $(M, \MA), (N, \MB) \in \Maninf$.
	\begin{itemize}
		\item Let $F: M \rightarrow N$. Then $F$ is said to be \tbf{$(\MA, \MB)$-smooth} if for each $p \in M$, there exist $(U, \phi) \in \MA$ and $(V, \psi) \in \MB$ such that $p \in U$, $F(U) \subset V$ and $\psi \circ F|_U \circ \phi^{-1} \in C^{\infty}(\phi(U), \psi(V))$. 
		\item We define $C^{\infty}((M, \MA), (N, \MB)) \defeq \{F \in N^M: \text{$F$ is $(\MA, \MB)$-smooth}\}$.
	\end{itemize}
\end{defn}
...
\begin{note}\
	...
	\begin{itemize}
		\item we may say ``smooth" instead of ``$(\MA, \MB)$-smooth",
		\item we may write $C^{\infty}(M,N)$ in place of $C^{\infty}((M, \MA), (N, \MB))$,
		\item we may write $C^{\infty}(M)$ in place of $C^{\infty}(M, R)$.
	\end{itemize} 
\end{note}

Intuitively, if the \tcr{ref prev def} were an appropriate generalization of our previous notion of smoothness for maps between Euclidean (half) spaces discussed in \tcr{ref def in prereq material}, we would need the two definitions to be equivalent when $M$ and $N$ are Euclidean (half) spaces. We would also need the two definitions to be compatible in the \tcr{natural?} intuitive way when only one of $M$ or $N$ is a Euclidean (half) space. Explicitly, we would need the generalized definition to satisfy that for each $U \in \MT_{\H^m}$, $V \in \MT_{\H^n}$ and $F:U \rightarrow V$, $F$ is $(\MA_U, \MA_V)$-smooth (as in \tcr{the prev def}) iff $F$ is smooth (as in \tcr{ref prereq def}). The following exercise states that the \tcr{prev def} indeed satisfies this requirement. 

\begin{ex}
	Let $m,n \in \N_0$, $M \in \MT_{\H^m}$, $N \in \MT_{\H^n}$. Then $C^{\infty}((M, \MA_M), (N, \MA_N)) = C^{\infty}(M,N)$.
\end{ex}

\begin{proof}\
	\begin{itemize}
		\item Let $F \in C^{\infty}((M, \MA_M), (N, \MA_N))$ and $p \in M$. By \tcr{prev def}, there exists $(U, \phi) \in \MA_M$ and $(V, \psi) \in \MA_N)$ such that $p \in U$, $F(U) \subset V$ and $\psi \circ F|_U \circ \phi^{-1} \in C^{\infty}(\phi(U), \psi(V))$.
		Define $F_0 \in C^{\infty}(\phi(U), \psi(V))$ by $F_0 \defeq \psi \circ F|_U \circ \phi^{-1}$. By \tcr{ref definition of standard smooth structure on open subsets of $\H^m$}, $(M, \id_M) \in \MA_M$. Since $(U, \phi), (M, \id_M) \in \MA_M$ and $\MA_M \in \Atlasinf(M)$, we have that $(U, \phi)$ and $(M, \id_M)$ are smoothly compatible. Hence
		\begin{align*}
			\phi \circ \id_U 
			& = \phi \circ (\id_M|^U_U){-1} \\
			& \in \Diffeo(U, \phi(U)).
		\end{align*}
		Similarly, $\id_V \circ \psi^{-1} \in \Diffeo(\psi(V), V)$. \tcr{ref ex in prereq material about composition of smooth functions being smooth} then implies that  
		\begin{align*}
			F|_U
			& = (\psi^{-1} \circ \psi) \circ F|_U \circ (\phi^{-1} \circ \phi) \\
			& = \psi^{-1} \circ (\psi \circ F|_U \circ \phi^{-1}) \circ \phi \\
			& = \psi^{-1} \circ F_0 \circ \phi \\
			& = (\id_V \circ \psi^{-1}) \circ F_0 \circ (\phi \circ \id_U) \\
			& \in C^{\infty}(U, V). 
		\end{align*}
		Since $p \in M$ is arbitrary, we have that for each $p \in M$, there exists $U \in \opn{\MN}(p, \MT_M)$ and $V \in \opn{\MN}(p, \MT_N)$ such that $F(U) \subset V$ and  $F|_U \in C^{\infty}(U,V)$. \tcr{make/ref ex in prereq material showing that if for each $p \in M$, there exists $U \in U \in \opn{\MN}(p, \MT_M)$ and $V \in \opn{\MN}(p, \MT_N)$ such that $F(U) \subset V$ and $F|_U \in C^{\infty}(U,V)$, then $F \in C^{\infty}(M,N)$} implies that $F \in C^{\infty}(M,N)$. Since $F \in C^{\infty}((M, \MA_M), (N, \MA_N))$ is arbitrary, we have that for each $F \in C^{\infty}((M, \MA_M), (N, \MA_N))$, $F \in C^{\infty}(M,N)$. Thus $C^{\infty}((M, \MA_M), (N, \MA_N)) \subset C^{\infty}(M,N)$.
		\item $(\impliedby)$: \\
		Let $F \in C^{\infty}(M,N)$, and $p \in M$. Define $(U, \phi) \in \MA_M$ and $(V, \psi) \in \MA_N$ by $(U, \phi) \defeq (M, \id_M)$ and $(V, \psi) \defeq (N, \id_N)$. Then $p \in U$, 
		\begin{align*}
			F(U)
			& = F(M) \\
			& \subset N \\
		...
		and 
		\begin{align*}
			\psi \circ F|_U \circ \phi^{-1} 
			& = \id_N \circ F \circ \id_M \\
			& = F \\
			& \in C^{\infty}(M,N) \\
			& = C^{\infty}(\phi(U), \psi(V)). 
		\end{align*}
		Since $p \in M$ is arbitrary, we have that for each $p \in M$, there exists $(U, \phi) \in \MA_M$ and $(V, \psi) \in \MA_N$ such that $p \in U$, $F(U) \subset V$ and $\psi \circ F|_U \circ \phi^{-1} \in C^{\infty}(\phi(U), \psi(V))$. Thus $F \in C^{\infty}((M, \MA_M), (N, \MA_N))$. Since $F \in C^{\infty}(M,N)$ is arbitrary, we have that for each $F \in C^{\infty}(M,N)$, $F \in C^{\infty}((M, \MA_M), (N, \MA_N))$. Therefore $C^{\infty}(M,N) \subset C^{\infty}((M, \MA_M), (N, \MA_N))$. 
	\end{itemize}
	Since $C^{\infty}((M, \MA_M), (N, \MA_N)) \subset C^{\infty}(M,N)$ and $C^{\infty}(M,N) \subset C^{\infty}((M, \MA_M), (N, \MA_N))$, we have that $C^{\infty}((M, \MA_M), (N, \MA_N)) = C^{\infty}(M,N)$.
\end{proof}

As a consequence of the compatibility between \tcr{ref prev def} and \tcr{ref old def in prereq material}, we are provide an equivalent formulation of smoothness for mappings of a smooth manifold into a Euclidean space which is a sort of hybrid of the new and old definition. We formalize this in the following exercise. 

\begin{ex}
	Let $M \in \Maninf$, $n \in \N_0$. Then $C^{\infty}((M, \MA_{M}), (\R^n, \MA_{\R^n}))$ iff for each $(U, \phi) \in \MA_M$, $F|_U \circ \phi \in C^{\infty}(\phi(U), \R^n)$.  
\end{ex}

\begin{proof}\
	\tcr{FINISH!!!, similar to prev ex}
\end{proof}

\begin{ex} \lex{ex:smooth_maps_between_manifolds:0002}
	Let $M, N \in \Maninf$. Then $C^{\infty}(M,N) \subset C(M, N)$.
\end{ex}

\begin{proof}
	Let $F \in C^{\infty}(M,N)$ and $p \in M$. Since $\MA_M \in \Atlasinf(M)$ and $\MA_N \in \Atlasinf(N)$, there exist $(U, \phi) \in \MA_M$ and $(V, \psi) \in \MA_N$ such that $p \in U$, $F(U) \subset V$ and $\psi \circ F \circ \phi^{-1}$ is smooth. Define $F_0: \phi(U) \rightarrow \psi(V) $ by $$ F_0 = \psi \circ F \circ \phi^{-1}$$  
	By definition, $F_0$ is smooth. \rex{ex:differentiation_on_subspaces:0002} implies that $F_0 \in C(\phi(U), \psi(V))$. Since $\phi \in \Homeo(U, \phi(U))$ and $\psi \in \Homeo(V, \psi(V))$ and $F|_U = \psi^{-1} \circ F_0 \circ \phi$, we have that $F|_U \in C(U, N)$. Thus, $F$ is continuous at $p$. Since $p \in M$ is arbitrary, we have that for each $p \in M$, $F$ is continuous at $p$. Hence $F \in C(M,N)$. Since $F \in C^{\infty}(M,N)$ is arbitrary, we have that for each $F \in C^{\infty}(M,N)$, $F \in C(M,N)$.
\end{proof}
...
\begin{defn} 
	Let $(M, \MA), (N, \MB) \in \Maninf$.
	\begin{itemize}
		\item Let $F: M \rightarrow N$. Then $F$ is said to be a \tbf{$(\MA, \MB)$-diffeomorphism} if $F$ is a bijection, $F \in C^{\infty}((M, \MA), (N, \MB))$ and $F^{-1} \in C^{\infty}((N, \MB), (M, \MA))$.
		\item We define $\Diffeo((M, \MA), (N, \MB)) \defeq \{F \in C^{\infty}((M, \MA), (N, \MB)): \text{$F$ is a $(\MA,\MB)$-diffeomorphism}\}$.
	\end{itemize}
\end{defn}
...
\begin{note}
	When the context is clear, 
	\begin{itemize}
		\item we may say ``diffeomorphism" instead of $(\MA,\MB)$-diffeomorphism,
		\item we may write $\Diffeo(M, N)$ in place of $\Diffeo((M, \MA), (N, \MB))$.
	\end{itemize}
\end{note}

\begin{ex}
	Let $U \in \MT_{\H^m}$, $V \in \MT_{\H^n}$ and $F:U \rightarrow V$. Then $F \in \Diffeo(U, V)$ iff $F$ is a diffeomorphism.
\end{ex}

\begin{proof}
	\tcr{FINISH!!!, ref prev ex on equivalence of old and new definitions of smoothness }
\end{proof}

\begin{ex}
	Let $M \in \Maninf$ and $(U, \phi) \in \MA_M$. Then $\phi \in \Diffeo(U, \phi(U))$. 
\end{ex}

\begin{proof}
	By definition, $\phi \in \Homeo(U, \phi(U))$. By definition, for each $(V, \psi) \in \MA_M$, $\id_{\R^n} \circ \psi$ \tcr{FINISH!!!}
\end{proof}
...
\begin{proof} Set $m \defeq \dim M$ and $n \defeq \dim N$.
	\begin{enumerate}
		\item $(1) \implies (2)$: \\
		Suppose that $F$ is smooth. Let $\MA_0 \subset \MA$ and $\MB_0 \subset \MB$. Suppose that $\MA_0$ is an atlas on $M$ and $\MB_0$ is an atlas on $N$. Let $(U_0, \phi_0) \in \MA_0$ and $(V_0, \psi_0) \in \MB_0$. Since $\MA_0 \subset \MA$ and $\MB_0 \subset \MB$, we have that $(U_0, \phi_0) \in \MA$ and $(V_0, \psi_0) \in \MB$. Since $F$ is smooth, \rex{ex:smooth_maps_between_manifolds:0002} implies that $F$ is continuous and therefore $U_0 \cap F^{-1}(V_0) \in \MT_M$. Define $F_0:\phi_0(U_0 \cap F^{-1}(V_0)) \rightarrow \psi_0(V_0)$ by $F_0 \defeq \psi_0 \circ F \circ \phi_0|_{U_0 \cap F^{-1}(V_0)}^{-1}$. Let $a \in \phi_0(U_0 \cap F^{-1}(V_0))$. Define $p \in M$ by $p \defeq \phi_0^{-1}(a)$. Since $F$ is smooth, there exists $(U_1, \phi_1) \in \MA$ and $(V_1, \psi_1) \in \MB$ such that $p \in U_1$, $F(U_1) \subset V_1$ and $\psi_1 \circ F \circ \phi_1^{-1}$ is smooth. Define $U \subset M$, $\al: \phi_1(U_0 \cap U_1) \rightarrow \phi_0(U_0 \cap U_1)$, $\be: \psi_1(V_0 \cap V_1) \rightarrow \psi_0(V_0 \cap V_1)$ and $F_1: \phi_1(U_1) \rightarrow \psi_1(V_1)$ by $U \defeq U_0 \cap U_1 \cap F^{-1}(V_0 \cap V_1)$, $\al \defeq \phi_0|_{U_0 \cap U_1} \circ \phi_1|_{U_0 \cap U_1}^{-1}$, $\be \defeq \psi_0|_{V_0 \cap V_1} \circ \psi_1|_{V_0 \cap V_1}^{-1}$ and $F_1 \defeq \psi_1 \circ F \circ \phi_1^{-1}$.
		 We note the following:
		\begin{itemize}
			\item since $F$ is continuous, $V_0, V_1 \in \MT_N$ and $U_0, U_1 \in \MT_M$, we have that $U \in \MT_M$,
			\item since $p \in U$ and $a = \phi_0(p)$, we have that $a \in \phi_0(U)$,
			\item since $\phi_0(U) \subset \phi_0(U_0 \cap F^{-1}(V_0))$, $\phi_0(U) \in \MT_{\H^m}$ and $\phi_0(U_0 \cap F^{-1}(V_0)) \in \MT_{\H^m}$, we have that $\phi_0(U) \in \MT_{\H^m} \cap \phi_0(U_0 \cap F^{-1}(V_0))$,
		...
		\begin{itemize}
			\item Since $a \in \phi_0(U_0 \cap F^{-1}(V_0))$ is arbitrary, we have that for each $a \in \phi_0(U_0 \cap F^{-1}(V_0))$, there exists $A \in \MT_{\H^m} \cap \phi_0(U_0 \cap F^{-1}(V_0))$ such that $a \in A$, and $F_0|_A$ is smooth. \rex{ex:differentiation_on_subspaces:0004} implies that $F_0$ is smooth. 
			\item Since $(U_0, \phi_0) \in \MA_0$ and $(V_0, \psi_0) \in \MB_0$ are arbitrary, we have that for each $(U, \phi) \in \MA_0$ and $(V, \psi) \in \MB_0$, $\psi \circ F \circ \phi|_{U \cap F^{-1}(V)}^{-1}$ is smooth. 
			\item Since $\MA_0 \subset \MA$ and $\MB_0 \subset \MB$ such that $\MA_0$ is an atlas on $M$ and $\MB_0$ is an atlas on $N$ are arbitrary, we have that for each $\MA_0 \subset \MA$ and $\MB_0 \subset \MB$, if $\MA_0$ is an atlas on $M$ and $\MB_0$ is an atlas on $N$, then for each $(U, \phi) \in \MA_0$ and $(V, \psi) \in \MB_0$, $U \cap F^{-1}(V)$ is open in $M$ and $\psi \circ F \circ \phi|_{U \cap F^{-1}(V)}^{-1}$ is smooth.
		\end{itemize}
		\item $(2) \implies (3)$: \\
...
\begin{ex} \lex{ex:smooth_maps_open_boundary_submanifolds:0004.1}
	Let $M \in \Obj(\bManz)$ and $\MA_1, \MA_2$ smooth structures on $M$. Define $\iota:M \rightarrow M$ by $\iota(p) = p$. If $\iota \in \Iso_{\bManBndinf}[(M, \MA_1), (M, \MA_2)]$, then $\MA_1 = \MA_2$.
\end{ex}
...
\begin{ex} \lex{ex:smooth_maps:smooth_functions:0002} 
	Let $(M, \MA)$ be a smooth manifold and $f: M \rightarrow \R$. Then $f$ is $(\MA, \MA_{\R})$-smooth iff for each $(U, \phi) \in \MA$, $f \circ \phi^{-1}$ is smooth.
\end{ex}

\begin{proof}\
	\begin{itemize}
		\item $(\implies)$: \\
		Suppose that for each $(U, \phi) \in \MA$, $f \circ \phi^{-1}$ is smooth. Let $(U, \phi) \in \MA$. Since $\id_{\R} \circ f \circ \phi^{-1} = f \circ \phi^{-1}$ and $f \circ \phi^{-1}$ is smooth, we have that $\id_{\R} \circ f \circ \phi^{-1}$ is smooth. Since $\MA = \al(\MA)$ and $\MA_{\R} = \al((\R, \id_{\R}))$, \tcr{an exercise in the section on smooth maps} implies that $f$ is $(\MA, \MA_{\R})$-smooth.
		\item $(\impliedby)$: \\
		Suppose that $f$ is $(\MA, \MA_{\R})$-smooth. Let $(U, \phi) \in \MA$. Since $(\R, \id_{\R}) \in \MA_{\R}$ and $f \circ \phi^{-1} = \id_{\R} \circ f \circ \phi^{-1}$, we have that $f \circ \phi^{-1}$ is smooth. Since $(U, \phi) \in \MA$ is arbitrary, we have that for each $(U, \phi) \in \MA$, $f \circ \phi^{-1}$ is smooth.
	\end{itemize}
\end{proof}
...
\begin{defn} \ld{def:smooth_maps:smooth_functions:0006} 
	Let $(M, \MA)$ be a smooth manifold, $(U, \phi) \in \MA$ with $\phi = (x^1, \cdots, x^n)$, $f \in C^{\infty}(U)$ and $i \in \{1, \cdots, n\}$. We define the \tbf{partial derivative of $f$ with respect to $x^i$}, denoted 
	$$\p f / \p x^i \in C^{\infty}(M),$$ 
	by
	\begin{equation*}
...
\begin{note}
	Another common notation for $\p f/\p x^i$ is $\p_i f$.
\end{note}
...
\begin{ex} \lex{ex:smooth_maps:smooth_functions:0012}
	Let $M, N \in \Obj(\bManBndinf)$ and $p \in M$. Set $n \defeq \dim N$. Then for each $b \in $ $(U, \phi) \in \MA_M$ and $(V, \psi) \in \MA_N$. Then for each $b \in $  \tcr{FINISH (diff eq question for jet bundles)}
\end{ex}
...
\begin{ex} \lex{ex:smooth_maps:smooth_functions:0013}
	Let $M \in \Obj(\bManBndinf)$ and $(U, \phi) \in \MA_M$. Write $\phi = (x^1, \cdots, x^n)$. Then for each $i,j \in \{1, \cdots, n\}$,  
...
\begin{ex} \lex{ex:smooth_maps:smooth_functions:0014} \tbf{Change of Coordinates:}\\
	Let $M \in \Obj(\bManBndinf)$ and $(U, \phi), (V, \psi) \in \MA_M$. Write $\phi = (x^1, \cdots, x^n)$ and $\psi = (y^1, \cdots, y^n)$. Then for each $j \in \{1, \cdots, n\}$, $p \in U \cap V$ and $f \in C^{\infty}(M)$ 
...
\begin{defn} \ld{def:smooth_maps:smooth_functions:0016}
	Let $M \in \Obj(\bManBndinf)$ and $(U, \phi), (V, \psi) \in \MA_M$. Write $\phi = (x^1, \cdots, x^n)$ and $\psi = (y^1, \cdots, y^n)$. 
\end{defn}

\begin{defn} \ld{def:smooth_maps:smooth_functions:0017}
	Let $M, N \in \Obj(\bManBndinf)$, $F \in \Hom_{\bManBndinf}(M, N)$, $(U, \phi) \in \MA_M$ and $(V, \psi) \in \MA_N$. Set $m \defeq \dim M$, $n \defeq \dim N$ and write $\phi = (x^1, \cdots, x^m)$ and $\psi = (y^1, \cdots, y^n)$. Let $I, J \in \MI_n^{\otimes k}$. Write $I = (i_1, \ldots, i_k)$ and $J = (j_1, \ldots j_k)$. We define $\p (y^J \circ F)/ \p x^I \in C^{\infty}(U)$ by 
...
\begin{ex} \lex{ex:smooth_maps:smooth_functions:0018}
	Let $M \in \Obj(\bManBndinf)$, $(U, \phi)$ and $(V, \psi) \in \MA_M$. Set $n \defeq \dim M$ and write $\phi = (x^1, \cdots, x^n)$ and $\psi = (y^1, \cdots, y^n)$. Let $I, J \in \MI_n^{\otimes k}$. Write $I = (i_1, \ldots, i_k)$ and $J = (j_1, \ldots j_k)$. Then 
...
\begin{defn}
	Let $p \in M$, $U \in \opn{\MN}(p)$ and $\rho \in C_c^{\infty}(M)$. Then $\rho$ is said to be a \tbf{bump function at $p$ supported in $U$} if 
	\begin{enumerate}
		\item $\rho \geq 0$ 
		\item there exists $V \in \opn{\MN}(p)$ such that $\rho|_V = 1$ 
		\item $\supp \rho \subset U$
	\end{enumerate}
	
	\tcr{maye need to change, typically bump functions $\rho$ are $1$ on a compact $K$ and $\supp \rho \subset U$ where $U$ is open and $K \subset U$}
\end{defn}
...
\begin{defn}
	Let $M, N \in \Maninf$ and $p \in M$. 
	\begin{itemize}
		\item Let $F, G \in C^{\infty}(M,N)$. Then $F$ is said to \tbf{share the same germ at $p$}, denoted $F \sim_p G$, if there exists $U \in \opn{\MN}(p)$ such that $F|_U = G|_U$. 
		\item We define the \tbf{germ of $C^{\infty}(M,N)$ at $p$}, denoted $\germinf_p(M,N) \subset C^{\infty}(M,N) \times C^{\infty}(M,N)$ by 
		$$\germinf_p(M,N) \defeq \{(F, G) \in C^{\infty}(M,N) \times C^{\infty}(M,N): F \sim_p G \}.$$
	\end{itemize}
\end{defn}

\begin{note}
	When the context is clear, we may write $\germinf_p$ in place of $\germinf_p(M,N)$.
\end{note}

\begin{ex}
	Let $M, N \in \bManinf$ and $p \in M$. Then $\germinf_p \in \Equi(C^{\infty}(M))$.
\end{ex}
...
\begin{defn}
	Let $M, N \in \Maninf$ and $p \in M$. We define $C^{\infty}_p(M,N) \defeq C^{\infty}(M,N)/\germinf_p$. 
\end{defn}

\begin{ex}
	We have that 
	\begin{enumerate}
		\item $\add:C^{\infty}(M) \times C^{\infty}(M) \rightarrow C^{\infty}(M)$ is $((\germinf_p)^{\times 2}, \germinf_p)$-invariant
		\item \item $\scalmult:C^{\infty}(M) \times C^{\infty}(M) \rightarrow C^{\infty}(M)$ is $((\germinf_p)^{\times 2}, \germinf_p)$-invariant
		\item $\mult:C^{\infty}(M) \times C^{\infty}(M) \rightarrow C^{\infty}(M)$ is $((\germinf_p)^{\times 2}, \germinf_p)$-invariant
	\end{enumerate}
\end{ex}
...
\begin{defn}
	Let $M \in \Maninf$ and $p \in M$. Since $\add, \scalmult, \mult$ are $((\germinf_p)^{\times 2}, \germinf_p)$-invariant, \tcr{ref universal algebra notes} implies that
	\begin{itemize}
		\item there exists a unique $add_p: C_p^{\infty}(M) \times C_p^{\infty}(M) \rightarrow C_p^{\infty}(M)$ such that $\pi_{\germinf_p} \circ \add = add_p \circ \pi^{\times 2}_{\germinf_p}$, i.e. the following diagram commutes:
		\[ 
		\begin{tikzcd}
			C^{\infty}(M) \times C^{\infty}(M)  \arrow[r, "\add"]  \arrow[d, "\pi^{\times 2}_{\germinf_p}"'] & C^{\infty}(M)   \arrow[d, "\pi_{\germinf_p}"]\\
			C_p^{\infty}(M) \times C_p^{\infty}(M) \arrow[r, "add_p"'] &  C_p^{\infty}(M) \\
		\end{tikzcd}
		\]
		\item \tcr{FINISH!!! for $\scalmult_p$}
		\item there exists a unique $\mult_p: C_p^{\infty}(M) \times C_p^{\infty}(M) \rightarrow C_p^{\infty}(M)$ such that $\pi_{\germinf_p} \circ \mult = \mult_p \circ\pi^{\times 2}_{\germinf_p}$, i.e. the following diagram commutes:
		\[ 
		\begin{tikzcd}
			C^{\infty}(M) \times C^{\infty}(M)  \arrow[r, "\mult"]  \arrow[d, "\pi^{\times 2}_{\germinf_p}"'] & C^{\infty}(M)   \arrow[d, "\pi_{\germinf_p}"]\\
			C_p^{\infty}(M) \times C_p^{\infty}(M) \arrow[r, "\mult_p"'] &  C_p^{\infty}(M) \\
		\end{tikzcd}
		\]
	\end{itemize}
\end{defn}
...
\begin{note}
	When the context is clear, we may write $f + g$, $\lam f$ and $fg$ in place of $\add_p(f,g)$, $\scalmult_p(\lam, f)$ and $\mult_p(f,g)$ respectively.
\end{note}

\begin{ex}
	Let $M \in \Maninf$ and $p \in M$. Then 
	\begin{enumerate}
		\item $(C_p^{\infty}(M), \add_p, \scalmult_p, \mult_p) \in \AlgR$. 
		\item $\pi_{\germinf_p} \in \Hom_{\AlgR}(C^{\infty}(M), C_p^{\infty}(M))$
		\item $\ker \pi_{\germinf_p} = \pi_{\germinf_p}(0)$.
	\end{enumerate}
\end{ex}

\begin{proof}\ \tcr{should be clear from universal algebra}
	\begin{enumerate}
		\item 
		\item 
		\item 
...
%	\end{defn}
	...
	\begin{defn}
		Let $M \in \Obj(\bManBndinf)$ and $(U, \phi) \in \MA$ and $p \in U$. Set $n \defeq \dim M$ and write $\phi = (x^1, \cdots, x^n)$. For $j \in [n]$, we define the \tbf{partial derivative operator with respect to $x^i$ at $p$}, denoted 
		$$\frac{\p}{\p x^i} \bigg|_p: C^{\infty}(M) \rightarrow \R,$$ 
		by 
		$$ \frac{\p}{\p x^i} \bigg |_p  f \defeq  \frac{\p f}{\p x^i}(p).$$
	\end{defn}
	
	\begin{note}
		Another common notation for $\p / \p x^i|_p$. is $\p_i|_p$.
	\end{note}

	\begin{ex} \tbf{Change of Coordinates:}\\
		Let $M \in \Obj(\bManBndinf)$, $(U, \phi), (V, \psi) \in \MA$, $p \in U \cap V$ and $f \in C^{\infty}(M)$. Set $n \defeq \dim M$ and write $\phi = (x^1, \cdots, x^n)$, $\psi = (y^1, \cdots, y^n)$. Then for each $j \in \{1, \cdots, n\}$, 
	...
	\begin{defn}
		Let $M \in \Obj(\bManBndinf)$ and $p \in M$.
		\begin{itemize}
			\item Let $v: C^{\infty}(M) \rightarrow \R$. Then $v$ is said to be 
			\begin{itemize}
				\item \tbf{Leibnizian at $p$} if for each $f,g \in  C^{\infty}(M)$, 
				$$v(fg) = v(f)g(p) + f(p)v(g)$$ 
				\item a \tbf{derivation on $C^{\infty}(M)$ at $p$} if for each $f, g \in C^{\infty}(M)$ and $a \in \R$,
				\begin{enumerate}
					\item $v$ is linear 
					\item $v$ is Leibnizian
				\end{enumerate}
			\end{itemize}
			\item We define $\Derivinf_p(M) \subset \R^{\C^{\infty}(M)}$ by $\Derivinf_p(M) \defeq \{v \in  \R^{\C^{\infty}(M)}: \text{$v$ is a derivation on $C^{\infty}(M)$ at $p$}\}$.
		\end{itemize}
	\end{defn}
	
	\begin{ex}
		Let $M \in \Obj(\bManBndinf)$, $p \in M$ and $v \in \Derivinf_p(M)$. Then $v$ is $\germinf_p$-invariant. \\
		\tbf{Hint:} bump function and Leibnizianity
	\end{ex}
	
	\begin{proof}
		Let $f,g \in C^{\infty}(M)$. Suppose that $f \sim_p g$. Then there exists $U \in \opn{\MN}(p)$ such that $f|_U = g|_U$. Define $h \in C^{\infty}(M)$ by $h \defeq f-g$. Then $h|_U = 0$. Since $\{p\} \in \MK(M)$ and $U \in \opn{\MN}(p)$, \tcr{make/ref ex on bump functions and partition of unity} implies that there exists $\rho \in C^{\infty}(M)$ such that $\rho(p) = 0$ and $\rho|_{U^c} \equiv 1$. Then $\rho h = h$ and 
		\begin{align*}
			v(h)
			& = v(\rho h) \\
			& = v(\rho) h(p) + \rho(p) v(h) \\
		...
		\begin{align*}
			...
			& = v(g).
		\end{align*}
		Since $f,g \in C^{\infty}(M)$ with $f \sim_p g$ are arbitrary, we have that for each $f,g \in C^{\infty}(M)$, $f \sim_p g$ implies that $v(f) = v(g)$. Thus $v$ is $\germinf_p$-invariant.
	\end{proof}
	
	\begin{note}
		Let $v \in \Derivinf_p(M)$. Since $v$ is $\germinf_p$-invariant, \tcr{(ref ex)} implies that there exists a unique $\bar{v}:C^{\infty}_p \rightarrow \R$ such that $v = \bar{v} \circ \pi_{\germinf_p}$, i.e the following diagram commutes:
		\[ 
		\begin{tikzcd}
			C^{\infty}(M)  \arrow[dr, "v"]  \arrow[d, "\pi_{\germinf_p}"']  & \\
			C_p^{\infty}(M) \arrow[r, "\bar{v}"'] &  \R  \\
		\end{tikzcd}
		\]
	\end{note}
	
	\begin{defn}
		Let $M \in \Maninf$ and $p \in M$.
		\item Let $v \in \Derivinf_p(M)$. We define the \tbf{tangent vector to $M$ at $p$ induced by $v$}, denoted $v_p: C_p^{\infty}(M) \rightarrow \R$, to be the unique $v_p:C^{\infty}_p \rightarrow \R$ such that $v = v_p \circ \pi_{\germinf_p}$, i.e the following diagram commutes:
		\[ 
		\begin{tikzcd}
			C^{\infty}(M)  \arrow[dr, "v"]  \arrow[d, "\pi_{\germinf_p}"']  & \\
			C_p^{\infty}(M) \arrow[r, "v_p"'] &  \R  \\
		\end{tikzcd}
		\]
		\item  We define the \tbf{tangent space of $M$ at $p$}, denoted $T_pM$, by 
		$$T_pM = \{ v_p \in \R^{C^{\infty}_p(M)}: v \in \Derivinf_p(M) \}$$
	\end{defn}
	...
	\begin{note}
		We often identify $\Derivinf_p(M)$ with $T_pM$. \tcr{and abuse notation}
	\end{note}
	
	\begin{ex}
		\rex{ex:smooth_maps:smooth_functions:0007}
	\end{ex}
	...
	\begin{ex}
		Let $(U, \phi) \in \MA$ with $\phi = (x^1, \cdots, x^n)$ and $p \in U$. Then 
		$$\bigg \{\frac{\p}{\p x^1} \bigg|_p, \cdots, \frac{\p}{\p x^n} \bigg|_p \bigg \}$$ is a basis for $T_pM$ and $\dim T_pM = n$.
	\end{ex}

	\begin{proof}
		Clearly $\frac{\p}{\p x^1} \bigg|_p, \cdots, \frac{\p}{\p x^n} \bigg|_p \in T_pM$. Let $a_1, \cdots, a_n \in \R$. Suppose that 
		$$v = \sum_{i=1}^n a_i \frac{\p}{\p x^i} \bigg |_p  = 0$$
		Then 
		\begin{align*}
			0
			& = v x^j \\
		...
		Now, let $v \in T_pM$ and $f \in \C^{\infty}(M)$. By Taylor's theorem, there exist $g_1, \cdots g_n \in C_p^{\infty}(M)$ such that 
		$$f = f(p) + \sum_{i=1}^n(x^i - x^i(p)) g_i$$ and for each $i \in \{1, \cdots, n\}$, 
		$$g_i(p) = \frac{\p}{\p x^i} \bigg |_p  f. $$ 
		Then 
		\begin{align*}
			v(f)
		...
		\end{align*}
		Thus
		$$v = \sum_{i=1}^nv(x^i)\frac{\p}{\p x^i} \bigg |_p  $$ and $$v \in \spn \bigg \{\frac{\p}{\p x^1} \bigg|_p, \cdots, \frac{\p}{\p x^n} \bigg|_p \bigg \}$$
	\end{proof}
	...
	\begin{ex}
		Let $M \in \Obj(\bManBndinf)$, $(U, \phi) \in \MA_M$ and $p \in U$. Write $\phi = (x^1, \ldots, x^n)$. Then for each $j \in [n]$, 
	...
	\begin{ex}
		...
		$B_{\phi}$ and $B_{\psi}$ is 
		$$([DF(p)]_{\phi, \psi})_{j, k} =  \frac{\p (y^j \circ F)}{\p x^k}(p)$$
	\end{ex}
	...
	\subsection{Velocities along Paths}
	
	\begin{defn}
		\tcr{define calc I derivative and identiyfy with new manifold derifivative definition, show $f'(0)$ can be identified with }
	\end{defn}
	
	\begin{defn}
		Let $M \in \Obj(\bManBndinf)$. We define the \tbf{paths on $M$}, denoted $\Paths(M)$, by $\Paths(M) \defeq \coprod\limits_{I \in \MT_{\R}} \Hom_{\bManBndinf}(I, M)$.
	\end{defn}
	
	\begin{defn} 
		Let $M \in \Obj(\bManBndinf)$, $(I, \gam) \in \Paths(M)$ and $t_0 \in I$. We define the \tbf{velocity of $\gam$ at $t_0$}, denoted $\gam'(t_0) \in T_pM$, by 
		$$\gam'(t_0) \defeq D\gam(t_0) \bigg(\frac{d}{dt}\bigg|_{t_0}\bigg).$$ 
	\end{defn}
	
	\begin{note}
		Another common notation for $\gam'(t_0)$ is $\dot{\gam}(t_0)$.
	\end{note}
	
	\begin{defn}
		Let $M \in \Obj(\bManBndinf)$ and $(U, \phi) \in \MA_M$. Set $n \defeq \dim M$ and write $\phi = (x^1, \ldots, x^n)$. Let $(I, \gam) \in \Paths(M)$. Suppose that $I \subset U$. For each $k \in [n]$, we define the \tbf{$k$-th component functions of $\gam$ with respect to $(U, \phi)$}, denoted $\gam^k \in \Hom_{\bManBndinf}(I, \R)$, by $\gam^k(t) \defeq \prj^[n]_k \circ \phi \circ \gam$.  
	\end{defn}
	
	Let $\gam \in \Hom_{\bManBndinf}(I, M)$, $t_0 \in I$ and $(U, \phi) \in \MA_M$. Write $\phi = (x^1, \ldots, x^n)$. Define $p \in M$ by $p \defeq \gam(t_0)$. If $p \in U$, then $\gam'(t_0) = $
	
	\begin{ex}
		Let $I \in \MT_{\R}$, $t_0 \in \R$. Then 
	\end{ex}
	
	\begin{ex}
		Let $M \in \Obj(\bManBndinf)$, $I \in \MT_{\R}$, $\gam \in \Hom_{\bManBndinf}(I, M)$ and $t_0 \in I$. Define $p \in M$ by $p \defeq \gam(t_0)$. Then 
		$(f \circ \gam)'(t_0) = Df(p) (\gam'(t_0))$. 
	\end{ex}
	...
	\begin{proof}
		...
		\begin{align*}
			(f \circ \gam)'(t_0)
			& = D(f \circ \gam)(t_0) \bigg(\frac{d}{dt}\bigg|_{t_0}\bigg) \\
			& = Df(\gam(t_0)) \circ D\gam(t_0) \bigg(\frac{d}{dt}\bigg|_{t_0}\bigg) \\
			& = Df(p) \circ D \gam (t_0) \bigg(\frac{d}{dt}\bigg|_{t_0}\bigg) \\
			& = Df(p) \bigg[ D \gam (t_0) \bigg(\frac{d}{dt}\bigg|_{t_0}\bigg) \bigg] \\
			& = Df(p) (\gam'(t_0)). 
		\end{align*}
	\end{proof}
	
	\begin{defn}
		Let $M \in \Obj(\bManBndinf)$. 
		\begin{itemize}
			\item For each $\ep > 0$, we define the \tbf{open interval of radius $\ep$ cenetered at $0$}, denoted $I_{\ep} \subset \R$, by $I_{\ep} \defeq (-\ep, \ep)$. 
			\item We define the \tbf{paths on $M$}, denoted $\Paths(M)$, by $\Paths(M) \defeq \bigcup\limits_{\ep > 0} \Hom_{\bManBndinf}(I_{\ep}, M)$
			\item For each $p \in M$, we define 
			\begin{itemize}
				\item the \tbf{paths on $M$ centered at $p$}, denoted $\Paths_p(M) \subset \Paths(M)$, by $\Paths_p(M) \defeq \{\gam \in \Paths(M): \gam(0) = p \}$
				\item $\sim_p^v \subset \Paths_p(M) \times \Paths_p(M)$ by $\gam_1 {\sim_p^v} \gam_2$ iff for each $f \in C^{\infty}(M)$, $[f \circ \gam_1]'(0) = [f \circ \gam_2]'(0)$.
			\end{itemize}
		\end{itemize} 
	\end{defn}
	
	\begin{ex}
		Let $M \in \Obj(\bManBndinf)$ and $p \in M$. Then $\sim_p^v$ is an equivalence relation on $\Paths_p(M)$.  
	\end{ex}
	...
	\begin{ex}
		Define $\tan_p:\Paths_p(M) \rightarrow Tp_M$ by $\tan_p(\gam) \defeq \gam'(0)$. Then $\tan_p$ is $\sim_p^v$-invariant.  
	\end{ex}
	
	\begin{proof}
		Let $\gam_1, \gam_2 \in \Paths_p(M)$. Suppose that $\gam_1 \sim_p^v \gam_2$. Then for each $f \in C^{\infty}(M)$, 
		\begin{align*}
			Df(p) \circ D \gam_1(0) (\bigg(\frac{d}{dt}\bigg|_{t_0}\bigg))
			& = Df(\gam_1(t_0)) \circ D \gam(t_0) (0) \\
			& = D(f \circ \gam_1)'(0) \\
			& = (f \circ \gam_1)'(0) \\
			& = D(f \circ \gam_2)'(0) \\
			& = Df(\gam_2(t_0)) \circ D \gam(t_0)  \bigg(\frac{d}{dt}\bigg|_{t_0}\bigg) \\
			& = Df(p) \circ D \gam(0) \bigg(\frac{d}{dt}\bigg|_{t_0}\bigg).
		\end{align*}
	\end{proof}
	
	\begin{defn}
		Let $M \in \Obj(\bManBndinf)$ and $p \in M$. We define $V_pM \subset \MP(\Paths_p(M))$ by $V_pM \defeq \Paths_p(M) / {\sim_p^v}$.
	\end{defn}

	\begin{ex}
		Let $M \in \Obj(\bManBndinf)$ and $p \in M$. Then $\Iso_{\bVectR}(V_pM, T_pM) \neq \varnothing$. 
	\end{ex}
	
	\begin{proof}
		Define $A: V_pM \rightarrow T_pM$ by $A/\pi_p$. 
	\end{proof}
	...
	\section{Tangent Space of Product Manifolds}
	
	\begin{ex} \lex{ex:tangent_spaces:products:0001}
		Let $M,N \in \Obj(\bManBndinf)$. Set $m \defeq \dim M$ and $n \defeq \dim N$. Let $(U_M, \phi_M) \in \MA_M$ and $(U_N, \phi_N) \in \MA_N$. Write $\phi_M = (x^1, \ldots, x^m)$ and $\phi_N = (y^1, \ldots, y^n)$. Define $(U, \phi) \in \MA_M \otimes \MA_N$ by $U \defeq U_M \times U_N$ and $\phi \defeq \phi_M \times \phi_N$. Write $\phi = (\tl{x}^1, \ldots, \tl{x}^m, \tl{y}^1, \ldots, \tl{y}^n)$. Then 
		\begin{enumerate}
	...
	\begin{ex} \lex{ex:tangent_spaces:products:0002}
			Let $M,N \in \Obj(\bManBndinf)$, $p \in M$ and $q \in N$. Set $m \defeq \dim M$ and $n \defeq \dim N$. Define $\al \in \Hom_{\bVectR}(T_{(p, q)} (M \times N), T_p M \times T_q N)$ by $\al \defeq (D \pi_{M}(p, q), D \pi_N(p,q))$. Then
			\begin{enumerate}
				...
				\item $\al \in \Iso_{\bVectR}(T_{p, q} (M \times N), T_p M \times  T_q N)$.
			\end{enumerate}		
	\end{ex}
	...
	\begin{proof}\
		\begin{enumerate}
			...
			\begin{align*}
				...
				& = \dim( T_pM \times  T_qN),
			\end{align*}
			we have that $\al$ is surjective and therefore $\al$ is an isomorphism and $\al \in \Iso_{\bVectR}(T_{p, q} (M \times N), T_p M \times  T_q N)$. 
		\end{enumerate}
	\end{proof}

	\begin{ex} \lex{ex:tangent_spaces:products:0003}
		Let $M_1, M_2, N_1, N_2 \in \Obj(\bManinf)$, $F_1 \in \Hom_{\bManinf}(M_1, N_1)$, $F_2 \in \Hom_{\bManinf}(M_2, N_2)$, $(U_1, \phi_1) \in \MA_{M_1}$, $(U_2, \phi_2) \in \MA_{M_2}$, $(V_1, \psi_1) \in \MA_{N_1}$, $(V_2, \psi_2) \in \MA_{N_2}$ and $(p_1, p_2) \in M_1 \times M_2$. Set $m_1 \defeq \dim M_1, m_2 \defeq \dim M_2$, $n_1 \defeq \dim N_1$ and $n_2 \defeq \dim N_2$.Write $\phi_1 = (x_1^1, \ldots, x_1^{m_1})$ and $\phi_2 = (x_2^1, \ldots, x_2^{m_2})$, $\psi_1 = (y_1^1, \ldots, y_1^{n_1})$ and $\psi_2 = (y_2^1, \ldots, y_2^{n_2})$. Define $(U, \phi) \in \MA_{M_1} \otimes \MA_{M_2}$ and $(V, \psi) \in \MA_{N_1} \otimes \MA_{N_2}$ by $U \defeq U_1 \times U_2$, $\phi \defeq \phi_1 \times \phi_2$, $V \defeq V_1 \times V_2$ and $\psi \defeq \psi_1 \times \psi_2$. Write $\phi = (\tl{x}_1^1, \ldots, \tl{x}_1^{m_1}, \tl{x}_2^1, \ldots, \tl{x}_2^{m_2})$ and $\psi = (\tl{y}_1^1, \ldots, \tl{y}_1^{n_1}, \tl{y}_2^1, \ldots, \tl{y}_2^{n_2})$. Then for each $i \in [m_1]$, $j \in [m_2]$, $k \in [n_1]$ and $l \in [n_2]$,
		\begin{align*}
	...
	\begin{ex} \lex{ex:tangent_spaces:products:0004}
		Let $M_1, M_2, N_1, N_2 \in \Obj(\bManinf)$, $F_1 \in \Hom_{\bManinf}(M_1, N_1)$, $F_2 \in \Hom_{\bManinf}(M_2, N_2)$ and $(p_1, p_2) \in M_1 \times M_2$. Define $\al_M \in \Iso_{\bVectR}(T_{(p_1, p_2)}(M_1 \times M_2), T_{p_1}M_1 \times  T_{p_2}M_2)$ and $\al_N \in \Iso_{\bVectR}(T_{(F_1(p_1), F(p_2))}(N_1 \times N_2), T_{F(p_1)}N_1 \times  T_{F(p_2)}N_2)$ as in \rex{ex:tangent_spaces:products:0002}. Then 
	...
	\begin{proof}
		...
		\end{align*}
		\rex{ex:tangent_spaces:products:0002} then implies that 
		\begin{align*}
	...
	\section{The Categories $\bManz$, $\bManBndinf$ and $\bManinf$}
	...
	\begin{ex}
		Let $M, N \in \Obj(\bManinf)$ and $F \in \Hom_{\bManinf}(M, N)$. Suppose that $\dim M = m$ and $\dim N = n$, $F$ has constant rank and $\rnk F = r$. Then for each $p \in M$, there exist $(U, \phi) \in \MA$ and $(V, \psi) \in \MB$ such that $p \in U$, $F(\cl U) \subset V$, $\cl U$ is compact and 
		$$\psi \circ F \circ \phi^{-1}(x^1, \ldots, x^k, x^{k+1}, \ldots, x^m) = (x^1, \ldots, x^k, 0, \ldots, 0)$$ 
	\end{ex}
	...
	\begin{ex}
		Let $M,N \in \Obj(\bManinf)$ and $F \in \Hom_{\bManinf}(M,N)$. Suppose that $F$ has constant rank. 
		\begin{enumerate}
			\item 
			\item 
			\item 
	...
	\begin{ex} \lex{def:maps_of_constant_rank:0003.1} \tbf{Global Rank Theorem:} \\
		...
		\begin{enumerate}
			...
			\item 
		\end{enumerate}
		If $F$ is surjective, then $F$ is a $\bManinf$-submersion,
	\end{ex}

	\begin{proof}
		Set $m \defeq \dim M$, $n \defeq \dim N$ and $r \defeq \rnk F$. Suppose that $F$ is surjective. For the sake of contradiction, suppose that $F$ is not a $\bManinf$ submersion. Then $r < n$. 
	...
	\begin{proof}
		...
		\begin{enumerate}
			\item Suppose that $F$ is surjective. For the sake of contradiction, suppose that $F$ is not a $\bManinf$-submersion. Then $r < n$. 
			\item 
			\item 
		\end{enumerate}
	\end{proof}
	...
	\begin{defn} \ld{def:immersions:0001}
		Let $M, N \in \Obj(\bManBndinf)$ and $F \in \Hom_{\bManBndinf}(M, N)$. Then $F$ is said to be a \tbf{$\bManBndinf(U,N)$-immersion} if for each $p \in M$, $DF(p):T_pM\rightarrow T_{F(p)}N$ is injective.
	\end{defn}
	
	\begin{note}
		When the context is clear, we say ``immersion" instead of ``$\bManBndinf(U,N)$-immersion".
	\end{note}

	\begin{ex} \lex{ex:immersions:0002} \tbf{Local Immersion Theorem:} \\
		Let $M, N \in \Obj(\bManBndinf)$, $F \in \Hom_{\bManBndinf}(M, N)$ and $p \in M$. If $DF(p)$ is injective, then there exists $U \in \MT_M$ such that $p \in U$ and $F|_U$ is an immersion. 
	\end{ex}

	\begin{proof}
		Suppose that $DF(p)$ is injective. Set $m \defeq \dim M$ and $n \defeq \dim N$. Since $DF(p)$ is injective, $m \leq n$. Since $F \in \Hom_{\bManBndinf}(M, N)$, there exists $(U_0, \phi_0) \in \MA_M$ and $(V, \psi) \in \MA_N$ such that $p \in U_0$, $F(U_0) \subset V$ and $\psi \circ F \circ \phi_0^{-1}$ is smooth. Define $F_0:\phi_0(U_0) \rightarrow \psi(V)$ by $F_0 \defeq \psi \circ F \circ \phi_0^{-1}$. \tcr{An ex in anlaysis notes} implies that $DF_0 \in \Hom_{\bManinf}(U_0, L(\R^m, \R^n))$. Define $A: L(\R^m, \R^n) \rightarrow M(m, n)$ \tcr{(notation?)} by $A(e_i \otimes e_j^*) \defeq E^{(ij)}$ (where $E^{(ij)}$ is $n \times m$ matrix of real numbers with $(i,j)$-th entry is $1$ and the rest are $0$) \tcr{(Define $A$ in prelim material)} \tcr{A prev ex (make ex after smooth manifold section ex about $L(V, M)$ being a smooth manifold)} implies that $A \in \Hom_{\bManinf}(L(\R^m, \R^n), M(m, n))$. Define $G: \phi_0(U_0) \rightarrow M(m, n)$ by $G \defeq A \circ DF_0$. We note that $G = [DF(\cdot)]_{\phi_0, \psi}$ \tcr{(show this)}. Define $a_0 \in \phi_0(U_0)$ by $a_0 \defeq \phi_0(p)$. Since $\rnk G(a_0) = m$, there exists $P \in \Perm(n), Q \in \Perm(m)$ such that $\prj_{m, m} P^* G(a_0) Q \in GL(m)$, \tcr{where $\prj_{m,m}$ means projecting $M \in M(m,n)$ onto $B \in M(m,m)$ given by $B(j,k) \defeq M_{j,k}$ for $j, k \in [m]$. (define this in prelim material)} Define $H:\phi_0(U_0) \rightarrow M(m, m)$ by $H(a) \defeq \prj_{m, m} P^* G(a) Q \in GL(m)$. Then $H \in \Hom_{\bManinf}(\phi(U_0), M(m,m))$. Since $GL(m) \in \MT_{M(m,m)}$ \tcr{(ref ex)}, $H(a_0) \in GL(m)$, and $H$ is continuous, we have that $H^{-1}(GL(m)) \in \MT_{\phi_0(U_0)}$ and $a_0 \in H^{-1}(GL(m))$. Set $U \defeq \phi_0^{-1}(H^{-1}(GL(m)))$ and $\phi \defeq \phi_0|_U$. Then $U \in \MT_M$, $p \in U$ and $F|_U$ is an immersion \tcr{work out explicitly}.   
		\tcr{FINISH!!!!}\\
		\tcr{FINISH!!!!}\\
		\tcr{FINISH!!!!}
	\end{proof}
	...
	\begin{ex} \lex{ex:immersions:0002.1}
		Let $M, N \in \Obj(\bManinf)$ and $F \in \Hom_{\bManinf}(M,N)$. Define $U \subset M$ by $U \defeq \{p \in M: \rnk DF(p) = \dim M\}$. Then 
		\begin{enumerate}
			\item $U \in \MT_M$,
	...
	\begin{defn}  \ld{def:immersions:0003}
		Let $M, N \in \Obj(\bManBndinf)$ and $F \in \Hom_{\bManBndinf}(M, N)$. Then $F$ is said to be a \tbf{$\bManBndinf$-embedding} if 
		\begin{enumerate}
			\item $F$ is a $\bManBndinf$-immersion, 
			\item $F|^{F(M)} \in \Iso_{\bTop}[(M, \MT_M), (F(M), \MT_N \cap F(M))]$.
		\end{enumerate}	 
	\end{defn}	
	...
	\begin{ex} \lex{ex:immersions:0003.1}
		Let $M,N \in \Obj(\bManBndinf)$, $F \in \Hom_{\bManBndinf}(M,N)$. Suppose that $F$ is an immersion. Then for each $U \in \MT_M$, $F|_U$ is an immersion. 
	\end{ex}
	...
	\begin{ex} \lex{ex:immersions:0003.2} \tbf{Local Embedding Theorem:} \\
		Let $M, N \in \Obj(\bManinf)$ and $F \in \Hom_{\bManinf}(M, N)$. Then $F$ is an immersion iff for each $p \in M$, there exists $U \in \MT_M$ such that $p \in U$ and $F|_U:U \rightarrow N$ is a $\bManinf$-embedding. \tcr{generalize to $\bManBndinf$ with local embedding theorem for manifolds with boundary with Lee pg 87}
	\end{ex}
	...
	\begin{proof} Set $\dim M = m$ and $\dim N = n$. 
		\begin{itemize}
			...
			\begin{itemize}
				\item Let $p \in M$. \rex{ex:maps_of_constant_rank:0003} implies that there exists $(U_0, \phi_0) \in \MA_M$ and $(V, \psi) \in \MA_N$ such that $p \in U$, $F(U_0) \subset V$, and $\psi \circ F \circ \phi^{-1} = (\id_{\phi(U_0)}, 0)$. Thus $\psi \circ F \circ \phi^{-1}$ is injective. Since $\phi, \psi$ are bijections and $F|_{U_0} = \psi^{-1} \circ (\psi \circ F \circ \phi^{-1}) \circ \phi$, we have that $F|_{U_0}$ is injective. Choose $K \subset U_0$ such that $K$ is compact and $p \in \Int K$. Since $F|_{U_0}$ is injective and continuous, $F|_{K}$ is injective and continuous. Since $K$ is compact and $N$ is Hausdorff, the \tcr{closed map lemma in the analysis notes section on compact spaces and continuity} implies that $F|_K: K \rightarrow F(K)$ is a homeomorphism. Set $U \defeq \Int K$. Then $F|_U:U \rightarrow F(U)$ is a homeomorphism. Since $F$ is an immersion, $F|_U$ is an immersion. Hence $F|_U$ is a $\bManinf$-embedding. 
				\tcr{generalize to boundary using Lee pg 87}
			\end{itemize}
			\item $(\impliedby):$ \\
			Suppose that for each $p \in M$, there exists $U \in \MT_M$ such that $p \in U$ and $F|_U:U \rightarrow N$ is a $\bManinf$-embedding. Let $p \in M$. Then there exists $U \in \MT_M$ such that $p \in U$ and $F|_U:U \rightarrow N$ is a $\bManinf$-embedding. Since $F|_U$ is a $\bManinf$-embedding, $F|_U$ is a $\bManinf$-immersion. Thus $DF|_U(p): T_pU \rightarrow T_pN$ is injective. Since $DF(p) = DF|_U(p)$, $DF(p): T_pM \rightarrow T_pN$ is injective. Since $p \in M$ is arbitrary, we have that for each $p \in M$, $DF(p)$ is injective. Hence $F$ is a $\bManinf$-immersion.
		\end{itemize}
	\end{proof}
...
\begin{ex} \lex{ex:immersions:0006}
	Let $M_1, M_2, N_1, N_2 \in \Obj(\bManinf)$, $F_1 \in \Hom_{\bManinf}(M_1, N_1)$ and $F_2 \in \Hom_{\bManinf}(M_2, N_2)$. 
	\begin{enumerate}
...
\begin{proof}\
	\begin{enumerate}
		\item Suppose that $F_1$ and $F_2$ are immersions. Set $n_1 \defeq \dim N_1$ and $n_2 \defeq \dim N_2$. Since $F_1, F_2$ are immersions, $\dim \Im DF_1(p_1) = n_1$ and $\dim \Im DF_2(p_2) = n_2$. Let $(p_1, p_2) \in M_1 \times M_2$. Then \tcr{cite exercise in section on products of tangent spaces}
		\begin{align*}
		...
		\item Suppose that $F_1$ and $F_2$ are embeddings. The previous part implies that $F_1 \times F_2$ is an immersion. Since $F_1$ and $F_2$ are embeddings, $F_1|^{F_1(M_1)} \in \Iso_{\bTop}(M_1, F_1(M_1))$ and $F_2|^{F_2(M_2)} \in \Iso_{\bTop}(M_2, F_2(M_2))$. \rex{} \tcr{(cite exercise here)} implies that 
		\begin{align*}
			(F_1 \times F_2)|^{F_1 \times F_2(M_1 \times M_2)} 
			& = F_1|^{F_1(M_1)} \times F_1|^{F_1(M_1)} \\
			& \in \Iso_{\bTop}( M_1 \times M_2, F_1(M_1) \times F_2(M_2)).
		\end{align*}
		Hence $F_1 \times F_2$ is a $\bTop$-embedding. Thus $F_1 \times F_2$ is an embedding.
	\end{enumerate}
	\tcr{FIX!!!}
\end{proof}

\begin{ex} \lex{ex:immersions:0006.01}
	Let $M, N_1, N_2 \in \Obj(\bManinf)$, $F_1 \in \Hom_{\bManinf}(M, N_1)$ and $F_2 \in \Hom_{\bManinf}(M, N_2)$. 
	\begin{enumerate}
...
\begin{proof}
	...
	\begin{enumerate}
		...
		\item Suppose that $F_1$ and $F_2$ are embeddings. The previous part implies that $G$ is an immersion. Since $F_1$ and $F_2$ are embeddings, $F_1|^{F_1(M)} \in \Iso_{\bTop}(M, F_1(M))$ and $F_2|^{F_2(M)} \in \Iso_{\bTop}(M, F_2(M))$. \rex{} \tcr{cite exercise here} implies that 
		\begin{align*}
			G|^{G(M)} 
			& = (F_1|^{F_1(M_1)}, F_1|^{F_1(M_1)}) \\
			& \in \Iso_{\bTop}( M, G(M)).
		\end{align*}
		Hence $G$ is a $\bTop$-embedding. Thus $G$ is an embedding.
		\tcr{FIX!!!}
	\end{enumerate}
\end{proof}

\begin{ex} \lex{ex:immersions:0007} \tbf{Local Representation of Immersions:} \\
	Let $M,N \in \Obj(\bManinf)$ and $F \in \Hom_{\bManinf}(M, N)$. Then $F$ is am immersion iff for each $p \in M$, there exist $(U, \phi) \in \MA_M$ and $(V, \psi) \in \MA_N$ such that $p \in U$, $\phi(U) = V$, and $\psi \circ F \circ \phi^{-1} = (\id_{\phi(U)}, 0)$.
\end{ex}
...
\end{ex}
	...
	\begin{defn} \ld{def:submersions:0001}
		Let $M, N \in \Obj(\bManBndinf)$ and $F \in \Hom_{\bManBndinf}(M,N)$. Then $F$ is said to be a \tbf{submersion} if for each $p \in M$, $DF(p) :T_pM\rightarrow T_{F(p)}N$ is surjective.
	\end{defn}

	\begin{ex} \lex{ex:submersions:0001.01}
		Let $M, N \in \Obj(\bManBndinf)$, $F \in \Hom_{\bManBndinf}(M, N)$ and $p \in M$. If $DF(p)$ is surjective, then there exists $U \in \MT_M$ such that $p \in U$ and $F|_U$ is a smooth submersion.
	\end{ex}
	...
	\begin{ex} \lex{ex:submersions:0001.02}
		Let $M, N \in \Obj(\bManinf)$ and $F \in \Hom_{\bManinf}(M,N)$. Define $U \subset M$ by $U \defeq \{p \in M: \rnk DF(p) = \dim N\}$. Then 
		\begin{enumerate}
			\item $U \in \MT_M$,
	...
	\begin{ex} \lex{ex:submersions:0001.1}
		Let $M,N \in \Obj(\bManinf)$. Then $\pi_M: M \times N \rightarrow M$ and $\pi_N: M \times N \rightarrow N$ are submersions.
	\end{ex}
	...
	\begin{ex} \lex{ex:submersions:0001.2}
		Let $E, M,N  \in \Obj(\bManinf)$,  $F \in \Hom_{\bManinf}(E, M)$, $G \in \Hom_{\bManinf}(M, N)$. If $F, G$ are submersions, then $G \circ F$ is a submersion.
	\end{ex}
	...
	\begin{ex} \lex{ex:submersions:0001.3}
		Let $M,N  \in \Obj(\bManinf)$ and  $F \in \Hom_{\bManinf}(M, N)$. Then $F$ is a submersion iff for each $p \in M$, there exists $U \in \MT_M$ such that $p \in M$ and $F|_U$ is a submersion.
	\end{ex}
	...
	\begin{ex} \lex{ex:submersions:0002}
		Let $M,N \in \Obj(\bManinf)$ be smooth manifolds, $F \in \Hom_{\bManinf}(M,N)$ a smooth map and $p \in M$.
		\begin{enumerate}
			\item If that $DF(p)$ is injective, then there exists $U \subset M$ such that $U$ is open and $F|_U$ is a smooth immersion.
			\item If $DF(p)$ is surjective, then there exists $U \subset M$ such that $U$ is open and $F|_U$ is a smooth submersion.
		\end{enumerate}
	...
	\begin{ex} \lex{ex:submersions:0002.1} \tbf{Local Representation of Submersions:} \\
		Let $E, M \in \Obj(\bManinf)$ and $\pi \in \Hom_{\bManinf}(E, M)$. Then $\pi$ is a submersion iff for each $a \in E$, there exist $(U, \phi) \in \MA_M$ and $(V, \psi) \in \MA_E$ such that $a \in V$, $U = \pi(V)$, and $\phi \circ \pi \circ \psi^{-1} = \prj^{n+k}_{[n]}|_{\psi(V)}$.
	\end{ex}
	...
	\begin{ex} \lex{ex:submersions:0002.2}
		Let $E, M \in \Obj(\bManinf)$ and $\pi \in \Hom_{\bManinf}(E, M)$.
		\begin{enumerate}
	...
	\begin{defn} \ld{def:submersions:0002.3}
		Let $E, M \in \Obj(\bManBndinf)$, $\pi \in \Hom_{\bManBndinf}(E, M)$ a surjection and $s: M \rightarrow E$. Then $s$ is said to be a $\bManBndinf$-section of $\pi$ if 
		\begin{enumerate}
			\item $s \in \Hom_{\bManBndinf}(M, E)$
			\item $s$ is a section of $\pi$
		\end{enumerate}
		We define $$\Gam(\pi) \defeq \{s \in \Hom_{\bManBndinf}(M, E): \text{$s$ is a $\bManBndinf$-section of $\pi$.}\}$$
	\end{defn}
	
	\begin{defn} \ld{def:submersions:0003}
		Let $E, M \in \Obj(\bManBndinf)$, $\pi \in \Hom_{\bManBndinf}(E, M)$, $U \in \MT_M$ and $s : U \rightarrow E$. Then 
		\begin{itemize}
			\item $(U, s)$ is said to be a \tbf{smooth local section of $\pi$} if $s \in \Gam(\pi|_{\pi^{-1}(U)})$, 
			\item for each $p \in M$, we define 
			$$\Gam_p(\pi) \defeq \{(U, s): \text{$(U, s)$ is a $\bManBndinf$-local section of $\pi$ and $p \in U$}\}$$
		\end{itemize}
	\end{defn}

	\begin{ex} \lex{ex:submersions:0004} \tbf{Local Section Theorem:} \\
		Let $E, M \in \Obj(\bManinf)$ and $\pi \in \Hom_{\bManinf}(E, M)$. Then $\pi$ is a submersion iff for each $p \in M$ and $a \in \pi^{-1}(\{p\})$, there exists $(U, s) \in \Gam_p(\pi)$ such that $s(p) = a$.
	\end{ex}
	...
	\begin{ex} \lex{ex:submersions:0005}
		Let $E, M, N \in \Obj(\bManinf)$, $\pi \in \Hom_{\bManinf}(E, M)$ and $F: M \rightarrow N$. Suppose that $\pi$ is a surjective submersion. Then $F \in \Hom_{\bManinf}(M, N)$ iff $F \circ \pi \in \Hom_{\bManinf}(E, N)$, in which case the following diagram commutes in $\bManinf$:
		\[ 
		\begin{tikzcd}
	...
	\begin{ex} \lex{ex:submersions:0007}
		Let $E, M, N \in \Obj(\bManinf)$, $\pi \in \Hom_{\bManinf}(E, M)$ and $F \in \Hom_{\bManinf}(E, N)$. Suppose that $\pi$ is a surjective submersion. If for each $a,b \in E$, $\pi(a) = \pi(b)$ implies that $F(a) = F(b)$, then there exists a unique $\tl{F} \in \Hom(\bManinf)(M, N)$ such that $\tl{F} \circ \pi = F$, i.e. the following diagram commutes:
		\[ 
		\begin{tikzcd}
...
\begin{defn} \ld{def:submanifolds:introduction:0000.1}
	Let $M, S \in \Obj(\bManinf)$. Suppose that $S \subset M$.
	\begin{itemize}
...
\begin{ex} \lex{ex:submanifolds:introduction:0000.2}
	Let $M, N, S \in \Obj(\bManinf)$ and $F \in \Hom_{\bManinf}(M, N)$. Suppose that $S$ is an immersed submanifold of $M$. Then $F|_S \in \Hom_{\bManinf}(S, N)$.   
\end{ex}

\begin{proof} 
	Since $S$ is an immersed submanifold of $M$, the inclusion $\iota_S \in \Hom_{\bManinf}(S, M)$. Therefore
	\begin{align*}
		F|_S
		& = F \circ \iota \\
		& \in \Hom_{\bManinf}(S, N).
...
\end{proof}
	...
	\begin{defn} \ld{def:submanifolds:introduction:0001}
		Let $(M, \MT_M, \MA_M), (S, \MT_S, \MA_S) \in \Obj(\bManinf)$. Suppose that $S \subset M$. Then $S$ is said to be an \tbf{embedded submanifold} of $M$ if the inclusion map $\iota_S:(S, \MT_S, \MA_S) \rightarrow (M, \MT_M, \MA_M)$ is a $\bManinf$-embedding.
	\end{defn}

	\begin{ex} \lex{ex:submanifolds:introduction:0002}
		Let $M, S \in \Obj(\bManinf)$. Suppose that $S \subset M$. If $S$ is an embedded submanifold of $M$, then $S$ is an immersed submanifold of $M$.
	\end{ex}
	...
	\begin{ex} \lex{ex:submanifolds:introduction:0002.1} \tbf{Immersed Implies Locally Embedded:} \\
		Let $M, S \in \Obj(\bManinf)$. Suppose that $S \subset M$. Then $S$ is an immersed submanifold of $M$ iff for each $p \in S$, there exists $U \in \MT_S$ such that $p \in U$ and $U$ is an embedded submanifold of $M$.
	\end{ex}

	\begin{proof}\
		\begin{itemize}
			\item $(\implies):$ \\
			Suppose that $S$ is an immersed submanifold fo $M$. Then $\iota_S:S \rightarrow M$ is an immersion. Let $p \in S$. Since $\iota_S$ is an immersion, \rex{ex:immersions:0003.2} implies that there exists $U \in \MT_S$ such that $p \in U$ and $\iota_S|_U$ is a $\bManinf$-embedding. Since $\iota_S|_U = \iota_U$, we have that $\iota_U$ is a $\bManinf$-embedding and $U$ is an embedded submanifold of $M$.
			\item $(\impliedby):$ \\
			Suppose that for each $p \in S$, there exists $U \in \MT_S$ such that $p \in U$ and $U$ is an embedded submanifold of $M$. Let $p \in S$. By assumption, there exists  $U \in \MT_S$ such that $p \in U$ and $U$ is an embedded submanifold of $M$. Thus $\iota_U$ is a $\bManinf$-embedding. Since $\iota_U = \iota_S|_U$, we have that $\iota_S|_U$ is a $\bManinf$-embedding. Since $p \in S$ is arbitrary, we have that for each $p \in S$, there exists $U \in \MT_S$ such that $p \in U$ and $\iota_S|_U$ is a $\bManinf$-embedding. \rex{ex:immersions:0003.2} implies that $\iota_S$ is an immersion. Thus $S$ is an immersed submanifold of $M$.
		\end{itemize}
	\end{proof}

	\begin{ex} \lex{ex:submanifolds:introduction:0002.2}  \tbf{Uniqueness of Topology for Embedded Submanifolds}
		Let $(M, \MT_M, \MA_M), (S, \MT_S, \MA_S) \in \Obj(\bManinf)$. Suppose that $S \subset M$ and $(S, \MT_S, \MA_S)$ is an embedded submanifold of $(M, \MT_M, \MA_M)$. Then $\MT_S = \MT_M \cap S$. 
	\end{ex}
	
	\begin{proof}
		Since $(S, \MT_S, \MA_S)$ is an embedded submanifold of $(M, \MT_M, \MA_M)$, $\iota_S \in \Iso_{\bTop}[(S, \MT_S), (S, \MT_M \cap S)]$. \tcb{An exercise in the analysis notes section on subspaces} implies that $\MT_S = \MT_M \cap S$.
...
%		\tcr{Make this an exercise in the analysis notes section on topology and subspaces, then just cite that exercise here in the context of smooth manifolds.}
	...
	\begin{ex} \lex{ex:submanifolds:introduction:0003}
		Let $M, N \in \Obj(\bManinf)$, $p \in M$ and $q \in N$. Then $M \times \{q\}$ and $N \times \{p\}$ are embedded submanifold of $M \times N$. 
		\tcr{move to later in products section as corollary}
	\end{ex}
	...
	\begin{defn} \ld{def:submanifolds:introduction:0005}
		Let $(M, \MA), (S, \MB) \in \Obj(\bManinf)$. Suppose that $S \subset M$ and $(S, \MB)$ is an embedded submanifold of $(M, \MA)$. Then $(S, \MB)$ is said to be \tbf{properly embedded} if $\iota_S:S \rightarrow M$ is proper.
	\end{defn}

	\begin{ex} \lex{ex:submanifolds:introduction:0006}
		Let $(M, \MA), (S, \MB) \in \Obj(\bManinf)$. Suppose that $S \subset M$ and $(S, \MB)$ is an embedded submanifold of $(M, \MA)$. Then $(S, \MB)$ is properly embedded iff $S$ is closed in $M$. 
	\end{ex}
	...
	\begin{defn} \ld{def:submanifolds:introduction:0010}
	...
	\begin{ex} \lex{ex:submanifolds:introduction:0011}
	...
	\begin{defn} \ld{def:submanifolds:introduction:0012}
	...
	\begin{ex} \lex{ex:submanifolds:introduction:0013}
	...
	\begin{ex} \lex{ex:submanifolds:introduction:0014}
	...
	\begin{ex} \lex{ex:submanifolds:introduction:0015}
		Let $M,S \in \Obj(\bManinf)$. Suppose that $S \subset M$. If $S$ is a $k$-dimensional embedded submanifold of $M$, then $S$ satisfies the local $k$-slice condition with respect to $M$. \\
	...
	\begin{ex} \lex{ex:submanifolds:introduction:0016}
		Let $(M, \MA) \in \Obj(\bManinf)$ and $S \subset M$. Suppose that $\dim M = n$ and $S$ satisfies the local $k$-slice condition with respect to $M$. Then 
		\begin{enumerate}
			\item for each $(U, \phi) \in \S^k(M; S)$, if $U \cap S \neq \varnothing$, then $(U \cap S, \pi_{n, k} \circ \phi|_{U \cap S}) \in X^k(S)$, 
			\item $(S, \MT_M \cap S) \in \Obj(\bManz)$ and $\dim S = k$.
		\end{enumerate}
	\end{ex}
	...
	\begin{proof}\
		\begin{enumerate}
			...
			\end{enumerate}
			Thus $(S, \MT_M \cap S) \in \Obj(\bManz)$ and $\dim S = k$.
		\end{enumerate}
	\end{proof}

	\begin{defn} \ld{def:submanifolds:introduction:0017}
		Let $(M, \MA) \in \Obj(\bManinf)$ and $S \subset M$. Suppose that $\dim M = n$ and $S$ satisfies the local $k$-slice condition with respect to $M$. We define 
		$$\MA|_S^0 \defeq \{(U \cap S, \pi^n_{[k]} \circ \phi_{U \cap S}): (U, \phi) \in \S^k(M; S)\}.$$
	\end{defn}

	\begin{ex} \lex{ex:submanifolds:introduction:0018}
		Let $(M, \MA) \in \Obj(\bManinf)$ and $S \subset M$. Suppose that $S$ satisfies the local $k$-slice condition with respect to $M$. Then 
		\begin{enumerate}
	...
	\begin{defn} \ld{def:submanifolds:introduction:0019}
			Let $(M, \MA) \in \Obj(\bManinf)$ and $S \subset M$. Suppose that $S$ satisfies the local $k$-slice condition with respect to $M$. We define the \tbf{embedded smooth structure on $S$ induced by $\MA$}, denoted $\MA|_S$, by 
			$$\MA|_S \defeq \al(\MA|_S^0).$$
	\end{defn}
	
	\begin{ex} \lex{ex:submanifolds:introduction:0020}
		Let $(M, \MA) \in \Obj(\bManinf)$ and $S \subset M$. Suppose that $S$ satisfies the local $k$-slice condition with respect to $M$. Then $(S, \MT_M \cap S, \MA|_S)$ is an embedded submanifold of $(M, \MT_M, \MA)$,
	\end{ex}
	...
	\begin{note}
		Let $(M, \MA) \in \Obj(\bManinf)$ and $S \subset M$. Suppose that $S$ satisfies the local $k$-slice condition with respect to $M$. Unless otherwise specified, we equip $S$ with $\MA|_S$.
	\end{note}

	\begin{ex} \lex{ex:submanifolds:introduction:0021}
		Let $M, N, S \in \Obj(\bManinf)$, $F \in \Hom_{\bManinf}(N, M)$. Suppose that $S \subset M$ and $S$ is an immersed submanifold of $M$, $F(N) \subset S$ and $F \in \Hom_{\bTop}(N, S)$. Then $F|^S \in \Hom_{\bManinf}(N, S)$. \\
		\tbf{Hint:} Note that $F = \iota_S \circ F|^S$.
	\end{ex}

	\begin{proof}
		Set $m \defeq \dim M$, $k \defeq \dim S$ and $n \defeq \dim N$. Set $F_0 \defeq F|^S$. We note that $\iota_S \circ F_0 = F$. Since $S$ is an immersed submanifold of $M$, $\iota_S$ is an immersion. Let $p \in N$. Define $q \in S$ by $q \defeq F(p)$. \rex{ex:immersions:0003.2} implies that there exist $(U, \phi) \in \MA_M$ and $(V, \psi) \in \MA_S$ such that $q \in V$, $\iota_S(V) \subset U$ and $\phi \circ \iota_S \circ \psi^{-1} = (\id_{\psi(V)}, 0)$. Since $F_0$ is $(\MT_N, \MT_S)$-continous, $F_0^{-1}(V) \in \MT_N$. Then there exists $(W, \eta) \in \MA_N$ such that $p \in W$ and $W \subset F_0^{-1}(V)$. Define $\wh{F}: \eta(W) \rightarrow \phi(U)$ and $\wh{F}_0: \eta(W) \rightarrow \psi(V)$ by $\wh{F} \defeq \phi \circ F \circ \eta^{-1}$ and $\wh{F}_0 \defeq \psi \circ F_0 \circ \eta^{-1}$. Since $F$ is smooth, $\wh{F}$ is smooth. Then 
		\begin{align*}
			(\wh{F}_0, 0)
	...
	\begin{ex} \lex{ex:submanifolds:introduction:0022}
		Let $M, N \in \Obj(\bManinf)$, $F \in \Hom_{\bManinf}(N, M)$ and $S \subset M$. Suppose that $S$ is an embedded submanifold of $M$ and $F(N) \subset S$. Then $F \in \Hom_{\bManinf}(N, S)$.
	\end{ex}

	\begin{proof}
		Since $S$ is an embedded submanifold of $M$, $\iota_S$ is a $\bManinf$-embedding. Let $V \in \MT_S$. Then 
		\begin{align*}
			V
			& = \iota_S(V) \\
			& \in \MT_M \cap S.
		...
		\begin{align*}
			...
			& \in \MT_N.
		\end{align*}
		Since $V \in \MT_S$ is arbitrary, we have that for each $V \in \MT_S$, $F^{-1}(V) \in \MT_N$. Hence $F \in \Hom_{\bTop}(N, S)$. Since $S$ is an embedded submanifold of $M$, $S$ is an immersed submanifold of $M$. \rex{ex:submanifolds:introduction:0021} then implies that $F \in \Hom_{\bManinf}(N, S)$.
	\end{proof}

	\begin{ex} \lex{ex:submanifolds:introduction:0023} \tbf{Uniqueness of Topological and Smooth Structure for Embedded Submanifolds} \\
		Let $(M, \MT_M, \MA_M), (S, \MT_S, \MA_S) \in \Obj(\bManinf)$. Suppose that $S \subset M$. If $(S, \MT_S, \MA_S)$ is an embedded submanifold of $(M, \MT_M, \MA_M)$, then 
		\begin{enumerate}
	...
	\begin{proof}
		...
		\begin{enumerate}
			\item Since $(S, \MT_S, \MA_S)$ is an embedded submanifold of $(M, \MT_M, \MA_M)$, $\iota_S \in \Iso_{\bTop}[(S, \MT_S), (S, \MT_M \cap S)]$. \tcb{An exercise in the analysis notes section on subspaces} implies that $\MT_S = \MT_M \cap S$. 
			\item Define $\iota:S \rightarrow S$ by $\iota(p) \defeq p$. Clearly, $\iota$ is a bijection. Since $(S, \MT_S, \MA_S)$ is an embedded submanifold of $(M, \MT_M, \MA_M)$, \rex{ex:submanifolds:introduction:0015} implies that $S$ satisfies the local $k$-slice condition with respect to $M$. \rex{ex:submanifolds:introduction:0020} then implies that $(S, \MT_M \cap S, \MA_M|_S)$ is an embedded submanifold of $(M, \MT_M, \MA_M)$.
			\begin{itemize}
				\item Since $(S, \MT_S, \MA_S)$ is an embedded submanifold of $(M, \MT_M, \MA_M)$, $\iota \in \Hom_{\bManinf}[(S, \MT_S, \MA_S), (M, \MT_M, \MA_M)]$. Since $\iota(S) = S$, \rex{ex:submanifolds:introduction:0022} implies that $\iota \in \Hom_{\bManinf}[(S, \MT_S, \MA_S), (S, \MT_M \cap S, \MA_M|_S)]$. 
				\item Since $(S, \MT_M \cap S, \MA_M|_S)$ is an embedded submanifold of $(M, \MT_M, \MA_M)$, $\iota^{-1} \in \Hom_{\bManinf}[(S, \MT_M \cap S, \MA_M|_S), (M, \MT_M, \MA_M)]$. Since $\iota^{-1}(S) = S$, \rex{ex:submanifolds:introduction:0022} again implies that $\iota^{-1} \in \Hom_{\bManinf}[(S, \MT_M \cap S, \MA_M|_S), (S, \MT_S, \MA_S)]$. 
			\end{itemize}
			\rex{ex:smooth_maps_open_boundary_submanifolds:0004.1} then implies that $\iota$ is a diffeomorphism and $\MA_S = \MA_M|_S$. 
		\end{enumerate}
	\end{proof}

	\begin{ex} \lex{ex:submanifolds:introduction:0024} \tbf{Uniqueness of Smooth Structure for Immersed Submanifolds}
		Let $(M, \MT_M, \MA_M) \in \Obj(\bManinf)$, $(S, \MT_S) \in \Obj(\bManz)$ and $\MA_1, \MA_2$ smooth structures on $(S, \MT_S)$. Suppose that $S \subset M$. If $(S, \MT_S, \MA_1)$ and $(S, \MT_S, \MA_2)$ are immersed submanifolds of $(M, \MT_M, \MA_M)$, then $\MA_1 = \MA_2$. 
	\end{ex}
	
	\begin{proof}
		Let $p \in S$. Since $(S, \MT_S, \MA_1)$, $(S, \MT_S, \MA_2)$ are immersed submanifolds of $(M, \MT_M, \MA_M)$, there exists $W_1, W_2 \in \MT_S$ such that $p \in W_1 \cap W_2$ and $(W_1, \MT_S \cap W_1, \MA_1|_{W_1})$, $(W_2, \MT_S \cap W_2, \MA_2|_{W_2})$ are embedded submanifolds of $(M, \MT_M, \MA_M)$. Define $W \in \MT_S$ by $W \defeq W_1 \cap W_2$. \rex{arg1} \tcr{(reference previous exercise about open submanifolds here)} implies that $(W, \MT_S \cap W, \MA_1|_W)$, $(W, \MT_S \cap W, \MA_2|_W)$ are embedded submanifolds of $(M, \MT_M, \MA_M)$. \rex{ex:submanifolds:introduction:0023} then implies that $\MT_S \cap W = \MT_M \cap W$ and 
		\begin{align*}
			\MA_1|_W
			& = \MA_M|_W \\
			& = \MA_2|_W.
	...
	\begin{ex} \lex{ex:submanifolds:introduction:0025}
		Let $M, S \in \Obj(\bManinf)$. Suppose that $S \subset M$ and $S$ is an immersed submanifold of $M$. If for each $p \in S$, there exists $U \in \MT_M$ such that $p \in U$ and $S \cap U$ is an embedded submanifold of $U$, then $S$ is an embedded submanifold of $M$. 
	\end{ex}
	...
	\begin{ex} \lex{ex:submanifolds:introduction:0026}
		\tcr{talk about the boundary as an embedded submanifold. In particular if $\dim M = n$, then $\p M$ satisfies the local $n-1$-slice condition} Let $M \in \Obj(\bManBndinf)$. Then $\p M$ is an embedded submanifold of $M$.  
	\end{ex}	
	...
	\begin{note}
		Let $M, N \in \Obj(\bManBndinf)$ and  $F \in \Hom_{\bManBndinf}(N, M)$. Suppose that $F$ is a  $\bManBndinf$-embedding. We recall that \rex{ex:top_submanifolds:embedded:0001} implies that $(F(N), \MT_M \cap F(N)) \in \Obj(\bManz)$.
	\end{note}
	
	\begin{ex} \lex{ex:submanifolds:embedded:embedded_images:0002}
		Let $M, N \in \Obj(\bManBndinf)$ and  $F \in \Hom_{\bManBndinf}(N, M)$. Suppose that $F$ is a  $\bManBndinf$-embedding. 
		$$\MA_0 \defeq \{(F(V), \psi \circ F|_V^{-1}): (V, \psi) \in \MA_N\}$$
		Then $\MA_0$ is a smooth atlas on $(F(N), \MT_M \cap F(N))$.
	\end{ex}
	
	\begin{proof}
		Set $n \defeq \dim N$. We note that since $F$ is a $\bManBndinf$-embedding, $F \in \Iso_{\bTop}((N, \MT_N), (F(N), \MT_M \cap F(N)))$. Since $\MA_N \subset X^n(N, \MT_N))$, \rex{ex:top_submanifolds:embedded:0001} implies that $\MA_0 \subset X^n(F(N), \MT_M \cap F(N))$.
		\begin{enumerate}
			...
			\item Let $(U, \phi), (V, \psi) \in \MA_0$. Then there exist $(U_0, \phi_0), (V_0, \psi_0) \in \MA_N$ such that $U = F(U_0)$, $V = F(V_0)$, $\phi = \phi_0 \circ F|_{U_0}^{-1}$ and $\psi = \psi_0 \circ F|_{V_0}^{-1}$. Since $\MA_N$ is a smooth atlas, $\psi_0|_{U_0 \cap V_0} \circ (\phi_0|_{U_0 \cap V_0})^{-1} \in \Iso_{\bManBndinf}(\phi_0(U_0 \cap V_0), \psi_0(U_0 \cap V_0))$. We note that  
			\begin{align*}
				\phi(U \cap V) 
			...
			\begin{align*}
				...
				& \in \Iso_{\bManBndinf}(\phi_0(U_0 \cap V_0), \psi_0(U_0 \cap V_0)) \\
				& = \Iso_{\bManBndinf}(\phi(U \cap V), \psi(U \cap V)).
			\end{align*}
	...
	\begin{defn} \ld{def:submanifolds:embedded:embedded_images:0003}
		Let $M, N \in \Obj(\bManBndinf)$ and $F \in \Hom_{\bManBndinf}(N, M)$. Suppose that $F$ is a  $\bManBndinf$-embedding. We define the \tbf{pushforward} of $\MA|_N$ to $F(N)$, denoted $F_*\MA_N$, by 
		$$F_*\MA_N \defeq \al (\{(F(V), \psi \circ F|_V^{-1}): (V, \psi) \in \MA_N\})$$
	\end{defn}

	\begin{ex} \lex{ex:submanifolds:embedded:embedded_images:0004}
		Let $M, N \in \Obj(\bManinf)$ and  $F \in \Hom_{\bManinf}(N, M)$. Suppose that $F$ is a  $\bManinf$-embedding. Then
		\begin{enumerate}
			\item $F|^{F(N)} \in \Iso_{\bManBndinf}((N, \MA_N), (F(N), F_*\MA_N))$
	...
	\begin{proof} Define $G:N \rightarrow F(N)$ by $G \defeq F|^{F(N)}$. Since $F$ is a $\bManBndinf$-embedding, $G \in \Iso_{\bTop}((N, \MT_N), (F(N), \MT_M \cap F(N)))$. 
		\begin{enumerate}
			\item 
			\begin{itemize}
			...
			\end{itemize}
			Hence $G \in \Iso_{\bManBndinf}((N, \MA_N), (F(N), F_*\MA_N))$.
			\item We note that $\iota_{F(N)} = F \circ G^{-1}$. Since $G$ is a diffeomorphism, $G$ is a $\bManBndinf$-embedding. Since $F$ is a $\bManBndinf$-embedding, \rex{} \tcr{make exercise about compositions of embeddings being embeddings} implies that $\iota_{F(N)}$ is a $\bManBndinf$-embedding. Hence $(F(N), F_*\MA_N)$ is an embedded submanifold of $M$. 
			\item \rex{ex:submanifolds:introduction:0023} implies that $F_*\MA_N = \MA_M|_{F(N)}$.
		\end{enumerate}
	\end{proof}
	
	\begin{ex} \lex{ex:submanifolds:embedded:level_sets:0005}
		Let $M,S \in \Obj(\bManBndinf)$. Suppose that $S \subset M$. Then $S$ is an embedded submanifold of $M$ iff there exists $N \in \Obj(\bManBndinf)$ and $F \in \Hom_{\bManBndinf}(N, M)$ such that $F$ is a $\bManBndinf$-embedding and $F(N) = S$.
	\end{ex}	
	...
	\begin{ex} \lex{ex:submanifolds:embedded:embedded_images:0006}
		Let $M \in \Obj(\bManinf)$. Then $\Del_M \in \Obj(\bManinf)$ and $\Del_M$ is an embedded submanifold of $M \times M$.
	\end{ex}

	\begin{proof}
		Define $F \in \Hom_{\bManinf}(M, M \times M)$ by $F \defeq (\id_M, \id_M)$. \rex{ex:immersions:0006.01} implies that $F$ is an embedding. Since $F(M) = \Del_M$, \rex{ex:submanifolds:embedded:embedded_images:0004} implies that $\Del_M$ is an embedded submanifold of $M \times M$.
	\end{proof}
	...
	\begin{ex} \lex{ex:submanifolds:embedded:level_sets:0007} \tbf{Constant Rank Level Set Theorem}: \\
		Let $M, N \in \Obj(\bManinf)$, $F \in \Hom_{\bManinf}(M,N)$ and $q_0 \in F(M)$. Suppose $F$ has constant rank and $\rnk F = r$. Then 
		\begin{enumerate}
	...
	\begin{ex} \lex{ex:submanifolds:embedded:level_sets:0008} \tbf{Submersion Level Set Theorem}: \\
		Let $M, N \in \Obj(\bManinf)$, $F \in \Hom_{\bManinf}(M,N)$. Set $m \defeq \dim M$ and $n \defeq \dim N$. Suppose $F$ is a submersion. Then for each $q \in N$, 
		\begin{enumerate}
	...
	\begin{defn} \ld{def:submanifolds:embedded:level_sets:0009}
		Let $M, N \in \Obj(\bManinf)$, $F \in \Hom_{\bManinf}(M,N)$ and $p \in M$ and $q \in N$. Then $p$ is said to be a 
		\begin{itemize}
	...
	\begin{ex} \lex{ex:submanifolds:embedded:level_sets:0010}
		Let $M, N \in \Obj(\bManinf)$ and $F \in \Hom_{\bManinf}(M,N)$. If $F$ is a submersion, then for each $q \in N$, $q$ is a regular value of $F$. 
	\end{ex}
	...
	\begin{defn} \ld{def:submanifolds:embedded:level_sets:0011}
		Let $M, N \in \Obj(\bManinf)$ $S \subset M$ and $F \in \Hom_{\bManinf}(M,N)$. Then $S$ is said to be a \tbf{regular level set of $F$} if there exists $q \in N$ such that $q$ is a regular value of $F$ and $S = F^{-1}(\{q\})$.
	\end{defn}
	
	\begin{ex} \lex{ex:submanifolds:embedded:level_sets:0012} \tbf{Regular Level Set Theorem}: \\
		Let $M, N \in \Obj(\bManinf)$, $S \subset M$ and $F \in \Hom_{\bManinf}(M,N)$. Set $m \defeq \dim M$ and $n \defeq \dim N$. Suppose that $S$ is a regular level set of $F$. Then
		\begin{enumerate}
	...
	\begin{proof}
		Define $U \subset M$ by $U \defeq \{p \in M: \rnk DF(p) = \dim N\}$. \rex{ex:submersions:0001.02} implies that $U \in \MT_M$ and $F|_U$ is a submersion. Since $S$ is a regular level set of $F$, there exists $q \in N$ such that $q$ is a regular value of $F$ and $S = F^{-1}(\{q\})$. Since $q$ is a regular value of $F$, for each $x \in S$, $x$ is a regular point of $F$. Thus for each $x \in S$, $DF(x)$ is surjective. Hence $S \subset U$. Since $F|_U$ is a submersion and 
		\begin{align*}
			S
			& = S \cap U \\
			& = F^{-1}(\{q\}) \cap U \\
			& = F|_U^{-1}(\{q\}),
		\end{align*}
		\rex{ex:submanifolds:embedded:level_sets:0008} implies that $S$ satisfies the local $(m-n)$-slice condition with respect to $U$ and $S$ is a properly embedded submanifold of $U$. Since $U \in \MT_M$, $U$ is a properly embedded submanifold of $M$. Hence $F^{-1}(\{q\})$ is a properly embedded submanifold of $M$. \tcr{(flesh out some of the last details here, like composition of proper maps is proper, composition of $\bManinf$-embeddings is a $\bManinf$-embedding, etc)}
		\tcr{FINISH!!!}
	\end{proof}

	\begin{defn} \ld{def:submanifolds:embedded:level_sets:0012.1}
		Let $M, N \in \Obj(\bManinf)$, $S \subset M$, $U \in \MT_M$ and $F \in \Hom_{\bManinf}(U, N)$. Suppose that $S \subset M$. Then $F$ is said to be a 
		\begin{itemize}
	...
	\begin{ex} \lex{ex:submanifolds:embedded:level_sets:0013}
		Let $M \in \Obj(\bManinf)$ and $S \subset M$. Set $m \defeq \dim M$. Then $S$ satisfies the local $k$-slice condition with respect to $M$ iff for each $p \in S$, there exists $U \in \MT_M$ and $F \in \Hom_{\bManinf}(U, \R^{m-k})$ such that $p \in U$ and $F$ is a local defining map for $S$.
	\end{ex}
	...
	\begin{proof}\
		\begin{itemize}
			\item $(\implies):$ \\
			\begin{itemize}
				\item Suppose that $S$ satisfies the local $k$-slice condition with respect to $M$. Let $p \in S$. Then there exists $(U, \phi) \in \S^k(M; S)$ such that $p \in U$. Define $(U_0, \phi_0) \in \MA_M|_S^0$ by $U_0 \defeq U \cap S$ and $\phi_0 \defeq \prj^m_{k} \circ \phi|_{U_0}$. Set $r \defeq m - k$ and define $F \in \Hom_{\bManinf}(U, \R^r)$ by $F \defeq \prj^m_{[-r]} \circ \phi$. Then $F \in \Hom_{\bManinf}(U, \R^r)$ and $p \in U$. By definition of $\S^k(M; S)$, $\phi(S \cap U) = \phi(U) \cap \S^{m,k}$. Hence 
				\begin{align*}
					F(S \cap U)
			...
			\end{itemize}
			Hence $F^{-1}(\{0\}) = S \cap U$. Let $q \in S \cap U$. \rex{ex:tangent_spaces:products:0005} implies that $[D \phi (q)]_{\phi, \id_{\R^m}} = 
			\begin{pmatrix}
			...
			\item $(\impliedby):$ \\
			Suppose that for each $p \in S$, there exists $U \in \MT_M$ and $F \in \Hom_{\bManinf}(U, \R^{m-k})$ such that $p \in U$ and $F$ is a local defining map for $S$. Let $p \in S$. By assumption, there exists $U \in \MT_M$ and $F \in \Hom_{\bManinf}(U, \R^{m-k})$ such that $p \in U$ and  $F$ is a local defining map for $S$. Thus $S \cap U$ is a regular level set of $F$. \rex{ex:submanifolds:embedded:level_sets:0012} then implies that $S \cap U$ satisfies the local $k$-slice condition with respect to $M$. Since $p \in S$ is arbitrary, we have that for each $p \in S$, there exists $U \in \MT_M$ such that $p \in U$ and $S \cap U$ satisfies the local $k$-slice condition with respect to $M$. \tcr{(need exercise showing that if the local $k$-slice condition is satisfied locally, around each point in $S$, it is satisfied globally.)} \rex{} implies that $S$ satisfies the local $k$-slice condition with respect to $M$.
		\end{itemize}
	\end{proof}
	...
	\begin{ex} \lex{ex:submanifolds:sub_embedded:001}
		Let $M, N, S \in \Obj(\bManinf)$. Suppose that $N \subset S$, $S \subset M$ and $S$ is an embedded submanifold of $M$. Then $N$ is an embedded submanifold of $S$ iff $N$ is an embedded submanifold of $M$.
	\end{ex}
	
	\begin{proof}\
		\begin{itemize}
			\item $(\implies):$ \\
			Suppose that $N$ is an embedded submanifold of $S$. Define $F \in \Hom_{\bManinf}(N, S)$ by $F(p) \defeq p$. Since $N$ is an embedded submanifold of $S$, $F$ is a $\bManinf$-embedding. We note that $\iota_N = \iota_S \circ F$. Since $S$ is an embedded submanifold of $M$, $\iota_S$ is a $\bManinf$-embedding. \rex{} \tcr{cite exercise about composition of embeddings is embedding} then implies that $\iota_N$ is a $\bManinf$-embedding. Since $\iota_N(N) = N$, \rex{ex:submanifolds:embedded:embedded_images:0004} implies that $N$ is an embedded submanifold of $M$.
			\item $(\impliedby):$ \\
			Suppose that $N$ is an embedded submanifold of $M$. Define $F:N \rightarrow S$ by $F \defeq \iota_N|^S$. Since $\iota_N \in \Hom_{\bManinf}(N, M)$, \rex{ex:submanifolds:introduction:0022} implies that $F \in \Hom_{\bManinf}(N, S)$. Since $N$ is an embedded submanifold of $M$, $\iota_N \in \Hom_{\bManinf}(N, M)$ is a $\bManinf$-embedding. Let $p \in N$. Since $\iota_N = \iota_S \circ F$, we have that 
			\begin{align*}
				D\iota_N(p) 
			...
			\begin{align*}
				...
				& \leq \dim T_pN.
			\end{align*} 
			Hence $\dim [ \Im DF(p)] = \dim T_pN$ and $DF(p)$ is injective. Since $p \in N$ is arbitrary, we have that for each $p \in N$, $DF(p)$ is injective. Thus $F$ is an immersion. \rex{ex:submanifolds:introduction:0023} implies that $\MT_N = \MT_M \cap N$ and $\MT_S = \MT_M \cap S$. \tcb{An exrcise in the analysis notes section on subspace topology} implies that $\MT_M \cap N = (\MT_M \cap S) \cap N$. Therefore 
			\begin{align*}
				\MT_N
				& = \MT_M \cap N \\
				& = (\MT_M \cap S) \cap N \\
				& = \MT_S \cap N. 
			\end{align*} 
			Hence $F|^N \in \Iso_{\bTop}((N, \MT_N), (N, \MT_S \cap N))$. Thus $F$ is a $\bManinf$-embedding and $N$ is an embedded submanifold of $S$.
		\end{itemize}
	\end{proof}
	...
	\begin{ex} \lex{ex:submanifolds:product_embedded:001}
		Let $M, N, E, F \in \Obj(\bManinf)$. Suppose that $E \subset M$, $F \subset N$, $E$ is an embedded submanifold of $M$ and $F$ is an embedded submanifold of $N$. Then $E \times F$ is an embedded submanifold of $M \times N$.
	\end{ex}

	\begin{proof}
		Since $\iota_E$ and $\iota_F$ are $\bManinf$-embeddings, \rex{ex:immersions:0006} implies that $\iota_E \times \iota_F$ is a $\bManinf$-embedding. Since $\iota_E \times \iota_F = \iota_{E \times F}$, $E \times F$ is an embedded submanifold of $M \times N$.  
	\end{proof}
	...
	\begin{defn}
		Let $M, S \in \Obj(\bManinf)$ and $p \in S$. Suppose that $S \subset M$ and $S$ is an embedded submanifold of $M$. We define the \tbf{embedded tangent space of $S$ with respect to $M$ at $p$}, denoted $\wt{T}_p (S, M)$, by 
		$$\wt{T}_p (S, M) \defeq \Im D \iota^M_S(p).$$
		\tcr{Need to make official notation the inclusion $\iota^M_S:S \rightarrow M$, then make note that when context is clear, we write $\iota_S$.}
	\end{defn}
	...
	\begin{note}
		Since $\iota_S$ is an immersion, $D\iota_S(p)|^{\wt{T}_pS} \in \Iso_{\bVectR}(T_pS, \wt{T}_pS)$ and $\dim \wt{T}_p S = \dim T_pS$.
	\end{note}

	\begin{ex} \lex{ex:submanifolds:tangent_space:0002}
		Let $M, N \in \Obj(\bManinf)$ and $F \in \Hom_{\bManinf}(N, M)$. Suppose that $F$ is a $\bManinf$-embedding. Then for each $p \in N$,
		\begin{enumerate}
			\item $DF|^{F(N)}(p) \in \Iso_{\bVectR}(T_pN, T_{F(p)}F(N))$
	...
	\begin{proof}
		...
		\begin{enumerate}
			\item \rex{ex:submanifolds:embedded:embedded_images:0004} implies that $G \in \Iso_{\bManinf}(N, S)$. Thus $DG(p) \in \Iso_{\bVectR}(T_pN, T_{F(p)}S)$.
			\item Since 
			\begin{align*}
				DF(p)
				& = D[\iota_S \circ G](p) \\
			...
			\begin{align*}
				...
				& = \wt{T}_{F(p)}S.
			\end{align*}
			\item Since $F$ is a $\bManinf$-embedding, $DF(p)$ is injective. Thus 
			\begin{align*}
				\dim \wt{T}_{F(p)}S
				& = \dim \Im DF(p) \\
				& = \dim DF(p)(T_p N) \\
	...
	\begin{ex} \lex{ex:submanifolds:tangent_space:0002.1}
		Let $M, S \in \Obj(\bManinf)$ and $p \in S$. Suppose that $S \subset M$ and $S$ is an embedded submanifold of $M$. Then $\dim \wt{T}_p S = \dim T_pS$. 
	\end{ex}

	\begin{proof}
		Define $F \in \Hom_{\bManinf}(S, M)$ by $F \defeq \iota_S$. \rex{ex:submanifolds:tangent_space:0002} implies that
		\begin{align*}
			\dim \wt{T}_p S
			& = \dim \wt{T}_{F(p)} F(S) \\
			& = \dim T_p S.
	...
	\begin{ex} \lex{ex:submanifolds:tangent_space:0003}
		Let $M, N, S \in \Obj(\bManinf)$, $U \in \MT_M$ and $F \in \Hom_{\bManinf}(U, N)$. Suppose that $S \subset M$, $S$ is an embedded submanifold of $M$ and $F$ is a local defining map for $S$. Then for each $p \in S \cap U$, $\wt{T}_p S = \ker D F(p)$.
	\end{ex}
	...
	\begin{proof}
		...
		\begin{itemize}
			...
			\begin{align*}
				...
				& \subset \ker DF(p).
			\end{align*}
			\item Set $m \defeq \dim M$, $n \defeq \dim N$ and $k \defeq \dim S$. Since $q$ is a regular value of $F$, $DF(p)$ is surjective. \rex{ex:submanifolds:embedded:level_sets:0012} implies that $k = m-n$ and therefore
			\begin{align*}
				\dim \ker DF(p)
	...
	\begin{ex} \lex{ex:submanifolds:tangent_space:0005}
		Let $M, S \in \Obj(\bManinf)$. Suppose that $S \subset M$ and $S$ is an embedded submanifold of $M$. Let $(U_0, \phi_0) \in \S^k(M;S)$. Write $\phi_0 = (x^1, \ldots, x^m)$. Define $\sig \in S_m$ and $G_0 \in \Hom_{\bManinf}(U_0, \R^{m-k})$ by $\sig \defeq 
		\begin{pmatrix}
	...
	\begin{proof}
		Define $F_0 \in \Hom_{\bManinf}(U_0, \R^k)$ by $F_0 \defeq \prj^m_k \circ \phi_0$. 
		\begin{itemize}
...
\begin{ex} \lex{ex:submanifolds:tangent_space:0006}
	Let $M \in \Obj(\bManinf)$ and $p \in M$. Define $\al \in \Iso_{\bVectR}(T_{(p,p)}(M \times M), T_pM \times  T_pM)$ as in \rex{ex:tangent_spaces:products:0002}. Then $\al (\wt{T}_{(p,p)} \Del_M) = \Del_{T_p M}$ and $\dim T_{(p,p) \Del_M} = \dim T_p M$.
\end{ex}

\begin{proof}
	Set $n \defeq \dim M$. Define $F \in \Hom_{\bManinf}(M, M \times M)$ by $F \defeq (\id_M, \id_M)$. \rex{ex:immersions:0006.01} implies that $F$ is an embedding. \rex{ex:submanifolds:embedded:embedded_images:0004} implies that $F|^{\Del_M} \in \Iso_{\bManinf}(M, \Del_M)$. Set $G \defeq F|^{\Del_M}$. Let $(U, \phi) \in \MA_M$. Write $\phi = (x^1, \ldots, x^n)$. Define $(V, \psi) \in \MA_M \otimes \MA_M$ by $V \defeq U \times U$ and $\psi \defeq \phi \times \phi$. Write $\psi = (\tl{y}^1, \ldots, \tl{y}^n, \tl{z}^1, \ldots, \tl{z}^n)$.
	\rex{ex:tangent_spaces:products:0006} implies that 
	\begin{align*}
	...
	\end{align*}
	Hence $\al \circ DF(p)(T_p M) = \Del_{T_p M}$. \rex{ex:submanifolds:tangent_space:0002} then implies that 
	\begin{align*}
		\al (\wt{T}_{(p,p)} \Del_M)
		& = \al \circ DF(p)(T_p M) \\
		& = \Del_{T_p M}.
	\end{align*}
	Since $\al$ and $DF(p)$ are injective, \rex{ex:submanifolds:tangent_space:0002} and \rex{ex:submanifolds:tangent_space:0002.1} imply that
	\begin{align*}
		\dim T_pM 
		& = \dim \Del_{T_p M} \\
...
\begin{ex} \lex{ex:submanifolds:tangent_space:0006.1}
	Let $M, S \in \Obj(\bManinf)$ and $p \in S$. Suppose that $S \subset M$ and $S$ is an immersed submanifold of $M$. Define $\al_M \in \Iso_{\bVectR}(T_{(p,p)}(M \times M), T_pM \times  T_pM)$ as in \rex{ex:tangent_spaces:products:0002}. Then for each $p \in M$, 
	$$\al_M (\wt{T}_{(p,p)} (\Del_S, M \times M)) = \Del_{\wt{T}_p (S, M)}.$$
\end{ex}

\begin{proof}
	Set $n \defeq \dim M$. Define $\al_S \in \Iso_{\bVectR}(T_{(p,p)}(S \times S), T_pS \times  T_pS)$ as in \rex{ex:tangent_spaces:products:0002}. \rex{ex:submanifolds:tangent_space:0006} implies that $\al_S(\wt{T}_{(p,p)} (\Del_S, S \times S)) = \Del_{T_p S}$. \rex{ex:tangent_spaces:products:0004} then implies that $\al_M \circ D\iota_{S \times S}(p,p) = {D \iota_S(p) \times D \iota_S (p)} \circ \al_S$,
...
\begin{ex} \lex{ex:submanifolds:tangent_space:0006.2}
	Let $M, S \in \Obj(\bManinf)$ and $p \in S$. Define $\prj_1 \in \Hom_{\bManinf}(M \times M, M)$ as in \ld{arg1} \tcr{(cite def)}. Suppose that $S \subset M$ and $S$ is an immersed submanifold of $M$. Then 
	$$D \prj_1(p,p) (\wt{T}_{(p,p)} \Del_{S}) = \wt{T}_p S.$$
\end{ex}

\begin{proof}
	\rex{ex:tangent_spaces:products:0002} \rex{ex:submanifolds:tangent_space:0001} \rex{ex:tangent_spaces:products:0006}
	\tcr{FINISH!!!}
\end{proof}

\begin{ex} \lex{ex:submanifolds:tangent_space:0007}
	Let $M, N, S, E \in \Obj(\bManinf)$, $p \in S$ and $q \in E$. Suppose that $S \subset M$, $S$ is an immersed submanifold of $M$, $E \subset N$, $E$ is an immersed submanifold of $N$. Define $\al \in \Iso_{\bVectR}(T_{(p,p)}(M \times M), T_pM \times  T_pM)$. Then
	$$\al (\wt{T}_{(p, q)} (S \times E)) = \wt{T}_pS \times \wt{T}_qE. $$
\end{ex}

\begin{proof}
	Define $\al_0 \in \Iso_{\bVectR}(T_{(p,q)}(S \times E), T_pS \times T_q E)$ and $\al \in \Iso_{\bVectR}(T_{(p, q)} (M \times N), T_p M \times T_q N)$ as in \rex{ex:tangent_spaces:products:0002}. \rex{ex:tangent_spaces:products:0004} then implies that $\al \circ D\iota_{S \times E}(p,q) = {D \iota_S(p) \times D \iota_E (q)} \circ \al_0$,
	i.e. following diagram commutes: 
	\[ 
	\begin{tikzcd}
...
\end{proof}
	...
	\begin{defn} \ld{def:submanifolds:transverality:0001}
		Let $M, S, S_2 \in \Obj(\bManinf)$. Suppose that $S_1, S_2 \subset M$, $S_1, S_2$ are immersed submanifolds of $M$. Then $S_1$ and $S_2$ are said to be 
		\begin{itemize}
			\item \tbf{transverse} if for each $p \in S_1 \cap S_2$, $T_pM = \wt{T}_p S_1 + \wt{T}_p S_2$
			\item \tbf{strictly transverse} if for each $p \in S_1 \cap S_2$, $T_pM = \wt{T}_p S_1 \oplus \wt{T}_p S_2$ \tcr{(maybe change vocab here from strict to strong or something)}
		\end{itemize} 
	\end{defn}
	...
	\begin{ex} \lex{ex:submanifolds:transverality:0003}
		Let $M, S \in \Obj(\bManinf)$ and $p \in S$. Suppose that $S \subset M$, $S$ is an embedded submanifold of $M$ and $\dim S < \dim M$. Then there exists $S' \in \Obj(\bManinf)$ such that $S' \subset M$, $S'$ is an immersed submanifold of $M$, $p \in S'$ and $S$, $S'$ are strictly transverse. 
	\end{ex}
...
%		\tcr{a previous exercise in the section on intro to submanifolds} implies that there exists $V \in \MT_S$ such that $p \in V$ and $(V, \MT_S \cap V, \MA_S|_V)$ is an embedded submanifold of $(M, \MT_M, \MA_M)$. 
		...
		\begin{align*}
			...
			& = p.
		\end{align*}
		\rex{ex:submanifolds:tangent_space:0001} implies that for each $j \in [k]$
		\begin{align*}
	...
	\begin{ex} \lex{ex:submanifolds:transverality:0003.1}
		Let $M, S \in \Obj(\bManinf)$ and $p \in S$. Suppose that $S \subset M$, $S$ is an embedded submanifold of $M$ and $\dim S < \dim M$. Then there exists $S' \in \Obj(\bManinf)$ such that $S' \subset M$, $S'$ is an embedded submanifold of $M$, $p \in S'$ and $S$, $S'$ are strictly transverse. 
	\end{ex}
	
	\begin{proof}
		Similar proof to previous ex, but use the fact that immersed submanifolds are locally embedded. \tcr{FINISH!!!!!}
	\end{proof}

	\begin{ex} \lex{ex:submanifolds:transverality:0004}
		Let $M, N, S_1, S_2, E_1, E_2 \in \Obj(\bManinf)$. Suppose that $S_1, S_2 \subset M$, $S_1, S_2$ are immersed submanifolds of $M$, $E_1, E_2 \subset N$ and $E_1, E_2$ are immersed submanifolds of $N$. If $S_1, S_2$ are strictly transverse and $E_1, E_2$ are strictly transverse, then $S_1 \times E_1$ and $S_2 \times E_2$ are strictly transverse. \tcr{(need exercise saying product of immersed submanifolds is immersed submanifold of product)}
	\end{ex}
	...
	\begin{proof}
		...
		\rex{ex:submanifolds:tangent_space:0007} implies that $\wt{T}_{p_M}S_1 \times \wt{T}_{p_N}E_1 = \al (\wt{T}_{(p_M, p_N)} (S_1 \times E_1))$ and $\wt{T}_{p_M}S_2 \times \wt{T}_{p_N}E_2 = \al (\wt{T}_{(p_M, p_N)} (S_2 \times E_2))$. Therefore
		\begin{align*}
			T_{(p_M, p_N)}(M \times N)
	...
	\begin{defn} \ld{def:submanifolds:transverality:0005}
		Let $M, N, S \in \Obj(\bManinf)$ and $F \in \Hom_{\bManinf}(N, M)$. Suppose that $S \subset M$, $S$ is an immersed submanifold of $M$. Then $F$ is said to be \tbf{transverse to $S$} if for each $p \in F^{-1}(S)$, 
		$$DF(p)(T_pN) + \wt{T}_{F(p)}S = T_{F(p)}M$$
	\end{defn}

	\begin{ex} \lex{ex:submanifolds:transverality:0006}
		Let $M, N, S \in \Obj(\bManinf)$ and $F \in \Hom_{\bManinf}(N, M)$. Suppose that $S \subset M$, $S$ is an immersed submanifold of $M$. If $F$ is a submersion, then $F$ is transverse to $S$.  
	\end{ex}
	...
	\begin{ex} \lex{ex:submanifolds:transverality:0007}
		Let $M, N, S \in \Obj(\bManinf)$ and $F \in \Hom_{\bManinf}(N, M)$. Suppose that $S \subset M$, $S$ is an embedded submanifold of $M$. If $F$ is transverse to $S$, then $F^{-1}(S)$ is an embedded submanifold of $N$ and $\codim F^{-1}(S) = \codim S$. 
	\end{ex}

	\begin{proof}
		Suppose that $F$ is transverse to $S$. Set $m \defeq \dim M$, $n \defeq \dim N$ and $k \defeq \dim S$. Let $p \in F^{-1}(S)$. Since $S$ is an embedded submanifold of $M$, there exists $(U_0, \phi_0) \in \S^k(M;S)$ such that $F(p) \in U_0$. Define $\sig \in S_m$ and $G_0 \in \Hom_{\bManinf}(U_0, \R^{m-k})$ by $\sig \defeq 
		\begin{pmatrix}
			1   & \ldots k & k+1 &  \ldots &  n \\
			k+1 & \ldots n & 1   &  \ldots &  k
		\end{pmatrix}$ 
		and $G_0 \defeq  \prj_{n-k} \circ (\sig \cdot \phi_0)$. Since $\phi_0 \in \S^k(M;S)$, $\phi_0(S \cap U_0) = \phi_0(U_0) \cap \S^{m,k}$. Therefore $G_0(S \cap U_0) = \{0\}$ and $G_0^{-1}(\{0\}) = S \cap U_0$. \rex{ex:submanifolds:tangent_space:0005} implies that $G_0$ is a submersion and for each $q \in U_0 \cap S$, $\ker DG_0(q) = \wt{T}_qS$. Define $V \in \MT_N$ and $G \in \Hom_{\bManinf}(V, \R^{m-k})$ by $V \defeq F^{-1}(U_0)$ and $G \defeq G_0 \circ F|_{V}$. Then 
		\begin{align*}
			G^{-1}(\{0\})
			& = (G_0 \circ F|_{V})^{-1}(\{0\}) \\
			& = F|_{V}^{-1} (G_0^{-1}(\{0\})) \\
	...
	\begin{note}
		\tcr{(check) I dont think the previous fact remains true in the case that $S$ is immersed. Since immersed submanifolds are locally embedded, you can use the previous result to show that $F^{-1}(S)$ is locally embedded and therefore immersed, maybe make this an exercise?} 
	\end{note}

	\begin{ex} \lex{ex:submanifolds:transverality:0008}
		Let $M, N, S \in \Obj(\bManinf)$ and $F \in \Hom_{\bManinf}(N, M)$. Suppose that $S \subset M$, $S$ is an embedded submanifold of $M$. If $F$ is a submersion, then $F^{-1}(S)$ is an embedded submanifold of $N$.
	\end{ex}
	...
	\begin{ex} \lex{ex:submanifolds:transverality:0009}
		Let $M, S_1, S_2 \in \Obj(\bManinf)$. Suppose that $S_1, S_2 \subset M$ and $S_1, S_2$ are embedded submanifolds of $M$. If $S_1$ and $S_2$ are transverse, then $S_1 \cap S_2$ is an embedded submanifold of $M$ and $\dim S_1 \cap S_2 = \dim S_1 + \dim S_2 - \dim M$. 
	\end{ex}
	...
	\begin{proof}
		...
		\begin{align*}
			...
			& = T_pM.
		\end{align*} 
		Since $p \in \iota_{S_1}^{-1}(S_2)$ is arbitrary, we have that for each $p \in \iota_{S_1}^{-1}(S_2)$, $D\iota_{S_1}(p)(T_p S_1) + \wt{T}_pS_2 = T_pM$ and $\iota_{S_1}$ is transverse to $S_2$. Since $S_1 \cap S_2 = \iota_{S_1}^{-1}(S_2)$, \rex{ex:submanifolds:transverality:0007} implies that $S_1 \cap S_2$ is an embedded submanifold of $S_1$ and 
	...
	\begin{note}
		Let $M, R \in \Obj(\bManinf)$. Suppose that $R \subset M \times M$. We denote the projection maps from $M \times M \rightarrow M$ by $\prj_1, \prj_2$. If $R$ is an equivalence relation on $M$, then 
		\begin{enumerate}
	...
	\begin{ex} \lex{ex:quotients:intro:0001}
		Let $M, R \in \Obj(\bManinf)$. Suppose that $R$ is a properly embedded submanifold of $M \times M$, $R$ is an equivlance relation on $M$, and $\prj_2|_R$ is a submersion. Then 
		\begin{enumerate}
	...
	\begin{ex} \lex{ex:quotients:intro:0002}
		Let $M, R \in \Obj(\bManinf)$. Suppose that $R$ is a properly embedded submanifold of $M \times M$, $R$ is an equivlance relation on $M$, and $\prj_1|_R$ is a submersion. Then for each $p \in M$, $\pi(p)$ is a properly embedded submanifold of $M$ and $\dim \pi(p) = \dim R - \dim M$. \\
		\tbf{Hint:} For each $p \in M$, $\pi(p) = \prj_1|_R(\prj_2|_R^{-1}(\{p\}))$ and $\prj_1|_{M \times \{p\}} \in \Iso_{\bManinf}(M \times \{p\}, M)$.
	\end{ex}

	\begin{proof}
		Let $p \in M$. \rex{arg1} implies that $\prj_1: M \times M \rightarrow M$ is a submersion. \rex{} implies that $M \times \{p\}$ is an embedded submanifold of $M \times M$. \rex{arg1} implies that $\prj_2|_R$ is a submersion. Since $\prj_2|_R$ is a surjective submersion, \rex{arg1} implies that $\prj_2|_R^{-1}(\{p\})$ is a properly embedded submanifold of $R$ and $\dim \prj_2|_R^{-1}(\{p\}) = \dim R - \dim M$. Since $R$ is an embedded submanifold of $M \times M$, \rex{arg1} \tcr{(need to make) exercise in section on embedded submanifolds subsection on subspaces} implies that $\prj_2|_R^{-1}(\{p\})$ is an embedded submanifold of $M \times M$. Since $\prj_2|_R^{-1}(\{p\}) \subset M \times \{p\}$ \rex{ex:submanifolds:sub_embedded:001} implies that $\prj_2|_R^{-1}(\{p\})$ is an embedded submanifold of $M \times \{p\}$. Since $\prj_1|_{M \times \{p\}} \in \Iso_{\bManinf}(M \times \{p\}, M)$ is a diffeomorphism and $\pi(p) = \prj_1|_{M \times \{p\}}(\prj_2|_R^{-1}(\{p\}))$, \rex{arg1} \tcr{make exercise in the section on embedded submanifolds} implies that $\pi(p)$ is an embedded submanifold of $M$ and $\dim \pi(p) = \dim R - \dim M$.
	\end{proof}

	\begin{ex} \lex{ex:quotients:intro:0003}
		Let $M, R, S' \in \Obj(\bManinf)$ and $p \in S'$. Suppose that $R \subset M \times M$, $R$ is a properly embedded submanifold of $M \times M$, $R$ is an equivlance relation on $M$, $\prj_1|_R$ is a submersion, $S' \subset M$, $S'$ is an embedded submanifold of $M$, $\dim S' = \dim M - \dim \pi(p)$ and $S'$ is transverse to $\pi(p)$. Define $Z \subset R$ by $Z \defeq \prj_2|_R^{-1}(S')$. Then 
		\begin{enumerate}
			\item $(p,p) \in Z$
			\item $Z$ is an embedded submanifold of $R$ and $\dim Z = \dim M$
			\item $D\prj_1|_Z(p,p) \in \Iso_{\bVectR}(T_{(p,p)}Z, T_pM)$ \\
			\tbf{Hint:} 
			\begin{enumerate}
	...
	\begin{proof}\
		\begin{enumerate}
			...
			\begin{align*}
				...
				& = Z.
			\end{align*}
			\item \tcr{since $\prj_1|_R$ is a submersion and $R$ is an equivalence relation, $\prj_2|_R$ is a submersion.} Since $S'$ is an embedded submanifold of $M$, \rex{ex:submanifolds:transverality:0008} implies that $Z$ is an embedded submandifold of $R$. \rex{} \tcr{Another exercise in the section on level sets as embedded submanifolds} implies that
			\begin{align*}
				\dim Z 
				& = \dim M \times M - \dim M \\
				& = 2 \dim M - \dim M \\
			...
			\begin{enumerate}
				\item 
				\begin{itemize}
					\item We note that \rex{ex:submanifolds:embedded:embedded_images:0006} implies that $\Del_{S'}$ is an embedded submanifold of $S' \times S'$. Since $S'$ is an embedded submanifold of $M$, \rex{ex:submanifolds:product_embedded:001} implies that $S' \times S'$ is an embedded submanifold of $M \times M$. \rex{ex:submanifolds:sub_embedded:001} then implies that $\Del_{S'}$ is an embedded submanifold of $M \times M$. 
					...
					\begin{align*}
						...
						& = Z.
					\end{align*}
					\item Define $N \in \Obj(\bManinf)$ by $N \defeq N$. \rex{ex:quotients:intro:0002} implies that $\pi(p)$ is an embbeded submanifold of $M$. \rex{arg1} \tcr{(need exercise)} implies that $\{p\}$ is an embedded submanifold of $M$. \rex{ex:submanifolds:product_embedded:001} then implies that $N$ is an embedded submanifold of $M \times M$. We note that by definition, $N \subset R$. Since for each $(a,p) \in N$, 
					\begin{align*}
						\prj_2(a, p) 
						& = p \\
						& \in S',
					...
					\end{align*}
				\end{itemize}
				Since $Z$ is an embedded submanifold of $R$ and $R$ is an embedded submanifold of $M \times M$, \rex{ex:submanifolds:sub_embedded:001} implies that $Z$ is an embedded submanifold of $M \times M$. Since $\Del_{S'}, N \subset Z$ and $\Del_{S'}, N$ are embedded submanifolds of $M \times M$, \rex{ex:submanifolds:sub_embedded:001} then implies that $\Del_{S'}, N$ are embedded submanifolds of $Z$. 
				\item Define $\al \in \Iso_{\bVectR}(T_{(p,p) M \times M}, T_pM \times T_p M)$ as in \rex{ex:tangent_spaces:products:0002}. \rex{ex:submanifolds:tangent_space:0006.1} and \rex{ex:submanifolds:tangent_space:0007} then imply that
				\begin{align*}
				...
				\end{align*}
				We note that \rex{ex:submanifolds:tangent_space:0002.1} and \rex{ex:submanifolds:tangent_space:0006} imply that
				\begin{align*}
					...
					& = \dim \wt{T}_{p} (S', M).
				\end{align*} 
				\rex{ex:tangent_spaces:products:0004} \rex{ex:submanifolds:tangent_space:0007} and the injectivity of $\al$ imply that
				\begin{align*}
			...
			\begin{align*}
				...
				& = T_p M
			\end{align*}
			Thus $D \prj_1|_Z(p,p)$ is surjective. Since $\dim Z = \dim M$, we have that $D \prj_1|_Z(p,p)$ is a bijection. Hence $D \prj_1|_Z(p,p) \in \Iso_{\bVectR}(T_{(p,p)} Z, T_p M)$.
			\tcr{FIX!!!}
		\end{enumerate}
	\end{proof}
...
\begin{defn} \ld{def:quotients:godement:0001}
	Let $M, R, S \in \Obj(\bManinf)$, $U \in \MT_M$ and $q \in \Hom_{\bManinf}(U, S)$. Suppose that $R$ is a properly embedded submanifold of $M \times M$, $R$ is an equivlance relation on $M$, $S \subset U$ and $S$ is a properly embedded submanifold of $U$. Then $(S, q)$ is said to be a \tbf{$R$-slice of $U$} if for each $p \in U$, $\pi(p) \cap S = \{q(p)\}$
\end{defn}

\begin{ex} \lex{ex:quotients:godement:0002}
...
\begin{ex} \lex{ex:quotients:godement:0003} \tbf{Slice Theorem:}
	Let $M, R \in \Obj(\bManinf)$. Suppose that $R$ is a properly embedded submanifold of $M \times M$, $R$ is an equivlance relation on $M$. Then for each $p \in M$, there exists $U \in \MT_M$, $S \in \Obj(\bManinf)$ and $q \in \Hom_{\bManinf}(U, S)$ such that $p \in U$, $S \subset U$ and $S$ is a properly embedded submanifold of $U$ and $(S,q)$ is a $R$-slice of $U$. 
\end{ex}

\begin{proof}
	Let $p \in M$. Define $S' \in \EmbSubmaninf(M)$ by $S' \defeq \pi(p)$. Then $\dim S' = \tcr{????}$. \rex{ex:submanifolds:transverality:0003.1} implies that there exists  $S \in \EmbSubmaninf(M)$, such that $p \in S$ and $S, S'$ are strictly transverse. Define $Z \in \EmbSubmaninf(R)$ by $Z \defeq \prj_2|_R^{-1}(S)$. \rex{ex:quotients:intro:0003} implies that \tcr{(verify assumption in this ex)} $D\prj_1|_Z(p,p) \in \Iso_{\bVectR}(T_{(p,p)}Z, T_pM)$. \tcr{ref ex on local diffeomorphisms} implies that there exists \tcr{FINISH!!!}
\end{proof}
...
\begin{defn}
	Let $M \in \Obj(\bManBndinf)$. Set $n \defeq \dim M$. We define the \tbf{tangent bundle of $M$}, denoted $TM$, by  
...
\begin{ex}
	Let $M \in \Obj(\bManBndinf)$ and $(U, \phi) \in \MA_M$. Set $n \defeq \dim M$. Then
	\begin{itemize}
...
\begin{ex}
	Let $M \in \Obj(\bManBndinf)$. Then there exists a unique topology $\MT_{TM}$ on $TM$ and smooth structure $\MA_{TM}$ on $(TM, \MT_{TM})$ such that $(TM, \MT_{TM}, \MA_{TM}) \in \Obj(\bManBndinf)$,  $(\pi^{-1}(U_{\al}), \tl{\phi}_{\al})_{\al \in \Gam} \subset \MA_{TM}$ and $\pi \in \Hom_{\bManBndinf}(TM, M)$. 
\end{ex}
...
\begin{proof}
	...
	\begin{enumerate}[label=(\alph*)]
		...
		\item Since $M \in \Obj(\bManBndinf)$, $M$ is second-countable. Thus $M$ is Lindelof. Since $(U_{\al}, \phi_{\al})_{\al \in A}$ is an atlas on $M$, $(U_{\al})_{\al \in \Gam}$ is an open cover of $M$. Hence there exists $\Gam' \subset \Gam$ such that $\Gam'$ is countable and $M \subset \bigcup\limits_{\al \in \Gam'} U_{\al}$. Hence 
		\begin{align*}
			TM
			& = \pi^{-1}(M) \\
		...
		\end{itemize}
	\end{enumerate}
	\rex{ex:smooth_manifold_introduction:0008} implies that there exists a unique topology $\MT_{TM}$ on $TM$ and smooth structure $\MA_{TM}$ on $(TM, \MT_{TM})$ such that $(TM, \MT_{TM}, \MA_{TM}) \in \Obj(\bManBndinf)$ and $(\pi^{-1}(U_{\al}), \tl{\phi}_{\al})_{\al \in \Gam} \subset \MA_{TM}$. \vspace{.2cm} \\
...
\begin{ex}
	Let $M \in \Obj(\bManBndinf)$. Then $\pi: TM \rightarrow M$ is a submersion.
\end{ex}
...
\begin{ex}
	Let $M \in \Obj(\bManBndinf)$ and $(U, \phi) \in \MA_M$. Set $n \defeq \dim M$ and write $\phi = (x^1, \ldots, x^n)$ and $\tl{\phi} = (\tl{x}^1, \ldots, \tl{x}^n, \tl{y}^1, \ldots, \tl{y}^n)$. Then for each $(p,v) \in \pi^{-1}(U)$,
	\begin{enumerate}
...
\begin{defn}
	Let $M,N \in \Obj(\bManBndinf)$ and $F \in \Hom_{\bManBndinf}(M, N)$. We define the \tbf{pushforward of $F$}, denoted by $DF: TM \rightarrow TN$ by 
	$$DF(p, v) \defeq (F(p), DF(p)(v))$$
\end{defn}

\begin{note}
	Another common notation for $DF$ is $TF$.
\end{note}

\begin{ex}
	Let $M,N \in \Obj(\bManBndinf)$ and $F \in \Hom_{\bManBndinf}(M, N)$. Then 
	\begin{enumerate}
		\item $\pi_{TN} \circ DF = F \circ \pi_{TM}$, i.e. the following diagram commutes: 
		\[ 
		\begin{tikzcd}
			TM \arrow[r, "DF"] \arrow[d, "\pi_{TM}"'] & TN \arrow[d, "\pi_{TN}"]  \\
			M \arrow[r, "F"']                   & N
		\end{tikzcd}
		\]
		\item for each $V \in \MT_N$, $DF^{-1}(\pi_{T_N}^{-1}(V)) = \pi_{TM}^{-1}(F^{-1}(V))$
	\end{enumerate}
\end{ex}
...
\begin{proof}\
	\begin{enumerate}
		...
		\begin{align*}
			...
			& = F \circ \pi_{TM}(p,v).
		\end{align*}
		Thus $\pi_{TN} \circ DF = F \circ \pi_{TM}$.
...
\begin{ex}
	Let $M,N \in \Obj(\bManBndinf)$ and $F \in \Hom_{\bManBndinf}(M, N)$. Then $DF \in \Hom_{\bManBndinf}(TM, TN)$.
\end{ex}

\begin{proof}
	Let $(p,v) \in TM$. Since $\MA_M$ is an atlas on $M$ and $\MA_N$ is an atlas on $N$, there exist $(U, \phi) \in \MA_M$ and $(V, \psi) \in \MA_N$ such that $p \in U$ and $F(p) \in V$. Since $p \in U$, $(p,v) \in \pi_{TM}^{-1}(U)$. \tcr{The previous exercise} implies that $DF^{-1}(\pi_{TN}^{-1}(V)) = \pi_{TM}^{-1}(F^{-1}(V))$. Since $F$ is smooth, $U \cap F^{-1}(V) \in \MT_M$. Since $\pi_{TM}$ is smooth, we have that 
	\begin{align*}
		\pi_{TM}^{-1}(U) \cap DF^{-1}(\pi_{TN}^{-1}(V)) 
		& = \pi_{TM}^{-1}(U) \cap \pi_{TM}^{-1}(F^{-1}(V)) \\
		& = \pi_{TM}^{-1}(U \cap F^{-1}(V)) \\
		& \in \MT_{TM}.
	\end{align*}
	Set $m \defeq \dim M$, $n \defeq \dim N$ and write $\phi = (x^1, \ldots, x^m)$ and $\psi = (y^1, \ldots, y^n)$.  Then for each $(a, \xi^1, \ldots, \xi^m) \in \tl{\phi}[\pi_{TM}^{-1}(U) \cap DF^{-1}(\pi_{TN}^{-1}(V))]$, we have that
	\begin{align*}
		\tl{\psi} \circ DF \circ \tl{\phi}^{-1}(a, \xi^1, \ldots, \xi^m)
		& = \tl{\psi} \circ DF\bigg(\phi^{-1}(a), \sum_{j=1}^n \xi^j \frac{\p}{\p x^j} \bigg|_{\phi^{-1}(a)} \bigg) \\
	...
	\end{align*}
	Thus $\tl{\psi} \circ DF \circ \tl{\phi}|_{\pi_{TM}^{-1}(U) \cap DF^{-1}(\pi_{TN}^{-1}(V))}^{-1}$ is smooth. \rex{ex:smooth_maps_between_manifolds:0003} implies that $DF$ is smooth. \tcr{(maybe add more details here)}.
\end{proof}

\begin{ex}
	Let $M,N, E \in \Obj(\bManBndinf)$, $F \in \Hom_{\bManBndinf}(M, N)$ and $G \in \Hom_{\bManBndinf}(N, E)$. Then
	\begin{enumerate}
		\item for each $p \in M$, $DF|_{\{p\} \times T_pM} = \id_{\{p\}} \times DF(p)$.
		\item $D(G \circ F) = DG \circ DF$
		\item $D(\id_M) = \id_{TM}$
		\item $F \in \Iso_{\bManBndinf}(M, N)$ implies that $DF \in \Iso_{\bManBndinf}(TM, TN)$ and $D(F^{-1}) = DF^{-1}$. 
	\end{enumerate}
\end{ex}
...
\begin{defn}
	Let $M \in \Obj(\bManBndinf)$. We define the \tbf{vector fields on $M$}, denoted $\MFX(M)$, by $\MFX(M) \defeq \Gam(\pi_{TM})$. 
\end{defn}

\begin{ex}
	Let $M \in \Obj(\bManBndinf)$ and $X: M \rightarrow TM$. If $X$ is a section of $\pi_{TM}$, then for each $p \in M$, $X(p) \in \{p\} \times T_p M$.
\end{ex}
...
\begin{note}\
	\begin{itemize}
		\item When the context is clear, we may write $X_p$ in place of $X(p)$.
		\item If 
		\begin{align*}
			X_p 
			& = (p, v) \\
			& \in \{p\} \times T_pM,
		\end{align*}
		we identify $X_p$ with $v$. So we may write $X_p \in T_pM$ or $X_p \in TM$.
	\end{itemize}
\end{note}

\begin{ex}
	Let $M \in \Obj(\bManBndinf)$ and $U \in \MT_M$. Then 
	\begin{enumerate}
		\item $X(U) \subset TU$
		\item for each $X \in \MFX(M)$, $X|_U^{TU} \in \MFX(U)$. 
	\end{enumerate}
\end{ex}

\begin{proof}
	\tcr{(need to make exercise and develope tangent space in terms of germs so tangent space is independent of neighborhood of point and $T_pU = T_pM$, then make exercise in section on tangent bundle that $TU \subset TM$)}
	\begin{enumerate}
		\item \tcr{An ex in tangent bundle intro section} implies that for each $p \in U$,
		\begin{align*}
			X_p
			& \in \{p\} \times T_pM \\
			& = \{p\} \times T_pU \\
		...
		\end{align*}
		Thus $X(U) \subset TU$.
		Thus there exists $v \in T_pM$ such that 
		\item 
		\begin{itemize}
			\item Since $X \in \Hom_{\bManBndinf}(M, TM)$ and $\iota_U \in \Hom_{\bManBndinf}(U, M)$, we have that 
			\begin{align*}
				X|_U 
				& = X \circ \iota_U \\
				& \in \Hom_{\bManBndinf}(U, TM).
			\end{align*}
			Since  
			\begin{align*}
				X|_U(U) 
				...
				& \subset TU.
			\end{align*}
			and $TU$ is an open submanfold of $TM$ \tcr{(make/ref ex about this)}, \tcr{a prev ex in embedded submanifolds} implies that $X|_U^{TU} \in \Hom_{\bManBndinf}(U, TU)$. 
			\item Since for each $p \in U$,
			\begin{align*}
				\pi_{TU} \circ X|_U (p)
				& = \pi_{TM} \circ X|_U(p) \\
				& = \pi_TM \circ [X \circ \iota_U](p) \\
				& = [\pi_TM \circ X] \circ \iota_U(p) \\
				& = \id_M \circ \iota_U(p) \\
				& = \iota_U(p) \\
				& = p \\
				& = \id_U(p).
			\end{align*}
			Thus $\pi_{TU} \circ X|_U^{TU} = \id_U$.
		\end{itemize}
		Hence $X|_U^{TU} \in \MFX(U)$.
	\end{enumerate}
\end{proof}

\begin{defn}
	Let $M \in \Obj(\bManBndinf)$ and $(U, \phi) \in \MA_M$. Set $n \defeq \dim M$ and write $\phi = (x^1, \ldots, x^n)$. Let $X: M \rightarrow TM$. Suppose that $X$ is a section of $\pi_{TM}$.For each $j \in [n]$, we define the \tbf{$j$-th component function of $X$ with respect to $(U, \phi)$}, denoted $X^j:U \rightarrow \R$, by $X^j(p) \defeq dx^j_p(X_p)$. 
\end{defn}

\begin{note}
	In particular, for each $p \in U$, $[\tl{\phi} \circ X](p) = (\phi(p), X^1_p, \ldots, X^n_p)$ and 
	$$X_p = \sum\limits_{j=1}^n X^j(p) \frac{\p}{\p x^j} \bigg|_p.$$ 
\end{note}

\begin{ex}
	Let $M \in \Obj(\bManBndinf)$ and $(U, \phi) \in \MA_M$. Set $n \defeq \dim M$ and write $\phi = (x^1, \ldots, x^n)$. Let $X: M \rightarrow TM$. Suppose that $X$ is a section of $\pi_{TM}$. Then $X|_U \in \MFX(U)$ iff for each $j \in [n]$, $X^j \in C^{\infty}(U)$.
\end{ex}
...
\begin{ex}
	Let $M \in \Obj(\bManBndinf)$ and $X: M \rightarrow TM$. Set $n \defeq \dim M$. Suppose that $X$ is a section of $\pi_{TM}$. Then $X \in \MFX(M)$ iff for each $(U, \phi) \in \MA_M$, $X^1, \ldots, X^n \in C^{\infty}(U)$.
\end{ex}
...
\begin{ex}
	Let $M \in \Obj(\bManBndinf)$ and $(U, \phi) \in \MA_M$. Set $n \defeq \dim M$ and write $\phi = (x^1, \ldots, x^n)$. Then for each $j \in [n]$, $\frac{\p}{\p x^j} \in \MFX(U)$.
\end{ex}
...
\begin{ex} 
	...
	\begin{enumerate}
		...
		\begin{enumerate}
			...
			\item $X+Y \in \MFX(M)$
		\end{enumerate}
		\item $\MFX(M) \in \Obj(\bMod_{C^{\infty}(M)})$.
	\end{enumerate}
\end{ex}
...
\begin{defn}
	Let $M \in \Obj(\bManBndinf)$ and $D: C^{\infty}(M) \rightarrow C^{\infty}(M)$. Then $D$ is said to be a \tbf{derivation on $C^{\infty}(M)$} if 
	\begin{itemize}
		\item (linearity): \\
...
\begin{ex}
	Let $M \in \Obj(\bManBndinf)$ and $D \in \Derivinf(M)$.
\end{ex}

\begin{defn}
	Let $M \in \Obj(\bManBndinf)$. 
	\begin{itemize}
		\item For each $D_1, D_2 \in \Derivinf(M)$, we define $D_1 + D_2 : C^{\infty}(M) \rightarrow C^{\infty}(M)$ by 
		$$[D_1 + D_2] (g) \defeq D_1(g) + D_2(g).$$
		\item For each $D \in \Derivinf(M)$ and $f \in C^{\infty}(M)$, we define $f D : C^{\infty}(M) \rightarrow C^{\infty}(M)$ by 
		$$[f D] (g) \defeq f D(g).$$
	\end{itemize}
\end{defn}
...
\begin{ex} 
	...
	\begin{enumerate}
		...
		\end{enumerate}
		\item $\Derivinf(M) \in \Obj(\bMod_{C^{\infty}(M)})$.
	\end{enumerate}
\end{ex}
...
\begin{defn}
	Let $M \in \Obj(\bManBndinf)$ and $X: M \rightarrow TM$. Suppose that $X$ is a section of $\pi_{TM}$. For each $f \in C^{\infty}(M)$, we define $Xf: M \rightarrow \R$ by 
	$$(Xf)_p \defeq X_p(f).$$
\end{defn}

\begin{ex}
	Let $M \in \Obj(\bManBndinf)$, $X: M \rightarrow TM$ and $(U, \phi) \in \MA_M$. Suppose that $X$ is a section of $\pi_{TM}$. Set $n \defeq \dim M$ and write $\phi = (x^1, \cdots, x^n)$. Then 
...
\begin{ex}
	Let $M \in \Obj(\bManBndinf)$ and $X \in \MFX(M)$. Then for each $f \in C^{\infty}(M)$, $Xf \in C^{\infty}(M)$.
\end{ex}
...
\begin{defn}
	Let $M \in \Obj(\bManBndinf)$ and $X \in \MFX(M)$. We define the \tbf{derivation on $C^{\infty}(M)$ induced by $X$}, denoted $D^X: C^{\infty}(M) \rightarrow C^{\infty}(M)$, by $D^X(f) \defeq Xf$.
\end{defn}

\begin{ex}
	Let $M \in \Obj(\bManBndinf)$ and $X \in \MFX(M)$. Then $D^X \in \Derivinf(M)$.
\end{ex}
...
\begin{defn}
	Let $M \in \Obj(\bManBndinf)$. We define the \tbf{Derivation map}, denoted $\Der:\MFX(M) \rightarrow \Derivinf(M)$, by $\Der(X) \defeq D^X$.
\end{defn}

\begin{ex}
	Let $M \in \Obj(\bManBndinf)$. Then $\Der \in \Hom_{\bMod_{C^{\infty}(M)}}(\MFX(M), \Derivinf(M))$.
\end{ex}
...
\begin{ex}
	Let $M \in \Obj(\bManBndinf)$, $X: M \rightarrow TM$. Suppose that $X$ is a section of $\pi_{TM}$. Then the following are equivalent:
	\begin{enumerate}
		\item $X$ is smooth
...
\begin{defn}
	Let $M \in \Obj(\bManBndinf)$ and $D \in \Derivinf(M)$. For each $p \in M$, we define $X^D_p : C^{\infty}(M) \rightarrow \R$ by $X^D_p(f) \defeq D(f)(p)$.
\end{defn}

\begin{ex}
	Let $M \in \Obj(\bManBndinf)$ and $D \in \Derivinf(M)$. Then or each $p \in M$, $X^D_p \in T_pM$.
\end{ex}
...
\begin{defn}
	Let $M \in \Obj(\bManBndinf)$ and $D \in \Derivinf(M)$. We define $X^D:M \rightarrow TM$ by $X^D(p) \defeq (p, X^D_p)$.
\end{defn}

\begin{ex}
	Let $M \in \Obj(\bManBndinf)$ and $D \in \Derivinf(M)$. Then $X^D \in \MFX(M)$.
\end{ex}
...
\begin{proof}
	...
	\begin{align*}
		(X^D)^j
		& = X^D|_U(x^j) \\
		& = D|_U(x^j) \quad \text{(\tcr{need to define and show something like $D|_U \in \Derivinf(U)$})} \\
		& \in C^{\infty}(U)
	\end{align*}
	...
	\tcr{Then ref prev ex on smoothness of derivations and vector fields}
\end{proof}

\begin{ex}
	Let $M \in \Obj(\bManBndinf)$. Then $\Der \in \Iso_{\bMod_{C^{\infty}(M)}}(\MFX(M), \Derivinf(M))$.
\end{ex}
...
\begin{proof}\
	...
	\end{itemize}
	Thus $\Der \in \Iso_{\bMod_{C^{\infty}(M)}}(\MFX(M), \Derivinf(M))$.
\end{proof}

\begin{note}
	For each $X \in \MFX(M)$ and $D \in \Derivinf(M)$, we identify 
	\begin{itemize}
		\item $X$ with $D^X \in \Derivinf(M)$,
		\item $D$ with $X^D \in \MFX(M)$
	\end{itemize}
	via the isomorphism \tcr{above in prev ex}. 
\end{note}
...
\newpage
\section{The Commutator of Vector Fields}

\begin{defn}
	Let $M \in \Obj(\bManBndinf)$ and $X,Y \in \MFX(M)$. We define $XY: C^{\infty}(M) \rightarrow C^{\infty}(M)$ by $XY(f) \defeq X(Yf)$.
\end{defn}
...
\begin{defn}
	Let $M \in \Obj(\bManBndinf)$ and $X,Y \in \MFX(M)$. We define the \tbf{commutator on $\MFX(M)$}, denoted $[\cdot, \cdot] : C^{\infty}(M) \times C^{\infty}(M) \rightarrow C^{\infty}(M)$, by 
	$$[X,Y] \defeq XY - YX.$$
\end{defn}

\begin{ex}
	Let $M \in \Obj(\bManBndinf)$ and $X,Y \in \MFX(M)$. Then $[X,Y] \in \MFX(M)$.
\end{ex}
...
\begin{proof}
	...
	\begin{itemize}
		...
		\begin{align*}
			...
			& = ([X,Y](f))g + f([X,Y](g)).
		\end{align*}
		Thus $[X,Y]$ is Leibnizian.
	\end{itemize}
	Hence $[X, Y] \in \MFX(M)$.
\end{proof}

\begin{ex} \tbf{Jacobi Identity}: \\
	Let $M \in \Obj(\bManBndinf)$ and $X,Y, Z \in \MFX(M)$. Then 
	$$[X, [Y, Z]] + [Y, [Z, X]] + [Z, [X, Y]] = 0.$$
\end{ex}
...
Given two smooth manifolds $M,N$ and a smooth map $F:M \rightarrow N$, \tcr{a previous section explains} how at a specific point $p \in M$, the derivative $DF(p):T_pM \rightarrow T_{F(p)}N$ allows us to push a vector $v$ tangent to $M$ at $p$ forward to a vector $DF(p)(v)$ tangent to $N$ at $F(p)$. In light of the machinery developed in \tcr{tangent bundle and vector field sections} thus far, it is natural to ask if the tangent bundle map $DF:TM \rightarrow TN$ would us to generalize this idea and push a vector field $X$ on $M$ forward to a vector field $F_*X$ on $N$. In general, this is not the case and the obstruction is captured in the following exercise.

\begin{ex}
	Let $M, N \in \Obj(\bManBndinf)$ and $F \in \Hom_{\bManBndinf}(M, N)$. Then for each $X \in \MFX(M)$, $\pi_{TN} \circ DF \circ X = F$.
\end{ex}

\begin{proof}
	Let $X \in \MFX(M)$. Since  
	\begin{align*}
		X 
		& \in \MFX(M) \\
		& = \Gam(\pi_{TM}),
	\end{align*}
	we have that $\pi_{TM} \circ X = \id_M$. \tcr{A prev ex in tangent bundle intro section implies that} $\pi_{TN} \circ DF = F \circ \pi_{TM}$. Therefore
	\begin{align*}
		\pi_{TN} \circ (DF \circ X) 
		& = (\pi_{TN} \circ DF) \circ X \\
		& = (F \circ \pi_{TM}) \circ X \\
		& = F \circ (\pi_{TM} \circ X) \\
		& = F \circ \id_M \\
		& = F. 
	\end{align*}
...
The previous exercise exposes a sufficient assumption on $F$ that would allow us to push forward vector fields on $M$ to vector fields on $N$, namely that $F$ be a diffeomorphism. 

\begin{ex}
	Let $M, N \in \Obj(\bManBndinf)$ and $F \in \Iso_{\bManBndinf}(M, N)$. Then for each $X \in \MFX(M)$, $DF \circ X \circ F^{-1} \in \MFX(N)$.
\end{ex}

\begin{proof}
	Let $X \in \MFX(M)$. Define $Y: N \rightarrow TN$ by $Y \defeq DF \circ X \circ F^{-1}$. Since $DF$, $F$ and $F^{-1}$ are smooth, we have that $Y$ is smooth. \tcr{The previous exercise} implies that 
	\begin{align*}
		\pi_{TN} \circ Y
		& = \pi_{TN} \circ (DF \circ X \circ F^{-1}) \\
		& = (\pi_{TN} \circ DF \circ X) \circ F^{-1} \\
		& = F \circ F^{-1} \\
		& = \id_N.
	\end{align*}
	...
	\begin{align*}
		...
		& \in \MFX(Y).
	\end{align*}
	Since $X \in \MFX(M)$ is arbitrary, we have that for each $X \in \MFX(M)$, $DF \circ X \circ F^{-1} \in \MFX(N)$. 
\end{proof}

\begin{defn}
	Let $M, N \in \Obj(\bManBndinf)$, $F \in \Iso_{\bManBndinf}(M, N)$, $f \in C^{\infty}(M)$ and $X \in \MFX(M)$. We define the 
	\begin{itemize}
		\item \tbf{pushforward of $f$ by $F$}, denoted $F_*f \in C^{\infty}(N)$, by $F_*f \defeq f \circ F^{-1}$,
		\item \tbf{pushforward of $X$ by $F$}, denoted $F_*X \in \MFX(N)$, by $F_*X \defeq DF \circ X \circ F^{-1}$.
	\end{itemize}
\end{defn}

\begin{ex}
	Let $M, N \in \Obj(\bManBndinf)$ and $F \in \Iso_{\bManBndinf}(M, N)$. Then for each $X, Y \in \MFX(M)$ and $f \in C^{\infty}(M)$, $F_*(X + f Y) = F_*X + (F_*f) F_*Y$.
\end{ex}

\begin{proof}
	Let $X, Y \in \MFX(M)$, $\lam \in \R$ and $q \in N$. Define $p \in M$ by $p \defeq F^{-1}(q)$. Since $DF|_{\{p\} \times T_pM} = \id_{\{p\}} \times DF(p)$, and $DF(p): T_pM \rightarrow T_qN$ is $\R$-linear, we have that $DF|_{\{p\} \times T_pM} $ is $\R$-linear and 
	\begin{align*}
		[F_*(X + f Y)]_q
		& = [DF \circ (X + f Y) \circ F^{-1}](q) \\
		& = [DF \circ (X + f Y)](p) \\
		& = DF([X + f Y]_p) \\
		& = DF(p, v^{[X + f Y]_p}) \\
		& = DF(p)(v^{X_p + f(p) Y_p}) \\
		& = DF(p)(v^{X_p} + f(p) v^{Y_p}) \\
		& = DF(p)(v^{X_p}) + f(p) DF(p)(v^{Y_p}) \\
		& = DF(p, v^{X_p}) + f(p) DF(p, v^{Y_p}) \\
		& = DF(X_p) + f(p) DF (Y_p) \\
		& = [DF \circ X](p) + f(p) [DF \circ Y](p) \\
		& = [DF \circ X \circ F^{-1}](q) + f \circ F^{-1}(q) [DF \circ Y \circ F^{-1}](q) \\
		& = [F_* X +  (F_*f)  F_* Y]_q.
	\end{align*}
	Since $q \in N$ is arbitrary, we have that $F_*(X + f Y) = F_* X + (F_*f) F_* Y$.
\end{proof}

Let $X$ is be a vector field on on $M$ and $Y$ a vector field on $N$. If $F:M \rightarrow N$ is a diffeomorphism, then by the definition of $F_*X$ (\tcr{ref prev def}), asking whether $Y = F_*X$ is equivalent to asking whether $Y \circ F = DF \circ X$. 

If however $F$ is not a diffeomorphism, this second characterization provides a relation between $X$ and $Y$ that still captures the idea of $Y$ being a pushforward of $X$ by $F$ ``in spirit". We formalize this idea in the following definition.

\begin{defn} \tcr{move to vector fields section} \\
	Let $M, N \in \Obj(\bManBndinf)$, $F \in \Hom_{\bManBndinf}(M, N)$, $X \in \MFX(M)$ and $Y \in \MFX(N)$. Then $Y$ is said to be \tbf{$F$-related to $X$} if $Y \circ F = DF \circ X$.
\end{defn}

\begin{ex}
	Let $M, N \in \Obj(\bManBndinf)$ and $F \in \Iso_{\bManBndinf}(M, N)$. Then for each $X \in \MFX(M)$, there exists a unique $Y \in \MFX(N)$ such that $X$ is $F$-related to $Y$. 
\end{ex}

\begin{proof}\
	\begin{itemize}
		\item \tbf{Existence:} \\
		Define $Y \in \MFX(N)$ by $Y \defeq F_*X$. Then 
		\begin{align*}
			Y \circ F 
			& = (DF \circ X \circ F^{-1}) \circ F \\
			& = (DF \circ X) \circ (F^{-1} \circ F) \\
			& = (DF \circ X) \circ \id_M \\
			& = DF \circ X.
		\end{align*}
		Hence $Y$ is $F$-related to $X$. 
		\item \tbf{Uniqueness:} \\
		Let $Y' \in \MFX(N)$. Suppose that $Y'$ is $F$-related to $X$. Then 
		\begin{align*}
			Y' 
			& = Y' \circ \id_N \\
			& = Y' \circ (F \circ F^{-1}) \\
			...
			& = Y.
		\end{align*}
		Since $Y' \in \MFX(N)$ such that $Y$ is $F$-related to $X$ is arbitrary, we have that for each $Y' \in \MFX(N)$, $Y$ is $F$-related to $X$ implies that $Y' = Y$. 
	\end{itemize}
\end{proof}

If $F$ is a diffeomorphism, then it is straightforward to show that for each $f \in C^{\infty}(N)$, $[F_*X]f = X(f \circ F) \circ F^{-1}$. Thus when identifying $\MFX(N)$ with $\Derivinf(N)$, asking whether $Y = F_*X$ is equivalent to asking whether for each $f \in C^{\infty}(N)$, $Yf \circ F = X(f \circ F)$. 

If however $F$ is not a diffeomorphism, this second characterization provides a derivation-centered formulation of $F$-relatedness. We formalize this idea in the following exercise.

\begin{ex}
	Let $M, N \in \Obj(\bManBndinf)$, $F \in \Hom_{\bManBndinf}(M, N)$, $X \in \MFX(M)$ and $Y \in \MFX(N)$. Then $Y$ is $F$-related to $X$ iff for each $f \in C^{\infty}(N)$, $(Yf) \circ F = X(f \circ F)$. 
\end{ex}

\begin{proof}
	We note that for each $f \in C^{\infty}(N)$ and $p \in M$, 
	\begin{align*}
		[(Yf) \circ F] (p) 
		& = (Yf)(F(p)) \\
		& = Y_{F(p)}f \\
	...
	\begin{itemize}
		\item $(\implies)$: \\
		Suppose that $Y$ is $F$-related to $X$. Let $f \in C^{\infty}(N)$. Then for each $p \in M$,
		\begin{align*}
			[(Yf) \circ F] (p) 
			& = [Y \circ F](p) (f) \\
			& = [DF \circ X](p) (f) \\
			& = [X(f \circ F)](p).
		\end{align*}
		Thus $(Yf) \circ F = X(f \circ F)$. Since $f \in C^{\infty}(N)$ is arbitrary, we have that for each $f \in C^{\infty}(N)$, $(Yf) \circ F = X(f \circ F)$. 
		\item $(\impliedby)$: \\
		Suppose that for each $f \in C^{\infty}(N)$, $(Yf) \circ F = X(f \circ F)$. Let $p \in M$. Then for each $f \in C^{\infty}(N)$,
		\begin{align*}
			[Y \circ F](p)(f)
			& = [(Yf) \circ F](p) \\
			& = [X(f \circ F)](p) \\
			& = [DF \circ X](p) (f).
		\end{align*}
		Thus $[Y \circ F](p) = [DF \circ X](p)$. Since $p \in M$ is arbitrary, we have for each $p \in M$, $[Y \circ F](p) = [DF \circ X](p)$. Therefore $Y \circ F = DF \circ X$. Hence $Y$ is $F$-related to $X$. 
	\end{itemize}
\end{proof}

\begin{ex}
	Let $M, N \in \Obj(\bManBndinf)$, $F \in \Hom_{\bManBndinf}(M, N)$, $X_1,X_2 \in \MFX(M)$ and $Y_1, Y_2 \in \MFX(N)$. If $Y_1$ is $F$-related to $X_1$ and $Y_2$ is $F$-related to $X_2$, then $[Y_1, Y_2]$ is $F$-related to $[X_1, X_2]$.  
\end{ex}

\begin{proof}
	Suppose that $Y_1$ is $F$-related to $X_1$ and $Y_2$ is $F$-related to $X_2$. Let $f \in C^{\infty}(N)$. Define $g_1, g_2 \in C^{\infty}(N)$ by $g_1 \defeq Y_1f$ and $g_2 \defeq Y_2f$. Then 
	\begin{align*}
		([Y_1, Y_2] (f)) \circ F 
		& = [(Y_1 Y_2 - Y_2Y_1)(f)] \circ F \\
		& = [Y_1 Y_2(f) -  Y_2Y_1(f)] \circ F \\
		& = [Y_1 Y_2(f)] \circ F - [Y_2Y_1(f)] \circ F \\
		& = (Y_1 g_2) \circ F - (Y_2 g_1) \circ F \\
		& = X_1(g_2 \circ F) - X_2(g_1 \circ F) \\
		& = X_1( [Y_2f] \circ F ) - X_2( [Y_1f] \circ F ) \\
		& = X_1( X_2(f \circ F) ) - X_2(X_1(f \circ F)) \\
		& = X_1X_2(f \circ F) - X_2X_1(f \circ F) \\
		& = (X_1X_2 - X_2X_1)(f \circ F) \\
		& = [X_1, X_2] (f \circ F).
	\end{align*}
	Since $f \in C^{\infty}(N)$ is arbitrary, we have that for each $f \in C^{\infty}(N)$, $([Y_1, Y_2] (f)) \circ F = [X_1, X_2](f \circ F)$. \tcr{The prev ex} implies that $[Y_1, Y_2]$ is $F$-related to $[X_1, X_2]$. 
\end{proof}

\begin{defn} 
	Let $M \in \Obj(\bManBndinf)$, $\Gam \subset \Aut_{\bManBndinf}(M)$ and $X \in \MFX(M)$.
	\begin{enumerate}
		\item Then $X$ is said to be \tbf{$\Gam$-invariant} if for each $\phi \in \Gam$, $X$ is $\phi$-related to $X$. \\
		\item We define $\MFX^{\Gam}(M) \subset \MFX(M)$ by $\MFX^{\Gam}(M) \defeq \{X \in \MFX(M): X \text{ is $\Gam$-invariant}\}$.
	\end{enumerate}
\end{defn}

\begin{note} 
	When $\Gam = \{\phi\}$, we may say ``$\phi$-invariant" instead of $``\{\phi\}$-invariant".
\end{note}

\begin{ex} 
	Let $M \in \Obj(\bManBndinf)$ and $\Gam \subset \Aut_{\bManBndinf}(M)$. Then $\MFX^{\Gam}(M)$ is a vector subspace of $\MFX(M)$
\end{ex}

\begin{proof}\
	Let $X, Y \in \MFX^{\Gam}(M)$, $\lam \in \R$. Then for each $\phi \in \Gam$,
	\begin{align*}
		\phi_*(X + \lam Y)
		& = \phi_*X + \lam \phi_*Y \quad \text{(by \tcr{prev ex})}\\
		& = X + \lam Y \quad \text{(since $X,Y \in \MFX^{\Gam}(M)$)}.
	\end{align*}
	Hence $X + \lam Y \in \MFX^{\Gam}(M)$. Since $X, Y \in \MFX^{\Gam}(M)$ and $\lam \in \R$ are arbitrary, we have that for each $X, Y \in \MFX^{\Gam}(M)$ and $\lam \in \R$,  $X + \lam Y \in \MFX^{\Gam}(M)$. Thus $\MFX^{\Gam}(M)$ is a vector subsapce of $\MFX(M)$. 
\end{proof}
...
\begin{defn} \tcr{change/move this somewhere?, this def works for any set} \\
	Let $G \in \Maninf \cap \Grp$. For each $g \in G$, we define $\iota^l_g:G \rightarrow G \times G$ and $\iota^r_g:G \rightarrow G \times G$ by $\iota^l_g(x) = (g, x)$ and $\iota^r_g(x) = (x, g)$ respectively. 
\end{defn}

\begin{note} 
	If $G \in \Maninf$, \rex{ex:smooth_maps_and_product_manifolds:0008} implies that for each $g \in G$, $\iota^l_g, \iota^r_h \in C^{\infty}(G, G \times G)$. 
\end{note}

\begin{defn}
	Let $(G, \mult) \in \Grp$. We define the \tbf{inversion map}, denoted $\inv: G \rightarrow G$, by $\inv(g) \defeq g^{-1}$.
\end{defn}
...
\begin{defn}
	Let $G \in \bManinf$ and $\mult:G \times G \rightarrow G$. Then $(G, \mult)$ is said to be a \tbf{Lie group} if 
	\begin{enumerate}
		\item $(G, \mult) \in \Grp$,
		\item $\mult \in  C^{\infty}(G \times G, G)$,
		\item $\inv \in  C^{\infty}(G, G)$.
	\end{enumerate}
\end{defn}
...
\begin{ex} 
	...
	\begin{enumerate}
		...
		\item $l_g, r_g \in  C^{\infty}(G, G)$,
		\item $l_g, r_g \in \Aut_{\bManinf}(G)$. 
	\end{enumerate}
\end{ex}
...
\begin{proof} Let $g \in G$.
	\begin{enumerate}
		...
		\item Since $l_g \in  C^{\infty}(G, G)$ and 
		\begin{align*}
			l_g^{-1}
			& = l_{g^{-1}} \\
			& \in  C^{\infty}(G, G),
		\end{align*}
		we have that $l_g \in \Aut_{\bManinf}(G)$. Similarly, $r_g \in \Aut_{\bManinf}(G)$.
	\end{enumerate}
\end{proof}

\begin{ex}
	Let $G \in \Obj(\bManinf)$. Suppose that $G$ is a Lie Group. Then $\p G = \varnothing$. 
\end{ex}
...
\begin{ex}
	Let $G \in \Obj(\bManinf)$. Suppose that $G$ is a group. Define $f:G \times G \rightarrow G$ by $f(g, h) = g h^{-1}$. Then $G$ is a Lie group iff $f \in  C^{\infty}(G \times G, G)$. 
\end{ex}
...
\begin{ex}
	Let $G, H \in \Maninf$ and $\phi: G \rightarrow H$. Suppose that $G, H$ are Lie groups. Then $\phi$ is said to be a \tbf{Lie group homomorphism} if $\phi \in \Hom_{\bManinf}(G, H) \cap \Hom_{\bGrp}(G, H)$.
\end{ex}

\begin{defn}
	We define the category of Lie groups, denoted $\bLieGrp$, by 
	\begin{itemize}
		\item $\Obj(\bLieGrp) = \{G: G \text{ is a Lie group} \}$ 
		\item For $G_1, G_2 \in \Obj(\bLieGrp)$, 
		\begin{align*}
			& \Hom_{\bLieGrp}(G_1, G_2) = \Hom_{\bManinf}(G, H) \cap \Hom_{\bGrp}(G, H)
		\end{align*}
		\item For 
		\begin{itemize}
			\item $G_1, G_2, G_3 \in \Obj(\bLieGrp)$ 
			\item $\phi_{12} \in \Hom_{\bLieGrp}(G_1, G_2)$
			\item $\phi_{23} \in \Hom_{\bLieGrp}(G_2, G_3)$
		\end{itemize}
		we define $ \phi_{23} \circ_{\bLieGrp} \phi_{12} \in \Hom_{\bLieGrp}(G_1, G_3)$ by 
		$$ \phi_{23} \circ_{\bLieGrp} \phi_{12} = \phi_{23} \circ_{\bSet} \phi_{12}$$
	\end{itemize}
\end{defn}

\begin{ex}
	We have that $\bLieGrp$ is a subcategory of $\bGrp$ and $\bManinf$.
\end{ex}
...
\begin{ex}
	Let $G, H \in \Obj(\bLieGrp)$ and $\phi \in \Hom_{\bLieGrp}(G, H)$. Then $\phi$ has constant rank. 
\end{ex}
...
\begin{proof}
	...
	\end{align*}
	Since $l_g \in \Aut_{\bManinf}(G), l_{\phi(g)} \in \Aut_{\bManinf}(H)$, we have that $D l_g (x) \in \Iso_{\bVectR}(T_x G, T_{g x} G)$ and $D l_{\phi(g)}(\phi(x)) \in \Iso_{\bVectR}(T_{\phi(x)}H, T_{\phi(g) \phi(x)}H)$. Hence 
	\begin{align*}
		\rnk D\phi(gx)
...
\begin{ex}
	Let $G, H \in \Obj(\bLieGrp)$ and $\phi \in \Hom_{\bLieGrp}(G, H)$. Then $\phi \in \Iso_{\bLieGrp}(G, H)$ iff $\phi$ is a bijection.
\end{ex}
...
\begin{defn}
	Let $G, H \in \Obj(\bLieGrp)$ and $\phi \in \Hom_{\bLieGrp}(G, H)$. Then $\phi$ is said to be a 
	\begin{itemize}
		\item \tbf{$\bLieGrp$-immersion} if $\phi$ is a $\bManinf$-immersion  
		\item \tbf{$\bLieGrp$-embedding} if $\phi$ is a $\bManinf$-embedding  
	\end{itemize}
\end{defn}

\begin{ex}
	Let $G, H \in \Obj(\bLieGrp)$ and $\phi \in \Hom_{\bLieGrp}(G, H)$. Suppose that $\phi$ is a $\bLieGrp$-immersion. If $G$ is compact, then $\phi$ is a $\bLieGrp$-embedding.
\end{ex}
...
\begin{defn}
	Let $G, H \in \Obj(\bLieGrp)$. Suppose that $G \leqslant H$. Then $H$ is said to be an
	\begin{itemize}
...
\begin{ex}
	Let $G, H \in \Obj(\bLieGrp)$. Suppose that $G \leqslant H$. 
\end{ex}
...
\begin{defn}
	Let $G, H \in \Obj(\bLieGrp)$. Suppose that $G \subset H$. Then $G$ is said to be a \tbf{Lie subgroup} of $H$ if 
	\begin{enumerate}
		\item $G \leqslant H$ 
...
\begin{defn}
	Let $V \in \Obj(\bVectK)$ and $[\cdot, \cdot]: V \times V \rightarrow V$. Then 
	\begin{itemize}
		\item $[\cdot, \cdot]$ is said to be a \tbf{$\K$-Lie bracket on $V$} if 
		\begin{enumerate}
			\item \tbf{(bilinearity):} for each $x, y, z \in V$ and $\lam \in \K$, $[x + \lam y, z] = [x, z] + \lam [y, z]$ 
			\item \tbf{(antisymmetry):} for each $x, y \in V$, $[x, y] = -[y,x]$
			\item \tbf{(Jacobi identity):} for each $x,y, z \in V$, $[x, [y,z]] + [y, [z,x]] + [z, [x,y]] = 0$
		\end{enumerate}
		\item $(V, [\cdot, \cdot])$ is said to be a \tbf{$\K$-Lie Algebra} if $[\cdot, \cdot]$ is a $\K$-Lie bracket on $V$.
	\end{itemize}
\end{defn}

\begin{note}
	When the context is clear, we may say 
	\begin{itemize}
		\item ``Lie bracket" instead of ``$\K$-Lie bracket",
		\item ``Lie algebra" instead of ``$\K$-Lie algebra".
	\end{itemize}
\end{note}

\begin{ex}
	Let $M \in \Obj(\bManinf)$. Then $(\MFX(M), [\cdot, \cdot])$ is an $\R$-Lie Algebra.
\end{ex}
...
\section{Lie Subalgebras}

\begin{defn} \ld{ex:lie_alg_subalg:0001}
	Let $(V, [\cdot, \cdot]_V), (W, [\cdot, \cdot]_W)$ be $\K$-Lie algebras. Then $(W, [\cdot, \cdot]_W)$ is said to be a \tbf{Lie subalgebra of $(V, [\cdot, \cdot])$} if 
	\begin{enumerate}
		\item $W \subset V$
		\item $[\cdot, \cdot]_W = [\cdot, \cdot]_V|_{W \times W}^W$. 
	\end{enumerate}
\end{defn}

\begin{ex} \lex{ex:lie_alg_subalg:0002}
	Let $(V, [\cdot, \cdot])$ be a Lie algebra and $W \subset V$. If 
	\begin{enumerate}[label=(\alph*)]
		\item $W$ is a vector subsapce of $V$, \label{ex:lie_alg_subalg:0002_alph1}
		\item for each $x, y \in W$, $[x, y] \in W$, \label{ex:lie_alg_subalg:0002_alph2}
	\end{enumerate} 
	then $(W, [\cdot, \cdot]|^W_{W \times W})$ is a Lie subalgebra of $(V, [\cdot, \cdot])$.
\end{ex}

\begin{proof}
	Suppose that $W$ is a vector subsapce of $V$ and for each $x, y \in W$, $[x, y] \in W$. Define $[\cdot, \cdot]_W: W \times W \rightarrow W$ by $[\cdot, \cdot]_W \defeq [\cdot, \cdot]|_{W \times W}^W$. 
	\begin{itemize}
		\item 
		\begin{enumerate}
			\item \tbf{(bilinearity):} Let $x,y,z \in W$ and $\lam \in \K$. \ref{ex:lie_alg_subalg:0002_alph1} implies that $\lam y \in W$. Therefore
			\begin{align*}
				[x+ \lam y, z]_W 
				& = [x+ \lam y, z] \\
				& = [x, z] + \lam [y, z] \quad \text{(since $[\cdot, \cdot]$ is a Lie bracket on $V$)} \\
				& = [x, z]_W + \lam [y, z]_W \quad \text{(by definition}. 
			\end{align*}
			Since $x,y,z \in W$ and $\lam \in \K$ are arbitrary, we have that for each $x,y,z \in W$ and $\lam \in \K$, $[x+ \lam y, z]_W = [x, z]_W + \lam [y, z]_W$. 
			\item \tbf{(antisymmetry):} For each $x,y \in W$,
			\begin{align*}
				[x,y]_W 
				& = [x,y] \\
				& = -[y,x] \quad \text{(since $[\cdot, \cdot]$ is a Lie bracket on $V$)} \\
				& = -[y,x]_W 
			\end{align*}
			\item \tbf{(Jacobi identity):} For each $x,y, z \in W$,
			\begin{align*}
				[x, [y,z]_W]_W + [y, [z,x]_W]_W + [z, [x,y]_W]_W \\
				& = [x, [y,z]] + [y, [z,x]] + [z, [x,y]] \\
				& = 0 \quad \text{(since $[\cdot, \cdot]$ is a Lie bracket on $V$)}.
			\end{align*}
		\end{enumerate}
		Thus $[\cdot, \cdot]_W$ is a Lie bracket on $W$.
		\item Since $W$ is a vector space and $[\cdot, \cdot]_W$ is a Lie bracket on $W$, $(W, [\cdot, \cdot]_W)$ is a Lie algebra. 
	\end{itemize}
	\begin{enumerate}
		\item By assumption, $W \subset V$. 
		\item By definition, $[\cdot, \cdot]_W = [\cdot, \cdot]|_{W \times W}^W$.
	\end{enumerate}
	Thus $(W, [\cdot, \cdot]_W)$ is a Lie subalgebra of $(V, [\cdot, \cdot])$.
\end{proof}

\begin{ex} \lex{ex:lie_alg_subalg:0003}
...
\begin{ex} \lex{ex:lie_alg_subalg:0004}
	Let $M \in \Maninf$ and $\Gam \subset \Aut_{\bManinf}(M)$. Then $\MFX^{\Gam}(M)$ is a Lie subalgebra of $\MFX(M)$.
\end{ex}

\begin{proof}\
	\begin{enumerate}
		\item \tcr{A prev ex implies that} $\MFX^{\Gam}(M)$ is a vector subsapce of $\MFX(M)$.
		\item Let $X, Y \in \MFX^{\Gam}(M)$. Then 
		\begin{align*}
			\phi_*[X, Y]
			& = [\phi_*X, \phi_*Y] \quad \text{(by \tcr{ex in section on vectorvields and smooth maps})} \\ 
			& = [X,Y] \quad \text{(since $X,Y \in \MFX^{\Gam}(M)$)}.
		\end{align*}
		Hence $[X, Y] \in \MFX^{\Gam}(M)$. Since $X, Y \in \MFX^{\Gam}(M)$ are arbitrary, we have that for each $X, Y \in \MFX^{\Gam}(M)$, $[X, Y] \in \MFX^{\Gam}(M)$. Thus $\MFX^{\Gam}(M)$ is a Lie subalgebra of $\MFX(M)$.
	\end{enumerate}
\end{proof}
...
\begin{defn} \ld{def:lie_alg_lie_group:0001}
	Let $G$ be a Lie group. We define the \tbf{Lie algebra of $G$}, denoted $\Lie(G) \subset \MFX(G)$, by $\Lie(G) \defeq \MFX^{(L_g)_{g \in G}}(G)$. 
\end{defn}

\begin{ex}
	Let $G$ be a Lie group. Then $\eval_e \in \Iso_{\bVectR}(\Lie(G), T_eG)$.
\end{ex}

\begin{proof}
	\begin{itemize}
		\item \tbf{injectivity:} \\
		Let $X \in \ker \eval_e$. Then 
		\begin{align*}
			X_e
			& = \eval_e(X) \\
			& = 0.
		\end{align*}
		Since $X \in \Lie(G)$, we have that for each $g \in G$, 
		\begin{align*}
			X_g
			& = X_{L_g(e)} \\
			& = [X \circ L_g](e)
			& = [DL_g \circ X](e) \quad \text{(since $(L_g)_*X = X$)} \\
			& = DL_g(X_e) \\
			& = D(L_g)(e)(0) \\
			& = 0. 
		\end{align*}
		Hence $X = 0$. Since $X \in \ker \eval_e$ is arbitrary, we have that for each $X \in \ker \eval_e$, $X = 0$. Thus $\ker \eval_e = \{0\}$ and $\eval_e$ is injective.  
...
\end{proof}
	...
	\begin{defn} \ld{def:fiber_bundles:0000.01}
		Let $E, M \in \Obj(\bSet)$, $\pi \in \Epi_{\bSet}(E, M)$ and $A \subset M$. We define the \tbf{fibers of $\pi$ over $A$}, denoted $E_A^{\pi} \subset E$ by $E_A \defeq \pi^{-1}(A)$. 
	\end{defn}
	
	\begin{note}
		When the context is clear, we may write $E_A$ in place of $E_A^{\pi}$.
	\end{note}

	\begin{defn} \ld{def:fiber_bundles:0001}
		Let $E, M, F \in \Obj(\bSet)$, $\pi \in \Hom_{\bSet}(E, M)$ a surjection, $U \subset M$ and $\Phi: E_U \rightarrow U \times F$. 
		\begin{itemize}
			\item Then $(U, \Phi)$ is said to be a \tbf{local trivialization of $\pi$ over $U$ with fiber $F$}  if 
			\begin{enumerate}
				\item $\Phi$ is a bijection
				\item $\prj_1 \circ \Phi = \pi|_{E_U}$, i.e. the following diagram commutes:
				\[ 
				\begin{tikzcd}
					E_U \arrow[r, "\Phi"] \arrow[dr, "\pi"'] & U \times F \arrow[d, "\prj_1"]  \\
					& U
				\end{tikzcd}
				\]
			\end{enumerate}
			\item We define $\LocTriv(\pi, F) \defeq \{(U, \Phi): \text{$U \subset M$, $\Phi \in (U \times F)^{E_U}$ and $(U, \Phi)$ is a local trivialization of $\pi$ over $U$ with fiber $F$}\}$.
		\end{itemize}
	\end{defn}
	
	\begin{ex} \lex{ex:fiber_bundles:0002}
		Let $E, M, F \in \Obj(\bSet)$ and $\pi \in \Hom_{\bSet}(E, M)$ a surjection, $U \subset M$ and $\Phi: \pi^{-1}(U) \rightarrow U \times F$ a local trivialization with respect to $\pi$ of $E$ over $U$ with fiber $F$. Then for each $A \subset U$, 
		$$\Phi(E_A) = A \times F.$$
	...
	\subsection{$\bManz$ Fiber Bundles}
	
	\begin{defn} \lex{ex:fiber_bundles:0003}
		Let $E, M, F \in \Obj(\bManz)$ and $\pi \in \Epi_{\bManz}(E, M)$, $U \subset M$ and $\Phi: E_U \rightarrow U \times F$. 
		\begin{itemize}
			\item Then $(U, \Phi)$ is said to be a \tbf{$\bManz$-local trivialization of $\pi$ over $U$ with fiber $F$}  if 
			\begin{enumerate}
				\item $U \in \MT_M$,
				\item $(U, \Phi) \in \LocTriv(\pi, F)$,
				\item $\Phi \in \Iso_{\bTop}(E_U, U \times F)$ is a homeomorphism
			\end{enumerate}
			\item We define $\LocTriv^0(\pi, F) \defeq \{(U, \Phi) \in \LocTriv(\pi, F): \text{$(U, \Phi)$ is a $\bManz$-local trivialization of $\pi$ over $U$ with fiber $F$}\}$.
		\end{itemize}
	\end{defn}
 
	\begin{defn} \ld{def:fiber_bundles:0004}
		Let $E,M, F \in \Obj(\bManz)$ and $\pi \in \Epi_{\bManz}(E, M)$. Then $(E, M, \pi, F)$ is said to be a \tbf{$\bManz$ fiber bundle with total space $E$, base space $M$, fiber $F$ and projection $\pi$} if for each $p \in M$, there exist $(U, \Phi) \in \LocTriv^0(\pi, F)$ such that $U \in \MT_M$ and $\Phi: \pi^{-1}(U) \rightarrow U \times F$ such that $p \in U$.
	\end{defn}
	
	\begin{defn}
		\tcr{Define category $\bBunz$} \tcr{FINISH!!!}
	\end{defn}

	\begin{ex} \lex{ex:fiber_bundles:0005} \tbf{$\bManz$ Fiber Bundle Chart Lemma:} \\
		Let $E \in \Obj(\bSet)$, $M, F \in \Obj(\bManz)$, $\pi \in \Epi_{\bSet}(E, M)$, $\Gam$ an index set and for each $\al \in \Gam$, $(U_{\al} , \Phi_{\al}) \in \LocTriv(\pi, F)$. Set $n \defeq \dim M$ and $k \defeq \dim F$. Suppose that 
		\begin{itemize}
		...
		\end{itemize}
		Then there exist a unique $\MT_E \in \Top(E)$ such that
		\begin{enumerate}
			\item $(E, \MT_E) \in \Obj(\bManz)$ and $\dim E = n + k$,
			\item $\pi \in \Epi(E, M)$ and 
			\item for each $\al \in \Gam$, $(U_{\al}, \Phi_{\al}) \in \LocTrivz(\pi, F)$
			\item $(E, M, \pi, F) \in \Obj(\bBunz)$.
		\end{enumerate}
	\end{ex}
	...
	\begin{proof}\
		\begin{enumerate}
			...
			\begin{itemize}
				...
				Let $a \in E$. Set $p = \pi(a)$. Choose $(\al, \mu) \in S^M$ such that $p \in V_{\al, \mu}^M$. Since $V^M_{\al, \mu} \subset U_{\al}$, $a \in E_{U_{\al}}$ which implies that
				\begin{align*}
					p
					& = \pi(a) \\
			...
			\end{itemize}
			The topological manifold chart lemma (\rex{ex:topological_manifolds_introduction:0023}) implies that there exists a unique topology $\MT_E$ on $E$ such that $(E, \MT_E) \in \Obj(\bManz)$, $\dim E = n+k$ and $(V^E_{\al, \mu, \nu}, \psi^E_{\al, \mu, \nu})_{(\al, \mu, \nu) \in \Pi^E} \subset X^{n+k}(E, \MT_E)$.
			...
			\end{align*}
			Hence $E_{U_{\al}} \in \MT_E$, $(V^E_{\al, \mu, \nu})_{(\mu, \nu) \in \Pi^M_{\al} \times \Pi^F}$ is an open cover of $E_{U_{\al}}$ and $\Phi_{\al}$ is a local homeomorphism. Since $\Phi_{\al}$ is a bijection, \tcr{ref ex in topology notes} implies that $\Phi_{\al} \in \Iso_{\bTop}(E_{U_{\al}}, U_{\al} \times F)$. Since $\al \in \Gam$ is arbitrary, we have that for each $\al \in \Gam$, $\Phi_{\al}) \in \Iso_{\Top}(E_{U_{\al}}, U_{\al} \times F)$. Let $(\al, \mu, \nu) \in \Pi^E$. Since 
			\begin{itemize}
				\item $V^E_{\al, \mu, \nu} \subset E_{U_{\al}}$
				\item $\prj_1: M \times F \rightarrow M$ is continuous
				\item $\Phi_{\al}: E_{U_{\al}} \rightarrow U_{\al} \times F$ is continuous
				\item $\pi|_{V^E_{\al, \mu, \nu}} = \prj_1 \circ \Phi|_{V^E_{\al, \mu, \nu}}$
			\end{itemize}
			we have that $\pi|_{V^E_{\al, \mu, \nu}}: V^E_{\al, \mu, \nu} \rightarrow V^M_{\al, \mu}$ is continuous. Since $(\al, \mu, \nu) \in \Pi^E$ is arbitrary and $(V^E_{\al, \mu, \nu})_{(\al, \mu, \nu) \in \Pi^E }$ is an open cover of $E$, \tcr{ref ex in topology notes} implies that $\pi \in \Hom_{\bManz}(E, M)$. Since $\pi$ is surjective, $\pi \in \Epi_{\bManz}(E, M)$.
			\item Let $\al \in \Gam$. Since $E,M,F \in \Obj(\bManz)$, $\pi \in \Epi_{\bManz}(E,M)$ and 
			\begin{itemize}
				\item $U_{\al} \in \MT_M$,
				\item $(U_{\al}, \Phi_{\al}) \in \LocTriv(\pi, F)$,
				\item $\Phi_{\al} \in \Iso_{\Top}(E_{U_{\al}}, U_{\al} \times F)$,
			\end{itemize}
			we have that $(U_{\al}, \Phi_{\al}) \in \LocTrivz(\pi, F)$. Since $\al \in \Gam$ is arbitrary, we have that for each $\al \in \Gam$, $(U_{\al}, \Phi_{\al}) \in \LocTrivz(\pi, F)$.
			\item Let $p \in M$. By assumption, there exists $\al \in \Gam$ such that $p \in U_{\al}$. The previous part implies that $(U_{\al}, \Phi_{\al}) \in \LocTrivz(\pi, F)$. Since $p \in M$ is arbitrary, we have that for each $p \in M$, there exists $\al \in \Gam$ such that $p \in U_{\al}$ and $(U_{\al}, \Phi_{\al}) \in \LocTrivz(\pi, F)$. Hence $(E, M, \pi, F) \in \Obj(\bBunz)$.
		\end{enumerate}
	\end{proof}
	...
	\subsection{$\bManinf$ Fiber Bundles}
	
	\begin{defn} \ld{def:fiber_bundles:0006}
		Let $E, M, F \in \Obj(\bManinf)$ and $\pi \in \Hom_{\bManinf}(E, M)$ a surjection, $U \subset M$ and $\Phi: \pi^{-1}(U) \rightarrow U \times F$. Then $(U, \Phi)$ is said to be a \tbf{smooth fiber bundle local trivialization with respect to $\pi$ of $E$ over $U$ with fiber $F$}  if 
		\begin{enumerate}
			\item $U$ is open in $M$
	...
	\begin{defn} \ld{def:fiber_bundles:0007}
		Let $E,M, F \in \Obj(\bManinf)$ and $\pi \in \Hom_{\bManinf}(E, M)$ a surjection. Then $(E, M, \pi, F)$ is said to be a \tbf{$\bManinf$ fiber bundle with total space $E$, base space $M$, fiber $F$ and projection $\pi$} if for each $p \in M$, there exist $U \in \MT_M$ and $\Phi: \pi^{-1}(U) \rightarrow U \times F$ such that $p \in U$ and $(U, \Phi)$ is a smooth local trivialization of $E$ over $U$ with fiber $F$. For $p \in M$, we define the \tbf{fiber over $p$}, denoted $E_p$, by $E_p = \pi^{-1}(\{p\})$.
	\end{defn}

	\begin{ex} \lex{ex:fiber_bundles:0008} \tbf{$\bManinf$ Fiber Bundle Chart Lemma:} \\
		Let $E \in \Obj(\bSet)$, $M, F \in \Obj(\bManinf)$, $\pi \in \Epi_{\bSet}(E, M)$, $\Gam$ an index set and for each $\al \in \Gam$, $(U_{\al}, \Phi_{\al}) \in \LocTriv(\pi, F)$. Set $n \defeq \dim M$ and $k \defeq \dim F$. Suppose that 
		\begin{itemize}
			\item for each $\al \in \Gam$, $U_{\al} \in \MT_M$
			\item $M \subset \bigcup\limits_{\al \in \Gam} U_{\al}$ 
		...
		\begin{enumerate}
			...
			\item $(E, M, \pi, F)$ is a $\bManinf$ fiber bundle
		\end{enumerate}
	\end{ex}
	...
	\begin{proof}
		...
		\begin{itemize}
			...
			\item $(E, M, \pi, F)$ is an $\bManz$ fiber bundle
		\end{itemize}
		\begin{enumerate}
			\item Define $(V^E_{\al, \mu, \nu}, \psi^E_{\al, \mu, \nu})_{(\al, \mu, \nu) \in \Pi^E} \subset X^{n+k}(E, \MT_E)$ as in the proof of the $\bManz$ fiber bundle chart lemma. Let $(\al_1, \mu_1, \nu_1), (\al_2, \mu_2, \nu_2) \in \Pi^E$. For notational convenience, set $\psi^E_1 = \psi^E_{\al_1, \mu_1, \nu_1}$, $\psi^E_2 = \psi_{\al_2, \mu_2, \nu_2}$, $V^E = V^E_{\al_1, \mu_1, \nu_1} \cap V^E_{\al_2, \mu_2, \nu_2}$, $V^M = V_{\al_1, \mu_1}^M \cap V^M_{\al_2, \mu_2}$ and $V^F = V^F_{\nu_1} \cap V^F_{\nu_2}$. Then $\psi_2|_{V^E} \circ (\psi_1|_{V^E})^{-1}: \psi_1(V^E) \rightarrow \psi_2(V^E)$ is given by
			\begin{align*}
				\psi^E_2|_{V^E} \circ (\psi^E_1|_{V^E})^{-1}
				& = [(\psi_{\al_2, \mu_2}^M|_{V^M} \times \psi_{\nu_2}^F|_{V^F}) \circ \Phi_{\al_2}|_{V^E}] \circ [(\psi_{\al_1, \mu_1}^M|_{V^M} \times \psi_{\nu_1}^F|_{V^F}) \circ \Phi_{\al_1}|_{V^E}]^{-1} \\
				& = [(\psi_{\al_2, \mu_2}^M|_{V^M} \times \psi_{\nu_2}^F|_{V^F}) \circ \Phi_{\al_2}|_{V^E}] \circ [(\Phi_{\al_1}|_{V^E})^{-1} \circ (\psi_{\al_1, \mu_1}^M|_{V^M} \times \psi_{\nu_1}^F|_{V^F})^{-1}] \\
			...
			\item Let $p \in M$. By assumption, there exists $\al \in \Gam$ such that $p \in U_{\al}$, $U_{\al} \in \MT_M$. Since $E,M,F \in \Obj(\bManinf)$, $\pi \in \Hom_{\bManinf}(E,M)$ is a surjection, and 
			\begin{itemize}
				\item $U_{\al}$ is open
			...
			\end{itemize}
			we have that $(U_{\al}, \Phi_{\al})$ is a smooth local trivialization with respect to $\pi$ of $E$ over $U_{\al}$ with fiber $F$. Since $p \in M$ is arbitrary, $(E, M, \pi, F)$ is a $\bManinf$ fiber bundle.  
		\end{enumerate}
	\end{proof}

	\begin{defn} \ld{def:fiber_bundles:0009}
		Let $(E_1, M_1, \pi_1, F_1)$ and $(E_2, M_2, \pi_2, F_2)$ be $\bManinf$ fiber bundles, $\Phi \in \Hom_{\bManinf}(E_1, E_2)$ and $\phi \in \Hom_{\bManinf}(M_1, M_2)$. Then $(\Phi, \phi)$ is said to be a \tbf{smooth bundle morphism} from $(E_1, M_1, \pi_1, F_1)$ to $(E_2, M_2, \pi_2, F_2)$ if 
	...
	\begin{ex}
		Let $(E_1, M_1, \pi_1, F_1)$ and $(E_2, M_2, \pi_2, F_2)$ be $\bManinf$ fiber bundles, $\Phi \in \Hom_{\bManinf}(E_1, E_2)$ and $\phi \in \Hom_{\bManinf}(M_1, M_2)$. If $(\Phi, \phi)$ is a smooth bundle morphism from $(E_1, M_1, \pi_1, F_1)$ to $(E_2, M_2, \pi_2, F_2)$, then for each $p \in M_1$, $\Phi({(E_1)_p}) \subset (E_2)_{\phi(p)}$.
	\end{ex}
	...
	\begin{defn} \ld{def:fiber_bundles:0010}
		We define the category of $\bManinf$ fiber bundles, denoted $\bBuninf$, by 
		\begin{itemize}
			\item $\Obj(\bBuninf) \defeq \{(E, M, \pi, F): (E, M, \pi, F) \text{ is a $\bManinf$ fiber bundle}\}$ 
			\item For $(E_1, M_1, \pi_1, F_1), (E_2, M_2, \pi_2, F_2) \in \Obj(\bBuninf)$, 
			\begin{align*}
				& \Hom_{\bBuninf}((E_1, M_1, \pi_1, F_1), (E_2, M_2, \pi_2, F_2)) \defeq \\
				& \{(\Phi, \phi): (\Phi, \phi) \text{ is a smooth bundle morphism from $(E_1, M_1, \pi_1, F_1)$ to $(E_2, M_2, \pi_2, F_2)$} \}
			\end{align*}
			\item For 
			\begin{itemize}
				\item $(E_1, M_1, \pi_1, F_1), (E_2, M_2, \pi_2, F_2), (E_3, M_3, \pi_3) \in \Obj(\bBuninf)$ 
				\item $(\Phi_{12}, \phi_{12}) \in \Hom_{\bBuninf}((E_1, M_1, \pi_1, F_1), (E_2, M_2, \pi_2, F_2))$
				\item $(\Phi_{23}, \phi_{23}) \in \Hom_{\bBuninf}((E_2, M_2, \pi_2, F_2), (E_3, M_3, \pi_3))$ 
			\end{itemize}
			we define $(\Phi_{23}, \phi_{23}) \circ (\Phi_{12}, \phi_{12}) \in \Hom_{\bBuninf}((E_1, M_1, \pi_1, F_1), (E_3, M_3, \pi_3))$ by 
			$$(\Phi_{23}, \phi_{23}) \circ (\Phi_{12}, \phi_{12}) \defeq (\Phi_{23} \circ \Phi_{12}, \phi_{23} \circ \phi_{12})$$
		\end{itemize}
	\end{defn}

	\begin{ex} \lex{ex:fiber_bundles:0011}
		We have that $\bBuninf$ is a full subcategory of $(\id_{\bManinf} \downarrow \id_{\bManinf})$.
	\end{ex}
	
	\begin{proof} Set $\MC = (\id_{\bManinf} \downarrow \id_{\bManinf})$. We note that 
		\begin{itemize}
			\item $\Obj(\bBuninf) \subset \Obj(\MC)$
			\item for each $(E_1, M_1, \pi_1, F_1), (E_2, M_2, \pi_2, F_2) \in \Obj(\bBuninf)$, 
			$$\Hom_{\bBuninf}((E_1, M_1, \pi_1, F_1), (E_2, M_2, \pi_2, F_2)) = \Hom_{\MC}((E_1, M_1, \pi_1, F_1), (E_2, M_2, \pi_2, F_2))$$
		\end{itemize}
		So $\bBuninf$ is a full subcategory of $\MC$.
	\end{proof}

	\begin{ex}
		Let $(E, M, \pi, F) \in \bBuninf$. Then $\pi$ is a submersion.
	\end{ex}

	\begin{proof}
		Let $a \in E$. Set $p \defeq \pi(a)$. Since $(E, M, \pi, F) \in \bBuninf$, there exists $U \in \MT_M$ and $\Phi \in \Hom_{\bManinf}(\pi^{-1}(U), U \times F)$ such that $p \in U$ and $(U, \Phi)$ is a smooth fiber bundle local trivialization of $E$ over $U$ with fiber $F$ with respect to $\pi$. Then $\Phi$ is a diffeomorphism and $\prj_1 \circ \Phi = \pi|_{\pi^{-1}(U)}$. \rex{ex:submersions:0001.1} implies that $\prj_1: U \times F \rightarrow U$ is a submersion. Since $\Phi$ is a diffeomorphism, $\Phi$ is a submersion. \rex{ex:submersions:0001.2} then implies that $\pi|_{\pi^{-1}(U)}$ is a submersion. Since $a \in E$ is arbitrary, we have that for each $a \in E$, there exists $V \in \MT_E$ such that $a \in V$ and $\pi|_V$ is a submersion. \tcr{ (cite exercise)} \rex{arg1} implies that $\pi$ is a submersion.
	\end{proof}

	\begin{ex} 
		Let $(E, M, \pi, F) \in \bBuninf$ and $(U, \Phi)$ a local trivialization of $E$ over $U$. For each $p \in M$,
		\begin{enumerate}
	...
	\begin{ex} \lex{ex:fiber_bundles:0012}
		Let $(E, M, \pi, F) \in \bBuninf$, $(U, \Phi)$ a local trivialization of $E$ over $U$ and $(V, \Psi)$ a local trivialization of $E$ over $V$. Then 
		\begin{enumerate}
			\item $\prj_1 \circ \Psi|_{\pi^{-1}(U \cap V)} \circ \Phi|^{-1}_{\pi^{-1}(U \cap V)} = \prj_1$
			\item there exists $\sig \in \Hom_{\bManinf}(( U \cap V) \times F,  F)$ such that $\Psi|_{\pi^{-1}(U \cap V)} \circ \Phi|^{-1}_{\pi^{-1}(U \cap V)} = (\prj_1, \sig)$ and for each $p \in U \cap V$, $\sig(p, \cdot) \in \Aut_{\bManinf}(F)$.  
		\end{enumerate}
	\end{ex}
	...
	\begin{proof}\
		\begin{enumerate}
			...
			\item Define $\sig, \tau \in \Hom_{\bManinf}(( U \cap V) \times F,  F)$ by $\sig \defeq \prj_2 \circ \Psi|_{\pi^{-1}(U \cap V)} \circ (\Phi|_{\pi^{-1}(U \cap V)})^{-1}$ and $\tau \defeq \prj_2 \circ \Phi|_{\pi^{-1}(U \cap V)} \circ (\Psi|_{\pi^{-1}(U \cap V)})^{-1}$. Part $(1)$ implies that for each $(p, x) \in (U \cap V) \times F$, 
			\begin{align*}
	...
	\begin{defn} \ld{def:fiber_bundles:0013}
		Let $(E, M, \pi, F) \in \bBuninf$, $A$ an index set and for each $\al \in A$, $(U_{\al}, \Phi_{\al})$ a smooth local trivializations of $E$. Then $(U_{\al}, \Phi_{\al})_{\al \in A}$ is said to be a \tbf{smooth fiber bundle atlas on $(E, M, \pi, F)$} if for each $p \in M$, there exists $\al \in A$ such that $p \in U_{\al}$.
	\end{defn}
	
	
	\begin{defn}
		Let $(E, M, \pi, F) \in \Obj(\bBuninf)$, $A$ an index set and $(U_{\al}, \Phi_{\al})_{\al \in A}$ a smooth fiber bundle atlas on $(E, M, \pi, F)$. For each $\al,\be \in A$, we define $U_{\al,\be} \subset M$ and $\Phi_{\al,\be}: U_{\al,\be} \times F \rightarrow U_{\al,\be} \times F$ by 
		\begin{itemize}
	...
	\begin{ex}
		Let $(E, M, \pi, F) \in \Obj(\bBuninf)$ , $A$ an index set and $(U_{\al}, \Phi_{\al})_{\al \in A}$ a smooth fiber bundle atlas on $(E, M, \pi, F)$. Then for each $\al, \be \in A$ and $p \in U_{\al, \be}$, $\Phi_{\al,\be}(p, \cdot) \in \Aut_{\bManinf}(F)$.
	\end{ex}
	...
	\begin{defn}
		Let $(E, M, \pi_E, F), (E', M, \pi', F') \in \Obj(\bBuninf)$. Then $(E', M, \pi', F')$ is said to be a \tbf{subbundle of $(E, M, \pi_E, F)$} if 
		\begin{enumerate}
			\item $E' \subset E$
			\item $\pi' = \pi|_{E'}$
			\item for each $(U, \Phi) \in \LocTriv(E, \pi, F, U)$
		\end{enumerate}
	\end{defn}
	...
	\begin{defn}
		Let $(E, M, \pi) \in \Obj(\bBuninf)$. We define the \tbf{vertical bundle associated to $(E, M, \pi)$ }, denoted $(VE, M, \pi_{V}) \in \bBuninf$, by 
	...
	\subsection{$\bManinf$ Vector Bundles}
	...
	\begin{defn} \ld{def:vector_bundles:0000.1}
		Let $E, M \in \Obj(\bManinf)$ and $\pi \in \Hom_{\bManinf}(E, M)$ a surjection, $U \subset M$ and $\Phi: \pi^{-1}(U) \rightarrow U \times \R^k$. Then $(U, \Phi)$ is said to be a \tbf{smooth vector bundle local trivialization of $E$ over $U$}  if 
		\begin{enumerate}
			\item $U$ is open in $M$
			\item $(U, \Phi)$ is a smooth local trivialization of $E$ over $U$ with fiber $\R^k$ (\rd{def:fiber_bundles:0006})
			\item for each $q \in U$, $\Phi|_{E_q} \in \Iso_{\bVectR}(E_q , \R^k)$
		\end{enumerate}
	\end{defn}

	\begin{defn} \ld{def:vector_bundles:0001}
		Let $E,M \in \Obj(\bManinf)$ and $\pi \in \Hom_{\bManinf}(E, M)$ a surjection. Then $(E, M, \pi)$ is said to be a \tbf{rank-$k$ smooth vector bundle} if 
		\begin{enumerate}
			\item $(E, M, \pi, \R^k) \in \Obj(\bBuninf)$
			\item for each $p \in M$, $E_p$ is a $k$-dimensional real vector space and there exists $U \in \MT_M$ and $\Phi \in \Hom_{\bManinf}(\pi^{-1}(U), U \times \R^k)$ such that
			\begin{enumerate}
				\item $p \in U$
...
\begin{ex} \lex{ex:vector_bundles:0001.01}
	...
	\begin{enumerate}
		\item $\prj_1 \circ \Psi|_{\pi^{-1}(U \cap V)} \circ \Phi|^{-1}_{\pi^{-1}(U \cap V)} = \prj_1$
		\item there exists $\tau \in \Hom_{\bManinf}(U \cap V, GL(k, \R))$ such that for each $(p, v) \in (U \cap V) \times \R^k$, $\Psi|_{\pi^{-1}(U \cap V)} \circ \Phi|^{-1}_{\pi^{-1}(U \cap V)}(p, v) = (p, \tau(p)(v))$.
	\end{enumerate}
\end{ex}

\begin{proof}
	\rex{ex:fiber_bundles:0012} implies that there exists $\sig \in \Hom_{\bManinf}(( U \cap V) \times \R^k,  \R^k)$ such that $\Psi|_{\pi^{-1}(U \cap V)} \circ \Phi|^{-1}_{\pi^{-1}(U \cap V)} = (\prj_1, \sig)$ and for each $p \in U \cap V$, $\sig(p, \cdot) \in \Aut_{\bManinf}(\R^k)$. Define $\tau: U \cap V \rightarrow \Aut_{\bManinf}(\R^k)$ by $\tau(p) = \sig(p, \cdot)$. Since $(U,\Phi)$, $(V, \Psi)$ are smooth vector bundle local trivializations, for each $q \in U\cap V$, $\Phi|_{E_q} \rightarrow \{q\} \times \R^k$ and $\Psi|_{E_q} \rightarrow \{q\} \times \R^k$ are linear isomorphism. Let $q \in U \cap V$. Since $\Psi|_{E_q} \circ \Phi|^{-1}_{E_q}: \{q\} \times \R^k \rightarrow \{q\} \times \R^k$, is a vector space isomorphism and for each $v \in \R^k$,
	\begin{align*}
...
\begin{ex} \lex{ex:vector_bundles:0001.02} \tbf{Smooth Vector Bundle Chart Lemma:} \\
	Let $M \in \Obj(\bManinf)$ and $(E_p)_{p \in M} \subset \Obj(\bVectR)$. Set $n \defeq \dim M$. Suppose that for each $p \in M$, $\dim E_p = k$. We define $E \in \Obj(\bSet)$ and $\pi \in \Hom_{\bSet}(E, M)$ by
	...
	\begin{enumerate}
		...
		\item $\pi:E \rightarrow M$ is smooth 
		\item $(E, M, \pi)$ is a rank-$k$ $\bManinf$ vector bundle.
	\end{enumerate}
\end{ex}
...
\begin{proof}
	...
	\begin{enumerate}
		...
		\item $\pi: E \rightarrow M$ is smooth,
		\item $(E, M, \pi, \R^k)$ is an $\bManinf$ fiber bundle. 
		\begin{itemize}
			\item As noted above, $(E, M, \pi, \R^k) \in \Obj(\bBuninf)$.
...
\begin{defn} \ld{def:vector_bundles:0002}
	Let $(E_1, M_1, \pi_1)$ and $(E_2, M_2, \pi_2)$ be rank-$k_1$ and rank-$k_2$ smooth vector bundles respectively, $(\Phi, \phi) \in \Hom_{\bBuninf}((E_1, M_1, \pi_1, \R^{k_1}), (E_2, M_2, \pi_2, \R^{k_2}))$. Then $(\Phi, \phi)$ is said to be a \tbf{smooth vector bundle morphism} from $(E_1, M_1, \pi_1)$ to $(E_2, M_2, \pi_2)$ if for each $p \in M_1$, $\Phi|_{(E_1)_p}: (E_1)_p \rightarrow (E_2)_{\phi(p)}$ is linear.
\end{defn}


\begin{defn} \ld{def:vector_bundles:0003}
	We define the category of smooth vector bundles, denoted $\bVecBuninf$, by 
	\begin{itemize}
		\item $\Obj(\bVecBuninf) \defeq \{(E, M, \pi): (E, M, \pi) \text{ is a smooth vector bundle}\}$
		\item For $(E_1, M_1, \pi_1), (E_2, M_2, \pi_2) \in \Obj(\bVecBuninf)$ with $\rnk (E_1, M_1, \pi_1) = k_1$ and $\rnk (E_2, M_2, \pi_2) = k_2$,  
		\begin{align*}
			\Hom_{\bVecBuninf}((E_1, M_1, \pi_1), (E_2, M_2, \pi_2)) 
			& \defeq \{ (\Phi, \phi) \in \Hom_{\bBuninf}((E_1, M_1, \pi_1, \R^{k_1}), (E_2, M_2, \pi_2, \R^{k_2})): \\
...
\begin{ex} \lex{ex:vector_bundles:0004}
	We have that $\bVecBuninf$ is a subcategory of $\bBuninf$.
\end{ex}

\begin{proof} We note that 
	\begin{itemize}
		\item $\Obj(\bVecBuninf) \subset \Obj(\bBuninf)$
		\item for each $(E_1, M_1, \pi_1), (E_2, M_2, \pi_2) \in \Obj(\bVecBuninf)$ with $\rnk (E_1, M_1, \pi_1) = k_1$ and $\rnk (E_2, M_2, \pi_2) = k_2$, 
		$$\Hom_{\bVecBuninf}((E_1, M_1, \pi_1), (E_2, M_2, \pi_2)) = \Hom_{\bBuninf}((E_1, M_1, \pi_1, \R^{k_1}), (E_2, M_2, \pi_2, \R^{k_2}))$$
	\end{itemize}
\tcr{FINISH!!!}\\
	So $\bBuninf$ is a subcategory of $\MC$.
\end{proof}

\begin{ex} \lex{ex:vector_bundles:0005}
Let $M \in \Obj(\bManinf)$. Set $n \defeq \dim M$, $E \defeq M \times \R^k$ and define $\pi: E \rightarrow M$ by $\pi(p, x) \defeq p $. Then $(E, M, \pi)$ is a rank-$k$ smooth vector bundle.
\end{ex}
...
\begin{defn}
	Let $(E, M, \pi_E), (D, M, \pi_D) \in \Obj(\bVecBuninf)$. Then $(D, M, \pi_D)$ is said to be a \tbf{subbundle of $(E, M , \pi_E)$} if 
	\begin{enumerate}
...
\begin{defn}
	Let $(E_1, M, \pi_1), (E_2, M, \pi_2) \in \Obj(\bVecBuninf)$. We define the \tbf{tensor product of $(E_1, M, \pi_1)$ and $(E_2, M, \pi_2)$}, denoted $(E_1 \otimes E_2, M, \pi)$, by
\end{defn}
...
\begin{defn}
Let $(E_1, M, \pi_1), (E_2, M, \pi_2) \in \Obj(\bVecBuninf)$. Set
\begin{itemize}
...
, 			\end{align*}
	...
	\begin{defn}
		Let $(E, M, \pi) \in \Obj(\bVecBuninf)$. For each $k \in \N_0$, we define the \tbf{$E$-valued $k$-forms on $M$}, denoted $\Om^k(M; E)$ by $\Om^k(M; E) \defeq \Gam(\Lam^k T^*M \otimes E)$. 
	\end{defn}

	\begin{note}
		Let $(E, M, \pi) \in \Obj(\bVecBuninf)$ and $V \in \Obj(\bVectR)$. Then we write $\Om^k(M; V)$ in place of $\Om^k(M; M \times V)$.
	\end{note}
...
\begin{defn}
	Let $M,N \in \Obj(\bManinf)$ and $F \in \Hom_{\bManinf}(M,N)$. We define the \tbf{push-forward of $F$}, denoted $F_*:TM \rightarrow TN$, by $F_*(p, v) = (F(p), DF(p)(v))$. 
\end{defn}

\begin{ex}
	Let $M,N \in \Obj(\bManinf)$ and $F \in \Hom_{\bManinf}(M,N)$. Then $F_* \in \Hom_{\bManinf}(TM, TN)$.
\end{ex}
...
\begin{defn}
	Let $M,N \in \Obj(\bManinf)$ and $F \in \Hom_{\bManinf}(M,N)$. We define the \tbf{tangent functor}, denoted $T: \bManinf \rightarrow \bManinf$, by 
	\begin{itemize}
		\item $T(M) = TM$
		\item $TF = F_*$ 
	\end{itemize}
...
\begin{ex}
	Let $M,N \in \Obj(\bManinf)$ and $F \in \Hom_{\bManinf}(M,N)$. Then $T: \bManinf \rightarrow \bManinf$ is a functor.
\end{ex}
...
\begin{ex}
	Let $(P, X, \pi, G) \in \Obj(\bBuninf)$ and $\triangleleft \in \Hom_{\bManinf}(P \times G, P)$. Suppose that $\trl$ is a right group action. Then $\pi$ is $\trl$-invariant iff for each $x \in X$, $P_x \trl G = P_x$.
\end{ex}
...
\begin{defn}
	Let $(P, X, \pi, G) \in \Obj(\bBuninf)$ and $\triangleleft \in \Hom_{\bManinf}(P \times G, P)$. Suppose that
	\begin{itemize}
...
\begin{ex}
	Let Let $(P, X, \pi, G) \in \Obj(\bBuninf)$ and $\triangleleft \in \Hom_{\bManinf}(P \times G, P)$. Suppose that \begin{itemize}
		\item $G$ is a Lie group 
		\item $\trl$ a right group action
		\item $\pi$ is $\trl$-invariant.
	\end{itemize}  
...
\begin{defn}
	Let $P, X, G \in \Obj(\bManinf)$, $\pi \in \Hom_{\bManinf}(P, X)$ a surjection, $\trl \in \Hom_{\bManinf}(P \times G , P)$, $U \in \MT_X$ and $\Phi \in \Hom_{\bManinf}(\pi^{-1}(U), U \times G)$. Suppose that  
	\begin{itemize}
		\item $G$ is a Lie Group, 
...
\begin{defn} \ld{}
	Let $P,X, G \in \Obj(\bManinf)$ and $\pi \in \Hom_{\bManinf}(P, X)$ a surjection and $\trl \in \Hom_{\bManinf}(P \times G , P)$. Suppose that 
	\begin{itemize}
		\item $G$ is a Lie Group,
		\item $\trl$ is a right group action.
	\end{itemize}
	Then $(P, X, \pi, G, \trl)$ is said to be a \tbf{$\bManinf$ principle bundle with total space $P$, base space $X$, structure group $G$, projection $\pi$ and action $\trl$} if 
	\begin{enumerate}
		\item $(P, X, \pi, G) \in \Obj(\bBuninf)$, 
		\item $\pi$ is $\trl$-invariant,
		\item for each $x \in X$,
		\begin{enumerate}
			\item  $\trl_x: P_x \times G \rightarrow P_x$ is transitive and free,
			\item  there exists $U \in \MT_X$ and $\Phi \in \Hom_{\bManinf}(\pi^{-1}(U), U \times G)$ such that $(U, \Phi)$ is a smooth principle bundle local trivialization of $P$ over $U$ with respect to $\pi$ and $\trl$.
		\end{enumerate}
	\end{enumerate}
\end{defn}
...
\begin{defn} 
	We define the category of $\bManinf$-principle bundles, denoted $\bPrinBuninf$, by 
	\begin{itemize}
		\item $\Obj(\bPrinBuninf) \defeq \{(P, X, \pi, G, \trl): (P, X, \pi, G) \text{ is a $\bManinf$-principal bundle}\}$ 
		\item For $(P_1, X_1, \pi_1, G_1, \trl_1), (P_2, X_2, \pi_2, G_2, \trl_2) \in \Obj(\bPrinBuninf)$, 
		\begin{align*}
			& \Hom_{\bBuninf}((P_1, X_1, \pi_1, G_1, \trl_1), (P_2, X_2, \pi_2, G_2, \trl_2)) \defeq \\
			& \{(\Phi, \phi): (\Phi, \phi) \text{ is a smooth bundle morphism from $(E_1, M_1, \pi_1, F_1)$ to $(E_2, M_2, \pi_2, F_2)$} \}
		\end{align*}
		\item For 
		\begin{itemize}
...
\begin{defn} \ld{def:fiber_manifolds:0001}
	Let $E, M \in \Obj(\bManBndinf)$ and $\pi \in \Hom_{\bManBndinf}(E, M)$. Then $(E, M, \pi)$ is said to be a \tbf{fibered manifold} if $\pi$ is a surjective submersion. 
\end{defn}

\begin{defn}
	Let $E,F, M, N \in \Obj(\bManBndinf)$ and $\pi \in \Hom_{\bManBndinf}(E, M)$, $\tau \in \Hom_{\bManBndinf}(F, N)$, $\Phi \in \Hom_{\bManBndinf}(E, F)$ and $\phi \in \Hom_{\bManBndinf}(M, N)$. Suppose that $(E, M, \pi)$ and $(F, N, \tau)$ are fibered manifolds. Then $(\Phi, \phi)$ is said to be a \tbf{fibered manifold morphism} if $\tau \circ \Phi = \phi \circ \pi$, i.e. the following diagram commutes: \\
	\[ 
	\begin{tikzcd}
...
\begin{ex} \lex{ex:fiber_manifolds:0004}
	Let $(E, M, \pi, F) \in \Obj(\bBuninf)$. Then $(E, M, \pi)$ is a smooth fibered manifold. 
\end{ex}

\begin{proof}
	Since $(E, M, \pi, F) \in \Obj(\bBuninf)$, $\pi$ is surjective. \tcr{An exercise in the section on smooth fiber bundles} implies that $\pi$ is a submersion. Since $\pi$ is a surjective submersion, $(E, M, \pi)$ is a smooth fibered manifold.
\end{proof}
...
\begin{defn}
	Let $M, N \in \Obj(\bManBndinf)$, $A, B \in \MT_M$, $F \in \Hom_{\bManinf}(A, N)$, $G \in \Hom_{\bManinf}(B, N)$, $p \in A \cap B$ and $r \in \N_0$. Set $m \defeq \dim M$ and $n \defeq \dim N$. Then $F$ and $G$ are said to have a \tbf{contact of order $r$ at $p$} if there exists $(U, \phi) \in \MA_M|_{A \cap B}$ with $\phi = (x^1, \ldots, x^m)$ and $(V, \psi) \in \MA_N$ with $\psi = (y^1, \ldots, y^n)$ such that $p \in U$, $F(p), G(p) \in V$ and for each $j \in [n]$ and $\al \in \N_0$, $|\al| \leq r$ implies that
	$$\frac{\p^{|\al|} (y^j \circ F)}{\p x^{\al}}(p) =  \frac{\p^{|\al|} (y^j \circ G)}{\p x^{\al}}(p)$$
\end{defn}

\begin{ex}
	Let $M, N \in \Obj(\bManBndinf)$. Set $m \defeq \dim M$ and $n \defeq \dim N$. For $a \in \N_0$, we define 
	$$A_a \defeq \{(\be, \gam, \del, t, v) : \be, \del \in \N_0^m, \gam \in \N_0^n, |\be|, |\gam|, |\del| \leq a, t \in [m], v \in [n]\}.$$ 
	Then
	\begin{enumerate}
		\item For each $j \in [n]$ and $\al \in \N_0^n$, there exists a $P_{j, \al} \in \R[X_{\be, v}, X_{\gam}, X_{\del, t}: (\be, \gam, \del, t, v) \in A_{|\al|}]$ such that for each $F \in \Hom_{\bManBndinf}(M, N)$, $(U, \phi), (\tl{U}, \tl{\phi}) \in \MA_M$, $(V, \psi), (\tl{V}, \tl{\psi}) \in \MA_N$ with $\phi = (x^1, \ldots, x^m)$, $\tl{\phi} = (\tl{x}^1, \ldots, \tl{x}^m)$, $\psi = (y^1, \ldots, y^n)$, $\tl{\psi} = (\tl{y}^1, \ldots, \tl{y}^n)$ and $p \in (U \cap \tl{U}) \cap F^{-1}(V \cap \tl{V})$,
		...
		\item Let $F, G \in \Hom_{\bManBndinf}(M, N)$, $r \in \N_0$ and $p_0 \in M$. Suppose that $F$ and $G$ have a contact of order $r$ at $p_0$. Let $(U, \phi), (\tl{U}, \tl{\phi}) \in \MA_M$, $(V, \psi), (\tl{V}, \tl{\psi}) \in \MA_N$ with $\phi = (x^1, \ldots, x^m)$, $\tl{\phi} = (\tl{x}^1, \ldots, \tl{x}^m)$, $\psi = (y^1, \ldots, y^n)$, $\tl{\psi} = (\tl{y}^1, \ldots, \tl{y}^n)$. If $p_0 \in (U \cap \tl{U}) \cap F^{-1}(V \cap \tl{V})$, then 
...
\begin{defn}
	Let $M, N \in \Obj(\bManBndinf)$, $p \in M$, $q \in N$ and $r \in \N_0$. We define 
	\begin{itemize}
		\item $C^{\infty}_p(M, N) \defeq \{ F \in \Hom_{\bManBndinf}(U, N): U \in \MT_M, p \in U\}$ \tcr{move this definition to before definition of contact order at $p$ and rework that defintion using this notation.}
		\item ${\sim_r} \subset C^{\infty}_{p}(M, N) \times C^{\infty}_p(M, N)$ by $F {\sim_r} G$ iff $F$ and $G$ have a contact of order $r$ at $p$.  
	\end{itemize}
\end{defn}

\begin{ex}
	Let $M, N \in \Obj(\bManBndinf)$, $p \in M$, $q \in N$ and $r \in \N_0$. Then $\sim_r$ is an equivlaence relation on $C^{\infty}_p(M, N)$.
\end{ex}
...
\begin{defn}
	Let $M, N \in \Obj(\bManBndinf)$, $p \in M$, $q \in N$, $r \in \N_0$ and $F \in C^{\infty}_{(p,q)}(M, N)$. We define the 
	\begin{itemize}
...
\begin{defn}
	Let $(E, M, \pi, F) \in \Obj(\bBuninf)$, $r \in \N_0$ and $(V, \psi) \in \MA_E^{\pi}$. Set $n \defeq \dim M$ and $k \defeq \dim E - n$. Write $\psi = (x^1, \ldots, x^n, y^1, \ldots, y^k)$. We define the \tbf{$r$-th jet manifold of $\pi$}, denoted $J^r \pi$, by $J^r \pi \defeq \coprod\limits_{p \in M} J^r_p(M, E) \cap \Gam_p(\pi)$. We define the \tbf{$r$-th jet bundle projection of $J^r \pi$ onto $M$}, denoted $\pi_r: J^r \pi \rightarrow M$, by $\pi_r(p, s) \defeq p$. 
\end{defn}

\begin{note}
	Let $(E, M, \pi, F) \in \Obj(\bBuninf)$ and $(U, \phi) \in \MA_E^{\pi}$. Set $n \defeq \dim M$ and $k \defeq \dim E - n$. Then there exists $(U_0, \phi_0) \in \MA_M$ such that $U_0 = \pi(U)$ and $\prj^{n+k}_{n} \circ \psi = \psi_0 \circ \pi$. 
	\begin{itemize}
...
$J^r \pi \defeq \coprod\limits_{p \in M} J^r_p(M, E) \cap \Gam_p(\pi)$. We would identify $J^r_p s$ with $(p, J^r_p s)$ in the disjoint union. Define projective system of projections $(\pi_{r,k})_{r \leq k} \in \prod\limits_{r \leq k} \Hom_{\bTop}(J^k \pi, J^r \pi)$ defined by $\pi_{r, k}(J^k_p s) \defeq J^r_p s$. Can we define the smooth structure on $J^r \pi$ inductively to be the unique smooth structure such that $(\pi_{r, k})_{r \leq k}$ are smooth? 
...
\begin{ex}
	Let $(E, M, \pi, F) \in \Obj(\bBuninf)$. Then there exists a unique topology $\MT_{J^r \pi}$ on $J^r \pi$ and smooth structure $\MA_{J^r \pi} \subset X^{n+k}(J^r \pi, \MT_{J^r \pi})$ on $J^r \pi$ such that
	\begin{enumerate}
		...
		\item $(J^r \pi, M, \pi_r, F \times \R^{N-k-n}) \in \Obj(\bBuninf)$ 
	\end{enumerate}
...
\end{ex}
	...
	\begin{defn}
		Let $(P, X, \pi, G, \trl) \in \Obj(\bPrinBuninf)$ and $p \in P$. Set $x \defeq \pi(p)$. We define the \tbf{verticle tangent space of $P$ at $p$}, denoted $V_p$, by $V_p \defeq T_p(P_x)$. 
	\end{defn}
	
	\begin{ex}
		Let $(P, X, \pi, G, \trl) \in \Obj(\bPrinBuninf)$. For each $p \in P$, $V_p = \ker D \pi(p)$.
	\end{ex}
	...
	\begin{ex}
		Let $(E, M, \pi) \in \Obj(\bVecBuninf)$. There exists $\phi: \Conn_1 \rightarrow \Conn_2$ such that $\phi$ is a bijection. 
	\end{ex}
	...
	\begin{defn}
		Let $(E, M, \pi) \in \Obj(\bBuninf)$, $I \subset \R$ an interval, $\al \in \Hom_{\bManinf}(I, M)$ and $\gam \in \Hom_{\bManinf}(I, E)$. Then $\gam$ is said to be a \tbf{section of $E$ over $\al$} if $\pi \circ \gam = \al$. We denote the set of sections of $E$ over $\al$ by $\Gam(E, \al)$. 
	\end{defn}

	\begin{defn}
		Let $(E, M, \pi) \in \Obj(\bBuninf)$, $I \subset \R$ an interval, $\al \in \Hom_{\bManinf}(I, M)$ and $\gam \in \Gam(E, \al)$. Then $\gam$ is said to be said to be \tbf{extendible} if there exists $U \in \MN_{\al(I)}$ and $\tilde{\gam} \in \Gam(E|_U)$ such that $U$ is open and $\tilde{\gam} \circ \al = \gam $.
	\end{defn}
	...
	\begin{ex}
		Let $(E, M, \pi) \in \Obj(\bVecBuninf)$, $\nab$ a connection on $E$, $I \subset \R$ an interval and $\al \in \Hom_{\bManinf}(I, M)$. There exists a unique $D_{\al}: \Gam(E, \al) \rightarrow \Gam(E, \al)$ such that 
		\begin{enumerate}
...
\end{proof}
	...
	\begin{defn}
		Let $M \in \Obj(\bManinf)$ and $\om \in \Om^2(M)$. Then $\om$ is said to be \tbf{symplectic} if 
		\begin{enumerate}
