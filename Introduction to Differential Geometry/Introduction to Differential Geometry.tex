...
%	\end{defn}
	...
	\begin{ex} \lex{ex:maps_of_constant_rank:0003} \tbf{Constant Rank Theorem:} \\ \tcr{rework for $\H^m$ instead of $\R^m$}
	...
	\begin{defn} \ld{def:immersions:0001}
		Let $(M, \MA)$ and $(N, \MB)$ be smooth manifolds, $F: M \rightarrow N$ a smooth map. Then $F$ is said to be a \tbf{smooth immersion} if for each $p \in M$, $DF(p):T_pM\rightarrow T_{F(p)}N$ is injective.
	\end{defn}
	...
	\begin{defn}  \ld{def:immersions:0003}
		Let $(M, \MT_M, \MA_M), (N, \MT_N, \MA_N) \in \Obj(\Maninfbnd)$ and $F \in \Hom_{\Maninfbnd}(M,N)$. Then $F$ is said to be a \tbf{smooth embedding} if 
		\begin{enumerate}
	...
	\begin{defn} \ld{def:submersions:0001}
		Let $(M, \MA)$ and $(N, \MB)$ be smooth manifolds, $F: M \rightarrow N$ a smooth map. Then $F$ is said to be a \tbf{submersion} if for each $p \in M$, $DF(p) :T_pM\rightarrow T_{F(p)}N$ is surjective.
	\end{defn}
	...
	\begin{ex} \lex{ex:submersions:0002.2}
		Let $E, M$ be smooth manifolds, $\pi:E \rightarrow M$ smooth. Suppose that $\p E, \p M = \varnothing$. 
		\begin{enumerate}
	...
	\begin{defn} \ld{def:submersions:0003}
		Let $E, M$ be smooth manifolds, $\pi:E \rightarrow M$ smooth, $U \subset M$ open and $\sig:U \rightarrow E$. Then 
		\begin{itemize}
			\item $(U, \sig)$ is said to be a \tbf{smooth local section of $\pi$} if $\sig$ is smooth and $\sig$ is a section of $\pi|_{\pi^{-1}(U)}: \pi^{-1}(U) \rightarrow U$, 
			\item for each $p \in M$, we define 
			$$\Gam_p(\pi) \defeq \{(U, \sig): \text{$(U, \sig)$ is a local section of $\pi$ and $p \in U$}\}$$
		\end{itemize}
	\end{defn}

	\begin{ex} \lex{ex:submersions:0004}
		Let $(M, \MA)$, $(N, \MB)$ be smooth manifolds and $\pi: M \rightarrow N$. Suppose that $\pi$ is a surjective smooth submersion. Then $\pi$ admits local sections. \tcr{define this, maybe each $a \in E$ is in the image of a smooth section, or for each $p \in M$, there is a local section around $p$, or both}
	\end{ex}
...
\end{proof}
	...
	\tcr{TODO: start by defining topological manifold with boundary, then define manifold as a special case, but do so with $\R^n$ instead of $\Int \H^n_j$, then reserve $\Maninf$ for manifolds without boundary} and $\Maninf_{\p}$ for manifolds with boundary. Also, need to define $\Maninf$ as manifolds $M$ with $\p M = \varnothing$ and $\Maninfbnd$ for ones with boundary

	\begin{defn} \ld{def:imm_sub:embedded_submanifolds:0001}
		Let $M, S$ be smooth manifolds. Suppose that $S \subset M$. Then $S$ is said to be an \tbf{embedded submanifold} of $M$ if the inclusion map $\iota_S:S \rightarrow M$ is a smooth embedding.
	\end{defn}
	...
	\begin{proof} \
		\begin{enumerate}
			\item By definition, $\iota_S$ is a topological embedding \tcr{(check this)}. Let $p \in S$. Since $S$ atisfies the local $k$-slice condition with respect to $M$, there exists $(V_0, \psi_0) \in \S^k(M; S)$ such that $p \in V_0$. Set $V \defeq V_0 \cap S$ and $\psi \defeq \pi_{n,k} \circ \psi_0|_V$. By definition,  
			\begin{align*}
				(V, \psi) 
				& \in \MA|_S^0 \\
				& \subset \MA|_S.
	...
	\begin{ex}
		Let $M, N \in \Obj(\Maninf)$, $F \in \Hom_{\Maninf}(N,M)$ and $S \subset M$. Suppose that $S$ satisfies the local $k$-slice condition with respect to $M$, $F(N) \subset S$ and $F \in \Hom_{\Top}(N, S)$. Then $F \in \Hom_{\Maninf}(N, S)$.
	\end{ex}
	...
	\begin{proof}
		...
		\begin{align*}
 		...
 		\end{align*}
 		is smooth. Since $(U, \phi) \in (\MA_M)|_S^0$ and $(V, \psi) \in \MA_N$ are arbitrary, we have that for each $(U, \phi) \in (\MA_M)|_S^0$ and $(V, \psi) \in \MA_N$,  $\phi \circ F \circ \psi|_{V \cap F^{-1}(U)}^{-1}$ is smooth. \rex{ex:smooth_maps_between_manifolds:0003} implies that $F \in \Hom_{\Maninf}(N, S)$.  
	\end{proof}
	...
	\begin{ex}
	\tcr{talk about the boundary as an embedded submanifold. In particular if $\dim M = n$, then $\p M$ satisfies the local $n-1$-slice condition} Let $M \in \Obj(\Maninfbnd)$. Then $\p M$ is an embedded submanifold of $M$.  
	\end{ex}	
	...
	\begin{note}
		\tcr{reconcile this with subsection on group actions, try to just include new stuff about manifolds here and put stuff pertaining to just group action stuff in the other section}
	\end{note}
	...
	\begin{defn}
			Let $(P, M, \pi, G) \in \Obj(\Buninf)$ and $\triangleleft \in \Hom_{\Maninf}(P \times G, P)$. Suppose that $G$ is a Lie group and $\trl$ a smooth group action. Suppose that $\pi$ is $\trl$-invariant. Let $x \in M$. We define the \tbf{action of $G$ on $P_x$ induced by $\trl$}, denoted $\trl_x$, by $\trl_x \defeq \trl|_{P_x \times G}$. 
	\end{defn}
	...
	\begin{defn}
		Let $(P, M, \pi, G) \in \Obj(\Buninf)$ and $\triangleleft \in \Hom_{\Maninf}(P \times G, P)$. Suppose that $G$ is a Lie group and $\trl$ a group action. Then $(P, M, \pi, G, \trl)$ is said to be a \tbf{principal bundle} if 
		\begin{enumerate}
...
\begin{ex} \lex{ex:vector_bundles:0001.1} \tbf{Smooth Vector Bundle Chart Lemma:} \\
...
\begin{ex} \lex{ex:fiber_manifolds:0003}
	...
	\tbf{Hint:} Constant rank theorem \tcr{reference ex from submersions section}
\end{ex}
