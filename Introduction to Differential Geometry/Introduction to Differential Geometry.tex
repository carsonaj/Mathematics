...
%	\end{defn}
	...
	\begin{defn} \ld{def:submersions:0001}
		Let $(M, \MA)$ and $(N, \MB)$ be smooth manifolds, $F: M \rightarrow N$ a smooth map. Then $F$ is said to be a \tbf{smooth submersion} if for each $p \in M$, $DF(p) :T_pM\rightarrow T_{F(p)}N$ is surjective.
	\end{defn}
...
\end{proof}
	...
	\begin{ex}
		Let $M, R \in \Obj(\Maninf)$. Suppose that $R$ is a properly embedded submanifold of $M \times M$, $R$ is an equivlance relation on $M$, and $\prj_1|_R:R \rightarrow M$ is a surjective submersion. Then 
		\begin{enumerate}
