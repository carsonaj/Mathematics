...
\end{proof}		
	...
	\begin{ex} \tbf{Topological Manifold Chart Lemma:} \\
		...
		\begin{enumerate}[label=(\alph*)]
			...
			\item for each $\al, \be \in A$, $\phi_{\be}|_{U_{\al} \cap U_{\be}} \circ (\phi_{\al}|_{U_{\al} \cap U_{\be}})^{-1}: \phi_{\al}(U_{\al} \cap U_{\be}) \rightarrow \phi_{\be}(U_{\al} \cap U_{\be})$ is continuous
		...
		\end{enumerate}
		Then there exists a unique topology $\MT$ on $M$ such that $(U_{\al})_{\al \in A} \subset \MT$. Assumtion $(c)$ implies that \\\
		\tcb{FINISH!!!}
	\end{ex}
	
	\begin{proof}
			We define $\MB = \{\phi_{\al}^{-1}(V): V \subset \H^n \text{ is open in $\H^n$ and $\al \in A$} \}$. 
		\begin{enumerate}
			\item By assumption, $M \subset \bigcup\limits_{\al \in A} U_{\al}$
			\item Let $U_1, U_2 \in \MB$ and $x \in U_1 \cap U_2$. Then there exist $\al_1, \al_2 \in A$ and $V_1, V_2 \subset \H^n$ such that $V_1$, $V_2$ are open in $\H^n$, $U_1 = \phi_{\al_1}^{-1}(V_1)$ and $U_2 = \phi_{\al_2}^{-1}(V_2)$. Then $U_1 \cap U_2 = \phi$. 
		\end{enumerate}
	\end{proof}
