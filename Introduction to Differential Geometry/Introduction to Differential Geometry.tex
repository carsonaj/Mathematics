...
%	\end{defn}
	...
	\begin{ex} \tbf{Change of Coordinates:}\\
		Let $(U, \phi), (V, \psi) \in \MA$ with $\phi = (x^1, \cdots, x^n)$ and $\psi = (y^1, \cdots, y^n)$, $p \in U \cap V$ and $f \in C^{\infty}(M)$. Then for each $i \in \{1, \cdots, n\}$, 
		 $$\frac{\p}{\p y^i} \bigg|_p = \sum_{j =1}^n \frac{\p x^j}{\p y^i} (p) \frac{\p}{\p x^j} \bigg|_p.$$
	\end{ex}
	...
	\begin{proof}
		...
		\frac{\p}{\p y^i} \bigg|_p f 
			...
			&= \sum_{j=1}^n \bigg(\frac{\p}{\p u^j} \bigg|_{h \circ \psi (p)} f \circ \phi^{-1} \bigg)  \bigg( \frac{\p}{\p u^i} \bigg|_{\psi(p)} h^j \bigg) \\
			&= \sum_{j=1}^n \bigg(\frac{\p}{\p u^j} \bigg|_{\phi (p)} f \circ \phi^{-1}  \bigg) \bigg( \frac{\p}{\p u^i} \bigg|_{\psi(p)} x^j \circ \psi^{-1} \bigg) \\
			&= \sum_{j=1}^n \bigg( \frac{\p}{\p x^j} \bigg|_p f \bigg)  \bigg(   \frac{\p}{\p y^i} \bigg|_p x^j  \bigg)\\
			& = \bigg[ \sum_{j =1}^n \frac{\p x^j}{\p y^i} (p) \frac{\p}{\p x^j} \bigg|_p \bigg] f 
		\end{align*}
		Since $f \in C^{\infty}(M)$ is arbitrary, we have that 
		$$\frac{\p}{\p y^i} \bigg|_p = \sum_{j =1}^n \frac{\p x^j}{\p y^i} (p) \frac{\p}{\p x^j} \bigg|_p.$$
	\end{proof}
	...
	\begin{ex} \lex{ex:tangent_spaces:products:0001}
		Let $M,N \in \Obj(\ManBndinf)$. Set $m \defeq \dim M$ and $n \defeq \dim N$. Let $(U_M, \phi_M) \in \MA_M$ and $(U_N\phi_N) \in \MA_N$. Write $\phi_M = (x^1, \ldots, x^m)$ and $\phi_N = (y^1, \ldots, y^n)$. Define $\phi \in \MA_M \otimes \MA_N$ by $\phi \defeq \phi_M \times \phi_N$. Write $\phi = (\tl{x}^1, \ldots, \tl{x}^m, \tl{y}^1, \ldots, \tl{y}^n)$. Then 
		\begin{enumerate}
	...
	\begin{ex}
		there exists $\al \in \Iso_{\VectR}(T_{p, q} (M \times N), T_p M \times T_q N)$ such that $\al \bigg( \frac{\p}{\p \tl{x}^j } \bigg|_{(p,q)} \bigg) = \bigg( \frac{\p}{\p \tl{x}^j } \bigg|_{(p,q)} \bigg)$  i.e. the following diagram commutes:
	\end{ex}
...
\end{proof}
	...
	\begin{ex} \lex{ex:submanifolds:introduction:0002.1} \tbf{Immersed Implies Locally Embedded:} \\
		Let $M, S \in \Obj(\Maninf)$. Suppose that $S \subset M$. Then $S$ is an immersed submanifold fo $M$ iff for each $p \in S$, there exists $U \in \MT_S$ such that $p \in U$ and $U$ is an embedded submanifold of $M$.
	\end{ex}
	...
	\begin{ex}
		Let $U \subset \R^n$ and $S \subset U$. Suppose that $S$ is a $k$-slice of $U$. Define $\pi^n_{[k]}: \R^n \rightarrow \R^k$ by $$\pi(u^1, \dots, u^k, \dots, u^n) = (u^1, \dots, u^k)$$ Then $\pi^n_{[k]}|_{S} \rightarrow \pi(S)$ is a diffeomorphism.
	\end{ex}	
	...
	\begin{ex} \tbf{Constant Rank Level Set Theorem}: \\
	...
	\begin{ex} \tbf{Submersion Level Set Theorem}: \\
	...
	\begin{ex} \tbf{Regular Level Set Theorem}: \\
		Let $M, N, S \in \Obj(\Maninf)$ and $F \in \Hom_{\Maninf}(M,N)$. Set $m \defeq \dim M$, $n \defeq \dim N$ and $k \defeq \dim S$. Suppose that $S \subset M$ and $S$ is a regular level set of $F$. Then
		\begin{enumerate}
	...
	\begin{ex}
		Let $M, S \in \Obj(\Maninf)$. Set $m \defeq \dim M$ and $k \defeq \dim S$. Suppose that $S \subset M$. Then $S$ is an embedded submanifold of $M$ iff for each $p \in S$, there exists $U \in \MT_M$ and $F \in \Hom_{\Maninf}(U, \R^{n-k})$ such that $p \in U$, $F$ is a smooth submersion and $S \cap U$ is a regular level set of $F$.
	\end{ex}
	...
	\begin{ex}
		Let $M, S \in \Obj(\Maninf)$. Set $m \defeq \dim M$ and $k \defeq \dim S$. Suppose that $S \subset M$. Then $S$ is an embedded submanifold of $M$ iff for each $p \in S$, there exists $U \in \MT_M$ and $F \in \Hom_{\Maninf}(U, \R^{m-k})$ such that $p \in U$ and $F$ is a local defining map for $S \cap U$.
	\end{ex}
	...
	\begin{ex}
		Let $M, S \in \Obj(\Maninf)$. Suppose that $S \subset M$ and $S$ is an embedded submanifold of $M$. Set $n \defeq \dim M$ and $k \defeq \dim S$. Let $(U_0, \phi_0) \in \S^k(M;S)$ with $\phi_0 = (x^1, \ldots, x^n)$. Set $U \defeq U_0 \cap S$ and $\phi \defeq \pi^n_k \circ \phi_0|_U$ so that $(U, \phi) \in \MA_M|_S^0$. Let $p \in U$. Then for each $j \in [k]$, 
...
%		\tcr{a previous exercise in the section on intro to submanifolds} implies that there exists $V \in \MT_S$ such that $p \in V$ and $(V, \MT_S \cap V, \MA_S|_V)$ is an embedded submanifold of $(M, \MT_M, \MA_M)$. 
		Then there exists $(U, \phi) \in \MA_M|_S^0$ such that $p \in U$ and $\phi(p) = 0$. Then there exists $(U_0, \phi_0) \in \S^k(M;S)$ such that $U = U_0 \cap S$ and $\phi = \pi^n_k \circ \phi_0|_U$. Thus $\phi_0(p) = 0$. Write $\phi_0 = (x^1, \ldots, x^n)$ and $\phi = (\tl{x}^1, \ldots, \tl{x}^k )$. 
	...
	\begin{ex}
		Let $M, N, S_1, S_2, E_1, E_2 \in \Obj(\Maninf)$. Suppose that $S_1, S_2 \subset M$, $S_1, S_2$ are immersed submanifolds of $M$, $E_1, E_2 \subset N$, $E_1, E_2$ are immersed submanifolds of $N$. If $S_1, 2_1$ are transverse and $E_1, E_2$ are transverse, then $S_1 \times E_1$ and $S_2 \times E_2$ are transverse.
	\end{ex}
	...
	\tcr{need exercise about $S \subset M, E\subset N$ are embedded submanifolds, then $S \times E \subset M \times N$ is embedded submanifold}

	\begin{ex}
		\tcr{generalize the preimage submanifold result using transversality}
	\end{ex}
	...
	\begin{ex}
		Let $M, R \in \Obj(\Maninf)$. Suppose that $R$ is a properly embedded submanifold of $M \times M$, $R$ is an equivlance relation on $M$, and $\prj_1|_R, \prj_2|_R:R \rightarrow M$ the projection maps. Then for each $p \in M$, $\pi(p)$ is a properly embedded submanifold of $M$ and $\dim \pi(p) = \dim R - \dim M$. \\
		\tbf{Hint:} For each $p \in M$, $\pi(p) = \prj_1|_R(\prj_2|_R^{-1}(\{p\}))$ and $\prj_1|_{M \times \{p\}}$ is a diffeomorphism.
	\end{ex}

	\begin{proof}
		Let $p \in M$. \rex{arg1} implies that $\prj_1: M \times M \rightarrow M$ is a submersion. \rex{} implies that $M \times \{p\}$ is an embedded submanifold of $M \times M$. \rex{arg1} implies that $\prj_2|_R$ is a submersion. Since $\prj_2|_R$ is a surjective submersion, \rex{arg1} implies that $\prj_2|_R^{-1}(\{p\})$ is a properly embedded submanifold of $R$ and $\dim \prj_2|_R^{-1}(\{p\}) = \dim R - \dim M$. \tcr{need to show $\prj_2|_R^{-1}(\{p\})$ is an embedded submanifold of $M \times \{p\}$}. Since $\prj_1|_{M \times \{p\}}$ is a diffeomorphism and $\pi(p) = \prj_1|_{M \times \{p\}}(\prj_2|_R^{-1}(\{p\}))$, \rex{arg1} \tcr{make exercise in the section on embedded submanifolds} implies that $\pi(p)$ is an embedded submanifold of $M$ and $\dim \pi(p) = \dim R - \dim M$.
	\end{proof}
...
\begin{defn} \ld{def:fiber_manifolds:0001}
	Let $E, M \in \Obj(\Maninf)$ and $\pi \in \Hom_{\Maninf}(E, M)$. Then $(E, M, \pi)$ is said to be a \tbf{fibered manifold} if $\pi$ is a surjective submersion. 
\end{defn}

\begin{defn}
	Let $E,F, M, N \in \Obj(\Maninf)$ and $\pi \in \Hom_{\Maninf}(E, M)$, $\tau \in \Hom_{\Maninf}(F, N)$, $\Phi \in \Hom_{\Maninf}(E, F)$ and $\phi \in \Hom_{\Maninf}(M, N)$. Suppose that $(E, M, \pi)$ and $(F, N, \tau)$ are fibered manifolds. Then $(\Phi, \phi)$ is said to be a \tbf{fibered manifold morphism} if $\tau \circ \Phi = \phi \circ \pi$, i.e. the following diagram commutes: \\
	\[ 
	\begin{tikzcd}
...
\begin{defn} \ld{def:fiber_manifolds:0002}
	...
	\end{enumerate}
	We define $\F(\pi) \defeq \{(U, \phi) \in \MA_E: \text{$(U, \phi)$ is $\pi$-fibered} \}$.
\end{defn}

\begin{ex} \lex{ex:fiber_manifolds:0003}
	Let $(E, M, \pi)$ be a smooth fibered manifold. Suppose that $\p E, \p M = \varnothing$. Then for each $a \in E$, there exists $(V, \psi) \in \F(\pi)$ such that $a \in V$. \\
...
\begin{ex} \lex{ex:fiber_manifolds:0004}
	Let $(E, M, \pi, F) \in \Obj(\Maninf)$. Then $(E, M, \pi)$ is a smooth fibered manifold. 
\end{ex}

\begin{proof}
	Since $(E, M, \pi, F) \in \Obj(\Maninf)$, $\pi$ is surjective. \tcr{An exercise in the section on smooth fiber bundles} implies that $\pi$ is a submersion. Since $\pi$ is a surjective submersion, $(E, M, \pi)$ is a smooth fibered manifold.
\end{proof}
