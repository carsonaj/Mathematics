...
\begin{defn}
	Let $M, N \in \Obj(\ManBndinf)$, $F, G: M \rightarrow N$, $p \in M$ and $r \in \N_0$. Set $m \defeq \dim M$ and $n \defeq \dim N$. Then $F$ and $G$ are said to have a \tbf{contact of order $r$ at $p$} if there exists $(U, \phi) \in \MA_M$ with $\phi = (x^1, \ldots, x^m)$ and $(V, \psi) \in \MA_N$ with $\psi = (y^1, \ldots, y^n)$ such that $p \in U$, $F(p), G(p) \in V$ and for each $j \in [n]$ and $\al \in \N_0$, $|\al| \leq r$ implies that
...
\begin{defn}
	...
	\begin{itemize}
		...
		\item \tbf{$r$-jets with source $p$ and target $q$}, denoted $J_{(p,q)}^r$, by $J_{(p,q)}^r \defeq C^{\infty}_{(p, q)}(M, N) / {\sim_r}$
	\end{itemize}
\end{defn}
...
\newpage
\section{Jet Bundles of Fibered Manifolds}

\begin{defn}
	Let $(E, M, \pi, F) \in \Obj(\Buninf)$, $r \in \N_0$, $a \in E$ and $(V, \psi) \in \MA_E^{\pi}$. Set $n \defeq \dim M$ and $k \defeq \dim E - n$. Write $\psi = (x^1, \ldots, x^n, y^1, \ldots, y^k)$. We define the \tbf{$r$-th jet bundle of $\pi$}, denoted $J^r \pi$, by $J^r \pi \defeq \{ J^r_p s: s \in \Gam_p(\pi)\}$.
\end{defn}

\begin{defn}
	Let $(E, M, \pi, F) \in \Obj(\Buninf)$, $a \in E$ and $(V, \psi) \in \MA_E^{\pi}$. Set $n \defeq \dim M$ and $k \defeq \dim E - n$. Write $\psi = (x^1, \ldots, x^n, y^1, \ldots, y^k)$. We define $\Psi_{\psi}: J$
\end{defn}
