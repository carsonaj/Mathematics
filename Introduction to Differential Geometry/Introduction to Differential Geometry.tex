...
%	\end{defn}
	...
	\begin{ex} \lex{ex:immersions:0001}
	...
	\begin{proof}\
		\begin{enumerate}
			...
			\begin{align*}
				...
				& = \prj_2 \circ
			\end{align*}
			\rex{ex:smooth_maps_and_product_manifolds:0008} implies that $\sig(p, \cdot): F \rightarrow F$ is a diffeomorphism \tcb{(needs more justification, show that it is a local diffeo and bijection.)}.  \tcr{FINISH!!!, note: $F$ doesnt have boundary and $\iota_p^F$ is a smooth embedding with $\iota_p^F(F)  $ is an embedded submanifold of $(U \cap V) \times F$, so $\iota_p^F:F \rightarrow \iota_p^F(F)$ is a diffeomorphism, so $\p \iota_p^F(F) = \varnothing$. Also, $\Psi \circ \Phi|_{\iota_p^F}$ is diffeo, so $\Psi \circ \Phi|_{\iota_p^F} \circ \iota_p^F$ is diffeo whose image is an embedded submanifold with no boundary. Maybe use constant rank theorem} 
		\end{enumerate}
	\end{proof}
