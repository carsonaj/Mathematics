...
\end{proof}		
	...
	\begin{proof}\
		\begin{enumerate}
			...
			\begin{align*}
				...
				& \subset \Int M \cup \p M
			\end{align*}   
			Suppose that $\phi(p) \not \in \p \H^n$. Then  $\phi(p) \in \Int \H^n$. Hence there exists $B \subset \phi(U)$ such that $B$ is open in $\R^n$ and $\phi(p) \in B$. Set $U' = \phi^{-1}(B)$ and $\phi' = \phi|_{U'}$. Then $p \in U'$, $U'$ is open in $M$ and $(U', \phi') \in X_{\Int}(M)$. Therefore, 
			\begin{align*}
				p 
				& \in \Int M \\
				& \subset \Int M \cup \p M
			...
			\item For the sake of contradiction, suppose that $\Int M \cap \p M \neq \varnothing$. Then there exists $p \in M$ such that $p \in \Int M \cap \p M$. By definition, there exists $(U, \phi) \in X_{\Int}(M)$, $(V, \psi) \in X_{\p}(M)$ such that $p \in U \cap V$ and $\psi(p) \in \p \H^n$. Then $\psi(U \cap V)$ is open in $\H^n$ and $\phi(U \cap V)$ is open in $\R^n$ and $\phi \circ \psi^{-1}: \psi^{-1}(U \cap V) \rightarrow \phi(U \cap V)$ is a homeomorphism. Since $\psi(U \cap V)$ is open in $\H^n$, there exists an open ball $B_{\psi} \subset \H^n$ such $B_{\psi} \subset \psi(U \cap V)$ and $\psi(p) \in B_{\psi}$. Set $B_{\phi} = \phi \circ \psi^{-1}(B_{\psi})$. Since $\phi(U \cap V)$ is open in $\R^n$, $B_{\phi}$ is open in $\R^n$. Set $B_{\phi}' = B_{\phi} \setminus \{\phi(p)\}$ and $B_{\psi}' = B_{\psi} \setminus \{\psi(p)\}$. Then $\phi \circ \psi^{-1}: B_{\psi}' \rightarrow B_{\phi}'$ is a homeomorphism. Since $\psi(p) \in \p \H^n$, $B_{\psi}'$ is simply connected. Since $B_{\phi}$ is open in $\R^n$, $B_{\phi}'$ is not simply connected. This is a contradiction since $B_{\phi}'$ is homeomorphic to $B_{\psi}'$. So $\p M \cap \Int M = \varnothing$. 
		\end{enumerate}
	\end{proof}
	...
	\begin{proof}
		Suppose that $p \in \partial M$. Then there exists a $(V, \psi) \in X_{\p}(M)$ such that $p \in V$ and $\psi(p) \in \partial \H^n$. Then $\phi \circ \psi^{-1} : \psi(V \cap U) \rightarrow \phi(V \cap U)$ is a homeomorphism. Since $\psi(U \cap V)$ is open in $\H^n$, there exists an open ball $B_{\psi} \subset \H^n$ such $B_{\psi} \subset \psi(U \cap V)$ and $\psi(p) \in B_{\psi}$. Set $B_{\phi} = \phi \circ \psi^{-1}(B_{\psi})$. \\
	...
	\begin{ex}
		Let $M$ be an $n$-dimensional manifold. Let $(U, \phi) \in X(M)$. Then 
		\begin{enumerate}
			\item for each $p \in U$, $p \in \p M$ iff $\phi(p) \in \p \H^n$.
			\item $\phi(U \cap \p M) = \phi(U) \cap \p \H^n$ and $\phi(U \cap \Int M) = \phi(U) \cap \Int \H^n$ 
		\end{enumerate}
	\end{ex}

	\begin{proof}\
		\begin{enumerate}
			\item Let $p \in U$. Suppose that $p \in \p M$. The previous exercise implies that $\phi(p) \in \p \H^n$. Conversely, suppose that $\phi(p) \in \p \H^n$. By definition, $p \in \p M$.
			\item \
			\begin{itemize}
				\item Part $(1)$ implies that $\phi(U \cap \p M) \subset \p \H^n$. Since $\phi(U \cap \p M) \subset \phi(U)$, we have that $\phi(U \cap \p M) \subset \phi(U) \cap \p \H^n$. Let $q \in \phi(U) \cap \p \H^n$. Then there exists $p \in U$ such that $q = \phi(p)$. Part $(1)$ implies that $p \in \p M$. Hence $p \in U \cap \p M$ and $q \in \phi(U \cap \p M)$. Since $q \in \phi(U) \cap \p \H^n$ is arbitrary, $\phi(U) \cap \p \H^n \subset \phi(U \cap \p M)$. Thus $\phi(U \cap \p M) = \phi(U) \cap \p \H^n$.
				\item Similarly, $\phi(U \cap \Int M) = \phi(U) \cap \Int \H^n$.
			\end{itemize}
		\end{enumerate}
	\end{proof}
	...
	\begin{proof}\
		\begin{enumerate}
			\item Let $p \in \Int M$. Then there exists a coordinate chart $(U, \phi) \in X(M)$ such that $p \in U$.
			\begin{itemize}
				 \item If $(U, \phi)$ is a boundary chart, then $\phi(U)$ is open in $\H^n$. Since $p \not \in \p M$, $\phi(p) \not \in \p \H^n$. Hence $p \in \Int \H^n$. Since $\Int H^n$ is open in $\R^n$, $\phi^{-1}(U \cap \Int )$ there exists $U \subset \R^n$ such that $\phi(p) \in U$
			\end{itemize}
			\item 
		\end{enumerate}
	\end{proof}
	...
	\begin{defn}
		Let $\MA \subset X(M)$. Then $\MA$ is said to be an \textbf{atlas on $M$} if  $\bigcup\limits_{(U,\phi) \in \MA} U = M$.
	\end{defn}

	\begin{defn} Let $M$ be a topological space.
		\begin{enumerate}
			...
			\item Let $\MA$ be an atlas on $M$. Then $(M, \MA)$ is said to be an \textbf{$n$-dimensional smooth manifold} if $(M, \MA)$ is an $n$-dimensional manifold and $\MA$ is a smooth structure on $M$. 
		\end{enumerate}
	\end{defn}
