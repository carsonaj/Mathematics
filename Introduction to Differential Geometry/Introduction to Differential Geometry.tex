...
\end{proof}		
	...
	\begin{defn}
		...
		We endow $\p \H^n$ and $(\H^n)^{\circ}$ with the subspace topology.
	\end{defn}
	...
	$$f(x_1, \ldots, x_{n-1}, 0) = (x_1, \ldots, x_n)$$ 
	...
	\begin{defn}
		...
		Define $\MA|_{\p M} = \{(\bar{U}, \bar{\phi}): (U, \phi) \in \MA\}$.
	\end{defn}

	\begin{ex}
		Let $(M, \MA)$ be a topological $n$-manifold. Then 
		\begin{enumerate}
			\item $\MA|_{\p M}$ is an atlas on $\p M$  
			\item $(\p M, \MA|_{\p M})$ is a topological $n-1$-manifold
	...
	\begin{proof}\
	\begin{enumerate}
		\item Let $(\bar{U}, \bar{\psi}) \in \MA|_{\p M}$
		\item 
		\item 
	\end{enumerate}
	\end{proof}
