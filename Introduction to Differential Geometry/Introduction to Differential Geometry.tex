...
\end{proof}		
	...
	\begin{itemize}
		\item \tcr{redo in terms of all upper half spaces, this will make dealing with products much easier}
	...
	\begin{defn} \ld{def:topological_manifolds_introduction:0006}
		Let $(M, \MT)$ be a topological space, $n \in \N$, $j \in [n]$, $U \subset M$, $V \subset \H^n_j $ and $\phi:U \rightarrow V$. Then $(U, \phi)$ is said to be an \tbf{$(n,j)$-coordinate chart on $(M, \MT)$} if 
		\begin{itemize}
			\item $U \in \MT$ 
			\item $V \in \MT_{\H^n_j}$, 
		...
		\end{itemize}
	We define 
	$$X^{n,j}(M, \MT) \defeq \{(U, \phi): (U, \phi) \text{ is an $(n,j)$-coordinate chart on $(M, \MT)$} \}$$
	\end{defn}
...
\begin{defn} \ld{def:smooth_product_manifolds:0003}
	Let $M$, $N$ be smooth manifolds of dimension $m$ and $n$ respectively, $\MA \subset X^m(M)$ and $\MB \subset X^n(N)$. Suppose that $\MA$ and $\MB$ are smooth atlases on $M$ and $N$ respectively and $\p N = \varnothing$. We define the \tbf{product atlas of $\MA$ and $\MB$ on $M \times N$}, denoted $\MA \otimes_0 \MB$, by 
...
\begin{ex} \lex{ex:smooth_product_manifolds:0004}
	Let $M$, $N$ be smooth manifolds of dimension $m$ and $n$ respectively, $\MA \subset X^m(M)$ and $\MB \subset X^n(N)$. Suppose that $\MA$ and $\MB$ are smooth atlases on $M$ and $N$ respectively and $\p N = \varnothing$. Then $\MA \otimes_0 \MB$ is a smooth atlas on $M \times N$. 
\end{ex}
...
%	\end{defn}
	...
	\begin{ex} \lex{ex:maps_of_constant_rank:0003} \tbf{Constant Rank Theorem:} \\ \tcr{rework for $\H^m$ instead of $\R^m$}
			Let $(M, \MA)$ and $(N, \MB)$ be smooth manifolds of dimensions $m$ and $n$ respectively, $F \in C^{\infty}(M,N)$. Suppose that, $\p M, \p N = \varnothing$, $F$ has constant rank and $\rnk F = k$. Then for each $p \in M$, there exist $(U, \phi) \in \MA$ and $(V, \psi) \in \MB$ such that $p \in U$, $F(p) \in V$ and
	...
	\begin{ex} \tbf{Local Immersion Theorem:} \\
		local immersion for manifolds with boundary
	\end{ex}
	...
	\begin{defn}
		Let $(M, \MA)$ and $(N, \MB)$ be smooth manifolds and $F:M \rightarrow N$ smooth. Then $F$ is said to be a \tbf{smooth embedding} if 
		\begin{enumerate}
			\item $F:M \rightarrow F(M)$ is a homeomorphism,
			\item $F$ is a smooth immersion. 
		\end{enumerate}	 
	\end{defn}	
	...
	\begin{defn}
		...
		\begin{itemize}
			\item $(U, \sig)$ is said to be a \tbf{smooth local section of $\pi$} if $\sig$ is a section of $\pi|_{\pi^{-1}(U)}: \pi^{-1}(U) \rightarrow U$ if, 
	...
	\begin{ex} \lex{ex:fiber_bundles:0012}
		...
		\begin{enumerate}
			\item $\prj_{U \cap V} \circ \Psi|_{\pi^{-1}(U \cap V)} \circ \Phi|^{-1}_{\pi^{-1}(U \cap V)} = \prj_1$
