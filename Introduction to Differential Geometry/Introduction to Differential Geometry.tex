...
\end{proof}		
	...
	\begin{defn}
		Let $M$ be a topological space and $n \in \N_0$. Let $U \subset M$ and $V \subset \H^n$ and $\phi:U \rightarrow V$. Then $(U, \phi)$ is said to be a \tbf{$n$-coordinate chart on $M$} if 
		\begin{itemize}
			\item $U$ is open in $M$
			\item $V$ is open in $\H^n$
...
\begin{proof} \
	\begin{enumerate}
		...
		\begin{enumerate}
			...
			Set $\MT_{M} = \tau(\MB)$. Let $\al \in \Gam$. By definition, for each $B \subset \MT_{\H^n}$, 
			\begin{align*}
				\phi_{\al}^{-1}(B) 
				& \in \MB \\
				& \subset \MT_M
...
\end{proof}
	...
	\begin{proof}
		Set $\MB = \{\phi_{\al}^{-1}(V): \al \in \Gam \text{ and } V \in \MT_{\H^n}\}$ and $\MA' = \{(U_{\al}, \phi_{\al}) : \al \in \Gam\}$. The topological manifold chart lemma implies that $(M, \tau(\MB))$ is an $n$-dimensional topological manifold and $\MA' \subset X^n(M)$. Since $M = \bigcup_{\al \in \Gam} U_{\al}$, $\MA'$ is an atlas on $M$. Since for each $\al, \be \in \Gam$, $\phi_{\be}|_{U_{\al} \cap U_{\be}} \circ (\phi_{\al}|_{U_{\al} \cap U_{\be}})^{-1}: \phi_{\al}(U_{\al} \cap U_{\be}) \rightarrow \phi_{\be}(U_{\al} \cap U_{\be})$ is smooth, we have that $\MA'$ is smooth. A previous exercise implies that there exists a unique smooth structure $\MA$ on $M$ such that $\MA' \subset \MA$. 
	\end{proof}
