...
\end{defn}
	...
	\begin{ex} \tbf{Topological Fiber Bundle Chart Lemma:} \\
		...
		\begin{itemize}
			\item for each $\al \in \Gam$, $U_{\al}$ is open
			\item $M \subset \bigcup\limits_{\al \in \Gam} U_{\al}$ 
			\item for each $\al \in \Gam$, $(U_{\al}, \Phi_{\al})$ is a continuous local trivialization of $E$ over $U$
	...
	\begin{proof}
		Let $(p,q) \in M \times F$. There exists $\al_p \in \Gam$, $(V_p^M, \psi^M_p) \in X^n(M, \MT_M)$ and $(V_p^M, \psi^M_p) \in X^k(F, \MT_F)$ such that $p \in V^M_p$, $V^M_p \subset U_{\al_p}$ and $q \in V_q^F$. \\ 
		Define $V_{p,q} \subset E$ and $\psi_{p,q}: V_{p,q} \rightarrow \psi_p^M(V_p^M) \times \psi_q^F(V_q^F)$ by 
		\begin{itemize}
			\item $V_{p,q} = \Phi_{\al_p}^{-1}(V^M_p \times V^F_q)$ 
			\item $\psi_{p,q} = (\psi_p^M \times \psi_q^F) \circ \Phi_{\al_p}|_{V_{p,q}}$
		\end{itemize}
		We have the following:
		\begin{itemize}
			\item Let $(p,q) \in M \times F$. Then $\psi_{p,q}(V_{p,q}) = \psi_p^M(V_p^M) \times \psi_q^F(V_q^F)$ and thus $\psi_{p,q}(V_{p,q}) \in \MT_{\H^{n + k}}$ 
			\item for each $(p_1,q_1), (p_2, q_2) \in M \times F$, 
			\begin{align*}
				\psi_{p_1, q_1}(V_{p_1, q_1} \cap V_{p_2, q_2}) 
				& = (\psi_{p_1}^M \times \psi_{q_1}^F) \circ \Phi_{\al_{p_1}}|_{V_{p_1,q_1}} (\Phi_{\al_{p_1}}^{-1}([V^M_{p_1} \times V^F_{q_1})] \cap [V^M_{p_2} \times V^F_{q_2}])) \\
				& = (\psi_{p_1}^M \times \psi_{q_1}^F)([V^M_{p_1} \times V^F_{q_1})] \cap [V^M_{p_2} \times V^F_{q_2}]) \\
				& = (\psi_{p_1}^M \times \psi_{q_1}^F)([V^M_{p_1} \cap V^M_{p_2}] \times [V^F_{q_1} \cap V^F_{q_2}]) \\
				& = \psi_{p_1}^M (V^M_{p_1} \cap V^M_{p_2}) \times \psi_{q_1}^F(V^F_{q_1} \cap V^F_{q_2}) \\
				& \in \MT_{\H^{n+k}}
			\end{align*}
			\item Let $(p,q) \in M \times F$. Then $\psi_{p,q}: V_{p,q} \rightarrow \psi_p^M(V_p^M) \times \psi_q^F(V_q^F)$ is a bijection
			\item Let $(p_1,q_1), (p_2, q_2) \in M \times F$. For notational convenience, set $\psi_1 = \psi_{p_1, q_1}$, $\psi_2 = \psi_{p_2, q_2}$, $V = V_{p_1, q_1} \cap V_{p_2, q_2}$, $V^M = V_{p_1}^M \cap V^M_{p_2}$ and $V^F = V^F_{q_1} \cap V^F_{q_2}$. Then $\psi_2|_V \circ (\psi_1|_V)^{-1}: \psi_1(V) \rightarrow \psi_2(V)$ is given by
			\begin{align*}
				\psi_2|_V \circ (\psi_1|_V)^{-1}
				& = [(\psi_{p_2}^M|_{V^M} \times \psi_{q_2}^F|_{V^F}) \circ \Phi_{\al_{p_2}}|_V] \circ [(\psi_{p_1}^M|_{V^M} \times \psi_{q_1}^F|_{V^F}) \circ \Phi_{\al_{p_1}}|_V]^{-1} \\
				& = [(\psi_{p_2}^M|_{V^M} \times \psi_{q_2}^F|_{V^F}) \circ \Phi_{\al_{p_2}}|_{V}] \circ [(\Phi_{\al_{p_1}}|_{V})^{-1} \circ (\psi_{p_1}^M|_{V^M} \times \psi_{q_1}^F|_{V^F})^{-1}] \\
				& = (\psi_{p_2}^M|_{V^M} \times \psi_{q_2}^F|_{V^F}) \circ [\Phi_{\al_{p_2}}|_{V} \circ (\Phi_{\al_{p_1}}|_{V})^{-1}] \circ (\psi_{p_1}^M|_{V^M} \times \psi_{q_1}^F|_{V^F})^{-1}
			\end{align*}
			Since $\Phi_{\al_{p_2}}|_V \circ (\Phi_{\al_{p_1}}|_V)^{-1}$ is continuous, we have that $\psi_{p_2, q_2}|_{V_{p_1, q_1} \cap V_{p_2, q_2}} \circ (\psi_{p_1, q_1}|_{V_{p_1, q_1} \cap V_{p_2, q_2}})^{-1}: \psi_{p_1, q_1}(V_{p_1, q_1} \cap V_{p_2, q_2}) \rightarrow \psi_{p_2, q_2}(V_{p_1, q_1} \cap V_{p_2, q_2})$ is continuous.
			\item \tcb{countable}
			\item \tcb{Hausdorff}
		\end{itemize}
		\tcb{FINISH!!!} 
