...
\tcb{in order for a smooth structure on a product to make sense, we need to have a notion of smoothness of a map $\R^m \times \R^m \rightarrow \R^k$ to make sense. But we already have this, think of them as banach spaces, use the definitions of differnetiation of maps from a product}.
...
\section{$\R$-Valued Smooth Maps}	

\begin{defn} \ld{42001}
	Let $(M, \MA)$ be a smooth manifold and $f: M \rightarrow \R$. Then $f$ is said to be \tbf{smooth} if for each coordinate chart $(U, \phi) \in \MA$, $f \circ \phi^{-1}: \phi(U) \rightarrow \R$ is smooth. The set of all smooth functions on $M$ is denoted $C^{\infty}(M, \MA)$. 
\end{defn}
...
\begin{ex}
	Let $(M, \MA)$ be a smooth manifold, $\MB \subset \MA$. Suppose that $\MB$ is an atlas on $M$ and $f:M \rightarrow \R$. Then $f$ is smooth iff for each $(U, \phi) \in \MB$, $f \circ \phi^{-1}: \phi(U) \rightarrow \R$ is smooth.
\end{ex}
...
\begin{defn}
	...
	\begin{itemize}
		\item \tbf{smooth} if for each $(U, \phi) \in \MA$ and $(V, \psi) \in \MB$, $$\psi|_{F(U) \cap V} \circ F|_{U \cap F^{-1}(V)} \circ \phi|_{U \cap F^{-1}(V)}^{-1}: \phi(U \cap F^{-1}(V)) \rightarrow \psi(F(U) \cap V)$$ is smooth 
		\item a \tbf{diffeomorphism} if $F$ is a bijection and $F,F^{-1}$ are smooth. 
	\end{itemize}
\end{defn}

\begin{ex}
	Let $(M, \MA)$ and $(N, \MB)$ be smooth manifolds, $\MA_0 \subset \MA$, $\MB_0 \subset \MB$ and $F: M \rightarrow N$. Suppose that $\MA_0$ is an atlas on $M$ and $\MB_0$ is an atlas on $N$. Then $F$ is smooth iff for each $(U, \phi) \in \MA_0$ and $(V, \psi) \in \MB_0$, $\psi|_{F(U) \cap V} \circ F|_{U \cap F^{-1}(V)} \circ \phi|_{U \cap F^{-1}(V)}^{-1}: \phi(U \cap F^{-1}(V)) \rightarrow \psi(F(U) \cap V)$ is smooth.
\end{ex}

\begin{proof}\
	\begin{itemize}
		\item $(\implies)$: \\
		Suppose that $F$ is smooth. Since $\MA_0 \subset \MA$ and $\MB_0 \subset \MB$, by definition, for each $(U, \phi) \in \MA_0$ and $(V, \psi) \in \MB_0$, $\psi|_{F(U) \cap V} \circ F|_{U \cap F^{-1}(V)} \circ \phi|_{U \cap F^{-1}(V)}^{-1}: \phi(U \cap F^{-1}(V)) \rightarrow \psi(F(U) \cap V)$ is smooth.
		\item $(\impliedby)$: \\
		Suppose that for each $(U, \phi) \in \MA_0$ and $(V, \psi) \in \MB_0$, $\psi|_{F(U) \cap V} \circ F|_{U \cap F^{-1}(V)} \circ \phi|_{U \cap F^{-1}(V)}^{-1}: \phi(U \cap F^{-1}(V)) \rightarrow \psi(F(U) \cap V)$ is smooth. \\
		\tcb{FINISH!!!}
	\end{itemize}
\end{proof}

\begin{ex}
	Let $(M, \MA)$ and $(N, \MB)$ be smooth manifolds and $F: M \rightarrow N$. Then $F$ is smooth iff for each $U \subset M$, $U$ is open in $M$ implies that $F|_U: U \rightarrow N$ is smooth. 
\end{ex}

\begin{proof}\
	\begin{itemize}
		\item $(\implies)$: \\
		Suppose that $F:M \rightarrow N$ is smooth. Let $U \subset M$. Suppose that $U$ is open in $M$. Set $G = F|_U$. Let $(W, \eta) \in \MA$ and $(V, \psi) \in \MB$. Suppose that $W \subset U$. Since $F$ is smooth, $\psi|_{F(W) \cap V} \circ F|_{W \cap F^{-1}(V)} \circ \phi|_{W \cap F^{-1}(V)}^{-1}$ is smooth. Since $W \subset U$, 
		\begin{itemize}
			\item $\psi|_{F(W) \cap V} = \psi|_{G(W) \cap V}$
			\item $\phi|_{W \cap F^{-1}(V)} = \phi|_{W \cap G^{-1}(V)}$
			\item $F|_{W \cap F^{-1}(V)} = G|_{W \cap G^{-1}(V)}$
		\end{itemize}
		we have that 
		$$\psi|_{G(W) \cap V} \circ G|_{W \cap G^{-1}(V)} \circ \phi|_{W \cap G^{-1}(V)}^{-1} = \psi|_{F(W) \cap V} \circ F|_{W \cap F^{-1}(V)} \circ \phi|_{W \cap F^{-1}(V)}^{-1}$$ 
		Therefore $\psi|_{G(W) \cap V} \circ G|_{W \cap G^{-1}(V)} \circ \phi|_{W \cap G^{-1}(V)}^{-1}$ is smooth. Since $(W, \eta) \in \MA$ with $W \subset U$ is arbitrary, we have that $\psi|_{G(W) \cap V} \circ G|_{G^{-1}(V)}$\\
		\tcb{FINISH!!!}
		\item $(\impliedby)$: \\
		Suppose that for each $U \subset M$, $U$ is open in $M$ implies that $F|_U: U \rightarrow N$ is smooth. Let $(U, \phi) \in \MA$ and $(V, \psi) \in \MB$. An exercise in the section on smooth open submanifolds implies that $(U, \phi) \in \MA|_{U}$. Set $G \defeq F|_U$. By assumption, $G:U \rightarrow N$ is smooth. Therefore $\psi|_{G(U) \cap V} \circ G_{U \cap G^{-1}(V)} \circ \phi|_{U \cap G^{-1}(V)}^{-1}$ is smooth. Since 
		\begin{itemize}
			\item $G^{-1}(V) = U \cap F^{-1}(V)$
			\item $G(U) = F(U)$
			\item $\psi|_{G(U) \cap V} \circ G_{U \cap G^{-1}(V)} \circ \phi|_{U \cap G^{-1}(V)}^{-1}$ is smooth
			\item $\psi|_{F(U) \cap V} \circ F|_{U \cap F^{-1}(V)} \circ \phi|_{U \cap F^{-1}(V)}^{-1} = \psi|_{G(U) \cap V} \circ G_{U \cap G^{-1}(V)} \circ \phi|_{U \cap G^{-1}(V)}^{-1}$
		\end{itemize}
		we have that $\psi|_{F(U) \cap V} \circ F|_{U \cap F^{-1}(V)} \circ \phi|_{U \cap F^{-1}(V)}^{-1}$ is smooth. Since $(U, \phi) \in \MA$ and $(V, \psi) \in \MB$ are arbitrary, $F$ is smooth.
	\end{itemize}
\end{proof}
...
\begin{ex}
	Let $(M, \MA)$, $(N, \MB)$ $(E, \MC)$be smooth manifolds and $F: M \rightarrow N$, $G: N \rightarrow E$ smooth. Then $G \circ F: M \rightarrow E$ is smooth.  
\end{ex}
...
\begin{ex}
	Let  $(M, \MA)$ and $(N, \MB)$ be a smooth manifolds and $F: M \rightarrow N$ a diffeomorphism. Then 
	\begin{enumerate}
...
\begin{proof} Set $n \defeq \dim M$. 
	\begin{enumerate}
		\item Let $(V, \psi) \in \MB$. 
		\begin{enumerate}
			\item Since $(V, \psi) \in \MB$ and $F|_{F^{-1}(V)}^{-1}$ is a homeomorphisms, we have that
			\begin{itemize}
				\item $F^{-1}(V)$ is open in $M$.
				\item $\psi(V)$ is open in $\H^n$
				\item $\psi \circ F|_{F^{-1}(V)}: F^{-1}(V)  \rightarrow \psi(V)$ is a homeomorphism
			\end{itemize}
			So $(F^{-1}(V), \psi \circ F|_{F^{-1}(V)}) \in X^n(M)$. \tcb{discuss in terms of the pullback of charts are charts}
			\item Since $\psi:V \rightarrow \psi(V)$ and $F|_{F^{-1}(V)}:$ are diffeomorphisms, $$(\psi \circ F) \circ \phi^{-1}: \phi(F^{-1}(V) \cap U) \rightarrow \psi(V \cap F(U))$$
			and 
			$$\phi \circ (F^{-1} \circ \psi^{-1}): \psi(F(U) \cap V) \rightarrow \phi(U \cap F^{-1}(V))$$ are smooth. 
		\end{enumerate}
		Therefore $(F(U), \phi \circ F^{-1})$ and $(V, \psi)$ are smoothly compatible. Since $\MB$ is maximal, $(F(U), \phi \circ F^{-1}) \in \MB$.
		\item Let $(U, \phi) \in \MB$. 
		\begin{enumerate}
		...
		\end{enumerate}
		Therefore $(F(U), \phi \circ F^{-1})$ and $(V, \psi)$ are smoothly compatible. Since $\MB$ is maximal, $(F(U), \phi \circ F^{-1}) \in \MB$.
		\item Let $(U, \phi) \in \MA$. 
	\end{enumerate}
\end{proof}

\begin{ex}
	Let $(M, \MA)$, $(N, \MB)$ and $(O, \MC)$ be smooth manifolds and $F: M \rightarrow N$, $G:N \rightarrow O$. If $F$ and $G$ are smooth, then $G \circ F$ are smooth. 
\end{ex}

\begin{proof}
	Let $(U, \phi) \in \MA$ and $(V, \psi) \in \MC$. 
\end{proof}
...
\begin{ex}
	Let $(M, \MA)$ and $(N, \MB)$ be smooth manifolds and $F: M \rightarrow N$. If $F$ is a diffeomorphism, then $F$ is a homeomorphism. 
\end{ex}	
...
\begin{defn}
	Let $M$ and $N$ be smooth manifolds. We define the \tbf{projection maps onto $M$ and $N$}, denoted by $\pi_M: M \times N \rightarrow M$ and $\pi_N: M \times N \rightarrow N$  respectively, by 
	\begin{itemize}
		\item $\pi_M(p,q) = p$ 
		\item $\pi_N(p,q) = q$ 
	\end{itemize}
