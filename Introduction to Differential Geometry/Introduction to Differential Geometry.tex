...
\newpage
	...
	\begin{note}
		We denote the standard basis on $\R^n$ by $(e_1, \ldots, e^n)$.
	\end{note}
	...
	\begin{defn}
		Define $\phi: S_n \rightarrow GL(n, \R)$ as in the previous exercise. Let $P \in GL(n, \R)$. Then $P$ is said to be a \tbf{permutation matrix} if there exists $\sig \in S_n$ such that $P = \phi(\sig)$. We denote the set of $n \times n$ permutation matrices by $\Perm(n, \R)$.
	\end{defn}

	\begin{ex}
		We have that
		\begin{enumerate} 
			\item $\Perm(n, \R)$ is a subgroup of $GL(n, \R)$
			\item $\Perm(n, \R)$ is a subgroup of $O(n)$
		\end{enumerate}
	\end{ex}

	\begin{proof}\
		\begin{enumerate}
			\item By definition, $\Perm(n, \R) = \Im \phi$. Since $\phi: S_n \rightarrow GL(n, \R)$ is a group homomorphism, $\Im \phi$ is a subgroup of $GL(n, \R)$. Hence $\Perm(n, \R)$ is a subgroup of $GL(n, \R)$.
			\item Let $P \in \Perm(n, \R)$. Then there exists $\sig \in S_n$ such that $P = \phi(\sig)$. Then 
			\begin{align*}
				PP^*
				& = 
				\begin{pmatrix}
				...
				& = I
			\end{align*}
			A previous exercise implies that $P^*P = I$. Hence $P \in O(n)$. Since $P \in \Perm(n, \R)$ is arbitrary, $\Perm(n, \R) \subset O(n)$. Part $(1)$ implies that $\Perm(n, \R)$ is a group. Hence $\Perm(n, \R)$ is a subgroup of $O(n)$
		\end{enumerate}
	\end{proof}
...
\end{proof}		
	...
	\begin{defn} \ld{42001}
		Let $(M, \MA)$ be a smooth manifold and $f: M \rightarrow \R$. Then $f$ is said to be smooth if for each coordinate chart $(U, \phi) \in \MA$, $f \circ \phi^{-1}$ is smooth. The set of all smooth functions on $M$ is denoted $C^{\infty}(M)$. 
	\end{defn}
	...
	\begin{ex}
		Let $(M, \MA)$ be a smooth manifold and $U \subset M$. 
	\end{ex}
...
%	\end{defn}
	...
	\begin{ex} \tbf{Constant Rank Theorem:} \\
			Let $(M, \MA)$ and $(N, \MB)$ be smooth manifolds of dimensions $m$ and $n$ respectively, $F \in C^{\infty}(M,N)$. Suppose that $F$ has constant rank. Then for each $p \in U$, there exist $(U', \phi') \in \MA$ and $(V', \psi') \in \MB$ such that $U' \subset U$, $V' \subset V$, $p \in U'$, $F(p) \in V'$ and
		$$\psi \circ F \circ \phi^{-1}(x^1, \ldots, x^k, x^{k+1}, \ldots, x^m) = (x^1, \ldots, x^k, 0, \ldots, 0)$$ 
	\end{ex}
	
	\begin{proof}
		Let $p \in U$. Since $\rnk DF(p) = k$ there exist $A \in GL(k, \R)$, $B \in \R^{(m-k) \times k}$, $C \in \R^{(n-k) \times k}$, $D \in \R^{(m-k) \times (n-k)}$, $\sig \in S_m$ and $\tau \in S_n$ such that 
		$$P_{\tau} DF(p) P_{\sig}^* = 
		\begin{pmatrix}
			A & B \\
			C & D
		\end{pmatrix}
		$$
		We define the change of coordinates $\phi_{\sig}: \R^n \rightarrow \R^n$ and $\psi_{\tau}: \R^m \rightarrow \R^m$ by $\phi_{\sig} = \sig \id_{\R^n}|_{U}$ and $\psi_{\tau} = \tau \id_{\R^n}|_{V}$. Define $F_{\sig, \tau}: \phi_{\sig}(U) \rightarrow \psi_{\tau}(V)$ by 
		$$F_{\sig, \tau} = \psi_{\tau}|_V \circ F \circ \phi_{\sig}^{-1}|_{\phi_{\sig}(U)}$$
		The previous exercises imply that $D\psi_{\tau}(F(p)) = P_{\tau}$ and 
		\begin{align*}
			D \phi^{-1}_{\sig}(\phi_{\sig}(p))
			& = [D \phi_{\sig}(p)]^{-1} \\
			& = P_{\sig}^{-1} \\
			& = P_{\sig}^*
		\end{align*}
		Hence
		\begin{align*}
			D F_{\sig, \tau}(\phi_{\sig}(p))
			& = D \psi_{\tau}(F(p)) \circ DF(p) \circ D\phi^{-1}(\phi_{\sig}(p)) \\
			& = P_{\tau} DF(p) P_{\sig}^* \\
			& = 
			\begin{pmatrix}
				A & B \\
			...
			\end{pmatrix}
		\end{align*}
		Define $\pi_1: $ so that $F = (Q, R)$. Define $\phi:U \rightarrow \R^m$ by $\phi(u_1, u_2) = (Q(u_1, u_2), u_2)$. Then for each $u_1 \in \R^{k}$, $u_2 \in \R^{m-k}$, 
		\begin{align*}
			[D \phi (u_1, u_2)] 
			& = \begin{pmatrix}
				D_{u_1}Q(u_1, u_2) & D_{u_2}Q(u_1, u_2) \\
				D_{u_1} \pi_{u_2}(u_1, u_2) &  	D_{u_2} \pi_{u_2}(u_1, u_2) 
			\end{pmatrix}  \\
			& = 
			\begin{pmatrix}
				D_{u_1}Q(u_1, u_2) & D_{u_2}Q(u_1, u_2) \\
	...
	\begin{defn}
		...
		\begin{itemize}
			...
			\item an \tbf{immersion} if for each $p \in M$, $dF_p:T_pM\rightarrow T_{F(p)}N$ is injective
			\item a \tbf{submersion} if for each $p \in M$, $dF_p:T_pM\rightarrow T_{F(p)}N$ is surjective
		\end{itemize}
	\end{defn}
...
\end{enumerate}	 
	...
	\newpage
	\chapter{Fiber Bundles and Sections}
	...
	\begin{defn}
		Let $E$ and $M$ be smooth manifolds $\pi : E \rightarrow M$ a smooth submersion. Then $(E, M, \pi)$ is said to be a \tbf{fibered manifold with total space $E$, base space $M$ and projection $\pi$}.
	\end{defn}
	...
	\begin{ex}
		Let $(E, M, \pi, F)$ be a smooth fiber bundle. Then for each $p \in M$, $E_p$ is diffeomorphic to $F$.
	\end{ex}

	\begin{proof}
		Let $p \in M$. By assumption, there exist $U \in \MN_p$ and $\Phi: \pi^{-1}(U) \rightarrow U \times F$ such that $U$ is open and $(U, \Phi)$ is a smooth local trivialization of $E$ over $U$ with fiber $F$. By definition $\Phi$ is a diffeomorphism. Hence $\Phi|_{E_p}: E_p \rightarrow \{p\} \times F$.
	\end{proof}
...
\end{align*}	 
	...
	\begin{defn}
		Let $\al: M \rightarrow T^r_sM$. Then $\al$ is said to be a \tbf{$(r,s)$-tensor field on $M$} if for each $p \in M$, $\al_p \in T^r_s(T_pM)$. \\
	...
	\begin{defn}
		Define the \tbf{exterior product} $$\wedge: \Om_k(M) \times \Om_l(M) \rightarrow \Om_{k+l}(M) $$ by $$(\al \wedge \be)_p = (\al)_p \wedge (\be)_p$$
	\end{defn}
	...
	\begin{defn}
		We define the \tbf{exterior derivative} $d: \Om_k(M) \rightarrow \Om_{k+1}(M)$ inductively by 
		\begin{enumerate}
	...
	\begin{defn}
		Let $(N, \MB)$ be a smooth manifold and $F: M \rightarrow N$ be a diffeomorphism. Define the \tbf{pullback of $F$}, denoted $F^*: \Om_k(N) \rightarrow \Om_k(M)$ by  $$(F^* \om)_p (v_1, \cdots, v_k) = \om_{F(p)} (dF_p(v_1), \cdots, dF_p(v_k))$$ for $\om \in \Om_k(N)$, $p \in M$ and $v_1, \cdots, v_k \in T_{p}M$
	\end{defn}
