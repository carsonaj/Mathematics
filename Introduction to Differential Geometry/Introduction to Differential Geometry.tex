...
\end{enumerate}	 
	...
	\begin{ex}
	Let $(M, \MA)$ be a smooth $n$-dimensional manifold and $S \subset M$. If $S$ satisfies the local $k$-slice condition, then there exists a unique smooth structure $\tMA$ on $S$ such that $S$ is an embedded submanifold of $M$.
	\end{ex}	
	
	\begin{proof}
	Suppose that $S$ satisfies the local $k$-slice condition. Define $\pi: \R^n \rightarrow \R^k$ as above For $(U, \phi) \in \MA$, define $\tU = U \cap S$ and $\tphi: \tU \rightarrow \R^k$ by $\tphi = \pi \circ \phi|_{\tU}$
	\end{proof}
	
	
	\begin{ex}
	Let $(M, \MA)$ and  $(N, \MB)$ be smooth manifolds. Suppose that $M \subset N$ and $M$ is an embedded submanifold. Then 
	\end{ex}	
	...
	\begin{defn}
		Let $E$, $M$ and $F$ be topological spaces and $\pi: E \rightarrow M$ a continuous surjection, $U \subset M$ open and $\Phi: \pi^{-1}(U) \rightarrow U \times F$. Then $(U, \Phi)$ is said to be a \textbf{local trivialization of $E$ over $U$}  if 
		\begin{enumerate}
	...
	\begin{defn}
		Let $E$ and $M$ be topological spaces and $\pi: E \rightarrow M$ a continuous surjection. Suppose that
		\begin{enumerate}
			\item for each $p \in M$, $\pi^{-1}(\{p\})$ is a $n$-dimensional real vector space.
			\item for each $p \in M$, there exist open $U \in \MN_p$ and $\Phi: \pi^{-1}(U) \rightarrow U \times \R^n$ such that $(U, \Phi)$ is a local trivialization of $E$ over $U$.
			\item for each $p \in M$, $$\Phi|_{\pi^{-1}(\{p\})}: \pi^{-1}(\{p\}) \rightarrow \{p\} \times \R^n$$ is an isomorphism. 
				
		\end{enumerate}
	
	Then $(E, M, \pi)$ is said to be a \textbf{vector bundle of rank $n$}.
	\end{defn}
