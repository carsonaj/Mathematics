...
\begin{document}
	...
	\begin{enumerate}
	\item Then $\phi \in L^1_{\loc}(\R^n)$ and there exists $a \in \R$ such that $$\int_{(0,a]}\phi dm \neq 0$$
	\item Define $$c = \bigg[ \int_{(0,a]}\phi dm \bigg]^{-1}$$ 
	Then  For each $x \in \R$, $$\phi(x) = c\int_{(x, x+a]}\phi dm$$ 
	\item $\phi$ is differentiable and for each $x \in \R$, $\phi'(x) = c(\phi(x+a) - \phi(x))$
	\item Define $b = c(\phi(a) - 1)$ and $g \in C(\R)$ by $g(x) = e^{bx} \phi(x)$. Then $g$ is constant and there exists $\xi \in \R$ such that $b = 2 \pi i \xi $
	\end{enumerate}
	\end{ex}	
	
	\begin{proof}
	\begin{enumerate}
	\item Let $K \subset \R$ be compact. Then $$\int_K |\phi| dm = m(K) < \infty$$ So $\phi \in L^1_{\loc}(\R^n)$. For the sake of contradiction, suppose that for each $a \in \R$, $$\int_{(0,a]}\phi dm = 0$$ 
	Then the FTC implies that $\phi = 0$ a.e., which is a contradiction. So there exists $a \in \R$ such that $$\int_{(0,a]}\phi dm \neq 0$$
	\item Then 
	\begin{align*}
	\phi(x) 
	...
	&= c \int_{(x,x+a]} \phi dm 
	\end{align*}
	\item Define $f \in C^1(\R)$ by $$f(x) = \int_{(0, x]} \phi dm$$ Then 
	\begin{align*}
	\phi(x) 
	&= c \int_{(x,x+a]} \phi dm \\
	&= c(f(x+a) - f(x))
	...
	\begin{ex}
	Then there exists $\xi \in \R^n$ such that for each $x \in \R^n$, $\phi(x) = e^{2 \pi i \l \xi, x\r}$. 
	\end{ex}
