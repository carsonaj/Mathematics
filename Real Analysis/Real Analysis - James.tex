...
\begin{ex}
Let $(X, \MA, \mu)$ be a measure space, $(A_n)_{n \in \N} \subset \MA$ and $A \in \MA$. Suppose that $\mu\bigg(\sup\limits_{n \in \N}A_n\bigg) < \infty$. Then $A_n \rightarrow A$ implies that $\mu(A_n) \rightarrow \mu(A)$. (Hint: it may be helpful to show that $\limsup\limits_{n \rightarrow \infty}\mu(A_n) \leq \liminf\limits_{n \rightarrow \infty}\mu(A_n)$)
\end{ex} 

\begin{proof}
Suppose that $A_n \rightarrow A$. Recall that $\limsup\limits_{n \rightarrow \infty}A_n = \inf\limits_{n \in \N} \bigg(\sup\limits_{k \geq n}A_k\bigg)$. Since, $\mu\bigg(\sup\limits_{k \geq 1}A_k\bigg) < \infty$ and $\bigg(\sup\limits_{k \geq n}A_k \bigg)_{n \in \N}$ is decreasing, the previous exercise tells us that 
\begin{align*}
\mu(A)
&= \mu\bigg(\limsup\limits_{n \rightarrow \infty}A_n \bigg)\\
&= \inf_{n \in \N}\mu\bigg(\sup_{k \geq n}A_k\bigg) \\
&= \inf_{n \in \N}\bigg(\sup_{k \geq n}\mu (A_k)\bigg) \\
&= \limsup_{n \rightarrow \infty}\mu(A_n)
\end{align*}
Recall that $\liminf\limits_{n \rightarrow \infty} A_n = \sup\limits_{n \in \N} \bigg( \inf\limits_{k \geq n}A_k \bigg)$. Note that for each $n \in \N$, $\inf\limits_{k \geq n}A_k \subset A_n$, so for each $n \in \N$, $\mu\bigg(\inf\limits_{k \geq n}A_k\bigg) \leq \mu(A_n)$. Since $\bigg(\inf\limits_{k \geq n}A_k\bigg)_{n \in \N}$ is increasing the last exercise tells us that 
\begin{align*}
\mu(\limfn A_n)
&= \sup\limits_{n \in \N} \mu \bigg( \inf\limits_{k \geq n}A_k \bigg) \\
&= \liminf_{n \rightarrow \infty} \mu \bigg( \inf\limits_{k \geq n}A_k \bigg) \\
& \leq \liminf_{n \rightarrow \infty} \mu(A_n)
\end{align*}
Since $\liminf\limits_{n \rightarrow \infty} A_n = \limsup\limits_{n \rightarrow \infty} A_n$, we have that 
\begin{align*}
\limsup_{n \rightarrow \infty}\mu(A_n)
&= \mu \bigg(\limsup_{n \rightarrow \infty}A_n \bigg) \\
&= \mu \bigg(\liminf_{n \rightarrow \infty}A_n \bigg) \\
& \leq \liminf_{n \rightarrow \infty} \mu(A_n)
\end{align*}
