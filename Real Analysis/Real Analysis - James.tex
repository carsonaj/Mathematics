...
\begin{defn}
Let $(X,\MA, \mu), (Y,\MB, \nu)$ be measurable spaces. Put $\ME = \{A \times B: A \in \MA \text{ and } B \in \MB\}$. Then $\ME$ is an elementary family and thus $\MM_0 = \{\bigcup_{i =1}^n M_i: (M_i)_{i=1 }^n \subset \ME \text{ are disjoint}\}$ is an algebra on $X \times Y$. We define $\pi_0: \MM_0 \rightarrow \RG$ by $$\pi_0\bigg(\bigcup_{i=1}^n A_i \times B_i \bigg) = \sum_{i=1}^n\mu(A_i)\nu(B_i)$$ Since $\MA \otimes \MB = \sig(\MM_0)$, we define a product measure $\mu \times \nu$ on $(X \times Y, \MA \otimes \MB)$ to be an extension of $\pi_0$ to $\MA \otimes \MB$. The existence of which is guaranteed by Caratheodory's theorem and on $\MA \otimes \MB$, 
...
\begin{defn}
Let $(\Om, \MF, P)$ be a probability space and $\MC \subset \MF$. Then $\MC$ is said to be \textbf{independent} if for each $A_1, A_2, \cdots, A_n \in \MC$, $$P \bigg( \bigcap_{k=1}^nA_k\bigg) = \prod_{k=1}^{n}P(A_k)$$
\end{defn}

\begin{defn}
Let $(\Om, \MF, P)$ be a probability space and $\MC_1, \MC_2, \cdots, \MC_n \subset \MF$. Then $\MC_1, \MC_2, \cdots, \MC_n $ are said to be \textbf{independent} if for each $A_1 \in \MC_1, A_2 \in \MC_2, \cdots, A_n \in \MC_n$, $A_1, A_2, \cdots, A_n$ are independent. 
\end{defn}
