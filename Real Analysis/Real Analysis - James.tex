...
\begin{ex}
Let $f \in \L^1(\R^n)$. Suppose that $m(\{x \in X: f(x) \neq 0\}) >0$. Then there exist $C,R>0$ such that for each $x \in \R^n$ if $|x| > R$, then $Hf(x) \geq C|x|^{-n}$. Hence there exists $C' > 0$ such that for each $\al >0$, $m(\{x \in X: Hf(x)>\alpha\}) > C'/\al$ when $\al$ is small. 
\end{ex}
