...
\begin{document}
	...
	\begin{defn}
		Let $(X, \leq_X)$ and $(Y, \leq_Y)$ be posets, $f: X \rightarrow Y$ and $g:Y \rightarrow X$. Then 
		\begin{itemize}
			\item $f$ is said to be a \tbf{left adjoint of $g$} if $(f,g)$ is a monotone Galois connection,
			\item $g$ is said to be a \tbf{right adjoint of $f$} if $(f,g)$ is a monotone Galois connection,
		\end{itemize}
	\end{defn}
	...
	\begin{ex}
		...
		\begin{enumerate}
			\item for each $A \subset X$, if $\sup (A, \leq_X)$ exists, then $f(\sup (A, \leq_X)) = \sup(f(A), \leq_Y)$.
			\item for each $B \subset Y$, if $\sup (B, \leq_Y)$ exists, then $g(\sup (B, \leq_Y)) = \sup(g(B), \leq_X)$.
		\end{enumerate} 
	\end{ex}
	
	\begin{proof}\
		\begin{enumerate}
			\item Let $A \subset X$. Suppose that $\sup (A, \leq_X)$ exists. 
			\item 
		\end{enumerate}
	\end{proof}
	
	\begin{ex}
		Let $(X, \leq_X)$ and $(Y, \leq_Y)$ be posets and $f: X \rightarrow Y$. Then there exists $g:Y \rightarrow X$ such that $(f, g)$ is a monotone Galois connection iff for each $y \in Y$, there exists $x \in X$ such that $f^{-1}(\doar y) = \doar x$.
	\end{ex}
	
	\begin{proof}\
		\begin{itemize}
			\item $(\implies)$: \\
			Suppose that there exists $g:Y \rightarrow X$ such that $(f, g)$ is a monotone Galois connection. \tcr{a prev ex in galois connections on prosets section} \rex{} then implies that $f$ and $g$ are monotone, $\id_X \leq_X g \circ f$ and $f \circ g \leq_Y \id_Y$. Let $y \in Y$. Define $x \in X$ by $x \defeq g(y)$. 
			\begin{itemize}
			...
			\begin{align*}
				...
				& \iff x \leq_Y g(y).
			\end{align*}
			Thus $(f, g)$ is a monotone Galois connection. 
		\end{itemize}
	\end{proof}
	...
	\begin{proof}\
		\begin{enumerate}
			\item Let $x, y \in \supSet A$. \rex{ex:orderings:prosets:0025} implies that $x \sim_{\leq} y$. \rex{ex:orderings:posets:0002} then implies that $x = y$.
			\item Similar to $(1)$.
		\end{enumerate}
	\end{proof}
	...
	\begin{ex}
		...
		\begin{enumerate}
			...
			\item $f$ is finite $(\leq_{\dmd_X}^{\vee}, \leq_{\dmd_Y}^{\vee})$-join preserving
			\item $f$ is finite $(\leq_{\dmd_X}^{\wedge}, \leq_{\dmd_Y}^{\wedge})$-meet preserving
		\end{enumerate}
	\end{ex}
	...
	\tcr{maybe make exercise about galois connections and right adjoints preserving joins, left preserving meets}
	...
	\begin{note}
		We the context is clear, we say ``zero" and ``one" inplace of ``$(\vee, \wedge)$-zer" and ``$(\vee, \wedge)$-one" respectively.
	\end{note}
	...
	\begin{defn}
		Let $(L, \vee_L, \wedge_L)$ and $(M, \vee_M, \wedge_M)$ be lattices. Then $(M, \vee_M, \wedge_M)$ is a sublattice of $(L, \vee_L, \wedge_L)$ if 
		\begin{enumerate}
			\item $M \subset L$
			\item $\vee_L|_{M^2} = \vee_M$ and $\wedge_L|_{M^2} = \wedge_M$. 
		\end{enumerate}  
	...
	\begin{ex}
		Let $(X, \leq_X), (Y, \leq_Y)$ be complete lattices, $f:X \rightarrow Y$ and $g: Y \rightarrow X$. 
		\begin{enumerate}
			\item If $f$ is arbitrary join preserving, then there exists $f_*: Y \rightarrow X$ such that $g$ is a right adjoint of $f$. 
			\item If $g$ is arbitrary meet preserving, then there exists $g^*: X \rightarrow Y$ such that $g$ is a left adjoint of $g$. 
		\end{enumerate}
	\end{ex}
	...
	\begin{ex}
		\tcr{move to section on complete lattices}
		Let $(X, \leq_X)$ and $(Y, \leq_Y)$ be posets, $f: X \rightarrow Y$. Then there exists $g:Y \rightarrow X$ such that $(f,g)$ is a monotone galois connection iff $f$ preserves arbitrary joins. 
	\end{ex}
	...
	\tcr{move to after universal algebra, because we doing the same things}
